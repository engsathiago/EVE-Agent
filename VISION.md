...
EVE is the AI that actually does things.
...
EVE started as a personal playground to learn AI and build something genuinely useful:
an assistant that can run real tasks on a real computer.
It evolved through several names and shells: Warelay -> Clawdbot -> Moltbot -> EVE.
...
EVE runtime code reads the current configuration schema only.
...
`eve doctor --fix` should detect the old shape, explain it, back it up when needed, and rewrite it to the canonical format.
...
Security in EVE is a deliberate tradeoff: strong defaults without killing capability.
...
EVE has an extensive plugin API.
...
- Code plugins run EVE plugin code and are appropriate for deeper runtime extension.
...
EVE docs should document core extension points; plugin promotion belongs in ClawHub, preferably under vetted org publishers for official plugins.
...
EVE supports MCP as both a server and a runtime integration surface.
...
EVE is currently terminal-first by design.
...
EVE is primarily an orchestration system: prompts, tools, protocols, and integrations.
TypeScript was chosen to keep EVE hackable by default.
