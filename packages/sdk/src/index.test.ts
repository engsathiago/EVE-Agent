...
class FakeTransport implements EVETransport {
...
class EventsOnlyTransport implements EVETransport {
...
describe("EVE SDK", () => {
  ...
  it("splits provider-qualified model refs and rejects unsupported run options", async () => {
    ...
    await expect(
      ...
      "EVE Gateway does not support per-run SDK options yet: workspace, runtime, environment, approvals",
    );
  });
  ...
  it("throws explicit unsupported errors for SDK namespaces without Gateway RPCs", async () => {
    ...
    await expect(oc.environments.create({ provider: "testbox" })).rejects.toThrow(
      "oc.environments.create is not supported by the current EVE Gateway yet",
    );
    await expect(oc.environments.delete("environment_123")).rejects.toThrow(
      "oc.environments.delete is not supported by the current EVE Gateway yet",
    );
    expect(transport.calls).toStrictEqual([]);
  });
  ...
  it("keeps close terminal when it races a pending connect", async () => {
    ...
    await expect(connect).rejects.toThrow("EVE SDK client is closed");
    await close;
    await expect(oc.agents.list()).rejects.toThrow("EVE SDK client is closed");
    await expect(oc.events()[Symbol.asyncIterator]().next()).rejects.toThrow(
      "EVE SDK client is closed",
    );
    expect(() => oc.rawEvents()).toThrow("EVE SDK client is closed");
  ...
  it("rejects normalized event streams when the event pump fails before yielding", async () => {
    ...
    let futureIterator: AsyncIterator<EVEEvent> | undefined;
  ...
  it("uses cumulative text for the first replayed chat projection", async () => {
    ...
    let text = "";
    let iterator: AsyncIterator<EVEEvent> | undefined;
