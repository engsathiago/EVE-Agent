...
import { GatewayClientTransport, EVE } from "./index.js";
...
describe("EVE SDK websocket e2e", () => {
  afterEach(async () => {
    await Promise.all(
...
describe("EVE SDK real Gateway e2e", () => {
...
const liveGatewayUrl = process.env.EVE_SDK_LIVE_GATEWAY_URL;
const liveGatewayToken = process.env.EVE_SDK_LIVE_GATEWAY_TOKEN;
...
liveGatewayDescribe("EVE SDK live Gateway e2e", () => {
  it("connects to a configured Gateway, streams a real run, and waits for completion", async () => {
    ...
    try {
      ...
      const agent = await oc.agents.get(process.env.EVE_SDK_LIVE_AGENT_ID ?? "main");
      const run = await agent.run({
        input: "Reply with exactly: EVE_SDK_LIVE_OK",
      ...
      expect(events.text).toContain("EVE_SDK_LIVE_OK");
    } finally {
      await oc.close();
    }
  }, 240_000);
