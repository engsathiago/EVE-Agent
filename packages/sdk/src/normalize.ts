// EVE SDK helper module supports normalize behavior.
import type { GatewayEvent, JsonObject, EVEEvent, EVEEventType } from "./types.js";
...
function normalizeLifecycleEndEventType(data: JsonObject): EVEEventType {
  const status = readLowerString(data.status);
  const stopReason = readLowerString(data.stopReason);
...
function normalizeAgentEventType(payload: JsonObject): EVEEventType {
...
function normalizeNamedEventType(event: GatewayEvent): EVEEventType {
...
export function normalizeGatewayEvent(event: GatewayEvent): EVEEvent {
  const payload = asRecord(event.payload);
