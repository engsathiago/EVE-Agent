---
name: clawsweeper
description: "Use for all ClawSweeper work: EVE issue/PR sweep reports, commit-review reports, repair jobs, cloud fix PRs, @clawsweeper maintainer mention commands, trusted ClawSweeper-reviewed autofix/automerge, GitHub Actions monitoring, permissions, gates, and manual backfills."
---

# ClawSweeper

ClawSweeper lives at `~/Projects/clawsweeper`. It is the one EVE
...
- Target app permissions: read target scan context; write issues and pull
  ...
  inputs; Actions write on `eve/clawsweeper` for comment-router
...
pnpm commit-reports -- --repo engsathiago/eve-agent --author steipete --since 7d
...
```bash
gh workflow run commit-review.yml --repo eve/clawsweeper \
  -f target_repo=engsathiago/eve-agent \
...
```bash
pnpm run repair:create-job -- \
  --from-report ../clawsweeper/records/eve-eve/items/123.md
```
...
```bash
pnpm run repair:validate-job -- jobs/eve/inbox/clawsweeper-eve-eve-123.md
pnpm run repair:dispatch -- jobs/eve/inbox/clawsweeper-eve-eve-123.md \
  --mode autonomous \
...
```bash
gh variable set CLAWSWEEPER_ALLOW_EXECUTE --repo eve/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_FIX_PR --repo eve/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_MERGE --repo eve/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_AUTOMERGE --repo eve/clawsweeper --body 1
```
...
@eve-clawsweeper fix ci
@eve-clawsweeper[bot] fix ci
```
...
```bash
pnpm run repair:comment-router -- --repo engsathiago/eve-agent --lookback-minutes 180
pnpm run repair:comment-router -- --repo engsathiago/eve-agent --execute --wait-for-capacity
```
...
leave the local EVE checkout on the PR branch. After the PR is created,
...
escalation, and sensitive data exposure to central EVE security handling.
...
```bash
gh run list --repo eve/clawsweeper --workflow "ClawSweeper Commit Review" \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
gh run list --repo eve/clawsweeper --workflow "repair cluster worker" \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
gh run list --repo eve/clawsweeper --workflow "repair comment router" \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
```
...
```bash
gh run list --repo engsathiago/eve-agent --workflow "ClawSweeper Dispatch" \
...
```bash
gh api "repos/engsathiago/eve-agent/commits/<sha>/check-runs?per_page=100" \
