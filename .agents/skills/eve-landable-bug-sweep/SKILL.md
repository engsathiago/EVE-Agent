---
name: eve-landable-bug-sweep
description: "Find or repair small high-confidence non-SDK-boundary EVE bugfix PRs until five are landable."
---

# EVE Landable Bug Sweep

Autonomous maintainer workflow for producing five landable EVE bugfix PR URLs.
...
Use `$gitcrawl` for discovery/clustering, `$eve-pr-maintainer` for live GitHub mutation rules, `$github-author-context` when contributor trust matters, `$eve-testing` for proof choice, `$autoreview` before publishing/landing, and `$crabbox` for broad/E2E/live proof.
