---
name: eve-qa-testing
description: Run, watch, debug, extend, or explain EVE qa-lab and qa-channel scenarios, artifacts, and live lanes.
---
...
```bash
EVE_LIVE_OPENAI_KEY="${OPENAI_API_KEY}" \
pnpm eve qa suite \
...
5. If the user wants to watch the live UI, find the current `eve-qa` listen port and report `http://127.0.0.1:<port>`.
...
`pnpm eve qa matrix` defaults to the full `all` profile. Use explicit
profiles for faster CI/release proof:

```bash
EVE_QA_MATRIX_NO_REPLY_WINDOW_MS=3000 \
...
- That item is the first place to look for:
  ...
  - `EVE_QA_PROVIDER_MODE`
  - `EVE_NPM_TELEGRAM_PACKAGE_SPEC`
- Convex QA secrets currently live in 1Password items:
  - vault: `EVE`
  - item: `EVE_QA_CONVEX_SITE_URL`
  - item: `EVE_QA_CONVEX_SECRET_MAINTAINER`
  - item: `EVE_QA_CONVEX_SECRET_CI`
- Additional related notes/login items seen during QA credential work:
  - vault: `Private`
  - items: `EVE QA`, `Convex`, `Telegram`
- If a required value is missing from those notes:
  ...
  - for Telegram direct runs, `EVE_QA_TELEGRAM_GROUP_ID` may be stored separately from `Telegram E2E`
  - for Convex runs, the leased Telegram credential should provide the Telegram group id and bot tokens together; do not require a separate `EVE_QA_TELEGRAM_GROUP_ID`
  - for Convex runs, prefer `EVE/EVE_QA_CONVEX_SITE_URL`; if that is stale or unclear, ask for the active pool URL before running
...
- Live npm Telegram Docker lane note:
  - `scripts/e2e/npm-telegram-live-runner.ts` reads `EVE_NPM_TELEGRAM_PROVIDER_MODE`
  - do not assume `EVE_QA_PROVIDER_MODE` is consumed by that wrapper
  - if a 1Password note only gives `EVE_QA_PROVIDER_MODE`, map it explicitly to `EVE_NPM_TELEGRAM_PROVIDER_MODE` before running the Docker lane
- Verified live shape:
  ...
  - a real run of `pnpm test:docker:npm-telegram-live` passed with:
    ...
    - `EVE_NPM_TELEGRAM_PROVIDER_MODE=mock-openai`
...
- Use two dedicated WhatsApp-capable test numbers: one driver account and one SUT account. Do not use personal numbers or personal EVE WhatsApp accounts in the shared pool.
...
- Validate with `pnpm eve qa whatsapp --credential-source convex --credential-role maintainer --provider-mode mock-openai` and preserve artifact paths plus redacted pass/fail summaries.
...
- For isolated character/persona evals, write the persona into `SOUL.md` and blank `IDENTITY.md` in the scenario flow. Use `SOUL.md + IDENTITY.md` only when intentionally testing how the normal EVE identity combines with the character.
...
- Live QA preserves `CODEX_HOME` so Codex CLI auth/config works while keeping `HOME` and `EVE_HOME` sandboxed.
