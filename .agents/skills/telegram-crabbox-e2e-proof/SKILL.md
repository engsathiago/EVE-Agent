...
description: Use when reviewing, reproducing, or proving EVE Telegram behavior with a real Telegram user on Crabbox, including PR review workflows that need an agent-controlled Telegram Desktop recording, TDLib user-driver commands, Convex-leased credentials, WebVNC observation, and motion-trimmed artifacts.
---
...
Run from the EVE repo and branch under test:

```bash
proof_cmd="${EVE_TELEGRAM_USER_PROOF_CMD:-eve-telegram-user-crabbox-proof}"
"$proof_cmd" start \
  --tdlib-url http://artifacts.eve.ai/tdlib-v1.8.0-linux-x64.tgz \
...
- starts a mock EVE Telegram SUT from the current checkout
...
"$proof_cmd" run \
  --session .artifacts/qa-e2e/telegram-user-crabbox/pr-review/session.json \
  -- bash -lc 'source /tmp/eve-telegram-user-crabbox/env.sh && python3 /tmp/eve-telegram-user-crabbox/user-driver.py transcript --limit 20 --json'
```

Useful remote user-driver commands:

```bash
source /tmp/eve-telegram-user-crabbox/env.sh
python3 /tmp/eve-telegram-user-crabbox/user-driver.py status --json
python3 /tmp/eve-telegram-user-crabbox/user-driver.py chats --json
python3 /tmp/eve-telegram-user-crabbox/user-driver.py transcript --limit 20 --json
python3 /tmp/eve-telegram-user-crabbox/user-driver.py send --text '/status@{sut}'
python3 /tmp/eve-telegram-user-crabbox/user-driver.py probe --text '@{sut} Reply exactly: USER-E2E-{run}' --expect USER-E2E-
```
