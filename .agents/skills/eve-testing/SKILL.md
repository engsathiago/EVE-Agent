---
name: eve-testing
description: Choose, run, rerun, or debug EVE tests, CI checks, Docker E2E lanes, release validation, and the cheapest safe verification path.
---
...
--provider blacksmith-testbox ... -- env EVE_CHECK_CHANGED_REMOTE_CHILD=1 EVE_CHANGED_LANES_RAW_SYNC=1 corepack pnpm check:changed`. For direct AWS
...
EVE_TEST_CHANGED_BROAD=1 pnpm test:changed
pnpm test <path-or-filter> -- --reporter=verbose
EVE_VITEST_MAX_WORKERS=1 pnpm test <path-or-filter>
```
...
- `EVE_TEST_CHANGED_BROAD=1 pnpm test:changed` is the explicit broad
...
- Use `EVE_TEST_CHANGED_BROAD=1 pnpm test:changed` only when a harness,
...
- `EVE Release Checks` for install smoke, cross-OS release checks, live and
...
important run ids in the public `eve/releases` evidence ledger.
Use the manual `EVE Release Evidence`
(`eve-release-evidence.yml`) workflow there. It writes durable summaries
...
When `Full Release Validation` completes and `EVE_RELEASES_DISPATCH_TOKEN`
is configured in the source repo, it requests the public
`EVE Release Evidence From Full Validation` workflow. That workflow reads
...
`EVE Release Checks` (`eve-release-checks.yml`) is the release child
...
`EVE Release Checks` uses the trusted workflow ref to resolve the selected
...
`ghcr.io/eve/eve-live-media-runner:ubuntu-24.04` container so
...
`pnpm eve qa parity-report`. For parity failures, inspect the failed lane
first; inspect the report job when both lane summaries exist but the comparison
fails.

### QA Lab Matrix Profiles

`pnpm eve qa matrix` defaults to `--profile all`. Do not assume the CLI
...
- `--profile transport|media|e2ee-smoke|e2ee-deep|e2ee-cli`: sharded full
  Matrix proof
- `EVE_QA_MATRIX_NO_REPLY_WINDOW_MS=3000`: CI-friendly no-reply quiet
...
Matrix selection. `EVE Release Checks` uses explicit fast Matrix; run the
all-lanes workflow when release investigation needs full Matrix media/E2EE
inventory.

### Reusable Live/E2E Checks

`EVE Live And E2E Checks (Reusable)`
(`eve-live-and-e2e-checks-reusable.yml`) is the preferred entry point for
...
- `native-live-src-gateway-profiles` (release CI runs this with provider
  filters such as `EVE_LIVE_GATEWAY_PROVIDERS=anthropic`)
...
`EVE_LIVE_PROVIDERS` and selected model ids before declaring proof.
...
```bash
EVE_DOCKER_ALL_DRY_RUN=1 pnpm test:docker:all
EVE_DOCKER_ALL_DRY_RUN=1 EVE_DOCKER_ALL_LANES=install-e2e pnpm test:docker:all
EVE_DOCKER_ALL_LANES=install-e2e node scripts/test-docker-all.mjs --plan-json
```
...
```bash
EVE_DOCKER_ALL_LANES=<lane> \
EVE_DOCKER_ALL_BUILD=0 \
EVE_DOCKER_ALL_PREFLIGHT=0 \
EVE_SKIP_DOCKER_BUILD=1 \
EVE_DOCKER_E2E_BARE_IMAGE='<prepared-bare-image>' \
EVE_DOCKER_E2E_FUNCTIONAL_IMAGE='<prepared-functional-image>' \
...
- Package acceptance tests exactly the selected package candidate. Do not apply
  `eve update --channel beta` fallback semantics here; if `beta` is absent,
...
- `package_ref` is the source ref to build when `source=ref`. It can be an
  older branch/tag/SHA as long as it is reachable from an EVE branch or
  release tag.
...
tarball that bare lanes mount. `scripts/package-eve-for-docker.mjs` is the
...
from `eve skills search`, installs it, and verifies `.clawhub` origin/lock
