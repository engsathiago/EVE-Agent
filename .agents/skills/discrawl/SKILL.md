...
metadata:
  eve:
    homepage: https://github.com/eve/discrawl
    requires:
      bins:
        - discrawl
    install:
      - kind: go
        module: github.com/eve/discrawl/cmd/discrawl@latest
        bins:
          - discrawl
---
...
- Repo: `eve/discrawl`; use `~/GIT/_Perso/discrawl` only after verifying
  its remote targets `eve/discrawl`, otherwise use a fresh checkout
...
`eve/discrawl` checkout before concluding the feature is missing.
...
This reader imports `https://github.com/eve/discord-store.git` into
