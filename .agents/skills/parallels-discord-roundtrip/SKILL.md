...
```bash
export EVE_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.eve/eve.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env EVE_PARALLELS_DISCORD_TOKEN \
...
- Use the `eve` wrapper for guest `message send/read`; `node eve.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/eve-parallels-smoke.*`
...
- host inbound nonce appears in `eve message read` output
