...
// EVE npm postpublish tests validate postpublish verification behavior.
...
import {
  ...
  parseEVENpmPostpublishVerifyArgs,
  resolveInstalledBinaryCommandInvocation,
  resolveInstalledBinaryPath,
  retryNpmRegistryProvenanceRead,
  verifyNpmProvenanceAttestation,
  verifyNpmRegistrySignatures,
} from "../scripts/eve-npm-postpublish-verify.ts";

const INSTALLED_ROOT_DIST_JS_FILE_SCAN_LIMIT = 10_000;

describe("parseEVENpmPostpublishVerifyArgs", () => {
  it("supports help and package-manager separators", () => {
    expect(parseEVENpmPostpublishVerifyArgs(["--help"])).toEqual({
      help: true,
      version: "",
    });
    expect(parseEVENpmPostpublishVerifyArgs(["--", "2026.3.23"])).toEqual({
      help: false,
      version: "2026.3.23",
    });
  });
  ...
  it("rejects missing, option-like, and extra arguments before verification", () => {
    ...
    expect(() => parseEVENpmPostpublishVerifyArgs(["2026.3.23", "extra"])).toThrow(
      "Unexpected eve npm postpublish verifier argument: extra",
    );
  });
});
...
describe("npm registry provenance verification", () => {
  ...
  const provenancePayload = {
    ...
    predicate: {
      buildDefinition: {
        externalParameters: {
          workflow: {
            repository: "https://github.com/engsathiago/eve-agent",
            path: ".github/workflows/eve-npm-release.yml",
  ...
  it("fetches npm registry JSON with bounded response handling", async () => {
    ...
    await expect(
      fetchRegistryJson("https://registry.example/eve", {
        fetchImpl,
        timeoutMs: 1234,
      }),
  ...
  it("requires a trusted GitHub release identity for the exact SLSA provenance attestation", async () => {
    ...
    expect(verificationPolicy).toEqual({
      ...
      certificateIdentityURI:
        "https://github.com/engsathiago/eve-agent/.github/workflows/eve-npm-release.yml@refs/heads/release/2026.3.23",
    });
  ...
  it("rejects matching provenance from an untrusted source before Sigstore verification", async () => {
    ...
    ).rejects.toThrow("does not bind 2026.3.23 to the trusted EVE GitHub release workflow");
...
describe("collectInstalledPackageErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "eve-postpublish-package-"));
  }
...
describe("collectInstalledContextEngineRuntimeErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "eve-postpublish-context-engine-"));
  }
...
describe("collectInstalledPluginSdkZodArtifactErrors", () => {
  function withInstalledPackageRoot(run: (packageRoot: string) => void): void {
    const packageRoot = mkdtempSync(join(tmpdir(), "eve-postpublish-zod-sdk-"));
    try {
      run(packageRoot);
    } finally {
      rmSync(packageRoot, { recursive: true, force: true });
...
describe("normalizeInstalledBinaryVersion", () => {
  it("accepts decorated CLI version output", () => {
    expect(normalizeInstalledBinaryVersion("EVE 2026.4.8 (9ece252)")).toBe("2026.4.8");
    expect(normalizeInstalledBinaryVersion("EVE 2026.4.8-beta.1 (9ece252)")).toBe(
      "2026.4.8-beta.1",
    );
    expect(normalizeInstalledBinaryVersion("EVE 2026.4.8-alpha.1 (9ece252)")).toBe(
      "2026.4.8-alpha.1",
    );
  });
});
...
describe("resolveInstalledBinaryCommandInvocation", () => {
  it("runs the Unix installed binary directly", () => {
    expect(
      resolveInstalledBinaryCommandInvocation("/tmp/eve-prefix", ["--version"], {
        platform: "linux",
      }),
    ).toEqual({
      command: "/tmp/eve-prefix/bin/eve",
      args: ["--version"],
    });
  });
...
describe("collectInstalledRootDependencyManifestErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "eve-postpublish-root-deps-"));
  }
