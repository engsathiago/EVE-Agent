...
import { bundledPluginRoot } from "eve-agent/plugin-sdk/test-fixtures";
...
type OfficialChannelInstall = NonNullable<
  NonNullable<OfficialChannelCatalogEntry["eve"]>["install"]
>;
...
function requireInstall(entry: OfficialChannelCatalogEntry | undefined): OfficialChannelInstall {
  const install = entry?.eve?.install;
  if (!install) {
...
function summarizeCatalogEntry(entry: OfficialChannelCatalogEntry) {
  return {
    ...
    source: entry.source,
    plugin: entry.eve?.plugin,
    channel: entry.eve?.channel,
    install: entry.eve?.install,
  };
}
...
describe("buildOfficialChannelCatalog", () => {
  it("includes publishable official channel plugins and skips non-publishable entries", () => {
    const repoRoot = makeRepoRoot("eve-official-channel-catalog-");
    ...
    expect(
      summarizeCatalogEntry(
        findCatalogEntry(entries, (entry) => entry.name === "@wecom/wecom-eve-plugin"),
      ),
    ).toEqual({
      name: "@wecom/wecom-eve-plugin",
      description: "EVE WeCom channel plugin by the Tencent WeCom team.",
      source: "external",
      plugin: {
        id: "wecom-eve-plugin",
        label: "WeCom",
    ...
    expect(
      summarizeCatalogEntry(
        findCatalogEntry(entries, (entry) => entry.name === "eve-plugin-yuanbao"),
      ),
    ).toEqual({
      name: "eve-plugin-yuanbao",
      description: "EVE Yuanbao channel plugin by the Tencent Yuanbao team.",
      source: "external",
      plugin: {
        id: "eve-plugin-yuanbao",
        label: "Yuanbao",
      },
      channel: {
        id: "yuanbao",
  ...
  it("keeps third-party official external catalog npm sources exactly pinned", () => {
    const repoRoot = makeRepoRoot("eve-official-channel-catalog-policy-");
    const entries = buildOfficialChannelCatalog({ repoRoot }).entries.filter(
      (entry) => entry.source === "external" && !entry.name?.startsWith("@eve/"),
    );
  ...
  it("allows official EVE channel npm specs without integrity during launch", () => {
    const repoRoot = makeRepoRoot("eve-official-channel-catalog-eve-policy-");
    const twitch = buildOfficialChannelCatalog({ repoRoot }).entries.find(
      (entry) => entry.eve?.channel?.id === "twitch",
    );

    expect({
      name: twitch?.name,
      install: twitch?.eve?.install,
    }).toEqual({
      name: "@eve/twitch",
      install: {
        npmSpec: "@eve/twitch",
  ...
  it("preserves ClawHub specs when generating publishable channel catalog entries", () => {
    const repoRoot = makeRepoRoot("eve-official-channel-catalog-clawhub-");
    writeJson(path.join(repoRoot, "extensions", "storepack-chat", "package.json"), {
      name: "@eve/storepack-chat",
      eve: {
        channel: {
          id: "storepack-chat",
        ...
        install: {
          clawhubSpec: "clawhub:@eve/storepack-chat",
          npmSpec: "@eve/storepack-chat",
          defaultChoice: "clawhub",
        },
        release: {
          publishToNpm: true,
    ...
    const entry = buildOfficialChannelCatalog({ repoRoot }).entries.find(
      (candidate) => candidate.eve?.channel?.id === "storepack-chat",
    );
  ...
  it("writes the official catalog under dist", () => {
    const repoRoot = makeRepoRoot("eve-official-channel-catalog-write-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@eve/whatsapp",
      eve: {
        channel: {
