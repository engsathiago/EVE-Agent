...
import { createEVETestInstance, type EVETestInstance } from "./eve-test-instance.js";

export { extractFirstTextBlock };

export type GatewayInstance = EVETestInstance;
...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  const inst = await createEVETestInstance({ name });
  try {
