...
const embeddedAgentMocks = getSharedMocks("eve.trigger-handling.embedded-agent-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("eve.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("eve.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("eve.trigger-handling.web-session-mocks", () => ({
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("EVE_HOME", snapshot.eveHome);
  restoreKey("EVE_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.EVE_STATE_DIR = join(home, ".eve");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "eve-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".eve", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): EVEConfig {
