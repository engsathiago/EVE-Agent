// EVE test instance helper spawns isolated EVE processes.
...
import {
  createEVETestState,
  type EVETestState,
  type EVETestStateOptions,
} from "../../src/test-utils/eve-test-state.js";
import { sleep } from "../../src/utils.js";

export type EVETestInstanceOptions = {
  name: string;
  cwd?: string;
  port?: number;
  gatewayToken?: string;
  ...
  state?: Omit<EVETestStateOptions, "applyEnv" | "gateway" | "env">;
  gatewayArgs?: string[];
  startTimeoutMs?: number;
  stopTimeoutMs?: number;
};

export type EVETestInstanceCommandResult = {
  code: number | null;
...
type EVETestChildProcess = Pick<ChildProcessWithoutNullStreams, "kill" | "pid">;
...
async function prepareGatewayEntrypoint(cwd: string): Promise<string[]> {
  ...
  const child = spawn("node", ["scripts/run-node.mjs", "--help"], {
    ...
    detached: shouldUseEVETestProcessGroup(),
  });
  ...
  if (completed === null) {
    signalEVETestProcess(child, "SIGKILL");
...
export async function createEVETestInstance(
  options: EVETestInstanceOptions,
): Promise<EVETestInstance> {
  ...
  const instance: EVETestInstance = {
    name: options.name,
    port,
    ...
    stopGateway: async () => {
      ...
      if (!hasChildExited(child) && !child.killed) {
        try {
          signalEVETestProcess(child, "SIGTERM");
        } catch {
          // ignore
        }
      }
...
function shouldUseEVETestProcessGroup(): boolean {
  return process.platform !== "win32";
}

function signalEVETestProcess(
  child: EVETestChildProcess,
  signal: NodeJS.Signals,
  killProcess: (pid: number, signal: NodeJS.Signals) => boolean = process.kill,
): void {
  if (shouldUseEVETestProcessGroup() && typeof child.pid === "number") {
    try {
