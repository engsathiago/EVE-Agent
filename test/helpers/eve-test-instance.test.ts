// EVE test instance tests cover spawned test instance lifecycle.
import fs from "node:fs/promises";
import path from "node:path";
import { describe, expect, it, vi } from "vitest";
import { createEVETestInstance, testing } from "./eve-test-instance.js";
...
describe("eve test instance", () => {
  ...
  it("signals test instance process groups on POSIX", () => {
    ...
    testing.signalEVETestProcess(child, "SIGKILL", killProcess);
  ...
  it("creates isolated config and spawn env without mutating process env", async () => {
    ...
    const inst = await createEVETestInstance({
      name: "instance-unit",
    ...
    try {
      ...
      expect(inst.stateDir).toBe(path.join(inst.homeDir, ".eve"));
      expect(inst.configPath).toBe(path.join(inst.stateDir, "eve.json"));
      expect(inst.env.HOME).toBe(inst.homeDir);
      expect(inst.env.EVE_STATE_DIR).toBe(inst.stateDir);
      expect(inst.env.EVE_CONFIG_PATH).toBe(inst.configPath);
      expect(inst.env.EVE_SKIP_CRON).toBe("0");
