...
const LIVE =
  isLiveTestEnabled() &&
  isTruthyEnvValue(process.env.EVE_LIVE_INFER_CLI_TEST) &&
...
describeLive("image generation infer CLI live", () => {
  it("generates an image through eve infer", () => {
    const outputBase = path.join(os.tmpdir(), `eve-infer-image-${process.pid}.png`);
