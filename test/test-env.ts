...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.EVE_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "EVE_TEST_FAST", value: process.env.EVE_TEST_FAST },
    {
      key: "EVE_STRICT_FAST_REPLY_CONFIG",
      value: process.env.EVE_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "EVE_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.EVE_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "EVE_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.EVE_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "EVE_STATE_DIR", value: process.env.EVE_STATE_DIR },
    { key: "EVE_CONFIG_PATH", value: process.env.EVE_CONFIG_PATH },
    { key: "EVE_GATEWAY_PORT", value: process.env.EVE_GATEWAY_PORT },
    { key: "EVE_BRIDGE_ENABLED", value: process.env.EVE_BRIDGE_ENABLED },
    { key: "EVE_BRIDGE_HOST", value: process.env.EVE_BRIDGE_HOST },
    { key: "EVE_BRIDGE_PORT", value: process.env.EVE_BRIDGE_PORT },
    { key: "EVE_CANVAS_HOST_PORT", value: process.env.EVE_CANVAS_HOST_PORT },
    { key: "EVE_TEST_HOME", value: process.env.EVE_TEST_HOME },
    { key: "EVE_AGENT_DIR", value: process.env.EVE_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "eve-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.EVE_TEST_HOME = tempHome;
  process.env.EVE_TEST_FAST = "1";
  process.env.EVE_STRICT_FAST_REPLY_CONFIG = "1";
  delete process.env.EVE_ALLOW_SLOW_REPLY_TESTS;
  ...
  delete process.env.EVE_BRIDGE_ENABLED;
  delete process.env.EVE_BRIDGE_HOST;
  delete process.env.EVE_BRIDGE_PORT;
  delete process.env.EVE_CANVAS_HOST_PORT;
  ...
  if (process.platform === "win32") {
    process.env.EVE_STATE_DIR = path.join(tempHome, ".eve");
  }
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.EVE_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.EVE_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".eve");
  const priorIsolatedHome = params.env.EVE_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".eve")
  ) {
    realStateDir = path.join(params.realHome, ".eve");
  }
  const tempStateDir = path.join(params.tempHome, ".eve");
  fs.mkdirSync(tempStateDir, { recursive: true });
  fs.mkdirSync(path.join(params.tempHome, ".gemini"), { recursive: true });

  const realConfigPath = params.env.EVE_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.EVE_CONFIG_PATH, params.realHome)
...
export function installTestEnv(options?: { loadProfileEnv?: boolean }): {
  ...
  const live =
    process.env.LIVE === "1" ||
    process.env.EVE_LIVE_TEST === "1" ||
    process.env.EVE_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.EVE_LIVE_USE_REAL_HOME);
