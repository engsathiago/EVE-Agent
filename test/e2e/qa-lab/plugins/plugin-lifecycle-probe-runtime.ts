...
type ProbeEnv = Pick<NodeJS.ProcessEnv, "HOME" | "EVE_CONFIG_PATH" | "EVE_STATE_DIR">;
...
function stateDir(env: ProbeEnv = process.env) {
  return env.EVE_STATE_DIR || path.join(env.HOME ?? os.homedir(), ".eve");
}

function configPath(env: ProbeEnv = process.env) {
  return env.EVE_CONFIG_PATH || path.join(stateDir(env), "eve.json");
}
...
function createMatrixStateEnv(resourceDir: string): MatrixEnv {
  ...
  const configFile = path.join(stateDir, "eve.json");
  ...
  return {
    ...
    EVE_TEST_WORKSPACE_DIR: workspaceDir,
    EVE_AUTH_PROFILE_SECRET_KEY: randomBytes(32).toString("hex"),
  };
}
...
function packageEntrypoint(prefix: string) {
  ...
  throw new Error(`EVE package entrypoint not found under ${packageRoot}/dist/`);
}
...
async function installEVEPackage(prefix: string, env: MatrixEnv) {
  const packageTgz = env.EVE_CURRENT_PACKAGE_TGZ;
  assertProbe(packageTgz, "EVE_CURRENT_PACKAGE_TGZ is required");
  const installLog = "/tmp/eve-plugin-lifecycle-install.log";
  process.stdout.write("Installing mounted EVE package...\n");
  await runCommand(
    "npm",
    ["install", "-g", "--prefix", prefix, packageTgz, "--no-fund", "--no-audit"],
    {
      ...
      timeoutMs: parseDurationMs(env.EVE_E2E_NPM_INSTALL_TIMEOUT, "600s"),
    },
  );
}
...
export async function runPluginLifecycleMatrix() {
  ...
  const packageName = "@eve/lifecycle-claw";
  const resourceDir = tempDirs.make("eve-plugin-lifecycle-matrix-");
  ...
  try {
    await installEVEPackage(npmPrefix, env);
