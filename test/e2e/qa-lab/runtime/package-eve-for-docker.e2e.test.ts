// Package EVE For Docker tests cover QA Lab package artifact evidence.
...
import {
  ...
  packEVEPackageForDocker,
  parseArgs,
  runCommandForTest,
} from "../../../../scripts/package-eve-for-docker.mjs";
...
describe("package-eve-for-docker", () => {
  ...
  it("uses build-all as the single bounded package artifact build step", async () => {
    ...
    const previousTimeout = process.env.EVE_DOCKER_PACKAGE_BUILD_TIMEOUT_MS;
    process.env.EVE_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = "1234";
    ...
    try {
      await buildPackageArtifacts("/repo", {
        runImpl: async (
          ...
          calls.push({
            ...
            args,
            cwd,
            noPnpm: options.env?.EVE_BUILD_ALL_NO_PNPM,
            skipDts: options.env?.EVE_RUN_NODE_SKIP_DTS_BUILD,
      ...
      if (previousTimeout === undefined) {
        delete process.env.EVE_DOCKER_PACKAGE_BUILD_TIMEOUT_MS;
      } else {
        process.env.EVE_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = previousTimeout;
      }
    }
  ...
  it("rejects loose package artifact timeout env values", async () => {
    ...
    try {
      for (const value of ["1e3", "123.9", "9007199254740993", "0"]) {
        process.env.EVE_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = value;
        ...
        await expect(
          ...
          "EVE_DOCKER_PACKAGE_BUILD_TIMEOUT_MS must be a positive timeout in milliseconds",
        );
      }
    } finally {
      if (previousTimeout === undefined) {
  ...
  it("trims and restores the changelog around ignore-scripts package artifacts", async () => {
    const calls: string[] = [];
    const tarball = await packEVEPackageForDocker("/repo", "/out", {
  ...
  it("rejects path-like npm pack stdout before resolving Docker package tarballs", async () => {
    for (const filename of [
      ...
      await expect(
        packEVEPackageForDocker("/repo", "/out", {
  ...
  it("ignores unsafe output directory tarball names when npm stdout is not usable", async () => {
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "eve-docker-pack-"));
    try {
      fs.writeFileSync(path.join(outputDir, "eve-C:evil.tgz"), "");
      fs.writeFileSync(path.join(outputDir, String.raw`eve-nested\evil.tgz`), "");
      await expect(
      ...
      ).rejects.toThrow("missing packed EVE tarball");
  ...
  it("kills timed-out child process groups", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "eve-package-timeout-"));
  ...
  it("keeps fallback SIGKILL armed for descendants after the direct child exits", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "eve-package-descendant-"));
    const childPidPath = path.join(tempDir, "child.pid");
    let childPid;
    try {
      const childScript = ["process.on('SIGTERM', () => {});", "setInterval(() => {}, 1000);"].join(
  ...
  it("forwards external termination to active child process groups", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "eve-package-signal-"));
    const childPidPath = path.join(tempDir, "child.pid");
    const scriptUrl = pathToFileURL(path.resolve("scripts/package-eve-for-docker.mjs")).href;
    let childPid = 0;
    let runnerPid;
    try {
