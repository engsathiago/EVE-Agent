// Resolve Eve Package Candidate tests cover resolve eve package candidate script behavior.
...
import {
  ...
  runCommandForTest,
  validateEVEPackageSpec,
} from "../../scripts/resolve-eve-package-candidate.mjs";
...
describe("resolve-eve-package-candidate", () => {
  ...
  it("rejects package candidate output names that escape the output directory", () => {
    ...
    expect(parseArgs(["--output-name", "eve-current.tar.gz"]).outputName).toBe(
      "eve-current.tar.gz",
    );
  });
  ...
  it("keeps npm pack filenames inside the package candidate output directory", async () => {
    ...
    await expect(readFile(path.join(dir, "eve-current.tgz"), "utf8")).resolves.toBe("package");
  });
  ...
  it("rejects path-like npm pack filenames instead of renaming outside the output directory", async () => {
    ...
    for (const filename of unsafeFilenames) {
      await expect(
        moveNewestPackedTarballForTest(dir, JSON.stringify([{ filename }]), "eve-current.tgz"),
  ...
  it("kills timed-out package runner process groups", async () => {
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "eve-package-runner-timeout-"));
    tempDirs.push(dir);
  ...
  it("rejects timed-out package runner commands when descendants exit cleanly", async () => {
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "eve-package-runner-timeout-clean-"));
    tempDirs.push(dir);
    const childPidPath = path.join(dir, "child.pid");
    ...
    const childScript = [
      "const fs = require('node:fs');",
      "process.on('SIGTERM', () => {",
      "  fs.writeFileSync(process.env.EVE_TEST_CHILD_CLEANUP, 'clean');",
      "  setTimeout(() => process.exit(0), 75);",
      "});",
      "fs.writeFileSync(process.env.EVE_TEST_CHILD_READY, 'ready');",
      "setInterval(() => {}, 1000);",
    ].join("");
    const parentScript = [
      "const { spawn } = require('node:child_process');",
      ...
      "  env: {",
      "    ...process.env,",
      "    EVE_TEST_CHILD_CLEANUP: process.env.EVE_TEST_CHILD_CLEANUP,",
      "    EVE_TEST_CHILD_READY: process.env.EVE_TEST_CHILD_READY,",
      "  },",
      "});",
      "fs.writeFileSync(process.env.EVE_TEST_CHILD_PID, String(child.pid));",
    ...
    const timeoutAssertion = expect(
      runCommandForTest(process.execPath, ["-e", parentScript], {
        env: {
          ...process.env,
          EVE_TEST_CHILD_CLEANUP: cleanupPath,
          EVE_TEST_CHILD_PID: childPidPath,
          EVE_TEST_CHILD_READY: readyPath,
        },
        killAfterMs: 1000,
        timeoutMs: 1000,
      }),
  ...
  it("fails successful ref candidates when package source worktree cleanup fails", async () => {
    await expect(
      cleanupPackageSourceWorktreeForTest("/tmp/eve-package-source-stuck", {
        runImpl: async () => {
  ...
  it("preserves original ref candidate failures when worktree cleanup also fails", async () => {
    ...
    expect(warnings).toEqual([
      "warning: failed to remove temporary package source worktree /tmp/eve-package-source-stuck: worktree remove denied",
    ]);
  });

  it("loads named trusted package URL source policies", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "eve-trusted-package-source-"));
    tempDirs.push(dir);
  ...
  it("rejects unsafe package_url downloads before fetching private targets", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "eve-package-download-"));
    tempDirs.push(dir);
    const target = path.join(dir, "eve.tgz");

    await expect(
      downloadUrl("http://packages.example/eve.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "93.184.216.34", family: 4 }]),
      }),
    ).rejects.toThrow("package_url must use https");
    await expect(
      downloadUrl("https://user@packages.example/eve.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "93.184.216.34", family: 4 }]),
      }),
    ).rejects.toThrow("package_url must not include credentials");
    await expect(
      downloadUrl("https://localhost/eve.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "127.0.0.1", family: 4 }]),
      }),
    ).rejects.toThrow(/private\/internal\/special-use/iu);
    await expect(
      downloadUrl("https://packages.example/eve.tgz", target, {
        fetchImpl: unexpectedFetch,
  ...
  it("allows private package_url downloads only through an explicit trusted source policy", async () => {
    ...
    await expect(
      downloadUrl("https://packages.internal:8443/other/eve.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "10.0.0.8", family: 4 }]),
        trustedSource,
      }),
  ...
  it("does not forward trusted package auth headers to redirect hosts", async () => {
    ...
    tempDirs.push(dir);
    const target = path.join(dir, "eve.tgz");
    const previousToken = process.env.EVE_TRUSTED_PACKAGE_TOKEN;
    process.env.EVE_TRUSTED_PACKAGE_TOKEN = "token-123";
    const trustedSource = {
      allowPrivateNetwork: true,
      auth: { type: "bearer" },
      hosts: ["packages.internal"],
    ...
    try {
      ...
      if (previousToken === undefined) {
        delete process.env.EVE_TRUSTED_PACKAGE_TOKEN;
      } else {
        process.env.EVE_TRUSTED_PACKAGE_TOKEN = previousToken;
      }
    }
  ...
  it("validates redirects for package_url downloads", async () => {
    ...
    expect(requestedUrls).toEqual(["https://packages.example/eve.tgz"]);
  });
  ...
  it("bounds package_url downloads and writes completed files atomically", async () => {
    ...
    await downloadUrl("https://packages.example/eve.tgz", target, {
      fetchImpl: async () =>
  ...
  it("times out stalled package_url response bodies", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "eve-package-download-timeout-"));
    tempDirs.push(dir);
    const target = path.join(dir, "eve.tgz");
    let bodyCancelled = false;
    const startedAt = Date.now();
    ...
    await expect(
      ...
      "package_url download timed out after 25ms: https://packages.example/eve.tgz",
    );
  ...
  it("reads package source metadata from package artifacts", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "eve-package-candidate-"));
    tempDirs.push(dir);
    await writeFile(
  ...
  it("accepts uppercase package artifact SHA-256 metadata", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "eve-package-sha-"));
    tempDirs.push(dir);
  ...
  it("rejects source artifact scans that exceed the filesystem entry limit", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "eve-package-artifact-scan-"));
    tempDirs.push(dir);
  ...
  it("rejects source artifact directories with multiple tarballs", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "eve-package-artifact-duplicates-"));
    tempDirs.push(dir);

    await writeFile(path.join(dir, "eve-a.tgz"), "a");
    ...
    expect(message).toContain("eve-a.tgz");
    expect(message).not.toContain(path.join(dir, "nested.tar.gz"));
    expect(message).not.toContain(path.join(dir, "eve-a.tgz"));
  });

  it("reads the source SHA from packed npm build metadata", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "eve-package-build-info-"));
    tempDirs.push(dir);
    ...
    const tarball = path.join(dir, "eve.tgz");
