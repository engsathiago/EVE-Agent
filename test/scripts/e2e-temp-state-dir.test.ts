...
describe("E2E temp state dirs", () => {
  it("cleans generated state dirs", async () => {
    const state = await createE2eStateDir("eve-e2e-temp-state-test-", {
      EVE_STATE_DIR: "",
    });
  ...
  it("leaves caller-provided state dirs alone", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "eve-e2e-temp-state-existing-"));
    try {
      const state = await createE2eStateDir("eve-e2e-temp-state-test-", {
        EVE_STATE_DIR: root,
      });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const root = mkdtempSync(path.join(tmpdir(), "eve-e2e-temp-state-retry-"));
  ...
  it("cleans generated state dirs on termination signals", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "eve-e2e-temp-state-signal-"));
    try {
...
const state = await createE2eStateDir("eve-e2e-temp-state-signal-", {
  EVE_STATE_DIR: "",
});
