...
describe("Docker E2E client scripts", () => {
  ...
  it("keeps session runtime-context checks wired to packaged transcript behavior", () => {
    ...
    expect(source).toContain("eve-agent/plugin-sdk/agent-sessions");
    ...
    expect(source).toContain("<<<BEGIN_EVE_INTERNAL_CONTEXT>>>");
    expect(source).toContain("eve.runtime-context");
