...
function runProbe(command: string, payload: unknown): void {
  const root = mkdtempSync(path.join(tmpdir(), "eve-plugin-update-probe-"));
...
describe("plugin update unchanged Docker E2E", () => {
  ...
  it("bounds the update command and prints diagnostics on hangs", () => {
    ...
    expect(script).not.toContain(
      'plugin_update_timeout_seconds="${EVE_PLUGIN_UPDATE_TIMEOUT_SECONDS:-180}"',
    );
    ...
    expect(script).toContain("eve_e2e_print_log /tmp/plugin-update-output.log");
    expect(script).toContain("eve_e2e_print_log /tmp/eve-e2e-registry.log");
    expect(script).not.toContain("cat /tmp/plugin-update-output.log");
    expect(script).not.toContain("cat /tmp/eve-e2e-registry.log");
  });
  ...
  it("waits for the local registry process during cleanup", () => {
    ...
    expect(script).toContain('eve_e2e_stop_process "${registry_pid:-}"');
  ...
  it("bounds corrupt plugin update commands and prints diagnostics on hangs", () => {
    ...
    expect(script).toContain("EVE_UPDATE_CORRUPT_PLUGIN_TIMEOUT_SECONDS");
    expect(script).toContain(
      "eve_e2e_read_positive_int_env EVE_UPDATE_CORRUPT_PLUGIN_TIMEOUT_SECONDS 900",
    );
    expect(script).not.toContain(
    ...
    expect(script.match(/eve_e2e_print_log \/tmp\/eve-update-corrupt-/g)).toHaveLength(8);
    expect(script).not.toContain("cat /tmp/eve-update-corrupt-");
  });
        ...
        outcomes: [
          {
            ...
            status: "skipped",
            message: `Disabled "${CORRUPT_PLUGIN_ID}" after plugin update failure; EVE will continue without it. Failed to update ${CORRUPT_PLUGIN_ID}: registry timeout`,
          },
        ],
      },
    };
        ...
        warnings: [
          {
            ...
            message:
              ...
              `Run eve plugins inspect ${CORRUPT_PLUGIN_ID} --runtime --json for details.`,
          },
        ],
      }),
    ).not.toThrow();
