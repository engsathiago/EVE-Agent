// Eve Test State tests cover eve test state script behavior.
...
const scriptPath = path.join(repoRoot, "scripts/lib/eve-test-state.mjs");
...
function cleanupTestStateHomeTrap(): string {
  return [
    'cleanup_eve_test_state_home() { [ -z "${EVE_TEST_STATE_HOME:-}" ] || rm -rf "$EVE_TEST_STATE_HOME"; }',
    "trap cleanup_eve_test_state_home EXIT",
  ].join("; ");
}

const secretKeyPattern = /^[a-f0-9]{64}$/u;

describe("scripts/lib/eve-test-state", () => {
  it("creates a sourceable env file and JSON description", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "eve-test-state-script-"));
    ...
    try {
      ...
      expect(payload.stateDir).toBe(path.join(payload.home, ".eve"));
      expect(payload.configPath).toBe(path.join(payload.stateDir, "eve.json"));
      expect(payload.workspaceDir).toBe(path.join(payload.home, "workspace"));
      expect(payload.env.EVE_AUTH_PROFILE_SECRET_KEY).toMatch(secretKeyPattern);
      expect(payload.env).toEqual({
        HOME: payload.home,
        USERPROFILE: payload.home,
        EVE_HOME: payload.home,
        EVE_STATE_DIR: payload.stateDir,
        EVE_CONFIG_PATH: payload.configPath,
        EVE_AUTH_PROFILE_SECRET_KEY: payload.env.EVE_AUTH_PROFILE_SECRET_KEY,
      });
      ...
      expect(envFileText).toContain("export EVE_HOME=");
      expect(envFileText).toContain("export EVE_STATE_DIR=");
      expect(envFileText).toContain("export EVE_CONFIG_PATH=");
      expect(envFileText).toContain("export EVE_AUTH_PROFILE_SECRET_KEY=");

      const probe = await execFileAsync("bash", [
        "-lc",
        `source ${shellQuote(envFile)}; node -e 'const fs=require("node:fs"); const config=JSON.parse(fs.readFileSync(process.env.EVE_CONFIG_PATH,"utf8")); process.stdout.write(JSON.stringify({home:process.env.HOME,stateDir:process.env.EVE_STATE_DIR,secretKey:process.env.EVE_AUTH_PROFILE_SECRET_KEY,channel:config.update.channel}));'`,
      ]);
      expect(JSON.parse(probe.stdout)).toEqual({
        home: payload.home,
        stateDir: payload.stateDir,
        secretKey: payload.env.EVE_AUTH_PROFILE_SECRET_KEY,
        channel: "stable",
      });
  ...
  it("renders a Docker-friendly shell snippet", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "eve-test-state-shell-"));
    ...
    try {
      ...
      expect(stdout).toContain(
        'EVE_TEST_STATE_TMP_ROOT="${EVE_TEST_STATE_TMPDIR:-${TMPDIR:-/tmp}}"',
      );
      expect(stdout).toContain(
        'mktemp -d "$EVE_TEST_STATE_TMP_ROOT/eve-update-channel-switch-update-stable-home.XXXXXX"',
      );
      expect(stdout).toContain("EVE_TEST_STATE_JSON");
      expect(stdout).toContain('"channel": "stable"');
      await fs.writeFile(snippetFile, stdout, "utf8");

      const probe = await execFileAsync("bash", [
        "-lc",
        `${cleanupTestStateHomeTrap()}; source ${shellQuote(snippetFile)}; node -e 'const fs=require("node:fs"); const config=JSON.parse(fs.readFileSync(process.env.EVE_CONFIG_PATH,"utf8")); process.stdout.write(JSON.stringify({home:process.env.HOME,eveHome:process.env.EVE_HOME,workspace:process.env.EVE_TEST_WORKSPACE_DIR,secretKey:process.env.EVE_AUTH_PROFILE_SECRET_KEY,channel:config.update.channel}));'`,
      ]);

      const payload = JSON.parse(probe.stdout);
      expect(payload.home.startsWith(os.tmpdir())).toBe(true);
      expect(path.basename(payload.home)).toMatch(
        /^eve-update-channel-switch-update-stable-home\./u,
      );
      expect(payload.eveHome).toBe(payload.home);
      ...
      const customProbe = await execFileAsync("bash", [
        "-lc",
        `${cleanupTestStateHomeTrap()}; export EVE_TEST_STATE_TMPDIR=${shellQuote(customTemp)}; source ${shellQuote(snippetFile)}; node -e 'process.stdout.write(JSON.stringify({home:process.env.HOME,tmpRoot:process.env.EVE_TEST_STATE_TMP_ROOT}));'`,
      ]);
      ...
      const trailingSlashProbe = await execFileAsync("bash", [
        "-lc",
        `${cleanupTestStateHomeTrap()}; export EVE_TEST_STATE_TMPDIR=${shellQuote(`${customTemp}/`)}; source ${shellQuote(snippetFile)}; node -e 'process.stdout.write(JSON.stringify({home:process.env.HOME,tmpRoot:process.env.EVE_TEST_STATE_TMP_ROOT,stateDir:process.env.EVE_STATE_DIR}));'`,
      ]);
  ...
  it("keeps shell key generation independent of node", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "eve-test-state-path-node-"));
    ...
    try {
      ...
      const shellProbe = await execFileAsync("bash", [
        "-lc",
        `${cleanupTestStateHomeTrap()}; export PATH=${shellQuote(fakeBin)}:$PATH; source ${shellQuote(snippetFile)}; printf '%s' "$EVE_AUTH_PROFILE_SECRET_KEY"`,
      ]);
      ...
      const functionProbe = await execFileAsync("bash", [
        "-lc",
        `${cleanupTestStateHomeTrap()}; export PATH=${shellQuote(fakeBin)}:$PATH; export EVE_TEST_STATE_TMPDIR=${shellQuote(path.join(tempRoot, "function-tmp"))}; source ${shellQuote(functionFile)}; eve_test_state_create "path node" minimal; printf '%s' "$EVE_AUTH_PROFILE_SECRET_KEY"`,
      ]);
  ...
  it("renders a reusable Docker shell function", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "eve-test-state-function-"));
    ...
    try {
      ...
      expect(stdout).toContain("eve_test_state_create()");
      expect(stdout).toContain("unset EVE_AGENT_DIR");
      expect(stdout).toContain("update-stable");
      await fs.writeFile(snippetFile, stdout, "utf8");

      const probe = await execFileAsync("bash", [
        "-lc",
        `${cleanupTestStateHomeTrap()}; export EVE_TEST_STATE_TMPDIR=${shellQuote(path.join(tempRoot, "function-tmp"))}; source ${shellQuote(snippetFile)}; export EVE_AGENT_DIR=/tmp/outside-agent; eve_test_state_create "onboard case" minimal; node -e 'const fs=require("node:fs"); const config=JSON.parse(fs.readFileSync(process.env.EVE_CONFIG_PATH,"utf8")); process.stdout.write(JSON.stringify({home:process.env.HOME,tmpDir:process.env.EVE_TEST_STATE_TMPDIR,agentDir:process.env.EVE_AGENT_DIR || null,workspace:process.env.EVE_TEST_WORKSPACE_DIR,secretKey:process.env.EVE_AUTH_PROFILE_SECRET_KEY,config}));'`,
      ]);

      const payload = JSON.parse(probe.stdout);
      expect(payload.home).toBe(`${payload.tmpDir}/${path.basename(payload.home)}`);
      expect(payload.home).toContain("/eve-onboard-case-minimal-home.");
      ...
      const trailingProbe = await execFileAsync("bash", [
        "-lc",
        `${cleanupTestStateHomeTrap()}; export EVE_TEST_STATE_TMPDIR=${shellQuote(`${trailingTmpDir}/`)}; source ${shellQuote(snippetFile)}; eve_test_state_create "onboard case" minimal; node -e 'process.stdout.write(JSON.stringify({home:process.env.HOME,tmpDir:process.env.EVE_TEST_STATE_TMPDIR,stateDir:process.env.EVE_STATE_DIR,workspace:process.env.EVE_TEST_WORKSPACE_DIR}));'`,
      ]);
      ...
      expect(trailingPayload.stateDir).toBe(`${trailingPayload.home}/.eve`);
      ...
      const existingProbe = await execFileAsync("bash", [
        "-lc",
        `source ${shellQuote(snippetFile)}; eve_test_state_create ${shellQuote(existingHome)} minimal; firstKey="$EVE_AUTH_PROFILE_SECRET_KEY"; export firstKey; printf '{"kept":true}\\n' > "$EVE_CONFIG_PATH"; eve_test_state_create ${shellQuote(existingHome)} empty; node -e 'const fs=require("node:fs"); const config=JSON.parse(fs.readFileSync(process.env.EVE_CONFIG_PATH,"utf8")); process.stdout.write(JSON.stringify({home:process.env.HOME,secretKey:process.env.EVE_AUTH_PROFILE_SECRET_KEY,firstKey:process.env.firstKey,config}));'`,
      ]);
  ...
  it("keeps onboard Docker temp homes on the shared test-state helper", async () => {
    ...
    expect(scriptText).toContain("EVE_TEST_STATE_FUNCTION_B64");
    expect(scriptText).toContain("scripts/e2e/lib/onboard/scenario.sh");
    expect(scenarioText).toContain("set_isolated_eve_env local-basic");
