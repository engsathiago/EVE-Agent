...
function makeEnvMutationFixture(): string {
  const repoRoot = createTempDir("eve-test-env-mutations-");
  writeRepoFile(
    repoRoot,
    "src/runtime.ts",
    `
process.env.EVE_STATE_DIR = "ignored because this is not a test file";
`,
  );
  writeRepoFile(
    repoRoot,
    "src/example.test.ts",
    `
process.env.EVE_STATE_DIR = "state";
delete process.env["EVE_CONFIG_PATH"];
vi.stubEnv("HOME", "home");
vi.stubEnv("EVE_WORKSPACE_DIR", "workspace");
process.env = { ...process.env, EVE_HOME: "home", OTHER_KEY: "ignored" };
const dynamicEnvKey = "EVE_STATE_DIR";
process.env[dynamicEnvKey] = "dynamic";
delete process.env[dynamicEnvKey];
const fixture = 'process.env.EVE_HOME = "not code"';
process.env.NOT_EVE = "ignored";
`,
  );
  writeRepoFile(
    repoRoot,
    ...
    `
process.env.EVE_CONFIG_PATH = "script-harness";
`,
  );
  writeRepoFile(
    repoRoot,
    ...
    `
process.env.EVE_AGENT_DIR = "agent";
`,
  );
  writeRepoFile(
    repoRoot,
    "src/test-utils/eve-test-state.ts",
    `
...
describe("collectTestEnvMutationReport", () => {
  it("reports active EVE env mutations while ignoring strings and non-test files", () => {
    ...
    expect(
      ...
      { file: "src/example.test.ts", key: "EVE_STATE_DIR", operation: "assign" },
      { file: "src/example.test.ts", key: "EVE_CONFIG_PATH", operation: "delete" },
      { file: "src/example.test.ts", key: "HOME", operation: "stubEnv" },
      { file: "src/example.test.ts", key: "EVE_WORKSPACE_DIR", operation: "stubEnv" },
      { file: "src/example.test.ts", key: "EVE_HOME", operation: "replace" },
  ...
  it("renders a non-blocking text report for active findings", () => {
    ...
    expect(rendered).toContain("EVE test env mutation report");
    ...
    expect(rendered).toContain("L2 EVE_STATE_DIR assign process.env");
