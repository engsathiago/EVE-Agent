...
const PODMAN_RUN_PATH = "scripts/run-eve-podman.sh";
...
const RELEASE_CHECKS_WORKFLOW_PATH = ".github/workflows/eve-release-checks.yml";
const LIVE_E2E_WORKFLOW_PATH = ".github/workflows/eve-live-and-e2e-checks-reusable.yml";
...
describe("test-install-sh-docker", () => {
  ...
  it("supports npm update package specs without a separate expected-version env", () => {
    ...
    expect(script).toContain(
      'UPDATE_EXPECT_VERSION="${EVE_INSTALL_SMOKE_UPDATE_EXPECT_VERSION:-}"',
    );
  ...
  it("uses npm latest as the update baseline and resolves it to the concrete packed version", () => {
    ...
    expect(script).toContain(
      'UPDATE_BASELINE_VERSION="${EVE_INSTALL_SMOKE_UPDATE_BASELINE:-latest}"',
    );
    ...
    expect(workflow).toContain(
      "EVE_INSTALL_SMOKE_UPDATE_BASELINE: ${{ inputs.update_baseline_version || 'latest' }}",
    );
  });
  ...
  it("can reuse dist from the already-built root Docker smoke image", () => {
    ...
    expect(script).toContain('UPDATE_DIST_IMAGE="${EVE_INSTALL_SMOKE_UPDATE_DIST_IMAGE:-}"');
    ...
    expect(script).toContain(
      'DOCKER_COMMAND_TIMEOUT="${DOCKER_COMMAND_TIMEOUT:-${EVE_INSTALL_SMOKE_DOCKER_COMMAND_TIMEOUT:-600s}}"',
    );
  ...
  it("bounds installer smoke container runs", () => {
    ...
    expect(script).toContain(
      'INSTALL_SMOKE_DOCKER_RUN_TIMEOUT="${EVE_INSTALL_SMOKE_DOCKER_RUN_TIMEOUT:-2700s}"',
    );
  ...
  it("passes the baked browser build arg through Docker setup", () => {
    ...
    expect(script).toContain('export EVE_INSTALL_BROWSER="${EVE_INSTALL_BROWSER:-}"');
    expect(script).toContain("EVE_INSTALL_BROWSER \\");
    expect(script).toContain('--build-arg "EVE_INSTALL_BROWSER=${EVE_INSTALL_BROWSER}"');
  });
  ...
  it("bounds Docker setup image pulls", () => {
    ...
    expect(script).toContain('DOCKER_PULL_TIMEOUT="${EVE_DOCKER_SETUP_PULL_TIMEOUT:-600s}"');
    expect(script).toContain("run_docker_pull()");
    expect(script).toContain('eve_host_timeout_cmd "$DOCKER_PULL_TIMEOUT" docker pull "$image"');
  ...
  it("bounds Podman setup image builds", () => {
    ...
    expect(script).not.toContain('podman build -t "$EVE_IMAGE"');
  });

  it("bounds detached Podman launches without timing out onboarding", () => {
    const script = readFileSync(PODMAN_RUN_PATH, "utf8");

    expect(script).toContain('PODMAN_RUN_TIMEOUT="${EVE_PODMAN_RUN_TIMEOUT:-600s}"');
    expect(script).toContain("EVE_PODMAN_RUN_TIMEOUT|EVE_PODMAN_GATEWAY_HOST_PORT");
    expect(script).toContain('source "$SCRIPT_DIR/lib/host-timeout.sh"');
    expect(script).toContain("run_podman_detached()");
    expect(script).toContain('eve_host_timeout_cmd "$PODMAN_RUN_TIMEOUT" podman run "$@"');
  ...
  it("passes image-scoped pip packages through Docker and Podman setup", () => {
    ...
    expect(dockerfile).toContain("ARG EVE_IMAGE_PIP_PACKAGES");
    expect(dockerfile).toContain(
    ...
    expect(podmanSetup).toContain(
      'BUILD_ARGS+=(--build-arg "EVE_IMAGE_PIP_PACKAGES=${EVE_IMAGE_PIP_PACKAGES}")',
    );
    expect(podmanSetup).not.toContain("EVE_DOCKER_PIP_PACKAGES");
  });
  ...
  it("allows repository branch history and release tags for secret-backed Docker release checks", () => {
    ...
    expect(workflow).toContain("reachable from an EVE branch or release tag");
  });
  ...
  it("writes the package dist inventory before packing ignore-scripts tarballs", () => {
    ...
    expect(script).toContain("node scripts/check-eve-package-tarball.mjs");
  });
  ...
  it("runs candidate tarballs through the installer script instead of direct npm", () => {
    ...
    expect(wrapper).toContain('-v "$ROOT_DIR/scripts/install.sh:/tmp/eve-install.sh:ro"');
  ...
  it("uses public npm latest as the non-root installer expectation", () => {
    ...
    expect(wrapper).toContain('-e EVE_INSTALL_EXPECT_VERSION="$LATEST_VERSION"');
  });
});
...
describe("install-sh E2E runner", () => {
  it("normalizes Docker wrapper timing and toggle knobs before forwarding", () => {
    ...
    expect(wrapper).toContain(
      'SESSION_SCAN_BYTES="$(\n  docker_e2e_read_positive_int_env EVE_INSTALL_E2E_SESSION_SCAN_BYTES 16777216\n)"',
    );
    expect(wrapper).toContain(
      'SESSION_LINE_BYTES="$(\n  docker_e2e_read_positive_int_env EVE_INSTALL_E2E_SESSION_LINE_BYTES 1048576\n)"',
    );
    expect(wrapper).toContain(
      'SESSION_SCAN_DEPTH="$(docker_e2e_read_positive_int_env EVE_INSTALL_E2E_SESSION_SCAN_DEPTH 64)"',
    );
    expect(wrapper).toContain(
      'SESSION_SCAN_NODES="$(docker_e2e_read_positive_int_env EVE_INSTALL_E2E_SESSION_SCAN_NODES 100000)"',
    );
    expect(wrapper).toContain(
      '-e EVE_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS="$OPENAI_PROVIDER_TIMEOUT_SECONDS"',
    );
    expect(wrapper).toContain(
    ...
    expect(wrapper).toContain('-e EVE_INSTALL_E2E_SESSION_SCAN_DEPTH="$SESSION_SCAN_DEPTH"');
    expect(wrapper).toContain('-e EVE_INSTALL_E2E_SESSION_SCAN_NODES="$SESSION_SCAN_NODES"');
    expect(wrapper).not.toContain(
      'EVE_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS="${EVE_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS:-}"',
    );
  });

  it.each([
    ["turn timeout", "EVE_INSTALL_E2E_AGENT_TURN_TIMEOUT_SECONDS", "300s"],
    ["provider timeout", "EVE_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS", "1e3"],
    ["parallel toggle", "EVE_INSTALL_E2E_AGENT_TURNS_PARALLEL", "2"],
    ["tool smoke toggle", "EVE_INSTALL_E2E_AGENT_TOOL_SMOKE", "false"],
    ["session scan bytes", "EVE_INSTALL_E2E_SESSION_SCAN_BYTES", "16mb"],
    ["session line bytes", "EVE_INSTALL_E2E_SESSION_LINE_BYTES", "1mb"],
    ["session scan depth", "EVE_INSTALL_E2E_SESSION_SCAN_DEPTH", "0"],
    ["session scan nodes", "EVE_INSTALL_E2E_SESSION_SCAN_NODES", "100k"],
  ...
  it("validates agent timing and toggle knobs before running provider setup", () => {
    ...
    expect(script).toContain(
      'OPENAI_PROVIDER_TIMEOUT_SECONDS="$(read_positive_int_env EVE_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS "$AGENT_TURN_TIMEOUT_SECONDS")"',
    );
  ...
  it.each([
    ...
    expect(result.status).toBe(2);
    expect(result.stderr).toContain(`invalid ${envName}: ${value}`);
    expect(result.stderr).not.toContain("EVE_E2E_MODELS=both requires");
  });
});
...
describe("install-sh smoke runner", () => {
  it("wraps long npm/update operations with heartbeat and install-size audits", () => {
    ...
    expect(script).toContain(
      'HEARTBEAT_INTERVAL="$(read_nonnegative_int_env EVE_INSTALL_SMOKE_HEARTBEAT_INTERVAL 60)"',
    );
    expect(script).toContain(
      'INSTALL_COMMAND_TIMEOUT="$(read_positive_int_env EVE_INSTALL_SMOKE_COMMAND_TIMEOUT 900)"',
    );
    ...
    expect(script).toContain("eve update --tag");
  ...
  it.each([
    ["command timeout", "EVE_INSTALL_SMOKE_COMMAND_TIMEOUT", "900s"],
    ["heartbeat interval", "EVE_INSTALL_SMOKE_HEARTBEAT_INTERVAL", "60s"],
    ...
    expect(result.status).toBe(2);
    expect(result.stderr).toContain(`invalid ${envName}: ${value}`);
    expect(result.stderr).not.toContain("unsupported EVE_INSTALL_SMOKE_MODE");
  });
  ...
  it("covers plain npm global installs and npm-driven updates", () => {
    ...
    expect(script).toContain('SKIP_NPM_GLOBAL="${EVE_INSTALL_SMOKE_SKIP_NPM_GLOBAL:-0}"');
    expect(script).toContain('NPM_CACHE_DIR="${EVE_INSTALL_SMOKE_NPM_CACHE_DIR:-}"');
    ...
    expect(script).toContain("EVE_INSTALL_SMOKE_MODE=npm-global");
  ...
  it("forwards smoke-runner control knobs into Docker containers", () => {
    ...
    for (const envName of [
      "EVE_INSTALL_ALLOW_LEGACY_UPDATE_WARNING",
      "EVE_INSTALL_SELF_UPDATE_WARNING_FIXED_VERSION",
      "EVE_INSTALL_SMOKE_COMMAND_TIMEOUT",
      "EVE_INSTALL_SMOKE_HEARTBEAT_INTERVAL",
      "EVE_INSTALL_SMOKE_PREVIOUS",
      "EVE_INSTALL_SMOKE_SKIP_PREVIOUS",
    ]) {
...
describe("bun global install smoke", () => {
  it("packs the current tree and verifies image-provider discovery through Bun", () => {
    ...
    expect(script).toContain("EVE_BUN_GLOBAL_SMOKE_DIST_IMAGE");
    expect(script).toContain('source "$ROOT_DIR/scripts/lib/docker-e2e-container.sh"');
    expect(script).toContain(
  ...
  it("rejects invalid Bun global install command timeouts before Bun setup", () => {
    const result = spawnSync("bash", [BUN_GLOBAL_SMOKE_PATH], {
      ...
      env: {
        ...process.env,
        EVE_BUN_GLOBAL_SMOKE_TIMEOUT_MS: "180000ms",
      },
    });

    expect(result.status).toBe(2);
    expect(result.stderr).toContain("invalid EVE_BUN_GLOBAL_SMOKE_TIMEOUT_MS: 180000ms");
  ...
  it.runIf(process.platform !== "win32" && existsSync("/usr/bin/time"))(
    ...
    () => {
      const tempDir = tempDirs.make("eve-bun-global-timeout-grace-");
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            EVE_BUN_GLOBAL_SMOKE_TIMEOUT_KILL_GRACE_MS: "1000",
          },
          timeout: 5_000,
        },
      );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = tempDirs.make("eve-bun-global-parent-signal-");
      const readyPath = path.join(tempDir, "ready");
      ...
      const runner = spawn(
        ...
        ],
        {
          env: {
            ...process.env,
            EVE_BUN_GLOBAL_SMOKE_TIMEOUT_KILL_GRACE_MS: "100",
          },
  ...
  it("gates workflow Bun install smoke to scheduled and release-check runs", () => {
    ...
    expect(workflow).toContain(
      "EVE_BUN_GLOBAL_SMOKE_DIST_IMAGE: ${{ needs.root_dockerfile_image.outputs.image_ref }}",
    );
    expect(workflow).toContain(
    ...
    expect(workflow).toContain("EVE_CI_WORKFLOW_BUN_GLOBAL_INSTALL_SMOKE");
    ...
    expect(workflow).toContain("EVE_INSTALL_URL: file:///tmp/eve-install.sh");
    expect(workflow).toContain("EVE_INSTALL_CLI_URL: file:///tmp/eve-install-cli.sh");
    expect(workflow).toContain('EVE_INSTALL_SMOKE_SKIP_CLI: "0"');
    ...
    expect(workflow).toContain("bash /tmp/install-cli.sh --prefix /tmp/eve-cli");
    ...
    expect(workflow).toContain('EVE_INSTALL_SMOKE_SKIP_NPM_GLOBAL: "1"');
  ...
  it("kills Bun global install smoke commands that ignore TERM after timeout", () => {
    const result = spawnSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          EVE_BUN_GLOBAL_SMOKE_TIMEOUT_KILL_GRACE_MS: "50",
        },
        timeout: 5000,
      },
    );
