// Check Eve Package Tarball tests cover check eve package tarball script behavior.
...
const CHECK_SCRIPT = "scripts/check-eve-package-tarball.mjs";
...
function withTarball(
  ...
  const root = mkdtempSync(join(tmpdir(), "eve-package-tarball-test-"));
  try {
    ...
    const tarFiles =
      options.includeControlUi === false
        ? files
        : {
            "dist/control-ui/index.html": "<!doctype html><eve-app></eve-app>",
    ...
    const tarball = join(root, "eve.tgz");
...
describe("check-eve-package-tarball", () => {
  ...
  it("rejects option-like and extra arguments before tar inspection", () => {
    ...
    expect(unknown.stderr).toContain("Unknown EVE package tarball check option: --tag");
    expect(unknown.stderr).not.toContain("EVE package tarball does not exist");

    const extra = spawnSync("node", [CHECK_SCRIPT, "eve.tgz", "extra"], {
      encoding: "utf8",
    });

    expect(extra.status).not.toBe(0);
    expect(extra.stderr).toContain("Unexpected EVE package tarball check argument: extra");
    expect(extra.stderr).not.toContain("EVE package tarball does not exist");
  });

  it.runIf(process.platform !== "win32")(
    "removes the extract dir when tar extraction fails",
    () => {
      const root = mkdtempSync(join(tmpdir(), "eve-package-tarball-extract-fail-"));
      try {
        ...
        writeFileSync(
          ...
          [
            ...
            "fs.writeFileSync(process.env.EVE_TEST_EXTRACT_DIR_FILE, outputDir);",
        ...
        const result = spawnSync("node", [CHECK_SCRIPT, tarball], {
          ...
          env: {
            ...process.env,
            EVE_TEST_EXTRACT_DIR_FILE: extractDirFile,
  ...
  it("allows legacy private QA inventory entries omitted from shipped tarballs through 2026.4.25", () => {
    withTarball(
      ...
      (tarball) => {
        ...
        expect(result.stdout).toContain("EVE package tarball integrity passed.");
      },
      "2026.4.25-beta.10",
    );
  });
