...
const SHARED_TEST_SETUP = Symbol.for("eve.sharedTestSetup");
const EMBEDDED_RUN_STATE = Symbol.for("eve.embeddedRunState");
const REPLY_RUN_REGISTRY = Symbol.for("eve.replyRunRegistry");
...
function getSharedTestHome(): string | undefined {
  ...
  return globalState[SHARED_TEST_SETUP]?.tempHome ?? process.env.EVE_TEST_HOME;
}
...
function resetEVEGlobalRunState(): void {
...
export default class EVENonIsolatedRunner extends TestRunner {
  override onCollectStart(file: { filepath: string }) {
    super.onCollectStart(file);
    restoreRealTimers();
    restoreNativeTimerGlobals();
    restoreSharedTestHomeAfterEnvUnstub(getSharedTestHome());
    const orderLogPath = process.env.EVE_VITEST_FILE_ORDER_LOG?.trim();
    if (orderLogPath) {
