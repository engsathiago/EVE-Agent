...
function createTempHome(): string {
  return makeTempDir(tempDirs, "eve-test-env-real-home-");
}
...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-eve.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.eve/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.eve/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    writeFile(path.join(realHome, ".eve", "credentials", "token.txt"), "secret\n");
    writeFile(
      path.join(realHome, ".eve", "external-plugins", "glueclaw", "eve.plugin.json"),
      '{"id":"glueclaw"}\n',
    );
    writeFile(
      path.join(realHome, ".eve", "agents", "main", "agent", "auth-profiles.json"),
    ...
    process.env.EVE_LIVE_TEST_QUIET = "1";
    process.env.EVE_CONFIG_PATH = "~/custom-eve.json5";
    process.env.EVE_TEST_HOME = priorIsolatedHome;
    process.env.EVE_STATE_DIR = path.join(priorIsolatedHome, ".eve");
    ...
    expect(process.env.EVE_TEST_HOME).toBe(testEnv.tempHome);
    expect(process.env.TEST_PROFILE_ONLY).toBe("from-profile");

    const copiedConfigPath = path.join(testEnv.tempHome, ".eve", "eve.json");
    ...
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".eve", "agents", "main", "agent", "auth-profiles.json"),
      ),
    ).toBe(true);
  ...
  it("allows explicit live runs against the real HOME", () => {
    ...
    process.env.USERPROFILE = realHome;
    process.env.EVE_LIVE_TEST = "1";
    process.env.EVE_LIVE_USE_REAL_HOME = "1";
    process.env.EVE_LIVE_TEST_QUIET = "1";
  ...
  it("does not load ~/.profile for normal isolated test runs", () => {
    ...
    delete process.env.EVE_LIVE_TEST;
    delete process.env.EVE_LIVE_GATEWAY;
    delete process.env.EVE_LIVE_USE_REAL_HOME;
    delete process.env.EVE_LIVE_TEST_QUIET;
