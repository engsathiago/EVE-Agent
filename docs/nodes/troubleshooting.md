...
```bash
eve nodes describe --node <idOrNameOrIp>
eve nodes canvas snapshot --node <idOrNameOrIp>
eve logs --follow
```
...
eve nodes status
eve approvals get --node <idOrNameOrIp>
eve approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`eve approvals get --node ...`), not in the gateway pairing record.
