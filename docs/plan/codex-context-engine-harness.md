...
summary: "Specification for making the bundled Codex app-server harness honor EVE context-engine plugins"
read_when:
  ...
  - You are comparing embedded EVE and Codex app-server context behavior
---
...
Make the bundled Codex app-server harness honor the same EVE context-engine
lifecycle contract that embedded EVE turns already honor.
...
EVE-level compaction policy as far as the Codex app-server boundary allows.
...
as built-in EVE attempts:

- `extensions/codex/src/app-server/run-attempt.ts`

That means the required hook point is in EVE-controlled code. The external
boundary is the Codex app-server protocol itself: EVE can control what it
...
Built-in EVE attempts call the context-engine lifecycle directly:
...
For Codex harness turns, EVE should preserve this lifecycle:

1. Read the mirrored EVE session transcript.
...
8. Mirror the Codex result back into the EVE transcript.
...
11. Preserve Codex native compaction signals and EVE compaction hooks.

## Design constraints

### Codex app-server remains canonical for native thread state

Codex owns its native thread and any internal extended history. EVE should
not try to mutate the app-server's internal history except through supported
protocol calls.

EVE's transcript mirror remains the source for EVE features:
...
The context-engine interface returns EVE `AgentMessage[]`, not a Codex
...
- Label it clearly as EVE assembled context.
...
inside EVE and preserves context-engine semantics.
...
we need strict EVE parity, run context-engine assembly before hook
...
1. EVE context-engine `compact()`
2. Codex app-server native `thread/compact/start`

Do not silently conflate them.

#### `/compact` and explicit EVE compaction

When the selected context engine has `info.ownsCompaction === true`, explicit
EVE compaction should prefer the context engine's `compact()` result for
the EVE transcript mirror and plugin state.
...
- If Codex native compaction fails after context-engine compaction succeeds,
  do not fail the whole EVE compaction when the context engine is primary.
...
Avoid requiring lossless-claw in EVE core tests. Use a small in-repo fake
...
- Context-engine-owned compaction remains primary for EVE/plugin state.
