---
summary: "CLI reference for `eve qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
eve qr
eve qr --setup-code-only
eve qr --json
eve qr --remote
eve qr --url wss://gateway.example/ws
```
...
- With `--remote`, EVE requires either `gateway.remote.url` or
