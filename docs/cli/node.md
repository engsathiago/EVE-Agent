---
summary: "CLI reference for `eve node` (headless node host)"
read_when:
...
`eve node run` and `eve node install` resolve gateway auth from config/env (no `--token`/`--password` flags on node commands):

- `EVE_GATEWAY_TOKEN` / `EVE_GATEWAY_PASSWORD` are checked first.
...
trusted private-DNS names, set `EVE_ALLOW_INSECURE_PRIVATE_WS=1`; without
it, node startup fails closed and asks you to use `wss://`, an SSH tunnel, or
Tailscale. This is a process-environment opt-in, not an `eve.json` config
key.
`eve node install` persists it into the supervised node service when it is
...
```bash
eve node install --host <gateway-host> --port 18789
```
...
Use `eve node run` for a foreground node host (no service).
...
- `eve approvals --node <id|name|ip>` (edit from the Gateway)

For approved async node exec, EVE prepares a canonical `systemRunPlan`
