---
summary: "CLI reference for `eve nodes` (status, pairing, invoke, camera/canvas/screen)"
read_when:
...
```bash
eve nodes list
eve nodes list --connected
eve nodes list --last-connected 24h
eve nodes pending
eve nodes approve <requestId>
eve nodes reject <requestId>
eve nodes remove --node <id|name|ip>
eve nodes rename --node <id|name|ip> --name <displayName>
eve nodes status
eve nodes status --connected
eve nodes status --last-connected 24h
```
...
- `eve nodes pending` only needs pairing scope.
...
- `eve nodes approve <requestId>` inherits extra scope requirements from the
  pending request:
...
```bash
eve nodes invoke --node <id|name|ip> --command <command> --params <json>
```
...
For shell execution on a node, use the `exec` tool with `host=node` instead of `eve nodes run`.
The `nodes` CLI is now capability-focused: direct RPC via `nodes invoke`, plus pairing, camera,
screen, location, Canvas, and notifications. Canvas commands are implemented by the bundled experimental Canvas plugin; core keeps a compatibility hook so they remain under `eve nodes canvas`.
