...
read_when:
  - You run eve with no command after setup and want to understand Crestodian
  - You need a configless-safe way to inspect or repair EVE
...
Crestodian is EVE's local setup, repair, and configuration helper. It is
designed to stay reachable when the normal agent path is broken.

Running `eve` with no command starts classic onboarding first when the
active config file is missing or has no authored settings (empty or
metadata-only). After a config file has authored settings, running `eve`
with no command starts Crestodian in an interactive terminal. Running
`eve crestodian` starts the same helper explicitly.

## What Crestodian shows

On startup, interactive Crestodian opens the same TUI shell used by
`eve tui`, with a Crestodian chat backend. The chat log starts with a short
greeting:
...
Crestodian uses the same EVE reference discovery as regular agents. In a Git checkout,
...
[https://github.com/engsathiago/eve-agent](https://github.com/engsathiago/eve-agent), with explicit
...
eve crestodian
eve crestodian --json
eve crestodian --message "models"
eve crestodian --message "validate config"
eve crestodian --message "setup workspace ~/Projects/work model openai/gpt-5.5" --yes
eve crestodian --message "set default model openai/gpt-5.5" --yes
eve onboard --modern
```
...
config set-ref gateway.auth.token env EVE_GATEWAY_TOKEN
gateway status
restart gateway
agents
...
plugin install clawhub:eve-codex-app-server
plugin uninstall eve-codex-app-server
talk to work agent
...
`eve --help` and `eve --version` still use the normal fast paths.
Noninteractive bare `eve` exits with a short message instead of printing
...
```text
~/.eve/audit/crestodian.jsonl
```

Discovery is not audited. Only applied operations and writes are logged.

`eve onboard --modern` starts Crestodian as the modern onboarding preview.
Plain `eve onboard` still runs classic onboarding.
...
planner turn through EVE's normal runtime paths. It first uses the
configured EVE model. If no configured model is usable yet, it can fall
...
`eve tui`, `eve chat`, and `eve terminal` still open the normal
...
EVE: Crestodian rescue mode. Gateway reachable: no. Config valid: no.
You: /crestodian restart gateway
EVE: Plan: restart the Gateway. Reply /crestodian yes to apply.
You: /crestodian yes
EVE: Applied. Audit entry written.
```
...
- Remote rescue cannot open the local TUI or switch into an interactive agent
  session. Use local `eve` for agent handoff.
