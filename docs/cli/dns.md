---
summary: "CLI reference for `eve dns` (wide-area discovery helpers)"
read_when:
  - You want wide-area discovery (DNS-SD) via Tailscale + CoreDNS
  - You're setting up split DNS for a custom discovery domain (example: eve.internal)
title: "DNS"
---
...
```bash
eve dns setup
eve dns setup --domain eve.internal
eve dns setup --apply
```
...
- `--domain <domain>`: wide-area discovery domain (for example `eve.internal`)
...
- zone file path
- current tailnet IPs
- recommended `eve.json` discovery config
...
- If `--domain` is omitted, EVE uses `discovery.wideArea.domain` from config.
