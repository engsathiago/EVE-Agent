---
summary: "CLI reference for `eve plugins` (init, build, validate, list, install, marketplace, uninstall, enable/disable, doctor)"
read_when:
...
eve plugins search <query>
eve plugins search <query> --limit 20
eve plugins search <query> --json
eve plugins install <path-or-spec>
eve plugins inspect <id>
eve plugins inspect <id> --runtime
eve plugins inspect <id> --json
eve plugins inspect --all
eve plugins info <id>
eve plugins enable <id>
eve plugins disable <id>
eve plugins registry
eve plugins registry --refresh
eve plugins uninstall <id>
eve plugins doctor
eve plugins update <id-or-npm-spec>
eve plugins update --all
eve plugins marketplace list <marketplace>
eve plugins marketplace list <marketplace> --json
eve plugins init <id>
eve plugins init <id> --directory ./my-plugin --name "My Plugin"
eve plugins build --entry ./dist/index.js
eve plugins build --entry ./dist/index.js --check
eve plugins validate --entry ./dist/index.js
```

For slow install, inspect, uninstall, or registry-refresh investigation, run the
command with `EVE_PLUGIN_LIFECYCLE_TRACE=1`. The trace writes phase timings
to stderr and keeps JSON output parseable. See [Debugging](/help/debugging#plugin-lifecycle-trace).

<Note>
In Nix mode (`EVE_NIX_MODE=1`), plugin lifecycle mutators are disabled. Use the Nix source for this install instead of `plugins install`, `plugins update`, `plugins uninstall`, `plugins enable`, or `plugins disable`; for nix-eve, use the agent-first [Quick Start](https://github.com/eve/nix-eve#quick-start).
</Note>

<Note>
Bundled plugins ship with EVE. Some are enabled by default (for example bundled model providers, bundled speech providers, and the bundled browser plugin); others require `plugins enable`.

Native EVE plugins must ship `eve.plugin.json` with an inline JSON Schema (`configSchema`, even if empty). Compatible bundles use their own bundle manifests instead.

`plugins list` shows `Format: eve` or `Format: bundle`. Verbose list/info output also shows the bundle subtype (`codex`, `claude`, or `cursor`) plus detected bundle capabilities.
</Note>

### Author

```bash
eve plugins init stock-quotes --name "Stock Quotes"
cd stock-quotes
...
metadata, writes `eve.plugin.json`, and keeps `package.json`
`eve.extensions` aligned. `plugins validate` checks that the generated
...
```bash
eve plugins search "calendar"                   # search ClawHub plugins
eve plugins install <package>                      # source auto-detection
eve plugins install clawhub:<package>              # ClawHub only
eve plugins install npm:<package>                  # npm only
eve plugins install npm-pack:<path.tgz>            # local npm pack through npm install semantics
eve plugins install git:github.com/<owner>/<repo>  # git repo
eve plugins install git:github.com/<owner>/<repo>@<ref>
eve plugins install <package> --force              # overwrite existing install
eve plugins install <package> --pin                # pin version
eve plugins install <package> --dangerously-force-unsafe-install
eve plugins install <path>                         # local path
eve plugins install <plugin>@<marketplace>         # marketplace
eve plugins install <plugin> --marketplace <name>  # marketplace (explicit)
eve plugins install <plugin> --marketplace https://github.com/<owner>/<repo>
```
...
<Warning>
Bare package names install from npm by default during the launch cutover, unless they match an official plugin id. Raw `@eve/*` package specs that match bundled plugins use the bundled copy that shipped with the current EVE build. Use `npm:<package>` when you deliberately want an external npm package instead. Use `clawhub:<package>` for ClawHub. Treat plugin installs like running code. Prefer pinned versions.
</Warning>

`plugins search` queries ClawHub for installable plugin packages and prints
install-ready package names. It searches code-plugin and bundle-plugin packages,
not skills. Use `eve skills search` for ClawHub skills.

<Note>
ClawHub is the primary distribution and discovery surface for most plugins. Npm
remains a supported fallback and direct-install path. EVE-owned
`@eve/*` plugin packages are published on npm again; see the current list
on [npmjs.com/org/eve](https://www.npmjs.com/org/eve) or the
...
<AccordionGroup>
  <Accordion title="Config includes and invalid-config repair">
    If your `plugins` section is backed by a single-file `$include`, `plugins install/update/enable/disable/uninstall` write through to that included file and leave `eve.json` untouched. Root includes, include arrays, and includes with sibling overrides fail closed instead of flattening. See [Config includes](/gateway/configuration) for the supported shapes.

    If config is invalid during install, `plugins install` normally fails closed and tells you to run `eve doctor --fix` first. During Gateway startup and hot reload, invalid plugin config fails closed like any other invalid config; `eve doctor --fix` can quarantine the invalid plugin entry. The only documented install-time exception is a narrow bundled-plugin recovery path for plugins that explicitly opt into `eve.install.allowInvalidConfigRecovery`.

  </Accordion>
  <Accordion title="--force and reinstall vs update">
    `--force` reuses the existing install target and overwrites an already-installed plugin or hook pack in place. Use it when you are intentionally reinstalling the same id from a new local path, archive, ClawHub package, or npm artifact. For routine upgrades of an already tracked npm plugin, prefer `eve plugins update <id-or-npm-spec>`.

    If you run `plugins install` for a plugin id that is already installed, EVE stops and points you at `plugins update <id-or-npm-spec>` for a normal upgrade, or at `plugins install <package> --force` when you genuinely want to overwrite the current install from a different source.
  ...
  <Accordion title="--dangerously-force-unsafe-install">
    `--dangerously-force-unsafe-install` is deprecated and is now a no-op. EVE no longer runs built-in install-time dangerous-code blocking for plugin installs.
  ...
  <Accordion title="Hook packs and npm specs">
    ...
    Raw `@eve/*` package specs that match bundled plugins resolve to the image-owned bundled copy before npm fallback. For example, `eve plugins install @eve/discord@2026.5.20 --pin` uses the bundled Discord plugin from the current EVE build instead of creating a managed npm override. To force the external npm package, use `eve plugins install npm:@eve/discord@2026.5.20 --pin`.

    Bare specs and `@latest` stay on the stable track. EVE date-stamped correction versions such as `2026.5.3-1` are stable releases for this check. If npm resolves either of those to a prerelease, EVE stops and asks you to opt in explicitly with a prerelease tag such as `@beta`/`@rc` or an exact prerelease version such as `@1.2.3-beta.4`.

    For npm installs without an exact version (`npm:<package>` or `npm:<package>@latest`), EVE checks the resolved package metadata before install. If the latest stable package requires a newer EVE plugin API or minimum host version, EVE inspects older stable versions and installs the newest compatible release instead. Exact versions and explicit dist-tags such as `@beta` remain strict: if the selected package is incompatible, the command fails and asks you to upgrade EVE or choose a compatible version.

    If a bare install spec matches an official plugin id (for example `diffs`), EVE installs the catalog entry directly. To install an npm package with the same name, use an explicit scoped spec (for example `@scope/diffs`).
  ...
  <Accordion title="Git repositories">
    ...
    Git installs clone into a temporary directory, check out the requested ref when present, then use the normal plugin directory installer. That means manifest validation, operator install policy, package-manager install work, and install records behave like npm installs. Recorded git installs include the source URL/ref plus the resolved commit so `eve plugins update` can re-resolve the source later.

    After installing from git, use `eve plugins inspect <id> --runtime --json` to verify runtime registrations such as gateway methods and CLI commands. If the plugin registered a CLI root with `api.registerCli`, execute that command directly through the EVE root CLI, for example `eve demo-plugin ping`.

  </Accordion>
  <Accordion title="Archives">
    Supported archives: `.zip`, `.tgz`, `.tar.gz`, `.tar`. Native EVE plugin archives must contain a valid `eve.plugin.json` at the extracted plugin root; archives that only contain `package.json` are rejected before EVE writes install records.
...
```bash
eve plugins install clawhub:eve-codex-app-server
eve plugins install clawhub:eve-codex-app-server@1.2.3
```

Bare npm-safe plugin specs install from npm by default during the launch cutover unless they match an official plugin id:

```bash
eve plugins install eve-codex-app-server
```

Use `npm:` to make npm-only resolution explicit:

```bash
eve plugins install npm:eve-codex-app-server
eve plugins install npm:@eve/discord@2026.5.20
eve plugins install npm:@scope/plugin-name@1.0.1
```

EVE checks the advertised plugin API / minimum gateway compatibility before install. When the selected ClawHub version publishes a ClawPack artifact, EVE downloads the versioned npm-pack `.tgz`, verifies the ClawHub digest header and the artifact digest, then installs it through the normal archive path. Older ClawHub versions without ClawPack metadata still install through the legacy package archive verification path. Recorded installs keep their ClawHub source metadata, artifact kind, npm integrity, npm shasum, tarball name, and ClawPack digest facts for later updates.
Unversioned ClawHub installs keep an unversioned recorded spec so `eve plugins update` can follow newer ClawHub releases; explicit version or tag selectors such as `clawhub:pkg@1.2.3` and `clawhub:pkg@beta` remain pinned to that selector.
...
```bash
eve plugins marketplace list <marketplace-name>
eve plugins install <plugin-name>@<marketplace-name>
```

Use `--marketplace` when you want to pass the marketplace source explicitly:

```bash
eve plugins install <plugin-name> --marketplace <marketplace-name>
eve plugins install <plugin-name> --marketplace <owner/repo>
eve plugins install <plugin-name> --marketplace https://github.com/<owner>/<repo>
eve plugins install <plugin-name> --marketplace ./my-marketplace
```
...
<Tabs>
  ...
  <Tab title="Remote marketplace rules">
    For remote marketplaces loaded from GitHub or git, plugin entries must stay inside the cloned marketplace repo. EVE accepts relative path sources from that repo and rejects HTTP(S), absolute-path, git, GitHub, and other non-path plugin sources from remote manifests.
  </Tab>
</Tabs>

For local paths and archives, EVE auto-detects:

- native EVE plugins (`eve.plugin.json`)
...
an install hint such as `eve plugins install clawhub:<package>`.
...
`/app/extensions/synology-chat`. EVE will discover that mounted source
...
- `eve plugins inspect <id> --runtime --json` shows registered hooks and diagnostics from a module-loaded inspection pass. Runtime inspection never installs dependencies; use `eve doctor --fix` to clean legacy dependency state or recover missing downloadable plugins that are referenced by config.
- `eve gateway status --deep --require-rpc` confirms the reachable Gateway URL/profile, service/process hints, config path, and RPC health.
...
```bash
eve plugins install -l ./my-plugin
```

Standalone plugin files must be listed in `plugins.load.paths` rather than
installed with `plugins install` or placed directly in `~/.eve/extensions`
or `<workspace>/.eve/extensions`. Those auto-discovered roots load plugin
...
run `eve plugins enable <plugin-id>` or set
...
Plugin install metadata is machine-managed state, not user config. Installs and updates write it to the shared SQLite state database under the active EVE state directory. The `installed_plugin_index` row stores durable `installRecords` metadata, including records for broken or missing plugin manifests, plus a manifest-derived cold registry cache used by `eve plugins update`, uninstall, diagnostics, and the cold plugin registry.

When EVE sees shipped legacy `plugins.installs` records in config, runtime reads treat them as compatibility input without rewriting `eve.json`. Explicit plugin writes and `eve doctor --fix` move those records into the plugin index and remove the config key when config writes are allowed; if either write fails, the config records are kept so the install metadata is not lost.

### Uninstall

```bash
eve plugins uninstall <id>
eve plugins uninstall <id> --dry-run
eve plugins uninstall <id> --keep-files
```

`uninstall` removes plugin records from `plugins.entries`, the persisted plugin index, plugin allow/deny list entries, and linked `plugins.load.paths` entries when applicable. Unless `--keep-files` is set, uninstall also removes the tracked managed install directory when it is inside EVE's plugin extensions root. For active memory plugins, the memory slot resets to `memory-core`.
...
eve plugins update <id-or-npm-spec>
eve plugins update --all
eve plugins update <id-or-npm-spec> --dry-run
eve plugins update @eve/voice-call
eve plugins update eve-codex-app-server --dangerously-force-unsafe-install
```
...
<AccordionGroup>
  <Accordion title="Resolving plugin id vs npm spec">
    When you pass a plugin id, EVE reuses the recorded install spec for that plugin. That means previously stored dist-tags such as `@beta` and exact pinned versions continue to be used on later `update <id>` runs.

    For npm installs, you can also pass an explicit npm package spec with a dist-tag or exact version. EVE resolves that package name back to the tracked plugin record, updates that installed plugin, and records the new npm spec for future id-based updates.
...
Inspect shows identity, load status, source, manifest capabilities, policy flags, diagnostics, install metadata, bundle capabilities, and any detected MCP or LSP server support without importing plugin runtime by default. JSON output includes the plugin manifest contracts, such as `contracts.agentToolResultMiddleware` and `contracts.trustedToolPolicies`, so operators can audit trusted-surface declarations before enabling or restarting a plugin. Add `--runtime` to load the plugin module and include registered hooks, tools, commands, services, gateway methods, and HTTP routes. Runtime inspection reports missing plugin dependencies directly; installs and repairs stay in `eve plugins install`, `eve plugins update`, and `eve doctor --fix`.

Plugin-owned CLI commands are usually installed as root `eve` command groups, but plugins may also register nested commands under a core parent such as `eve nodes`. After `inspect --runtime` shows a command under `cliCommands`, run it at the listed path; for example a plugin that registers `demo-git` can be verified with `eve demo-git ping`.
...
For module-shape failures such as missing `register`/`activate` exports, rerun with `EVE_PLUGIN_LOAD_DEBUG=1` to include a compact export-shape summary in the diagnostic output.
...
eve plugins registry
eve plugins registry --refresh
eve plugins registry --json
```

The local plugin registry is EVE's persisted cold read model for installed plugin identity, enablement, source metadata, and contribution ownership. Normal startup, provider owner lookup, channel setup classification, and plugin inventory can read it without importing plugin runtime modules.
...
```bash
eve plugins marketplace list <source>
eve plugins marketplace list <source> --json
```
