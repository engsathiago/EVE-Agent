---
summary: "CLI reference for `eve commitments` (inspect and dismiss inferred follow-ups)"
read_when:
...
With no subcommand, `eve commitments` lists pending commitments.

## Usage

```bash
eve commitments [--all] [--agent <id>] [--status <status>] [--json]
eve commitments list [--all] [--agent <id>] [--status <status>] [--json]
eve commitments dismiss <id...> [--json]
```
...
```bash
eve commitments dismiss cm_abc123 cm_def456
```

Export as JSON:

```bash
eve commitments --all --json
```
