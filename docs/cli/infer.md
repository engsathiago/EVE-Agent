...
read_when:
  - Adding or modifying `eve infer` commands
  - Designing stable headless capability automation
title: "Inference CLI"
---

`eve infer` is the canonical headless surface for provider-backed inference workflows.
...
```text
Read https://docs.eve.ai/cli/infer, then create a skill that routes my common workflows to `eve infer`.
...
- prefer `eve infer ...` in examples and suggestions
...
- `eve infer audio transcribe`
- `eve infer tts convert`
- `eve infer web search`
- `eve infer embedding create`

## Why use infer

`eve infer` provides one consistent CLI for provider-backed inference tasks inside EVE.

Benefits:

- Use the providers and models already configured in EVE instead of wiring up one-off wrappers for each backend.
...
- Prefer a first-party EVE surface when the task is fundamentally "run inference."
- Use the normal local path without requiring the gateway for most infer commands.

For end-to-end provider checks, prefer `eve infer ...` once lower-level
...
| Task                          | Command                                                                                       | Notes                                                 |
| ----------------------------- | --------------------------------------------------------------------------------------------- | ----------------------------------------------------- |
| Run a text/model prompt       | `eve infer model run --prompt "..." --json`                                              | Uses the normal local path by default                 |
| Run a model prompt on images  | `eve infer model run --prompt "Describe this" --file ./image.png --model provider/model` | Repeat `--file` for multiple image inputs             |
| Generate an image             | `eve infer image generate --prompt "..." --json`                                         | Use `image edit` when starting from an existing file  |
| Describe an image file or URL | `eve infer image describe --file ./image.png --prompt "..." --json`                      | `--model` must be an image-capable `<provider/model>` |
| Transcribe audio              | `eve infer audio transcribe --file ./memo.m4a --json`                                    | `--model` must be `<provider/model>`                  |
| Synthesize speech             | `eve infer tts convert --text "..." --output ./speech.mp3 --json`                        | `tts status` is gateway-oriented                      |
| Generate a video              | `eve infer video generate --prompt "..." --json`                                         | Supports provider hints such as `--resolution`        |
| Describe a video file         | `eve infer video describe --file ./clip.mp4 --json`                                      | `--model` must be `<provider/model>`                  |
| Search the web                | `eve infer web search --query "..." --json`                                              |                                                       |
| Fetch a web page              | `eve infer web fetch --url https://example.com --json`                                   |                                                       |
| Create embeddings             | `eve infer embedding create --text "..." --json`                                         |                                                       |

## Behavior

- `eve infer ...` is the primary CLI surface for these workflows.
...
```bash
eve infer model run --prompt "Reply with exactly: smoke-ok" --json
eve infer model run --prompt "Summarize this changelog entry" --model openai/gpt-5.4 --json
eve infer model run --prompt "Describe this image in one sentence" --file ./photo.jpg --model google/gemini-2.5-flash --json
eve infer model run --prompt "Use more reasoning here" --thinking high --json
eve infer model providers --json
eve infer model inspect --name gpt-5.5 --json
```
...
```bash
eve infer model run --local --model anthropic/claude-sonnet-4-6 --prompt "Reply with exactly: pong" --json
eve infer model run --local --model cerebras/zai-glm-4.7 --prompt "Reply with exactly: pong" --json
eve infer model run --local --model google/gemini-2.5-flash --prompt "Reply with exactly: pong" --json
eve infer model run --local --model groq/llama-3.1-8b-instant --prompt "Reply with exactly: pong" --json
eve infer model run --local --model mistral/mistral-medium-3-5 --prompt "Reply with exactly: pong" --json
eve infer model run --local --model mistral/mistral-small-latest --prompt "Reply with exactly: pong" --json
eve infer model run --local --model openai/gpt-5.5 --prompt "Reply with exactly: pong" --json
eve infer model run --local --model ollama/qwen2.5vl:7b --prompt "Describe this image." --file ./photo.jpg --json
```
...
- Codex Responses local probes are the narrow exception: EVE adds a minimal system instruction so the transport can populate its required `instructions` field, without adding full agent context, tools, memory, or session transcript.
...
- `model run --file` is best when you want to test the selected multimodal text model directly. Use `infer image describe` when you want EVE's image-understanding provider selection and default image-model routing.
...
- Use `model run --gateway` when you need to test Gateway routing, agent-runtime setup, or Gateway-managed provider state while keeping the model input raw. Use `eve agent` or chat surfaces when you want the full agent context, tools, memory, and session transcript.
...
```bash
eve infer image generate --prompt "friendly lobster illustration" --json
eve infer image generate --prompt "cinematic product photo of headphones" --json
eve infer image generate --model openai/gpt-image-1.5 --output-format png --background transparent --prompt "simple red circle sticker on a transparent background" --json
eve infer image generate --model openai/gpt-image-2 --quality low --openai-moderation low --prompt "low-cost draft poster" --json
eve infer image generate --prompt "slow image backend" --timeout-ms 180000 --json
eve infer image edit --file ./logo.png --model openai/gpt-image-1.5 --output-format png --background transparent --prompt "keep the logo, remove the background" --json
eve infer image edit --file ./poster.png --prompt "make this a vertical story ad" --size 2160x3840 --aspect-ratio 9:16 --resolution 4K --json
eve infer image describe --file ./photo.jpg --json
eve infer image describe --file https://example.com/photo.png --json
eve infer image describe --file ./receipt.jpg --prompt "Extract the merchant, date, and total" --json
eve infer image describe-many --file ./before.png --file ./after.png --prompt "Compare the screenshots and list visible UI changes" --json
eve infer image describe --file ./ui-screenshot.png --model openai/gpt-5.4-mini --json
eve infer image describe --file ./photo.jpg --model ollama/qwen2.5vl:7b --prompt "Describe the image in one sentence" --timeout-ms 300000 --json
```
...
```bash
eve infer audio transcribe --file ./memo.m4a --json
eve infer audio transcribe --file ./team-sync.m4a --language en --prompt "Focus on names and action items" --json
eve infer audio transcribe --file ./memo.m4a --model openai/whisper-1 --json
```
...
```bash
eve infer tts convert --text "hello from eve" --output ./hello.mp3 --json
eve infer tts convert --text "Your build is complete" --output ./build-complete.mp3 --json
eve infer tts providers --json
eve infer tts status --json
```
...
```bash
eve infer video generate --prompt "cinematic sunset over the ocean" --json
eve infer video generate --prompt "slow drone shot over a forest lake" --resolution 768P --duration 6 --json
eve infer video describe --file ./clip.mp4 --json
eve infer video describe --file ./clip.mp4 --model openai/gpt-5.4-mini --json
```
...
```bash
eve infer web search --query "EVE docs" --json
eve infer web search --query "EVE infer web providers" --json
eve infer web fetch --url https://docs.eve.ai/cli/infer --json
eve infer web providers --json
```
...
```bash
eve infer embedding create --text "friendly lobster" --json
eve infer embedding create --text "customer support ticket: delayed shipment" --model openai/text-embedding-3-large --json
eve infer embedding providers --json
```
...
For generated media commands, `outputs` contains files written by EVE. Use
...
```bash
# Bad
eve infer media image generate --prompt "friendly lobster"

# Good
eve infer image generate --prompt "friendly lobster"
```

```bash
# Bad
eve infer audio transcribe --file ./memo.m4a --model whisper-1 --json
...
- `eve capability ...` is an alias for `eve infer ...`.
