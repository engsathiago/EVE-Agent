---
summary: "CLI reference for `eve secrets` (reload, audit, configure, apply)"
read_when:
...
Use `eve secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
eve secrets reload
eve secrets reload --json
eve secrets reload --url ws://127.0.0.1:18789 --token <token>
```
...
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `eve.json` refs)
...
```bash
eve secrets configure
eve secrets configure --plan-out /tmp/eve-secrets-plan.json
eve secrets configure --apply --yes
eve secrets configure --providers-only
eve secrets configure --skip-provider-setup
eve secrets configure --agent ops
eve secrets configure --json
```
...
- `configure` targets secret-bearing fields in `eve.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, EVE fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
eve secrets apply --from /tmp/eve-secrets-plan.json --json
```
...
- `eve.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.eve/.env` known secret keys whose values were migrated
