---
summary: "CLI reference for `eve devices` (device pairing + token rotation/revocation)"
read_when:
...
```
eve devices remove <deviceId>
eve devices remove <deviceId> --json
```

### `eve devices clear --yes [--pending]`

Clear paired devices in bulk.

```
eve devices clear --yes
eve devices clear --yes --pending
eve devices clear --yes --pending --json
```

### `eve devices approve [requestId] [--latest]`

Approve a pending device pairing request by exact `requestId`. If `requestId`
is omitted or `--latest` is passed, EVE only prints the selected pending
request and exits; rerun approval with the exact request ID after verifying
the details.

<Note>
If a device retries pairing with changed auth details (role, scopes, or public key), EVE supersedes the previous pending entry and issues a new `requestId`. Run `eve devices list` right before approval to use the current ID.
</Note>

If the device is already paired and asks for broader scopes or a broader role,
EVE keeps the existing approval in place and creates a new pending upgrade
request. Review the `Requested` vs `Approved` columns in `eve devices list`
or use `eve devices approve --latest` to preview the exact upgrade before
approving it.
...
eve devices approve --latest
```

## Paperclip / `eve_gateway` first-run approval

When a new Paperclip agent connects through the `eve_gateway` adapter for the first time, the Gateway may require a one-time device pairing approval before runs can succeed. If Paperclip reports `eve_gateway_pairing_required`, approve the pending device and retry.
...
The preview prints the exact `eve devices approve <requestId>` command. Verify the request details, then rerun that command with the request ID to approve it.

For remote gateways or explicit credentials, pass the same options while previewing and approving:

```bash
eve devices approve --latest --url <gateway-ws-url> --token <gateway-token>
```
...
If approval keeps failing, run `eve devices list` first to confirm a pending request exists.
...
### `eve devices rotate --device <id> --role <role> [--scope <scope...>]`
...
```
eve devices rotate --device <deviceId> --role operator --scope operator.read --scope operator.write
```
...
### `eve devices revoke --device <id> --role <role>`
...
```
eve devices revoke --device <deviceId> --role node
```
...
```bash
eve devices rotate --device <deviceId> --role operator
```
