---
summary: "CLI reference for `eve onboard` (interactive onboarding)"
read_when:
...
Full guided onboarding for local or remote Gateway setup. Use this when you want EVE to walk through model auth, workspace, gateway, channels, skills, and health in one flow.
...
<CardGroup cols={2}>
  ...
  <Card title="Onboarding overview" href="/start/onboarding-overview" icon="map">
    How EVE onboarding fits together.
  </Card>
...
eve onboard
eve onboard --modern
eve onboard --flow quickstart
eve onboard --flow manual
eve onboard --flow import
eve onboard --import-from hermes --import-source ~/.hermes
eve onboard --skip-bootstrap
eve onboard --mode remote --remote-url wss://gateway-host:18789
```

`--flow import` uses plugin-owned migration providers such as Hermes. It only runs against a fresh EVE setup; if existing config, credentials, sessions, or workspace memory/identity files are present, reset or choose a fresh setup before importing.

`--modern` starts the Crestodian conversational onboarding preview. Without
`--modern`, `eve onboard` keeps the classic onboarding flow.

On a fresh install where the active config file is missing or has no authored
settings (empty or metadata-only), bare `eve` also starts the classic
onboarding flow. Once a config file has authored settings, bare `eve`
...
`EVE_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
EVE marks common vision model IDs as image-capable automatically. Pass `--custom-image-input` for unknown custom vision IDs, or `--custom-text-input` to force text-only metadata.
...
```bash
export EVE_GATEWAY_TOKEN="your-token"
eve onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `eve gateway run`.
...
<AccordionGroup>
  ...
  <Accordion title="Other behaviors">
    ...
    - Fastest first chat: `eve dashboard` (Control UI, no channel setup).
...
Use `eve setup` instead when you only need the baseline config/workspace. Use `eve configure` later for targeted changes and `eve channels add` for channel-only setup.
