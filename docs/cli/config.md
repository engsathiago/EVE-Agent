---
summary: "CLI reference for `eve config` (get/set/patch/unset/file/schema/validate)"
read_when:
...
Config helpers for non-interactive edits in `eve.json`: get/set/patch/unset/file/schema/validate values by path and print the active config file. Run without a subcommand to open the configure wizard (same as `eve configure`).

<Note>
When `EVE_NIX_MODE=1`, EVE treats `eve.json` as immutable. Read-only commands such as `config get`, `config file`, `config schema`, and `config validate` still work, but config writers refuse. Agents should edit the Nix source for the install instead; for the first-party nix-eve distribution, use [nix-eve Quick Start](https://github.com/eve/nix-eve#quick-start) and set values under `programs.eve.config` or `instances.<name>.config`.
</Note>

## Root options

<ParamField path="--section <section>" type="string">
  Repeatable guided-setup section filter when you run `eve config` without a subcommand.
</ParamField>
...
```bash
eve config file
eve config --section model
eve config --section gateway --section daemon
eve config schema
eve config get browser.executablePath
...
eve config set agents.defaults.heartbeat.every "2h"
eve config set 'agents.list[0].tools.exec.node' "node-id-or-name"
eve config set agents.defaults.models '{"openai/gpt-5.4":{}}' --strict-json --merge
eve config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN
eve config set secrets.providers.vaultfile --provider-source file --provider-path /etc/eve/secrets.json --provider-mode json
eve config patch --file ./eve.patch.json5 --dry-run
eve config unset plugins.entries.brave.config.webSearch.apiKey
eve config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN --dry-run
eve config validate
eve config validate --json
```

### `config schema`

Print the generated JSON schema for `eve.json` to stdout as JSON.
...
```bash
eve config schema > eve.schema.json
```

### Paths

Paths use dot or bracket notation. Quote bracket-notation paths in shell examples so shells such as zsh do not expand `[0]` as a glob before EVE receives the path:

```bash
eve config get agents.defaults.workspace
eve config get 'agents.list[0].id'
```

Use the agent list index to target a specific agent:

```bash
eve config get agents.list
eve config set 'agents.list[1].tools.exec.node' "node-id-or-name"
```
...
```bash
eve config set agents.defaults.heartbeat.every "0m"
eve config set gateway.port 19001 --strict-json
eve config set channels.whatsapp.groups '["*"]' --strict-json
```
...
```bash
eve config set agents.defaults.models '{"openai/gpt-5.4":{}}' --strict-json --merge
eve config set models.providers.ollama.models '[{"id":"llama3.2","name":"Llama 3.2"}]' --strict-json --merge
```
...
`eve config set` supports four assignment styles:

<Tabs>
  <Tab title="Value mode">
    ```bash
    eve config set <path> <value>
    ```
  </Tab>
  <Tab title="SecretRef builder mode">
    ```bash
    eve config set channels.discord.token \
  ...
  <Tab title="Provider builder mode">
    ...
    ```bash
    eve config set secrets.providers.vault \
      --provider-source exec \
      --provider-command /usr/local/bin/eve-vault \
      --provider-arg read \
...
```bash
eve config patch --file ./eve.patch.json5 --dry-run
eve config patch --file ./eve.patch.json5
```

You can also pipe a patch over stdin, which is useful for remote setup scripts:

```bash
ssh eve-host 'eve config patch --stdin --dry-run' < ./eve.patch.json5
ssh eve-host 'eve config patch --stdin' < ./eve.patch.json5
```
...
```bash
eve config patch --file ./discord.patch.json5 --replace-path 'channels.discord.guilds["123"].channels'
```
...
eve config set secrets.providers.vaultfile \
  '{"source":"file","path":"/etc/eve/secrets.json","mode":"json"}' \
  --strict-json
```
...
Use `--dry-run` to validate changes without writing `eve.json`.
...
`eve config set` and other EVE-owned config writers validate the full post-change config before committing it to disk. If the new payload fails schema validation or looks like a destructive clobber, the active config is left alone and the rejected payload is saved beside it as `eve.json.rejected.*`.

<Warning>
The active config path must be a regular file. Symlinked `eve.json` layouts are unsupported for writes; use `EVE_CONFIG_PATH` to point directly at the real file instead.
</Warning>

Prefer CLI writes for small edits:

```bash
eve config set gateway.reload.mode hybrid --dry-run
eve config set gateway.reload.mode hybrid
eve config validate
```
...
Direct editor writes are still allowed, but the running Gateway treats them as untrusted until they validate. Invalid direct edits fail startup or are skipped by hot reload; Gateway does not rewrite `eve.json`. Run `eve doctor --fix` to repair prefixed/clobbered config or restore the last-known-good copy. See [Gateway troubleshooting](/gateway/troubleshooting#gateway-rejected-invalid-config).
...
- `config file`: Print the active config file path (resolved from `EVE_CONFIG_PATH` or default location). The path should name a regular file, not a symlink.
...
After `eve config validate` is passing, you can use the local TUI to have an embedded agent compare the active config against the docs while you validate each change from the same terminal:

<Note>
If validation is already failing, start with `eve configure` or `eve doctor --fix`. `eve chat` does not bypass the invalid-config guard.
</Note>
...
<Steps>
  ...
  <Step title="Apply targeted edits">
    Apply targeted edits with `eve config set` or `eve configure`.
  </Step>
  <Step title="Re-validate">
    Rerun `eve config validate` after each change.
  </Step>
  <Step title="Doctor for runtime issues">
    If validation passes but the runtime is still unhealthy, run `eve doctor` or `eve doctor --fix` for migration and repair help.
  </Step>
</Steps>
