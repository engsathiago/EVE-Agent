...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an EVE Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`eve acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to EVE channel
conversations instead of hosting an ACP harness session, use
[`eve mcp serve`](/cli/mcp) instead.
...
- EVE acts as an ACP server
- an IDE or ACP client connects to EVE
- EVE forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where EVE runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to EVE: use `eve acp`
- EVE should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Session lineage metadata                                              | Implemented | Session listings and session info snapshots include EVE parent and child lineage in `_meta` so ACP clients can render subagent graphs without private Gateway side channels.                                                                     |
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the EVE gateway or agent instead.                                                                                                                                          |
...
# Remote Gateway
eve acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
eve acp --url wss://gateway-host:18789 --token-file ~/.eve/gateway.token

# Attach to an existing session key
eve acp --session agent:main:main

# Attach by label (must already exist)
eve acp --session-label "support inbox"

# Reset the session key before the first prompt
eve acp --session agent:main:main --reset-session
```
...
eve acp client --server-args --url wss://gateway-host:18789 --token-file ~/.eve/gateway.token

# Override the server command (default: eve)
eve acp client --server "node" --server-args eve.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run EVE through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass "yolo" switch for that harness session.

## Protocol smoke testing

For protocol-level debugging, start a Gateway with isolated state and drive
`eve acp` over stdio with an ACP JSON-RPC client. Cover `initialize`,
...
Avoid using `eve gateway call sessions.list` as the only ACP proof. That
...
it to drive an EVE Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `eve acp` over stdio.

Example config (persisted):

```bash
eve config set gateway.remote.url wss://gateway-host:18789
eve config set gateway.remote.token <token>
```
...
```bash
eve acp --session agent:main:main
eve acp --session agent:design:main
eve acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see EVE plugin tools or selected
...
[EVE tools MCP bridge](/tools/acp-agents-setup#eve-tools-mcp-bridge).

## Use from `acpx` (Codex, Claude, other ACP clients)

If you want a coding agent such as Codex or Claude Code to talk to your
EVE bot over ACP, use `acpx` with its built-in `eve` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx eve` at `eve acp`.
3. Target the EVE session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default EVE ACP session
acpx eve exec "Summarize the active EVE session state."

# Persistent named session for follow-up turns
acpx eve sessions ensure --name codex-bridge
acpx eve -s codex-bridge --cwd /path/to/repo \
  "Ask my EVE work agent for recent context relevant to this repo."
```

If you want `acpx eve` to target a specific Gateway and session key every
time, override the `eve` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "eve": {
      "command": "env EVE_HIDE_BANNER=1 EVE_SUPPRESS_NOTES=1 eve acp --url ws://127.0.0.1:18789 --token-file ~/.eve/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local EVE checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env EVE_HIDE_BANNER=1 EVE_SUPPRESS_NOTES=1 node eve.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an EVE agent without scraping a terminal.
...
In Zed, open the Agent panel and select "EVE ACP" to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`EVE_GATEWAY_TOKEN`, `EVE_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`EVE_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `EVE_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `eve acp client` sets `EVE_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `eve`).
