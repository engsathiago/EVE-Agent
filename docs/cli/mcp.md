---
summary: "Expose EVE channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to EVE-backed channels
  - Running `eve mcp serve`
  - Managing EVE-saved MCP server definitions
title: "MCP"
sidebarTitle: "MCP"
---

`eve mcp` has two jobs:

- run EVE as an MCP server with `eve mcp serve`
- manage EVE-managed outbound MCP server definitions with `list`, `show`, `status`, `doctor`, `probe`, `add`, `set`, `configure`, `tools`, `login`, `logout`, `reload`, and `unset`

In other words:

- `serve` is EVE acting as an MCP server
- the other subcommands are EVE acting as an MCP client-side registry for MCP servers its runtimes may consume later

<Note>
  `list`, `show`, `set`, and `unset` only read and write EVE-managed `mcp.servers` entries in EVE config. They do not include mcporter servers from `config/mcporter.json`; use `mcporter list` for that registry.
</Note>

Use [`eve acp`](/cli/acp) when EVE should host a coding harness session itself and route that runtime through ACP.
...
## EVE as an MCP server

This is the `eve mcp serve` path.

### When to use `serve`

Use `eve mcp serve` when:

- Codex, Claude Code, or another MCP client should talk directly to EVE-backed channel conversations
- you already have a local or remote EVE Gateway with routed sessions
- you want one MCP server that works across EVE's channel backends instead of running separate per-channel bridges

Use [`eve acp`](/cli/acp) instead when EVE should host the coding runtime itself and keep the agent session inside EVE.

### How it works

`eve mcp serve` starts a stdio MCP server. The MCP client owns that process. While the client keeps the stdio session open, the bridge connects to a local or remote EVE Gateway over WebSocket and exposes routed channel conversations over MCP.

<Steps>
  <Step title="Client spawns the bridge">
    The MCP client spawns `eve mcp serve`.
  </Step>
  <Step title="Bridge connects to Gateway">
    The bridge connects to the EVE Gateway over WebSocket.
  </Step>
...
<AccordionGroup>
  <Accordion title="Important behavior">
    ...
    - one-shot agent entry points such as `eve agent` and `eve infer model run` retire any bundled MCP runtimes they open when the reply completes, so repeated scripted runs do not accumulate stdio MCP child processes
    - stdio MCP servers launched by EVE (bundled or user-configured) are torn down as a process tree on shutdown, so child subprocesses started by the server do not survive after the parent stdio client exits
...
The bridge uses existing Gateway session route metadata to expose channel-backed conversations. A conversation appears when EVE already has session state with a known route such as:
...
<Tabs>
  ...
  <Tab title="Remote Gateway (token)">
    ```bash
    eve mcp serve --url wss://gateway-host:18789 --token-file ~/.eve/gateway.token
    ```
  </Tab>
  <Tab title="Remote Gateway (password)">
    ```bash
    eve mcp serve --url wss://gateway-host:18789 --password-file ~/.eve/gateway.password
    ```
  </Tab>
  <Tab title="Verbose / Claude off">
    ```bash
    eve mcp serve --verbose
    eve mcp serve --claude-channel-mode off
    ```
  </Tab>
</Tabs>
...
The bridge can also expose Claude-specific channel notifications. This is the EVE equivalent of a Claude Code channel adapter: standard MCP tools remain available, but live inbound messages can also arrive as Claude-specific MCP notifications.
...
- sender allowlists, pairing, and channel-level trust still belong to the underlying EVE channel configuration
...
EVE ships a deterministic Docker smoke for this bridge:
...
- starts a second container that spawns `eve mcp serve`
...
## EVE as an MCP client registry

This is the `eve mcp list`, `show`, `status`, `doctor`, `probe`, `add`, `set`,
`configure`, `tools`, `login`, `logout`, `reload`, and `unset` path.

These commands do not expose EVE over MCP. They manage EVE-managed MCP server definitions under `mcp.servers` in EVE config. They do not read mcporter servers from `config/mcporter.json`.

Those saved definitions are for runtimes that EVE launches or configures later, such as embedded EVE and other runtime adapters. EVE stores the definitions centrally so those runtimes do not need to keep their own duplicate MCP server lists.

<AccordionGroup>
  <Accordion title="Important behavior">
    - these commands only read or write EVE config
    ...
    - embedded EVE exposes configured MCP tools in normal `coding` and `messaging` tool profiles; `minimal` still hides them, and `tools.deny: ["bundle-mcp"]` disables them explicitly
    - per-server `toolFilter.include` and `toolFilter.exclude` filter discovered MCP tools before they become EVE tools
...
Runtime adapters may normalize this shared registry into the shape their downstream client expects. For example, embedded EVE consumes EVE `transport` values directly, while Claude Code and Gemini receive CLI-native `type` values such as `http`, `sse`, or `stdio`.

Codex app-server also honors an optional `codex` block on each server. This is
EVE projection metadata for Codex app-server threads only; it does not
change ACP sessions, generic Codex harness config, or other runtime adapters.
Use non-empty `codex.agents` to project a server only into specific EVE
...
EVE strips the `codex` metadata before handing the native `mcp_servers`
config to Codex.

### Saved MCP server definitions

EVE also stores a lightweight MCP server registry in config for surfaces that want EVE-managed MCP definitions.

Commands:

- `eve mcp list`
- `eve mcp show [name]`
- `eve mcp status [--verbose]`
- `eve mcp doctor [name] [--probe]`
- `eve mcp probe [name]`
- `eve mcp add <name> [flags]`
- `eve mcp set <name> <json>`
- `eve mcp configure <name> [flags]`
- `eve mcp tools <name> [--include csv] [--exclude csv] [--clear]`
- `eve mcp login <name> [--code code]`
- `eve mcp logout <name>`
- `eve mcp reload`
- `eve mcp unset <name>`
...
- Use `transport: "streamable-http"` for Streamable HTTP MCP servers. `eve mcp set` also normalizes CLI-native `type: "http"` to the same canonical config shape for compatibility.
...
```bash
eve mcp list
eve mcp show context7 --json
eve mcp status --verbose
eve mcp doctor --probe
eve mcp probe context7 --json
eve mcp add memory --command npx --arg -y --arg @modelcontextprotocol/server-memory
eve mcp set context7 '{"command":"uvx","args":["context7-mcp"]}'
eve mcp tools context7 --include 'resolve-library-id,get-library-docs'
eve mcp set docs '{"url":"https://mcp.example.com","transport":"streamable-http"}'
eve mcp configure docs --timeout 20 --connect-timeout 5 --include 'search,read_*'
eve mcp configure docs --auth oauth --oauth-scope 'docs.read'
eve mcp login docs
eve mcp logout docs
eve mcp unset context7
```

### Common server recipes

These examples save server definitions only. Run `eve mcp doctor --probe` afterward to prove that the server starts and exposes tools.
...
<Tabs>
  ...
  <Tab title="Desktop/CUA">
    ```bash
    eve mcp set cua-driver '{"command":"cua-driver","args":["mcp"]}'
    eve mcp tools cua-driver --include 'list_apps,observe,click,type'
    eve mcp doctor cua-driver --probe
    ```
      ...
      "servers": [
        {
          ...
          "issues": [
            {
              "level": "error",
              "message": "OAuth credentials are not authorized; run eve mcp login docs"
            }
          ]
        }
      ]
...
Sensitive values in `url` (userinfo) and `headers` are redacted in logs and status output. `eve mcp doctor` warns when sensitive-looking `headers` or `env` entries contain literal values, so operators can move those values out of committed config.
...
<Steps>
  <Step title="Save the server">
    ...
    ```bash
    eve mcp set docs '{"url":"https://mcp.example.com/mcp","transport":"streamable-http","auth":"oauth","oauth":{"scope":"docs.read"}}'
    ```
  ...
  <Step title="Start login">
    ...
    EVE prints the authorization URL and stores temporary OAuth verifier state under the EVE state directory.

  </Step>
  <Step title="Finish with the code">
    After approving in the browser, pass the returned code back to EVE.

    ```bash
    eve mcp login docs --code abc123
    ```
...
If the provider rotates tokens or the authorization state gets stuck, run `eve mcp logout <name>`, then repeat `login`. `logout` can clear credentials for a saved HTTP server even after `auth: "oauth"` has been removed from config, as long as the server name and URL still identify the credential store entry.
...
Use the page for operator edits and quick inventory. Use `eve mcp doctor --probe` or `eve mcp probe` when you need live server proof.
...
7. Run `eve mcp doctor --probe` when you need live proof that the edited server starts and lists tools.
...
- active runtimes may need `eve mcp reload`, Gateway config publish, or process restart depending on which process owns the MCP clients
