---
summary: "CLI reference for `eve workboard` cards, dispatch, and worker runs"
read_when:
...
`eve workboard` is the terminal surface for the bundled
...
```bash
eve workboard list
eve workboard list --board default --status ready
eve workboard list --json
```
...
```bash
eve workboard show 7f4a2c10
eve workboard show 7f4a2c10 --json
```
...
eve workboard dispatch --json
eve workboard dispatch --url http://127.0.0.1:18789 --token "$EVE_GATEWAY_TOKEN"
```
...
Local `list`, `create`, and `show` commands operate on the local EVE state
directory used by the current profile. Use `--dev` or `--profile <name>` on the
top-level `eve` command when you need a different state root.
...
Then retry `eve workboard dispatch`. Data-only fallback is useful for local
