---
summary: "CLI reference for `eve message` (send + channel actions)"
read_when:
...
- `eve message` resolves the selected channel to its owning plugin when `--channel` or a channel-prefixed target is present; otherwise it loads configured channel plugins for default-channel inference.
...
- On cache miss, EVE will attempt a live directory lookup when the provider supports it.
...
- `eve message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, EVE does not force a `default` account SecretRef scope
...
```
eve message send --channel discord \
...
```bash
eve message send --channel googlechat --target spaces/AAA... \
  --message "Choose:" \
...
```
eve message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
eve message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
eve message poll --channel msteams \
...
```
eve message react --channel slack \
...
```
eve message react --channel signal \
...
```
eve message send --channel telegram --target @mychat --message "Choose:" \
...
```
eve message send --channel telegram --target 123456789 --message "Open app:" \
...
```bash
eve message send --channel telegram --target @mychat \
