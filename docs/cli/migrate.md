---
summary: "CLI reference for `eve migrate` (import state from another agent system)"
read_when:
  - You want to migrate from Hermes or another agent system into EVE
...
```bash
eve migrate list
eve migrate claude --dry-run
eve migrate codex --dry-run
eve migrate codex --skill gog-vault77-google-workspace
eve migrate codex --plugin google-calendar --dry-run
eve migrate codex --plugin google-calendar --verify-plugin-apps --dry-run
eve migrate hermes --dry-run
eve migrate hermes
eve migrate apply codex --yes --skill gog-vault77-google-workspace
eve migrate apply codex --yes --plugin google-calendar
eve migrate apply codex --yes
eve migrate apply claude --yes
eve migrate apply hermes --yes
eve migrate apply hermes --include-secrets --yes
eve onboard --flow import
eve onboard --import-from claude --import-source ~/.claude
eve onboard --import-from hermes --import-source ~/.hermes
```

<ParamField path="<provider>" type="string">
  Name of a registered migration provider, for example `hermes`. Run `eve migrate list` to see installed providers.
</ParamField>
...
<ParamField path="--no-backup" type="boolean">
  Skip the pre-apply backup. Requires `--force` when local EVE state exists.
</ParamField>
...
<AccordionGroup>
  <Accordion title="Preview before apply">
    ...
    `eve migrate apply <provider>` previews the plan and prompts before changing state unless `--yes` is set. In non-interactive mode, apply requires `--yes`.

  </Accordion>
  <Accordion title="Backups">
    Apply creates and verifies an EVE backup before applying the migration. If no local EVE state exists yet, the backup step is skipped and the migration can continue. To skip a backup when state exists, pass both `--no-backup` and `--force`.
  </Accordion>
...
- Project `CLAUDE.md` and `.claude/CLAUDE.md` into the EVE agent workspace.
...
- Claude command Markdown files converted into EVE skills with manual invocation only.

### Archive and manual-review state

Claude hooks, permissions, environment defaults, local memory, path-scoped rules, subagents, caches, plans, and project history are preserved in the migration report or reported as manual-review items. EVE does not execute hooks, copy broad allowlists, or import OAuth/Desktop credential state automatically.
...
Use this provider when moving to the EVE Codex harness and you want to
...
Running `eve migrate codex` in an interactive terminal previews the full
...
start checked unless the target EVE Codex plugin config already has that
...
```bash
eve migrate codex --dry-run --skill gog-vault77-google-workspace
eve migrate apply codex --yes --skill gog-vault77-google-workspace
```
...
```bash
eve migrate codex --dry-run --plugin google-calendar
eve migrate apply codex --yes --plugin google-calendar
```
...
- Personal AgentSkills under `$HOME/.agents/skills`, copied into the current
  EVE agent workspace when you want per-agent ownership.
- Source-installed `openai-curated` Codex plugins discovered through Codex
  ...
  native Codex harness; they are not exposed to EVE provider runs,
...
- `SOUL.md` and `AGENTS.md` into the EVE agent workspace.
- `memories/MEMORY.md` and `memories/USER.md` appended to workspace memory files.
- Memory config defaults for EVE file memory, plus archive or manual-review items for external memory providers such as Honcho.
...
Hermes state that EVE cannot safely interpret is copied into the migration report for manual review, but it is not loaded into live EVE config or credentials. This preserves opaque or unsafe state without pretending EVE can execute or trust it automatically:
...
Migration sources are plugins. A plugin declares its provider ids in `eve.plugin.json`:
...
Provider plugins can use `eve-agent/plugin-sdk/migration` for item construction and summary counts, plus `eve-agent/plugin-sdk/migration-runtime` for conflict-aware file copies, archive-only report copies, cached config-runtime wrappers, and migration reports.

## Onboarding integration

Onboarding can offer migration when a provider detects a known source. Both `eve onboard --flow import` and `eve setup --wizard --import-from hermes` use the same plugin migration provider and still show a preview before applying.

<Note>
Onboarding imports require a fresh EVE setup. Reset config, credentials, sessions, and the workspace first if you already have local state. Backup-plus-overwrite or merge imports are feature-gated for existing setups.
</Note>
...
- [Migrating](/install/migrating): move EVE to a new machine.
