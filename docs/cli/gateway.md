---
summary: "EVE Gateway CLI (`eve gateway`) — run, query, and discover gateways"
read_when:
...
The Gateway is EVE's WebSocket server (channels, nodes, sessions, hooks). Subcommands in this page live under `eve gateway …`.
...
<CardGroup cols={3}>
  ...
  <Card title="Discovery overview" href="/gateway/discovery">
    How EVE advertises and finds gateways.
  </Card>
...
<AccordionGroup>
  <Accordion title="Startup behavior">
    - By default, the Gateway refuses to start unless `gateway.mode=local` is set in `~/.eve/eve.json`. Use `--allow-unconfigured` for ad-hoc/dev runs.
    - `eve onboard --mode local` and `eve setup` are expected to write `gateway.mode=local`. If the file exists but `gateway.mode` is missing, treat that as a broken or clobbered config and repair it instead of assuming local mode implicitly.
...
<ParamField path="--token <token>" type="string">
  Token override (also sets `EVE_GATEWAY_TOKEN` for the process).
</ParamField>
...
<ParamField path="--bind custom + gateway.customBindHost" type="string">
  Expects an IPv4 address today. For IPv6-only BYOH, place an IPv4 sidecar or proxy in front of the Gateway and point EVE at that IPv4 endpoint.
</ParamField>
...
```bash
eve gateway restart
eve gateway restart --safe
eve gateway restart --safe --skip-deferral
eve gateway restart --force
```

`eve gateway restart --safe` asks the running Gateway to preflight active EVE work before restarting. If queued operations, reply delivery, embedded runs, or task runs are active, the Gateway reports the blockers, coalesces duplicate safe restart requests, and restarts once the active work drains. Plain `restart` keeps the existing service-manager behavior for compatibility. Use `--force` only when you explicitly want the immediate override path.

`eve gateway restart --safe --skip-deferral` runs the same EVE-aware coordinated restart as `--safe`, but bypasses the active-work deferral gate so the Gateway emits the restart immediately even when blockers are reported. Use it as the operator escape hatch when a deferral has been pinned by a stuck task run and `--safe` alone would wait indefinitely. `--skip-deferral` requires `--safe`.
...
- Set `EVE_GATEWAY_STARTUP_TRACE=1` to log phase timings during Gateway startup, including per-phase `eventLoopMax` delay and plugin lookup-table timings for installed-index, manifest registry, startup planning, and owner-map work.
- Set `EVE_GATEWAY_RESTART_TRACE=1` to log restart-scoped `restart trace:` lines for restart signal handling, active-work drain, shutdown phases, next start, ready timing, and memory metrics.
- Set `EVE_DIAGNOSTICS=timeline` with `EVE_DIAGNOSTICS_TIMELINE_PATH=<path>` to write a best-effort JSONL startup diagnostics timeline for external QA harnesses. You can also enable the flag with `diagnostics.flags: ["timeline"]` in config; the path is still env-provided. Add `EVE_DIAGNOSTICS_EVENT_LOOP=1` to include event-loop samples.
...
```bash
eve gateway health --url ws://127.0.0.1:18789
```
...
```bash
eve gateway usage-cost
eve gateway usage-cost --days 7
eve gateway usage-cost --json
```
...
```bash
eve gateway stability
eve gateway stability --type payload.large
eve gateway stability --bundle latest
eve gateway stability --bundle latest --export
eve gateway stability --json
```
...
<AccordionGroup>
  <Accordion title="Privacy and bundle behavior">
    ...
    - On fatal Gateway exits, shutdown timeouts, and restart startup failures, EVE writes the same diagnostic snapshot to `~/.eve/logs/stability/eve-stability-*.json` when the recorder has events. Inspect the newest bundle with `eve gateway stability --bundle latest`; `--limit`, `--type`, and `--since-seq` also apply to bundle output.
...
eve gateway diagnostics export --output eve-diagnostics.zip
eve gateway diagnostics export --json
```
...
eve gateway status --require-rpc
```
...
<AccordionGroup>
  ...
  <Accordion title="Common warning codes">
    ...
    - `multiple_gateways`: distinct gateway identities were reachable, or EVE could not prove reachable targets are the same gateway. An SSH tunnel, proxy URL, or configured remote URL to the same gateway does not trigger this warning.
...
```bash
eve gateway call status
eve gateway call logs.tail --params '{"sinceMs": 60000}'
```
...
responsible for eventually exec'ing `eve` or Node with those args.

```bash
cat > ~/.local/bin/eve-doppler <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
exec doppler run --project my-project --config production -- eve "$@"
EOF
chmod +x ~/.local/bin/eve-doppler

eve gateway install --wrapper ~/.local/bin/eve-doppler --force
eve gateway restart
```
...
`EVE_WRAPPER` in the service environment for later forced reinstalls, updates, and doctor
repairs.

```bash
EVE_WRAPPER="$HOME/.local/bin/eve-doppler" eve gateway install --force
eve doctor
```

To remove a persisted wrapper, clear `EVE_WRAPPER` while reinstalling:

```bash
EVE_WRAPPER= eve gateway install --force
eve gateway restart
```
...
<AccordionGroup>
  ...
  <Accordion title="Lifecycle behavior">
    ...
    - `gateway restart --safe` asks the running Gateway to preflight active EVE work and defer the restart until reply delivery, embedded runs, and task runs drain. `--safe` cannot be combined with `--force` or `--wait`.
    ...
    - `gateway restart --safe --skip-deferral` runs the EVE-aware safe restart but bypasses the deferral gate so the Gateway emits the restart immediately even when blockers are reported. Operator escape hatch for stuck-task-run deferrals; requires `--safe`.
  ...
  <Accordion title="Auth and SecretRefs at install time">
    ...
    - For password auth on `gateway run`, prefer `EVE_GATEWAY_PASSWORD`, `--password-file`, or a SecretRef-backed `gateway.auth.password` over inline `--password`.
    - In inferred auth mode, shell-only `EVE_GATEWAY_PASSWORD` does not relax install token requirements; use durable config (`gateway.auth.password` or config `env`) when installing a managed service.
...
`gateway discover` scans for Gateway beacons (`_eve-gw._tcp`).

- Multicast DNS-SD: `local.`
- Unicast DNS-SD (Wide-Area Bonjour): choose a domain (example: `eve.internal.`) and set up split DNS + a DNS server; see [Bonjour](/gateway/bonjour).
...
```bash
eve gateway discover --timeout 4000
eve gateway discover --json | jq '.beacons[].wsUrl'
```
