---
summary: "CLI reference for `eve path` (inspect and edit workspace files via the `oc://` addressing scheme)"
read_when:
...
```bash
eve plugins enable oc-path
```

## Why use it

EVE state is spread across human-edited markdown, commented JSONC config,
...
`eve path` gives those callers a stable address instead of a one-off grep,
...
You probably do not need `eve path` for ordinary whole-file edits, rich
...
```bash
eve path resolve 'oc://config.jsonc/plugins/github/enabled'
```

Preview a write without touching disk:

```bash
eve path set 'oc://config.jsonc/plugins/github/enabled' 'true' --dry-run
```

Find matching records in an append-only JSONL log:

```bash
eve path find 'oc://session.jsonl/[event=tool_call]/name'
```
...
```bash
eve path resolve 'oc://AGENTS.md/runtime-safety/eve-gateway'
```

Validate a path in CI or a preflight script before the script reads or writes:

```bash
eve path validate 'oc://AGENTS.md/tools/$last/risk'
```
...
# Read a leaf
eve path resolve 'oc://gateway.jsonc/version'

# Wildcard search
eve path find 'oc://session.jsonl/*/event' --file ./logs/session.jsonl

# Dry-run a write
eve path set 'oc://gateway.jsonc/version' '2.0' --dry-run

# Dry-run a write as a unified diff
eve path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff

# Apply the write
eve path set 'oc://gateway.jsonc/version' '2.0'

# Byte-fidelity round-trip (diagnostic)
eve path emit ./AGENTS.md
```
...
eve path resolve 'oc://config.jsonc/agents.defaults.models/"anthropic/claude-opus-4-7"/alias'

# Deep JSON/JSONC paths can use slash segments; they normalize to dotted subsegments
eve path set 'oc://eve.json/agents/list/0/tools/exec/security' 'allowlist' --dry-run

# Replace a JSONC leaf with a parsed object
eve path set 'oc://eve.json/gateway/auth/token' '{"source":"file","provider":"secrets","id":"/test"}' --value-json --dry-run

# Predicate search over JSONC children
eve path find 'oc://config.jsonc/plugins/[enabled=true]/id'

# Insert into a JSONC array
eve path set 'oc://config.jsonc/items/+1' '{"id":"new","enabled":true}' --dry-run

# Insert a JSONC object key
eve path set 'oc://config.jsonc/plugins/+github' '{"enabled":true}' --dry-run

# Append a JSONL event
eve path set 'oc://session.jsonl/+' '{"event":"checkpoint","ok":true}' --file ./logs/session.jsonl

# Resolve the last JSONL value line
eve path resolve 'oc://session.jsonl/$last/event' --file ./logs/session.jsonl

# Resolve a YAML workflow step
eve path resolve 'oc://workflow.yaml/steps/0/id'

# Update a YAML scalar
eve path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --dry-run

# Address markdown frontmatter
eve path resolve 'oc://AGENTS.md/[frontmatter]/name'

# Insert markdown frontmatter
eve path set 'oc://AGENTS.md/[frontmatter]/+description' 'Agent instructions' --dry-run

# Find markdown item fields
eve path find 'oc://SKILL.md/Tools/*/send_email'

# Validate a session-scoped path
eve path validate 'oc://AGENTS.md/Tools/$last/risk?session=cron-daily'
```
...
```bash
$ eve path resolve 'oc://x.md/[frontmatter]/tier' --file frontmatter.md --human
leaf @ L4: "core" (string)

$ eve path resolve 'oc://x.md/tools/gh/gh' --file frontmatter.md --human
leaf @ L9: "GitHub CLI" (string)

$ eve path find 'oc://x.md/tools/*' --file frontmatter.md --human
...
```bash
$ eve path resolve 'oc://config.jsonc/plugins/github/enabled' --file config.jsonc --human
leaf @ L4: "true" (boolean)

$ eve path set 'oc://config.jsonc/plugins/slack/enabled' 'true' --file config.jsonc --dry-run
...
```bash
$ eve path find 'oc://session.jsonl/[event=action]/userId' --file session.jsonl --human
1 match for oc://session.jsonl/[event=action]/userId:
  oc://session.jsonl/L2/userId  →  leaf @ L2: "u1" (string)

$ eve path resolve 'oc://session.jsonl/L2/ts' --file session.jsonl --human
leaf @ L2: "2" (number)
```
...
```bash
$ eve path resolve 'oc://workflow.yaml/steps/0/id' --file workflow.yaml --human
leaf @ L3: "fetch" (string)

$ eve path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --file workflow.yaml --dry-run
...
```bash
eve path resolve 'oc://AGENTS.md/tools/gh/risk' --human
eve path resolve 'oc://gateway.jsonc/server/port' --json
```
...
```bash
eve path find 'oc://AGENTS.md/tools/**/risk'
eve path find 'oc://session.jsonl/[event=action]/userId'
eve path find 'oc://config.jsonc/plugins/{github,slack}/enabled'
```
...
eve path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff
eve path set 'oc://gateway.jsonc/version' '2.0'
eve path set 'oc://AGENTS.md/Tools/+gh/risk' 'low'
```
...
```bash
$ eve path validate 'oc://AGENTS.md/tools/gh' --human
...
```bash
eve path emit ./AGENTS.md
eve path emit ./gateway.jsonc --json
```
...
`eve path` is TTY-aware: human-readable output on a terminal, JSON when
...
- `set` writes bytes through the substrate's emit path, which applies the
  ...
  `__EVE_REDACTED__` (verbatim or as a substring) is refused at write
  time.
