---
summary: "CLI reference for `eve voicecall` (voice-call plugin command surface)"
read_when:
...
eve voicecall smoke    [-t <phone>] [--message <text>] [--mode <m>] [--yes] [--json]
eve voicecall call     -m <text> [-t <phone>] [--mode <m>]
eve voicecall start    --to <phone> [--message <text>] [--mode <m>]
eve voicecall continue --call-id <id> --message <text>
eve voicecall speak    --call-id <id> --message <text>
eve voicecall dtmf     --call-id <id> --digits <digits>
eve voicecall end      --call-id <id>
eve voicecall status   [--call-id <id>] [--json]
eve voicecall tail     [--file <path>] [--since <n>] [--poll <ms>]
eve voicecall latency  [--file <path>] [--last <n>]
eve voicecall expose   [--mode <m>] [--path <p>] [--port <port>] [--serve-path <p>]
```
...
| `--message <text>` | `EVE voice call smoke test.` | Message to speak during the smoke call. |
...
```bash
eve voicecall smoke
eve voicecall smoke --to "+15555550123"        # dry run
eve voicecall smoke --to "+15555550123" --yes  # live notify call
```
...
```bash
eve voicecall call --to "+15555550123" --message "Hello"
eve voicecall call -m "Heads up" --mode notify
```
...
```bash
eve voicecall status
eve voicecall status --json
...
```bash
eve voicecall expose --mode serve
eve voicecall expose --mode funnel
eve voicecall expose --mode off
```
