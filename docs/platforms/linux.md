...
2. `npm i -g eve-agent@latest`
3. `eve onboard --install-daemon`
...
EVE installs a systemd **user** service by default. Use a **system**
service for shared or always-on servers. `eve gateway install` and
`eve onboard --install-daemon` already render the current canonical unit
...
Create `~/.config/systemd/user/eve-gateway[-<profile>].service`:

```
[Unit]
Description=EVE Gateway (profile: <profile>, v<version>)
...
sessions and channel connections. EVE therefore biases transient child
processes to be killed before the Gateway when possible.

For eligible Linux child spawns, EVE starts the child through a short
...
- EVE-launched browser/Chrome processes.

The wrapper is Linux-only and is skipped when `/bin/sh` is unavailable. It is
also skipped if the child env sets `EVE_CHILD_OOM_SCORE_ADJ=0`, `false`,
`no`, or `off`.
