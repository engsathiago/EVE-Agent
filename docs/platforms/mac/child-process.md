...
service via the external `eve` CLI (no embedded runtime). This gives you
...
- The app installs a per-user LaunchAgent labeled `ai.eve.gateway`
  (or `ai.eve.<profile>` when using `--profile`/`EVE_PROFILE`; legacy `com.eve.*` is supported).
