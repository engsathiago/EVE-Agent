...
EVE ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/eve`, or
  `$EVE_HOME/eve` when `EVE_HOME` is set; override with
  `EVE_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
eve update --tag 2026.4.1-beta.1
...
- The tag is not persisted. Your next `eve update` uses your configured
  channel as usual.
- For package installs, EVE pre-packs GitHub/git source specs into a
...
- Downgrade protection: if the target version is older than your current version,
  EVE prompts for confirmation (skip with `--yes`).
...
Preview what `eve update` would do without making changes:

```bash
eve update --dry-run
eve update --channel beta --dry-run
eve update --tag 2026.4.1-beta.1 --dry-run
eve update --dry-run --json
```
...
When you switch channels with `eve update`, EVE also syncs plugin
sources:
