---
summary: "Host EVE on a Raspberry Pi for always-on self-hosting"
read_when:
  - Setting up EVE on a Raspberry Pi
  - Running EVE on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

Run a persistent, always-on EVE Gateway on a Raspberry Pi. Since the Pi is just the gateway (models run in the cloud via API), even a modest Pi handles the workload well — typical hardware cost is **$35–80 one-time**, no monthly fees.
...
<Steps>
  ...
  <Step title="Verify">
    ```bash
    eve status
    systemctl --user status eve-gateway.service
    journalctl --user -u eve-gateway.service -f
    ```
  </Step>
  ...
  <Step title="Access the Control UI">
    ...
    ```bash
    ssh user@gateway-host 'eve dashboard --no-open'
    ```
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/eve-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/eve-compile-cache
mkdir -p /var/tmp/eve-compile-cache
export EVE_NO_RESPAWN=1
EOF
source ~/.bashrc
```

`EVE_NO_RESPAWN=1` keeps routine Gateway restarts in-process, which avoids extra process handoffs and keeps PID tracking simple on small hosts.
...
**systemd drop-in for stable restarts** -- If this Pi is mostly running EVE, add a service drop-in:

```bash
systemctl --user edit eve-gateway.service
```

```ini
[Service]
Environment=EVE_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/eve-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```

Then `systemctl --user daemon-reload && systemctl --user restart eve-gateway.service`. On a headless Pi, also enable lingering once so the user service survives logout: `sudo loginctl enable-linger "$(whoami)"`.
...
EVE state lives under:

- `~/.eve/` — `eve.json`, per-agent `auth-profiles.json`, channel/provider state, sessions.
- `~/.eve/workspace/` — agent workspace (SOUL.md, memory, artifacts).
...
**Service will not start** -- Check logs with `journalctl --user -u eve-gateway.service --no-pager -n 100` and run `eve doctor --non-interactive`. If this is a headless Pi, also verify lingering is enabled: `sudo loginctl enable-linger "$(whoami)"`.
...
- [Updating](/install/updating) -- keep EVE up to date
