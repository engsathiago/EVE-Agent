---
summary: "Host EVE on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up EVE on Oracle Cloud
  - Looking for free VPS hosting for EVE
  - Want 24/7 EVE on a small server
title: "Oracle Cloud"
---

Run a persistent EVE Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    sudo tailscale up --ssh --hostname=eve
    ```

    From now on, connect via Tailscale: `ssh ubuntu@eve`.
  ...
  <Step title="Configure the gateway">
    ...
    ```bash
    eve config set gateway.bind loopback
    eve config set gateway.auth.mode token
    eve doctor --generate-gateway-token
    eve config set gateway.tailscale.mode serve
    eve config set gateway.trustedProxies '["127.0.0.1"]'
...
- `chmod 700 ~/.eve` to restrict credential file permissions.
- `eve security audit` for an EVE-specific posture check.
...
The Always Free tier is ARM (`aarch64`). Most EVE features work fine; a small number of native binaries need ARM builds:
...
- `~/.eve/` — `eve.json`, per-agent `auth-profiles.json`, channel/provider state, and session data.
- `~/.eve/workspace/` — the agent workspace (SOUL.md, memory, artifacts).
...
```bash
ssh -L 18789:127.0.0.1:18789 ubuntu@eve
```
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=eve --reset` to re-authenticate.

**Gateway will not start** -- Run `eve doctor --non-interactive` and check logs with `journalctl --user -u eve-gateway.service -n 50`.
