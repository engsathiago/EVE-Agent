---
summary: "Automated, hardened EVE installation with Ansible, Tailscale VPN, and firewall isolation"
read_when:
...
Deploy EVE to production servers with **[eve-ansible](https://github.com/engsathiago/eve-agent-ansible)** -- an automated installer with security-first architecture.

<Info>
The [eve-ansible](https://github.com/engsathiago/eve-agent-ansible) repo is the source of truth for Ansible deployment. This page is a quick overview.
</Info>
...
```bash
curl -fsSL https://raw.githubusercontent.com/eve/eve-ansible/main/install.sh | bash
```
...
5. **EVE** -- host-based, not containerized
...
<Steps>
  <Step title="Switch to the eve user">
    ```bash
    sudo -i -u eve
    ```
  </Step>
  <Step title="Run the onboarding wizard">
    The post-install script guides you through configuring EVE settings.
  </Step>
  ...
  <Step title="Verify the installation">
    ```bash
    sudo systemctl status eve
    sudo journalctl -u eve -f
    ```
  </Step>
...
# Restart gateway
sudo systemctl restart eve

# Provider login (run as eve user)
sudo -i -u eve
eve channels login
```
...
<Steps>
  ...
  <Step title="Clone the repository">
    ```bash
    git clone https://github.com/engsathiago/eve-agent-ansible.git
    cd eve-ansible
    ```
  </Step>
...
The Ansible installer sets up EVE for manual updates. See [Updating](/install/updating) for the standard update flow.
...
<AccordionGroup>
  ...
  <Accordion title="Provider login fails">
    Make sure you are running as the `eve` user:
    ```bash
    sudo -i -u eve
    eve channels login
    ```
...
For detailed security architecture and troubleshooting, see the eve-ansible repo:

- [Security Architecture](https://github.com/engsathiago/eve-agent-ansible/blob/main/docs/security.md)
- [Technical Details](https://github.com/engsathiago/eve-agent-ansible/blob/main/docs/architecture.md)
- [Troubleshooting Guide](https://github.com/engsathiago/eve-agent-ansible/blob/main/docs/troubleshooting.md)

## Related

- [eve-ansible](https://github.com/engsathiago/eve-agent-ansible) -- full deployment guide
