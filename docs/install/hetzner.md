---
summary: "Run EVE Gateway 24/7 on a cheap Hetzner VPS (Docker) with durable state and baked-in binaries"
read_when:
  - You want EVE running 24/7 on a cloud VPS (not your laptop)
  - You want a production-grade, always-on Gateway on your own VPS
  - You want full control over persistence, binaries, and restart behavior
  - You are running EVE in Docker on Hetzner or a similar provider
title: "Hetzner"
---

## Goal

Run a persistent EVE Gateway on a Hetzner VPS using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "EVE 24/7 for ~$5", this is the simplest reliable setup.
...
<Steps>
  ...
  <Step title="Create persistent host directories">
    ...
    ```bash
    mkdir -p /root/.eve/workspace

    # Set ownership to the container user (uid 1000):
    chown -R 1000:1000 /root/.eve
    ```
  ...
  <Step title="Configure environment variables">
    ...
    EVE_CONFIG_DIR=/root/.eve
    EVE_WORKSPACE_DIR=/root/.eve/workspace
...
- Infrastructure: [eve-terraform-hetzner](https://github.com/andreesg/eve-terraform-hetzner)
- Docker config: [eve-docker-config](https://github.com/andreesg/eve-docker-config)
