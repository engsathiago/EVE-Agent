---
summary: "Run EVE Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want EVE running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

Run a persistent EVE Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "EVE 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the EVE Gateway in Docker
- Persist `~/.eve` + `~/.eve/workspace` on the host (survives restarts/rebuilds)
- Access the Control UI from your laptop via an SSH tunnel

That mounted `~/.eve` state includes `eve.json`, per-agent
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-eve-project --name="EVE Gateway"
    gcloud config set project my-eve-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create eve-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh eve-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Clone the EVE repository">
    ```bash
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    EVE_IMAGE=eve:latest
    EVE_GATEWAY_TOKEN=
    EVE_GATEWAY_BIND=lan
    EVE_GATEWAY_PORT=18789

    EVE_CONFIG_DIR=/home/$USER/.eve
    EVE_WORKSPACE_DIR=/home/$USER/.eve/workspace

    GOG_KEYRING_PASSWORD=
    XDG_CONFIG_HOME=/home/node/.eve
    ```

    Set `EVE_GATEWAY_TOKEN` when you want to manage the stable gateway
    token through `.env`; otherwise configure `gateway.auth.token` before
    relying on clients across restarts. If neither source exists, EVE uses
    ...
    **Do not commit this file.**
    ...
    This `.env` file is for container/runtime env such as `EVE_GATEWAY_TOKEN`.
    ...
    services:
      eve-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - EVE_GATEWAY_BIND=${EVE_GATEWAY_BIND}
          - EVE_GATEWAY_PORT=${EVE_GATEWAY_PORT}
        ...
        ports:
          ...
          - "127.0.0.1:${EVE_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`EVE_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm eve-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh eve-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop eve-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type eve-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start eve-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create eve-deploy \
     --display-name="EVE Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-eve-project \
     --member="serviceAccount:eve-deploy@my-eve-project.iam.gserviceaccount.com" \
