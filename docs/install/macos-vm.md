---
summary: "Run EVE in a sandboxed macOS VM (local or hosted) when you need isolation or iMessage"
read_when:
  - You want EVE isolated from your main macOS environment
...
Run EVE in a sandboxed macOS VM on your existing Apple Silicon Mac using [Lume](https://cua.ai/docs/lume).
...
1. Install Lume
2. `lume create eve --os macos --ipsw latest`
3. Complete Setup Assistant, enable Remote Login (SSH)
4. `lume run eve --no-display`
5. SSH in, install EVE, configure channels
6. Done
...
```bash
lume create eve --os macos --ipsw latest
```
...
```bash
lume stop eve
lume run eve --no-display
```

The VM runs in the background. EVE's daemon keeps the gateway running.

To check status:

```bash
ssh youruser@192.168.64.X "eve status"
```
...
This is the killer feature of running on macOS. Use [iMessage](/channels/imessage) with `imsg` to add Messages to EVE.

Inside the VM:

1. Sign in to Messages.
2. Install `imsg`.
3. Grant Full Disk Access and Automation permission for the process running EVE/`imsg`.
4. Verify RPC support with `imsg rpc --help`.

Add to your EVE config:
...
```bash
lume stop eve
lume clone eve eve-golden
```

Reset anytime:

```bash
lume stop eve && lume delete eve
lume clone eve-golden eve
lume run eve --no-display
```
