---
summary: "Host EVE on Upstash Box with keep-alive and SSH tunnel access"
read_when:
  - Deploying EVE to Upstash Box
  - You want a managed Linux environment for EVE with SSH-tunneled dashboard access
title: "Upstash Box"
---

Run a persistent EVE Gateway on Upstash Box, a managed Linux environment
...
Upstash maintains its current EVE Box walkthrough at
[EVE Setup](https://upstash.com/docs/box/guides/eve-setup).

## Connect with an SSH tunnel

Forward the EVE dashboard port to your local machine. Use your Box API key
...
```bash
eve config set gateway.bind lan
nohup eve gateway > gateway.log 2>&1 &
```
