...
read_when:
  - You want to understand `eve.ai/install.sh`
...
<Tabs>
  <Tab title="install.sh">
    ...
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://eve.ai/install.sh | bash -s -- --help
    ```

  </Tab>
  <Tab title="install-cli.sh">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://eve.ai/install-cli.sh | bash
    ```

    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://eve.ai/install-cli.sh | bash -s -- --help
    ```
  ...
  <Tab title="install.ps1">
    ...
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://eve.ai/install.ps1))) -Tag beta -NoOnboard -DryRun
    ```
...
<Note>
If install succeeds but `eve` is not found in a new terminal, see [Node.js troubleshooting](/install/node#troubleshooting).
</Note>
...
<Steps>
  ...
  <Step title="Ensure Node.js 24 by default">
    Checks Node version and installs Node 24 if needed (Homebrew on macOS, NodeSource setup scripts on Linux apt/dnf/yum). On macOS, Homebrew is installed only when the installer needs it for Node or Git. EVE still supports Node 22 LTS, currently `22.19+`, for compatibility.
  ...
  <Step title="Install EVE">
    ...
    - `git` method: clone/update repo, install deps with pnpm, build, then install wrapper at `~/.local/bin/eve`

  </Step>
  <Step title="Post-install tasks">
    - Refreshes a loaded gateway service best-effort (`eve gateway install --force`, then restart)
    - Runs `eve doctor --non-interactive` on upgrades and git installs (best effort)
...
If run inside an EVE checkout (`package.json` + `pnpm-workspace.yaml`), the script offers:
...
<Tabs>
  ...
  <Tab title="Dry run">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://eve.ai/install.sh | bash -s -- --dry-run
    ```
  </Tab>
</Tabs>
...
(default `~/.eve`) and no system Node dependency. Supports npm installs
...
<Steps>
  ...
  <Step title="Install EVE under prefix">
    - `npm` method (default): installs under the prefix with npm, then writes wrapper to `<prefix>/bin/eve`
    - `git` method: clones/updates a checkout (default `~/eve`) and still writes the wrapper to `<prefix>/bin/eve`

  </Step>
  <Step title="Refresh loaded gateway service">
    If a gateway service is already loaded from that same prefix, the script runs
    `eve gateway install --force`, then `eve gateway restart`, and
...
<Tabs>
  ...
  <Tab title="Custom prefix + version">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://eve.ai/install-cli.sh | bash -s -- --prefix /opt/eve --version latest
    ```
  </Tab>
  <Tab title="Git install">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://eve.ai/install-cli.sh | bash -s -- --install-method git --git-dir ~/eve
    ```
  </Tab>
  <Tab title="Automation JSON output">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://eve.ai/install-cli.sh | bash -s -- --json --prefix /opt/eve
    ```
  </Tab>
  <Tab title="Run onboarding">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://eve.ai/install-cli.sh | bash -s -- --onboard
    ```
  </Tab>
</Tabs>
...
<Steps>
  ...
  <Step title="Ensure Node.js 24 by default">
    If missing, attempts install via winget, then Chocolatey, then Scoop. If no package manager is available, the script downloads the official Node.js Windows zip into `%LOCALAPPDATA%\EVE\deps\portable-node` and adds it to the current process and user PATH. Node 22 LTS, currently `22.19+`, remains supported for compatibility.
  </Step>
  <Step title="Install EVE">
    ...
    - `git` method: clone/update repo, install/build with pnpm, and install wrapper at `%USERPROFILE%\.local\bin\eve.cmd`. If Git is missing, the script bootstraps user-local MinGit under `%LOCALAPPDATA%\EVE\deps\portable-git` and adds it to the current process and user PATH.
...
<Tabs>
  ...
  <Tab title="Git install">
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://eve.ai/install.ps1))) -InstallMethod git
    ```
  </Tab>
  <Tab title="GitHub main checkout">
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://eve.ai/install.ps1))) -InstallMethod git -Tag main
    ```
  </Tab>
  <Tab title="Custom git directory">
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://eve.ai/install.ps1))) -InstallMethod git -GitDir "C:\eve"
    ```
  </Tab>
  <Tab title="Dry run">
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://eve.ai/install.ps1))) -DryRun
    ```
  </Tab>
...
| `EVE_GIT_DIR=<path>`          | Checkout directory |
| `EVE_NO_ONBOARD=1`            | Skip onboarding    |
| `EVE_GIT_UPDATE=0`            | Disable git pull   |
| `EVE_DRY_RUN=1`               | Dry run mode       |
...
<Tabs>
  <Tab title="install.sh (non-interactive npm)">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://eve.ai/install.sh | bash -s -- --no-prompt --no-onboard
    ```
  </Tab>
  <Tab title="install.sh (non-interactive git)">
    ```bash
    EVE_INSTALL_METHOD=git EVE_NO_PROMPT=1 \
      curl -fsSL --proto '=https' --tlsv1.2 https://eve.ai/install.sh | bash
    ```
  </Tab>
...
<AccordionGroup>
  ...
  <Accordion title='Windows: "eve is not recognized"'>
  ...
  <Accordion title="eve not found after install">
