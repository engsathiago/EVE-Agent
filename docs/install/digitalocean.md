---
summary: "Host EVE on a DigitalOcean Droplet"
read_when:
  - Setting up EVE on DigitalOcean
  - Looking for a simple paid VPS for EVE
title: "DigitalOcean"
---

Run a persistent EVE Gateway on a DigitalOcean Droplet (~$6/month for the 1 GB Basic plan).
...
<Steps>
  ...
  <Step title="Connect and install">
    ...
    # Create the non-root user that will own EVE state and services.
    adduser eve
    usermod -aG sudo eve
    loginctl enable-linger eve

    su - eve
    eve --version
    ```

    Use the root shell only for system bootstrap. Run EVE commands as the non-root `eve` user so state lives under `/home/eve/.eve/` and the Gateway installs as that user's systemd service.
  ...
  <Step title="Access the Control UI">
    ...
    **Option C: Tailnet bind (no Serve)**
    ...
    ```bash
    eve config set gateway.bind tailnet
    eve gateway restart
    ```
...
DigitalOcean snapshots back the whole Droplet up; `eve backup create` is portable across hosts.
