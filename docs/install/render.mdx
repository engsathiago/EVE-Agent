---
summary: "Deploy EVE on Render with Infrastructure-as-Code"
read_when:
  - Deploying EVE to Render
...
Deploy EVE on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/engsathiago/eve-agent)
...
repository configures everything needed to run EVE:
...
services:
  - type: web
    ...
    envVars:
      - key: EVE_GATEWAY_PORT
        value: "8080"
      - key: EVE_STATE_DIR
        value: /data/.eve
      - key: EVE_WORKSPACE_DIR
        value: /data/workspace
      - key: EVE_GATEWAY_TOKEN
...
your fork's `render.yaml` (but note: no persistent disk means EVE state
...
`EVE_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
If you use the original EVE repository, Render will not auto-deploy your EVE. To update it, run a manual Blueprint sync from the dashboard.
...
For EVE, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
- Missing `EVE_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `EVE_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export a full backup via `eve backup create` in the Render shell.
