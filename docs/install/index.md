---
summary: "Install EVE - installer script, npm/pnpm/bun, from source, Docker, and more"
read_when:
...
The fastest way to install. It detects your OS, installs Node if needed, installs EVE, and launches onboarding.
...
<Tabs>
  <Tab title="macOS / Linux / WSL2">
    ```bash
    curl -fsSL https://eve.ai/install.sh | bash -s -- --no-onboard
    ```
  </Tab>
  <Tab title="Windows (PowerShell)">
    ```powershell
...
Use this when you want EVE and Node kept under a local prefix such as
`~/.eve`, without depending on a system-wide Node install:

```bash
curl -fsSL https://eve.ai/install-cli.sh | bash
```
...
`eve update --channel dev` and `eve update --channel stable`. See
...
<Tabs>
  <Tab title="npm">
    ...
    for the EVE package install. If you install manually with npm, your own
...
Or skip the link and use `pnpm eve ...` from inside the repo. See [Setup](/start/setup) for full development workflows.
...
```bash
eve --version      # confirm the CLI is available
eve doctor         # check for config issues
eve gateway status # verify the Gateway is running
```

If you want managed startup after install:

- macOS: LaunchAgent via `eve onboard --install-daemon` or `eve gateway install`
- Linux/WSL2: systemd user service via the same commands
- Native Windows: Scheduled Task first, with a per-user Startup-folder login item fallback if task creation is denied

## Hosting and deployment

Deploy EVE on a cloud server or VPS:
...
<CardGroup cols={3}>
  ...
  <Card title="Uninstall" href="/install/uninstall" icon="trash-2">
    Remove EVE completely.
  </Card>
</CardGroup>

## Troubleshooting: `eve` not found

If the install succeeded but `eve` is not found in your terminal:
