---
summary: "Install EVE declaratively with Nix"
read_when:
...
Install EVE declaratively with **[nix-eve](https://github.com/eve/nix-eve)** - the first-party, batteries-included Home Manager module.

<Info>
The [nix-eve](https://github.com/eve/nix-eve) repo is the source of truth for Nix installation. This page is a quick overview.
</Info>
...
<Steps>
  ...
  <Step title="Create a local flake">
    Use the agent-first template from the nix-eve repo:
    ```bash
    mkdir -p ~/code/eve-local
    # Copy templates/agent-first/flake.nix from the nix-eve repo
    ```
  </Step>
...
See the [nix-eve README](https://github.com/eve/nix-eve) for full module options and examples.

## Nix-mode runtime behavior

When `EVE_NIX_MODE=1` is set (automatic with nix-eve), EVE enters a deterministic mode for Nix-managed installs. Other Nix packages can set the same mode; nix-eve is the first-party reference.

You can also set it manually:

```bash
export EVE_NIX_MODE=1
```

On macOS, the GUI app does not automatically inherit shell environment variables. Enable Nix mode via defaults instead:

```bash
defaults write ai.eve.mac eve.nixMode -bool true
```

### What changes in Nix mode

- Auto-install and self-mutation flows are disabled
- `eve.json` is treated as immutable. Startup-derived defaults stay runtime-only, and config writers such as setup, onboarding, mutating `eve update`, plugin install/update/uninstall/enable, `doctor --fix`, `doctor --generate-gateway-token`, and `eve config set` refuse to edit the file.
- Agents should edit the Nix source instead. For nix-eve, use the agent-first [Quick Start](https://github.com/eve/nix-eve#quick-start) and set config under `programs.eve.config` or `instances.<name>.config`.
...
<CardGroup cols={2}>
  <Card title="nix-eve" href="https://github.com/eve/nix-eve" icon="arrow-up-right-from-square">
