---
summary: "Developer workflow for EVE agent runtime: build, test, and live validation"
title: "EVE agent runtime workflow"
read_when:
  - Working on EVE agent runtime code or tests
  - Running agent-runtime lint, typecheck, and live test flows
---

A sane workflow for working on the EVE agent runtime in EVE.
...
```bash
EVE_LIVE_TEST=1 pnpm test src/agents/embedded-agent-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm eve agent --message "Hello" --thinking low`
...
State lives under the EVE state directory. Default is `~/.eve`. If `EVE_STATE_DIR` is set, use that directory instead.
...
- [EVE agent runtime architecture](/agent-runtime-architecture)
