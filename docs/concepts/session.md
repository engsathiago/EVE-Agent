---
summary: "How EVE manages conversation sessions"
read_when:
...
EVE organizes conversations into **sessions**. Each message is routed to a
...
Verify your setup with `eve security audit`.
...
EVE automatically bounds session storage over time. By default, it runs
...
`eve sessions cleanup --dry-run --fix-dm-scope`. Applying the same flag
retires those old direct-DM rows and keeps their transcripts as deleted
archives.

Preview with `eve sessions cleanup --dry-run`.

## Inspecting sessions

- `eve status` -- session store path and recent activity.
- `eve sessions --json` -- all sessions (filter with `--active <minutes>`).
