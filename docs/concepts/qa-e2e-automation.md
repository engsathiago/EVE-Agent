...
The private QA stack is meant to exercise EVE in a more realistic,
...
Every QA flow runs under `pnpm eve qa <subcommand>`. Many have `pnpm qa:*`
...
| `qa suite`                                          | Run repo-backed scenarios against the QA gateway lane. Aliases: `pnpm eve qa suite --runner multipass` for a disposable Linux VM.                                                                                                                                       |
| `qa coverage`                                       | Print the YAML scenario-coverage inventory (`--json` for machine output).                                                                                                                                                                                               |
| `qa parity-report`                                  | Compare two `qa-suite-summary.json` files and write the agentic parity report, or use `--runtime-axis --token-efficiency` to write Codex-vs-EVE runtime parity and token-efficiency reports from one runtime-pair summary.                                              |
...
Stable/LTS proof lane. When a command also needs an EVE root profile, put
the root profile before the QA command:

```bash
pnpm eve --profile work qa run --qa-profile smoke-ci
```
...
```bash
pnpm eve qa docker-build-image
pnpm qa:lab:build
pnpm qa:lab:up:fast
pnpm qa:lab:watch
```
...
`eve.run`, `eve.harness.run`, a latest GenAI semantic-convention
model-call span, `eve.context.assembled`, and `eve.message.delivery`
...
`eve.content.*` attributes must stay out of the trace. The raw OTLP
...
```bash
pnpm eve qa matrix --profile fast --fail-fast
```
...
CI uses the same command surface in `.github/workflows/qa-live-transports-convex.yml`. Scheduled and default manual runs execute the fast Matrix profile with live frontier credentials, `--fast`, and `EVE_QA_MATRIX_NO_REPLY_WINDOW_MS=3000`. Manual `matrix_profile=all` fans out into the five profile shards so the exhaustive catalog can run in parallel while keeping one artifact directory per shard.
...
```bash
pnpm eve qa mantis slack-desktop-smoke \
  --gateway-setup \
...
With `--gateway-setup`, Mantis leaves a persistent EVE Slack gateway
...
```bash
pnpm eve qa mantis visual-task \
...
`visual-driver`, captures `visual-task.png`, runs `eve infer image describe`
...
`eve-agent/plugin-sdk/qa-live-transport-scenarios`.
...
```bash
pnpm eve qa suite --runner multipass --scenario channel-chat-baseline
```

This boots a fresh Multipass guest, installs dependencies, builds EVE
...
- `EVE_QA_TELEGRAM_GROUP_ID` - numeric chat id (string).
...
The implicit default set always covers canary, mention gating, native command replies, command addressing, and bot-to-bot group replies. `mock-openai` defaults also include deterministic reply-chain and final-message streaming checks. `telegram-current-session-status-tool` remains opt-in because it is only stable when threaded directly after canary, not after arbitrary native command replies. Use `pnpm eve qa telegram --list-scenarios --provider-mode mock-openai` to print the current default/optional split with regression refs.
...
```bash
EVE_QA_CREDENTIAL_SOURCE=convex \
pnpm test:docker:npm-telegram-live
```

When `EVE_QA_CREDENTIAL_SOURCE=convex` is set, the package live wrapper
...
`EVE_NPM_TELEGRAM_RTT_SAMPLES`, `EVE_NPM_TELEGRAM_RTT_TIMEOUT_MS`,
or `EVE_NPM_TELEGRAM_RTT_MAX_FAILURES` to tune RTT measurement without
...
- `EVE_QA_DISCORD_SUT_APPLICATION_ID` - must match the SUT bot user id returned by Discord (the lane fails fast otherwise).

Optional:

- `EVE_QA_DISCORD_CAPTURE_CONTENT=1` keeps message bodies in observed-message artifacts.
- `EVE_QA_DISCORD_VOICE_CHANNEL_ID` selects the voice/stage channel for `discord-voice-autojoin`; without it, the scenario picks the first visible voice/stage channel for the SUT bot.
...
- `discord-qa-observed-messages.json` - bodies redacted unless `EVE_QA_DISCORD_CAPTURE_CONTENT=1`.
...
Targets one real private Slack channel with two distinct bots: a driver bot controlled by the harness and a SUT bot started by the child EVE gateway through the bundled Slack plugin.
...
- `EVE_QA_SLACK_CAPTURE_CONTENT=1` keeps message bodies in observed-message artifacts.
- `EVE_QA_SLACK_APPROVAL_CHECKPOINT_DIR` enables visual approval
...
- `EVE_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS` overrides the checkpoint
...
- `slack-qa-report.md`
- `qa-evidence.json` - evidence entries for the live transport checks.
- `slack-qa-observed-messages.json` - bodies redacted unless `EVE_QA_SLACK_CAPTURE_CONTENT=1`.
- `approval-checkpoints/` - only when Mantis sets
  `EVE_QA_SLACK_APPROVAL_CHECKPOINT_DIR`; contains checkpoint JSON,
...
```json
{
  "display_information": {
    "name": "EVE QA Driver",
    "description": "Test driver bot for EVE QA Slack live lane"
  },
  "features": {
    "bot_user": {
      "display_name": "EVE QA Driver",
      "always_online": true
    }
  },
  "oauth_config": {
...
```json
{
  "display_information": {
    "name": "EVE QA SUT",
    "description": "EVE QA SUT connector for EVE"
  },
  "features": {
    "bot_user": {
      "display_name": "EVE QA SUT",
      "always_online": true
    },
    "app_home": {
...
In the QA workspace, create a channel (e.g. `#eve-qa`) and invite both bots from inside the channel:

```
/invite @EVE QA Driver
/invite @EVE QA SUT
```
...
Two options. Use env vars for single-machine debugging (set the four `EVE_QA_SLACK_*` variables and pass `--credential-source env`), or seed the shared Convex pool so CI and other maintainers can lease them.
...
With `EVE_QA_CONVEX_SITE_URL` and `EVE_QA_CONVEX_SECRET_MAINTAINER` exported in your shell, register and verify:

```bash
pnpm eve qa credentials add \
  --kind slack \
  --payload-file slack-creds.json \
  --note "QA Slack pool seed"

pnpm eve qa credentials list --kind slack --status all --json
```
...
the harness and a SUT account started by the child EVE gateway through the
...
- `EVE_QA_WHATSAPP_GROUP_JID` enables group scenarios such as
  `whatsapp-mention-gating` and `whatsapp-group-allowlist-block`.
- `EVE_QA_WHATSAPP_CAPTURE_CONTENT=1` keeps message bodies in
...
`@eve/whatsapp/api.js` package surface instead of reaching into private
...
- `qa-evidence.json` - evidence entries for the live transport checks.
- `whatsapp-qa-observed-messages.json` - bodies redacted unless `EVE_QA_WHATSAPP_CAPTURE_CONTENT=1`.

### Convex credential pool

Telegram, Discord, Slack, and WhatsApp lanes can lease credentials from a shared Convex pool instead of reading the env vars above. Pass `--credential-source convex` (or set `EVE_QA_CREDENTIAL_SOURCE=convex`); QA Lab acquires an exclusive lease, heartbeats it for the duration of the run, and releases it on shutdown. Pool kinds are `"telegram"`, `"discord"`, `"slack"`, and `"whatsapp"`.
...
- `mock-openai` is the scenario-aware EVE mock. It remains the default
...
- how `eve qa <runner>` is mounted beneath the shared `qa` root
...
4. Mount the runner as `eve qa <runner>` instead of registering a competing root command. Runner plugins should declare `qaRunners` in `eve.plugin.json` and export a matching `qaRunnerCliRegistrations` array from `runtime-api.ts`. Keep `runtime-api.ts` light; lazy CLI and runner execution should stay behind separate entrypoints.
...
For the inventory of available scenarios - useful when sizing follow-up work or wiring a new transport - run `pnpm eve qa coverage` (add `--json` for machine-readable output).
When choosing focused proof for a touched behavior or file path, run `pnpm eve qa coverage --match <query>`.
