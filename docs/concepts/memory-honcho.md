...
[Honcho](https://honcho.dev) adds AI-native memory to EVE. It persists
...
```bash
eve plugins install @honcho-ai/eve-honcho
eve honcho setup
eve gateway --force
```
...
Settings live under `plugins.entries["eve-honcho"].config`:
    ...
    entries: {
      "eve-honcho": {
        config: {
          ...
          workspaceId: "eve", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `eve honcho setup` detects and
...
```bash
eve honcho setup                        # Configure API key and migrate files
eve honcho status                       # Check connection status
eve honcho ask <question>               # Query Honcho about the user
eve honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/eve-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho EVE integration guide](https://docs.honcho.dev/v3/guides/integrations/eve)
- [Memory](/concepts/memory) -- EVE memory overview
