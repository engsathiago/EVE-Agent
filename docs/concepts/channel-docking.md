---
summary: "Move one EVE session's reply route between linked chat channels"
...
Channel docking is call forwarding for one EVE session.
...
Alice can message EVE on Telegram and Discord:
...
EVE keeps the current session context and changes the reply route:
...
4. Keep the same EVE session, but receive future replies in Discord.
