---
summary: "What the EVE system prompt contains and how it is assembled"
read_when:
...
EVE builds a custom system prompt for every agent run. The prompt is **EVE-owned** and does not use a runtime default prompt.

The prompt is assembled by EVE and injected into each agent run.
...
the full EVE-owned prompt. The provider runtime can:
...
- **EVE Control**: tells the model to prefer the `gateway` tool for
  config/restart work and to avoid inventing CLI commands.
- **EVE Self-Update**: how to inspect config safely with
...
- **Documentation**: local path to EVE docs/source and when to read them.
...
EVE keeps large stable content, including **Project Context**, above the
...
EVE can render smaller system prompts for sub-agents. The runtime sets a
`promptMode` for each run (not a user-facing config):

- `full` (default): includes all sections above.
- `minimal`: used for sub-agents; omits **Memory Recall**, **EVE
...
For channel auto-reply runs, EVE omits the generic **Silent Replies**
...
EVE keeps committed prompt snapshots for the Codex runtime happy path under
...
EVE developer instructions, turn-scoped collaboration-mode instructions
when EVE provides them, user turn input, and references to the dynamic tool
specs.
...
collaboration-mode instructions inside the Codex runtime after EVE sends
...
On the native Codex harness, EVE avoids repeating stable workspace files
...
Codex developer instructions. The compact EVE skills list is also forwarded
...
EVE prompt according to their existing gates. `HEARTBEAT.md` is omitted on
...
occurs, EVE can inject a concise system-prompt warning notice; control this with
...
When eligible skills exist, EVE injects a compact **available skills list**
...
points to the local EVE docs directory (`docs/` in a Git checkout or the bundled npm
package docs). If local docs are unavailable, it falls back to
[https://docs.eve.ai](https://docs.eve.ai).

The same section also includes the EVE source location. Git checkouts expose the local
...
authority for EVE self-knowledge before the model understands how EVE works,
...
notes, and `memory_search` as instruction context or user memory rather than EVE
design or implementation knowledge. If docs are silent or stale, the model should say so
and inspect source. The prompt also tells the model to run `eve status` itself when
