...
Linux desktop, VNC rescue, Slack Web, a real EVE gateway, screenshots,
...
- Mantis artifacts: owned by the EVE run. They live under
...
- remote logs such as `slack-desktop-command.log`, `eve-gateway.log`,
...
- `crabbox.remote_run`: sync, browser launch, EVE install/build or
...
remote status after Mantis has copied metadata proving that either the EVE
