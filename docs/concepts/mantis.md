---
summary: "Mantis is the visual end-to-end verification system for reproducing EVE bugs on live transports, capturing before and after evidence, and attaching artifacts to PRs."
title: "Mantis"
read_when:
  - Building or running live visual QA for EVE bugs
...
Mantis is the EVE end-to-end verification system for bugs that need a real
...
Mantis lives in the EVE QA stack.

- EVE owns the scenario runtime, transport adapters, evidence schema, and
  local CLI under `pnpm eve qa mantis`.
...
- EVE agents drive Mantis through Codex when a scenario needs agentic setup,
  debugging, or stuck-state reporting.

This boundary keeps transport knowledge in EVE, machine scheduling in
...
```bash
pnpm eve qa mantis discord-smoke \
...
thread, calls EVE's `message.thread-reply` action with a repo-local
...
```bash
pnpm eve qa mantis desktop-browser-smoke \
...
`EVE_MANTIS_CRABBOX_PROVIDER` when running against another Crabbox fleet.

Useful desktop smoke flags:

- `--lease-id <cbx_...>` or `EVE_MANTIS_CRABBOX_LEASE_ID` reuses a warmed desktop.
...
- `--browser-profile-archive-env <name>` restores a base64 `.tgz` Chrome user-data-dir archive from the named environment variable before launching the browser. Use this for logged-in witnesses such as Discord Web. The default env var is `EVE_MANTIS_BROWSER_PROFILE_TGZ_B64`.
...
- `--keep-lease` or `EVE_MANTIS_KEEP_VM=1` keeps a newly created passing lease open for VNC inspection. Failed runs keep the lease by default when one was created so an operator can reconnect.
...
REST. When `EVE_QA_DISCORD_CAPTURE_UI_METADATA=1` is set, the scenario also
writes a Discord Web URL artifact. When `EVE_QA_DISCORD_KEEP_THREADS=1` is
...
the VM, runs `pnpm eve qa slack` inside that VM, opens Slack Web in the VNC
...
shape where the SUT EVE gateway and the browser both live inside the same
Linux desktop VM.

With `--gateway-setup`, the command prepares a persistent disposable EVE
home at `$HOME/.eve-mantis/slack-eve`, patches Slack Socket Mode
configuration for the selected channel, starts `eve gateway run` on port
...
- `EVE_LIVE_OPENAI_KEY` for the remote model lane. If only
  `OPENAI_API_KEY` is set locally, Mantis maps it to `EVE_LIVE_OPENAI_KEY`
  before invoking Crabbox so Crabbox's `EVE_*` env forwarding can carry it
  into the VM.
...
channel id, Socket Mode app token, and bot token as the `EVE_MANTIS_SLACK_*`
...
- `--gateway-setup` starts a persistent EVE Slack gateway in the VM instead of only running the bot-to-bot QA lane.
...
- `EVE_MANTIS_SLACK_BROWSER_PROFILE_DIR` controls the persistent Chrome profile inside the VM. The default is `$HOME/.config/eve-mantis/slack-chrome-profile`, so a manual Slack Web login survives reruns on the same lease.
...
Crabbox CLI from
`eve/crabbox` main so it can use the current desktop/browser lease flags
...
runs `pnpm eve qa mantis slack-desktop-smoke --gateway-setup` against that
...
worktree, runs `pnpm eve qa telegram --credential-source convex
...
`@eve-mantis telegram desktop proof`, from the Actions UI with freeform
...
EVE with the leased Telegram SUT bot token, starts `eve gateway run`
...
token never logs Telegram Desktop in; it only configures EVE. The desktop
...
- `--no-gateway-setup` installs and opens Telegram Desktop without configuring EVE.
...
comment or creates one. The workflows publish to `eve-crabbox-artifacts`
with public URLs under `https://artifacts.eve.ai`. They provide bucket,
...
```text
@eve-mantis discord status reactions
```
...
```text
@eve-mantis discord status reactions baseline=origin/main candidate=HEAD
```
...
6. Start a child EVE Gateway with an isolated state directory.
...
- It exercises a real EVE Gateway, Discord bot auth, message dispatch,
...
- `pnpm eve qa discord` already runs a live Discord lane with driver and
  SUT bots.
...
- **Headless automation**: default for CI. Chrome runs with CDP enabled, and
  Playwright or EVE browser control captures screenshots.
...
- Node 22 and pnpm
- EVE checkout and dependency cache
- Playwright Chromium browser cache when Playwright is used
- enough CPU and memory for one EVE Gateway, one browser, and one model run
...
- `EVE_QA_DISCORD_MANTIS_BOT_TOKEN`
- `EVE_QA_DISCORD_DRIVER_BOT_TOKEN`
- `EVE_QA_DISCORD_SUT_BOT_TOKEN`
- `EVE_QA_DISCORD_GUILD_ID`
- `EVE_QA_DISCORD_CHANNEL_ID`
- `EVE_QA_DISCORD_NOTIFY_CHANNEL_ID`
- `EVE_QA_REDACT_PUBLIC_METADATA=1` for public GitHub artifact uploads
- `EVE_QA_CONVEX_SITE_URL`
- `EVE_QA_CONVEX_SECRET_CI`
- `EVE_QA_MANTIS_CRABBOX_COORDINATOR`
- `EVE_QA_MANTIS_CRABBOX_COORDINATOR_TOKEN`
...
`EVE_QA_REDACT_PUBLIC_METADATA=1` for this reason.
