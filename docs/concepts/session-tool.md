...
EVE gives agents tools to work across sessions, inspect status, and
...
After the target responds, EVE can run a **reply-back loop** where the
...
the caller's current session; visible client labels such as `eve-tui` are
not session keys.
...
- `deliveryContext` is the persisted delivery route stored on the session,
  which EVE can reuse for later delivery even when the active surface
  differs.
...
`subagents` is the visibility helper for already spawned EVE
...
the completion origin only identifies a channel EVE can still reuse the
