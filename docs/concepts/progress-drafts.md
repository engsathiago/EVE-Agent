...
read_when:
  ...
  - Explaining how EVE updates one channel message while work is in progress
...
When progress drafts are enabled, EVE creates one visible work-in-progress
...
That is usually enough. EVE will pick an automatic one-word label, wait
...
EVE sends the final answer normally and cleans up or stops updating the
...
The default label is `auto`, which chooses from EVE's built-in
...
EVE renders only the `progress.text` in the channel progress UI. The
...
EVE uses the same formatter for progress drafts and `/verbose`:
...
EVE truncates long progress lines by default so repeated draft edits do not
...
With `toolProgress: false`, EVE still suppresses the older standalone
...
When the final answer is ready, EVE tries to keep the chat clean:

- If the draft can safely become the final answer, EVE edits it in place.
- If the channel uses native progress streaming, EVE finalizes that stream
...
- If the final answer has media, an approval prompt, an explicit reply target,
  too many chunks, or a failed edit/send, EVE sends the final answer through
...
Check `streaming.progress.toolProgress`. If it is `false`, EVE keeps the
