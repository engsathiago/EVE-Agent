...
read_when:
  - You want EVE to remember natural follow-ups
  - You want to understand how inferred check-ins differ from reminders
  - You want to review or dismiss follow-up commitments
---

Commitments are short-lived follow-up memories. When enabled, EVE can
notice that a conversation created a future check-in opportunity and remember
to bring it back later.

Examples:

- You mention an interview tomorrow. EVE may check in afterward.
- You say you are exhausted. EVE may ask later whether you slept.
- The agent says it will follow up after something changes. EVE may track
  that open loop.

Commitments are not durable facts like `MEMORY.md`, and they are not exact
reminders. They sit between memory and automation: EVE remembers a
...
```bash
eve config set commitments.enabled true
eve config set commitments.maxPerDay 3
```
...
After an agent reply, EVE may run a hidden background extraction pass in a
...
When it finds a high-confidence candidate, EVE stores a commitment with:
...
EVE never delivers an inferred commitment immediately after writing it.
...
eve commitments
eve commitments --all
eve commitments --agent main
eve commitments --status snoozed
eve commitments dismiss cm_abc123
```

See [`eve commitments`](/cli/commitments) for the command reference.
...
Stored commitments are local EVE state. They are operational memory, not
long-term memory. Disable the feature with:

```bash
eve config set commitments.enabled false
```
...
- Check `eve commitments --all` for pending, dismissed, snoozed, or expired
  records.
...
- [`eve commitments`](/cli/commitments)
