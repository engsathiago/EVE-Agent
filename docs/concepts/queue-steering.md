...
When a normal prompt arrives while a session run is already streaming, EVE
...
behavior. EVE and the native Codex app-server harness implement the delivery
details differently.

## Runtime boundary

Steering does not interrupt a tool call that is already running. EVE checks for
queued steering messages at model boundaries:

1. The assistant asks for tool calls.
2. EVE executes the current assistant message's tool-call batch.
3. EVE emits the turn end event.
4. EVE drains queued steering messages.
5. EVE appends those messages as user messages before the next LLM call.
...
The native Codex app-server harness exposes `turn/steer` instead of EVE runtime's
internal steering queue. EVE batches queued prompts for the configured
...
runtime cannot accept steering in `steer` mode, EVE waits for the active
...
- With default behavior, the active runtime receives all four messages in
  arrival order before its next model decision. EVE drains them at the next model
  boundary; Codex receives them as one batched `turn/steer`.
- With `/queue collect`, EVE does not steer. It waits until the active run
  ends, then creates a followup turn with compatible queued messages after the
  debounce window.
- With `/queue interrupt`, EVE aborts the active run and starts the newest
...
before sending batched `turn/steer`. For EVE, active steering itself does not use
the debounce timer because EVE naturally batches messages until the next model
boundary.
