...
alongside EVE. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** -- if QMD is unavailable, EVE falls back to the
...
EVE creates a self-contained QMD home under
`~/.eve/agents/<agentId>/qmd/` and manages the sidecar lifecycle
...
- EVE creates collections from your workspace memory files and any
...
- If QMD fails entirely, EVE falls back to the builtin SQLite engine.
  ...
  `eve memory status` and one-shot CLI probes still recheck QMD directly.
...
EVE keeps the QMD search path compatible with both current and older QMD
installs.

On startup, EVE checks the installed QMD help text once per manager. If the
binary advertises support for multiple collection filters, EVE searches all
...
Older QMD builds only accept one collection filter. When EVE detects one
...
process, so you can tune QMD globally without adding new EVE config:
...
collection under `~/.eve/agents/<id>/qmd/sessions/`.
...
When scope denies a search, EVE logs a warning with the derived channel and
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If EVE
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

If `qmd --version` works in your shell but EVE still reports
...
with `eve memory status --deep`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs EVE uses.

**Many QMD subprocesses during search?** Update QMD if possible. EVE uses
...
`memory.qmd.searchMode = "search"`. EVE treats that mode as lexical-only,
...
the next startup reconciliation. EVE recreates stale managed collections
...
EVE's builtin symlink rules. Keep temporary monorepo checkouts under
