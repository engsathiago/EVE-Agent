...
This is separate from `~/.eve/`, which stores config, credentials, and sessions.
...
When sandboxing is enabled and `workspaceAccess` is not `"rw"`, tools operate inside a sandbox workspace under `~/.eve/sandboxes`, not your host workspace.
</Warning>

## Default location

- Default: `~/.eve/workspace`
- If `EVE_PROFILE` is set and not `"default"`, the default becomes `~/.eve/workspace-<profile>`.
- Override in `~/.eve/eve.json`:
...
`eve onboard`, `eve configure`, or `eve setup` will create the workspace and seed the bootstrap files if they are missing.
...
Older installs may have created `~/eve`. Keeping multiple workspace directories around can cause confusing auth or state drift, because only one workspace is active at a time.

<Note>
**Recommendation:** keep a single active workspace. If you no longer use the extra folders, archive or move them to Trash (for example `trash ~/eve`). If you intentionally keep multiple workspaces, make sure `agents.defaults.workspace` points to the active one.

`eve doctor` warns when it detects extra workspace directories.
</Note>

## Workspace file map

These are the standard files EVE expects inside the workspace:
...
<Note>
If any bootstrap file is missing, EVE injects a "missing file" marker into the session and continues. Large bootstrap files are truncated when injected; adjust limits with `agents.defaults.bootstrapMaxChars` (default: 20000) and `agents.defaults.bootstrapTotalMaxChars` (default: 60000). `eve setup` can recreate missing defaults without overwriting existing files.
</Note>

## What is NOT in the workspace

These live under `~/.eve/` and should NOT be committed to the workspace repo:

- `~/.eve/eve.json` (config)
- `~/.eve/agents/<agentId>/agent/auth-profiles.json` (model auth profiles: OAuth + API keys)
- `~/.eve/agents/<agentId>/agent/codex-home/` (per-agent Codex runtime account, config, skills, plugins, and native thread state)
- `~/.eve/credentials/` (channel/provider state plus legacy OAuth import data)
- `~/.eve/agents/<agentId>/sessions/` (session transcripts + metadata)
- `~/.eve/skills/` (managed skills)
  ...
  <Step title="Add a private remote">
    <Tabs>
      ...
      <Tab title="GitHub CLI (gh)">
        ```bash
        gh auth login
        gh repo create eve-workspace --private --source . --remote origin --push
        ```
      </Tab>
...
If you must store sensitive references, use placeholders and keep the real secret elsewhere (password manager, environment variables, or `~/.eve/`).
</Warning>
...
<Steps>
  <Step title="Clone the repo">
    Clone the repo to the desired path (default `~/.eve/workspace`).
  </Step>
  <Step title="Update config">
    Set `agents.defaults.workspace` to that path in `~/.eve/eve.json`.
  </Step>
  <Step title="Seed missing files">
    Run `eve setup --workspace <path>` to seed any missing files.
  </Step>
  <Step title="Copy sessions (optional)">
    If you need sessions, copy `~/.eve/agents/<agentId>/sessions/` from the old machine separately.
  </Step>
</Steps>
