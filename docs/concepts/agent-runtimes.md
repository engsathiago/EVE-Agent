---
summary: "How EVE separates model providers, models, channels, and agent runtimes"
title: "Agent runtimes"
read_when:
  - You are choosing between EVE, Codex, ACP, or another native agent runtime
...
| ------------- | --------------------------------------- | -------------------------------------------------------------- |
| Provider      | `openai`, `anthropic`, `github-copilot` | How EVE authenticates, discovers models, and names model refs. |
| Model         | `gpt-5.5`, `claude-opus-4-6`            | The model selected for the agent turn.                         |
| Agent runtime | `eve`, `codex`, `copilot`, `claude-cli` | The low level loop or backend that executes the prepared turn. |
| Channel       | Telegram, Discord, Slack, WhatsApp      | Where messages enter and leave EVE.                            |
...
`eve doctor --fix` removes old whole-agent runtime pins and rewrites
...
- **Embedded harnesses** run inside EVE's prepared agent loop. Today this
  is the built-in `eve` runtime plus registered plugin harnesses such as
  `codex` and `copilot`.
...
| Surface                                          | EVE name/config                      | What it does                                                                                                   |
| ------------------------------------------------ | ------------------------------------ | -------------------------------------------------------------------------------------------------------------- |
...
the native app-server features available; `eve doctor --fix` owns legacy
...
That means EVE selects an OpenAI model ref, then asks the Codex app-server
runtime to run the embedded agent turn. It does not mean "use API billing," and
it does not mean the channel, model provider catalog, or EVE session store
becomes Codex.
...
3. If the user explicitly chooses **EVE for an OpenAI model**, keep the model ref
   as `openai/<model>` and set provider/model runtime policy to
   `agentRuntime.id: "eve"`. A selected `openai` OAuth profile is routed
   internally through EVE's Codex-auth transport.
4. If legacy config still contains **legacy Codex model refs**, repair it to
   `openai/<model>` with `eve doctor --fix`; doctor keeps the Codex auth
   ...
   app-server route; EVE no longer keeps a bundled Codex CLI backend.
...
| Surface                     | EVE embedded                        | Codex app-server                                                       |
| --------------------------- | ----------------------------------- | ---------------------------------------------------------------------- |
| Model loop owner            | EVE through the EVE embedded runner | Codex app-server                                                       |
| Canonical thread state      | EVE transcript                      | Codex thread, plus EVE transcript mirror                               |
| EVE dynamic tools           | Native EVE tool loop                | Bridged through the Codex adapter                                      |
| Native shell and file tools | EVE path                            | Codex-native tools, bridged through native hooks where supported       |
| Context engine              | Native EVE context assembly         | EVE projects assembled context into the Codex turn                     |
| Compaction                  | EVE or selected context engine      | Codex-native compaction, with EVE notifications and mirror maintenance |
| Channel delivery            | EVE                                 | EVE                                                                    |

This ownership split is the main design rule:

- If EVE owns the surface, EVE can provide normal plugin hook behavior.
- If the native runtime owns the surface, EVE needs runtime events or native hooks.
- If the native runtime owns canonical thread state, EVE should mirror and project context, not rewrite unsupported internals.

## Runtime selection

EVE chooses an embedded runtime after provider and model resolution:
...
3. In `auto` mode, registered plugin runtimes can claim supported provider/model
   pairs.
4. If no runtime claims a turn in `auto` mode, EVE uses `eve` as the
...
`EVE_AGENT_RUNTIME`, session `agentHarnessId`/`agentRuntimeOverride` state,
`agents.defaults.agentRuntime`, and `agents.list[].agentRuntime`. Run
`eve doctor --fix` to remove stale whole-agent runtime config and convert
legacy runtime model refs where EVE can preserve the intent.
...
selection/runtime error; it is never silently routed back to EVE.
...
harness. Explicit EVE runtime config remains an opt-in compatibility route for
`openai/*` agent turns; when paired with a selected `openai` OAuth profile,
EVE routes that path internally through the Codex-auth transport while
keeping the public model ref as `openai/*`. Stale OpenAI runtime session pins are
ignored by runtime selection and can be cleaned with `eve doctor --fix`.

If `eve doctor` warns that the `codex` plugin is enabled while
legacy Codex model refs remain in config, treat that as legacy route state. Run
`eve doctor --fix` to rewrite it to `openai/*` with the Codex runtime.

## GitHub Copilot agent runtime

The external `@eve/copilot` plugin registers an opt-in `copilot` runtime
...
`eve doctor` auto-loads. For configuration, auth, transcript mirroring,
...
When a runtime is not EVE, it should document what EVE surfaces it supports.
...
| Who owns canonical thread history?     | Determines whether EVE can edit history or only mirror it.                                   |
| Do EVE dynamic tools work?             | Messaging, sessions, cron, and EVE-owned tools rely on this.                                 |
| Do dynamic tool hooks work?            | Plugins expect `before_tool_call`, `after_tool_call`, and middleware around EVE-owned tools. |
...
| What is intentionally unsupported?     | Users should not assume EVE equivalence where the native runtime owns more state.            |
