...
This is handy when you need to toggle obscure settings without editing `eve.json`.

Examples:

```
/debug show
/debug set messages.responsePrefix="[eve]"
...
Use `EVE_PLUGIN_LIFECYCLE_TRACE=1` when plugin lifecycle commands feel slow
...
```bash
EVE_PLUGIN_LIFECYCLE_TRACE=1 eve plugins install tokenjuice --force
```
...
runtime with `node dist/entry.js ...` after `pnpm build`; `pnpm eve ...`
...
`EVE_RUN_NODE_CPU_PROF_DIR`:

```bash
EVE_RUN_NODE_CPU_PROF_DIR=.artifacts/cli-cpu pnpm eve status
```
...
```bash
EVE_TRACE_SYNC_IO=1 pnpm eve gateway --force
```

`pnpm gateway:watch` leaves this flag disabled by default for the watched
Gateway child. Set `EVE_TRACE_SYNC_IO=1` when you explicitly want Node
...
`eve-gateway-watch-main` (or a profile/port-specific variant such as
`eve-gateway-watch-dev-19001`) and auto-attaches from interactive terminals.
...
```bash
tmux attach -t eve-gateway-watch-main
```
...
# or
EVE_GATEWAY_WATCH_TMUX=0 pnpm gateway:watch
```

Disable auto-attach while keeping tmux management:

```bash
EVE_GATEWAY_WATCH_ATTACH=0 pnpm gateway:watch
```
...
`EVE_TRACE_SYNC_IO=1` with `--benchmark` when you explicitly want both CPU
...
`EVE_PROFILE`, `EVE_CONFIG_PATH`, `EVE_STATE_DIR`,
`EVE_GATEWAY_PORT`, and `EVE_SKIP_CHANNELS` into the pane. Put
...
`eve doctor --fix --non-interactive` once and restarts the Gateway child.
Use `EVE_GATEWAY_WATCH_AUTO_DOCTOR=0` when you want the original startup
...
extension `package.json` and `eve.plugin.json` metadata, `tsconfig.json`,
...
- **Global `--dev` (profile):** isolates state under `~/.eve-dev` and
...
```bash
pnpm gateway:dev
EVE_PROFILE=dev eve tui
```

If you don't have a global install yet, run the CLI via `pnpm eve ...`.

What this does:

1. **Profile isolation** (global `--dev`)
   - `EVE_PROFILE=dev`
   - `EVE_STATE_DIR=~/.eve-dev`
   - `EVE_CONFIG_PATH=~/.eve-dev/eve.json`
   - `EVE_GATEWAY_PORT=19001` (browser/canvas shift accordingly)
...
2. **Dev bootstrap** (`gateway --dev`)
   ...
   - Skips channel providers in dev mode (`EVE_SKIP_CHANNELS=1`).
...
```bash
EVE_PROFILE=dev eve gateway --dev --reset
```
...
## Raw stream logging (EVE)

EVE can log the **raw assistant stream** before any filtering/formatting.
...
```bash
pnpm gateway:watch --raw-stream --raw-stream-path ~/.eve/logs/raw-stream.jsonl
```
...
```bash
EVE_RAW_STREAM=1
EVE_RAW_STREAM_PATH=~/.eve/logs/raw-stream.jsonl
```

Default file:

`~/.eve/logs/raw-stream.jsonl`
...
```bash
EVE_RAW_STREAM_PATH=~/.eve/logs/raw-openai-completions.jsonl
```

Default file:

`~/.eve/logs/raw-openai-completions.jsonl`
...
- If you need to use the built EVE CLI for other tasks (i.e. `dashboard --no-open` if your debug session spawns a new auth token), you can execute it in another terminal as `node ./eve.mjs` or create a shell alias like `alias eve-build="node $(pwd)/eve.mjs"`
