...
- `pnpm test:docker:npm-telegram-live`
  ...
  - The wrapper mounts only the `qa-lab` harness source from the checkout; the
    ...
    under test.
  - Defaults to `EVE_NPM_TELEGRAM_PACKAGE_SPEC=eve@beta`; set
...
- Release upgrade user journey smoke: `pnpm test:docker:release-upgrade-user-journey` installs the newest published baseline older than the candidate tarball by default, configures provider/plugin/ClickClack state on the published package, upgrades to the candidate tarball, then reruns the core agent/plugin/channel journey. If no older published baseline exists, it reuses the candidate version. Override the baseline with `EVE_RELEASE_UPGRADE_BASELINE_SPEC=eve@<version>`.
