---
summary: "How EVE validates update paths, package migrations, and plugin install/update behavior"
read_when:
  - Changing EVE update, doctor, package acceptance, or plugin install behavior
...
- A user can move from an older published package to the candidate package
  ...
  channel config.
- `eve doctor --fix --non-interactive` owns legacy cleanup and repair
...
- `test:docker:plugins` validates plugin install smoke, local folder installs,
  ...
  and Claude-bundle enable/inspect. Set `EVE_PLUGINS_E2E_CLAWHUB=0` to
...
- `test:docker:plugin-update` validates that an unchanged installed plugin does
  not reinstall or lose install metadata during `eve plugins update`.
...
- `test:docker:published-upgrade-survivor` first installs a published baseline,
  configures it through a baked `eve config set` recipe, updates it to the
...
- `test:docker:update-restart-auth` installs the candidate package, starts a
  ...
  `eve update --yes --json`, and requires the candidate update command to
...
`EVE_UPGRADE_SURVIVOR_SCENARIOS=reported-issues` expands to all reported
...
- `source=trusted-url`: validate an HTTPS tarball with required
  ...
  `EVE_TRUSTED_PACKAGE_TOKEN` secret.
...
`last-stable-4` resolves to the four latest stable npm-published EVE
