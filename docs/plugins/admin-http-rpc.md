...
The plugin is included with EVE, but it is off by default. When disabled, the route is not registered. When enabled, it adds:
...
Use the WebSocket RPC path for EVE clients and interactive tools that can keep a Gateway WebSocket connection open.
...
<Tabs>
  <Tab title="CLI">
    ```bash
    eve plugins enable admin-http-rpc
    eve gateway restart
    ```
  </Tab>
  <Tab title="Config">
...
```bash
eve plugins disable admin-http-rpc
eve gateway restart
```
...
- For `token` and `password` auth, narrower `x-eve-scopes` headers are ignored and the normal full operator defaults are restored.
- Trusted identity-bearing HTTP modes honor `x-eve-scopes` when present.
...
The normal Gateway WebSocket RPC path remains the preferred control-plane API for EVE clients. Use admin HTTP RPC only for host tooling that needs a request/response HTTP surface.
