...
An **agent harness** is the low level executor for one prepared EVE agent
...
runtime and the normal EVE provider transport is the wrong abstraction.
...
- a model runtime that needs its own resume id in addition to the EVE
  session transcript
...
Before a harness is selected, EVE has already resolved:

- provider and model
- runtime auth state
- thinking level and context budget
- the EVE transcript/session file
...
The prepared attempt also includes `params.runtimePlan`, an EVE-owned
policy bundle for runtime decisions that must stay shared across EVE and native
harnesses:
...
Harnesses may use the plan for decisions that need to match EVE behavior, but
should still treat it as host-owned attempt state. Do not mutate it or use it to
switch providers/models inside a turn.

## Register a harness

**Import:** `eve-agent/plugin-sdk/agent-harness`

```typescript
import type { AgentHarness } from "eve-agent/plugin-sdk/agent-harness";
...
EVE chooses a harness after provider/model resolution:
...
3. `auto` asks registered harnesses if they support the resolved
   provider/model.
4. If no registered harness matches, EVE uses its embedded runtime.
...
provider/model. Once a plugin harness has claimed a run, EVE does not
...
`agents.list[].agentRuntime`, and `EVE_AGENT_RUNTIME`. `/status` shows the
...
EVE. The harness then claims that provider in `supports(...)`.
...
- app-server request: EVE sends the bare model id to Codex and lets the
...
EVE requires Codex app-server `0.125.0` or newer. The Codex plugin checks
the app-server initialize handshake and blocks older or unversioned servers so
EVE only runs against the protocol surface it has been tested with. The
`0.125.0` floor includes the native MCP hook payload support that landed in
Codex `0.124.0`, while pinning EVE to the newer tested stable line.
...
seam is for async tool-result transforms that must run before EVE or Codex
...
`eve-agent/plugin-sdk/agent-harness-runtime` when a completed turn produced no
visible assistant text. The helper returns `empty`, `reasoning-only`, or
`planning-only` so EVE's fallback policy can decide whether to retry on a
different model. `planning-only` requires the harness's explicit `planText`
field; EVE does not infer it from assistant prose. The helper intentionally
...
`eve-agent/plugin-sdk/agent-harness-runtime` after they finalize an attempt. It
dispatches the portable `agent_end` hook and EVE's research capture without
...
The bundled `codex` harness is the native Codex mode for embedded EVE
...
`eve doctor --fix`, and legacy `codex/*` model refs remain compatibility
aliases for the native harness.

When this mode runs, Codex owns the native thread id, resume behavior,
compaction, and app-server execution. EVE still owns the chat channel,
...
By default, EVE uses `auto` provider/model runtime policy: registered
...
fail hard. This does not block an explicit provider/model `agentRuntime.id: "eve"`.
...
Keep that binding explicitly associated with the EVE session, and keep
mirroring user-visible assistant/tool output into the EVE transcript.

The EVE transcript remains the compatibility layer for:

- channel-visible session history
- transcript search and indexing
- switching back to the built-in EVE harness on a later turn
- generic `/new`, `/reset`, and session deletion behavior

If your harness stores a sidecar binding, implement `reset(...)` so EVE can
clear it when the owning EVE session is reset.

## Tool and media results

Core constructs the EVE tool list and passes it into the prepared attempt.
...
on the same delivery path as EVE-backed runs.
