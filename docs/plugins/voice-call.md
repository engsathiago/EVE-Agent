...
read_when:
  - You want to place an outbound voice call from EVE
...
Voice calls for EVE via a plugin. Supports outbound notifications,
...
<Steps>
  <Step title="Install the plugin">
    <Tabs>
      <Tab title="From npm">
        ```bash
        eve plugins install @eve/voice-call
        ```
      </Tab>
      <Tab title="From a local folder (dev)">
        ```bash
        PLUGIN_SRC=./path/to/local/voice-call-plugin
        eve plugins install "$PLUGIN_SRC"
...
<AccordionGroup>
  ...
  <Accordion title="Legacy config migrations">
    ...
    `streaming.*` OpenAI keys are rewritten by `eve doctor --fix`.
    Runtime fallback still accepts the old voice-call keys for now, but
    the rewrite path is `eve doctor --fix` and the compat shim is
    temporary.
...
- Voice Call exposes the shared `eve_agent_consult` realtime tool by default. The realtime model can call it when the caller asks for deeper reasoning, current information, or normal EVE tools.
- `realtime.consultPolicy` optionally adds guidance for when the realtime model should call `eve_agent_consult`.
...
configured EVE agent without paying a full agent-consult round trip on
...
`eve_agent_consult` still run the full EVE agent and should be used
          ...
          "voice-call": {
            config: {
              ...
              realtime: {
                enabled: true,
                provider: "google",
                instructions: "Speak briefly. Call eve_agent_consult before using deeper tools.",
                toolPolicy: "safe-read-only",
                consultPolicy: "substantive",
...
- Legacy `tts.<provider>` keys inside plugin config (`openai`, `elevenlabs`, `microsoft`, `edge`) are repaired by `eve doctor --fix`; committed config should use `tts.providers.<provider>`.
...
```bash
eve voicecall call --to "+15555550123" --message "Hello from EVE"
eve voicecall start --to "+15555550123"   # alias for call
eve voicecall continue --call-id <id> --message "Any questions?"
eve voicecall speak --call-id <id> --message "One moment"
eve voicecall dtmf --call-id <id> --digits "ww123456#"
eve voicecall end --call-id <id>
eve voicecall status --call-id <id>
eve voicecall tail
eve voicecall latency                      # summarize turn latency from logs
eve voicecall expose --mode funnel
```
...
Provider signatures are checked against the public URL EVE reconstructs
...
Use `eve logs --follow` for the live phase trace. A healthy Twilio Meet
...
`eve voicecall tail` still shows persisted call records; it is useful for
...
- `eve logs --follow` shows realtime TwiML served, the realtime bridge
