---
summary: "Adds Microsoft Foundry model provider support to EVE."
read_when:
...
Adds Microsoft Foundry model provider support to EVE.

## Distribution

- Package: `@eve/microsoft-foundry`
...
- For Entra ID auth, install the Azure CLI and run `az login` before
  onboarding. EVE refreshes Microsoft Foundry runtime tokens through
...
EVE uses the Foundry `/openai/v1` endpoint for supported OpenAI-compatible
chat APIs:
...
Claude model ID, set `params.canonicalModelId` on the model entry so EVE
...
deployment through onboarding or include model metadata so EVE can verify
