...
read_when:
  - You see the EVE_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the EVE_EXTENSION_API_DEPRECATED warning
  - You used api.registerEmbeddedExtensionFactory before EVE 2026.4.25
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external EVE plugin
---

EVE has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`eve-agent/plugin-sdk/compat`** - a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`eve-agent/plugin-sdk/infra-runtime`** - a broad runtime helper barrel that
...
- **`eve-agent/plugin-sdk/config-runtime`** - a broad config compatibility barrel
  that still carries deprecated direct load/write helpers during the migration
  window.
- **`eve-agent/extension-api`** - a bridge that gave plugins direct access to
...
EVE does not remove or reinterpret documented plugin behavior in the same
...
The modern plugin SDK fixes this: each import path (`eve-agent/plugin-sdk/\<subpath\>`)
...
`eve-agent/plugin-sdk/realtime-voice`. The new controller owns the common Talk
...
`talk.providers` should be repaired with `eve doctor --fix`; runtime Talk
...
<Steps>
  <Step title="Migrate runtime config load/write helpers">
    ...
    `eve-agent/plugin-sdk/config-runtime` compatibility barrel. Use the narrow
    SDK subpath that matches the job:

    | Need | Import |
    | --- | --- |
    | Config types such as `EVEConfig` | `eve-agent/plugin-sdk/config-contracts` |
    | Already-loaded config assertions and plugin-entry config lookup | `eve-agent/plugin-sdk/plugin-config-runtime` |
    | Current runtime snapshot reads | `eve-agent/plugin-sdk/runtime-config-snapshot` |
    | Config writes | `eve-agent/plugin-sdk/config-mutation` |
    | Session store helpers | `eve-agent/plugin-sdk/session-store-runtime` |
    | Markdown table config | `eve-agent/plugin-sdk/markdown-table-runtime` |
    | Group policy runtime helpers | `eve-agent/plugin-sdk/runtime-group-policy` |
    | Secret input resolution | `eve-agent/plugin-sdk/secret-input-runtime` |
    | Model/session overrides | `eve-agent/plugin-sdk/model-session-runtime` |
  ...
  <Step title="Migrate embedded tool-result extensions to middleware">
    ...
    ```typescript
    // EVE and Codex runtime dynamic tools
  ...
  <Step title="Migrate approval-native handlers to capability facts">
    ...
    - Register channel-owned runtime objects such as clients, tokens, or Bolt
      apps through `eve-agent/plugin-sdk/channel-runtime-context`
  ...
  <Step title="Audit Windows wrapper fallback behavior">
    If your plugin uses `eve-agent/plugin-sdk/windows-spawn`, unresolved Windows
  ...
  <Step title="Find deprecated imports">
    ...
    grep -r "eve-agent/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "eve-agent/plugin-sdk/compat";

    // After (modern focused imports)
    import { createChannelReplyPipeline } from "eve-agent/plugin-sdk/channel-reply-pipeline";
    import { createPluginRuntimeStore } from "eve-agent/plugin-sdk/runtime-store";
    import { resolveControlCommandGate } from "eve-agent/plugin-sdk/command-auth";
    ```
    ...
    import { runEmbeddedAgent } from "eve-agent/extension-api";
  ...
  <Step title="Replace broad infra-runtime imports">
    `eve-agent/plugin-sdk/infra-runtime` still exists for external
    compatibility, but new code should import the focused helper surface it
    actually needs:

    | Need | Import |
    | --- | --- |
    | System event queue helpers | `eve-agent/plugin-sdk/system-event-runtime` |
    | Heartbeat wake, event, and visibility helpers | `eve-agent/plugin-sdk/heartbeat-runtime` |
    | Pending delivery queue drain | `eve-agent/plugin-sdk/delivery-queue-runtime` |
    | Channel activity telemetry | `eve-agent/plugin-sdk/channel-activity-runtime` |
    | In-memory dedupe caches | `eve-agent/plugin-sdk/dedupe-runtime` |
    | Safe local-file/media path helpers | `eve-agent/plugin-sdk/file-access-runtime` |
    | Dispatcher-aware fetch | `eve-agent/plugin-sdk/runtime-fetch` |
    | Proxy and guarded fetch helpers | `eve-agent/plugin-sdk/fetch-runtime` |
    | SSRF dispatcher policy types | `eve-agent/plugin-sdk/ssrf-dispatcher` |
    | Approval request/resolution types | `eve-agent/plugin-sdk/approval-runtime` |
    | Approval reply payload and command helpers | `eve-agent/plugin-sdk/approval-reply-runtime` |
    | Error formatting helpers | `eve-agent/plugin-sdk/error-runtime` |
    | Transport readiness waits | `eve-agent/plugin-sdk/transport-ready-runtime` |
    | Secure token helpers | `eve-agent/plugin-sdk/secure-random-runtime` |
    | Bounded async task concurrency | `eve-agent/plugin-sdk/concurrency-runtime` |
    | Numeric coercion | `eve-agent/plugin-sdk/number-runtime` |
    | Process-local async lock | `eve-agent/plugin-sdk/async-lock-runtime` |
    | File locks | `eve-agent/plugin-sdk/file-lock` |
  ...
  <Step title="Migrate channel route helpers">
    New channel route code should use `eve-agent/plugin-sdk/channel-route`.
...
<Accordion title="Common import path table">
  ...
  | `plugin-sdk/config-schema` | Root config schema export | `EVESchema` |
  ...
  | `plugin-sdk/bundled-channel-config-schema` | Bundled config schemas | EVE-maintained bundled plugins only; new plugins must define plugin-local schemas |
...
`@eve/discord@2026.3.13` package. Owner-specific helpers live inside the
...
<AccordionGroup>
  <Accordion title="command-auth help builders → command-status">
    **Old (`eve-agent/plugin-sdk/command-auth`)**: `buildCommandsMessage`,
    `buildCommandsMessagePaginated`, `buildHelpMessage`.

    **New (`eve-agent/plugin-sdk/command-status`)**: same signatures, same
    exports - just imported from the narrower subpath. `command-auth`
    re-exports them as compat stubs.

    ```typescript
    // Before
    import { buildHelpMessage } from "eve-agent/plugin-sdk/command-auth";

    // After
    import { buildHelpMessage } from "eve-agent/plugin-sdk/command-status";
    ```
  ...
  <Accordion title="Mention gating helpers → resolveInboundMentionDecision">
    **Old**: `resolveInboundMentionRequirement({ facts, policy })` and
    ...
    `eve-agent/plugin-sdk/channel-inbound` or
    `eve-agent/plugin-sdk/channel-mention-gating`.
  ...
  <Accordion title="Channel runtime shim and channel actions helpers">
    `eve-agent/plugin-sdk/channel-runtime` is a compatibility shim for older
    channel plugins. Do not import it from new code; use
    `eve-agent/plugin-sdk/channel-runtime-context` for registering runtime
    objects.

    `channelActions*` helpers in `eve-agent/plugin-sdk/channel-actions` are
  ...
  <Accordion title="Web search provider tool() helper → createTool() on the plugin">
    **Old**: `tool()` factory from `eve-agent/plugin-sdk/provider-web-search`.

    **New**: implement `createTool(...)` directly on the provider plugin.
    EVE no longer needs the SDK helper to register the tool wrapper.
  ...
  <Accordion title="Thinking policy hooks → resolveThinkingProfile">
    ...
    **New**: a single `resolveThinkingProfile(ctx)` that returns a
    ...
    ranked level list. EVE downgrades stale stored values by profile
    rank automatically.
  ...
  <Accordion title="EVESchemaType alias → EVEConfig">
    `EVESchemaType` re-exported from `eve-agent/plugin-sdk` is now a
    one-line alias for `EVEConfig`. Prefer the canonical name.

    ```typescript
    // Before
    import type { EVESchemaType } from "eve-agent/plugin-sdk";
    // After
    import type { EVEConfig } from "eve-agent/plugin-sdk/config-schema";
    ```
...
```bash
EVE_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 eve gateway run
EVE_SUPPRESS_EXTENSION_API_WARNING=1 eve gateway run
```
