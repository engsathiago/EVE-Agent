---
summary: "How EVE installs plugin packages and resolves plugin dependencies"
read_when:
  - You are debugging plugin package installs
  - You are changing plugin startup, doctor, or package-manager install behavior
  - You are maintaining packaged EVE installs or bundled plugin manifests
title: "Plugin dependency resolution"
sidebarTitle: "Dependencies"
---

EVE keeps plugin dependency work at install/update time. Runtime loading
does not run package managers, repair dependency trees, or mutate the EVE
package directory.
...
- SDK/core imports are peer or supplied EVE imports
- local development plugins bring their own already-installed dependencies
- npm and git plugins are installed into EVE-owned package roots

EVE owns only the plugin lifecycle:
...
EVE uses stable per-source roots:

- npm packages install into per-plugin projects under
  `~/.eve/npm/projects/<encoded-package>`
- git packages clone under `~/.eve/git`
...
```bash
cd ~/.eve/npm/projects/<encoded-package>
npm install --omit=dev --omit=peer --legacy-peer-deps --ignore-scripts --no-audit --no-fund
```

`eve plugins install npm-pack:<path.tgz>` uses that same per-plugin npm
project root for a local npm-pack tarball. EVE reads the tarball's npm
...
`node_modules` beside the plugin package. EVE scans the managed project
...
publishable lockfile during install, and EVE's managed npm project root
supports it through the normal npm install path. EVE-owned publishable
...
EVE does not require it for community packages, but npm will respect it
when present.

EVE-owned npm plugin packages can also publish with explicit
...
with `eve.release.bundleRuntimeDependencies: false`; those packages still
...
`eve` package does not bundle its full dependency tree.

Plugins that import `eve-agent/plugin-sdk/*` declare `eve` as a peer
dependency. EVE does not let npm install a separate registry copy of the
...
Local plugins are treated as developer-controlled directories. EVE does not
...
```bash
eve plugins update <id>
eve plugins install <source>
eve doctor --fix
```

`doctor --fix` can clean legacy EVE-generated dependency state and recover
...
Lightweight and core-critical bundled plugins are shipped as part of EVE.
...
In source checkouts, EVE treats the repository as a pnpm monorepo. After
...
Older EVE versions generated bundled-plugin dependency roots at startup or
...
`.eve-runtime-deps*` manifests, generated plugin `node_modules`, install
...
Older npm installs also used a shared `~/.eve/npm/node_modules` root.
