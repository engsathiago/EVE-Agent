...
read_when:
  - You want an EVE agent to join a Google Meet call
  - You want an EVE agent to create a new Google Meet call
  - You are configuring Chrome, Chrome node, or Twilio as a Google Meet transport
title: "Google Meet plugin"
---

Google Meet participant support for EVE — the plugin is explicit by design:
...
- `agent` is the default talk-back mode: realtime transcription listens, the
  configured EVE agent answers, and regular EVE TTS speaks into Meet.
...
provider plus regular EVE TTS. OpenAI is the default transcription
...
```bash
eve googlemeet setup --transport chrome-node --mode transcribe
```
...
before asking an agent to join. Use `eve googlemeet setup --json` for
...
```bash
eve googlemeet setup --transport twilio
```
...
```bash
eve googlemeet join https://meet.google.com/abc-defg-hij
```
...
```bash
eve googlemeet create --transport chrome-node --mode agent
```
...
```bash
eve googlemeet create --access-type OPEN --transport chrome-node --mode agent
```
...
- Browser fallback: used when OAuth credentials are absent. EVE uses the
  ...
  the EVE Chrome profile on the node to already be signed in to Google.
...
EVE's microphone/camera permission grant and avoid the Meet **Use
...
Use `eve googlemeet test-listen <meet-url> --transport chrome-node` when
...
Local Chrome joins through the signed-in EVE browser profile. Realtime mode
requires `BlackHole 2ch` for the microphone/speaker path used by EVE. For
...
You do **not** need a full EVE Gateway or model API key inside a macOS VM
...
- Gateway host: EVE Gateway, agent workspace, model/API keys, realtime
  provider, and the Google Meet plugin config.
- Parallels macOS VM: EVE CLI/node host, Google Chrome, SoX, BlackHole 2ch,
...
Install or update EVE in the VM, then enable the bundled plugin there:

```bash
eve plugins enable google-meet
```

Start the node host in the VM:

```bash
eve node run --host <gateway-host> --port 18789 --display-name parallels-macos
```
...
EVE_ALLOW_INSECURE_PRIVATE_WS=1 \
  eve node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
```
...
```bash
EVE_ALLOW_INSECURE_PRIVATE_WS=1 \
  eve node install --host <gateway-lan-ip> --port 18789 --display-name parallels-macos --force
eve node restart
```

`EVE_ALLOW_INSECURE_PRIVATE_WS=1` is process environment, not an
`eve.json` setting. `eve node install` stores it in the LaunchAgent
...
```bash
eve googlemeet test-speech https://meet.google.com/abc-defg-hij
```

During realtime join, EVE browser automation fills the guest name, clicks
...
If `chromeNode.node` is omitted, EVE auto-selects only when exactly one
...
- `No connected Google Meet-capable node`: start `eve node run` in the VM,
  approve pairing, and make sure `eve plugins enable google-meet` and
  `eve plugins enable browser` were run in the VM. Also confirm the
...
- Chrome opens but cannot join: sign in to the browser profile inside the VM, or
  keep `chrome.guestName` set for guest join. Guest auto-join uses EVE
...
- Duplicate Meet tabs: leave `chrome.reuseExistingTab: true` enabled. EVE
...
- No audio: in Meet, route microphone/speaker through the virtual audio device
  path used by EVE; use separate virtual devices or Loopback-style routing
...
EVE does not bundle or redistribute either package. The docs ask users to
...
installer or appliance that bundles BlackHole with EVE, review BlackHole's
...
Chrome transport opens the Meet URL through EVE browser control and joins
as the signed-in EVE browser profile. On macOS, the plugin checks for
...
```bash
eve googlemeet join https://meet.google.com/abc-defg-hij --transport chrome
eve googlemeet join https://meet.google.com/abc-defg-hij --transport chrome-node
```

Route Chrome microphone and speaker audio through the local EVE audio
...
meeting; EVE does not discover those from the Meet page.
      ...
      "voice-call": {
        ...
        config: {
          ...
          realtime: {
            enabled: true,
            provider: "google",
            instructions: "Join this Google Meet as an EVE agent. Be brief.",
...
```bash
eve config validate
eve plugins list | grep -E 'google-meet|voice-call'
eve googlemeet setup
```
...
`EVE_GOOGLE_MEET_*` environment variables.
...
`meetings.space.readonly` lets EVE resolve Meet URLs/codes to spaces.
`meetings.space.settings` lets EVE pass `SpaceConfig` settings such as
...
```bash
eve googlemeet auth login --json
```
...
```bash
EVE_GOOGLE_MEET_CLIENT_ID="your-client-id" \
EVE_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
eve googlemeet auth login --json
```
...
EVE_GOOGLE_MEET_CLIENT_ID="your-client-id" \
EVE_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
eve googlemeet auth login --json --manual
```
...
support existed, rerun `eve googlemeet auth login --json` so the refresh
...
```bash
eve googlemeet doctor --oauth --json
```
...
```bash
eve googlemeet doctor --oauth --create-space --json
eve googlemeet create --no-join --json
```
...
```bash
eve googlemeet doctor --oauth --meeting https://meet.google.com/abc-defg-hij --json
eve googlemeet resolve-space --meeting https://meet.google.com/abc-defg-hij
```
...
space. A refresh-token error means rerun `eve googlemeet auth login
...
- `EVE_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID`
- `EVE_GOOGLE_MEET_CLIENT_SECRET` or `GOOGLE_MEET_CLIENT_SECRET`
- `EVE_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN`
- `EVE_GOOGLE_MEET_ACCESS_TOKEN` or `GOOGLE_MEET_ACCESS_TOKEN`
- `EVE_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` or
  `GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT`
- `EVE_GOOGLE_MEET_DEFAULT_MEETING` or `GOOGLE_MEET_DEFAULT_MEETING`
- `EVE_GOOGLE_MEET_PREVIEW_ACK` or `GOOGLE_MEET_PREVIEW_ACK`
...
```bash
eve googlemeet preflight --meeting https://meet.google.com/abc-defg-hij
```

List meeting artifacts and attendance after Meet has created conference records:

```bash
eve googlemeet artifacts --meeting https://meet.google.com/abc-defg-hij
eve googlemeet attendance --meeting https://meet.google.com/abc-defg-hij
eve googlemeet export --meeting https://meet.google.com/abc-defg-hij --output ./meet-export
```
...
```bash
eve googlemeet latest --today
eve googlemeet calendar-events --today --json
eve googlemeet artifacts --event "Weekly sync"
eve googlemeet attendance --today --format csv --output attendance.csv
```
...
```bash
eve googlemeet latest --meeting https://meet.google.com/abc-defg-hij
eve googlemeet artifacts --conference-record conferenceRecords/abc123 --json
eve googlemeet attendance --conference-record conferenceRecords/abc123 --json
```
...
```bash
eve googlemeet end-active-conference https://meet.google.com/abc-defg-hij
```
...
EVE accepts a Meet URL, meeting code, or `spaces/{id}` input and resolves it
to the API space resource before ending the active conference.
It is separate from `googlemeet leave`: `leave` stops EVE's local/session
...
```bash
eve googlemeet artifacts --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-artifacts.md
eve googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-attendance.md
eve googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format csv --output meet-attendance.csv
eve googlemeet export --conference-record conferenceRecords/abc123 \
...
```bash
EVE_LIVE_TEST=1 \
EVE_GOOGLE_MEET_LIVE_MEETING=https://meet.google.com/abc-defg-hij \
...
```bash
eve googlemeet setup --transport chrome-node --mode transcribe
eve googlemeet test-listen https://meet.google.com/abc-defg-hij --transport chrome-node --timeout-ms 30000
```

Live smoke environment:

- `EVE_LIVE_TEST=1` enables guarded live tests.
- `EVE_GOOGLE_MEET_LIVE_MEETING` points at a retained Meet URL, code, or
  `spaces/{id}`.
- `EVE_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID` provides the OAuth
  client id.
- `EVE_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN` provides
  the refresh token.
- Optional: `EVE_GOOGLE_MEET_CLIENT_SECRET`,
  `EVE_GOOGLE_MEET_ACCESS_TOKEN`, and
  `EVE_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` use the same fallback names
  without the `EVE_` prefix.
...
{
  "source": "browser",
  "error": "google-login-required: Sign in to Google in the EVE browser profile, then retry meeting creation.",
  "manualActionRequired": true,
  "manualActionReason": "google-login-required",
  "manualActionMessage": "Sign in to Google in the EVE browser profile, then retry meeting creation.",
  "browser": {
    "nodeId": "ba0f4e4bc...",
    "targetId": "tab-1",
...
profile is signed out, EVE reports `manualActionRequired: true` or a
...
realtime transcription provider key, and a configured EVE TTS provider.
...
- `chrome.guestName: "EVE Agent"`: name used on the signed-out Meet guest
  screen
- `chrome.autoJoin: true`: best-effort guest-name fill and Join Now click
  through EVE browser automation on `chrome-node`
...
- `mode: "agent"`: default talk-back mode. Participant speech is transcribed by
  ...
  EVE agent in a per-meeting sub-agent session, and spoken back through the
  normal EVE TTS runtime.
- `mode: "bidi"`: fallback direct bidirectional realtime model mode. The
  ...
  `eve_agent_consult` for deeper/tool-backed answers.
...
- `realtime.instructions`: brief spoken replies, with
  `eve_agent_consult` for deeper answers
- `realtime.introMessage`: short spoken readiness check when the realtime bridge
  connects; set it to `""` to join silently
- `realtime.agentId`: optional EVE agent id for
  `eve_agent_consult`; defaults to `main`
...
VM. In both cases the model providers and `eve_agent_consult` run on the
Gateway host, so model credentials stay there. With the default `mode: "agent"`,
the realtime transcription provider handles listening, the configured EVE
agent produces the answer, and regular EVE TTS speaks it into Meet. Use
...
- `speechReady` / `speechBlockedReason` / `speechBlockedMessage`: whether
  managed Chrome speech is allowed now. `speechReady: false` means EVE did
...
transcripts are routed through the configured EVE agent, and the answer is
spoken through the normal EVE TTS runtime. Set `mode: "bidi"` when you want
...
| ------- | ----------------------------- | -------------------------------------- | ----------------------------------------------------- |
| `agent` | The configured EVE agent | Normal EVE TTS runtime            | You want "my agent is in the meeting" behavior        |
...
information, or normal EVE tools, it can call `eve_agent_consult`.

The consult tool runs the regular EVE agent behind the scenes with recent
meeting transcript context and returns a concise spoken answer. In `agent` mode,
EVE sends that answer directly to the TTS runtime; in `bidi` mode, the
...
Meet lane should consult a dedicated EVE agent workspace, model defaults,
...
```bash
eve googlemeet speak meet_... "Say exactly: I'm here and listening."
```
...
- `eve logs --follow` shows DTMF TwiML served before realtime TwiML, then a
...
```bash
eve plugins list | grep google-meet
eve googlemeet setup
```
...
hear you in the meeting?" That is Meet's audio-choice interstitial; EVE
clicks **Use microphone** through browser automation when available and keeps
waiting for the real meeting state. For create-only browser fallback, EVE
...
- For API creation: `oauth.clientId` and `oauth.refreshToken` are configured,
  or matching `EVE_GOOGLE_MEET_*` environment variables are present.
- For API creation: the refresh token was minted after create support was
  ...
  `eve googlemeet auth login --json` and update plugin config.
- For browser fallback: `defaultTransport: "chrome-node"` and
  `chromeNode.node` point at a connected node with `browser.proxy` and
  `googlemeet.chrome`.
- For browser fallback: the EVE Chrome profile on that node is signed in
...
- For browser fallback: if Meet shows "Do you want people to hear you in the
  meeting?", leave the tab open. EVE should click **Use microphone** or, for
...
```bash
eve googlemeet setup
eve googlemeet doctor
```

Use `mode: "agent"` for the normal STT -> EVE agent -> TTS talk-back path,
...
run `eve googlemeet status --json <session-id>` after participants speak
...
utterance but EVE did not see new output bytes reach the Chrome audio
bridge.
...
- Meet microphone and speaker are routed through the virtual audio path used by
  EVE. `doctor` should show `meet output routed: yes` for local Chrome
  realtime joins.
...
```bash
eve googlemeet recover-tab
eve googlemeet recover-tab https://meet.google.com/abc-defg-hij
```
...
```bash
eve voicecall smoke --to "+15555550123"
```
...
```bash
eve voicecall smoke --to "+15555550123" --yes
```
...
- Run `eve googlemeet doctor <session-id>` to confirm the delegated Twilio
  call ID, whether DTMF was queued, and whether the intro greeting was requested.
- Run `eve voicecall status --call-id <id>` and confirm the call is still
  active.
- Run `eve voicecall tail` and check that Twilio webhooks are arriving at
  the Gateway.
- Run `eve logs --follow` and look for the Twilio Meet sequence: Google
...
- Re-run `eve googlemeet setup --transport twilio`; a green setup check is
...
- If the participant joins but you do not hear the greeting, check
  `eve logs --follow` for the post-DTMF `voicecall.speak` request and
...
- `chrome.audioInputCommand` plus `chrome.audioOutputCommand`: EVE owns the
