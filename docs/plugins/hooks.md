...
Plugin hooks are in-process extension points for EVE plugins. Use them
...
current plugin options; EVE injects it per handler without mutating the
...
- `subagent_spawned` includes `resolvedModel` and `resolvedProvider` when EVE has resolved the child session's native model before launch.
...
use `eve sessions` or the Gateway session/status surfaces. When debugging
...
- EVE strips `toolResult.details` before provider replay and compaction
...
When a run is blocked, EVE stores only the replacement text in
...
`before_agent_start` and `agent_end` include `event.runId` when EVE can
...
identifier when EVE can derive one from the session key or delivery
metadata.
...
forever. A timeout is logged and EVE continues; it does not cancel
...
`durationMs`/`outcome`, and `upstreamRequestIdHash` when EVE can derive a
...
Codex native `Stop` hooks are relayed into this hook as EVE
...
reach the next model turn exactly once. EVE drains queued injections before
...
policy runs from EVE config, covers CLI install and update paths, and fails
closed when enabled but unavailable.

`before_install` is a plugin-runtime lifecycle hook. It runs after
`security.installPolicy` only in the EVE process where plugin hooks have
...
field remains in the event payload for compatibility, but EVE no longer
...
context when syncing external wake schedulers, and keep EVE as the
