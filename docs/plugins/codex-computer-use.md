---
summary: "Set up Codex Computer Use for Codex-mode EVE agents"
title: "Codex Computer Use"
read_when:
  - You want Codex-mode EVE agents to use Codex Computer Use
...
Computer Use is a Codex-native MCP plugin for local desktop control. EVE
...
Use this page when EVE is already using the native Codex harness. For the
runtime setup itself, see [Codex harness](/plugins/codex-harness).

## EVE.app and Peekaboo

EVE.app's Peekaboo integration is separate from Codex Computer Use. The
...
Use [Peekaboo bridge](/platforms/mac/peekaboo) when you want EVE.app to be
a permission-aware host for Peekaboo CLI automation. Use this page when a
Codex-mode EVE agent should have Codex's native `computer-use` MCP plugin
...
Instead, the iOS app connects as an EVE node and exposes mobile
...
EVE-managed runtimes to call TryCua's driver directly, use the upstream
`cua-driver mcp` server through EVE's MCP registry instead of the
Codex-specific marketplace flow.

After installing `cua-driver`, either ask it for the EVE command:

```bash
cua-driver mcp-config --client eve
```
...
available as a normal EVE MCP server. Use the Codex Computer Use setup on
...
that its app prompts for, such as Accessibility and Screen Recording. EVE
...
Computer Use in and lets EVE install or re-enable it before the turn:
...
With this config, EVE checks Codex app-server before each Codex-mode turn.
If Computer Use is missing but Codex app-server has already discovered an
installable marketplace, EVE asks Codex app-server to install or re-enable
the plugin and reload MCP servers. On macOS, when no matching marketplace is
registered and the standard Codex app bundle exists, EVE also tries to
...
plugin command surface is available. These are EVE chat/runtime commands,
not `eve codex ...` CLI subcommands:
...
EVE uses the same app-server API that Codex itself exposes. The
...
During install, EVE polls `plugin/list` for up to
`marketplaceDiscoveryTimeoutMs` milliseconds. The default is 60 seconds.

If multiple known marketplaces contain Computer Use, EVE prefers
...
`computer-use` is registered, EVE tries to add the standard bundled
...
EVE reports a stable setup reason internally and formats the user-facing
status for chat:
...
permissions before it can inspect or control apps. If EVE says Computer Use
...
EVE intentionally fails closed when `computerUse.enabled` is true. A
...
tool hook could not reach an active EVE relay through the local bridge or
Gateway fallback. Start a fresh EVE session with `/new` or `/reset`. If it
works once and then fails again on a later tool call, `/new` is only clearing the
current attempt; restart the Codex app-server or EVE Gateway so old threads
