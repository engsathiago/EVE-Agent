---
summary: "Install and use Codex, Claude, and Cursor bundles as EVE plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how EVE maps bundle content into native features
  - You are debugging bundle detection or missing capabilities
title: "Plugin bundles"
---

EVE can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
EVE maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native EVE plugins. Native plugins run
...
of requiring authors to rewrite them as native EVE plugins, EVE
...
<Steps>
  <Step title="Install from a directory, archive, or marketplace">
    ```bash
    # Local directory
    eve plugins install ./my-bundle

    # Archive
    eve plugins install ./my-bundle.tgz
...
## What EVE maps from bundles

Not every bundle feature runs in EVE today. Here is what works and what
...
| ------------- | -------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal EVE skills                                                 | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                   | Claude, Cursor |
| Hook packs    | EVE-style `HOOK.md` + `handler.ts` layouts                                                   | Codex          |
| MCP tools     | Bundle MCP config merged into embedded EVE settings; supported stdio and HTTP servers loaded | All formats    |
| LSP servers   | Claude `.lsp.json` and manifest-declared `lspServers` merged into embedded EVE LSP defaults  | Claude         |
| Settings      | Claude `settings.json` imported as embedded EVE defaults                                     | Claude         |

#### Skill content

- bundle skill roots load as normal EVE skill roots
- Claude `commands` roots are treated as additional skill roots
- Cursor `.cursor/commands` roots are treated as additional skill roots

This means Claude markdown command files work through the normal EVE skill
loader. Cursor command markdown works through the same path.

#### Hook packs

- bundle hook roots work **only** when they use the normal EVE hook-pack
  layout. Today this is primarily the Codex-compatible case:
  - `HOOK.md`
  - `handler.ts` or `handler.js`

#### MCP for embedded EVE

- enabled bundles can contribute MCP server config
- EVE merges bundle MCP config into the effective embedded EVE settings as
  `mcpServers`
- EVE exposes supported bundle MCP tools during embedded EVE agent turns by
...
- `transport` may be set to `"streamable-http"` or `"sse"`; when omitted, EVE uses `sse`
- `type: "http"` is a CLI-native downstream shape; use `transport: "streamable-http"` in EVE config. `eve mcp set` and `eve doctor --fix` normalize the common alias.
...
EVE registers bundle MCP tools with provider-safe names in the form
...
#### Embedded EVE settings

- Claude `settings.json` is imported as default embedded EVE settings when the
  bundle is enabled
- EVE sanitizes shell override keys before applying them
...
- EVE loads `.lsp.json` plus any manifest-declared `lspServers` paths
- bundle LSP config is merged into the effective embedded EVE LSP defaults
- only supported stdio-backed LSP servers are runnable today; unsupported
  transports still show up in `eve plugins inspect <id>`

### Detected but not executed

These are recognized and shown in diagnostics, but EVE does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit EVE best when they use skill roots and EVE-style
  ...
  <Accordion title="Claude bundles">
    ...
    - `settings.json` is imported into embedded EVE settings (shell override keys are sanitized)
    - `.mcp.json` exposes supported stdio tools to embedded EVE
    - `.lsp.json` plus manifest-declared `lspServers` paths load into embedded EVE LSP defaults
...
EVE checks for native plugin format first:

1. `eve.plugin.json` or valid `package.json` with `eve.extensions` — treated as **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) — treated as **bundle**

If a directory contains both, EVE uses the native path. This prevents
...
- Third-party compatible bundles do not get startup `npm install` repair. They
  should be installed through `eve plugins install` and ship everything
  they need in the installed plugin directory.
- EVE-owned bundled plugins are either shipped lightweight in core or
  downloadable through the plugin installer. Gateway startup never runs a
  package manager for them.
- `eve doctor --fix` removes legacy staged dependency directories and can
...
- EVE does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `eve plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded EVE settings from `settings.json` are supported. EVE does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    EVE hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
