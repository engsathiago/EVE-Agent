---
summary: "Bundled `oc-path` plugin: ships the `eve path` CLI for the `oc://` workspace-file addressing scheme"
read_when:
...
The bundled `oc-path` plugin adds the [`eve path`](/cli/path) CLI for the
`oc://` workspace-file addressing scheme. It ships in the EVE repo under
...
- **Local automation**: shell scripts can resolve or update one workspace value
  with `eve path … --json` instead of carrying separate markdown, JSONC,
...
eve path resolve 'oc://config.jsonc/plugins/github/enabled' --json

# Which tool-call names appear in this session log?
eve path find 'oc://session.jsonl/[event=tool_call]/name' --json
...
The plugin runs **in-process inside the `eve` CLI** on the host where you
...
The plugin metadata lives in `extensions/oc-path/eve.plugin.json`:
...
`eve path …`, so installs that never use the verb pay no cost.
...
state. Bare `eve path` invocations work immediately on the same host —
the CLI loads the plugin on demand.

Disable with:

```bash
eve plugins disable oc-path
```
...
| Surface                        | Provided by                                             |
| ------------------------------ | ------------------------------------------------------- |
| `eve path` CLI                 | `extensions/oc-path/cli-registration.ts`                |
...
`__EVE_REDACTED__` (verbatim or as a substring) is refused at write time
...
- [`eve path` CLI reference](/cli/path)
