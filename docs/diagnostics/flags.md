...
```bash
EVE_DIAGNOSTICS=telegram.http,telegram.payload
```
...
`EVE_DIAGNOSTICS=0` is a process-level disable override: it disables
...
```bash
EVE_DIAGNOSTICS=profiler eve gateway run
```

Enable only reply-dispatch profiler spans:

```bash
EVE_DIAGNOSTICS=reply.profiler eve gateway run
```

Enable only Codex app-server startup/tool/thread profiler spans:

```bash
EVE_DIAGNOSTICS=codex.profiler eve gateway run
```
...
```bash
EVE_DIAGNOSTICS=0 eve gateway run
```
...
```bash
EVE_DIAGNOSTICS=timeline \
EVE_DIAGNOSTICS_TIMELINE_PATH=/tmp/eve-timeline.jsonl \
eve gateway run
```
...
`EVE_DIAGNOSTICS_TIMELINE_PATH`. When `timeline` is enabled only from
config, the earliest config-loading spans are not emitted because EVE has
not read config yet; subsequent startup spans use the config flag.

`EVE_DIAGNOSTICS=1`, `EVE_DIAGNOSTICS=all`, and
`EVE_DIAGNOSTICS=*` also enable the timeline because they enable every
diagnostics flag. Prefer `timeline` when you only want the JSONL timing
artifact.

Timeline records use the `eve.diagnostics.v1` envelope. Events can include
...
```bash
ls -t /tmp/eve/eve-*.log | head -n 1
```

Filter for Telegram HTTP diagnostics:

```bash
rg "telegram http error" /tmp/eve/eve-*.log
```

Filter for Brave Search HTTP diagnostics:

```bash
rg "brave http" /tmp/eve/eve-*.log
```

Or tail while reproducing:

```bash
tail -f /tmp/eve/eve-$(date +%F).log | rg "telegram http error"
```

For remote gateways, you can also use `eve logs --follow` (see [/cli/logs](/cli/logs)).
