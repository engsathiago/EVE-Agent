---
summary: "How EVE builds prompt context and reports token usage + costs"
read_when:
...
EVE tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

EVE assembles its own system prompt on every run. It includes:
...
`toolResultMaxChars` is an advanced ceiling. When it is unset, EVE chooses
...
For images, EVE downscales transcript/tool image payloads before provider calls.
...
- `/usage off|tokens|full` → appends a **per-response usage footer** to every reply.
  ...
  - `/usage full` shows estimated cost only when EVE has usage metadata and
    local pricing for the active model. Otherwise it shows tokens only.
- `/usage cost` → shows a local cost summary from EVE session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `eve status --usage` and `eve channels list` show
...
available; otherwise EVE falls back to matching OAuth/API-key credentials
...
EVE keeps provider usage accounting separate from the current context
...
`cacheWrite`. If pricing is missing, EVE shows tokens only. Cost display is
...
After sidecars and channels reach the Gateway ready path, EVE starts an
...
Provider prompt caching only applies within the cache TTL window. EVE can
...
EVE sizes GA-capable Claude 4.x models such as Opus 4.8, Opus 4.7, Opus 4.6, and
...
Older configs can keep `context1m: true`, but EVE no longer sends
...
EVE preserves the OAuth-required Anthropic beta headers while stripping the
