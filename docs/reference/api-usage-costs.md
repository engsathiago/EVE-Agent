...
EVE features that can generate provider usage or paid API calls.
...
- If EVE has usage metadata and local pricing for the active model,
...
Anthropic note: Anthropic staff told us EVE-style Claude CLI usage is
...
Anthropic still does not expose a per-message dollar estimate that EVE can
show in `/usage full`.

**CLI usage windows (provider quotas)**

- `eve status --usage` and `eve channels list` show provider **usage windows**
...
- MiniMax note: its raw `usage_percent` / `usagePercent` fields mean remaining
  quota, so EVE inverts them before display. Count-based fields still win
  when present. If the provider returns `model_remains`, EVE prefers the
...
- Usage auth for those quota windows comes from provider-specific hooks when
  available; otherwise EVE falls back to matching OAuth/API-key
...
EVE can pick up credentials from:
...
EVE's local UI, such as **OpenAI Codex**, **Alibaba Cloud Model Studio
Coding Plan**, **MiniMax Coding Plan**, **Z.AI / GLM Coding Plan**, and
Anthropic's EVE Claude-login path with **Extra Usage** enabled.
...
`eve models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
