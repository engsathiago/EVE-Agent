---
summary: "EVE code mode: an opt-in exec/wait tool surface backed by QuickJS-WASI and a hidden run-scoped tool catalog"
title: "Code mode"
sidebarTitle: "Code mode"
read_when:
  - You want to enable EVE code mode for an agent run
...
Code mode is an experimental EVE agent-runtime feature. It is off by
default. When you enable it, EVE changes what the model sees for one run:
instead of exposing every enabled tool schema directly, the model sees only
`exec` and `wait`.

This page documents EVE code mode. It is not Codex Code mode. The two
...
- EVE code mode is disabled unless `tools.codeMode.enabled: true` is
  configured. It runs in the EVE generic agent runtime, where the model
...
surfaces. EVE code mode is an EVE-owned experimental tool-surface
adapter for generic EVE runs. It uses `quickjs-wasi`, a hidden EVE
tool catalog, and the normal EVE tool executor.

## What is this?

EVE code mode lets the model write a small JavaScript or TypeScript program
...
- `exec` evaluates model-generated JavaScript or TypeScript in a constrained
  QuickJS-WASI worker.
- Normal EVE tools are hidden from the model prompt and exposed inside the
  guest program through `ALL_TOOLS` and `tools`.
- Guest code can search the hidden catalog, describe a tool, and call a tool
  through the same EVE execution path used by normal agent turns.
...
surface. It does not replace EVE tools, plugin tools, MCP tools, auth,
...
- Provider neutral: it works for EVE, plugin, MCP, and client tools without
  depending on provider-native code execution.
- Existing policy stays in force: nested tool calls still go through EVE
...
- Clear failure mode: when code mode is explicitly enabled and the runtime is
  unavailable, EVE fails closed instead of falling back to broad direct tool
  exposure.
...
```bash
EVE_DEBUG_CODE_MODE=1 \
EVE_DEBUG_MODEL_TRANSPORT=1 \
EVE_DEBUG_MODEL_PAYLOAD=tools \
eve gateway
```
...
`EVE_DEBUG_MODEL_PAYLOAD=full-redacted` for a short debugging session.
...
- Stability: experimental EVE surface; Codex Code mode is a separate stable
  Codex harness surface.
- Target surface: generic EVE agent runs.
...
**Code mode** is the EVE runtime mode that hides normal model tools and
...
If code mode is enabled but QuickJS-WASI cannot load, EVE fails closed for
...
2. Build the effective EVE tool list.
...
- If `language` is `"typescript"`, EVE transpiles before evaluation.
...
final value is JSON-compatible after EVE's output adapter runs.
...
`wait` exists because nested EVE tools can be slow, interactive, approval
...
2. On suspension, EVE snapshots the QuickJS VM and records pending host
   work.
3. When pending work settles, `wait` restores the VM snapshot.
4. EVE re-registers host callbacks by stable names.
5. EVE delivers nested tool results into the restored VM.
6. EVE drains QuickJS pending jobs.
...
const hits = await tools.web_search({ query: "EVE code mode" });
```
...
state directory. For each code-mode `exec` call, EVE builds the run-scoped
...
inside the QuickJS program while EVE still shows only `exec` and `wait` to
the model.
...
7. EVE auto-drains ready namespace bridge calls inside the active
...
- `globalName` collisions with built-in code-mode globals such as `tools`,
  `namespaces`, `text`, `json`, `yield_control`, or `__eve*`
...
catalog for arbitrary enabled EVE, plugin, and client tools; use `MCP` for
...
- images and files travel through ordinary EVE tools, not through the
  code-mode bridge
...
Code mode supersedes the EVE Tool Search model surface for runs where it is
active.

When `tools.codeMode.enabled` is true and code mode activates:

- EVE does not expose `tool_search_code`, `tool_search`, `tool_describe`,
...
- MCP calls use the generated `MCP` namespace and its `$api()` headers instead
  of `tools.call(...)`.
- Nested calls dispatch through the same EVE executor path that Tool Search
  uses.

The existing [Tool Search](/tools/tool-search) page describes the EVE compact
catalog bridge. Code mode is the generic EVE alternative for runs that can
use `exec` and `wait`.

## Tool names and collisions

The model-visible `exec` tool is the code-mode tool. If the normal EVE
shell `exec` tool is enabled, it is hidden from the model and cataloged like any
other tool.

Inside the guest runtime:

- `tools.call("eve:core:exec", input)` can call the shell exec tool if
  policy allows it.
...
If two tools normalize to the same safe convenience name, EVE omits the
convenience function and requires `tools.call(id, input)`.

## Nested tool execution

Every nested tool call crosses the host bridge and re-enters EVE.
...
EVE loads `quickjs-wasi` as a direct dependency in the owning package. The
...
The runtime executes outside EVE's main event loop in a worker. A guest
...
inputs beyond existing EVE trajectory policy.
...
EVE_DEBUG_MODEL_TRANSPORT=1 \
EVE_DEBUG_MODEL_PAYLOAD=tools \
EVE_DEBUG_SSE=events \
eve gateway
```

For payload-shape debugging, use `EVE_DEBUG_MODEL_PAYLOAD=full-redacted`.
...
For stream debugging, use `EVE_DEBUG_SSE=peek` to log the first five
...
- `tools.search`, `tools.describe`, and `tools.call` work for EVE tools
...
5. Send an agent turn with EVE, plugin, MCP, and client test tools.
...
8. In `exec`, call EVE/plugin/client tools through `tools.search`,
