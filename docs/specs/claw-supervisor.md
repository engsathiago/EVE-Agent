...
description: Fleet supervision plan for Codex app-server sessions controlled by EVE.
readWhen:
  - Designing Codex fleet supervision
  - Building EVE tools that read, steer, or spawn Codex sessions
...
Claw Supervisor lets one always-on EVE instance monitor and drive a fleet of Codex sessions without changing the normal Codex user experience. A user can SSH into a host, start Codex, work in the TUI, and still have the supervisor read the session, steer it, interrupt it, spawn related sessions, and accept handoffs. Codex sessions can also call back into EVE through MCP.

## Product Model

Codex remains the primary work surface. EVE supervises Codex rather than hiding Codex inside an opaque EVE subagent.

The EVE plugin is named `codex-supervisor`. `crabfleet` remains the deployment
...
- Supervisor Claw: an always-on EVE agent with tools for fleet state, transcript reads, steering, interruption, spawning, and handoff.

EVE may use its existing subagent machinery internally, but the external contract is an attachable Codex session with a Codex thread id.
...
- EVE tools for Claw-to-Codex control.
...
Every supervised Codex gets an MCP server named `eve-codex-supervisor`.
...
- Supervisor state lives in EVE Gateway storage.
...
- Transcript reads redact secrets at EVE boundary.
...
- Durable state owner: EVE Gateway DB, Cloudflare Durable Object, or VPS database.
