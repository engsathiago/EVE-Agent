...
- Re-open anytime: `eve dashboard` (copies link, opens browser if possible, shows SSH hint if headless).
- If clipboard and browser delivery fail, `eve dashboard` still prints the
  clean URL and tells you to use the token from `EVE_GATEWAY_TOKEN` or
...
- **Shared-secret token source**: `gateway.auth.token` (or
  `EVE_GATEWAY_TOKEN`); `eve dashboard` can pass it via URL fragment
...
- If `gateway.auth.token` is SecretRef-managed, `eve dashboard`
...
- If `gateway.auth.token` is configured as a SecretRef and is unresolved in your
  current shell, `eve dashboard` still prints a non-tokenized URL plus
  actionable auth setup guidance.
- **Shared-secret password**: use the configured `gateway.auth.password` (or
  `EVE_GATEWAY_PASSWORD`). The dashboard does not persist passwords across
  reloads.
...
- Ensure the gateway is reachable (local: `eve status`; remote: SSH tunnel `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`).
...
- Retrieve or supply the shared secret from the gateway host:
  - Token: `eve config get gateway.auth.token`
  ...
  - SecretRef-managed token: resolve the external secret provider or export
    `EVE_GATEWAY_TOKEN` in this shell, then rerun `eve dashboard`
  - No shared secret configured: `eve doctor --generate-gateway-token`
