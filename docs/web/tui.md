...
```bash
eve tui --url ws://<host>:<port> --token <gateway-token>
```
...
- `eve chat` and `eve terminal` are aliases for `eve tui --local`.
- `--local` cannot be combined with `--url`, `--token`, or `--password`.
- Local mode uses the embedded agent runtime directly. Most local tools work, but Gateway-only features are unavailable.
- After a config file has authored settings, `eve` and `eve crestodian` also use this TUI shell, with Crestodian as the local setup and repair chat backend.
...
- To show the Gateway host for non-local URL-backed connections, opt in with:
  ...
  ```bash
  eve config set tui.footer.showRemoteHost true
  ```
...
- Local shell commands receive `EVE_SHELL=tui-local` in their environment.
...
If `eve config validate` is already failing, start with `eve configure`
or `eve doctor --fix` first. `eve chat` does not bypass the invalid-
config guard.
...
4. Apply narrow changes with `eve config set` or `eve configure`, then rerun `!eve config validate`.
5. If Doctor recommends an automatic migration or repair, review it and run `!eve doctor --fix`.

Tips:

- Prefer `eve config set` or `eve configure` over hand-editing `eve.json`.
- `eve docs "<query>"` searches the live docs index from the same machine.
- `eve config validate --json` is useful when you want structured schema and SecretRef/resolvability errors.
...
- If your terminal uses a light background and auto-detection is wrong, set `EVE_THEME=light` before launching `eve tui`.
- To force the original dark palette instead, set `EVE_THEME=dark`.
...
- Check the Gateway logs: `eve logs --follow`.
- Confirm the agent can run: `eve status` and `eve models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).

## Connection troubleshooting

- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `eve agents list` and your routing config.
- Empty session picker: you might be in global scope or have no sessions yet.

## Related

- [Control UI](/web/control-ui) — web-based control interface
- [Config](/cli/config) — inspect, validate, and edit `eve.json`
