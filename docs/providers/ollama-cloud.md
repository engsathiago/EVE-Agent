---
summary: "Use Ollama Cloud directly with EVE"
read_when:
...
Ollama Cloud is Ollama's hosted model API. It lets EVE call Ollama-hosted
...
`/api/chat` style, not the OpenAI-compatible `/v1` route. EVE registers it
...
- You want the same native Ollama chat API shape EVE uses for local
...
EVE discovers Ollama Cloud models from the live hosted catalog. Commonly
...
```bash
eve models list --provider ollama-cloud
eve models set ollama-cloud/kimi-k2.6
```
...
EVE_LIVE_OLLAMA_MODEL=kimi-k2.6 \
...
- Unknown model errors: run `eve models list --provider ollama-cloud` and
