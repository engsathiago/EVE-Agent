...
read_when:
  - You want to use Volcano Engine or Doubao models with EVE
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    eve onboard --auth-choice volcengine-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    eve models list --provider volcengine
    eve models list --provider volcengine-plan
    ```
  </Step>
</Steps>
...
For voice-note targets, EVE asks Volcengine for provider-native
...
<AccordionGroup>
  <Accordion title="Default model after onboarding">
    `eve onboard --auth-choice volcengine-api-key` currently sets
  ...
  <Accordion title="Model picker fallback behavior">
    ...
    loaded yet, EVE falls back to the unfiltered catalog instead of showing an
  ...
  <Accordion title="Environment variables for daemon processes">
    ...
    `~/.eve/.env` or via `env.shellEnv`).
  </Accordion>
</AccordionGroup>

<Warning>
When running EVE as a background service, environment variables set in your
...
<CardGroup cols={2}>
  ...
  <Card title="FAQ" href="/help/faq" icon="circle-question">
    Frequently asked questions about EVE setup.
  </Card>
</CardGroup>
