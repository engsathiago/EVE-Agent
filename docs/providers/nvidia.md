---
summary: "Use NVIDIA's OpenAI-compatible API in EVE"
read_when:
  - You want to use open models in EVE for free
...
[build.nvidia.com](https://build.nvidia.com/settings/api-keys). EVE
...
<Steps>
  ...
  <Step title="Export the key and run onboarding">
    ...
    eve onboard --auth-choice nvidia-api-key
    ```
  </Step>
  <Step title="Set an NVIDIA model">
    ```bash
    eve models set nvidia/nvidia/nemotron-3-ultra-550b-a55b
    ```
  </Step>
</Steps>
...
```bash
eve onboard --auth-choice nvidia-api-key --nvidia-api-key "nvapi-..."
```
...
When an NVIDIA API key is configured, EVE setup and model-selection paths
...
EVE release. When the live feed is available, the first returned model is
...
malformed, EVE falls back to the bundled catalog and bundled default below.

## Nemotron 3 Ultra

Nemotron 3 Ultra is the default NVIDIA model in EVE. NVIDIA's build page for
...
<AccordionGroup>
  ...
  <Accordion title="Catalog and pricing">
    EVE prefers NVIDIA's public featured-model catalog when NVIDIA auth is
  ...
  <Accordion title="Nemotron 3 Ultra reasoning params">
    ...
    and `reasoning_budget` for reasoning output. EVE's bundled Ultra row
