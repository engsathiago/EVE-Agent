---
summary: "Use Z.AI (GLM models) with EVE"
read_when:
  - You want Z.AI / GLM models in EVE
...
EVE uses the `zai` provider with a Z.AI API key.
...
GLM is a model family, not a separate provider. In EVE, GLM models use
...
<Tabs>
  <Tab title="Auto-detect endpoint">
    **Best for:** most users. EVE probes supported Z.AI endpoints with your API key and applies the correct base URL automatically.

    <Steps>
      <Step title="Run onboarding">
        ```bash
        eve onboard --auth-choice zai-api-key
        ```
      </Step>
      <Step title="Verify the model is listed">
        ```bash
        eve models list --all --provider zai
        ```
      </Step>
    </Steps>
    ...
    **Best for:** users who want to force a specific Coding Plan or general API surface.
    ...
    <Steps>
      <Step title="Pick the right onboarding choice">
        ...
        eve onboard --auth-choice zai-coding-global

        # Coding Plan CN (China region)
        eve onboard --auth-choice zai-coding-cn

        # General API
        eve onboard --auth-choice zai-global

        # General API CN (China region)
        eve onboard --auth-choice zai-cn
        ```
      </Step>
      <Step title="Verify the model is listed">
        ```bash
...
<Tip>
`zai-api-key` lets EVE detect the matching Z.AI endpoint from the key and
...
EVE ships the bundled `zai` provider catalog in the plugin manifest, so read-only
...
<Tip>
GLM-5.2 supports `off`, `low`, `high`, and `max` thinking levels. EVE maps
...
can change; run `eve models list --all --provider zai` to see the catalog
...
<AccordionGroup>
  ...
  <Accordion title="Thinking and preserved thinking">
    Z.AI thinking follows EVE's `/think` controls. With thinking off,
    EVE sends `thinking: { type: "disabled" }` to avoid responses that
    ...
    When enabled and thinking is on, EVE sends
  ...
  <Accordion title="Auth details">
    ...
    - The legacy env var `Z_AI_API_KEY` is still accepted; EVE copies it to `ZAI_API_KEY` at startup if `ZAI_API_KEY` is unset.
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration reference" href="/gateway/configuration-reference" icon="gear">
    Full EVE config schema, including provider and model settings.
  </Card>
</CardGroup>
