---
summary: "Use Synthetic's Anthropic-compatible API in EVE"
read_when:
...
EVE registers it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    eve onboard --auth-choice synthetic-api-key
    ```
  </Step>
...
<Warning>
EVE's Anthropic client appends `/v1` to the base URL automatically, so use
...
`eve models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    Remember that EVE appends `/v1` automatically.
