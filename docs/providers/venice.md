---
summary: "Use Venice AI privacy-focused models in EVE"
read_when:
  - You want privacy-focused inference in EVE
...
<Steps>
  ...
  <Step title="Configure EVE">
    ...
    <Tabs>
      <Tab title="Interactive (recommended)">
        ```bash
        eve onboard --auth-choice venice-api-key
        ```
  ...
  <Step title="Verify setup">
    ```bash
    eve agent --model venice/kimi-k2-5 --message "Hello, are you working?"
    ```
  </Step>
</Steps>
...
After setup, EVE shows all available Venice models. Pick based on your needs:
...
```bash
eve models set venice/kimi-k2-5
eve models set venice/claude-opus-4-6
```

List all available models:

```bash
eve models list --all --provider venice
```

You can also run `eve configure`, select **Model/auth**, and choose **Venice AI**.
...
`venice/deepseek-v4-flash`, EVE fills the required DeepSeek V4
...
EVE keeps that provider-specific replay fix separate from the native
...
EVE ships a manifest-backed Venice seed catalog for read-only model listing. Runtime refresh can still discover models from the Venice API, and falls back to the manifest catalog if the API is unreachable.
...
eve agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
eve agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
eve agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
eve agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
eve agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
<AccordionGroup>
  ...
  <Accordion title="Model not available">
    The Venice model catalog updates dynamically. Run `eve models list` to see currently available models. Some models may be temporarily offline.
  </Accordion>
