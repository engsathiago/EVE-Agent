...
read_when:
  - You need a beginner-friendly overview of EVE logging
  - You want to configure log levels, formats, or redaction
  - You are troubleshooting and need to find logs quickly
title: "Logging"
---

EVE has two main log surfaces:
...
EVE keeps up to five numbered archives beside the active file, such as
`eve-YYYY-MM-DD.1.log`, and keeps writing to a fresh active log instead of
suppressing diagnostics.

You can override this in `~/.eve/eve.json`:

```json
{
  "logging": {
    "file": "/path/to/eve.log"
  }
}
```
...
or times out before `logs.tail` answers, `eve logs` falls back to the
configured Gateway file log automatically. Explicit `--url` targets do not use
this fallback. `eve logs --follow` is stricter: on Linux it uses the active
...
```bash
eve channels logs --channel whatsapp
```
...
EVE preserves the original structured log arguments alongside these fields
...
`eve gateway` also has WebSocket protocol logging for RPC traffic:
...
```bash
eve gateway
eve gateway --verbose --ws-log compact
eve gateway --verbose --ws-log full
```

## Configuring logging

All logging configuration lives under `logging` in `~/.eve/eve.json`.

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/eve/eve-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
You can override both via the **`EVE_LOG_LEVEL`** environment variable (e.g. `EVE_LOG_LEVEL=debug`). The env var takes precedence over the config file, so you can raise verbosity for a single run without editing `eve.json`. You can also pass the global CLI option **`--log-level <level>`** (for example, `eve --log-level debug gateway run`), which overrides the environment variable for that command.
...
```bash
EVE_DEBUG_MODEL_TRANSPORT=1 eve gateway
EVE_DEBUG_MODEL_PAYLOAD=tools EVE_DEBUG_SSE=events eve gateway
```

Available flags:

- `EVE_DEBUG_MODEL_TRANSPORT=1`: emit request start, fetch response, SDK
  headers, first streaming event, stream completion, and transport errors at
  `info` level.
- `EVE_DEBUG_MODEL_PAYLOAD=summary`: include a bounded request payload
  summary in model request logs.
- `EVE_DEBUG_MODEL_PAYLOAD=tools`: include all model-facing tool names in
  the payload summary.
- `EVE_DEBUG_MODEL_PAYLOAD=full-redacted`: include a redacted, capped JSON
  payload snapshot. Use only while debugging; secrets are redacted but prompts
  and message text may still be present.
- `EVE_DEBUG_SSE=events`: emit first-event and stream-completion timing.
- `EVE_DEBUG_SSE=peek`: also emit the first five redacted SSE event
  payloads, capped per event.
- `EVE_DEBUG_CODE_MODE=1`: emit code-mode model-surface diagnostics,
  including when native provider tools are hidden because code mode owns the
  tool surface.

These flags log through normal EVE logging, so `eve logs --follow`
...
EVE writes the trace fields as top-level JSON keys (`traceId`, `spanId`,
...
EVE can redact sensitive tokens before they hit console output, file logs,
...
policy. EVE still redacts safety-boundary payloads that can be shown to UI
...
- **Diagnostics flags** — targeted debug-log flags that route extra logs to
  ...
  or via the `EVE_DIAGNOSTICS=...` env override. Full guide:
...
- **Gateway not reachable?** Run `eve doctor` first.
