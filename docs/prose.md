...
summary: "OpenProse is a markdown-first workflow format for multi-agent AI sessions. In EVE it ships as a plugin with a /prose slash command and a skill pack."
read_when:
  - You want to run or write .prose workflow files
  - You want to enable the OpenProse plugin
  - You need to understand how OpenProse maps to EVE primitives
---

OpenProse is a portable, markdown-first workflow format for orchestrating AI
sessions. In EVE it ships as a plugin that installs an OpenProse skill
...
For a local checkout: `eve plugins install ./path/to/local/open-prose-plugin`
...
Treat `.prose` files like code. Review them before running. Use EVE tool
...
<CardGroup cols={2}>
  ...
  <Card title="Subagents" href="/tools/subagents" icon="users">
    EVE's native multi-agent coordination layer.
  </Card>
