...
On the first agent run, EVE bootstraps the workspace (default
`~/.eve/workspace`):
...
For embedded/local model runs, EVE keeps `BOOTSTRAP.md` out of the
...
To skip this for a pre-seeded workspace, run `eve onboard --skip-bootstrap`.
...
host (for example, `user@gateway-host:~/.eve/workspace`).
</Note>
