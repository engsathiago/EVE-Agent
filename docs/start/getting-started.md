---
summary: "Get EVE installed and run your first chat in minutes."
read_when:
...
Install EVE, run onboarding, and chat with your AI assistant — all in
...
```bash
mkdir -p "$HOME/.eve/control-ui-custom"
...
```json
{
  "gateway": {
    "controlUi": {
      "enabled": true,
      "root": "$HOME/.eve/control-ui-custom"
    }
  }
}
```
...
<Accordion title="Advanced: environment variables">
  If you run EVE as a service account or want custom paths:

- `EVE_HOME` — home directory for internal path resolution
- `EVE_STATE_DIR` — override the state directory
- `EVE_CONFIG_PATH` — override the config file path
