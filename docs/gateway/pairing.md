...
eve nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
changes (for example, display name or client platform hints), EVE treats
...
Pairing state is stored under the Gateway state directory (default `~/.eve`):

- `~/.eve/nodes/paired.json`
- `~/.eve/nodes/pending.json`

If you override `EVE_STATE_DIR`, the `nodes/` folder moves with it.
