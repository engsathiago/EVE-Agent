...
read_when:
  - Setting up EVE for the first time
  - Looking for common configuration patterns
  - Navigating to specific config sections
title: "Configuration"
---

EVE reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.eve/eve.json`.
The active config path must be a regular file. Symlinked `eve.json`
layouts are unsupported for EVE-owned writes; an atomic write may replace
the path instead of preserving the symlink. If you keep config outside the
default state directory, point `EVE_CONFIG_PATH` directly at the real file.

If the file is missing, EVE uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `eve onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    eve onboard       # full onboarding flow
    eve configure     # config wizard
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.eve/eve.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
EVE only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

`eve config schema` prints the canonical JSON Schema used by Control UI
...
- Only diagnostic commands work (`eve doctor`, `eve logs`, `eve health`, `eve status`)
- Run `eve doctor` to see exact issues
- Run `eve doctor --fix` (or `--yes`) to apply repairs

The Gateway keeps a trusted last-known-good copy after each successful startup,
but startup and hot reload do not restore it automatically. If `eve.json`
...
Run `eve doctor --fix` (or `--yes`) to repair prefixed/clobbered config or
...
<AccordionGroup>
  ...
  <Accordion title="Choose and configure models">
    ...
    - Use `eve config set agents.defaults.models '<json>' --strict-json --merge` to add allowlist entries without removing existing models. Plain replacements that would remove entries are rejected unless you pass `--replace`.
  ...
  <Accordion title="Tune gateway WebSocket handshake timeout">
    ...
    - `EVE_HANDSHAKE_TIMEOUT_MS` still takes precedence for one-off service or shell overrides.
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push uses the hosted EVE relay by default: `https://ios-push-relay.eve.ai`.
    ...
    ```bash
    eve config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `EVE_APNS_RELAY_BASE_URL` and `EVE_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - Custom gateway relay URLs must match the relay base URL baked into the official/TestFlight iOS build.
    - `EVE_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
  ...
  <Accordion title="Set up webhooks (hooks)">
    ...
    - Use a dedicated `hooks.token`; do not reuse active Gateway auth secrets (`gateway.auth.token` / `EVE_GATEWAY_TOKEN` or `gateway.auth.password` / `EVE_GATEWAY_PASSWORD`).
    - Hook auth is header-only (`Authorization: Bearer ...` or `x-eve-token`); query-string tokens are rejected.
  ...
  <Accordion title="Split config into multiple files ($include)">
    ...
    - **EVE-owned writes**: when a write changes only one top-level section
      backed by a single-file include such as `plugins: { $include: "./plugins.json5" }`,
      EVE updates that included file and leaves `eve.json` intact
    - **Unsupported write-through**: root includes, include arrays, and includes
      with sibling overrides fail closed for EVE-owned writes instead of
      flattening the config
    - **Confinement**: `$include` paths must resolve under the directory holding
      `eve.json`. To share a tree across machines or users, set
      `EVE_INCLUDE_ROOTS` to a path-list (`:` on POSIX, `;` on Windows) of
...
The Gateway watches `~/.eve/eve.json` and applies changes automatically - no manual restart needed for most settings.

Direct file edits are treated as untrusted until they validate. The watcher waits
for editor temp-write/rename churn to settle, reads the final file, and rejects
invalid external edits without rewriting `eve.json`. EVE-owned config
...
config`, inspect the config, run `eve config validate`, then run `eve
...
When you edit a source file that is referenced through `$include`, EVE plans
...
```bash
eve gateway call config.get --params '{}'  # capture payload.hash
eve gateway call config.patch --params '{
...
EVE reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.eve/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, EVE runs your login shell and imports only the missing keys:
...
Env var equivalent: `EVE_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${EVE_GATEWAY_TOKEN}" } },
