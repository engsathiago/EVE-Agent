...
EVE can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. EVE requires a shared password.
- `off`: Default (no Tailscale automation).

Status and audit output use **Tailscale exposure** for this EVE Serve/Funnel
mode. `off` means EVE is not managing Serve or Funnel; it does not mean the
...
- `token` (default when `EVE_GATEWAY_TOKEN` is set)
- `password` (shared secret via `EVE_GATEWAY_PASSWORD` or config)
...
(`tailscale-user-login`) without supplying a token/password. EVE verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
EVE only treats a request as Serve when it arrives from loopback with
...
`https://eve.<tailnet-name>.ts.net/` instead of the device hostname.
...
Prefer `EVE_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
eve gateway --tailscale serve
eve gateway --tailscale funnel --auth password
```
...
- `gateway.tailscale.serviceName` applies only to Serve mode and is passed to
  ...
  `svc:<dns-label>` Service name format, for example `svc:eve`.
...
- Set `gateway.tailscale.resetOnExit` if you want EVE to undo `tailscale serve`
...
- Set `gateway.tailscale.preserveFunnel: true` to keep an externally configured
  ...
  gateway runs in `mode: "serve"`, EVE checks `tailscale funnel status`
  before re-applying Serve and skips it when a Funnel route already covers the
  gateway port. The EVE-managed Funnel password-only policy is unchanged.
