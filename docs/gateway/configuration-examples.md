---
summary: "Schema-accurate configuration examples for common EVE setups"
read_when:
  - Learning how to configure EVE
...
Save to `~/.eve/eve.json` and you can DM the bot from that number.
...
```json5
{
  ...
  session: {
    ...
    store: "~/.eve/agents/default/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
  ...
  gateway: {
    ...
    port: 18789,
    bind: "loopback",
    controlUi: { enabled: true, basePath: "/eve" },
    auth: {
      mode: "token",
      token: "gateway-token",
      allowTailscale: true,
...
```json5
{
  agents: {
    ...
    list: [
      ...
      { id: "docs", workspace: "~/.eve/workspace-docs", skills: ["docs-search"] },
    ],
  },
}
```
