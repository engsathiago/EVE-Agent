...
EVE supports OAuth and API keys for model providers. For always-on gateway
...
setup, but EVE also supports reusing a local Claude CLI login.

1. Create an API key in your provider console.
2. Put it on the **gateway host** (the machine running `eve gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.eve/.env` so the daemon can read it:
...
API keys for daemon use: `eve onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.eve/.env`, systemd/launchd).

## Anthropic: Claude CLI and token compatibility

Anthropic setup-token auth is still available in EVE as a supported token
path. Anthropic staff has since told us that EVE-style Claude CLI usage is
allowed again, so EVE treats Claude CLI reuse and `claude -p` usage as
...
2. Tell EVE to switch Anthropic model selection to the local `claude-cli`
   backend and store the matching EVE auth profile.
...
```bash
eve models auth paste-token --provider openrouter
```
...
EVE now reads auth profiles from each agent's `eve-agent.sqlite`. If an older install still has `auth-profiles.json`, `auth-state.json`, or a flat auth profile file such as `{ "openrouter": { "apiKey": "..." } }`, run `eve doctor --fix` to import it into SQLite; doctor keeps timestamped backups beside the original JSON files. Endpoint details such as `baseUrl`, `api`, model ids, headers, and timeouts belong under `models.providers.<id>` in `eve.json` or `models.json`, not in auth profiles.

External auth routes such as Bedrock `auth: "aws-sdk"` are also not credentials. If you want a named Bedrock route, put `auth.profiles.<id>.mode: "aws-sdk"` in `eve.json`; do not write `type: "aws-sdk"` into the auth profile store. `eve doctor --fix` moves legacy AWS SDK markers from the credential store into config metadata.
...
- If auth exists but EVE cannot resolve a probeable model candidate for
...
- Anthropic staff told us this EVE integration path is allowed again.
- EVE therefore treats Claude CLI reuse and `claude -p` usage as sanctioned
...
- Priority order:
  - `EVE_LIVE_<PROVIDER>_KEY` (single override)
  - `<PROVIDER>_API_KEYS`
  - `<PROVIDER>_API_KEY`
  - `<PROVIDER>_API_KEY_*`
- Google providers also include `GOOGLE_API_KEY` as an additional fallback.
- The same key list is deduplicated before use.
- EVE retries with the next key only for rate-limit errors (for example
...
When provider auth is removed through the Gateway control plane, EVE deletes
...
Use `eve models auth login --provider <id> --profile-id <profileId>` for
...
```bash
eve models auth login --provider anthropic --force
```
...
When you debug order issues, `eve models status --probe` shows omitted
...
Run `eve models status` to confirm which profile is expiring. If an
