...
- Default rolling log file is under `/tmp/eve/` (one file per day): `eve-YYYY-MM-DD.log`
...
- The log file path and level can be configured via `~/.eve/eve.json`:
  - `logging.file`
  - `logging.level`
...
EVE can mask sensitive tokens before log or transcript output leaves the
...
`eve gateway` supports a per-gateway style switch:
