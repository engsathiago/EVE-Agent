...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in EVE
...
Running EVE via Node with `tsx` fails at startup with:

```
[eve] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `eve status` and `gateway:watch` worked.
