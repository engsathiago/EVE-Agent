---
summary: "Run EVE on a Linux server or cloud VPS — provider picker, architecture, and tuning"
read_when:
  - You want to run the Gateway on a Linux server or cloud VPS
  - You need a quick map of hosting guides
  - You want generic Linux server tuning for EVE
title: "Linux server"
sidebarTitle: "Linux Server"
---

Run the EVE Gateway on any Linux server or cloud VPS. This page helps you
...
Before you install EVE on a public VPS, decide how you want to administer
the box itself.
...
- This is separate from Gateway access. You can still keep EVE bound to
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/eve-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- Add service env for a stable startup path:
  - `EVE_NO_RESPAWN=1`
  - `NODE_COMPILE_CACHE=/var/tmp/eve-compile-cache`
...
For the standard `eve onboard --install-daemon` path, edit the user unit:
...
`eve-gateway.service` via `sudo systemctl edit eve-gateway.service`.
