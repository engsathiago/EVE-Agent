...
- repo: `eve/docs`
- local checkout: `~/Projects/eve-docs`
...
- Docs translation workflows generate the same non-English locale set in `eve/docs`.
...
- Do not treat missing `th` or `fa` entries in generated `docs/docs.json` as a pipeline failure. Verify their generated folders in `eve/docs` instead.
...
- Sync metadata is written to `.eve-sync/source.json` in the publish repo.
- Source repo secret: `EVE_DOCS_SYNC_TOKEN`
- Publish repo secret: `EVE_DOCS_I18N_OPENAI_API_KEY`
- If locale output looks stale, check the `Translate All` workflow in `eve/docs` first.
