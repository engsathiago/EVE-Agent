...
read_when:
  ...
  - Reviewing EVE security posture
title: "Pairing"
---

"Pairing" is EVE's explicit access approval step.
...
Supported channels: `discord`, `feishu`, `googlechat`, `imessage`, `irc`, `line`, `matrix`, `mattermost`, `msteams`, `nextcloud-talk`, `nostr`, `eve-weixin`, `signal`, `slack`, `synology-chat`, `telegram`, `twitch`, `whatsapp`, `zalo`, `zalouser`.
...
3. On your phone, open the EVE iOS app → Settings → Gateway.
...
<Note>
An already paired device does not get broader access silently. If it reconnects asking for more scopes or a broader role, EVE keeps the existing approval as-is and creates a fresh pending upgrade request. Use `eve devices list` to compare the currently approved access with the newly requested access before you approve.
</Note>
...
- The legacy `node.pair.*` API (CLI: `eve nodes pending|approve|reject|remove|rename`) is a
