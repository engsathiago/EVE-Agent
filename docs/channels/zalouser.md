...
read_when:
  - Setting up Zalo Personal for EVE
  - Debugging Zalo Personal login or message flow
title: "Zalo personal"
---

Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside EVE.
...
Zalo Personal ships as a bundled plugin in current EVE releases, so normal
packaged builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo Personal,
install the npm package directly:

- Install via CLI: `eve plugins install @eve/zalouser`
- Pinned version: `eve plugins install @eve/zalouser@2026.5.2`
- Or from a source checkout: `eve plugins install ./path/to/local/zalouser-plugin`
...
1. Ensure the Zalo Personal plugin is available.
   - Current packaged EVE releases already bundle it.
   - Older/custom installs can add it manually with the commands above.
2. Login (QR, on the Gateway machine):
   - `eve channels login --channel zalouser`
...
```bash
eve directory self --channel zalouser
eve directory peers list --channel zalouser --query "name"
eve directory groups list --channel zalouser --query "work"
```
...
- `eve pairing list zalouser`
- `eve pairing approve zalouser <code>`
...
- On startup, EVE resolves group/user names in allowlists to IDs and logs the mapping only when `channels.zalouser.dangerouslyAllowNameMatching: true` is enabled.
...
- When a group message is skipped because mention is required, EVE stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in EVE state. Example:
...
Profile names select the saved Zalo login credentials in EVE state. Resolution order is:
...
- EVE sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, EVE sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `eve channels logout --channel zalouser && eve channels login --channel zalouser`
...
- The channel now runs fully in EVE without external CLI binaries.
