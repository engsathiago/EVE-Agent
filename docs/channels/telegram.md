...
```
    ...
    Telegram does **not** use `eve channels login telegram`; configure token in config/env, then start gateway.
...
```
  ...
  <Step title="Add the bot to a group">
    ...
    For first-time setup, get the group chat ID from `eve logs --follow`, a forwarded-ID bot, or Bot API `getUpdates`. After the group is allowed, `/whoami@<bot_username>` can confirm the user and group IDs.
...
After a successful startup, EVE caches the bot identity in the state directory for up to 24 hours so restarts can avoid an extra Telegram `getMe` call; changing or removing the token clears that cache.
</Note>
...
In Telegram groups and forum topics, an explicit mention of the configured bot handle (for example `@my_bot`) is treated as addressing the selected EVE agent, even when the agent persona name differs from the Telegram username. The group silence policy still applies to unrelated group traffic, but the bot handle itself is not considered "someone else."
...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `eve doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
    If you previously relied on pairing-store allowlist files, `eve doctor --fix` can recover entries into `channels.telegram.allowFrom` in allowlist flows (for example when `dmPolicy: "allowlist"` has no explicit IDs yet).
...
```
    ...
    - or read `chat.id` from `eve logs --follow`
...
- DM messages can carry `message_thread_id`; EVE preserves it for replies. DM topic sessions split only when Telegram `getMe` reports `has_topics_enabled: true` for the bot; otherwise DMs stay on the flat session.
...
- Long polling is guarded inside each gateway process so only one active poller can use a bot token at a time. If you still see `getUpdates` 409 conflicts, another EVE gateway, script, or external poller is likely using the same token.
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    EVE can stream partial replies in real time:
    ...
    - legacy `channels.telegram.streamMode`, boolean `streaming` values, and retired native draft preview keys are detected; run `eve doctor --fix` to migrate them to current streaming config

    Tool-progress preview updates are the short status lines shown while tools run, for example command execution, file reads, planning updates, patch summaries, or Codex preamble/commentary text in Codex app-server mode. Telegram keeps these enabled by default to match released EVE behavior from `v2026.4.22` and later.
    ...
    - if the final edit fails before the completed text is confirmed, EVE uses normal final delivery and cleans up the stale preview

    For complex replies (for example media payloads), EVE falls back to normal final delivery and then cleans up the preview message.

    Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, EVE skips the preview stream to avoid double-streaming.
...
```
    ...
    - Markdown text is rendered through EVE's Markdown IR and sent as Telegram rich HTML.
...
```
    ...
    - `deleteWebhook`, `deleteMyCommands`, or `setMyCommands` failing with `404: Not Found` while direct Bot API curl commands work can mean `channels.telegram.apiRoot` was set to the full `/bot<TOKEN>` endpoint. `apiRoot` must be only the Bot API root, and `eve doctor --fix` removes an accidental trailing `/bot<TOKEN>`.
    - `getMe returned 401` means Telegram rejected the configured bot token. Update `botToken`, `tokenFile`, or `TELEGRAM_BOT_TOKEN` with the current BotFather token; EVE stops before polling so this is not reported as a webhook cleanup failure.
...
```
  ...
  <Accordion title="Forum topics and thread behavior">
    ...
    **Thread-bound ACP spawn from chat**: `/acp spawn <agent> --thread here|auto` binds the current topic to a new ACP session; follow-ups route there directly. EVE pins the spawn confirmation in-topic. Requires `channels.telegram.threadBindings.spawnSessions` to remain enabled (default: `true`).
    ...
    The former `dm.threadReplies` and `direct.*.threadReplies` overrides are intentionally retired; use BotFather threaded mode as the single source of truth and run `eve doctor --fix` to remove stale config keys.
...
```
    ...
    Sticker descriptions are cached in EVE SQLite plugin state to reduce repeated vision calls.
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, EVE enqueues system events like:
  ...
  <Accordion title="Ack reactions">
    `ackReaction` sends an acknowledgement emoji while EVE is processing an inbound message. `ackReactionScope` decides *when* that emoji is actually sent.
...
```
  ...
  <Accordion title="Long polling vs webhook">
    ...
    In long-polling mode EVE persists its restart watermark only after an update dispatches successfully. If a handler fails, that update remains retryable in the same process and is not written as completed for restart dedupe.
    ...
    EVE then processes the update asynchronously through the same per-chat/per-topic bot lanes used by long polling, so slow agent turns do not hold Telegram's delivery ACK.
  ...
  <Accordion title="Limits, retry, and CLI targets">
    ...
    - `channels.telegram.mediaGroupFlushMs` (default 500) controls how long Telegram albums/media groups are buffered before EVE dispatches them as one inbound message. Increase it if album parts arrive late; decrease it to reduce album reply latency.
...
```bash
eve message send --channel telegram --target 123456789 --message "hi"
eve message send --channel telegram --target @name --message "hi"
eve message send --channel telegram --target -1001234567890:topic:42 --message "hi topic"
```

    Telegram polls use `eve message poll` and support forum topics:

```bash
eve message poll --channel telegram --target 123456789 \
  --poll-question "Ship it?" --poll-option "Yes" --poll-option "No"
eve message poll --channel telegram --target -1001234567890:topic:42 \
...
```
  ...
  <Accordion title="Exec approvals in Telegram">
    ...
    Channel delivery shows the command text in the chat; only enable `channel` or `both` in trusted groups/topics. When the prompt lands in a forum topic, EVE preserves the topic for the approval prompt and the follow-up. Exec approvals expire after 30 minutes by default.
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `eve channels status` warns when config expects unmentioned group messages.
    - `eve channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `eve logs --follow` for skip reasons
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, EVE now retries these as recoverable network errors.
    - During polling startup, EVE reuses the successful startup `getMe` probe for grammY so the runner does not need a second `getMe` before the first `getUpdates`.
    - If `deleteWebhook` fails with a transient network error during polling startup, EVE continues into long polling instead of making another pre-poll control-plane call. A still-active webhook surfaces as a `getUpdates` conflict; EVE then rebuilds the Telegram transport and retries webhook cleanup.
    ...
    - If logs include `Polling stall detected`, EVE restarts polling and rebuilds the Telegram transport after 120 seconds without completed long-poll liveness by default.
    - `eve channels status --probe` and `eve doctor` warn when a running polling account has not completed `getUpdates` after startup grace, when a running webhook account has not completed `setWebhook` after startup grace, or when the last successful polling transport activity is stale.
    ...
    - If the EVE managed proxy is configured through `EVE_PROXY_URL` for a service environment and no standard proxy env is present, Telegram uses that URL for Bot API transport too.
...
```
    ...
    - Node 22+ defaults to `autoSelectFamily=true` (except WSL2). Telegram DNS result order honors `EVE_TELEGRAM_DNS_RESULT_ORDER`, then `channels.telegram.network.dnsResultOrder`, then the process default such as `NODE_OPTIONS=--dns-result-order=ipv4first`; if none applies, Node 22+ falls back to `ipv4first`.
...
```
    ...
    - Environment overrides (temporary):
      - `EVE_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY=1`
      - `EVE_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY=1`
      - `EVE_TELEGRAM_DNS_RESULT_ORDER=ipv4first`
