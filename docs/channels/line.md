...
read_when:
  - You want to connect EVE to LINE
  - You need LINE webhook + credential setup
  - You want LINE-specific message options
title: LINE
---

LINE connects to EVE via the LINE Messaging API. The plugin runs as a webhook
...
```bash
eve plugins install @eve/line
```

Local checkout (when running from a git repo):

```bash
eve plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so EVE applies strict pre-auth body limits and timeout before verification.
- EVE processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
eve pairing list line
eve pairing approve line <CODE>
```
...
- Inbound media is saved under `~/.eve/media/inbound/` before it is passed
...
Outbound media URLs must be public HTTPS URLs. EVE validates the target hostname before handing the URL to LINE and rejects loopback, link-local, and private-network targets.
