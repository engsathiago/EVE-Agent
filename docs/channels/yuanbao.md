...
Tencent Yuanbao is Tencent's AI assistant platform. The EVE channel plugin
connects Yuanbao bots to EVE over WebSocket so they can interact with users
...
> **Requires EVE 2026.4.10 or above.** Run `eve --version` to check. Upgrade with `eve update`.

<Steps>
  <Step title="Add the Yuanbao channel with your credentials">
  ```bash
  eve channels add --channel yuanbao --token "appKey:appSecret"
  ```
...
```bash
eve channels login --channel yuanbao
```
...
```bash
eve pairing list yuanbao
eve pairing approve yuanbao <CODE>
```
