...
Nextcloud Talk ships as a bundled plugin in current EVE releases, so
...
```bash
eve plugins install @eve/nextcloud-talk
```
...
```bash
eve plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "EVE" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction
   ```
...
4. Configure EVE:
   ...
   ```bash
   eve channels add --channel nextcloud-talk \
...
- Approve via:
  - `eve pairing list nextcloud-talk`
  - `eve pairing approve nextcloud-talk <CODE>`
