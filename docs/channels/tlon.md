...
Tlon is a decentralized messenger built on Urbit. EVE connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current EVE releases, so normal packaged
...
```bash
eve plugins install ./path/to/local/tlon-plugin
```
...
By default, EVE blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `eve message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, EVE replies in-thread.
