...
EVE routes replies **back to the channel where a message came from**. The
...
EVE infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, EVE still records inbound session metadata, but it
...
when a guarded path must not create a new EVE session. In that mode,
EVE may update metadata and `lastRoute` for an existing session, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when EVE would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.eve/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.eve`):

- `~/.eve/agents/<agentId>/sessions/sessions.json`
