...
This hub links the core docs for how EVE connects, pairs, and secures
devices across localhost, LAN, and tailnet.

## Core model

Most operations flow through the Gateway (`eve gateway`), a single long-running process that owns channel connections and the WebSocket control plane.
...
- **Canvas host** is served on the same port as the Gateway (`/__eve__/canvas/`, `/__eve__/a2ui/`), protected by Gateway auth when bound beyond loopback.
