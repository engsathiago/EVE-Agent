---
summary: "Export redacted trajectory bundles for debugging an EVE agent session"
read_when:
  - Debugging why an agent answered, failed, or called tools a certain way
  - Exporting a support bundle for an EVE session
...
Trajectory capture is EVE's per-session flight recorder. It records a
...
EVE writes the bundle under the workspace:

```text
.eve/trajectory-exports/eve-trajectory-<session>-<timestamp>/
```
...
The custom path is resolved inside `.eve/trajectory-exports/`. Absolute
...
create the bundle; do not use allow-all. In group chats, EVE sends the
...
Trajectory capture is on by default for EVE agent runs.
...
| `metadata.json`       | EVE version, OS/runtime, model, config snapshot, plugins, skills, and prompt metadata          |
...
EVE also writes a best-effort pointer file beside the session:

```text
<session>.trajectory-path.json
```

Set `EVE_TRAJECTORY_DIR` to store runtime trajectory sidecars in a
dedicated directory:

```bash
export EVE_TRAJECTORY_DIR=/var/lib/eve/trajectories
```

When this variable is set, EVE writes one JSONL file per session id in that
directory.
...
Set `EVE_TRAJECTORY=0` before starting EVE:

```bash
export EVE_TRAJECTORY=0
```
...
EVE flushes runtime trajectory sidecars during agent cleanup. The default
cleanup timeout is 10,000 ms. On slow disks or large stores, set
`EVE_TRAJECTORY_FLUSH_TIMEOUT_MS` before starting EVE:

```bash
export EVE_TRAJECTORY_FLUSH_TIMEOUT_MS=30000
```

This controls when EVE logs an `eve-trajectory-flush` timeout and continues.
It does not change the trajectory size caps. To tune all agent cleanup steps
that do not pass an explicit timeout, set `EVE_AGENT_CLEANUP_TIMEOUT_MS`.

## Privacy and limits

Trajectory bundles are designed for support and debugging, not public posting.
EVE redacts sensitive values before writing export files:
...
- confirm EVE was started without `EVE_TRAJECTORY=0`
- check whether `EVE_TRAJECTORY_DIR` points to a writable directory
...
- do not pass `/tmp/...` or `~/...`
- keep the export inside `.eve/trajectory-exports/`
