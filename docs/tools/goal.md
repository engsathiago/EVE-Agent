...
read_when:
  - You want EVE to keep one objective visible across a long session
...
A **goal** is one durable objective attached to the current EVE session.
...
EVE waits for the next fresh session token snapshot and uses that as the
...
cost reporting, and context-window behavior still use the normal EVE
usage and model controls.

## Model tools

EVE exposes three core goal tools to agent harnesses:
...
The `/goal` command works in command-capable EVE sessions, including the
...
token snapshot yet. Usage refreshes as EVE records session usage and
