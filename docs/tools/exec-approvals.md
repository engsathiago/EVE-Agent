...
- **Gateway host** → `eve` process on the gateway machine.
...
- For shell scripts and direct interpreter/runtime file invocations, EVE also tries to bind one concrete local file operand. If that bound file changes after approval but before execution, the run is denied instead of executing drifted content.
...
`EVE_STATE_DIR` is set, the file follows that state directory;
otherwise it uses the default EVE state directory:

```text
$EVE_STATE_DIR/exec-approvals.json
# otherwise
~/.eve/exec-approvals.json
```

The default approval socket follows the same root:
`$EVE_STATE_DIR/exec-approvals.sock`, or
`~/.eve/exec-approvals.sock` when the variable is unset.
...
- `auto` - use allowlist policy, run deterministic matches directly, and send approval misses through EVE's native auto reviewer before falling back to a human approval route.
...
<ParamField path="askFallback" type='"deny" | "allowlist" | "full"'>
  ...
  field is omitted, EVE defaults to `deny`.
...
<ParamField path="strictInlineEval" type="boolean">
  When `true`, EVE treats inline code-eval forms as approval-only
...
<ParamField path="commandHighlighting" type="boolean" default="false">
  ...
  EVE may attach parser-derived command spans so Web approval
...
**both** policy layers - requested exec policy in EVE config
(`tools.exec.*`) **and** host-local approvals policy in
the execution host approvals file.

EVE defaults omitted `askFallback` to `deny`. Set host
...
- In YOLO mode, EVE does **not** add a separate heuristic command-obfuscation approval gate or script-preflight rejection layer on top of the configured host exec policy.
...
`--permission-mode bypassPermissions` when EVE's effective exec
policy is YOLO. For EVE-managed Claude live sessions, EVE's
...
If you want a more conservative setup, tighten EVE exec policy back to
...
approvals remotely, use `eve approvals set --gateway` or
`eve approvals set --node <id|name|ip>`.
...
```bash
eve approvals set --node <id|name|ip> --stdin <<'EOF'
{
  version: 1,
  defaults: {
    security: "full",
...
- `eve exec-policy` does not synchronize node approvals.
- `eve exec-policy set --host node` is rejected.
- Node exec approvals are fetched from the node at runtime, so node-targeted updates must use `eve approvals --node ...`.
...
specific argument shape. EVE evaluates the regular expression
...
entries instead of hand-editing the encoded value. If EVE cannot
...
CLI: `eve approvals` supports gateway or node editing - see
...
EVE posts the denial back into that session as an internal followup so the
agent can stop waiting on the async command and avoid a missing-result repair.
If there is no session or the session cannot be resumed, EVE can still
...
When an async exec approval is denied, EVE treats the host command as
...
session delivery is unavailable, EVE falls back to a concise operator or
