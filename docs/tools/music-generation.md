...
For session-backed agent runs, EVE starts music generation as a
...
from the completion reply, EVE sends an idempotent direct fallback with
...
<Note>
Not all providers support all parameters. EVE still validates hard
limits such as input counts before submission. When a provider supports
duration but uses a shorter maximum than the requested value, EVE
...
Provider request timeouts are operator configuration only. EVE uses
...
- **Status lookup:** `eve tasks list` or `eve tasks show <taskId>`
  inspects queued, running, and terminal status.
- **Completion wake:** EVE injects an internal completion event back
...
```bash
EVE_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts
```
