...
When a site requires login, **sign in manually** in the **host** browser profile (the eve browser).
...
EVE controls a **dedicated Chrome profile** (named `eve`, orange-tinted UI). This is separate from your daily browser profile.

For agent browser tool calls:

- Default choice: the agent should use its isolated `eve` browser.
...
```bash
eve browser start
eve browser open https://x.com
```

If you have multiple profiles, pass `--browser-profile <name>` (the default is `eve`).
...
```bash
eve browser open https://x.com --browser-profile eve
```
