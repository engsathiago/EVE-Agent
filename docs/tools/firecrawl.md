...
EVE can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `eve configure --section web` enables the installed Firecrawl plugin automatically.
...
- The explicitly selected Firecrawl `web_fetch` fallback works without an API key. When configured, EVE sends `plugins.entries.firecrawl.config.webFetch.apiKey` or `FIRECRAWL_API_KEY` for higher limits.
- Choosing Firecrawl during onboarding or `eve configure --section web` enables the plugin and selects Firecrawl for `web_fetch` unless another fetch provider is already configured.
- `firecrawl_scrape` requires an API key.
- `maxAgeMs` controls how old cached results can be (ms). Default is 2 days.
- Legacy `tools.web.fetch.firecrawl.*` config is auto-migrated by `eve doctor --fix`.
...
when you run Firecrawl yourself. EVE accepts `http://` only for loopback,
...
EVE always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
...
The selection knob is `tools.web.fetch.provider`. If you omit it, EVE
