...
EVE supports Kimi as a `web_search` provider, using Moonshot web search
...
`https://api.moonshot.cn/v1`), EVE reuses that same host for Kimi
...
Gateway environment. For a gateway install, put it in `~/.eve/.env`.

If you omit `baseUrl`, EVE defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, EVE defaults to `kimi-k2.6`.
...
EVE treats Kimi `web_search` as successful only after Moonshot returns
...
EVE returns a structured `kimi_web_search_ungrounded` error instead of
