...
read_when:
  - Looking for an overview of EVE's media capabilities
...
EVE generates images, videos, and music, understands inbound media
...
For async tools, EVE submits the request to the provider, returns a task
...
EVE wakes the agent with the generated media paths so it can tell the
...
EVE sends an idempotent direct fallback with only the missing media. Media
...
<AccordionGroup>
  ...
  <Accordion title="DeepInfra">
    ...
    registered until EVE has dedicated provider contracts for those
    categories.
  </Accordion>
  <Accordion title="xAI">
    ...
    not registered in EVE until the shared realtime-voice contract can
    represent it.
  </Accordion>
</AccordionGroup>
