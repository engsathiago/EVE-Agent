...
read_when:
  - Running EVE Gateway in WSL2 while Chrome lives on Windows
...
In the common split-host setup, EVE Gateway runs inside WSL2, Chrome runs on Windows, and browser control must cross the WSL2 and Windows boundary. The layered failure pattern from [issue #39369](https://github.com/engsathiago/eve-agent/issues/39369) means several independent problems can show up at once, which makes the wrong layer look broken first.
...
- EVE and Chrome are on the same machine
...
- EVE points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, EVE is not the problem yet.
...
Fix that before touching EVE config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point EVE at the address that is reachable from WSL2:
...
- use HTTP(S) when you want EVE to discover `/json/version`
- use WS(S) only when the browser provider gives you a direct DevTools socket URL
- test the same URL with `curl` before expecting EVE to succeed
...
```bash
eve browser open https://example.com --browser-profile remote
eve browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `eve browser tabs` returns the target
...
- stale viewport / dark-mode / locale / offline overrides after a remote session
  - run `eve browser stop --browser-profile remote`
...
3. EVE config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug EVE config or Control UI auth
