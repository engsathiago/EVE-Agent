...
title: "Tokenjuice"
read_when:
  - You want shorter `exec` or `bash` tool results in EVE
...
Today this applies to EVE embedded runs and EVE dynamic tools in the Codex
app-server harness. Tokenjuice hooks EVE's tool-result middleware and
...
```bash
eve plugins install clawhub:@eve/tokenjuice
```
...
```bash
eve config set plugins.entries.tokenjuice.enabled false
```

Or:

```bash
eve plugins disable tokenjuice
```
