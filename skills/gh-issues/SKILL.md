...
if [ -z "${GH_TOKEN:-}" ]; then
  CONFIG_PATH="${EVE_CONFIG_PATH:-${EVE_STATE_DIR:-$HOME/.eve}/eve.json}"
...
```text
${EVE_STATE_DIR:-$HOME/.eve}/gh-issues-<owner>-<repo>.json
```
...
Send completion/failure with eve message send if route provided.
```
