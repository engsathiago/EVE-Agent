...
1. Download the runtime for your OS (extracts into `$EVE_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.eve/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$EVE_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.eve/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active EVE config file (`$EVE_CONFIG_PATH`, default `~/.eve/eve.json`):
