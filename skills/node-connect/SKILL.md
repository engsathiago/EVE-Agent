---
name: node-connect
description: "Diagnose EVE Android, iOS, or macOS node pairing, QR/setup code, route, auth, and connection failures."
---

# Node Connect

Goal: find the one real route from node -> gateway, verify EVE is advertising that route, then fix pairing/auth.
...
- whether `eve devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `eve qr --json`. It uses the same setup-code payload Android scans.

```bash
eve config get gateway.mode
eve config get gateway.bind
eve config get gateway.tailscale.mode
eve config get gateway.remote.url
eve config get gateway.auth.mode
eve config get gateway.auth.allowTailscale
eve config get plugins.entries.device-pair.config.publicUrl
eve qr --json
eve devices list
eve nodes status
```

If this EVE instance is pointed at a remote gateway, also run:
...
If `eve qr --json` says `Gateway is only bound to loopback`:
...
```bash
eve devices list
eve devices approve --latest   # preview only; copy the requestId from output
...
- `eve devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run eve qr again, rescan, then approve the pending device pairing.`
