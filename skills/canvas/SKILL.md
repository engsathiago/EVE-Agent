---
name: canvas
description: "Present HTML on connected EVE node canvases, navigate/eval/snapshot, and debug canvas host URLs."
metadata: { "eve": { "emoji": "🖼️" } }
---
...
Active config: `$EVE_CONFIG_PATH` or `~/.eve/eve.json`.
...
4. Present the hosted URL: `/__eve__/canvas/<file>.html`.
5. Use `snapshot` when the user needs proof.

## URL shape

```text
http://<gateway-host>:<gateway.port>/__eve__/canvas/index.html
http://<gateway-host>:<gateway.port>/__eve__/canvas/games/snake.html
```

Path mapping:

- `/__eve__/canvas/index.html` -> `<canvas host root>/index.html`
- `/__eve__/canvas/games/snake.html` -> `<canvas host root>/games/snake.html`
