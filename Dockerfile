...
# Example: docker build --build-arg EVE_EXTENSIONS="diagnostics-otel,matrix" .
#
...
ARG EVE_EXTENSIONS=""
ARG EVE_BUNDLED_PLUGIN_DIR=extensions
ARG EVE_NODE_BOOKWORM_IMAGE="docker.io/library/node:24-bookworm@sha256:8530f76a96d88820d288761f022e318970dda93d01536919fbc16076b7983e63"
ARG EVE_NODE_BOOKWORM_SLIM_IMAGE="docker.io/library/node:24-bookworm-slim@sha256:242549cd46785b480c832479a730f4f2a20865d61ea2e404fdb2a5c3d3b73ecf"
ARG EVE_NODE_BOOKWORM_SLIM_DIGEST="sha256:242549cd46785b480c832479a730f4f2a20865d61ea2e404fdb2a5c3d3b73ecf"
# Keep in sync with .github/actions/setup-node-env/action.yml bun-version.
# To update: docker buildx imagetools inspect docker.io/oven/bun:<version> and use the manifest-list digest.
ARG EVE_BUN_IMAGE="docker.io/oven/bun:1.3.13@sha256:87416c977a612a204eb54ab9f3927023c2a3c971f4f345a01da08ea6262ae30e"
...
FROM ${EVE_NODE_BOOKWORM_IMAGE} AS workspace-deps
ARG EVE_EXTENSIONS
ARG EVE_BUNDLED_PLUGIN_DIR
...
RUN --mount=type=bind,source=packages,target=/tmp/packages,readonly \
    --mount=type=bind,source=${EVE_BUNDLED_PLUGIN_DIR},target=/tmp/${EVE_BUNDLED_PLUGIN_DIR},readonly \
    mkdir -p /out/packages "/out/${EVE_BUNDLED_PLUGIN_DIR}" && \
    ...
    for ext in $(printf '%s\n' "$EVE_EXTENSIONS" | tr ',' ' '); do \
      if [ -f "/tmp/${EVE_BUNDLED_PLUGIN_DIR}/$ext/package.json" ]; then \
        mkdir -p "/out/${EVE_BUNDLED_PLUGIN_DIR}/$ext" && \
        cp "/tmp/${EVE_BUNDLED_PLUGIN_DIR}/$ext/package.json" "/out/${EVE_BUNDLED_PLUGIN_DIR}/$ext/package.json"; \
      fi; \
    done

# ── Stage 2: Build ──────────────────────────────────────────────
FROM ${EVE_BUN_IMAGE} AS bun-binary
FROM ${EVE_NODE_BOOKWORM_IMAGE} AS build
ARG EVE_BUNDLED_PLUGIN_DIR
ARG EVE_EXTENSIONS
...
COPY --from=workspace-deps /out/${EVE_BUNDLED_PLUGIN_DIR}/ ./${EVE_BUNDLED_PLUGIN_DIR}/

# Reduce OOM risk on low-memory hosts during dependency installation.
# Docker builds on small VMs may otherwise fail with "Killed" (exit 137).
RUN --mount=type=cache,id=eve-pnpm-store,target=/root/.local/share/pnpm/store,sharing=locked \
...
RUN for dir in /app/${EVE_BUNDLED_PLUGIN_DIR} /app/.agent /app/.agents; do \
      if [ -d "$dir" ]; then \
...
RUN pnpm_config_verify_deps_before_run=false pnpm canvas:a2ui:bundle || \
    (echo "A2UI bundle: creating stub (non-fatal)" && \
     ...
     rm -rf vendor/a2ui apps/shared/EVEKit/Tools/CanvasA2UI)
RUN if printf '%s\n' "$EVE_EXTENSIONS" | tr ',' ' ' | tr ' ' '\n' | grep -qx 'qa-lab'; then \
      export EVE_BUILD_PRIVATE_QA=1 EVE_ENABLE_PRIVATE_QA_CLI=1; \
    fi && \
    NODE_OPTIONS=--max-old-space-size=8192 pnpm_config_verify_deps_before_run=false pnpm build:docker
# Force pnpm for UI build (Bun may fail on ARM/Synology architectures)
ENV EVE_PREFER_PNPM=1
...
RUN --mount=type=cache,id=eve-pnpm-store,target=/root/.local/share/pnpm/store,sharing=locked \
    ...
    CI=true pnpm prune --prod \
      ...
      --config.supportedArchitectures.cpu="$(node -p 'process.arch')" \
      --config.supportedArchitectures.libc=glibc && \
    EVE_EXTENSIONS="$EVE_EXTENSIONS" EVE_BUNDLED_PLUGIN_DIR="$EVE_BUNDLED_PLUGIN_DIR" node scripts/prune-docker-plugin-dist.mjs && \
...
ARG EVE_NODE_BOOKWORM_SLIM_DIGEST
LABEL org.opencontainers.image.base.name="docker.io/library/node:24-bookworm-slim" \
  org.opencontainers.image.base.digest="${EVE_NODE_BOOKWORM_SLIM_DIGEST}"
...
# - https://docs.eve.ai/install/docker
LABEL org.opencontainers.image.source="https://github.com/engsathiago/eve-agent" \
  org.opencontainers.image.url="https://eve.ai" \
  org.opencontainers.image.documentation="https://docs.eve.ai/install/docker" \
  org.opencontainers.image.licenses="MIT" \
  org.opencontainers.image.title="EVE" \
  org.opencontainers.image.description="EVE gateway and CLI runtime container image"
...
RUN --mount=type=cache,id=eve-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=eve-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    apt-get update && \
...
COPY --from=runtime-assets --chown=node:node /app/eve.mjs .
COPY --from=runtime-assets --chown=node:node /app/src/agents/templates ./src/agents/templates
COPY --from=runtime-assets --chown=node:node /app/${EVE_BUNDLED_PLUGIN_DIR} ./${EVE_BUNDLED_PLUGIN_DIR}
...
# Example: docker build --build-arg EVE_IMAGE_APT_PACKAGES="python3 wget" .
# Legacy alias: EVE_DOCKER_APT_PACKAGES is still accepted as a fallback.
ARG EVE_IMAGE_APT_PACKAGES
ARG EVE_DOCKER_APT_PACKAGES=""
RUN --mount=type=cache,id=eve-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=eve-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    packages="${EVE_IMAGE_APT_PACKAGES-$EVE_DOCKER_APT_PACKAGES}"; \
    if [ -n "$packages" ]; then \
      apt-get update && \
...
# Example: docker build --build-arg EVE_IMAGE_PIP_PACKAGES="requests humanize" .
ARG EVE_IMAGE_PIP_PACKAGES=""
RUN --mount=type=cache,id=eve-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=eve-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
...
# Build with: docker build --build-arg EVE_INSTALL_BROWSER=1 ...
# Adds ~300MB but eliminates the 60-90s Playwright install on every container start.
# Must run after node_modules COPY so playwright-core is available.
ARG EVE_INSTALL_BROWSER=""
...
# Build with: docker build --build-arg EVE_INSTALL_DOCKER_CLI=1 ...
# Adds ~50MB. Only the CLI is installed — no Docker daemon.
# Required for agents.defaults.sandbox to function in Docker deployments.
ARG EVE_INSTALL_DOCKER_CLI=""
ARG EVE_DOCKER_GPG_FINGERPRINT="9DC858229FC7DD38854AE2D88D81803C0EBFCD88"
RUN --mount=type=cache,id=eve-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=eve-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    if [ -n "$EVE_INSTALL_DOCKER_CLI" ]; then \
      apt-get update && \
      DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends \
      ...
      # Update EVE_DOCKER_GPG_FINGERPRINT when Docker rotates release keys.
      curl -fsSL https://download.docker.com/linux/debian/gpg -o /tmp/docker.gpg.asc && \
      expected_fingerprint="$(printf '%s' "$EVE_DOCKER_GPG_FINGERPRINT" | tr '[:lower:]' '[:upper:]' | tr -d '[:space:]')" && \
...
RUN ln -sf /app/eve.mjs /usr/local/bin/eve \
 && chmod 755 /app/eve.mjs
...
# the leaf /home/node/.config/eve inherits the correct parent permissions.
...
RUN install -d -m 0755 -o node -g node /home/node/.config && \
    install -d -m 0700 -o node -g node \
      /home/node/.eve \
      /home/node/.eve/workspace \
      /home/node/.config/eve && \
    ...
    stat -c '%U:%G %a' /home/node/.config/eve | grep -qx 'node:node 700'
