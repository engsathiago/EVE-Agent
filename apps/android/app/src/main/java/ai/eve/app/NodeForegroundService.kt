...
class NodeForegroundService : Service() {
  ...
  override fun onCreate() {
    super.onCreate()
    ensureChannel()
    val initial = buildNotification(title = "EVE Node", text = "Starting…")
    ...
    notificationJob =
      scope.launch {
        combine(
          ...
          val title =
            when {
              state.connected && state.mode == VoiceCaptureMode.TalkMode -> "EVE Node · Talk"
              state.connected -> "EVE Node · Connected"
              else -> "EVE Node"
            }
          val text =
  ...
  private fun ensureChannel() {
    ...
    val channel =
      NotificationChannel(
        ...
        description = "EVE node connection status"
        setShowBadge(false)
      }
  ...
  companion object {
    ...
    private const val ACTION_STOP = "ai.eve.app.action.STOP"
    private const val ACTION_SET_VOICE_CAPTURE_MODE = "ai.eve.app.action.SET_VOICE_CAPTURE_MODE"
    private const val EXTRA_VOICE_CAPTURE_MODE = "ai.eve.app.extra.VOICE_CAPTURE_MODE"
