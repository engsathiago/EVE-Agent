...
private const val NOTIFICATION_CHANNEL_BASE_ID = "eve.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "EVE Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "EVE Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "EVE Active")
      }
