...
internal fun eveAndroidVersionLabel(): String {
...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this EVE Android gateway connection failure.
    ...
    - tell me whether `eve devices list` should show a pending pairing request
    - if more signal is needed, ask for `eve qr --json`, `eve devices list`, and `eve nodes status`
    - give the next exact command or tap

    Debug info:
    - screen: $screen
    - app version: ${eveAndroidVersionLabel()}
    - device: $device
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("EVE gateway diagnostics", report))
