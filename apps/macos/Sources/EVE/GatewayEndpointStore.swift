...
actor GatewayEndpointStore {
    ...
    private static let staticLogger = Logger(subsystem: "ai.eve", category: "gateway-endpoint")
    ...
    {
        let raw = env["EVE_GATEWAY_PASSWORD"] ?? ""
    ...
    private let logger = Logger(subsystem: "ai.eve", category: "gateway-endpoint")
    ...
    init(deps: Deps = .live) {
        ...
        if let modeRaw {
            ...
            let seen = UserDefaults.standard.bool(forKey: "eve.onboardingSeen")
        ...
        let bind = GatewayEndpointStore.resolveGatewayBindMode(
            ...
            env: ProcessInfo.processInfo.environment)
        let customBindHost = GatewayEndpointStore.resolveGatewayCustomBindHost(root: EVEConfigFile.loadDict())
    ...
    {
        if let envBind = env["EVE_GATEWAY_BIND"] {
    ...
    private static func resolveGatewayScheme(
        ...
        env: [String: String]) -> String
    {
        if let envValue = env["EVE_GATEWAY_TLS"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !envValue.isEmpty
        {
