...
final class DashboardWindowController: NSWindowController, WKNavigationDelegate, WKUIDelegate, NSWindowDelegate {
    ...
    private static func installNativeChromeScript(into userContentController: WKUserContentController) {
        let css = """
        html.eve-native-macos {
          --eve-native-titlebar-height: 50px;
        }
        @media (min-width: 700px) {
          html.eve-native-macos .sidebar-shell {
            padding-top: max(14px, var(--eve-native-titlebar-height)) !important;
          }
          html.eve-native-macos .sidebar-shell__header {
        ...
        (() => {
          try {
            if (document.getElementById("eve-native-macos-chrome")) return;
            const style = document.createElement("style");
            style.id = "eve-native-macos-chrome";
            style.textContent = \(Self.jsStringLiteral(css));
            document.documentElement.classList.add("eve-native-macos");
        ...
        auth: DashboardWindowAuth)
    {
        ...
        (() => {
          try {
            ...
            Object.defineProperty(window, "__EVE_NATIVE_CONTROL_AUTH__", {
              value: \(json),
              configurable: true,
            });
          } catch {}
