...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: EVEProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.eve", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: EVEKit.AnyCodable]()) {
                $0[$1.key] = EVEKit.AnyCodable($1.value.base)
            }
        ...
        if let urlError = error as? URLError {
            ...
            case .notConnectedToInternet:
                if Self.isLikelyLocalNetworkPermissionBlock() {
                    return """
                    macOS is blocking EVE Local Network access.
                    Allow EVE in System Settings → Privacy & Security → Local Network, then relaunch the app.
                    """
                }
    ...
    private static func bridgeToProtocolArgs(
        _ value: EVEProtocol.AnyCodable?) -> [String: EVEProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: EVEProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: EVEKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: EVEProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("eve.control.heartbeat")
    static let controlAgentEvent = Notification.Name("eve.control.agent")
}
