...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["EVE_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/eve")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("eve-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("eve-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/eve/ (rolling or stdout), or nil if none exist.
