...
enum EVELogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = EVEOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = EVEFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        EVELogging.bootstrapIfNeeded()
        let label = EVELogging.makeLabel(subsystem: subsystem, category: category)
...
struct EVEOSLogHandler: AppLogLevelBackedHandler {
...
struct EVEFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = EVELogging.parseLabel(self.label)
