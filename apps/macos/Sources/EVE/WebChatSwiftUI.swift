...
private let webChatSwiftLogger = Logger(subsystem: "ai.eve", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "eve.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: EVEChatTransport {
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> EVEChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                EVEGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "session.message":
                ...
                guard let message = try? JSONDecoder().decode(
                    EVESessionMessageEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
    ...
    private static func mapModelChoice(_ model: EVEProtocol.ModelChoice) -> EVEChatModelChoice {
        EVEChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<EVEChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any EVEChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = EVEChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: EVEChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<EVEChatView>) -> NSViewController
    {
