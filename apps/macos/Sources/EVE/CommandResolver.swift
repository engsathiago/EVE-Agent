...
enum CommandResolver {
    private static let projectRootDefaultsKey = "eve.gatewayProjectRootPath"
    private static let helperName = "eve"
    ...
    static func gatewayEntrypoint(in root: URL) -> String? {
        ...
        let eveEntry = root.appendingPathComponent("eve.mjs").path
        if FileManager().isReadableFile(atPath: eveEntry) { return eveEntry }
        let binEntry = root.appendingPathComponent("bin/eve.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/eve")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let evePaths = self.eveManagedPaths(home: home)
        if !evePaths.isEmpty {
            extras.insert(contentsOf: evePaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + evePaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func eveManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".eve"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func eveExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectEVEExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("eve.mjs").path,
            root.appendingPathComponent("bin/eve.js").path,
        ]
    ...
    static func hasAnyEVEInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.eveExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        projectRoot: URL? = nil) -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let evePath = self.projectEVEExecutable(projectRoot: root) {
            return [evePath, subcommand] + extraArgs
        }
        if let evePath = self.eveExecutable(searchPaths: searchPaths) {
            return [evePath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "eve", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            eve entrypoint missing (looked for dist/index.js or eve.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real eve CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/eve.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/eve.mjs"
            node "$PRJ/eve.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/eve.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/eve.js"
            node "$PRJ/bin/eve.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent eve"
          pnpm --silent eve \(quotedArgs);
        else
          echo "eve CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? EVEConfigFile.loadDict()
