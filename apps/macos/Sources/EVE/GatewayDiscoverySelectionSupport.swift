...
enum GatewayDiscoverySelectionSupport {
        ...
        state: AppState)
    {
        ...
        if preferredTransport == .direct {
            EVEConfigFile.setRemoteGatewayTransport(AppState.RemoteTransport.direct.rawValue)
            if !state.remoteUrl.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                EVEConfigFile.setRemoteGatewayUrlString(state.remoteUrl)
            } else {
                EVEConfigFile.clearRemoteGatewayUrl()
            }
        } else {
            EVEConfigFile.setRemoteGatewayTransport(AppState.RemoteTransport.ssh.rawValue)
            EVEConfigFile.setRemoteGatewayUrlString(state.remoteUrl)
        }
    }
