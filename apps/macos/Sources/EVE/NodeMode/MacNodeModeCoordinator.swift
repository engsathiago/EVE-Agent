...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.eve", category: "mac-node")
    ...
    private func run() async {
        ...
        while !Task.isCancelled {
            ...
            let browserControlEnabled = EVEConfigFile.browserControlEnabled()
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: EVENodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
        ...
        connectionMode: AppState.ConnectionMode) -> [String]
    {
        ...
        if browserControlEnabled, connectionMode == .local {
            caps.append(EVECapability.browser.rawValue)
        }
        if cameraEnabled {
            caps.append(EVECapability.camera.rawValue)
        }
        if locationMode != .off {
            caps.append(EVECapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCaps() -> [String] {
        ...
        return Self.resolvedCaps(
            browserControlEnabled: EVEConfigFile.browserControlEnabled(),
            cameraEnabled: UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false,
            locationMode: EVELocationMode(rawValue: rawLocationMode) ?? .off,
            connectionMode: AppStateStore.shared.connectionMode)
    }
    ...
    nonisolated static func resolvedCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            EVESystemCommand.which.rawValue,
            EVESystemCommand.run.rawValue,
            EVESystemCommand.execApprovalsGet.rawValue,
            EVESystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(EVECapability.browser.rawValue) {
            commands.append(EVEBrowserCommand.proxy.rawValue)
        }
        if capsSet.contains(EVECapability.camera.rawValue) {
        ...
        if capsSet.contains(EVECapability.location.rawValue) {
