...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.eve.mac`) even if the shipped
    ...
    {
        if environment["EVE_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "eve.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "eve.nixMode") { return true }
