...
final class AppState {
    private static let logger = Logger(subsystem: "ai.eve", category: "app-state")
    ...
    init(preview: Bool = false) {
        ...
        let configRoot = EVEConfigFile.loadDict()
    ...
    private static func sshTunnelGatewayUrl(existingUrl: String?, expectedRemoteHost: String?) -> String {
        ...
        let preservePort: Bool = if LoopbackHost.isLoopbackHost(host) {
            ...
            EVEConfigFile.canonicalHostForComparison(host) ==
                EVEConfigFile.canonicalHostForComparison(expectedRemoteHost)
        } else {
            false
        }
    ...
    private func startConfigWatcher() {
        let configUrl = EVEConfigFile.url()
    ...
    func syncGatewayConfigNow() {
        ...
        let synced = Self.syncedGatewayRoot(
            currentRoot: EVEConfigFile.loadDict(),
            draft: .init(
        ...
        guard EVEConfigFile.saveDict(synced.root) else {
...
extension AppState {
    static var preview: AppState {
        ...
        state.remoteProjectRoot = "~/Projects/eve"
        state.remoteCliPath = ""
        return state
    }
}
