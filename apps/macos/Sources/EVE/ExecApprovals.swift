...
enum ExecApprovalsStore {
    private static let logger = Logger(subsystem: "ai.eve", category: "exec-approvals")
    ...
    static func fileURL() -> URL {
        EVEPaths.stateDirURL.appendingPathComponent("exec-approvals.json")
    }

    static func socketPath() -> String {
        EVEPaths.stateDirURL.appendingPathComponent("exec-approvals.sock").path
    }

    private static func legacyStateDirURLs() -> [URL] {
        if let home = EVEEnv.path("EVE_HOME") {
            var urls = [
                URL(fileURLWithPath: home, isDirectory: true)
                    .appendingPathComponent(".eve", isDirectory: true),
            ]
    ...
    private static func legacyFileURLIfPending() -> URL? {
        guard EVEEnv.path("EVE_STATE_DIR") != nil else { return nil }
