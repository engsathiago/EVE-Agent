...
private let appDefaultsSuites = ["ai.eve.mac", "ai.eve.mac.debug"]
...
func runConfigureRemote(_ args: [String]) {
    do {
        ...
        if opts.help {
            ...
            Usage:
              eve-mac configure-remote --ssh-target <user@host[:port]> [--local-port <port>]
                                          [--remote-port <port>] [--token <token>] [--password <password>]
                                          [--identity <path>] [--project-root <path>] [--cli-path <path>] [--json]
              eve-mac configure-remote --direct-url <ws://host:port|wss://host> [--token <token>]
            ...
            Options:
              ...
              --project-root <p>  Remote EVE checkout for CLI commands.
              --cli-path <path>   Remote eve executable or entrypoint.
              --json              Emit JSON.
              -h, --help          Show help.
            """)
            return
...
private func eveConfigURL() -> URL {
    if let raw = ProcessInfo.processInfo.environment["EVE_CONFIG_PATH"],
       !raw.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
    {
        return URL(fileURLWithPath: NSString(string: raw).expandingTildeInPath)
    }
    return FileManager().homeDirectoryForCurrentUser.appendingPathComponent(".eve/eve.json")
}
...
private func writeAppDefaults(opts: ConfigureRemoteOptions, target: String) {
    for suite in appDefaultsSuites {
        ...
        defaults.set("remote", forKey: "eve.connectionMode")
        setDefaultString(defaults, key: "eve.remoteTarget", value: target)
        defaults.set(true, forKey: "eve.onboardingSeen")
        defaults.set(appOnboardingVersion, forKey: "eve.onboardingVersion")
        setDefaultStringIfProvided(defaults, key: "eve.remoteIdentity", value: opts.identity)
        setDefaultStringIfProvided(defaults, key: "eve.remoteProjectRoot", value: opts.projectRoot)
        setDefaultStringIfProvided(defaults, key: "eve.remoteCliPath", value: opts.cliPath)
