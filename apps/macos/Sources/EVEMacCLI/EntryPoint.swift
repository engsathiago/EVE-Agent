...
private func printUsage() {
    ...
    Usage:
      eve-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
      ...
      eve-mac discover [--timeout <ms>] [--json] [--include-local]
      eve-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      eve-mac connect
      eve-mac configure-remote --ssh-target user@gateway.local --remote-port 18789
      eve-mac connect --url ws://127.0.0.1:18789 --json
      eve-mac discover --timeout 3000 --json
      eve-mac wizard --mode local
    """)
}
