...
struct MacNodeModeCoordinatorTests {
    @Test func `remote mode does not advertise browser proxy`() {
        ...
        #expect(!caps.contains(EVECapability.browser.rawValue))
        #expect(!commands.contains(EVEBrowserCommand.proxy.rawValue))
        #expect(commands.contains(EVECanvasCommand.present.rawValue))
