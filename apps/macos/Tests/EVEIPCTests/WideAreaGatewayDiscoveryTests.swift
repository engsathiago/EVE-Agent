...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() {
        let originalWideAreaDomain = getenv("EVE_WIDE_AREA_DOMAIN").map { String(cString: $0) }
        setenv("EVE_WIDE_AREA_DOMAIN", "eve.internal", 1)
        defer {
            if let originalWideAreaDomain {
                setenv("EVE_WIDE_AREA_DOMAIN", originalWideAreaDomain, 1)
            } else {
                unsetenv("EVE_WIDE_AREA_DOMAIN")
            }
        }
        let statusJson = """
        {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._eve-gw._tcp.eve.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
                    return "0 0 18789 steipetacstudio.eve.internal."
                }
                if recordType == "TXT" {
                    return "\"displayName=Peter\\226\\128\\153s Mac Studio (EVE)\" \"gatewayPort=18789\" \"tailnetDns=peters-mac-studio-1.sheep-coho.ts.net\" \"cliPath=/Users/steipete/eve/src/entry.ts\""
                }
                return ""
            })
        ...
        let expectedDisplay = "Peter\u{2019}s Mac Studio (EVE)"
        ...
        #expect(beacon.cliPath == "/Users/steipete/eve/src/entry.ts")
    }
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._eve-gw._tcp.eve.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
