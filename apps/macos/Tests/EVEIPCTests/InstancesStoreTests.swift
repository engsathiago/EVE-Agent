...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: EVEProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: EVEProtocol.AnyCodable] = [
            "presence": .init([EVEProtocol.AnyCodable(entry)]),
        ]
        let payload = EVEProtocol.AnyCodable(payloadMap)
