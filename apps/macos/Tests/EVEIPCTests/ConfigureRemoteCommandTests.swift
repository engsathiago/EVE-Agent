...
struct ConfigureRemoteCommandTests {
    @Test @MainActor func `configure remote writes ssh config and app defaults`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("eve-configure-remote-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }

        let defaultSuites = ["ai.eve.mac", "ai.eve.mac.debug"]
        let keys = [
            "eve.connectionMode",
            "eve.remoteTarget",
            "eve.onboardingSeen",
            "eve.onboardingVersion",
            "eve.remoteCliPath",
        ]
        ...
        try await TestIsolation.withIsolatedState(env: ["EVE_CONFIG_PATH": configURL.path]) {
            ...
            for (_, defaults) in defaultsBySuite {
                #expect(defaults.string(forKey: "eve.connectionMode") == "remote")
                #expect(defaults.string(forKey: "eve.remoteTarget") == "alice@gateway.example")
                #expect(defaults.bool(forKey: "eve.onboardingSeen") == true)
                #expect(defaults.string(forKey: "eve.remoteCliPath") == "/opt/homebrew/bin/eve")
            }
        }
    }
    ...
    @Test @MainActor func `configure remote preserves existing optional credentials when flags omitted`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("eve-configure-remote-preserve-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote can write direct private url`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("eve-configure-direct-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote rejects plaintext public prefix bypass`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("eve-configure-direct-reject-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }

        _ = await TestIsolation.withIsolatedState(env: ["EVE_CONFIG_PATH": configURL.path]) {
            #expect(throws: Error.self) {
