...
@Suite(.serialized)
struct EVEConfigFileTests {
    private func makeConfigOverridePath() -> String {
        FileManager().temporaryDirectory
            .appendingPathComponent("eve-config-\(UUID().uuidString)")
            .appendingPathComponent("eve.json")
            .path
    }
    ...
    func `config path respects env override`() async {
        ...
        await TestIsolation.withEnvValues(["EVE_CONFIG_PATH": override]) {
            #expect(EVEConfigFile.url().path == override)
        }
    }
    ...
    func `remote gateway port parses and matches host`() async {
        ...
        await TestIsolation.withEnvValues(["EVE_CONFIG_PATH": override]) {
            ...
            #expect(EVEConfigFile.remoteGatewayPort() == 19999)
            #expect(EVEConfigFile.remoteGatewayPort(matchingHost: "gateway.ts.net") == 19999)
            #expect(EVEConfigFile.remoteGatewayPort(matchingHost: "GATEWAY.ts.net.") == 19999)
            #expect(EVEConfigFile.remoteGatewayPort(matchingHost: "gateway") == nil)
            #expect(EVEConfigFile.remoteGatewayPort(matchingHost: "other.ts.net") == nil)
            #expect(EVEConfigFile.remoteGatewayPort(matchingHost: "gateway.attacker.tld") == nil)
        }
    }
    ...
    func `set remote gateway url string replaces scheme`() async {
        ...
        await TestIsolation.withEnvValues(["EVE_CONFIG_PATH": override]) {
            ...
            EVEConfigFile.setRemoteGatewayUrlString("ws://127.0.0.1:18789")
            let root = EVEConfigFile.loadDict()
    ...
    func `set remote gateway url preserves scheme`() async {
        ...
        await TestIsolation.withEnvValues(["EVE_CONFIG_PATH": override]) {
            ...
            EVEConfigFile.setRemoteGatewayUrl(host: "new-host", port: 2222)
            let root = EVEConfigFile.loadDict()
            let url = ((root["gateway"] as? [String: Any])?["remote"] as? [String: Any])?["url"] as? String
    ...
    func `clear remote gateway url removes only url field`() async {
        ...
        await TestIsolation.withEnvValues(["EVE_CONFIG_PATH": override]) {
            ...
            EVEConfigFile.clearRemoteGatewayUrl()
            let root = EVEConfigFile.loadDict()
    ...
    func `state dir override sets config path`() async {
        ...
        await TestIsolation.withEnvValues([
            ...
            #expect(EVEConfigFile.stateDirURL().path == dir)
            #expect(EVEConfigFile.url().path == "\(dir)/eve.json")
        }
    }
    ...
    func `save dict appends config audit log`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("eve-state-\(UUID().uuidString)", isDirectory: true)
        let configPath = stateDir.appendingPathComponent("eve.json")
        let auditPath = stateDir.appendingPathComponent("logs/config-audit.jsonl")

        defer { try? FileManager().removeItem(at: stateDir) }

        try await TestIsolation.withEnvValues([
            "EVE_STATE_DIR": stateDir.path,
            "EVE_CONFIG_PATH": configPath.path,
        ]) {
            EVEConfigFile.saveDict([
                "gateway": ["mode": "local"],
            ])
            ...
            #expect(auditRoot?["source"] as? String == "macos-eve-config-file")
    ...
    func `save dict preserves gateway auth unless explicitly allowed`() async throws {
        ...
        await TestIsolation.withEnvValues([
            ...
            let allowedRoot = EVEConfigFile.loadDict()
    ...
    func `load dict audits suspicious out-of-band clobbers`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            try EVEConfigFile.withTestingFileLock {
                EVEConfigFile.saveDict([
                    "update": ["channel": "beta"],
                ...
                let loaded = EVEConfigFile.loadDict()
    ...
    func `save dict records preserved gateway auth in audit`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            let saved = EVEConfigFile.saveDict([
                "gateway": [
                    "mode": "local",
                ],
                "browser": [
