...
struct WebChatSwiftUISmokeTests {
    private struct TestTransport: EVEChatTransport {
        func requestHistory(sessionKey: String) async throws -> EVEChatHistoryPayload {
            let json = """
            {"sessionKey":"\(sessionKey)","sessionId":null,"messages":[],"thinkingLevel":"off"}
            """
            return try JSONDecoder().decode(EVEChatHistoryPayload.self, from: Data(json.utf8))
        }
            ...
            attachments _: [EVEChatAttachmentPayload]) async throws -> EVEChatSendResponse
        {
            ...
            """
            return try JSONDecoder().decode(EVEChatSendResponse.self, from: Data(json.utf8))
        }
