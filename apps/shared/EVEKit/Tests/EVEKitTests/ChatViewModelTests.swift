...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> EVEChatHistoryPayload
{
    EVEChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> EVEChatSessionEntry {
    EVEChatSessionEntry(
        key: key,
        kind: nil,
        displayName: nil,
...
private func thinkingOption(_ id: String, label: String? = nil) -> EVEChatThinkingLevelOption {
    EVEChatThinkingLevelOption(id: id, label: label ?? id)
}
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> EVEChatSessionEntry
{
    EVEChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> EVEChatModelChoice {
    EVEChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [EVEChatHistoryPayload],
    sessionsResponses: [EVEChatSessionsListResponse] = [],
    modelResponses: [[EVEChatModelChoice]] = [],
    ...
    -> (TestChatTransport, EVEChatViewModel)
{
...
private func sendUserMessage(_ vm: EVEChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, EVEChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [EVEChatHistoryPayload]
    private let sessionsResponses: [EVEChatSessionsListResponse]
    private let modelResponses: [[EVEChatModelChoice]]
    ...
    private let stream: AsyncStream<EVEChatTransportEvent>
    private let continuation: AsyncStream<EVEChatTransportEvent>.Continuation
        ...
        healthResponses: [Bool] = [true])
    {
        ...
        var cont: AsyncStream<EVEChatTransportEvent>.Continuation!
        ...
        parentSessionKey: String?) async throws -> EVEChatCreateSessionResponse
    {
        ...
        return EVEChatCreateSessionResponse(ok: true, key: key, sessionId: "created-\(key)")
    }
    ...
    func requestHistory(sessionKey: String) async throws -> EVEChatHistoryPayload {
        ...
        return self.historyResponses.last ?? EVEChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
        ...
        attachments _: [EVEChatAttachmentPayload]) async throws -> EVEChatSendResponse
    {
        ...
        return EVEChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> EVEChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? EVEChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: EVEChatTransportEvent) {
...
struct ChatViewModelTests {
    @Test func `displays error message fallback only for assistant error turns`() throws {
        func decodeMessage(role: String, stopReason: String, contentText: String? = nil) throws -> EVEChatMessage {
        ...
        #expect(
            EVEChatMessage.errorDisplayText(
    ...
    @Test @MainActor func `bounded repeated same text reply invalidates older stale refresh`() async throws {
        ...
        transport.emit(EVEChatTransportEvent.seqGap)
    ...
    @Test func `session choices prefer main and recent`() async throws {
        ...
        let sessions = EVEChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func `bootstraps model selection from session and defaults`() async throws {
        ...
        let sessions = EVEChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: EVEChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func `selecting default model patches nil and updates selection`() async throws {
        ...
        await MainActor.run { vm.selectModel(EVEChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == EVEChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func `selecting provider qualified model disambiguates duplicate model I ds`() async throws {
        ...
        let sessions = EVEChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: EVEChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func `late model completion does not replay current session selection into previous session`() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = EVEChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = EVEChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func `explicit thinking level wins over history and persists changes`() async throws {
        let history = EVEChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func `decodes gateway thinking metadata from session list`() throws {
        ...
        let decoded = try JSONDecoder().decode(
            EVEChatSessionsListResponse.self,
            from: Data(json.utf8))
    ...
    @Test func `strips inbound metadata from history messages`() async throws {
        ...
        let vm = await MainActor.run { EVEChatViewModel(sessionKey: "main", transport: transport) }
