...
private struct EVEChatPreviewTransport: EVEChatTransport {
    enum Scenario {
        case connected
        case empty
        case loading
    ...
    func requestHistory(sessionKey: String) async throws -> EVEChatHistoryPayload {
        ...
        case .error:
            throw NSError(
                domain: "EVEChatPreviewTransport",
                code: 1,
    ...
    {
        EVEChatSendResponse(runId: idempotencyKey, status: "ok")
    }

    func listSessions(limit _: Int?) async throws -> EVEChatSessionsListResponse {
        EVEChatSessionsListResponse(
            ts: 0,
            path: nil,
            count: 2,
    ...
    private static func session(
        key: String,
        displayName: String,
        updatedAt: Double) -> EVEChatSessionEntry
    {
        EVEChatSessionEntry(
            key: key,
            kind: nil,
...
#Preview("Chat") {
    EVEChatPreview(scenario: .connected)
}
...
#Preview("Onboarding chat") {
    EVEChatView(
        viewModel: EVEChatViewModel(
            ...
            transport: EVEChatPreviewTransport()),
...
private struct EVEChatPreview: View {
    let scenario: EVEChatPreviewTransport.Scenario
    ...
    var body: some View {
        EVEChatView(
            viewModel: EVEChatViewModel(
                ...
                transport: EVEChatPreviewTransport(scenario: self.scenario)),
