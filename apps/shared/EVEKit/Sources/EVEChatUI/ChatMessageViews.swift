...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? EVEChatTheme.userText : EVEChatTheme.assistantText
    ...
    private var inlineAttachments: [EVEChatMessageContent] {
    ...
    private var toolCalls: [EVEChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [EVEChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? EVEChatTheme.userBubble
        }
        if self.style == .onboarding {
            return EVEChatTheme.onboardingAssistantBubble
        }
        return EVEChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return EVEChatTheme.onboardingAssistantBorder
        }
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? EVEChatTheme.userText : EVEChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: EVEChatMessageContent
    let isUser: Bool
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [EVEChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: EVEChatTheme.assistantText)
            }
        }
    }
}
