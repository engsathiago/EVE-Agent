...
@MainActor
public struct EVEChatView: View {
    public enum Style {
        case standard
        case onboarding
    }
    ...
    private let talkControl: EVEChatTalkControl?
    ...
    public init(
        ...
        talkControl: EVEChatTalkControl? = nil)
    {
    ...
    private var visibleMessages: [EVEChatMessage] {
        let base: [EVEChatMessage]
    ...
    private func mergeToolResults(in messages: [EVEChatMessage]) -> [EVEChatMessage] {
    ...
    private func isToolResultMessage(_ message: EVEChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: EVEChatMessage) -> Bool {
    ...
    private func primaryText(in message: EVEChatMessage) -> String {
        let parts = message.content.compactMap { content -> String? in
            let kind = (content.type ?? "text").lowercased()
            guard kind == "text" || kind.isEmpty else { return nil }
            return content.text
        }
        return EVEChatMessage.displayText(
    ...
    private func hasInlineAttachments(in message: EVEChatMessage) -> Bool {
    ...
    private func toolCalls(in message: EVEChatMessage) -> [EVEChatMessageContent] {
    ...
    private func inlineToolResults(in message: EVEChatMessage) -> [EVEChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: EVEChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: EVEChatMessage) -> String {
...
private struct ChatAssistantIntroCard: View {
    ...
    var body: some View {
        Text(self.text)
            .font(.system(size: 15))
            .lineSpacing(4)
            .foregroundStyle(EVEChatTheme.assistantText)
            .multilineTextAlignment(.leading)
            .padding(.vertical, 12)
            .padding(.horizontal, 14)
            .background(
                RoundedRectangle(cornerRadius: 16, style: .continuous)
                    .fill(EVEChatTheme.assistantBubble)
                    .overlay(
                        RoundedRectangle(cornerRadius: 16, style: .continuous)
