...
public struct EVEChatTalkControl {
...
@MainActor
struct EVEChatComposer: View {
    @Bindable var viewModel: EVEChatViewModel
    let style: EVEChatView.Style
    let showsSessionSwitcher: Bool
    let userAccent: Color?
    let assistantName: String?
    let assistantAvatarText: String?
    let assistantAvatarTint: Color?
    let composerChrome: EVEChatView.ComposerChrome
    let isComposerEnabled: Bool
    let messagePlaceholder: String?
    let talkControl: EVEChatTalkControl?
            ...
            if self.composerChrome == .full {
                ...
                if self.style == .standard {
                    let shape = UnevenRoundedRectangle(
                        ...
                        style: .continuous)
                    shape
                        .fill(EVEChatTheme.composerBackground)
                        .overlay(shape.strokeBorder(EVEChatTheme.composerBorder, lineWidth: 1))
    ...
    private var modelPicker: some View {
        ...
        {
            Text(self.viewModel.defaultModelLabel).tag(EVEChatViewModel.defaultModelSelectionID)
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \EVEPendingAttachment.id)
                { (att: EVEPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            EVEPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var fullEditor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 14, style: .continuous)
                .fill(EVEChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 14, style: .continuous)
                        .strokeBorder(EVEChatTheme.composerBorder)))
    ...
    private func talkButton(_ talkControl: EVEChatTalkControl) -> some View {
        Button {
    ...
    private func compactTalkButton(_ talkControl: EVEChatTalkControl) -> some View {
        Button {
            talkControl.toggle(self.viewModel.sessionKey)
        } label: {
            Image(systemName: talkControl.isEnabled ? "stop.fill" : "waveform")
    ...
    private func talkButtonFill(_ talkControl: EVEChatTalkControl) -> AnyShapeStyle {
        if talkControl.isEnabled {
            return AnyShapeStyle(EVEChatTheme.userBubble)
        }
        if !talkControl.isGatewayConnected {
            return AnyShapeStyle(Color.secondary.opacity(0.12))
        }
        return EVEChatTheme.subtleCard
    }

    private func talkButtonStroke(_ talkControl: EVEChatTalkControl) -> Color {
        if talkControl.isEnabled {
            return Color.white.opacity(0.18)
        }
        return EVEChatTheme.composerBorder
    }

    private func talkAccessibilityValue(_ talkControl: EVEChatTalkControl) -> String {
    ...
    private func talkHelpText(_ talkControl: EVEChatTalkControl) -> String {
    ...
    private var sendButtonFill: Color {
        self.userAccent ?? EVEChatTheme.userBubble
    }
