...
private let chatUILogger = Logger(subsystem: "ai.eve", category: "EVEChatUI")

@MainActor
@Observable
// swiftlint:disable:next type_body_length
public final class EVEChatViewModel {
    public static let defaultModelSelectionID = "__default__"
    static let maxAttachmentBytes = 5_000_000

    public private(set) var messages: [EVEChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var thinkingLevelOptions: [EVEChatThinkingLevelOption]
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [EVEChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [EVEPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [EVEChatPendingToolCall] = []
    public private(set) var sessions: [EVEChatSessionEntry] = []
    private let transport: any EVEChatTransport
    private var sessionDefaults: EVEChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: EVEChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any EVEChatTransport,
    ...
    public var sessionChoices: [EVEChatSessionEntry] {
    ...
    private static let baseThinkingLevelOptions: [EVEChatThinkingLevelOption] = [
        EVEChatThinkingLevelOption(id: "off", label: "off"),
        EVEChatThinkingLevelOption(id: "minimal", label: "minimal"),
    ...
    public func removeAttachment(_ id: EVEPendingAttachment.ID) {
    ...
    private func applyHistoryPayload(
        _ payload: EVEChatHistoryPayload,
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [EVEChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: EVEChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: EVEChatMessage) -> EVEChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> EVEChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return EVEChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: EVEChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: EVEChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: EVEChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private func adoptPendingLocalUserEcho(incoming: EVEChatMessage) -> Bool {
        ...
        updated[matchIndex] = EVEChatMessage(
            id: existing.id,
            role: incoming.role,
    ...
    private static func reconcileMessageIDs(
        previous: [EVEChatMessage],
        incoming: [EVEChatMessage]) -> [EVEChatMessage]
    {
    ...
    private static func reconcileRunRefreshMessages(
        previous: [EVEChatMessage],
        incoming: [EVEChatMessage],
        pendingLocalUserEchoIDs: Set<UUID>) -> [EVEChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [EVEChatMessage]) -> [EVEChatMessage] {
        var result: [EVEChatMessage] = []
    ...
    private static func dedupeKey(for message: EVEChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [EVEChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> EVEChatAttachmentPayload in
            EVEChatAttachmentPayload(
                type: att.type,
    ...
    private static func isUnsupportedCreateSessionError(_ error: Error) -> Bool {
        ...
        return nsError.domain == "EVEChatTransport"
    ...
    private func resolvedThinkingLevelOptions(
        for currentSession: EVEChatSessionEntry?) -> [EVEChatThinkingLevelOption]
    {
    ...
    private static func sessionModelMatchesDefaults(
        _ session: EVEChatSessionEntry,
        defaults: EVEChatSessionsDefaults?) -> Bool
    {
    ...
    private static func normalizedThinkingLevelOptions(
        _ levels: [EVEChatThinkingLevelOption]?) -> [EVEChatThinkingLevelOption]?
    {
        ...
        return Self.dedupedThinkingOptions(
            levels.compactMap { level in
                ...
                return EVEChatThinkingLevelOption(id: id, label: label.isEmpty ? id : label)
            })
    }

    private static func thinkingOptions(from labels: [String]?) -> [EVEChatThinkingLevelOption]? {
        guard let labels else { return nil }
        return Self.dedupedThinkingOptions(
            labels.compactMap { label in
                guard let id = Self.normalizedThinkingLevel(label) else { return nil }
                let trimmed = label.trimmingCharacters(in: .whitespacesAndNewlines)
                return EVEChatThinkingLevelOption(id: id, label: trimmed.isEmpty ? id : trimmed)
            })
    }

    private static func withCurrentThinkingOption(
        _ options: [EVEChatThinkingLevelOption],
        current: String) -> [EVEChatThinkingLevelOption]
    {
        guard !options.contains(where: { $0.id == current }) else { return options }
        return options + [EVEChatThinkingLevelOption(id: current, label: current)]
    }

    private static func dedupedThinkingOptions(
        _ options: [EVEChatThinkingLevelOption]) -> [EVEChatThinkingLevelOption]
    {
        var result: [EVEChatThinkingLevelOption] = []
        var seen = Set<String>()
    ...
    private func placeholderSession(key: String) -> EVEChatSessionEntry {
        EVEChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
    ...
    private func updateCurrentSessionThinkingLevel(_ thinkingLevel: String?, sessionKey: String) {
        ...
        self.sessions[index] = EVEChatSessionEntry(
            key: current.key,
            kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: EVEChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleSessionMessageEvent(_ payload: EVESessionMessageEventPayload) {
    ...
    private func handleChatEvent(_ chat: EVEChatEventPayload) {
    ...
    private func appendFinalChatMessageIfPresent(_ chat: EVEChatEventPayload) {
        guard chat.state == "final" else { return }
        guard let text = EVEChatEventText.assistantText(from: chat) else { return }

        let decoded = chat.message.flatMap {
            try? ChatPayloadDecoding.decode($0, as: EVEChatMessage.self)
        }
    ...
    private static func isAssistantMessage(_ message: EVEChatMessage) -> Bool {
        message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == "assistant"
    }

    private static func messageWithTimestampIfNeeded(_ message: EVEChatMessage) -> EVEChatMessage {
    ...
    private func handleAgentEvent(_ evt: EVEAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = EVEChatPendingToolCall(
    ...
    private func handleAgentLifecycleEvent(_ evt: EVEAgentEventPayload, isPendingRun: Bool) {
    ...
    private static func agentLifecycleErrorMessage(_ evt: EVEAgentEventPayload, aborted: Bool) -> String {
        if aborted {
            return "Run aborted"
        }
    ...
    private static func hasUnansweredLatestUser(in messages: [EVEChatMessage]) -> Bool {
        self.latestUserTurn(in: messages) != nil && !self.hasAssistantMessageAfterLatestUser(in: messages)
    }

    private static func latestUserTurn(in messages: [EVEChatMessage]) -> LatestUserTurn? {
    ...
    private static func hasAnsweredUser(
        ...
        in messages: [EVEChatMessage])
        -> Bool
    {
        guard let refreshKey = user.refreshKey else { return false }
        var occurrence = 0
        var latestMatchingUserIndex: [EVEChatMessage].Index?
    ...
    private static func hasAssistantMessageAfterLatestUser(in messages: [EVEChatMessage]) -> Bool {
        guard let lastUserIndex = messages.lastIndex(where: { $0.role.lowercased() == "user" }) else {
            return false
        }
