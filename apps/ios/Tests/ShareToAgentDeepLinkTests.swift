...
@Suite struct ShareToAgentDeepLinkTests {
    @Test func appGroupIdentifierUsesCanonicalEVEGroup() {
        #expect(EVEAppGroup.canonicalIdentifier == "group.ai.evefoundation.app.shared")
    }
    ...
    @Test func buildURLEncodesAgentRoute() {
        ...
        guard case let .agent(agent)? = parsed else {
            Issue.record("Expected eve://agent deep link")
            return
        }
