...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.evefoundation.app", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to EVE", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to EVE gateway…")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "EVEShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "EVE is not connected to a gateway yet."])
        }
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("eve-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
