...
@main
struct EVEWatchApp: App {
    @Environment(\.scenePhase) private var scenePhase
    @State private var inboxStore = WatchInboxStore(
        requestNotificationAuthorization: !EVEWatchApp.isScreenshotMode)
    @State private var receiver: WatchConnectivityReceiver?
    @State private var execApprovalRefreshTask: Task<Void, Never>?

    private static let screenshotModeDefaultsKey = "eve.watch.screenshotMode"
    private static let isScreenshotMode = ProcessInfo.processInfo.arguments.contains(
        "--eve-watch-screenshot-mode")
        || ProcessInfo.processInfo.environment["EVE_WATCH_SCREENSHOT_MODE"] == "1"
        || UserDefaults.standard.bool(forKey: EVEWatchApp.screenshotModeDefaultsKey)
    ...
    var body: some Scene {
        WindowGroup {
            WatchInboxView(
                ...
                .task {
                    if EVEWatchApp.isScreenshotMode {
