...
final class ContactsService: ContactsServicing {
    ...
    func search(params: EVEContactsSearchParams) async throws -> EVEContactsSearchPayload {
        ...
        return EVEContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: EVEContactsAddParams) async throws -> EVEContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return EVEContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return EVEContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> EVEContactPayload {
        EVEContactPayload(
