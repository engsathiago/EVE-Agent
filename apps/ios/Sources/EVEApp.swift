...
@MainActor
enum EVEAppModelRegistry {
    static var appModel: NodeAppModel?
}

@MainActor
final class EVEAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.evefoundation.app", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.evefoundation.app", category: "BackgroundWake")
    ...
    private func resolvedAppModel() -> NodeAppModel? {
        self.appModel ?? EVEAppModelRegistry.appModel
    }
...
enum WatchPromptNotificationBridge {
    static let typeKey = "eve.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "eve.watch.promptId"
    static let sessionKeyKey = "eve.watch.sessionKey"
    static let actionPrimaryIDKey = "eve.watch.action.primary.id"
    static let actionPrimaryLabelKey = "eve.watch.action.primary.label"
    static let actionSecondaryIDKey = "eve.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "eve.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "eve.watch.action.primary"
    static let actionSecondaryIdentifier = "eve.watch.action.secondary"
    static let actionIdentifierPrefix = "eve.watch.action."
    static let actionIDKeyPrefix = "eve.watch.action.id."
    static let actionLabelKeyPrefix = "eve.watch.action.label."
    static let categoryPrefix = "eve.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: EVEWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> EVEWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return EVEWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "EVE" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [EVEWatchAction]) -> [UNNotificationAction] {
...
struct EVEApp: App {
    ...
    @UIApplicationDelegateAdaptor(EVEAppDelegate.self) private var appDelegate
    ...
    init() {
        ...
        #endif
        EVEAppModelRegistry.appModel = appModel
    ...
    private static var screenshotModeEnabled: Bool {
        #if DEBUG
        ProcessInfo.processInfo.arguments.contains("--eve-screenshot-mode")
        #else
        false
        #endif
    }
...
extension EVEApp {
    ...
    private static func installUncaughtExceptionLogger() {
        NSLog("EVE: installing uncaught exception handler")
