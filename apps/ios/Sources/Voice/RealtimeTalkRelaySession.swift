...
final class RealtimeTalkRelaySession {
    private static let agentControlToolName = "eve_agent_control"
    ...
    private let logger = Logger(subsystem: "ai.evefoundation.app", category: "RealtimeTalkRelay")
    ...
    private func handleToolCall(_ payload: [String: AnyCodable]) async {
        ...
        do {
            ...
            let result: [String: Any] = completion.failed
                ? ["error": "EVE tool call failed"]
                : ["text": completion.text ?? "EVE finished with no text."]
                ...
                for await event in stream {
                    ...
                    if chatEvent.state == "final" {
                        return ChatCompletionResult(
                            text: EVEChatEventText.assistantText(from: chatEvent),
                            failed: false)
                    }
