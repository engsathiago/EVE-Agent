...
struct ProCard<Content: View>: View {
    ...
    var radius: CGFloat = EVEProMetric.cardRadius
    ...
    var body: some View {
        ...
        shape
            ...
            .overlay {
                if self.isProminent {
                    shape.strokeBorder(
                        EVEBrand.accent.opacity(self.colorScheme == .dark ? 0.12 : 0.07),
                        lineWidth: 1)
                        .padding(1)
                }
            }
...
extension View {
    func proPanelSurface(
        tint: Color? = nil,
        radius: CGFloat = EVEProMetric.cardRadius,
...
struct EVESidebarRevealButton: View {
    let headerAction: EVESidebarHeaderAction

    init(action: EVESidebarHeaderAction) {
...
struct EVESidebarHeaderLeadingSlot: View {
    let action: EVESidebarHeaderAction

    var body: some View {
        EVESidebarRevealButton(action: self.action)
            .frame(width: 44, height: 44, alignment: .center)
    }
}

struct EVEAdaptiveHeaderRow<Leading: View, Accessory: View>: View {
    let title: String
    let subtitle: String
...
struct EVEProMark: View {
    ...
    var body: some View {
        Image("EVEIcon")
            ...
            .shadow(color: EVEBrand.accent.opacity(0.18), radius: self.shadowRadius, y: self.shadowRadius / 3)
...
struct ProProgressBar: View {
    let progress: Double
    var color: Color = EVEBrand.accentHot
...
struct EVEGatewayCompactPill: View {
