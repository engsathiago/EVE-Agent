...
final class LiveActivityManager {
    ...
    private let logger = Logger(subsystem: "ai.evefoundation.app", category: "LiveActivity")
    private let connectingStaleSeconds: TimeInterval = 120
    private let hydrationStaleSeconds: TimeInterval = 300
    private var currentActivity: Activity<EVEActivityAttributes>?
    ...
    func showConnecting(statusText: String = "Connecting...", agentName: String, sessionKey: String) {
        ...
        let attributes = EVEActivityAttributes(agentName: agentName, sessionKey: sessionKey)
    ...
    private func hydrateCurrentAndPruneDuplicates() {
        let active = Activity<EVEActivityAttributes>.activities
    ...
    private func updateCurrent(state: EVEActivityAttributes.ContentState, staleDate: Date? = nil) {
    ...
    private func end(activity: Activity<EVEActivityAttributes>) {
        Task {
            await activity.end(
                ActivityContent(state: self.disconnectedState(), staleDate: nil),
                dismissalPolicy: .immediate)
    ...
    private func connectingState(statusText: String = "Connecting...") -> EVEActivityAttributes.ContentState {
        EVEActivityAttributes.ContentState(
    ...
    private func attentionState(statusText: String) -> EVEActivityAttributes.ContentState {
        EVEActivityAttributes.ContentState(
            statusText: statusText,
            isIdle: false,
            isDisconnected: false,
    ...
    private func disconnectedState() -> EVEActivityAttributes.ContentState {
        EVEActivityAttributes.ContentState(
