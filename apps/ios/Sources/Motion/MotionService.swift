...
final class MotionService: MotionServicing {
    func activities(params: EVEMotionActivityParams) async throws -> EVEMotionActivityPayload {
        ...
        let mapped: [EVEMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return EVEMotionActivityPayload(activities: mapped)
    }
        ...
        return try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    ...
                    let formatter = ISO8601DateFormatter()
                    let payload = EVEPedometerPayload(
