...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: EVEWatchNotifyParams) -> EVEWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [EVEWatchAction]?,
        kind: String?,
        promptId: String?) -> [EVEWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> EVEWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return EVEWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                EVEWatchAction(id: "approve", label: "Approve"),
                EVEWatchAction(id: "decline", label: "Decline", style: "destructive"),
                EVEWatchAction(id: "open_phone", label: "Open iPhone"),
                EVEWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            EVEWatchAction(id: "done", label: "Done"),
            EVEWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            EVEWatchAction(id: "open_phone", label: "Open iPhone"),
            EVEWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: EVEWatchRisk?,
        priority: EVENotificationPriority?) -> EVEWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: EVENotificationPriority?,
        risk: EVEWatchRisk?) -> EVENotificationPriority?
    {
