...
enum ExecApprovalNotificationBridge {
    ...
    static let categoryIdentifier = "eve.exec-approval"
    static let reviewActionIdentifier = "eve.exec-approval.review"
    ...
    static func approvalID(from userInfo: [AnyHashable: Any]) -> String? {
        let raw = self.evePayload(userInfo: userInfo)?["approvalId"] as? String
    ...
    static func payloadKind(userInfo: [AnyHashable: Any]) -> String {
        let raw = self.evePayload(userInfo: userInfo)?["kind"] as? String
        let trimmed = raw?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        return trimmed.isEmpty ? "unknown" : trimmed
    }

    private static func evePayload(userInfo: [AnyHashable: Any]) -> [String: Any]? {
