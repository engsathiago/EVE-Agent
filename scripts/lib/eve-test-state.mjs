#!/usr/bin/env node
// Creates isolated EVE test HOME/state directories and shell snippets.
...
function usage() {
  return `Usage:
  node scripts/lib/eve-test-state.mjs -- create [--label <name>] [--scenario <name>] [--env-file <path>] [--json]
  node scripts/lib/eve-test-state.mjs shell [--label <name>] [--scenario <name>]
  node scripts/lib/eve-test-state.mjs shell-function
...
function scenarioConfig(scenario, options = {}) {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      skills: {
        allowBundled: ["memory", "eve-testing"],
        limits: {
          maxSkillsInPrompt: 8,
  ...
  if (scenario === "gateway-loopback") {
    return {
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.token || "eve-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function renderAuthProfileSecretKeyExport() {
  return [
    'EVE_AUTH_PROFILE_SECRET_KEY_FILE="$EVE_TEST_STATE_HOME/.eve-test-auth-profile-secret-key"',
    'if [ -s "$EVE_AUTH_PROFILE_SECRET_KEY_FILE" ]; then',
    '  EVE_AUTH_PROFILE_SECRET_KEY="$(cat "$EVE_AUTH_PROFILE_SECRET_KEY_FILE")"',
    "else",
    '  EVE_AUTH_PROFILE_SECRET_KEY="$(od -An -N 32 -tx1 /dev/urandom | tr -d " \\n")"',
    '  ( umask 077; printf "%s\\n" "$EVE_AUTH_PROFILE_SECRET_KEY" > "$EVE_AUTH_PROFILE_SECRET_KEY_FILE" )',
    "fi",
    'if [ -z "$EVE_AUTH_PROFILE_SECRET_KEY" ]; then',
    '  echo "failed to generate EVE_AUTH_PROFILE_SECRET_KEY" >&2',
    "  return 1 2>/dev/null || exit 1",
    "fi",
    "export EVE_AUTH_PROFILE_SECRET_KEY",
  ];
}
...
function renderConfigWrite(configPathExpression, config) {
  ...
  return [
    `cat > ${configPathExpression} <<'EVE_TEST_STATE_JSON'`,
    json,
    "EVE_TEST_STATE_JSON",
  ].join("\n");
}
...
function buildCreatePlan(options = {}) {
  ...
  const env = {
    ...
    EVE_AUTH_PROFILE_SECRET_KEY: generateAuthProfileSecretKey(),
...
/** Create an isolated EVE test state directory and optional scenario config. */
export async function createState(options = {}) {
  const label = normalizeLabel(options.label);
  const root = await fs.mkdtemp(path.join(os.tmpdir(), `eve-${label}-`));
...
/** Render shell commands that create and export an isolated EVE test state. */
export function renderShellSnippet(options = {}) {
  const label = normalizeLabel(options.label);
  const scenario = requireScenario(options.scenario);
  const config = scenarioConfig(scenario, options);
  const env = scenarioEnv(scenario);
  const homeTemplate = `eve-${label}-${scenario}-home.XXXXXX`;
  const lines = [
    'EVE_TEST_STATE_TMP_ROOT="${EVE_TEST_STATE_TMPDIR:-${TMPDIR:-/tmp}}"',
    'EVE_TEST_STATE_TMP_ROOT="${EVE_TEST_STATE_TMP_ROOT%/}"',
    '[ -n "$EVE_TEST_STATE_TMP_ROOT" ] || EVE_TEST_STATE_TMP_ROOT="/tmp"',
    "export EVE_TEST_STATE_TMP_ROOT",
    'mkdir -p "$EVE_TEST_STATE_TMP_ROOT"',
    `EVE_TEST_STATE_HOME="$(mktemp -d "$EVE_TEST_STATE_TMP_ROOT/${homeTemplate}")"`,
    'export HOME="$EVE_TEST_STATE_HOME"',
    'export USERPROFILE="$EVE_TEST_STATE_HOME"',
    'export EVE_HOME="$EVE_TEST_STATE_HOME"',
    'export EVE_STATE_DIR="$EVE_TEST_STATE_HOME/.eve"',
    'export EVE_CONFIG_PATH="$EVE_STATE_DIR/eve.json"',
    ...renderAuthProfileSecretKeyExport(),
    'export EVE_TEST_WORKSPACE_DIR="$EVE_TEST_STATE_HOME/workspace"',
    'mkdir -p "$EVE_STATE_DIR" "$EVE_TEST_WORKSPACE_DIR"',
  ];
  for (const [key, value] of Object.entries(env)) {
    lines.push(`export ${key}=${shellQuote(value)}`);
  }
  const configWrite = renderConfigWrite('"$EVE_CONFIG_PATH"', config);
  if (configWrite) {
...
/** Render a reusable shell function for creating isolated EVE test state. */
export function renderShellFunction() {
  return `eve_test_state_create() {
  ...
  case "$scenario" in
    ...
    *)
      echo "unknown EVE test-state scenario: $scenario" >&2
      return 1
      ;;
  esac
  case "$raw_label" in
    /*)
      EVE_TEST_STATE_HOME="$raw_label"
      mkdir -p "$EVE_TEST_STATE_HOME"
      ;;
    *)
      label="$(printf "%s" "$label" | tr -cs "A-Za-z0-9_.-" "-" | sed -e "s/^-*//" -e "s/-*$//")"
      [ -n "$label" ] || label="state"
      local tmp_root="\${EVE_TEST_STATE_TMPDIR:-\${TMPDIR:-/tmp}}"
      tmp_root="\${tmp_root%/}"
      [ -n "$tmp_root" ] || tmp_root="/tmp"
      mkdir -p "$tmp_root"
      EVE_TEST_STATE_HOME="$(mktemp -d "$tmp_root/eve-$label-$scenario-home.XXXXXX")"
      ;;
  esac
  export HOME="$EVE_TEST_STATE_HOME"
  export USERPROFILE="$EVE_TEST_STATE_HOME"
  ...
  export EVE_TEST_WORKSPACE_DIR="$EVE_TEST_STATE_HOME/workspace"
  unset EVE_AGENT_DIR
  unset EVE_SERVICE_REPAIR_POLICY
  mkdir -p "$EVE_STATE_DIR" "$EVE_TEST_WORKSPACE_DIR"
  case "$scenario" in
    minimal)
      cat > "$EVE_CONFIG_PATH" <<'EVE_TEST_STATE_JSON'
{}
EVE_TEST_STATE_JSON
      ;;
    update-stable)
...
EVE_TEST_STATE_JSON
      ;;
    external-service)
      export EVE_SERVICE_REPAIR_POLICY="external"
      cat > "$EVE_CONFIG_PATH" <<'EVE_TEST_STATE_JSON'
{}
EVE_TEST_STATE_JSON
      ;;
