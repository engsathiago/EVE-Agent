...
REMOTE_URL="${EVE_REF_REMOTE:-https://github.com/engsathiago/eve-agent.git}"
REF=""
EXPECTED_SHA=""
FALLBACK_OK=0
...
usage() {
  cat >&2 <<'EOF'
Usage: resolve-eve-ref.sh --ref <ref> [--expected-sha <sha>] [--fallback-ok] [--github-output <file>]

Fast-resolves EVE branch and tag refs with git ls-remote. Full commit SHAs
...
echo "Failed to resolve EVE ref: ${REF}" >&2
exit 1
