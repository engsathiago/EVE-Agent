...
// Eve Npm Postpublish Verify script supports EVE repository automation.
...
export type EVENpmPostpublishVerifyArgs =
  | {
      help: false;
      version: string;
    }
...
export function eveNpmPostpublishVerifyUsage(): string {
  return "Usage: node --import tsx scripts/eve-npm-postpublish-verify.ts <version>";
}

export function parseEVENpmPostpublishVerifyArgs(
  argv: readonly string[],
): EVENpmPostpublishVerifyArgs {
  ...
  if (!version) {
    throw new Error(eveNpmPostpublishVerifyUsage());
  }
  if (version.startsWith("-")) {
    throw new Error(`Unknown eve npm postpublish verifier option: ${version}`);
  }
  const extraArg = args[1]?.trim();
  if (extraArg) {
    throw new Error(`Unexpected eve npm postpublish verifier argument: ${extraArg}`);
  }
...
const NPM_PROVENANCE_REPOSITORY = "https://github.com/engsathiago/eve-agent";
const NPM_PROVENANCE_WORKFLOW_PATH = ".github/workflows/eve-npm-release.yml";
...
function resolveNpmProvenanceVerificationPolicy(
  ...
  if (
    ...
    throw new Error(
      `npm provenance attestation does not bind ${version} to the trusted EVE GitHub release workflow.`,
    );
  }
...
export function collectInstalledPluginSdkDeclarationErrors(packageRoot: string): string[] {
  ...
  const errors: string[] = [];
  const forbiddenPrivateWorkspaceSpecifiers = ["@eve/llm-core"];
...
export function resolveInstalledBinaryPath(prefixDir: string, platform = process.platform): string {
  return platform === "win32"
    ? pathWin32.join(prefixDir, "eve.cmd")
    : pathPosix.join(prefixDir, "bin", "eve");
}
...
async function verifyPublishedRegistryProvenanceOnce(version: string): Promise<void> {
  ...
  console.log(
    `eve-npm-postpublish-verify: registry signature and provenance attestation verified (${version})`,
  );
}
...
function verifyScenario(version: string, scenario: PublishedInstallScenario): void {
  const workingDir = mkdtempSync(join(tmpdir(), `eve-postpublish-${scenario.name}.`));
  ...
  try {
    ...
    if (normalizeInstalledBinaryVersion(installedBinaryVersion) !== scenario.expectedVersion) {
      errors.push(
        `installed eve binary version mismatch: expected ${scenario.expectedVersion}, found ${installedBinaryVersion || "<missing>"}.`,
      );
    }
    ...
    console.log(`eve-npm-postpublish-verify: ${scenario.name} OK (${version})`);
  } finally {
...
async function main(argv = process.argv.slice(2)): Promise<void> {
  const args = parseEVENpmPostpublishVerifyArgs(argv);
  if (args.help) {
    console.log(eveNpmPostpublishVerifyUsage());
    return;
  }
...
if (entrypoint !== null && import.meta.url === entrypoint) {
  try {
    ...
    console.error(`eve-npm-postpublish-verify: ${formatErrorMessage(error)}`);
    process.exitCode = 1;
  }
}
