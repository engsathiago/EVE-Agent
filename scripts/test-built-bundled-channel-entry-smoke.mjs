...
process.env.EVE_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK ??= "1";

const { packageRoot } = parsePackageRootArg(
  process.argv.slice(2),
  "EVE_BUNDLED_CHANNEL_SMOKE_ROOT",
);
...
const installedLayoutEnv = "EVE_BUNDLED_CHANNEL_SMOKE_INSTALLED_LAYOUT";
...
function collectBundledChannelEntryFiles() {
  ...
  for (const dirent of fs.readdirSync(distExtensionsRoot, { withFileTypes: true })) {
    ...
    const extensionEntries =
      Array.isArray(packageJson.eve.extensions) && packageJson.eve.extensions.length > 0
    ...
    const setupEntry = packageJson.eve.setupEntry;
