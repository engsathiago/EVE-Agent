...
#
# Installs EVE through the public one-liner, verifies the resolved npm
...
MODELS_MODE="${EVE_E2E_MODELS:-both}" # both|openai|anthropic
INSTALL_TAG="${EVE_INSTALL_TAG:-latest}"
E2E_PREVIOUS_VERSION="${EVE_INSTALL_E2E_PREVIOUS:-}"
SKIP_PREVIOUS="${EVE_INSTALL_E2E_SKIP_PREVIOUS:-0}"
...
OPENAI_AGENT_MODEL="${EVE_INSTALL_E2E_OPENAI_MODEL:-openai/gpt-5.5}"
...
if [[ "$MODELS_MODE" != "both" && "$MODELS_MODE" != "openai" && "$MODELS_MODE" != "anthropic" ]]; then
  echo "ERROR: EVE_E2E_MODELS must be one of: both|openai|anthropic" >&2
  exit 2
fi

if [[ "$MODELS_MODE" == "both" ]]; then
  if [[ -z "$OPENAI_API_KEY" ]]; then
    echo "ERROR: EVE_E2E_MODELS=both requires OPENAI_API_KEY." >&2
    exit 2
  fi
  if [[ -z "$ANTHROPIC_API_TOKEN" && -z "$ANTHROPIC_API_KEY" ]]; then
    echo "ERROR: EVE_E2E_MODELS=both requires ANTHROPIC_API_TOKEN or ANTHROPIC_API_KEY." >&2
    exit 2
  fi
elif [[ "$MODELS_MODE" == "openai" && -z "$OPENAI_API_KEY" ]]; then
  echo "ERROR: EVE_E2E_MODELS=openai requires OPENAI_API_KEY." >&2
  exit 2
elif [[ "$MODELS_MODE" == "anthropic" && -z "$ANTHROPIC_API_TOKEN" && -z "$ANTHROPIC_API_KEY" ]]; then
  echo "ERROR: EVE_E2E_MODELS=anthropic requires ANTHROPIC_API_TOKEN or ANTHROPIC_API_KEY." >&2
  exit 2
fi
...
run_official_installer() {
  if [[ "$INSTALL_TAG" == "beta" ]]; then
    curl -fsSL "$INSTALL_URL" | EVE_BETA=1 bash
  elif [[ "$INSTALL_TAG" != "latest" ]]; then
    curl -fsSL "$INSTALL_URL" | EVE_VERSION="$INSTALL_TAG" bash
  else
...
verify_installed_version() {
  INSTALLED_VERSION="$(eve --version 2>/dev/null | head -n 1 | tr -d '\r')"
...
set_image_model() {
  ...
  local candidate
  for candidate in "$@"; do
    if eve --profile "$profile" models set-image "$candidate" >/dev/null 2>&1; then
      echo "$candidate"
      return 0
    fi
  done
...
set_agent_model() {
  ...
  shift
  for candidate in "$@"; do
    if eve --profile "$profile" models set "$candidate" >/dev/null 2>&1; then
      echo "$candidate"
      return 0
    fi
  done
...
dump_profile_debug() {
  ...
  for cmdline in /proc/[0-9]*/cmdline; do
    ...
    if [[ "$command" == *eve* || "$command" == *node* ]]; then
      echo "$pid $command"
    fi
  done
}
...
const maxBytes = readPositiveIntEnv("EVE_INSTALL_E2E_SESSION_SCAN_BYTES", 16 * 1024 * 1024);
const maxLineBytes = readPositiveIntEnv("EVE_INSTALL_E2E_SESSION_LINE_BYTES", 1024 * 1024);
const maxDepth = readPositiveIntEnv("EVE_INSTALL_E2E_SESSION_SCAN_DEPTH", 64);
const maxNodes = readPositiveIntEnv("EVE_INSTALL_E2E_SESSION_SCAN_NODES", 100000);
...
session_jsonl_path() {
  local profile="$1"
  local session_id="$2"
  echo "$HOME/.eve-${profile}/agents/main/sessions/${session_id}.jsonl"
}
...
run_profile() {
  ...
  if [[ "$agent_model_provider" == "openai" ]]; then
    agent_model="$(set_agent_model "$profile" \
      ...
      "openai/gpt-5.5" \
      "openai/gpt-5.4-mini")"
    eve --profile "$profile" config set models.providers.openai "{\"baseUrl\":\"https://api.openai.com/v1\",\"models\":[],\"timeoutSeconds\":${OPENAI_PROVIDER_TIMEOUT_SECONDS},\"agentRuntime\":{\"id\":\"eve\"}}" --strict-json >/dev/null
  ...
  eve --profile "$profile" gateway --port "$port" --bind loopback >"$GATEWAY_LOG" 2>&1 &
  GATEWAY_PID="$!"
  cleanup_profile() {
  ...
  for _ in $(seq 1 240); do
    if eve --profile "$profile" health --timeout 5000 --json >/dev/null 2>&1; then
      break
    fi
    sleep 0.25
  done
  if ! eve --profile "$profile" health --timeout 60000 --json >"$HEALTH_JSON" 2>&1; then
  ...
  if [[ "$AGENT_TOOL_SMOKE" == "0" ]]; then
    echo "Skip agent tool smoke ($profile, EVE_INSTALL_E2E_AGENT_TOOL_SMOKE=0)"
    cleanup_profile
    trap - EXIT
    return 0
  fi
...
if [[ "$MODELS_MODE" == "openai" || "$MODELS_MODE" == "both" ]]; then
  run_profile "e2e-openai" "18789" "/tmp/eve-e2e-openai" "openai"
fi

if [[ "$MODELS_MODE" == "anthropic" || "$MODELS_MODE" == "both" ]]; then
  run_profile "e2e-anthropic" "18799" "/tmp/eve-e2e-anthropic" "anthropic"
fi
