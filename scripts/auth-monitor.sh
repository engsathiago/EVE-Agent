...
#
# Suggested cron: */30 * * * * /home/admin/eve/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send EVE notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., eve-alerts)
...
STATE_FILE="$HOME/.eve/auth-monitor-state"
...
send_notification() {
    ...
    # Send via EVE if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use eve
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via EVE to $NOTIFY_PHONE..."
            eve send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! EVE is down. Run: ssh l36 '~/eve/scripts/mobile-reauth.sh'" "urgent"
    exit 1
