...
function usage() {
  return "Usage: node scripts/check-eve-package-tarball.mjs <eve.tgz>";
}
...
function parseArgs(argv) {
  ...
  if (tarball.startsWith("-")) {
    throw new Error(`Unknown EVE package tarball check option: ${tarball}`);
  }
  const extraArg = args[1]?.trim();
  if (extraArg) {
    throw new Error(`Unexpected EVE package tarball check argument: ${extraArg}`);
  }
...
if (!fs.existsSync(tarball)) {
  fail(`EVE package tarball does not exist: ${tarball}`);
}

const phaseTimingsEnabled = process.env.EVE_PACKAGE_TARBALL_CHECK_TIMINGS !== "0";
function runPhase(label, action) {
  ...
  try {
    ...
    if (phaseTimingsEnabled) {
      ...
      console.error(`check-eve-package-tarball: ${label} completed in ${durationMs}ms`);
    }
  }
}
...
const extractDir = fs.mkdtempSync(path.join(os.tmpdir(), "eve-package-tarball-"));
try {
...
if (errors.length > 0) {
  fs.rmSync(extractDir, { recursive: true, force: true });
  fail(`EVE package tarball integrity failed:\n${errors.join("\n")}`);
}

for (const warning of warnings) {
  console.warn(`EVE package tarball integrity warning: ${warning}`);
}
fs.rmSync(extractDir, { recursive: true, force: true });
console.log("EVE package tarball integrity passed.");
