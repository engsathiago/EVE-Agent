#!/usr/bin/env bash
# One-time host setup for rootless EVE in Podman. Uses the current
# non-root user throughout, builds or pulls the image into that user's Podman
# store, writes config under ~/.eve by default, and uses the repo-local
# launch script at ./scripts/run-eve-podman.sh.
#
...
#   Or set EVE_PODMAN_QUADLET=1 (or 0) to choose without a flag.
#
# After this, start the gateway manually:
#   ./scripts/run-eve-podman.sh launch
#   ./scripts/run-eve-podman.sh launch setup
# Or, if you used --quadlet:
#   systemctl --user start eve.service
set -euo pipefail

REPO_PATH="${EVE_REPO_PATH:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}"
source "$REPO_PATH/scripts/lib/host-timeout.sh"
RUN_SCRIPT_SRC="$REPO_PATH/scripts/run-eve-podman.sh"
QUADLET_TEMPLATE="$REPO_PATH/scripts/podman/eve.container.in"
EVE_USER="$(id -un)"
EVE_HOME="${HOME:-}"
EVE_CONFIG_DIR="${EVE_CONFIG_DIR:-}"
EVE_WORKSPACE_DIR="${EVE_WORKSPACE_DIR:-}"
EVE_IMAGE="${EVE_PODMAN_IMAGE:-${EVE_IMAGE:-eve:local}}"
EVE_CONTAINER_NAME="${EVE_PODMAN_CONTAINER:-eve}"
PLATFORM_NAME="$(uname -s 2>/dev/null || echo unknown)"
HOST_GATEWAY_PORT="${EVE_PODMAN_GATEWAY_HOST_PORT:-${EVE_GATEWAY_PORT:-18789}}"
QUADLET_GATEWAY_PORT="18789"
PODMAN_PULL_TIMEOUT="${EVE_PODMAN_SETUP_PULL_TIMEOUT:-600s}"
...
run_podman_build() {
  eve_host_timeout_cmd "$PODMAN_BUILD_TIMEOUT" podman build "$@"
}
...
PY
  ...
  echo "Missing dependency: need openssl or python3 (or od) to generate EVE_GATEWAY_TOKEN." >&2
  exit 1
}
...
if [[ -n "${EVE_PODMAN_QUADLET:-}" ]]; then
  case "${EVE_PODMAN_QUADLET,,}" in
...
if [[ "$EVE_IMAGE" == "eve:local" ]] && [[ ! -f "$REPO_PATH/Dockerfile" ]]; then
  echo "Dockerfile not found at $REPO_PATH. Set EVE_REPO_PATH to the repo root." >&2
  exit 1
fi
...
if [[ -z "$EVE_HOME" ]]; then
  EVE_HOME="$(resolve_user_home "$EVE_USER")"
fi
if [[ -z "$EVE_HOME" ]]; then
  echo "Unable to resolve HOME for user $EVE_USER." >&2
  exit 1
fi
if [[ -z "$EVE_CONFIG_DIR" ]]; then
  EVE_CONFIG_DIR="$EVE_HOME/.eve"
fi
if [[ -z "$EVE_WORKSPACE_DIR" ]]; then
  EVE_WORKSPACE_DIR="$EVE_CONFIG_DIR/workspace"
fi
validate_absolute_path "home directory" "$EVE_HOME"
validate_mount_source_path "config directory" "$EVE_CONFIG_DIR"
validate_mount_source_path "workspace directory" "$EVE_WORKSPACE_DIR"
validate_container_name "$EVE_CONTAINER_NAME"
validate_image_name "$EVE_IMAGE"
validate_port "gateway host port" "$HOST_GATEWAY_PORT"
validate_port "seed gateway port" "$SEED_GATEWAY_PORT"

install -d -m 700 "$EVE_CONFIG_DIR" "$EVE_WORKSPACE_DIR"
ensure_private_existing_dir_owned_by_user "config directory" "$EVE_CONFIG_DIR"
ensure_private_existing_dir_owned_by_user "workspace directory" "$EVE_WORKSPACE_DIR"

EVE_IMAGE_APT_PACKAGES="${EVE_IMAGE_APT_PACKAGES-${EVE_DOCKER_APT_PACKAGES:-}}"
EVE_IMAGE_PIP_PACKAGES="${EVE_IMAGE_PIP_PACKAGES:-}"
BUILD_ARGS=()
if [[ -n "$EVE_IMAGE_APT_PACKAGES" ]]; then
  BUILD_ARGS+=(--build-arg "EVE_IMAGE_APT_PACKAGES=${EVE_IMAGE_APT_PACKAGES}")
fi
if [[ -n "$EVE_IMAGE_PIP_PACKAGES" ]]; then
  BUILD_ARGS+=(--build-arg "EVE_IMAGE_PIP_PACKAGES=${EVE_IMAGE_PIP_PACKAGES}")
fi
if [[ -n "${EVE_EXTENSIONS:-}" ]]; then
  BUILD_ARGS+=(--build-arg "EVE_EXTENSIONS=${EVE_EXTENSIONS}")
fi
if [[ -n "${EVE_INSTALL_BROWSER:-}" ]]; then
  BUILD_ARGS+=(--build-arg "EVE_INSTALL_BROWSER=${EVE_INSTALL_BROWSER}")
fi

if [[ "$EVE_IMAGE" == "eve:local" ]]; then
  echo "Building image $EVE_IMAGE ..."
  run_podman_build -t "$EVE_IMAGE" -f "$REPO_PATH/Dockerfile" "${BUILD_ARGS[@]+"${BUILD_ARGS[@]}"}" "$REPO_PATH"
else
  if podman image exists "$EVE_IMAGE" >/dev/null 2>&1; then
    echo "Using existing image $EVE_IMAGE"
  else
    echo "Pulling image $EVE_IMAGE ..."
    run_podman_pull "$EVE_IMAGE"
  fi
fi

ENV_FILE="$EVE_CONFIG_DIR/.env"
if [[ ! -f "$ENV_FILE" ]]; then
...
EOF
  ...
  echo "Generated EVE_GATEWAY_TOKEN and wrote it to $ENV_FILE"
fi
upsert_env_var "$ENV_FILE" "EVE_PODMAN_CONTAINER" "$EVE_CONTAINER_NAME"
upsert_env_var "$ENV_FILE" "EVE_PODMAN_IMAGE" "$EVE_IMAGE"

CONFIG_JSON="$EVE_CONFIG_DIR/eve.json"
...
if [[ "$INSTALL_QUADLET" == true ]]; then
  QUADLET_DIR="$EVE_HOME/.config/containers/systemd"
  QUADLET_DST="$QUADLET_DIR/eve.container"
  echo "Installing Quadlet to $QUADLET_DST ..."
  mkdir -p "$QUADLET_DIR"
  ensure_safe_existing_dir "quadlet directory" "$QUADLET_DIR"
  EVE_HOME_ESCAPED="$(escape_sed_replacement_pipe_delim "$EVE_HOME")"
  EVE_CONFIG_ESCAPED="$(escape_sed_replacement_pipe_delim "$EVE_CONFIG_DIR")"
  EVE_WORKSPACE_ESCAPED="$(escape_sed_replacement_pipe_delim "$EVE_WORKSPACE_DIR")"
  EVE_IMAGE_ESCAPED="$(escape_sed_replacement_pipe_delim "$EVE_IMAGE")"
  EVE_CONTAINER_ESCAPED="$(escape_sed_replacement_pipe_delim "$EVE_CONTAINER_NAME")"
  sed \
    -e "s|{{EVE_HOME}}|$EVE_HOME_ESCAPED|g" \
    -e "s|{{EVE_CONFIG_DIR}}|$EVE_CONFIG_ESCAPED|g" \
    -e "s|{{EVE_WORKSPACE_DIR}}|$EVE_WORKSPACE_ESCAPED|g" \
    -e "s|{{IMAGE_NAME}}|$EVE_IMAGE_ESCAPED|g" \
    -e "s|{{CONTAINER_NAME}}|$EVE_CONTAINER_ESCAPED|g" \
    "$QUADLET_TEMPLATE" | write_file_atomically "$QUADLET_DST" 644

  if command -v systemctl >/dev/null 2>&1; then
    echo "Reloading and starting user service..."
    if systemctl --user daemon-reload && systemctl --user start eve.service; then
      echo "Quadlet installed and service started."
    else
      echo "Quadlet installed, but automatic start failed." >&2
      echo "Try: systemctl --user daemon-reload && systemctl --user start eve.service" >&2
...
echo
echo "Next:"
echo "  ./scripts/run-eve-podman.sh launch"
echo "  ./scripts/run-eve-podman.sh launch setup"
echo "  eve --container $EVE_CONTAINER_NAME dashboard --no-open"
