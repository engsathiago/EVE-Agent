...
const MANUAL_SECTION_START = "<!-- eve-plugin-reference:manual-start -->";
const MANUAL_SECTION_END = "<!-- eve-plugin-reference:manual-end -->";
...
function resolveDescription({ manifest, packageJson }) {
  ...
  if (channels.length > 0) {
    ...
    return `Adds the ${channelLabel}${channelNoun} surface for sending and receiving EVE messages.`;
  }

  const providers = Array.isArray(manifest.providers) ? manifest.providers : [];
  if (providers.length > 0) {
    return `Adds ${displayList(providers)} model provider support to EVE.`;
  }
  ...
  return packageDescription ? `${packageDescription}.` : "Provides an EVE plugin.";
}
...
function resolveDocs({ dirName, manifest, packageJson }) {
  ...
  const channelDoc = normalizeDocPath(packageJson.eve?.channel?.docsPath);
  if (channelDoc) {
...
function resolveInstallRoute(packageJson, status) {
  ...
  if (status === "core") {
    const release = packageJson.eve?.release;
    if (release?.publishToClawHub === true || release?.publishToNpm === true) {
      return `included in EVE; ${resolveInstallRoute(packageJson, "external")}`;
    }
    return "included in EVE";
  }
  const install = packageJson.eve?.install;
  const release = packageJson.eve?.release;
  const clawhubSpec =
...
function resolveStatus({ dirName, packageJson, excludedDirs }) {
  ...
  const hasInstallSpec =
    typeof packageJson.eve?.install?.clawhubSpec === "string" ||
    typeof packageJson.eve?.install?.npmSpec === "string";
...
function renderReferenceIndex(records) {
  ...
  return `---
summary: "Generated index of EVE plugin reference pages"
read_when:
  - You need a reference page for a specific EVE plugin
...
\`eve.plugin.json\`. Regenerate it with:
...
function collectPluginSourceEntries() {
  ...
  for (const dirName of fs
    ...
    const manifestPath = path.join(EXTENSIONS_DIR, dirName, "eve.plugin.json");
