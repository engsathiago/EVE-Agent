// Ios Version script supports EVE repository automation.
...
function main(argv = process.argv.slice(2)): number {
  ...
  if (options.format === "shell") {
    process.stdout.write(
      [
        `EVE_IOS_VERSION=${version.canonicalVersion}`,
        `EVE_MARKETING_VERSION=${version.marketingVersion}`,
        `EVE_BUILD_VERSION=${version.buildVersion}`,
      ].join("\n") + "\n",
    );
  } else {
