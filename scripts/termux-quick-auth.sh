...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/eve/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "EVE Re-Auth" -c "After getting key, run: ssh $SERVER '~/eve/scripts/mobile-reauth.sh'" --id eve-auth
        ;;
    *)
