...
export function resolveCompileConcurrency(
  ...
  const raw = env.EVE_EXTENSION_BOUNDARY_CONCURRENCY?.trim();
  if (raw) {
    return parsePositiveInt(raw, "EVE_EXTENSION_BOUNDARY_CONCURRENCY");
  }
...
async function runCompileCheck(extensionIds) {
  ...
  const verboseFreshLogs = process.env.EVE_EXTENSION_BOUNDARY_VERBOSE_FRESH === "1";
