...
Options:
  ...
  --keep                         Keep the synthetic EVE_HOME and workspace after the run.
...
export function parseArgs(argv) {
  ...
  const options = {
    fileCount: undefined,
    mode: process.env.EVE_MEMORY_FD_REPRO_MODE || "fixed",
    ...
    keep: process.env.EVE_MEMORY_FD_REPRO_KEEP === "1",
    allowNonDarwin: process.env.EVE_MEMORY_FD_REPRO_ALLOW_NON_DARWIN === "1",
  };
  ...
  options.fileCount ??= readPositiveNumberEnv("EVE_MEMORY_FD_REPRO_FILES", DEFAULT_FILE_COUNT);
  options.maxWorkspaceRegFds ??= readNumberEnv(
    "EVE_MEMORY_FD_REPRO_MAX_WORKSPACE_REG_FDS",
    DEFAULT_MAX_WORKSPACE_REG_FDS,
  );
  options.invokeTimeoutMs ??= readPositiveNumberEnv("EVE_MEMORY_FD_REPRO_TIMEOUT_MS", 30_000);
  options.sampleDelayMs ??= readNumberEnv("EVE_MEMORY_FD_REPRO_SAMPLE_DELAY_MS", 1_000);
  options.settleDelayMs ??= readNumberEnv("EVE_MEMORY_FD_REPRO_SETTLE_DELAY_MS", 5_000);
...
/**
 * Writes isolated EVE config for the synthetic memory workspace.
 */
export function writeConfig({ homeDir, workspaceDir, port, token }) {
  const configDir = path.join(homeDir, ".eve");
...
async function main() {
  ...
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "eve-memory-fd-repro-"));
  ...
  const env = {
    ...
    ...SKIP_GATEWAY_ENV,
    HOME: homeDir,
    EVE_STATE_DIR: path.join(homeDir, ".eve"),
