...
BASE_IMAGE="${BASE_IMAGE:-eve-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-eve-sandbox-common:bookworm-slim}"
...
EVE_DOCKER_BUILD_USE_BUILDX="${EVE_DOCKER_BUILD_USE_BUILDX:-0}"
EVE_DOCKER_BUILD_CACHE_FROM="${EVE_DOCKER_BUILD_CACHE_FROM:-}"
EVE_DOCKER_BUILD_CACHE_TO="${EVE_DOCKER_BUILD_CACHE_TO:-}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=eve.sandbox=1)
NOTE
