...
# EVE CLI installer (non-interactive, no onboarding)
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://eve.ai/install-cli.sh | bash -s -- [--json] [--prefix <path>] [--version <ver>] [--node-version <ver>] [--onboard]
...
EVE_EFFECTIVE_HOME="$(resolve_eve_effective_home)"
PREFIX="${EVE_PREFIX:-${HOME}/.eve}"
EVE_VERSION="${EVE_VERSION:-latest}"
NODE_VERSION="${EVE_NODE_VERSION:-22.22.0}"
NODE_VERSION_REQUESTED=0
if [[ -n "${EVE_NODE_VERSION:-}" ]]; then
  NODE_VERSION_REQUESTED=1
fi
MIN_NODE_VERSION="22.19.0"
APK_NODE_BIN_DIR="/usr/bin"
NPM_LOGLEVEL="${EVE_NPM_LOGLEVEL:-error}"
INSTALL_METHOD="${EVE_INSTALL_METHOD:-npm}"
GIT_DIR="${EVE_GIT_DIR:-${EVE_EFFECTIVE_HOME}/eve}"
...
Usage: install-cli.sh [options]
  ...
  --prefix <path>                     Install prefix (default: ~/.eve; use \$EVE_PREFIX to override)
...
Environment variables:
  ...
  EVE_HOME=...
  EVE_PREFIX=...
  EVE_VERSION=latest|next|<semver>
  EVE_GIT_DIR=...
  EVE_GIT_UPDATE=0|1
EOF
}
...
cleanup_legacy_submodules() {
  local repo_dir="${1:-${EVE_GIT_DIR:-${EVE_EFFECTIVE_HOME}/eve}}"
...
install_eve() {
  local requested="${EVE_VERSION:-latest}"
  if is_eve_source_package_install_spec "$requested"; then
    fail "npm installs do not support EVE GitHub source targets like '${requested}'. Use --install-method git --version main, latest, beta, an exact version, or a built .tgz package."
  fi
  ...
  emit_json "{\"event\":\"step\",\"name\":\"eve\",\"status\":\"start\",\"version\":\"${requested}\"}"
  log "Installing EVE (${requested})..."
...
install_eve_from_git() {
  ...
  emit_json "{\"event\":\"step\",\"name\":\"eve\",\"status\":\"start\",\"method\":\"git\",\"repo\":\"${repo_url//\"/\\\"}\"}"
  if [[ -d "$repo_dir/.git" ]]; then
    log "Installing Eve from git checkout: ${repo_dir}"
  else
    log "Installing Eve from GitHub (${repo_url})..."
  fi
...
EOF
  chmod +x "${PREFIX}/bin/eve"
  emit_json "{\"event\":\"step\",\"name\":\"eve\",\"status\":\"ok\",\"method\":\"git\"}"
}

resolve_eve_version() {
  local version=""
  if [[ -x "${PREFIX}/bin/eve" ]]; then
    version="$("${PREFIX}/bin/eve" --version 2>/dev/null | head -n 1 | tr -d '\r')"
  fi
  echo "$version"
}
...
main() {
  ...
  if [[ -n "$installed_version" ]]; then
    ...
    log "EVE installed (${installed_version})."
  else
  ...
  if [[ "$RUN_ONBOARD" -eq 1 ]]; then
    "${PREFIX}/bin/eve" onboard
  fi
}

if [[ "${EVE_INSTALL_CLI_SH_NO_RUN:-0}" != "1" ]]; then
  main "$@"
fi
