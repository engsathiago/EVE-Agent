...
Optional custom relay:
  EVE_PUSH_RELAY_BASE_URL=https://relay.example.com \
    scripts/ios-release-prepare.sh --build-number 7 [--team-id TEAMID]
...
- regenerates apps/ios/EVE.xcodeproj via xcodegen
EOF
}
...
DEFAULT_IOS_PUSH_RELAY_BASE_URL="https://ios-push-relay.eve.ai"
PUSH_RELAY_BASE_URL="${EVE_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-${DEFAULT_IOS_PUSH_RELAY_BASE_URL}}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid EVE_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid EVE_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid EVE_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid EVE_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ "${TEAM_ID}" != "${CANONICAL_TEAM_ID}" ]]; then
  echo "iOS App Store release must use canonical EVE Team ID ${CANONICAL_TEAM_ID}; got ${TEAM_ID}." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(EVE_URL_SLASH)$(EVE_URL_SLASH)#g'
)"
...
EVE_APP_BUNDLE_ID = ai.evefoundation.app
EVE_SHARE_BUNDLE_ID = ai.evefoundation.app.share
EVE_ACTIVITY_WIDGET_BUNDLE_ID = ai.evefoundation.app.activitywidget
EVE_WATCH_APP_BUNDLE_ID = ai.evefoundation.app.watchkitapp
EVE_APNS_ENTITLEMENT_ENVIRONMENT = production
EVE_PUSH_TRANSPORT = relay
EVE_PUSH_DISTRIBUTION = official
EVE_URL_SLASH = /
EVE_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
EVE_PUSH_APNS_ENVIRONMENT = production
EOF
