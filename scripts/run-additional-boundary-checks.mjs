...
export function parseCliArgs(args, env = process.env) {
  let shardSpec = env.EVE_ADDITIONAL_BOUNDARY_SHARD ?? "";
  let help = false;
  ...
  try {
    ...
    if (cliArgs.help) {
      ...
      const concurrencyRaw =
        process.env.EVE_ADDITIONAL_BOUNDARY_CONCURRENCY ??
        process.env.EVE_EXTENSION_BOUNDARY_CONCURRENCY;
      const concurrencyLabel =
        process.env.EVE_ADDITIONAL_BOUNDARY_CONCURRENCY === undefined
          ? "EVE_EXTENSION_BOUNDARY_CONCURRENCY"
          : "EVE_ADDITIONAL_BOUNDARY_CONCURRENCY";
      const concurrency = resolveConcurrency(concurrencyRaw, 4, concurrencyLabel);
      const checkTimeoutMs = resolvePositiveInteger(
        process.env.EVE_ADDITIONAL_BOUNDARY_TIMEOUT_MS,
        DEFAULT_CHECK_TIMEOUT_MS,
        "EVE_ADDITIONAL_BOUNDARY_TIMEOUT_MS",
      );
      const outputMaxBytes = resolvePositiveInteger(
        process.env.EVE_ADDITIONAL_BOUNDARY_OUTPUT_MAX_BYTES,
        DEFAULT_OUTPUT_MAX_BYTES,
        "EVE_ADDITIONAL_BOUNDARY_OUTPUT_MAX_BYTES",
      );
