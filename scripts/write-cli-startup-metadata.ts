// Write Cli Startup Metadata script supports EVE repository automation.
...
import type { EVEConfig } from "../src/config/config.js";
...
export function readBundledChannelCatalog(
  ...
  for (const dirEntry of readdirSync(extensionsDirOverride, { withFileTypes: true })) {
    ...
    try {
      ...
      const id = parsed.eve?.channel?.id;
      if (typeof id !== "string" || !id.trim()) {
        continue;
      }
      const orderRaw = parsed.eve?.channel?.order;
      const labelRaw = parsed.eve?.channel?.label;
      entries.push({
        id: id.trim(),
...
function createIsolatedRootHelpRenderContext(
  ...
  const stateDir = path.join(rootDir, ".eve-build-root-help");
  const workspaceDir = path.join(stateDir, "workspace");
  const homeDir = path.join(stateDir, "home");
  const env: NodeJS.ProcessEnv = {
    HOME: homeDir,
    LOGNAME: process.env.LOGNAME ?? process.env.USER ?? "eve-build",
    USER: process.env.USER ?? process.env.LOGNAME ?? "eve-build",
...
async function renderSourceCommandHelpText(
  ...
  return await spawnText(["eve.mjs", command, "--help"], {
    cwd: rootDir,
    env: {
      ...renderContext.env,
      EVE_DISABLE_CLI_STARTUP_HELP_FAST_PATH: "1",
