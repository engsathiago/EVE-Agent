#!/usr/bin/env bash
# Installs EVE from a prepared package tarball, installs @eve/codex
...
CANDIDATE_ROOT="${EVE_LIVE_DOCKER_REPO_ROOT:-$SCRIPT_ROOT_DIR}"
...
IMAGE_NAME="$(docker_e2e_resolve_image "eve-codex-npm-plugin-live-e2e" EVE_CODEX_NPM_PLUGIN_E2E_IMAGE)"
DOCKER_TARGET="${EVE_CODEX_NPM_PLUGIN_DOCKER_TARGET:-bare}"
HOST_BUILD="${EVE_CODEX_NPM_PLUGIN_HOST_BUILD:-1}"
PACKAGE_TGZ="${EVE_CURRENT_PACKAGE_TGZ:-}"
PROFILE_FILE="${EVE_CODEX_NPM_PLUGIN_PROFILE_FILE:-${EVE_TESTBOX_PROFILE_FILE:-$HOME/.eve-testbox-live.profile}}"
CODEX_PLUGIN_SPEC="${EVE_CODEX_NPM_PLUGIN_SPEC:-}"
CODEX_PLUGIN_MOUNT=()
CODEX_PLUGIN_PACK_DIR=""
ASSERT_MAX_TEXT_FILE_BYTES="$(
  docker_e2e_read_positive_int_env EVE_CODEX_NPM_PLUGIN_ASSERT_MAX_TEXT_FILE_BYTES 1048576
)"
ASSERT_MAX_ERROR_TAIL_BYTES="$(
  docker_e2e_read_positive_int_env EVE_CODEX_NPM_PLUGIN_ASSERT_MAX_ERROR_TAIL_BYTES 65536
)"
ASSERT_MAX_TRANSCRIPT_FILES="$(
  docker_e2e_read_positive_int_env EVE_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_FILES 64
)"
ASSERT_MAX_TRANSCRIPT_WALK_ENTRIES="$(
  docker_e2e_read_positive_int_env EVE_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_WALK_ENTRIES 4096
)"
ASSERT_MAX_TRANSCRIPT_SCAN_BYTES="$(
  docker_e2e_read_positive_int_env EVE_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_SCAN_BYTES 2097152
)"
...
prepare_package_tgz() {
  ...
  if [ "$HOST_BUILD" = "0" ] && [ -z "${EVE_CURRENT_PACKAGE_TGZ:-}" ]; then
    echo "EVE_CODEX_NPM_PLUGIN_HOST_BUILD=0 requires EVE_CURRENT_PACKAGE_TGZ" >&2
    exit 1
  fi
...
prepare_codex_plugin_spec() {
  ...
  if [ -z "$CODEX_PLUGIN_SPEC" ]; then
    CODEX_PLUGIN_PACK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/eve-codex-plugin-pack.XXXXXX")"
    (
      cd "$CANDIDATE_ROOT"
    ...
    ) >/tmp/eve-codex-plugin-pack.log 2>&1
    pack_output=()
    ...
    if [ "${#pack_output[@]}" -ne 1 ]; then
      ...
      docker_e2e_print_log /tmp/eve-codex-plugin-pack.log >&2
      exit 1
    fi
...
EVE_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 codex-npm-plugin-live empty)"
...
if ! docker_e2e_run_with_harness \
  ...
  -e EVE_CODEX_NPM_PLUGIN_ALLOW_BETA_COMPAT_DIAGNOSTICS="${EVE_CODEX_NPM_PLUGIN_ALLOW_BETA_COMPAT_DIAGNOSTICS:-0}" \
  -e EVE_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL="${EVE_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL:-1}" \
  -e EVE_CODEX_NPM_PLUGIN_MODEL="${EVE_CODEX_NPM_PLUGIN_MODEL:-codex/gpt-5.4}" \
  -e EVE_CODEX_NPM_PLUGIN_SPEC="$CODEX_PLUGIN_SPEC" \
  -e "EVE_CODEX_NPM_PLUGIN_ASSERT_MAX_TEXT_FILE_BYTES=$ASSERT_MAX_TEXT_FILE_BYTES" \
  -e "EVE_CODEX_NPM_PLUGIN_ASSERT_MAX_ERROR_TAIL_BYTES=$ASSERT_MAX_ERROR_TAIL_BYTES" \
  -e "EVE_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_FILES=$ASSERT_MAX_TRANSCRIPT_FILES" \
  -e "EVE_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_WALK_ENTRIES=$ASSERT_MAX_TRANSCRIPT_WALK_ENTRIES" \
  -e "EVE_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_SCAN_BYTES=$ASSERT_MAX_TRANSCRIPT_SCAN_BYTES" \
  -e "EVE_CODEX_NPM_PLUGIN_AGENT_TIMEOUT_SECONDS=$AGENT_TURN_TIMEOUT_SECONDS" \
  -e "EVE_E2E_COMMAND_TIMEOUT=$COMMAND_TIMEOUT" \
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
...
CODEX_PLUGIN_SPEC="${EVE_CODEX_NPM_PLUGIN_SPEC:?missing EVE_CODEX_NPM_PLUGIN_SPEC}"
MODEL_REF="${EVE_CODEX_NPM_PLUGIN_MODEL:?missing EVE_CODEX_NPM_PLUGIN_MODEL}"
POST_UNINSTALL_MODEL_REF="codex/${MODEL_REF#*/}"
SESSION_ID="codex-npm-plugin-live"
SUCCESS_MARKER="EVE-CODEX-NPM-PLUGIN-LIVE-OK"
AGENT_TURN_TIMEOUT_SECONDS="${EVE_CODEX_NPM_PLUGIN_AGENT_TIMEOUT_SECONDS:-420}"
PLUGIN_INSTALL_FLAGS=(--force)
if [ "${EVE_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL:-0}" = "1" ]; then
...
dump_debug_logs() {
  ...
  eve_e2e_dump_logs \
    /tmp/eve-install.log \
    /tmp/eve-codex-plugin-install.log \
    /tmp/eve-codex-plugin-enable.log \
    /tmp/eve-codex-plugins-list.json \
    /tmp/eve-codex-plugin-inspect.json \
    /tmp/eve-codex-preflight.log \
    /tmp/eve-codex-agent.json \
    /tmp/eve-codex-agent.err \
    /tmp/eve-codex-agent-turn1.json \
    /tmp/eve-codex-agent-turn1.err \
    /tmp/eve-codex-agent-turn2.json \
    /tmp/eve-codex-agent-turn2.err \
    /tmp/eve-codex-plugin-uninstall.log \
    /tmp/eve-codex-plugins-list-after-uninstall.json \
...
eve plugins install "$CODEX_PLUGIN_SPEC" "${PLUGIN_INSTALL_FLAGS[@]}" >/tmp/eve-codex-plugin-install.log 2>&1

node scripts/e2e/lib/codex-npm-plugin-live/assertions.mjs configure "$MODEL_REF"

echo "Enabling Codex plugin..."
eve plugins enable codex >/tmp/eve-codex-plugin-enable.log 2>&1

eve plugins list --json >/tmp/eve-codex-plugins-list.json
eve plugins inspect codex --runtime --json >/tmp/eve-codex-plugin-inspect.json
...
"$CODEX_BIN" exec \
  ...
  --color never \
  --skip-git-repo-check \
  "Reply exactly: ${SUCCESS_MARKER}-PREFLIGHT" >/tmp/eve-codex-preflight.log 2>&1 </dev/null
...
echo "Running EVE local agent turns through npm-installed Codex plugin..."
run_agent_turn \
  "turn1" \
  "${SUCCESS_MARKER}-TURN-1" \
  "Reply in one short sentence. Include token ${SUCCESS_MARKER}-TURN-1 and say hello from the EVE Codex plugin." \
  /tmp/eve-codex-agent-turn1.json \
  /tmp/eve-codex-agent-turn1.err
run_agent_turn \
  "turn2" \
...
eve plugins uninstall codex --force >/tmp/eve-codex-plugin-uninstall.log 2>&1
eve plugins list --json >/tmp/eve-codex-plugins-list-after-uninstall.json
...
if eve agent --local \
  ...
  --thinking low \
  --timeout 120 \
  --json >/tmp/eve-codex-agent-after-uninstall.json 2>/tmp/eve-codex-agent-after-uninstall.err; then
  echo "Expected EVE agent to fail after Codex uninstall, got status 0" >&2
  exit 1
fi
if ! grep -Fq 'Requested agent harness "codex" is not registered' /tmp/eve-codex-agent-after-uninstall.err &&
  ! grep -Fq 'Unknown model: codex/' /tmp/eve-codex-agent-after-uninstall.err; then
  echo "Unexpected post-uninstall agent error:" >&2
  tail -n 120 /tmp/eve-codex-agent-after-uninstall.err >&2 || true
  exit 1
fi
