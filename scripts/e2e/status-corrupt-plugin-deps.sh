...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/eve-status-corrupt-plugin-deps.XXXXXX")"
cleanup() {
  rm -rf "$TMP_DIR"
}
trap cleanup EXIT
...
const stageDir = process.env.EVE_PLUGIN_STAGE_DIR || "";
const renameResidue = path.join(stageDir, "node_modules", "ansi-escapes", ".eve-rename-tmp");
...
run_eve() {
  HOME="$HOME_DIR" \
  EVE_HOME="$STATE_DIR" \
  EVE_STATE_DIR="$STATE_DIR" \
  EVE_CONFIG_PATH="$CONFIG_PATH" \
  EVE_PLUGIN_STAGE_DIR="$STAGE_DIR" \
...
run_eve status --all --timeout 1 > "$BEFORE"
grep -F "e2e-corrupt-chat" "$BEFORE" >/dev/null
grep -F "plugin load failed: dependency tree corrupted; run eve doctor --fix" "$BEFORE" >/dev/null

run_eve doctor --fix --non-interactive --yes > "$DOCTOR"
if [[ -e "$STAGE_DIR" ]]; then
  echo "doctor --fix did not remove corrupt plugin stage dir: $STAGE_DIR" >&2
  exit 1
fi

run_eve status --all --timeout 1 > "$AFTER"
