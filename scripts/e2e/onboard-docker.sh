...
IMAGE_NAME="$(docker_e2e_resolve_image "eve-onboard-e2e" EVE_ONBOARD_E2E_IMAGE)"
EVE_TEST_STATE_FUNCTION_B64="$(docker_e2e_test_state_function_b64)"
MAX_MEMORY_MIB="$(docker_e2e_read_nonnegative_decimal_env EVE_ONBOARD_MAX_MEMORY_MIB 2048)"
MAX_CPU_PERCENT="$(docker_e2e_read_nonnegative_decimal_env EVE_ONBOARD_MAX_CPU_PERCENT 1200)"
DOCKER_RUN_TIMEOUT="${EVE_ONBOARD_DOCKER_RUN_TIMEOUT:-1200s}"
...
CONTAINER_NAME="eve-onboard-e2e-$$"
RUN_LOG="$(mktemp "${TMPDIR:-/tmp}/eve-onboard.XXXXXX")"
STATS_LOG="$(mktemp "${TMPDIR:-/tmp}/eve-onboard-stats.XXXXXX")"
...
DOCKER_COMMAND_TIMEOUT="$DOCKER_RUN_TIMEOUT" docker_e2e_docker_run_cmd run --name "$CONTAINER_NAME" "${DOCKER_E2E_HARNESS_ARGS[@]}" -t \
  ...
  -e "EVE_ONBOARD_GATEWAY_WAIT_ATTEMPTS=$GATEWAY_WAIT_ATTEMPTS" \
