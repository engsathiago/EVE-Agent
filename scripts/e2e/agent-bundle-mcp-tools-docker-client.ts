// EVE bundle MCP tools Docker harness.
...
server.tool("docker_probe", "Docker EVE MCP tool availability probe", async () => ({
...
async function main() {
  const tempState = await createE2eStateDir("eve-agent-bundle-mcp-");
