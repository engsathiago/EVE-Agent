...
IMAGE_NAME="$(docker_e2e_resolve_image "eve-mcp-code-mode-gateway-live-e2e" EVE_IMAGE)"
PORT="$(docker_e2e_read_tcp_port_env EVE_MCP_CODE_MODE_LIVE_GATEWAY_PORT 18789)"
...
CONTAINER_NAME="eve-mcp-code-mode-live-e2e-$$"
PROFILE_FILE="${EVE_MCP_CODE_MODE_LIVE_PROFILE_FILE:-${EVE_TESTBOX_PROFILE_FILE:-$HOME/.eve-testbox-live.profile}}"

CLIENT_LOG="$(mktemp -t eve-mcp-code-mode-live-log.XXXXXX)"
...
EVE_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 mcp-code-mode-gateway-live empty)"

# The profile is only a credential source. Keep this lane's EVE runtime
...
docker_e2e_run_with_harness \
  ...
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
  -e "EVE_DOCKER_OPENAI_BASE_URL=${OPENAI_BASE_URL:-https://api.openai.com/v1}" \
  -e "EVE_GATEWAY_TOKEN=$TOKEN" \
  -e "EVE_SKIP_CHANNELS=1" \
  -e "EVE_SKIP_GMAIL_WATCHER=1" \
  -e "EVE_SKIP_CRON=1" \
  -e "EVE_SKIP_CANVAS_HOST=1" \
  -e "EVE_SKIP_ACPX_RUNTIME=1" \
  -e "EVE_SKIP_ACPX_RUNTIME_PROBE=1" \
  -e "EVE_TEST_STATE_SCRIPT_B64=$EVE_TEST_STATE_SCRIPT_B64" \
  -e "GW_URL=http://127.0.0.1:$PORT" \
  -e "GW_TOKEN=$TOKEN" \
  -e "EVE_MCP_CODE_MODE_CLIENT_TIMEOUT_MS=$CLIENT_TIMEOUT_MS" \
  -e "EVE_MCP_CODE_MODE_CLIENT_BODY_MAX_BYTES=$CLIENT_BODY_MAX_BYTES" \
  -e "EVE_ALLOW_INSECURE_PRIVATE_WS=1" \
  -e "EVE_MCP_CODE_MODE_MODEL=${EVE_MCP_CODE_MODE_LIVE_MODEL:-eve/main}" \
  "${PROFILE_MOUNT[@]}" \
  "$IMAGE_NAME" \
  bash -lc "set -euo pipefail
    ...
    gateway_pid=\"\$(eve_e2e_start_gateway \"\$entry\" $PORT /tmp/mcp-code-mode-live-gateway.log)\"
    eve_e2e_wait_gateway_ready \"\$gateway_pid\" /tmp/mcp-code-mode-live-gateway.log 480 $PORT
