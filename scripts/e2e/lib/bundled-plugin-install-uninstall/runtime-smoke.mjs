...
const RUNTIME_PORT_BASE_ENV = "EVE_BUNDLED_PLUGIN_RUNTIME_PORT_BASE";
...
function manifestPath(pluginDir, pluginRoot) {
  const candidates = [
    ...(isNonEmptyString(pluginRoot) ? [path.join(pluginRoot, "eve.plugin.json")] : []),
    path.join(process.cwd(), "dist", "extensions", pluginDir, "eve.plugin.json"),
    path.join(process.cwd(), "dist-runtime", "extensions", pluginDir, "eve.plugin.json"),
  ];
  ...
  const child = childProcess.spawn(
    ...
    ],
    {
      env: {
        ...
        ...params.env,
        EVE_NO_ONBOARD: "1",
        EVE_SKIP_CHANNELS: params.skipChannels ? "1" : "0",
...
export async function rpcCall(method, params, options) {
  const rpcStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "eve-plugin-runtime-rpc-"));
  const args = [
    options.entrypoint,
    "gateway",
    "call",
...
async function smokePlugin(pluginId, pluginDir, requiresConfig, pluginIndex, pluginRoot) {
  ...
  const entrypoint = process.env.EVE_ENTRY;
  if (!entrypoint) {
    throw new Error("missing EVE_ENTRY");
  }
  ...
  const logPath = `/tmp/eve-plugin-runtime-${pluginIndex}-${pluginId}.log`;
...
async function smokeTtsGlobalDisable(pluginId, pluginDir, provider, pluginIndex, pluginRoot) {
  ...
  const logPath = `/tmp/eve-plugin-runtime-${pluginIndex}-${pluginId}-tts-disabled.log`;
...
async function smokeOpenAiTts(pluginIndex) {
  ...
  const logPath = `/tmp/eve-plugin-runtime-${pluginIndex}-openai-tts-live.log`;
  const child = startGateway({ entrypoint, port, logPath, env, skipChannels: true });
  try {
    await waitForReady({ child, port, logPath });
    await assertBaseGatewayProbes({ entrypoint, port, env });
...
export function createIsolatedStateEnv(label) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `eve-${label}-`));
