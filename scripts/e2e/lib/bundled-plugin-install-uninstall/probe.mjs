...
function resolveStateDir() {
  if (process.env.EVE_STATE_DIR) {
    return process.env.EVE_STATE_DIR;
  }
  return path.join(process.env.HOME || os.homedir(), ".eve");
}
...
function resolveEVEEntry() {
  if (process.env.EVE_ENTRY) {
    return process.env.EVE_ENTRY;
  }
  ...
  throw new Error("Missing EVE_ENTRY and dist/index.(m)js");
}

function readPluginsList() {
  const entry = resolveEVEEntry();
...
async function loadManifestEntries() {
  const explicit = (process.env.EVE_BUNDLED_PLUGIN_SWEEP_IDS || "")
    .split(/[,\s]+/u)
  ...
  return explicit.map((lookup) => {
    ...
    if (!found) {
      throw new Error(
        `EVE_BUNDLED_PLUGIN_SWEEP_IDS entry is not an installable bundled plugin in this package: ${lookup}. Available: ${available}`,
      );
    }
    return found;
  });
...
async function selectedManifestEntries() {
  ...
  const total = readPositiveIntEnv("EVE_BUNDLED_PLUGIN_SWEEP_TOTAL", 1);
  const index = readNonNegativeIntEnv("EVE_BUNDLED_PLUGIN_SWEEP_INDEX", 0);
  if (index >= total) {
    throw new Error(
      `EVE_BUNDLED_PLUGIN_SWEEP_INDEX must be in [0, ${total - 1}], got ${process.env.EVE_BUNDLED_PLUGIN_SWEEP_INDEX}`,
    );
  }
