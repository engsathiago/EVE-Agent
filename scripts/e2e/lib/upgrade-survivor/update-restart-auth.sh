...
write_update_restart_service_auth_env() {
  ...
  printf 'GATEWAY_AUTH_TOKEN_REF=%s\n' "$GATEWAY_AUTH_TOKEN_REF" >"$EVE_STATE_DIR/gateway.systemd.env"
}

prepare_update_restart_probe_current_install() {
  local port="$1"
  local log_file="$2"
  local command_timeout="${EVE_UPGRADE_SURVIVOR_COMMAND_TIMEOUT:-900s}"
  local start_epoch
  local ready_epoch
  ...
  env -u EVE_GATEWAY_TOKEN -u EVE_GATEWAY_PASSWORD eve gateway --port "$port" --bind loopback --allow-unconfigured >"$log_file" 2>&1 &
  gateway_pid="$!"
  printf '%s\n' "$gateway_pid" >"$EVE_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_PID_FILE"
  eve_e2e_wait_gateway_ready "$gateway_pid" "$log_file" 360 "$port"
  ready_epoch="$(node -e "process.stdout.write(String(Date.now()))")"
  start_seconds=$(((ready_epoch - start_epoch + 999) / 1000))
  write_update_restart_service_auth_env
  if ! eve_e2e_maybe_timeout "$command_timeout" env -u EVE_GATEWAY_TOKEN -u EVE_GATEWAY_PASSWORD eve gateway install --force --json >"$EVE_UPGRADE_SURVIVOR_BASELINE_SERVICE_INSTALL_JSON" 2>"$EVE_UPGRADE_SURVIVOR_BASELINE_SERVICE_INSTALL_ERR"; then
    echo "gateway service install failed" >&2
    cat "$EVE_UPGRADE_SURVIVOR_BASELINE_SERVICE_INSTALL_ERR" >&2 || true
    cat "$EVE_UPGRADE_SURVIVOR_BASELINE_SERVICE_INSTALL_JSON" >&2 || true
    return 1
  fi
}
