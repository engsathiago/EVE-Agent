        ...
        resolvedSkills: [
          {
            ...
            filePath: "/tmp/eve-old-package/skills/legacy-heavy-skill-cache/SKILL.md",
          },
        ],
      },
    },
...
function getScenario() {
  const scenario = process.env.EVE_UPGRADE_SURVIVOR_SCENARIO || "base";
  assert(SCENARIOS.has(scenario), `unknown upgrade survivor scenario: ${scenario}`);
  return scenario;
}

function getConfig() {
  return readJson(requireEnv("EVE_CONFIG_PATH"));
}

function getCoverage() {
  const file = process.env.EVE_UPGRADE_SURVIVOR_CONFIG_COVERAGE_JSON;
...
function seedState() {
  ...
  const workspace = requireEnv("EVE_TEST_WORKSPACE_DIR");
  ...
  writeJson(path.join(workspace, ".eve", "workspace-state.json"), {
    version: 1,
  ...
  for (const plugin of ["discord", "telegram", "whatsapp"]) {
    writeJson(path.join(runtimeRoot, plugin, ".eve-runtime-deps-stamp.json"), {
      version: 0,
      plugin,
      stale: true,
    });
...
function assertConfigSurvived() {
  ...
  if (hasCoverage(coverage) && acceptsIntent(coverage, "logging")) {
    assert(
      config.logging?.file === "~/eve-upgrade-survivor/gateway.jsonl",
...
function assertStateSurvived() {
  ...
  const workspace = requireEnv("EVE_TEST_WORKSPACE_DIR");
  const scenario = getScenario();
  const stage = process.env.EVE_UPGRADE_SURVIVOR_ASSERT_STAGE || "survival";
  ...
  if (scenario === "stale-source-plugin-shadow") {
    const staleRoot = path.join(stateDir, "extensions", "opik-eve");
    assert(
  ...
  if (scenario === "versioned-runtime-deps") {
    ...
    const staleVersionedRoots = fs.existsSync(runtimeRoot)
      ? fs.readdirSync(runtimeRoot).filter((entry) => entry.startsWith(`eve-${version}-`))
      : [];
    assert(
...
function readMigratedSessionStore(stateDir, targetStorePath) {
  ...
  const dbPath = path.join(stateDir, "agents", "main", "agent", "eve-agent.sqlite");
...
function assertExternalPluginInstall(records, pluginId, packageName) {
  ...
  const extensionsRoot = path.join(requireEnv("EVE_STATE_DIR"), "extensions");
  assert(
