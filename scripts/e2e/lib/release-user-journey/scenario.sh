...
export OPENAI_API_KEY="sk-eve-release-user-journey"
export EVE_GATEWAY_TOKEN="release-user-journey-token"
export CLICKCLACK_BOT_TOKEN="clickclack-release-token"

PORT="18789"
MOCK_PORT="44180"
CLICKCLACK_PORT="44181"
SUCCESS_MARKER="EVE_E2E_OK_RELEASE_USER_JOURNEY"
...
eve_e2e_install_package "$INSTALL_LOG"
...
eve plugins install "$plugin_a_dir" >"$PLUGIN_A_INSTALL_LOG" 2>&1
node scripts/e2e/lib/release-user-journey/assertions.mjs \
  ...
  "$plugin_a_dir"
eve journey-a ping >"$PLUGIN_A_CLI_LOG" 2>&1
...
eve plugins uninstall journey-plugin-a --force >"$PLUGIN_A_UNINSTALL_LOG" 2>&1
node scripts/e2e/lib/release-user-journey/assertions.mjs \
...
eve plugins install "$plugin_b_dir" >"$PLUGIN_B_INSTALL_LOG" 2>&1
eve journey-b ping >"$PLUGIN_B_CLI_LOG" 2>&1
...
eve plugins inspect journey-plugin-b --runtime --json >"$PLUGIN_B_AFTER_RESTART_JSON" 2>&1
eve channels status --json >"$STATUS_AFTER_RESTART_JSON" 2>"$STATUS_AFTER_RESTART_ERR"
...
eve doctor --repair --non-interactive >"$DOCTOR_LOG" 2>&1
