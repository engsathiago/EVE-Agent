...
function clickClackHttpTimeoutMs() {
  return readPositiveInt(
    process.env.EVE_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS,
    5000,
    "EVE_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS",
  );
}

function clickClackHttpBodyMaxBytes() {
  return readPositiveInt(
    process.env.EVE_RELEASE_USER_JOURNEY_HTTP_BODY_MAX_BYTES,
    1024 * 1024,
...
function assertOnboard() {
  ...
  assert(fs.existsSync(configPath()), "onboard did not write eve.json");
  const stateRaw =
