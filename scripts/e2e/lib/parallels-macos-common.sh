...
parallels_macos_desktop_user_exec_with_secret_file() {
  ...
  local secret_path
  secret_path="/tmp/eve-secret-${api_key_env:-env}-$RANDOM-$RANDOM"
  ...
  local wrapper
  local wrapper_path
  wrapper_path="/tmp/eve-secret-env-wrapper-$RANDOM-$RANDOM.sh"
  wrapper='#!/bin/bash
set -e
cleanup() {
  rm -f "${EVE_WRAPPER_FILE:-}"
}
trap cleanup EXIT
if [ -n "${EVE_SECRET_ENV_NAME:-}" ] && [ -n "${EVE_SECRET_FILE:-}" ] && [ -f "$EVE_SECRET_FILE" ]; then
  secret_value="$(cat "$EVE_SECRET_FILE")"
  rm -f "$EVE_SECRET_FILE"
  export "${EVE_SECRET_ENV_NAME}=${secret_value}"
fi
"$@"
'
  ...
  if [[ "$user_flag" == "current-user" ]]; then
    prlctl exec "$vm_name" --current-user /usr/bin/env \
      "PATH=$path_value" \
      "EVE_SECRET_ENV_NAME=$api_key_env" \
      "EVE_SECRET_FILE=$secret_path" \
      "EVE_WRAPPER_FILE=$wrapper_path" \
