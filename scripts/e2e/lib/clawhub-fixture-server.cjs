...
const packageName = "@eve/kitchen-sink";
...
async function buildNpmPackArtifact(fixture) {
  const packRoot = await fs.promises.mkdtemp(path.join(os.tmpdir(), "eve-clawhub-fixture-"));
  try {
    ...
    packageDetail(artifact) {
      ...
      const packageDetail = {
        package: {
          ...
          verification: {
            tier: "source-linked",
            sourceRepo: "https://github.com/eve/kitchen-sink",
            hasProvenance: false,
            scanStatus: "passed",
          },
...
async function main() {
  ...
  zip.file("package/eve.plugin.json", manifestJson, { date: new Date(0) });
  ...
  const artifactResolverDetail = {
    package: versionDetail.package ?? {
      name: packageName,
      displayName: packageDetail.package?.displayName ?? "EVE Kitchen Sink",
