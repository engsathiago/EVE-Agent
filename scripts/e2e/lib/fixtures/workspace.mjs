...
const AGENTS_DELETE_OUTPUT_MAX_BYTES = readPositiveIntEnv(
  "EVE_FIXTURE_AGENTS_DELETE_OUTPUT_MAX_BYTES",
  1024 * 1024,
);
...
function writeOpenWebUiWorkspace() {
  const workspace =
    process.env.EVE_WORKSPACE_DIR || path.join(process.env.HOME, ".eve", "workspace");
  write(
...
function writeAgentsDeleteConfig() {
  const stateDir = requireArg(process.env.EVE_STATE_DIR, "EVE_STATE_DIR");
  const sharedWorkspace = requireArg(process.env.SHARED_WORKSPACE, "SHARED_WORKSPACE");
  const gatewayToken = process.env.EVE_GATEWAY_TOKEN?.trim();
  fs.mkdirSync(sharedWorkspace, { recursive: true });
  writeJson(path.join(stateDir, "eve.json"), {
    agents: {
      list: [
