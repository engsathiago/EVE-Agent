...
export OPENAI_API_KEY="sk-eve-release-media-memory"
export EVE_QA_ALLOW_LOCAL_IMAGE_PROVIDER=1

PORT="18789"
MOCK_PORT="44200"
SUCCESS_MARKER="EVE_E2E_OK_MEDIA_MEMORY"
MEMORY_MARKER="release-media-memory-saffron-$(date +%s)"
media_root="$(mktemp -d /tmp/eve-release-media-memory.XXXXXX)"
...
entry="$(eve_e2e_package_entrypoint "$package_root")"
{
  ...
  printf 'HOME=%s\n' "$HOME"
  printf 'EVE_HOME=%s\n' "$EVE_HOME"
  printf 'EVE_STATE_DIR=%s\n' "$EVE_STATE_DIR"
  printf 'EVE_CONFIG_PATH=%s\n' "$EVE_CONFIG_PATH"
} >"$ENV_LOG"
...
mock_pid="$(eve_e2e_start_mock_openai "$MOCK_PORT" "$MOCK_OPENAI_LOG")"
...
cp "$EVE_CONFIG_PATH" "$CONFIG_JSON"
eve plugins list --json >"$PLUGINS_JSON" 2>"$PLUGINS_STDERR_LOG"
...
mkdir -p "$EVE_STATE_DIR/workspace/memory"
...
cat >"$EVE_STATE_DIR/workspace/MEMORY.md" <<EOF
# Long-term memory

- The release media memory marker is $MEMORY_MARKER.
EOF

eve memory index --force >"$INDEX_LOG" 2>&1
eve memory search "$MEMORY_MARKER" --json >"$SEARCH_BEFORE_JSON" 2>"$SEARCH_BEFORE_STDERR_LOG"
...
eve memory search "$MEMORY_MARKER" --json >"$SEARCH_AFTER_JSON" 2>"$SEARCH_AFTER_STDERR_LOG"
