...
const AGENT_OUTPUT_MAX_BYTES = readPositiveIntEnv(
  "EVE_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_MAX_BYTES",
  1024 * 1024,
);
const SESSION_FILE_LIST_LIMIT = 20;
const SESSION_SCAN_MAX_ENTRIES = readPositiveIntEnv(
  "EVE_LIVE_PLUGIN_TOOL_SESSION_SCAN_MAX_ENTRIES",
  50_000,
);
...
function configPath() {
  return process.env.EVE_CONFIG_PATH || path.join(stateDir(), "eve.json");
}

function agentOutputPath() {
  return process.env.EVE_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_PATH || "/tmp/eve-agent.json";
}

function agentErrorPath() {
  return process.env.EVE_LIVE_PLUGIN_TOOL_AGENT_ERROR_PATH || "/tmp/eve-agent.err";
}
...
function pluginInstallPath() {
  ...
  const inspect = fs.existsSync("/tmp/eve-plugin-inspect.json")
    ? readJson("/tmp/eve-plugin-inspect.json")
    : {};
...
function writeFixture() {
  ...
  writeJson(path.join(dir, "eve.plugin.json"), {
    id: pluginId,
    name: "E2E Slug Tool",
...
function assertInstalled() {
  ...
  const list = readJson("/tmp/eve-plugins-list.json");
  ...
  const inspect = readJson("/tmp/eve-plugin-inspect.json");
