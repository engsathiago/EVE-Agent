...
const scratchRoot = process.env.EVE_PLUGINS_TMP_DIR || os.tmpdir();
...
function readClawHubPreflightLimits() {
  return {
    bodyMaxBytes: readPositiveIntEnv(
      "EVE_PLUGINS_E2E_CLAWHUB_PREFLIGHT_BODY_MAX_BYTES",
      1024 * 1024,
    ),
    timeoutMs: readPositiveIntEnv("EVE_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS", 30_000),
  };
}
...
function getInstallRecords() {
  const configPath = eveConfigPath();
  const config = readEVEConfig();
  const allowLegacyCompat = process.env.EVE_PACKAGE_ACCEPTANCE_LEGACY_COMPAT === "1";
...
function eveConfigPath() {
  return path.join(process.env.HOME, ".eve", "eve.json");
}

function readEVEConfig() {
  const configPath = eveConfigPath();
  return fs.existsSync(configPath) ? readRequiredEVEConfig() : {};
}

function readRequiredEVEConfig() {
  const configPath = eveConfigPath();
  try {
    return readJson(configPath);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(`failed to read EVE config ${configPath}: ${message}`, { cause: error });
  }
}
...
function recordFixturePluginTrust() {
  ...
  const configPath = path.join(process.env.HOME, ".eve", "eve.json");
...
function assertGitPlugin() {
  ...
  const gitRoot = path.join(process.env.HOME, ".eve", "git");
...
function assertClawHubExternalInstallContract(installPath) {
  ...
  if (!fs.existsSync(evePeerPath)) {
    throw new Error(`missing ClawHub eve peer symlink: ${evePeerPath}`);
  }
  if (!fs.lstatSync(evePeerPath).isSymbolicLink()) {
    throw new Error(`ClawHub eve peer is not a symlink: ${evePeerPath}`);
  }
  const hostRoot = fs.realpathSync(process.cwd());
  const linkedHostRoot = fs.realpathSync(evePeerPath);
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected ClawHub eve peer ${linkedHostRoot} to target ${hostRoot}`);
  }
...
function assertNpmPlugin() {
  ...
  if (record.spec !== "@eve/demo-plugin-npm@0.0.1") {
    throw new Error(`unexpected npm spec: ${record.spec}`);
  }
  if (record.resolvedName !== "@eve/demo-plugin-npm") {
...
function assertInvalidEVEExtensionsRejected() {
  const pluginId = "demo-plugin-invalid-metadata";
  for (const expected of ["eve.extensions[1]", "non-empty string"]) {
    assertTextFileIncludes(
      scratchFile("plugins-invalid-eve-extensions.log"),
      expected,
      "malformed metadata install output",
    );
  }

  const list = readJson(scratchFile("plugins-invalid-eve-extensions-list.json"));
  ...
  const managedInstallPath = path.join(process.env.HOME, ".eve", "extensions", pluginId);
...
async function assertClawHubPreflight() {
  ...
  const baseUrl = (
    process.env.EVE_CLAWHUB_URL ||
    process.env.CLAWHUB_URL ||
    "https://clawhub.ai"
  ).replace(/\/+$/, "");
  const token =
    process.env.EVE_CLAWHUB_TOKEN ||
...
function assertClawHubInstalled() {
  ...
  const extensionsRoot = path.join(process.env.HOME, ".eve", "extensions");
...
function assertClawHubRemoved() {
  ...
  const configAfterUninstallPath = path.join(process.env.HOME, ".eve", "eve.json");
...
const commands = {
  ...
  "invalid-eve-extensions": assertInvalidEVEExtensionsRejected,
