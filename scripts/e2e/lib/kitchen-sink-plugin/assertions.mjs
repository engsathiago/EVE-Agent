...
function shouldScanLogFile(entry) {
  if (!(/\.(?:log|jsonl)$/u.test(entry) || /eve-kitchen-sink-/u.test(path.basename(entry)))) {
    return false;
  }
...
function scanLogs() {
  ...
  const roots = [scratchRoot, path.join(process.env.HOME, ".eve")];
  const deny = [
  ...
  if (scannedFiles === 0) {
    throw new Error(
      "kitchen-sink log scan found no files under the isolated scratch root or EVE home",
    );
  }
...
function assertClawHubExternalInstallContract(installPath) {
  ...
  if (!fs.lstatSync(evePeerPath).isSymbolicLink()) {
    throw new Error(`kitchen-sink eve peer is not a symlink: ${evePeerPath}`);
  }
  const hostRoot = fs.realpathSync(process.cwd());
  const linkedHostRoot = fs.realpathSync(evePeerPath);
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected kitchen-sink eve peer ${linkedHostRoot} to target ${hostRoot}`);
  }
