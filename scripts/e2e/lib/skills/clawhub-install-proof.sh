...
EVE_TEST_STATE_SCRIPT_B64="${EVE_TEST_STATE_SCRIPT_B64:-}"
eve_skill_install_owns_home=0
cleanup_clawhub_skill_install_home() {
  if [ "$eve_skill_install_owns_home" = "1" ] && [ -n "${HOME:-}" ]; then
    rm -rf "$HOME"
  fi
}
trap cleanup_clawhub_skill_install_home EXIT

if [ -n "$EVE_TEST_STATE_SCRIPT_B64" ]; then
  eve_e2e_eval_test_state_from_b64 "$EVE_TEST_STATE_SCRIPT_B64"
else
  export HOME="$(mktemp -d "${TMPDIR:-/tmp}/eve-skill-install-home.XXXXXX")"
  eve_skill_install_owns_home=1
  export USERPROFILE="$HOME"
  export EVE_HOME="$HOME"
  export EVE_STATE_DIR="$HOME/.eve"
...
if [ -n "${EVE_CURRENT_PACKAGE_TGZ:-}" ]; then
  ...
  eve_e2e_install_package /tmp/eve-skill-install-npm.log
fi

if [ -n "${EVE_CURRENT_PACKAGE_TGZ:-}" ] && command -v eve >/dev/null 2>&1; then
  EVE_CMD=(eve)
elif command -v pnpm >/dev/null 2>&1 && [ -f package.json ]; then
  if [ "${EVE_SKILL_INSTALL_E2E_BUILD_SOURCE:-0}" = "1" ]; then
    pnpm build >/tmp/eve-skill-install-build.log 2>&1
  fi
  EVE_CMD=(pnpm --silent eve)
elif command -v eve >/dev/null 2>&1; then
  EVE_CMD=(eve)
else
  echo "eve command not found; install package first or run from repo with pnpm" >&2
  exit 1
fi

mkdir -p "$(dirname "$EVE_CONFIG_PATH")"
node --input-type=module - "$EVE_CONFIG_PATH" <<'NODE'
...
query="${EVE_SKILL_INSTALL_E2E_QUERY:-homeassistant}"
requested_slug="${EVE_SKILL_INSTALL_E2E_SLUG:-}"
preferred_slug="${EVE_SKILL_INSTALL_E2E_PREFERRED_SLUG:-homeassistant-skill}"
search_json="/tmp/eve-skill-install-search.json"
resolve_json="/tmp/eve-skill-install-resolved.json"
install_log="/tmp/eve-skill-install.log"
info_json="/tmp/eve-skill-install-info.json"

echo "Searching live ClawHub skills for: $query"
"${EVE_CMD[@]}" skills search "$query" --limit 8 --json >"$search_json"
...
if ! "${EVE_CMD[@]}" skills install "$slug" --force >"$install_log" 2>&1; then
  echo "Skill install failed" >&2
  eve_e2e_dump_logs /tmp/eve-skill-install-npm.log "$search_json" "$resolve_json" "$install_log"
  exit 1
fi

workspace_dir="$HOME/.eve/workspace"
skill_dir="$workspace_dir/skills/$slug"
origin_json="$skill_dir/.clawhub/origin.json"
lock_json="$workspace_dir/.clawhub/lock.json"

eve_e2e_assert_file "$skill_dir/SKILL.md"
eve_e2e_assert_file "$origin_json"
eve_e2e_assert_file "$lock_json"

"${EVE_CMD[@]}" skills info "$slug" --json >"$info_json"
