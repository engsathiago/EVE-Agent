// Powershell script supports EVE repository automation.
...
export function windowsAgentTurnConfigPatchScript(modelId: string): string {
  ...
  return `$agentTurnConfigPatchPath = $env:EVE_CONFIG_PATH
if (-not $agentTurnConfigPatchPath) { $agentTurnConfigPatchPath = Join-Path $env:USERPROFILE '.eve\\eve.json' }
$agentTurnVersionText = Invoke-EVE --version 2>$null | Out-String
$agentTurnRuntimePolicySupported = $false
if ($agentTurnVersionText -match 'EVE\\s+(\\d{4})\\.(\\d{1,2})\\.(\\d{1,2})') {
...
$env:EVE_PARALLELS_AGENT_CONFIG_PATCH = @'
${payloadJson}
'@
$env:EVE_PARALLELS_AGENT_CONFIG_PATH = $agentTurnConfigPatchPath
$env:EVE_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED = if ($agentTurnRuntimePolicySupported) { '1' } else { '0' }
$agentTurnConfigPatchScriptPath = Join-Path ([System.IO.Path]::GetTempPath()) 'eve-agent-turn-config-patch.cjs'
@'
const fs = require("node:fs");
const path = require("node:path");
const configPath = process.env.EVE_PARALLELS_AGENT_CONFIG_PATH;
const payload = JSON.parse(process.env.EVE_PARALLELS_AGENT_CONFIG_PATCH || "{}");
const canWriteAgentRuntime = process.env.EVE_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED === "1";
...
fs.rmSync(path.join(stateDir, "npm", "node_modules", "@eve", "codex"), { recursive: true, force: true });
...
if (selectedModelEntry && typeof selectedModelEntry === "object" && !Array.isArray(selectedModelEntry)) {
  if (canWriteAgentRuntime) {
    selectedModelEntry.agentRuntime = { id: "eve" };
  } else {
...
Remove-Item Env:EVE_PARALLELS_AGENT_CONFIG_PATCH -Force -ErrorAction SilentlyContinue
Remove-Item Env:EVE_PARALLELS_AGENT_CONFIG_PATH -Force -ErrorAction SilentlyContinue
Remove-Item Env:EVE_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED -Force -ErrorAction SilentlyContinue
if ($agentTurnConfigPatchExit -ne 0) { throw "agent turn config patch failed" }`;
}

export const windowsEVEResolver = String.raw`$portableNode = if ($env:LOCALAPPDATA) { Join-Path $env:LOCALAPPDATA 'Programs\nodejs' } else { $null }
if ($portableNode -and (Test-Path (Join-Path $portableNode 'node.exe'))) {
  $env:PATH = "$portableNode;$env:PATH"
}
function Resolve-EVECommand {
  if ($script:EVEResolvedCommand) { return $script:EVEResolvedCommand }
  $shimCandidates = @()
  if ($env:APPDATA) {
    $shimCandidates += Join-Path $env:APPDATA 'npm\eve.cmd'
    $shimCandidates += Join-Path $env:APPDATA 'npm\eve.ps1'
  }
  foreach ($name in @('eve.cmd', 'eve.ps1', 'eve')) {
  ...
  if ($npmPrefix) {
    $shimCandidates += Join-Path $npmPrefix 'eve.cmd'
    $shimCandidates += Join-Path $npmPrefix 'eve.ps1'
  }
  foreach ($candidate in $shimCandidates) {
    if ($candidate -and (Test-Path $candidate)) {
      $script:EVEResolvedCommand = @{ Kind = 'shim'; Path = $candidate }
      return $script:EVEResolvedCommand
    }
  }
  $entryCandidates = @()
  if ($env:APPDATA) {
  ...
  if ($npmPrefix) {
    $entryCandidates += Join-Path $npmPrefix 'node_modules\eve\eve.mjs'
  }
  foreach ($candidate in $entryCandidates) {
    if ($candidate -and (Test-Path $candidate)) {
      $script:EVEResolvedCommand = @{ Kind = 'node'; Path = $candidate }
      return $script:EVEResolvedCommand
    }
  }
  throw 'eve command not found in PATH, APPDATA npm, or npm global prefix'
}
function Invoke-EVE {
  param([Parameter(ValueFromRemainingArguments = $true)][string[]] $EVEArgs)
  $command = Resolve-EVECommand
  ...
  try {
    if ($command.Kind -eq 'node') {
      & node.exe $command.Path @EVEArgs
    } else {
      & $command.Path @EVEArgs
    }
  } finally {
