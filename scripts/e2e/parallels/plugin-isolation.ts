// Plugin Isolation script supports EVE repository automation.
...
export function posixCodexPlatformPackageRepairFunction(): string {
  ...
  state_home="\${EVE_PARALLELS_HOME:-\${HOME:-}}"
  codex_manifest=""
  for candidate in "$state_home"/.eve/npm/projects/*/node_modules/@eve/codex/package.json; do
  ...
  project_root="\${codex_manifest%/node_modules/@eve/codex/package.json}"
  cache_dir="$(mktemp -d "\${TMPDIR:-/tmp}/eve-npm-cache.XXXXXX")"
...
export function windowsCodexPlatformPackageRepairFunction(): string {
  ...
  $projectsRoot = Join-Path $env:USERPROFILE '.eve\npm\projects'
  $codexManifest = Get-ChildItem -Path $projectsRoot -Filter package.json -File -Recurse -ErrorAction SilentlyContinue |
    Where-Object { $_.FullName -match 'node_modules[\\/]@eve[\\/]codex[\\/]package\.json$' } |
  ...
  $cacheDir = Join-Path ([System.IO.Path]::GetTempPath()) ('eve-npm-cache-' + [guid]::NewGuid().ToString('N'))
...
export function posixProviderOnlyPluginIsolationScript(options: PluginIsolationOptions): string {
  ...
  const homeEnv = options.homeFallback
    ? `EVE_PARALLELS_HOME=${shellQuote(options.homeFallback)} `
    : "";
...
export function windowsProviderOnlyPluginIsolationScript(options: PluginIsolationOptions): string {
  ...
  return `$env:EVE_PARALLELS_PLUGIN_ISOLATION = @'
${payloadJson}
'@
$isolationScriptPath = Join-Path ([System.IO.Path]::GetTempPath()) 'eve-parallels-plugin-isolation.cjs'
@'
...
Remove-Item Env:EVE_PARALLELS_PLUGIN_ISOLATION -Force -ErrorAction SilentlyContinue`;
}
...
function providerOnlyPluginIsolationNodeScript(options: PluginIsolationOptions): string {
  ...
  return `process.env.EVE_PARALLELS_PLUGIN_ISOLATION = ${JSON.stringify(payloadJson)};
${providerOnlyPluginIsolationNodeSource()}`;
}
...
const payload = JSON.parse(process.env.EVE_PARALLELS_PLUGIN_ISOLATION || "{}");
const home =
  process.env.EVE_PARALLELS_HOME ||
...
fs.rmSync(path.join(stateDir, "npm", "node_modules", "@eve", "codex"), {
  recursive: true,
  force: true,
});
