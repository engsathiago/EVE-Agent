...
// Macos Smoke script supports EVE repository automation.
...
const defaultOptions = (): MacosOptions => ({
  ...
  installUrl: "https://eve.ai/install.sh",
  installVersion: "",
  json: false,
  keepServer: false,
  latestVersion: "",
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://eve.ai/install.sh
...
class MacosSmoke {
  ...
  async run(): Promise<void> {
    ...
    this.runDir = await makeTempDir("eve-parallels-macos.");
    ...
    this.tgzDir = await makeTempDir("eve-parallels-macos-tgz.");
    try {
  ...
  private async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", this.targetInstallsDirectly() ? 420 : 420, () =>
      this.installMain("eve-main-fresh.tgz"),
    );
  ...
  private async runUpgradeLane(): Promise<void> {
    ...
    if (this.options.targetPackageSpec) {
      await this.phase("upgrade.install-main", this.targetInstallsDirectly() ? 420 : 420, () =>
        this.installMain("eve-main-upgrade.tgz"),
      );
...
exec node "$entry" ${argv}`,
  ...
  private resetState(): void {
    this.guestSh(String.raw`/usr/bin/pkill -f 'eve.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'eve-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'eve.mjs gateway' >/dev/null 2>&1 || true
...
${guestNpm} uninstall -g eve >/dev/null 2>&1 || true
rm -rf "$HOME/.eve"
...
curl -fsSL --connect-timeout 10 --max-time 120 --retry 2 --retry-delay 2 ${shellQuote(
      ...
      )} -o /tmp/eve-install.sh
bash /tmp/eve-install.sh --version ${shellQuote(this.installVersion)}
${guestEVE} --version`,
    );
  }
...
${guestEVE} --version`);
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestExec([guestEVE, "--version"]);
...
check_path "$root/eve"
check_path "$root/eve/extensions"
if [ -d "$root/eve/extensions" ]; then
  while IFS= read -r -d '' extension_dir; do
    check_path "$extension_dir"
  done < <(/usr/bin/find "$root/eve/extensions" -mindepth 1 -maxdepth 1 -type d -print0)
fi`);
  }
  ...
  private ensureGuestPnpm(): void {
    this.guestSh(String.raw`set -eu
bootstrap_root=/tmp/eve-smoke-pnpm-bootstrap
...
"$bootstrap_bin/pnpm" --version`);
  ...
  private runDevChannelUpdate(): void {
    ...
    this.guestSh(
      `set -eu
rm -rf ${shellQuote(`${home}/eve`)}
export PATH=${shellQuote(`/tmp/eve-smoke-pnpm-bootstrap/node_modules/.bin:${guestPath}`)}
${guestNode} - <<'JS'
const fs = require("node:fs");
const path = require("node:path");
const configPath = path.join(process.env.HOME || ${JSON.stringify(home)}, ".eve", "eve.json");
const config = JSON.parse(fs.readFileSync(configPath, "utf8"));
config.update = { ...(config.update || {}), channel: "dev" };
fs.writeFileSync(configPath, JSON.stringify(config, null, 2) + "\\n");
JS
/usr/bin/env NODE_OPTIONS=--max-old-space-size=8192 EVE_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 EVE_DISABLE_BUNDLED_PLUGINS=1 ${guestEVEEntryRunner} update --channel dev --yes --json
${guestEVEEntryRunner} --version
${guestEVEEntryRunner} update status --json`,
    );
  }

  private verifyDevChannelUpdate(): void {
    const status = this.guestEVEEntryExec(["update", "status", "--json"]);
  ...
  private startManualGatewayIfNeeded(): void {
    ...
    this.guestSh(
      `set -euo pipefail
trap '' HUP
/usr/bin/pkill -f 'eve.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'eve-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'eve.mjs gateway' >/dev/null 2>&1 || true
/usr/bin/env HOME=${shellQuote(home)} USER=${shellQuote(this.guestUser)} LOGNAME=${shellQuote(this.guestUser)} PATH=${shellQuote(guestPath)} ${shellQuote(
        `${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`,
      )} EVE_HOME=${shellQuote(home)} EVE_STATE_DIR=${shellQuote(`${home}/.eve`)} EVE_CONFIG_PATH=${shellQuote(
        `${home}/.eve/eve.json`,
      )} ${guestEVEEntryRunner} gateway run --bind loopback --port 18789 --force </dev/null >/tmp/eve-parallels-macos-gateway.log 2>&1 &
sleep 1`,
    );
  }
  ...
  private showGatewayStatusCompat(): void {
    const help = this.guestExec([guestEVE, "gateway", "status", "--help"], { check: false });
    const args = help.includes("--require-rpc")
      ? ["gateway", "status", "--deep", "--require-rpc"]
      : ["gateway", "status", "--deep"];
    if (!this.guestEVE(args, false)) {
      throw new Error("gateway status failed");
    }
  }

  private guestEVE(args: string[], check: boolean): boolean {
    const result = run(
      "prlctl",
      [
        "exec",
    ...
    if (check && result.status !== 0) {
      throw new Error(`eve ${args.join(" ")} failed`);
    }
...
while [ $SECONDS -lt $deadline ]; do
  if curl -fsSL --connect-timeout 2 --max-time 5 http://127.0.0.1:18789/ >/tmp/eve-dashboard-smoke.html 2>/dev/null; then
    if grep -F '<title>EVE Control</title>' /tmp/eve-dashboard-smoke.html >/dev/null &&
      grep -F '<eve-app></eve-app>' /tmp/eve-dashboard-smoke.html >/dev/null; then
      asset_paths="$(
        sed -nE 's/.*<(script|link)[^>]*(src|href)=["'"'"']([^"'"'"']+)["'"'"'].*/\3/p' /tmp/eve-dashboard-smoke.html |
          grep -E '(^|/)assets/' |
          grep -Ev '^(https?:)?//' |
          sort -u
      )"
...
exit 1`);
  ...
  private verifyTurn(): void {
    this.guestEVEEntryExec(["models", "set", this.auth.modelId]);
...
JSON
${guestEVEEntryRunner} config set --batch-file "$provider_config_batch" --strict-json
rm -f "$provider_config_batch"`);
    }
    this.guestEVEEntryExec([
      "config",
      "set",
    ...
    this.guestEVEEntryExec(["config", "set", "tools.profile", "minimal"]);
...
for attempt in 1 2; do
  ...
  output_file="$(mktemp)"
  set +e
  /usr/bin/env ${shellQuote(`${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`)} ${guestEVEEntryRunner} agent --local --agent main --session-id "$session_id" --message ${shellQuote(
...
if [ "$agent_ok" != true ]; then
  ...
  private ensureDiscordGatewayReady(): void {
    ...
    this.verifyGateway();
    const status = this.guestEVEEntryExec(["channels", "status", "--probe", "--json"]);
  ...
  private async extractLastVersion(phaseName: string): Promise<string> {
    return await extractLastEVEVersionFromLog(path.join(this.runDir, `${phaseName}.log`));
  }
