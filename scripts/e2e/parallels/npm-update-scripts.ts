// Npm Update Scripts script supports EVE repository automation.
...
const windowsStalePostSwapImportRegex = String.raw`node_modules\\eve\\dist\\[^\\]+-[A-Za-z0-9_-]+\.js`;
const macosGuestPath =
  "/opt/homebrew/bin:/opt/homebrew/opt/node/bin:/usr/local/bin:/usr/local/sbin:/opt/homebrew/sbin:/usr/bin:/bin:/usr/sbin:/sbin";
const macosEVECommand = '"$EVE_BIN"';
...
function posixPrintLogTailFunction(): string {
  ...
  log_file="$1"
  max_bytes="\${EVE_PARALLELS_NPM_UPDATE_LOG_TAIL_BYTES:-262144}"
  case "$max_bytes" in
...
for attempt in 1 2; do
  ...
  rm -f "$HOME/.eve/agents/main/sessions/$session_id.jsonl"
  output_file="$(mktemp)"
  set +e
  EVE_ALLOW_ROOT="\${EVE_ALLOW_ROOT:-}" ${input.auth.apiKeyEnv}=${shellQuote(input.auth.apiKeyValue)} ${command} agent --local --agent main --session-id "$session_id" --message 'Reply with exact ASCII text OK only.' --thinking off --timeout ${resolveParallelsModelTimeoutSeconds(platform)} --json >"$output_file" 2>&1
  rc=$?
  set -e
...
if [ "$agent_ok" != true ]; then
  echo "eve agent finished without OK response" >&2
  exit 1
fi`;
}

function windowsUpdateWithBundledPluginsDisabled(input: NpmUpdateScriptInput): string {
  return `$script:EVEUpdateExit = 0
$updateOutput = Invoke-WithScopedEnv @{ EVE_DISABLE_BUNDLED_PLUGINS = '1'; EVE_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS = '1' } {
  Invoke-EVE update --tag ${psSingleQuote(input.updateTarget)} --yes --json --no-restart 2>&1
  $script:EVEUpdateExit = $LASTEXITCODE
}
$updateExit = $script:EVEUpdateExit
$updateOutput`;
}

function windowsGatewayReadyScript(): string {
  return `function Wait-EVEGateway {
  $deadline = (Get-Date).AddSeconds(180)
  $attempt = 0
  while ((Get-Date) -lt $deadline) {
    Invoke-EVE gateway status --deep --require-rpc --timeout 15000
    if ($LASTEXITCODE -eq 0) { return }
    $attempt += 1
    if ($attempt -eq 4) {
      Invoke-EVE gateway start *>&1 | Out-Host
    }
    Start-Sleep -Seconds 5
  }
  throw "gateway did not become ready after update"
}
Invoke-EVE gateway restart *>&1 | Out-Host
...
${windowsCodexPlatformPackageRepairFunction()}
$sessionPath = Join-Path $env:USERPROFILE '.eve\\agents\\main\\sessions\\parallels-npm-update-windows.jsonl'
...
for ($attempt = 1; $attempt -le 2; $attempt++) {
  ...
  $sessionsDir = Join-Path $env:USERPROFILE '.eve\\agents\\main\\sessions'
  $sessionPath = Join-Path $sessionsDir "$sessionId.jsonl"
  Remove-Item $sessionPath -Force -ErrorAction SilentlyContinue
  $output = Invoke-EVE agent --local --agent main --session-id $sessionId --model ${psSingleQuote(input.auth.modelId)} --message 'Reply with exact ASCII text OK only.' --thinking off --timeout ${resolveParallelsModelTimeoutSeconds("windows")} --json 2>&1
...
if (-not $agentOk) { throw 'eve agent finished without OK response' }`;
}
...
EVE_BIN="$(resolve_required_command eve)"
scrub_future_plugin_entries() {
  python3 - <<'PY'
import json
from pathlib import Path
path = Path.home() / ".eve" / "eve.json"
if not path.exists():
    raise SystemExit(0)
try:
...
stop_eve_gateway_processes() {
  EVE_DISABLE_BUNDLED_PLUGINS=1 "$EVE_BIN" gateway stop || true
  pkill -f 'eve.*gateway' >/dev/null 2>&1 || true
...
start_eve_gateway() {
  stop_eve_gateway_processes
  rm -f /tmp/eve-parallels-macos-gateway.log
  trap '' HUP
  /usr/bin/env EVE_HOME="$HOME" EVE_STATE_DIR="$HOME/.eve" EVE_CONFIG_PATH="$HOME/.eve/eve.json" ${input.auth.apiKeyEnv}=${shellQuote(
    input.auth.apiKeyValue,
  )} "$EVE_BIN" gateway run --bind loopback --port 18789 --force >/tmp/eve-parallels-macos-gateway.log 2>&1 </dev/null &
  sleep 1
}
wait_for_gateway() {
  deadline=$((SECONDS + 240))
  while [ "$SECONDS" -lt "$deadline" ]; do
    if "$EVE_BIN" gateway status --deep --require-rpc --timeout 15000; then
      return
    fi
    sleep 2
  done
  print_log_tail /tmp/eve-parallels-macos-gateway.log >&2
  echo "gateway did not become ready after update" >&2
  exit 1
}
scrub_future_plugin_entries
stop_eve_gateway_processes
EVE_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 EVE_DISABLE_BUNDLED_PLUGINS=1 "$EVE_BIN" update --tag ${shellQuote(input.updateTarget)} --yes --json --no-restart
${posixVersionCheck(macosEVECommand, input.expectedNeedle)}
start_eve_gateway
wait_for_gateway
"$EVE_BIN" models set ${shellQuote(input.auth.modelId)}
${posixModelProviderConfigCommands(macosEVECommand, input.auth.modelId, "macos")}
"$EVE_BIN" config set agents.defaults.skipBootstrap true --strict-json
"$EVE_BIN" config set tools.profile minimal
${posixAgentWorkspaceScript("Parallels npm update smoke test assistant.")}
${posixAssertAgentOkScript(macosEVECommand, input, "macos", "parallels-npm-update-macos")}`;
}
...
function Remove-FuturePluginEntries {
  $configPath = Join-Path $env:USERPROFILE '.eve\\eve.json'
  if (-not (Test-Path $configPath)) { return }
  try { $config = Get-Content $configPath -Raw | ConvertFrom-Json } catch { return }
  $plugins = Get-EVEJsonProperty $config 'plugins'
  if ($null -eq $plugins) { return }
  $entries = Get-EVEJsonProperty $plugins 'entries'
  if ($null -ne $entries) {
    foreach ($pluginId in @('feishu', 'whatsapp', 'openai')) {
      Remove-EVEJsonProperty $entries $pluginId
    }
  }
  $allow = Get-EVEJsonProperty $plugins 'allow'
  if ($allow -is [array]) {
    Set-EVEJsonProperty $plugins 'allow' @($allow | Where-Object { $_ -notin @('feishu', 'whatsapp', 'openai') })
  }
  $config | ConvertTo-Json -Depth 100 | Set-Content -Path $configPath -Encoding UTF8
}
function Get-EVEJsonProperty {
...
function Remove-EVEJsonProperty {
  param([object]$Object, [string]$Name)
...
function Stop-EVEGatewayProcesses {
  Invoke-EVE gateway stop *>&1 | Out-Host
  Get-CimInstance Win32_Process -ErrorAction SilentlyContinue |
    Where-Object { $_.CommandLine -match 'eve.*gateway' } |
...
if ($updateExit -ne 0) {
  ...
  if (-not $stalePostSwapImport) { throw "eve update failed with exit code $updateExit" }
  Write-Host "eve update returned a stale post-swap module import; continuing to post-update health checks"
}
...
export EVE_ALLOW_ROOT=1
${posixPrintLogTailFunction()}
scrub_future_plugin_entries() {
  node - <<'JS'
const fs = require("node:fs");
const path = require("node:path");
const configPath = path.join(process.env.HOME || "/root", ".eve", "eve.json");
...
stop_eve_gateway_processes() {
  EVE_DISABLE_BUNDLED_PLUGINS=1 EVE_ALLOW_ROOT=1 eve gateway stop || true
  pkill -f 'eve.*gateway' >/dev/null 2>&1 || true
}
start_eve_gateway() {
  pkill -f "eve gateway run" >/dev/null 2>&1 || true
  rm -f /tmp/eve-parallels-linux-gateway.log
  setsid sh -lc ${shellQuote(
    `exec env EVE_HOME=/root EVE_STATE_DIR=/root/.eve EVE_CONFIG_PATH=/root/.eve/eve.json EVE_DISABLE_BONJOUR=1 EVE_ALLOW_ROOT=1 ${input.auth.apiKeyEnv}=${shellQuote(
      input.auth.apiKeyValue,
    )} eve gateway run --bind loopback --port 18789 --force >/tmp/eve-parallels-linux-gateway.log 2>&1`,
  )} >/dev/null 2>&1 < /dev/null &
}
wait_for_gateway() {
  deadline=$((SECONDS + 240))
  while [ "$SECONDS" -lt "$deadline" ]; do
    if eve gateway status --deep --require-rpc --timeout 15000; then
      return
    fi
    sleep 2
  done
  print_log_tail /tmp/eve-parallels-linux-gateway.log >&2
  echo "gateway did not become ready after update" >&2
  exit 1
}
scrub_future_plugin_entries
stop_eve_gateway_processes
EVE_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 EVE_DISABLE_BUNDLED_PLUGINS=1 eve update --tag ${shellQuote(input.updateTarget)} --yes --json --no-restart
${posixVersionCheck("eve", input.expectedNeedle)}
start_eve_gateway
wait_for_gateway
eve models set ${shellQuote(input.auth.modelId)}
${posixModelProviderConfigCommands("eve", input.auth.modelId, "linux")}
eve config set agents.defaults.skipBootstrap true --strict-json
eve config set tools.profile minimal
${posixAgentWorkspaceScript("Parallels npm update smoke test assistant.")}
${posixAssertAgentOkScript("eve", input, "linux", "parallels-npm-update-linux")}`;
}
...
while ($true) {
  $version = Invoke-EVE --version
  $version
  if ($LASTEXITCODE -eq 0) { break }
  if ((Get-Date) -ge $versionDeadline) { throw "eve --version failed with exit code $LASTEXITCODE" }
  Start-Sleep -Seconds 2
}`;
  }
...
while ($true) {
  ...
  if ((Get-Date) -ge $versionDeadline) {
    if ($LASTEXITCODE -ne 0) { throw "eve --version failed with exit code $LASTEXITCODE" }
    throw ${mismatch}
  }
  Start-Sleep -Seconds 2
}`;
