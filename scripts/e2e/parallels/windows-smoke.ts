...
// Windows Smoke script supports EVE repository automation.
...
const defaultOptions = (): WindowsOptions => ({
  ...
  installUrl: "https://eve.ai/install.ps1",
  installVersion: "",
  json: false,
  keepServer: false,
  latestVersion: "",
  ...
  snapshotHint: "pre-eve-native-e2e-2026-03-12",
  targetPackageSpec: "",
  upgradeFromPackedMain: false,
  vmName: "Windows 11",
});

const windowsPortableGitPathScript = `$portableGit = Join-Path (Join-Path (Join-Path $env:LOCALAPPDATA 'EVE\\deps') 'portable-git') ''
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: "pre-eve-native-e2e-2026-03-12"
  --mode <fresh|upgrade|both>
  --provider <openai|anthropic|minimax>
  --model <provider/model>    Override the model used for the agent-turn smoke.
  --api-key-env <var>        Host env var name for provider API key.
  --openai-api-key-env <var> Alias for --api-key-env (backward compatible)
  --install-url <url>        Installer URL for latest release. Default: https://eve.ai/install.ps1
  ...
  --upgrade-from-packed-main
                             ...
                             then run eve update --channel dev.
...
class WindowsSmoke extends SmokeRunController<WindowsOptions> {
  ...
  async run(): Promise<void> {
    this.runDir = await makeTempDir("eve-parallels-windows.");
    this.phases = new PhaseRunner(this.runDir);
    this.guest = new WindowsGuest(this.options.vmName, this.phases);
    this.tgzDir = await makeTempDir("eve-parallels-windows-tgz.");
    try {
  ...
  protected async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", 420, () => this.installMain("eve-main-fresh.tgz"));
  ...
  private guestPowerShell(
    ...
    return this.guest.powershell(`${windowsEVEResolver}\n${script}`, options);
  }
  ...
  private logGuestPreflight(cleanEVE: boolean): void {
    const cleanScript = cleanEVE
      ? "npm.cmd uninstall -g eve --no-fund --no-audit --loglevel=error 2>$null; $global:LASTEXITCODE = 0"
      : "";
...
if ($LASTEXITCODE -ne 0) { throw "eve --version failed with exit code $LASTEXITCODE" }`,
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestPowerShell("Invoke-EVE --version");
...
Invoke-EVE onboard --non-interactive --mode local --auth-choice ${psSingleQuote(this.auth.authChoice)} --secret-input-mode ref --gateway-port 18789 --gateway-bind loopback --install-daemon --skip-skills --skip-health --accept-risk --json
if ($LASTEXITCODE -ne 0) { throw "eve onboard failed with exit code $LASTEXITCODE" }
...
${windowsScopedEnvFunction}
$script:EVEUpdateExit = 0
Invoke-WithScopedEnv @{ EVE_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS = '1'; EVE_DISABLE_BUNDLED_PLUGINS = '1' } {
  Invoke-EVE update --channel dev --yes --json
  $script:EVEUpdateExit = $LASTEXITCODE
}
if ($script:EVEUpdateExit -ne 0) { throw "eve update failed with exit code $script:EVEUpdateExit" }
Invoke-EVE --version
Invoke-EVE update status --json`,
...
if ($LASTEXITCODE -ne 0) { throw "gateway ${action} failed with exit code $LASTEXITCODE" }`,
  ...
  private verifyGatewayReachable(): void {
    ...
    while (Date.now() < deadline) {
      const probe = this.guestPowerShell(
        "Invoke-EVE gateway probe --url ws://127.0.0.1:18789 --timeout 30000 --json",
      ...
      if (!recoveryTried && Date.now() - start >= this.gatewayRecoveryAfterMs) {
        ...
        this.guestPowerShell("Invoke-EVE gateway start", {
          check: false,
          timeoutMs: 120_000,
        });
        recoveryTried = true;
  ...
  private showGatewayStatusCompat(): void {
    const help = this.guestPowerShell("Invoke-EVE gateway status --help", {
      check: false,
    });
    const suffix = help.includes("--require-rpc") ? "--deep --require-rpc" : "--deep";
    this.guestPowerShell(`Invoke-EVE gateway status ${suffix}`);
  }
...
if (-not $agentOk) { throw 'eve agent finished without OK response' }`,
  ...
  private async extractLastVersion(phaseName: string): Promise<string> {
    return await extractLastEVEVersion(this.runDir, phaseName, /EVE\s+([0-9][^\s]*)/gi);
  }
