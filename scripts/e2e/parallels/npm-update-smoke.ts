...
// Npm Update Smoke script supports EVE repository automation.
...
const updateTimeoutSeconds = readPositiveIntEnv("EVE_PARALLELS_NPM_UPDATE_TIMEOUT_S", 1200);
const updateCleanupBackstopMs = 60_000;
const freshLaneTimeoutKillGraceMs = readPositiveIntEnv(
  "EVE_PARALLELS_NPM_UPDATE_FRESH_TIMEOUT_KILL_GRACE_MS",
  2_000,
);
...
export function freshLaneTimeoutMs(platform: Platform): number {
  ...
  return readPositiveIntEnv("EVE_PARALLELS_NPM_UPDATE_FRESH_TIMEOUT_S", defaultSeconds) * 1000;
}
...
function eveVersionFamily(version: string): string {
  return /^(\d{4}\.\d{1,2}\.\d{1,2})(?:[-.]|$)/u.exec(version.trim())?.[1] ?? "";
}

function parseEVEPackageSpecVersion(spec: string): string {
  const value = spec.trim();
  if (!value) {
    return "";
  }
  return resolveEVERegistryVersion(value) || "";
}
...
export class NpmUpdateSmoke {
  ...
  async run(): Promise<void> {
    ...
    this.runDir = await this.makeRunTempDir("eve-parallels-npm-update.");
    this.tgzDir = await this.makeRunTempDir("eve-parallels-npm-update-tgz.");
    try {
      await this.runSteps();
    } finally {
  ...
  protected async runSteps(): Promise<void> {
    ...
    say(`Run same-guest eve update to ${this.updateTargetEffective}`);
    ...
    if (this.options.platforms.has("linux")) {
      jobs.push(
        this.spawnFresh("Linux", "linux", ["--vm", this.linuxVm], {
          EVE_PARALLELS_LINUX_DISABLE_BONJOUR: "1",
        }),
      );
    }
  ...
  private async prepareUpdateTarget(): Promise<void> {
    ...
    this.updateExpectedNeedle = this.isExplicitPackageTarget(this.updateTargetEffective)
      ? ""
      : resolveEVERegistryVersion(this.updateTargetEffective) || this.updateTargetEffective;
  ...
  private async guestMacos(
    ...
    const scriptPath = this.writeGuestScript(
      this.macosVm,
      script,
      "eve-parallels-npm-update-macos",
    );
  ...
  private preflightRegistryUpdateTarget(): void {
    ...
    const baseline = resolveEVERegistryVersion(this.packageSpec);
  ...
  private async extractLastVersion(logPath: string): Promise<string> {
    return await extractLastEVEVersionFromLog(logPath);
  }
  ...
  private async configureTargets(): Promise<void> {
    ...
    if (this.options.betaValidation) {
      const version = resolveEVERegistryVersion(this.options.betaValidation);
      if (!version) {
      ...
      const version = resolveEVERegistryVersion(this.options.updateTarget);
      if (version) {
        this.options.updateTarget = version;
      }
    }
  ...
  private assertPublishedTargetMatchesHarnessCheckout(): void {
    if (process.env.EVE_PARALLELS_ALLOW_HARNESS_TARGET_MISMATCH === "1") {
      return;
    }
    const candidateVersion =
      this.targetTarballVersion ||
      (this.freshTargetSpec
        ? parseEVEPackageSpecVersion(this.freshTargetSpec)
        : parseEVEPackageSpecVersion(this.options.updateTarget));
    const targetFamily = eveVersionFamily(candidateVersion);
    if (!targetFamily) {
      return;
    }
    this.harnessTargetFamily = targetFamily;
    const checkoutFamily = eveVersionFamily(this.harnessCheckoutVersion);
    if (checkoutFamily === targetFamily) {
      return;
    }
    die(
      `refusing to run Parallels ${candidateVersion} target with harness checkout ${this.harnessCheckoutVersion || "unknown"}; checkout the matching release branch or set EVE_PARALLELS_ALLOW_HARNESS_TARGET_MISMATCH=1 for an intentional cross-version harness run`,
    );
  }
