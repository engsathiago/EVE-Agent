...
// Linux Smoke script supports EVE repository automation.
...
function parseEVEPackageVersion(value: string): string | null {
  return value.match(/\b(\d{4}\.\d{1,2}\.\d{1,2}(?:-[A-Za-z0-9.]+)?)\b/u)?.[1] ?? null;
}

function compareEVEPackageVersions(left: string, right: string): number {
  const parse = (value: string): [number, number, number] => {
    const match = parseEVEPackageVersion(value)?.match(/^(\d{4})\.(\d+)\.(\d+)/u);
    if (!match) {
      return [0, 0, 0];
    }
...
class LinuxSmoke extends SmokeRunController<LinuxOptions> {
  ...
  async run(): Promise<void> {
    this.runDir = await makeTempDir("eve-parallels-linux.");
    this.phases = new PhaseRunner(this.runDir);
    this.tgzDir = await makeTempDir("eve-parallels-linux-tgz.");
    try {
  ...
  protected async runUpgradeLane(): Promise<void> {
    ...
    await this.phase("upgrade.install-main", 420, () =>
      this.installMainTgz("eve-main-upgrade.tgz"),
    );
...
run_apt_with_lock_retry apt-get -o DPkg::Lock::Timeout=30 install -y curl ca-certificates`);
  ...
  private installLatestRelease(): void {
    this.downloadGuestFile(this.options.installUrl, "/tmp/eve-install.sh");
...
if command -v curl >/dev/null 2>&1; then
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestExec(["eve", "--version"]);
  ...
  private injectBadPluginFixture(): void {
    this.guestBash(String.raw`set -euo pipefail
plugin_dir=/root/.eve/test-bad-plugin
mkdir -p "$plugin_dir"
cat >"$plugin_dir/package.json" <<'JSON'
{"name":"@eve/test-bad-plugin","version":"1.0.0","eve":{"extensions":["./index.cjs"],"setupEntry":"./setup-entry.cjs"}}
JSON
cat >"$plugin_dir/eve.plugin.json" <<'JSON'
...
config_path = Path("/root/.eve/eve.json")
...
PY`);
  ...
  private shouldExpectBadPluginDiagnostic(lane: "fresh" | "upgrade"): boolean {
    const version = parseEVEPackageVersion(this.versionForLane(lane));
    if (!version) {
      return true;
    }
    return compareEVEPackageVersions(version, BAD_PLUGIN_DIAGNOSTIC_MIN_VERSION) >= 0;
  }
  ...
  private startGatewayBackground(): void {
    const bonjourEnv = this.disableBonjour ? " EVE_DISABLE_BONJOUR=1" : "";
    this.guestBash(
      String.raw`pkill -f "eve gateway run" >/dev/null 2>&1 || true
rm -f /tmp/eve-parallels-linux-gateway.log
setsid sh -lc ` +
        shellQuote(
          `exec env EVE_HOME=/root EVE_STATE_DIR=/root/.eve EVE_CONFIG_PATH=/root/.eve/eve.json EVE_ALLOW_ROOT=1${bonjourEnv} ${this.auth.apiKeyEnv}=${shellQuote(
  ...
  private showGatewayStatusCompat(check = true): boolean {
    const help = this.guestExec(["eve", "gateway", "status", "--help"], { check: false });
    const args = help.includes("--require-rpc")
      ? ["eve", "gateway", "status", "--deep", "--require-rpc"]
      : ["eve", "gateway", "status", "--deep"];
    const result = run(
      "prlctl",
      ["exec", this.options.vmName, "/usr/bin/env", "HOME=/root", "EVE_ALLOW_ROOT=1", ...args],
      {
        check: false,
        quiet: true,
        timeoutMs: this.remainingPhaseTimeoutMs(),
...
if isinstance(paths, list):
    load["paths"] = [path for path in paths if path != "/root/.eve/test-bad-plugin"]
allow = plugins.get("allow")
if isinstance(allow, list):
    plugins["allow"] = [plugin_id for plugin_id in allow if plugin_id != "test-bad-plugin"]
config_path.write_text(json.dumps(config, indent=2) + "\n")
PY
rm -rf /root/.eve/test-bad-plugin`);
  }
  ...
  private verifyLocalTurn(): void {
    this.guestExec(["eve", "models", "set", this.auth.modelId]);
...
JSON
eve config set --batch-file "$provider_config_batch" --strict-json
rm -f "$provider_config_batch"`);
    ...
    this.guestExec(["eve", "config", "set", "tools.profile", "minimal"]);
...
for attempt in 1 2; do
  ...
  output_file="$(mktemp)"
  set +e
  /usr/bin/env EVE_ALLOW_ROOT=1 ${shellQuote(`${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`)} eve agent --local --agent main --session-id "$session_id" --message ${shellQuote(
