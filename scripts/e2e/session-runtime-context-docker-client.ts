...
async function verifyRuntimeContextTranscriptShape(root: string) {
  const sessionFile = path.join(root, ".eve", "agents", "main", "sessions", "runtime.jsonl");
  ...
  assert(
    runtimeContextMessage.customType === "eve.runtime-context",
  ...
  assert(
    !userText.includes("EVE_INTERNAL_CONTEXT") && !userText.includes("secret docker context"),
...
async function main() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "eve-session-runtime-context-"));
  process.env.HOME = root;
  process.env.EVE_STATE_DIR = path.join(root, ".eve");
  process.env.EVE_CONFIG_PATH = path.join(process.env.EVE_STATE_DIR, "eve.json");
  try {
    ...
    if (process.env.EVE_SESSION_RUNTIME_CONTEXT_KEEP_ARTIFACTS !== "1") {
