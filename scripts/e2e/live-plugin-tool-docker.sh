...
IMAGE_NAME="$(docker_e2e_resolve_image "eve-live-plugin-tool-e2e" EVE_LIVE_PLUGIN_TOOL_E2E_IMAGE)"
DOCKER_TARGET="${EVE_LIVE_PLUGIN_TOOL_DOCKER_TARGET:-bare}"
HOST_BUILD="${EVE_LIVE_PLUGIN_TOOL_HOST_BUILD:-1}"
PACKAGE_TGZ="${EVE_CURRENT_PACKAGE_TGZ:-}"
PROFILE_FILE="${EVE_LIVE_PLUGIN_TOOL_PROFILE_FILE:-${EVE_TESTBOX_PROFILE_FILE:-$HOME/.eve-testbox-live.profile}}"
...
prepare_package_tgz() {
  ...
  if [ "$HOST_BUILD" = "0" ] && [ -z "${EVE_CURRENT_PACKAGE_TGZ:-}" ]; then
    echo "EVE_LIVE_PLUGIN_TOOL_HOST_BUILD=0 requires EVE_CURRENT_PACKAGE_TGZ" >&2
    exit 1
  fi
...
EVE_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 live-plugin-tool empty)"
...
if ! docker_e2e_run_with_harness \
  ...
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
  -e EVE_LIVE_PLUGIN_TOOL_MODEL="${EVE_LIVE_PLUGIN_TOOL_MODEL:-openai/gpt-5.5}" \
  ...
  -e "EVE_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS=$AGENT_TURN_TIMEOUT_SECONDS" \
...
MODEL_REF="${EVE_LIVE_PLUGIN_TOOL_MODEL:?missing EVE_LIVE_PLUGIN_TOOL_MODEL}"
PLUGIN_ID="e2e-slug-tool"
PLUGIN_NAME="@eve/e2e-slug-tool"
PLUGIN_VERSION="0.0.0-e2e.1"
TOOL_NAME="e2e_slug_probe"
SEED="EVE E2E Plugin Tool $(date +%s)-$RANDOM"
...
dump_debug_logs() {
  local status="$1"
  local agent_output_dump_bytes="${EVE_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_DUMP_BYTES:-16384}"
  echo "Live plugin tool scenario failed with exit code $status" >&2
  if [ -f /tmp/eve-agent.json ]; then
    echo "--- /tmp/eve-agent.json (last ${agent_output_dump_bytes} bytes) ---" >&2
    tail -c "$agent_output_dump_bytes" /tmp/eve-agent.json >&2 || true
    echo >&2
  fi
  eve_e2e_dump_logs \
    /tmp/eve-install.log \
    /tmp/eve-plugin-install.log \
    /tmp/eve-plugin-enable.log \
    /tmp/eve-plugins-list.json \
    /tmp/eve-plugin-inspect.json \
    /tmp/eve-live-plugin-tool-pack.log \
    /tmp/eve-agent.err
}
...
fixture_dir="$(mktemp -d /tmp/eve-live-plugin-tool.XXXXXX)"
...
(cd "$plugin_dir" && npm pack --pack-destination "$fixture_dir" --silent) \
  >/tmp/eve-live-plugin-tool-pack.log 2>&1
plugin_tgzs=()
...
if [ "${#plugin_tgzs[@]}" -ne 1 ]; then
  ...
  eve_e2e_dump_logs /tmp/eve-live-plugin-tool-pack.log
  exit 1
fi
plugin_tgz="${plugin_tgzs[0]}"

echo "Installing fixture plugin from npm-pack: $plugin_tgz"
eve plugins install "npm-pack:$plugin_tgz" --force >/tmp/eve-plugin-install.log 2>&1
node scripts/e2e/lib/live-plugin-tool/assertions.mjs configure
eve plugins enable "$PLUGIN_ID" >/tmp/eve-plugin-enable.log 2>&1
eve plugins list --json >/tmp/eve-plugins-list.json
eve plugins inspect "$PLUGIN_ID" --runtime --json >/tmp/eve-plugin-inspect.json
...
eve agent --local \
  ...
  --thinking off \
  --timeout "${EVE_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS:-300}" \
  --json >/tmp/eve-agent.json 2>/tmp/eve-agent.err
