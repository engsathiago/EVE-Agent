...
// Release Beta Smoke script supports EVE repository automation.
...
const DEFAULT_COMMAND_TIMEOUT_MS = readPositiveInt(
  process.env.EVE_RELEASE_BETA_SMOKE_COMMAND_MS,
  10 * 60_000,
  "EVE_RELEASE_BETA_SMOKE_COMMAND_MS",
);
const TELEGRAM_POLL_INTERVAL_MS = readPositiveInt(
  process.env.EVE_RELEASE_BETA_SMOKE_POLL_INTERVAL_MS,
  30_000,
  "EVE_RELEASE_BETA_SMOKE_POLL_INTERVAL_MS",
);
const TELEGRAM_POLL_TIMEOUT_MS = readPositiveInt(
  process.env.EVE_RELEASE_BETA_SMOKE_POLL_TIMEOUT_MS,
  4 * 60 * 60_000,
  "EVE_RELEASE_BETA_SMOKE_POLL_TIMEOUT_MS",
);
...
function resolveBetaVersion(beta: string): string {
  ...
  const match = versions
    ...
    .at(-1);
  if (!match) {
    throw new Error(`no eve registry version found for ${beta}`);
  }
  return match;
}
  ...
  const result = spawnSync(
    ...
    [
      ...
      [
        "set -euo pipefail",
        'token="$(gh auth token)"',
        'curl -fsS -H "Authorization: Bearer ${token}" -H "Accept: application/vnd.github+json" -H "X-GitHub-Api-Version: 2022-11-28" "${EVE_GITHUB_REST_URL}"',
      ].join("\n"),
    ],
    {
      encoding: "utf8",
      env: { ...process.env, EVE_GITHUB_REST_URL: url },
      killSignal: "SIGKILL",
      maxBuffer: CAPTURE_MAX_BUFFER_BYTES,
...
function appendTelegramProofToRelease(repo: string, version: string, runId: string): void {
  ...
  const notesFile = path.join(
    "/tmp",
    `eve-${version.replace(/[^a-zA-Z0-9.-]/g, "-")}-release-notes-${process.pid}.md`,
  );
