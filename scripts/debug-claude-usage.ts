// Debug Claude Usage script supports EVE repository automation.
...
Options:
  --agent <id>          EVE agent id to inspect (default: main)
...
const loadAuthProfiles = (agentId: string) => {
  const stateRoot = process.env.EVE_STATE_DIR?.trim() || path.join(os.homedir(), ".eve");
...
const resolveFetchTimeoutMs = (raw = process.env.EVE_DEBUG_CLAUDE_USAGE_FETCH_TIMEOUT_MS) => {
  return parseStrictIntegerOption({
    fallback: DEFAULT_FETCH_TIMEOUT_MS,
    label: "EVE_DEBUG_CLAUDE_USAGE_FETCH_TIMEOUT_MS",
    min: 1,
    raw,
  });
};
