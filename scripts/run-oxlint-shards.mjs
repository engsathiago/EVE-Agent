...
export function resolveWindowsExtensionChunkSize(env = process.env) {
  return resolvePositiveEnvIntWithFallback(
    env,
    "EVE_OXLINT_WINDOWS_EXTENSION_CHUNK_SIZE",
...
export function shouldRunOxlintShardsSerial({
  ...
  const explicitMode = env.EVE_OXLINT_SHARDS_SERIAL?.trim();
  if (explicitMode === "1") {
    return true;
  }
  if (platform === "win32") {
  ...
  const localCheckMode = env.EVE_LOCAL_CHECK_MODE?.trim().toLowerCase();
...
export function resolveOxlintShardConcurrency({
  ...
  const explicitConcurrency = resolvePositiveEnvInt(env, "EVE_OXLINT_SHARD_CONCURRENCY");
...
export function resolveShardKillGraceMs(env) {
  return resolveNonNegativeEnvInt(
    env,
    "EVE_OXLINT_SHARD_KILL_GRACE_MS",
