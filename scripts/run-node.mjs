#!/usr/bin/env node
// Development runner that rebuilds EVE, runs runtime postbuild steps, and
...
const listRequiredBundledPluginMetadataOutputs = (pluginEntries, deps) =>
  pluginEntries.flatMap(({ id, hasManifest, hasPackageJson }) => {
    ...
    if (hasManifest) {
      requiredPaths.push(path.join(builtPluginDir, "eve.plugin.json"));
    }
...
const listRequiredEVEExtensionAliasOutputs = (deps) => {
  const distRoot = resolveRuntimePostBuildDistRoot(deps);
...
const listRequiredStaticExtensionAssetOutputs = (deps) => {
  if (deps.env.EVE_RUNTIME_POSTBUILD_STATIC_ASSETS === "0") {
    return [];
  }
  const distRoot = resolveRuntimePostBuildDistRoot(deps);
  const runtimeRoot = resolveRuntimePostBuildRuntimeRoot(deps);
...
export const listRequiredRuntimePostBuildOutputs = (deps) => {
  ...
  return [
    ...
    ...listRequiredEVEExtensionAliasOutputs(deps),
...
export const resolveBuildRequirement = (deps) => {
  if (deps.env.EVE_FORCE_BUILD === "1") {
    return { shouldBuild: true, reason: "force_build" };
  }
  if (
    deps.env.EVE_BUILD_PRIVATE_QA === "1" &&
...
export const resolveRuntimePostBuildRequirement = (deps) => {
  if (deps.env.EVE_FORCE_RUNTIME_POSTBUILD === "1") {
...
const BUILD_REASON_LABELS = {
  force_build: "forced by EVE_FORCE_BUILD",
...
const RUNTIME_POSTBUILD_REASON_LABELS = {
  force_runtime_postbuild: "forced by EVE_FORCE_RUNTIME_POSTBUILD",
...
const RUN_NODE_OUTPUT_LOG_ENV = "EVE_RUN_NODE_OUTPUT_LOG";
const RUN_NODE_CPU_PROF_DIR_ENV = "EVE_RUN_NODE_CPU_PROF_DIR";
const RUN_NODE_CPU_PROF_MAX_FILES_ENV = "EVE_RUN_NODE_CPU_PROF_MAX_FILES";
const RUN_NODE_FILTER_SYNC_IO_STDERR_ENV = "EVE_RUN_NODE_FILTER_SYNC_IO_STDERR";
const RUN_NODE_BUILD_LOCK_TIMEOUT_ENV = "EVE_RUN_NODE_BUILD_LOCK_TIMEOUT_MS";
const RUN_NODE_BUILD_LOCK_POLL_ENV = "EVE_RUN_NODE_BUILD_LOCK_POLL_MS";
const RUN_NODE_BUILD_LOCK_STALE_ENV = "EVE_RUN_NODE_BUILD_LOCK_STALE_MS";
const RUN_NODE_SKIP_DTS_BUILD_ENV = "EVE_RUN_NODE_SKIP_DTS_BUILD";
...
const logRunner = (message, deps) => {
  if (deps.env.EVE_RUNNER_LOG === "0") {
    return;
  }
  const line = `[eve] ${message}\n`;
...
const shouldUseRunNodeProgress = (deps) =>
  ...
  deps.env.EVE_RUNNER_PROGRESS !== "0" &&
  deps.env.CI !== "true" &&
  !deps.outputTee;
...
const createRunNodeProgress = (label, deps) => {
  ...
  const render = () => {
    ...
    frameIndex += 1;
    deps.stderr.write(`\r[eve] ${frame} ${label} (${elapsedSeconds}s)`);
    visible = true;
  };
...
const resolveRunNodeCpuProfileArgs = (deps) => {
  ...
  const profileName = `eve-${commandName}-${pid}-${timestamp}.cpuprofile`;
...
const resolveRunNodeDiagnosticArgs = (deps) => {
  ...
  if (deps.env.EVE_TRACE_SYNC_IO === "1") {
...
const runEVE = async (deps) => {
  const diagnosticArgs = resolveRunNodeDiagnosticArgs(deps);
  const useProcessGroup = shouldUseRunNodeChildProcessGroup(deps);
  const nodeProcess = deps.spawn(deps.execPath, [...diagnosticArgs, "eve.mjs", ...deps.args], {
    cwd: deps.cwd,
...
const canUseStampedGatewayClientDist = (deps) => {
  ...
  if (
    ...
    deps.env.EVE_FORCE_RUNTIME_POSTBUILD === "1"
  ) {
    return false;
  }
...
export async function runNodeMain(params = {}) {
  ...
  if (deps.args[0] === "qa") {
    deps.env.EVE_BUILD_PRIVATE_QA = "1";
    deps.env.EVE_ENABLE_PRIVATE_QA_CLI = "1";
    deps.env.EVE_DISABLE_BUNDLED_PLUGINS ??= "0";
  }
