...
function parseArgs(argv, env = process.env) {
  const opts = parseFlagArgs(
    argv,
    {
      ...
      maxWallMs: readBudgetEnvNumber("EVE_TEST_PERF_MAX_WALL_MS", env),
      baselineWallMs: readBudgetEnvNumber("EVE_TEST_PERF_BASELINE_WALL_MS", env),
      maxRegressionPct: readBudgetEnvNumber("EVE_TEST_PERF_MAX_REGRESSION_PCT", env) ?? 10,
      reportOnly: readBooleanEnv("EVE_TEST_PERF_REPORT_ONLY", env),
    },
    [
