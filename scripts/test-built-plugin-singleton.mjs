...
const { clearPluginCommands, getPluginCommandSpecs, loadEVEPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadEVEPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "eve-build-smoke-"));
...
fs.writeFileSync(
  path.join(distPluginDir, "eve.plugin.json"),
  JSON.stringify(
    {
      id: pluginId,
      configSchema: {
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'eve-agent/plugin-sdk';",
...
const registry = loadEVEPlugins({
  ...
  env: {
    ...process.env,
    EVE_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "dist-runtime", "extensions"),
  },
  config: {
    plugins: {
      enabled: true,
