#!/usr/bin/env bash
# Rootless EVE in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-eve-podman.sh launch        # Start gateway
#   ./scripts/run-eve-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   eve --container eve dashboard --no-open
#   eve --container eve channels login
#
...
run_podman_detached() {
  eve_host_timeout_cmd "$PODMAN_RUN_TIMEOUT" podman run "$@"
}
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      EVE_GATEWAY_TOKEN|EVE_PODMAN_CONTAINER|EVE_PODMAN_IMAGE|EVE_IMAGE|EVE_PODMAN_PULL|EVE_PODMAN_RUN_TIMEOUT|EVE_PODMAN_GATEWAY_HOST_PORT|EVE_GATEWAY_PORT|EVE_PODMAN_BRIDGE_HOST_PORT|EVE_BRIDGE_PORT|EVE_GATEWAY_BIND|EVE_PODMAN_USERNS|EVE_BIND_MOUNT_OPTIONS|EVE_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-eve-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${EVE_CONFIG_DIR:-$EFFECTIVE_HOME/.eve}"
ENV_FILE="${EVE_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${EVE_CONFIG_DIR:-$EFFECTIVE_HOME/.eve}"
ENV_FILE="${EVE_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${EVE_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${EVE_PODMAN_CONTAINER:-eve}"
EVE_IMAGE="${EVE_PODMAN_IMAGE:-${EVE_IMAGE:-eve:local}}"
PODMAN_PULL="${EVE_PODMAN_PULL:-never}"
PODMAN_RUN_TIMEOUT="${EVE_PODMAN_RUN_TIMEOUT:-600s}"
HOST_GATEWAY_PORT="${EVE_PODMAN_GATEWAY_HOST_PORT:-${EVE_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${EVE_PODMAN_BRIDGE_HOST_PORT:-${EVE_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${EVE_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$EVE_IMAGE"
...
resolve_config_gateway_bind() {
  ...
  EVE_CONTAINER="" EVE_CONFIG_DIR="$config_dir" \
    eve config get gateway.bind 2>/dev/null || true
}
...
# EVE_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${EVE_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v eve >/dev/null 2>&1; then
    echo "Warning: eve not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    EVE_CONTAINER="" EVE_CONFIG_DIR="$config_dir" \
      eve config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    EVE_CONTAINER="" EVE_CONFIG_DIR="$config_dir" \
      eve config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  EVE_CONTAINER="" EVE_CONFIG_DIR="$config_dir" \
    eve config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${EVE_GATEWAY_TOKEN:-}" ]]; then
  export EVE_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "EVE_GATEWAY_TOKEN" "$EVE_GATEWAY_TOKEN"
  echo "Generated EVE_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/eve.json"
...
PODMAN_USERNS="${EVE_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported EVE_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (EVE_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${EVE_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${EVE_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$EVE_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.eve/.npm \
    -e EVE_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.eve:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.eve/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$EVE_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME eve dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME eve devices approve --latest  # if pairing required"
