// Bench Cli Startup script supports EVE repository automation.
...
function parseGatewayPortEnv(raw: string | undefined): number {
  ...
  if (bracketHostMatch) {
    return parsePositiveInt(bracketHostMatch[1], 32123, "EVE_GATEWAY_PORT");
  }
  ...
  return parsePositiveInt(portRaw, 32123, "EVE_GATEWAY_PORT");
}
...
function buildConfigFixture(commandCase: CommandCase): Record<string, unknown> | null {
  ...
  const port = parseGatewayPortEnv(process.env.EVE_GATEWAY_PORT);
  return {
    gateway: {
      auth: { mode: "none" },
      bind: "loopback",
...
async function runSample(params: {
  ...
  const runRoot = mkdtempSync(path.join(os.tmpdir(), "eve-cli-bench-home-"));
  const stateDir = path.join(runRoot, ".eve");
...
Options:
  ...
  --entry <path>               Primary entry file (default: eve.mjs)
...
async function main(): Promise<void> {
  ...
  const tmpDir = mkdtempSync(path.join(os.tmpdir(), "eve-cli-bench-"));
