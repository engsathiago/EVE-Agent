#!/usr/bin/env node
// Builds the EVE package artifact used by Docker E2E.
...
function resolvePackedEVEFileName(value) {
  ...
  if (
    ...
    throw new Error(`npm pack reported unsafe EVE tarball filename: ${filename}`);
  }
  return filename;
}
...
const PACKAGE_ARTIFACT_BUILD_STEPS = [
  {
    label: "Building EVE package artifacts",
    command: "node",
  ...
  for (const step of PACKAGE_ARTIFACT_BUILD_STEPS) {
    ...
    await runImpl(step.command, step.args, sourceDir, {
      ...
      timeoutMs: resolveTimeoutMs(
        "EVE_DOCKER_PACKAGE_BUILD_TIMEOUT_MS",
...
async function newestEVETarball(outputDir, packOutput) {
  let fromOutput = "";
  for (const line of packOutput.split(/\r?\n/u)) {
    const filename = resolvePackedEVEFileName(line);
    if (filename) {
      fromOutput = filename;
    }
  }
  ...
  const packed = entries
    .filter((entry) => {
      try {
        return resolvePackedEVEFileName(entry) === entry;
      } catch {
        return false;
      }
    })
    ...
    .at(-1);
  if (!packed) {
    throw new Error(`missing packed EVE tarball in ${outputDir}`);
  }
  return path.join(outputDir, packed);
}

async function cleanPackedEVETarballs(outputDir) {
  let entries;
  try {
...
export async function packEVEPackageForDocker(sourceDir, outputDir, options = {}) {
  ...
  console.error("==> Packing EVE package");
  await prepareChangelog(sourceDir);
  let packOutput;
  try {
    await cleanPackedEVETarballs(outputDir);
    packOutput = await runCaptureImpl(
      ...
      sourceDir,
      {
        ...
        timeoutMs: resolveTimeoutMs(
          "EVE_DOCKER_PACKAGE_PACK_TIMEOUT_MS",
  ...
  return await newestEVETarball(outputDir, packOutput);
}
...
async function main() {
  ...
  console.error("==> Writing EVE package inventory");
  await run(
    "node",
    [
      "--import",
    ...
    {
      timeoutMs: resolveTimeoutMs(
        "EVE_DOCKER_PACKAGE_INVENTORY_TIMEOUT_MS",
        DEFAULT_PACKAGE_INVENTORY_TIMEOUT_MS,
      ),
    },
  );

  let tarball = await packEVEPackageForDocker(sourceDir, outputDir);
  ...
  console.error("==> Checking EVE package tarball");
  const checkStartedAt = Date.now();
  await run(
    "node",
    [path.join(ROOT_DIR, "scripts/check-eve-package-tarball.mjs"), tarball],
    sourceDir,
    {
      timeoutMs: resolveTimeoutMs(
        "EVE_DOCKER_PACKAGE_TARBALL_CHECK_TIMEOUT_MS",
        DEFAULT_PACKAGE_TARBALL_CHECK_TIMEOUT_MS,
      ),
    },
  );
  console.error(
    `==> EVE package tarball check finished in ${Math.round((Date.now() - checkStartedAt) / 1000)}s`,
  );
