...
if [[ -n "${OPENAI_API_KEY:-}" || -n "${OPENAI_BASE_URL:-}" || -n "${GEMINI_API_KEY:-}" || -n "${GOOGLE_API_KEY:-}" ]]; then
  docker_env_dir="$(mktemp -d "${RUNNER_TEMP:-/tmp}/eve-subagent-live-env.XXXXXX")"
  TEMP_DIRS+=("$docker_env_dir")
  docker_env_file="$docker_env_dir/provider.env"
  {
    if [[ -n "${OPENAI_API_KEY:-}" ]]; then
      printf 'EVE_DOCKER_LIVE_OPENAI_API_KEY=%s\n' "${OPENAI_API_KEY}"
    fi
    if [[ -n "${OPENAI_BASE_URL:-}" ]]; then
      printf 'EVE_DOCKER_LIVE_OPENAI_BASE_URL=%s\n' "${OPENAI_BASE_URL}"
    fi
    if [[ -n "${GEMINI_API_KEY:-}" ]]; then
      printf 'EVE_DOCKER_LIVE_GEMINI_API_KEY=%s\n' "${GEMINI_API_KEY}"
    fi
    if [[ -n "${GOOGLE_API_KEY:-}" ]]; then
      printf 'EVE_DOCKER_LIVE_GOOGLE_API_KEY=%s\n' "${GOOGLE_API_KEY}"
    fi
  } >"$docker_env_file"
...
if [ -n "${EVE_DOCKER_LIVE_OPENAI_API_KEY:-}" ]; then
  export OPENAI_API_KEY="$EVE_DOCKER_LIVE_OPENAI_API_KEY"
  unset EVE_DOCKER_LIVE_OPENAI_API_KEY
fi
if [ -n "${EVE_DOCKER_LIVE_OPENAI_BASE_URL:-}" ]; then
  export OPENAI_BASE_URL="$EVE_DOCKER_LIVE_OPENAI_BASE_URL"
  unset EVE_DOCKER_LIVE_OPENAI_BASE_URL
fi
if [ -n "${EVE_DOCKER_LIVE_GEMINI_API_KEY:-}" ]; then
  export GEMINI_API_KEY="$EVE_DOCKER_LIVE_GEMINI_API_KEY"
  unset EVE_DOCKER_LIVE_GEMINI_API_KEY
fi
if [ -n "${EVE_DOCKER_LIVE_GOOGLE_API_KEY:-}" ]; then
  export GOOGLE_API_KEY="$EVE_DOCKER_LIVE_GOOGLE_API_KEY"
  unset EVE_DOCKER_LIVE_GOOGLE_API_KEY
fi
...
cd "$tmp_dir"
EVE_LIVE_TEST=1 \
EVE_LIVE_SUBAGENT_E2E=1 \
EVE_VITEST_MAX_WORKERS="${EVE_VITEST_MAX_WORKERS:-1}" \
...
echo "==> Model: ${EVE_LIVE_SUBAGENT_E2E_MODEL:-openai/gpt-5.5}"
echo "==> Profile file: $PROFILE_STATUS"
DOCKER_RUN_ARGS=()
eve_live_init_docker_run_args DOCKER_RUN_ARGS "${EVE_LIVE_SUBAGENT_DOCKER_RUN_TIMEOUT:-1200s}"
DOCKER_RUN_ARGS+=(--rm -t \
  ...
  -e EVE_LIVE_SUBAGENT_E2E=1 \
  -e EVE_LIVE_SUBAGENT_E2E_MODEL="${EVE_LIVE_SUBAGENT_E2E_MODEL:-}" \
  -e EVE_VITEST_FS_MODULE_CACHE=0 \
  -e EVE_VITEST_MAX_WORKERS="${EVE_VITEST_MAX_WORKERS:-1}")
