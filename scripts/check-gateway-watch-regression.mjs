...
function formatCapturedWatchLog(text, truncated) {
  return truncated
    ? `[eve] log truncated to last ${WATCH_LOG_CAPTURE_MAX_CHARS} chars\n${text}`
    : text;
}
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  const isolatedStateDir = path.join(isolatedHomeDir, ".eve");
  const isolatedConfigPath = path.join(isolatedStateDir, "eve.json");
  const shellSource = [
    'echo "$$" > "$EVE_WATCH_PID_FILE"',
    'mkdir -p "$EVE_STATE_DIR"',
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}},"plugins":{"enabled":false}}' > "$EVE_CONFIG_PATH"`,
    `exec node scripts/watch-node.mjs gateway --force --allow-unconfigured --port ${String(port)} --token watch-regression-token`,
  ].join("\n");
  const env = {
    EVE_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    EVE_HOME: isolatedHomeDir,
    EVE_CONFIG_PATH: isolatedConfigPath,
    EVE_STATE_DIR: isolatedStateDir,
...
export async function runTimedWatch(options, outputDir, deps = {}) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "eve-gateway-watch-"));
