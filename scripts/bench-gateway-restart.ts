// Bench Gateway Restart script supports EVE repository automation.
...
const GATEWAY_CASES: readonly GatewayBenchCase[] = [
  ...
  },
  {
    ...
    env: { EVE_ACPX_RUNTIME_STARTUP_PROBE: "1", EVE_SKIP_CHANNELS: "1" },
    config: BASE_CONFIG,
  },
  {
    id: "skipChannelsNoAcpxProbe",
    name: "gateway restart, skip channels, ACPX startup probe off",
    env: { EVE_ACPX_RUNTIME_STARTUP_PROBE: "0", EVE_SKIP_CHANNELS: "1" },
    config: BASE_CONFIG,
  },
  {
    id: "default",
...
function printUsage(): void {
  console.log(`EVE Gateway restart benchmark
...
async function runGatewaySample(options: {
  ...
  const root = mkdtempSync(path.join(tmpdir(), "eve-gateway-restart-bench-"));
