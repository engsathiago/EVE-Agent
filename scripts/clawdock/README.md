...
Inspired by Simon Willison's [Running EVE in Docker](https://til.simonwillison.net/llms/eve-docker).
...
Canonical docs page: https://docs.eve.ai/install/clawdock
...
On first command, ClawDock auto-detects your EVE directory:

- Checks common paths (`~/eve`, `~/workspace/eve`, etc.)
...
- `EVE_IMAGE_APT_PACKAGES` — extra apt packages to install (e.g. `ffmpeg`); also accepts legacy `EVE_DOCKER_APT_PACKAGES`
- `EVE_IMAGE_PIP_PACKAGES` — extra Python packages to install (e.g. `requests==2.32.5`); pin versions and use only package indexes you trust
- `EVE_INSTALL_BROWSER=1` — pre-install Chromium for browser automation (adds ~300MB, but skips the 60-90s Playwright install on each container start)
...
volumes:
  ...
  - ${EVE_AUTH_PROFILE_SECRET_DIR}:/home/node/.config/eve
```

This means:

- `~/.eve/.env` is available inside the container at `/home/node/.eve/.env` — EVE loads it automatically as the global env fallback
- `~/.eve/eve.json` is available at `/home/node/.eve/eve.json` — the gateway watches it and hot-reloads most changes
- `~/.eve-auth-profile-secrets` is available at `/home/node/.config/eve` — EVE stores the auth-profile encryption key there
- Downloadable external plugin packages and install records live under the mounted EVE home
- Bundled EVE channel plugins, such as Discord when present in the image,
  should normally load from the image-matched bundled copy. Avoid installing
  pinned `@eve/*` channel packages into the mounted home unless you
...
The project `.env` feeds Docker Compose directly (gateway token, image name, ports). The `~/.eve/.env` feeds the EVE process inside the container.
...
```bash
EVE_CONFIG_DIR=/Users/you/.eve
EVE_WORKSPACE_DIR=/Users/you/.eve/workspace
EVE_GATEWAY_PORT=18789
EVE_BRIDGE_PORT=18790
EVE_GATEWAY_BIND=lan
EVE_GATEWAY_TOKEN=<generated-by-docker-setup>
EVE_AUTH_PROFILE_SECRET_DIR=/Users/you/.eve-auth-profile-secrets
EVE_IMAGE=eve:local
```

### Env Precedence

EVE loads env vars in this order (highest wins, never overrides existing):
...
3. **`~/.eve/.env`** — global secrets (API keys, bot tokens)
4. **`eve.json` `env` block** — inline vars, applied only if still missing
5. **Shell env import** — optional login-shell scrape (`EVE_LOAD_SHELL_ENV=1`)

## Common Workflows

### Update EVE

> **Important:** `eve update` does not work inside Docker.
...
```bash
eve channels login --channel whatsapp --verbose
```
...
2. Configure it in the EVE config
...
- Bash or Zsh shell
- EVE project (run `scripts/docker/setup.sh`)
