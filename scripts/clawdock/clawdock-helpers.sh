#!/usr/bin/env bash
# ClawDock - Docker helpers for EVE
# Inspired by Simon Willison's "Running EVE in Docker"
# https://til.simonwillison.net/llms/eve-docker
#
# Installation:
...
_clawdock_browser_url_for_gateway_url() {
  ...
  published=$(_clawdock_compose port eve-gateway 18789 2>/dev/null | head -n 1 | tr -d '\r')
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found EVE at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/eve"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ EVE not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/engsathiago/eve-agent.git ~/eve"
    echo "  cd ~/eve && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^EVE_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d eve-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart eve-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f eve-gateway
}
...
clawdock-show-config() {
  ...
  local config_dir="${HOME}/.eve"
  echo -e "${_CLR_BOLD}Config directory:${_CLR_RESET} ${_CLR_CYAN}${config_dir}${_CLR_RESET}"
  echo ""

  # Show eve.json
  if [[ -f "${config_dir}/eve.json" ]]; then
    echo -e "${_CLR_BOLD}${config_dir}/eve.json${_CLR_RESET}"
    echo -e "${_CLR_DIM}$(cat "${config_dir}/eve.json")${_CLR_RESET}"
  else
    echo -e "${_CLR_YELLOW}No eve.json found${_CLR_RESET}"
  fi
  echo ""
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec eve-gateway \
    bash -c 'echo "alias eve=\"./eve.mjs\"" > /tmp/.bashrc_eve && bash --rcfile /tmp/.bashrc_eve'
}
...
clawdock-cli() {
  _clawdock_compose run --rm eve-cli "$@"
}
...
clawdock-update() {
  ...
  _clawdock_compose build eve-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d eve-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build eve-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "EVE_GATEWAY_TOKEN=$token" eve-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" eve-gateway \
    bash -c './eve.mjs config set gateway.remote.token "$TOKEN" && ./eve.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec eve-gateway \
    bash -c "./eve.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart eve-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm --no-deps eve-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec eve-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'eve config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for EVE${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (eve alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to eve project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.eve)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'eve channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'eve status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.eve.ai${_CLR_RESET}"
  echo ""
}
