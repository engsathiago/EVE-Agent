...
const DEFAULT_OUTPUT_NAME = "eve-current.tgz";
...
const TRUSTED_PACKAGE_SOURCE_TOKEN_ENV = "EVE_TRUSTED_PACKAGE_TOKEN";
...
export function resolveNpmPackageCandidatePackRunner(packageSpec, outputDir, params = {}) {
  validateEVEPackageSpec(packageSpec);
...
async function resolveTrustedRepoRef(ref) {
  ...
  throw new Error(
    `package_ref ${ref} resolved to ${selectedSha}, which is not reachable from an EVE branch or release tag`,
  );
}
...
async function preparePackageSourceWorktree(ref) {
  ...
  const sourceDir = path.join(
    ...
    `eve-package-source-${process.pid}`,
  );
...
async function moveNewestPackedTarball(outputDir, packOutput, outputName) {
  ...
  if (Array.isArray(parsed)) {
    ...
    if (packedFilename) {
      filename = resolvePackedEVETarballFilename(packedFilename);
    }
  }
  if (!filename) {
    for (const line of packOutput.split(/\r?\n/u)) {
      ...
      if (
        ...
        filename = resolvePackedEVETarballFilename(trimmed);
      }
    }
  }
  if (!filename) {
    ...
    filename = entries
      .filter((entry) => {
        try {
          return resolvePackedEVETarballFilename(entry) === entry;
        } catch {
          return false;
        }
      })
  ...
  if (!filename) {
    throw new Error(`npm pack produced no EVE tarball in ${outputDir}`);
  }
...
export const cleanPackedEVETarballsForTest = cleanPackedEVETarballs;
...
async function resolveCandidate(options) {
  ...
  console.error(`Checking EVE package tarball: ${target}`);
  const checkStartedAt = Date.now();
  await run("node", ["scripts/check-eve-package-tarball.mjs", target], {
    timeoutMs: 5 * 60 * 1000,
  });
  console.error(
