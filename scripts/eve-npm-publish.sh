...
usage() {
  echo "usage: bash scripts/eve-npm-publish.sh --publish [package.tgz]"
}
...
mapfile -t publish_plan < <(
  PACKAGE_VERSION="${package_version}" REQUESTED_PUBLISH_TAG="${EVE_NPM_PUBLISH_TAG:-}" \
    node --import tsx --input-type=module <<'EOF'
import { resolveNpmPublishPlan } from "./scripts/eve-npm-release-check.ts";
