...
// Eve Npm Release Check script supports EVE repository automation.
...
const EXPECTED_REPOSITORY_URL = "https://github.com/engsathiago/eve-agent";
...
const skipPackValidationEnv = "EVE_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function resolveNpmReleaseCheckCommandTimeoutMs(
  ...
  return positiveEnvInt(
    "EVE_NPM_RELEASE_CHECK_COMMAND_TIMEOUT_MS",
    env,
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`eve-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `eve-npm-release-check: validated ${channel} release ${pkg.version} (monthly patch version${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
