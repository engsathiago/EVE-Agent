...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.eve) ? packageJson.eve : null;
...
function getCatalogChannelId(entry) {
  return trimString(entry?.eve?.channel?.id) || trimString(entry?.name);
}
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.eve?.channel?.id) || left.name;
    const rightId = trimString(right.eve?.channel?.id) || right.name;
