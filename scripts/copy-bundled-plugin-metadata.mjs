...
function shouldCopyBundledPluginMetadata(id, env, buildablePluginDirs) {
  ...
  return env.EVE_BUILD_PRIVATE_QA === "1";
}
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "eve.plugin.json");
    ...
    if (packageJson.eve && "extensions" in packageJson.eve) {
      packageJson.eve = {
        ...packageJson.eve,
        extensions: rewritePackageExtensions(packageJson.eve.extensions),
        ...(typeof packageJson.eve.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.eve.setupEntry) }
          : {}),
      };
    }
