#!/usr/bin/env bun
// Discord Acp Plain Language Smoke script supports EVE repository automation.
...
type DriverMode = "token" | "webhook" | "eve";
...
const DEFAULT_EVE_CLI_TIMEOUT_MS = 60_000;
...
function resolveStateDir(): string {
  const override = process.env.EVE_STATE_DIR?.trim();
  if (override) {
    if (override === "~") {
  ...
  const home = process.env.EVE_HOME?.trim() || process.env.HOME || "";
...
function parseDriverMode(raw: string): DriverMode {
  ...
  if (normalized === "token" || normalized === "webhook" || normalized === "eve") {
    return normalized;
  }
  throw new Error(
    `Invalid --driver value ${JSON.stringify(raw)}; expected token, webhook, or eve.`,
  );
}
...
function usage(): string {
  return (
    ...
    "  --state-dir <p>              Override EVE state dir for plugin-state polling\n" +
    "  --eve-bin <path>        EVE CLI binary for driver=eve (default: eve)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  EVE_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  EVE_DISCORD_SMOKE_DRIVER\n" +
    "  EVE_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  EVE_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  EVE_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  EVE_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  EVE_DISCORD_SMOKE_AGENT\n" +
    "  EVE_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  EVE_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  EVE_DISCORD_SMOKE_POLL_MS\n" +
    "  EVE_STATE_DIR\n" +
    "  EVE_DISCORD_SMOKE_EVE_BIN"
  );
}
...
function parseArgs(argv = process.argv.slice(2)): Args {
  ...
  const driverToken =
    resolveArg("--token", argv) || process.env.EVE_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix", argv) ||
    process.env.EVE_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX ||
    "Bot";
  const botToken =
    resolveArg("--bot-token", argv) ||
    process.env.EVE_DISCORD_SMOKE_BOT_TOKEN ||
  ...
  const pollMs = parseNumber(
    resolveArg("--poll-ms", argv) || process.env.EVE_DISCORD_SMOKE_POLL_MS,
    1_500,
    "--poll-ms",
  );
...
async function eveCliJson<T>(params: {
  eveBin: string;
  args: string[];
  timeoutMs?: number;
}): Promise<T> {
  const result = await execFileAsync(params.eveBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
    timeout: params.timeoutMs ?? DEFAULT_EVE_CLI_TIMEOUT_MS,
    killSignal: "SIGKILL",
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`eve ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithEve(params: {
  eveBin: string;
  target: string;
  limit: number;
  timeoutMs?: number;
}): Promise<DiscordMessage[]> {
  const response = await eveCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    eveBin: params.eveBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "eve") {
    return await readMessagesWithEve({
      eveBin: params.args.eveBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        timeoutMs: remainingTimeoutMs(deadline),
        body: {
          name: `eve-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      if (!sentMessageId) {
        throw new Error("eve message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from EVE.`,
        diagnostics: {
          bindingCandidates: [
            {
