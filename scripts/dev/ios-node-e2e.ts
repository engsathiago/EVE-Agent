// Ios Node E2E script supports EVE repository automation.
...
function usage(): string {
  return [
    ...
    "Or set env: EVE_GATEWAY_URL / EVE_GATEWAY_TOKEN",
    "",
    "Options:",
...
const urlRaw = getArg("--url") ?? process.env.EVE_GATEWAY_URL;
const token = getArg("--token") ?? process.env.EVE_GATEWAY_TOKEN;
const nodeHint = getArg("--node");
const dangerous = hasFlag("--dangerous") || process.env.EVE_RUN_DANGEROUS === "1";
...
async function main() {
  ...
  const connectRes = await request("connect", {
    ...
    client: {
      id: "cli",
      displayName: "eve ios node e2e",
      version: "dev",
      platform: "dev",
      mode: "cli",
      instanceId: "eve-dev-ios-node-e2e",
    },
    locale: "en-US",
  ...
  const tests: TestCase[] = [
    ...
    { id: "device.status", command: "device.status" },
    {
      ...
      params: { title: "EVE E2E", body: `ios-node-e2e @ ${isoNow()}`, delivery: "system" },
    },
    {
      id: "contacts.search",
