...
# Checks both Claude Code and EVE auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
EVE_AUTH="$HOME/.eve/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    eve models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    eve_status=$(check_eve_auth 2>/dev/null || true)

    claude_expires=0
    eve_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        eve_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        eve_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$EVE_AUTH" 2>/dev/null || echo "0")
    fi
    ...
    jq -n \
        ...
        '{
            ...
            eve: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$eve_status" == EXPIRED* ]] || [[ "$eve_status" == MISSING* ]]; then
        echo "EVE_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$eve_status" == EXPIRING* ]]; then
        echo "EVE_EXPIRING"
        exit 2
    else
        echo "OK"
...
echo ""
echo "EVE Auth (~/.eve/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'eve doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active eve >/dev/null 2>&1; then
    echo -e "EVE service: ${GREEN}running${NC}"
else
    echo -e "EVE service: ${RED}NOT running${NC}"
fi
