...
const shouldRunAutoDoctor = (deps, autoDoctorAttempted) =>
  ...
  !AUTO_DOCTOR_DISABLE_VALUES.has(
    String(deps.env.EVE_GATEWAY_WATCH_AUTO_DOCTOR ?? "").toLowerCase(),
  );
...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[eve] ${message}\n`);
};
...
const printFriendlyWatchStartupError = (err) => {
  ...
  if (packageConfigPath) {
    console.error(`[eve] Invalid package config: ${packageConfigPath}`);
  }
  console.error("[eve] This usually means a file in node_modules is empty or truncated.");
  console.error("[eve] Recommended recovery:");
  console.error("[eve]   rm -rf node_modules");
  console.error("[eve]   pnpm store prune");
  console.error("[eve]   pnpm install");
  console.error("");
  console.error("[eve] Original error:");
  console.error(err);
};
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.EVE_WATCH_MODE = "1";
  childEnv.EVE_WATCH_SESSION = watchSession;
  ...
  childEnv.EVE_NO_RESPAWN = "1";
  if (deps.args.length > 0) {
    childEnv.EVE_WATCH_COMMAND = deps.args.join(" ");
  }
