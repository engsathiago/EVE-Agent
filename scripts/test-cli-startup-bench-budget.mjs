...
if (process.argv.slice(2).includes("--help")) {
  console.log(
    [
      ...
      "EVE_STARTUP_BENCH_ENFORCE_NONCANONICAL_ARCH=1 to force them.",
...
const shouldAutoSkipNonCanonicalBaselineChecks =
  process.arch !== "x64" && process.env.EVE_STARTUP_BENCH_ENFORCE_NONCANONICAL_ARCH !== "1";
if (shouldAutoSkipNonCanonicalBaselineChecks && !opts.skipBaseline) {
  console.warn(
    `[test-cli-startup-bench-budget] skipping x64 startup fixture budgets on ${process.arch}; response contracts and sample output validation still ran. Set EVE_STARTUP_BENCH_ENFORCE_NONCANONICAL_ARCH=1 to force fixture checks.`,
  );
  opts = {
    ...opts,
    skipBaseline: true,
