...
const PUBLISHED_INSTALLER_BASE_URL = "https://eve.ai";
...
export const CROSS_OS_AGENT_TURN_TIMEOUT_SECONDS = parsePositiveIntegerEnv(
  "EVE_CROSS_OS_AGENT_TURN_TIMEOUT_SECONDS",
  600,
);
...
const CROSS_OS_PROCESS_TREE_KILL_AFTER_MS = parsePositiveIntegerEnv(
  "EVE_CROSS_OS_PROCESS_TREE_KILL_AFTER_MS",
  15_000,
);
...
export function resolveProviderConfig(provider, env = process.env) {
  ...
  const providerEnvKey = `EVE_CROSS_OS_${provider.toUpperCase().replace(/[^A-Z0-9]+/gu, "_")}_MODEL`;
  const model = env[providerEnvKey]?.trim() || env.EVE_CROSS_OS_MODEL?.trim() || config.model;
  return { ...config, model };
}
...
function buildReleaseProviderConfigOverride(providerMeta) {
  ...
  return {
    ...
    ...(providerMeta.extensionId === "openai" ? { agentRuntime: { id: "eve" } } : {}),
    models: [],
...
const INSTALL_STAGE_DEBRIS_DIR_PATTERN = /^\.eve-install-stage(?:-[^/]+)?$/iu;
...
export const CROSS_OS_DISCORD_FETCH_TIMEOUT_MS = parsePositiveIntegerEnv(
  "EVE_CROSS_OS_DISCORD_FETCH_TIMEOUT_MS",
  10_000,
);
...
export const CROSS_OS_COMMAND_HEARTBEAT_SECONDS = parsePositiveIntegerEnv(
  "EVE_CROSS_OS_COMMAND_HEARTBEAT_SECONDS",
  60,
);
...
export function resolveCrossOsAgentTurnOptional(env = process.env) {
  return parseBooleanEnv("EVE_CROSS_OS_AGENT_TURN_OPTIONAL", false, env);
}
...
async function main(argv) {
  ...
  const summary = {
    ...
    runnerOs: process.env.EVE_RELEASE_CHECK_OS ?? "",
    runnerLabel: process.env.EVE_RELEASE_CHECK_RUNNER ?? "",
    provider,
    mode,
    suite,
    ref: inputRef || null,
  ...
  try {
    ...
    await runInstalledCli({
      ...
      env: {
        ...
        EVE_UPDATE_DEV_TARGET_REF: verificationRef,
      },
      cwd: lane.homeDir,
...
function createLaneState(name) {
  const rootDir = mkdtempSync(join(tmpdir(), `eve-${name}-`));
  const prefixDir = join(rootDir, "prefix");
  const homeDir = join(rootDir, "home");
  const stateDir = join(homeDir, ".eve");
...
function buildLaneEnv(lane, providerMeta, providerSecretValue) {
  ...
  return {
    ...
    EVE_HOME: lane.homeDir,
    EVE_STATE_DIR: lane.stateDir,
    EVE_CONFIG_PATH: join(lane.stateDir, "eve.json"),
...
export function buildRealUpdateEnv(env) {
  ...
  delete updateEnv.EVE_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
export function isRecoverableWindowsPackagedUpgradeTimeoutError(
  ...
  return (
    ...
    /[/\\]eve\.mjs update --tag http:\/\/127\.0\.0\.1:\d+\/eve[^/\s]*\.tgz --yes --json(?: --no-restart)? --timeout \d+/u.test(
      message,
    )
  );
}
...
if ($null -ne $npmCommand) {
  ...
  if (-not [string]::IsNullOrWhiteSpace($npmPrefix)) {
    ...
    foreach ($candidate in @(
      (Join-Path $npmPrefix 'eve.cmd'),
      (Join-Path $npmPrefix 'eve.ps1')
    )) {
...
if ([string]::IsNullOrWhiteSpace($commandPath)) {
  $cmd = Get-Command eve -ErrorAction Stop
...
Write-Output "__EVE_PATH__=$commandPath"
...
async function verifyFreshShellCommand(params) {
  if (process.platform === "win32") {
    ...
    const cliPath = normalizeWindowsInstalledCliPath(
      parseMarkerLine(result.stdout, "__EVE_PATH__="),
    );
    if (!cliPath) {
      throw new Error("Failed to resolve installed eve path from fresh Windows shell.");
    }
    return {
      cliPath,
  ...
  const script = [
    ...
    "command -v eve >/dev/null 2>&1",
    'printf "__EVE_PATH__=%s\\n" "$(command -v eve)"',
    "eve --version",
  ].join("\n");
  ...
  const cliPath = parseMarkerLine(result.stdout, "__EVE_PATH__=");
  const versionOutput = `${result.stdout}\n${result.stderr}`.trim();
  if (!cliPath) {
    throw new Error("Failed to resolve installed eve path from fresh POSIX shell.");
  }
...
async function ensureDevUpdateGitInstall(params) {
  ...
  // The dev-update lane must prove that `eve update --channel dev` landed on
...
async function configureDiscordSmoke(params) {
  ...
  if (!shouldUseManagedGatewayService()) {
    ...
    const gateway = await startManualGatewayFromInstalledCli({
      ...
      cliPath: params.cliPath,
      env: gatewayEnv,
      logPath: join(params.cwd, `.eve/logs/${params.lane.name}-discord-gateway.log`),
    });
...
export function dashboardHtmlMarkerStatus(html: string): {
  ...
  const title = html.includes("<title>EVE Control</title>");
  const app = html.includes("<eve-app></eve-app>");
...
async function runBundledPluginPostinstall(params) {
  ...
  delete installEnv.EVE_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
export function buildInstalledBrowserOverrideImportProbeScript(
  runtimeModuleSpecifier = "eve-agent/plugin-sdk/plugin-runtime",
) {
  return `
import { existsSync } from "node:fs";
import { startLazyPluginServiceModule } from ${JSON.stringify(runtimeModuleSpecifier)};

const startedPath = process.env.EVE_BROWSER_OVERRIDE_STARTED_PATH;
const stoppedPath = process.env.EVE_BROWSER_OVERRIDE_STOPPED_PATH;

if (!process.env.EVE_BROWSER_CONTROL_MODULE) {
  throw new Error("Missing EVE_BROWSER_CONTROL_MODULE.");
}
...
const handle = await startLazyPluginServiceModule({
  overrideEnvVar: "EVE_BROWSER_CONTROL_MODULE",
...
export async function startBrowserControlService() {
  writeFileSync(process.env.EVE_BROWSER_OVERRIDE_STARTED_PATH, "started\\n", "utf8");
}

export async function stopBrowserControlService() {
  writeFileSync(process.env.EVE_BROWSER_OVERRIDE_STOPPED_PATH, "stopped\\n", "utf8");
}
`.trim();
}
...
async function runInstalledBrowserOverrideImportSmoke(params) {
  ...
  await runCommand(process.execPath, [probePath], {
    ...
    env: {
      ...
      EVE_BROWSER_OVERRIDE_STARTED_PATH: startedPath,
      EVE_BROWSER_OVERRIDE_STOPPED_PATH: stoppedPath,
    },
    logPath: params.logPath,
    timeoutMs: 60_000,
  });
...
function installedEntryPath(prefixDir) {
  return join(installedPackageRoot(prefixDir), "eve.mjs");
}
