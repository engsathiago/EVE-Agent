...
LIVE_MAX_MODELS="${EVE_LIVE_MAX_MODELS:-}"
if [[ -n "$LIVE_MAX_MODELS" && ! "$LIVE_MAX_MODELS" =~ ^\+?[0-9]+$ ]]; then
  echo "invalid EVE_LIVE_MAX_MODELS: $LIVE_MAX_MODELS" >&2
  exit 2
fi
LIVE_MODEL_TIMEOUT_MS="${EVE_LIVE_MODEL_TIMEOUT_MS:-}"
if [[ -n "$LIVE_MODEL_TIMEOUT_MS" ]]; then
  LIVE_MODEL_TIMEOUT_MS="$(eve_live_read_positive_int_env EVE_LIVE_MODEL_TIMEOUT_MS "$LIVE_MODEL_TIMEOUT_MS")"
fi
TEMP_DIRS=()
DOCKER_HOME_MOUNT=()
cleanup_temp_dirs() {
...
if eve_live_truthy "${EVE_DOCKER_PROFILE_ENV_ONLY:-}"; then
  CONFIG_DIR="$(mktemp -d)"
  WORKSPACE_DIR="$(mktemp -d)"
  TEMP_DIRS+=("$CONFIG_DIR" "$WORKSPACE_DIR")
  EVE_DOCKER_AUTH_DIRS=none
else
...
elif [[ -n "${EVE_LIVE_PROVIDERS:-}" || -n "${EVE_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(
    {
      eve_live_collect_auth_dirs_from_csv "${EVE_LIVE_PROVIDERS:-}"
      eve_live_collect_auth_dirs_from_csv "${EVE_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    ...
    {
      eve_live_collect_auth_files_from_csv "${EVE_LIVE_PROVIDERS:-}"
      eve_live_collect_auth_files_from_csv "${EVE_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
else
  while IFS= read -r auth_dir; do
...
echo "==> Profile env only: ${EVE_DOCKER_PROFILE_ENV_ONLY:-0}"
echo "==> Profile file: $PROFILE_STATUS"
echo "==> External auth dirs: ${AUTH_DIRS_CSV:-none}"
echo "==> External auth files: ${AUTH_FILES_CSV:-none}"
DOCKER_RUN_ARGS=()
eve_live_init_docker_run_args DOCKER_RUN_ARGS "${EVE_LIVE_MODELS_DOCKER_RUN_TIMEOUT:-2100s}"
DOCKER_RUN_ARGS+=(--rm -t \
  ...
  -e EVE_LIVE_TEST=1 \
  -e EVE_LIVE_MODELS="${EVE_LIVE_MODELS:-modern}" \
  -e EVE_LIVE_PROVIDERS="${EVE_LIVE_PROVIDERS:-}" \
  -e EVE_LIVE_MAX_MODELS="$LIVE_MAX_MODELS" \
  -e EVE_LIVE_MODEL_TIMEOUT_MS="$LIVE_MODEL_TIMEOUT_MS" \
  -e EVE_LIVE_REQUIRE_PROFILE_KEYS="${EVE_LIVE_REQUIRE_PROFILE_KEYS:-}" \
  -e EVE_LIVE_GATEWAY_MODELS="${EVE_LIVE_GATEWAY_MODELS:-}" \
  -e EVE_LIVE_GATEWAY_PROVIDERS="${EVE_LIVE_GATEWAY_PROVIDERS:-}" \
  -e EVE_LIVE_GATEWAY_MAX_MODELS="${EVE_LIVE_GATEWAY_MAX_MODELS:-}" \
