...
function readInjectedVersion(): string | undefined {
  return typeof __EVE_VERSION__ === "string" ? __EVE_VERSION__ : undefined;
}
...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["EVE_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["EVE_VERSION"]];
  return (
    firstNonEmpty(
      ...preferredCandidates,
      params.env["EVE_SERVICE_VERSION"],
...
export function resolveCompatibilityHostVersion(
  ...
  const explicitCompatibilityVersion = firstNonEmpty(env.EVE_COMPATIBILITY_HOST_VERSION);
...
// Single source of truth for the current EVE version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: readInjectedVersion(),
  bundledVersion: process.env.EVE_BUNDLED_VERSION,
});
