...
import { normalizeCsvOrLooseStringList } from "@eve/normalization-core/string-normalization";
...
/** Parses the JSON5 EVE manifest block embedded inside a string frontmatter field. */
export function resolveEVEManifestBlock(params: {
  frontmatter: Record<string, unknown>;
  key?: string;
...
export type EVEManifestRequires = {
...
/** Extracts normalized runtime requirement lists from an EVE manifest block. */
export function resolveEVEManifestRequires(
  metadataObj: Record<string, unknown>,
): EVEManifestRequires | undefined {
  const requiresRaw =
...
export function resolveEVEManifestInstall<T>(
  metadataObj: Record<string, unknown>,
...
/** Extracts normalized OS allowlist entries from an EVE manifest block. */
export function resolveEVEManifestOs(metadataObj: Record<string, unknown>): string[] {
  return normalizeStringList(metadataObj.os);
}

export type ParsedEVEManifestInstallBase = {
...
export function parseEVEManifestInstallBase(
  input: unknown,
  allowedKinds: readonly string[],
): ParsedEVEManifestInstallBase | undefined {
  ...
  const spec: ParsedEVEManifestInstallBase = {
    raw,
    kind,
  };
...
export function applyEVEManifestInstallCommonFields<
  T extends { id?: string; label?: string; bins?: string[] },
>(spec: T, parsed: Pick<ParsedEVEManifestInstallBase, "id" | "label" | "bins">): T {
  if (parsed.id) {
    spec.id = parsed.id;
  }
  if (parsed.label) {
