...
function readDevicePairPublicUrlFromConfig(cfg: EVEConfig): string | undefined {
...
function shouldResolveLocalGatewayPasswordSecret(
  ...
  const envToken = trimToUndefined(env.EVE_GATEWAY_TOKEN);
...
async function resolveLocalGatewayPasswordSecretIfNeeded(cfg: EVEConfig): Promise<void> {
...
export function registerQrCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/qr", "docs.eve.ai/cli/qr")}\n`,
    )
    .option(
      "--remote",
    ...
    .action(async (opts: QrCliOptions) => {
      try {
        ...
        const lines: string[] = [
          ...
          "Scan this with the EVE mobile app (Onboarding -> Scan QR).",
          "",
        ];
        ...
        lines.push(
          ...
          `  ${theme.command("eve devices list")}`,
          `  ${theme.command("eve devices approve <requestId>")}`,
        );
