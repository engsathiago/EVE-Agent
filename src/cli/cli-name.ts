...
const DEFAULT_CLI_NAME = "eve";

const KNOWN_CLI_NAMES = new Set([DEFAULT_CLI_NAME]);
const CLI_PREFIX_RE = /^(?:((?:pnpm|npm|bunx|npx)\s+))?(eve)\b/;

/** Resolve the displayed CLI binary name from argv, falling back to `eve`. */
...
/** Replace a leading `eve` command prefix with the active CLI name. */
