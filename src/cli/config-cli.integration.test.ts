...
async function withExecDryRunConfigHarness(
  ...
  const envSnapshot = captureEnv(["EVE_CONFIG_PATH", "EVE_TEST_FAST"]);
  try {
    fs.writeFileSync(
      configPath,
      `${JSON.stringify(
...
describe("config cli integration", () => {
  beforeAll(async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "eve-config-cli-warmup-"));
    const configPath = path.join(tempDir, "eve.json");
    const envSnapshot = captureEnv(["EVE_CONFIG_PATH", "EVE_TEST_FAST"]);
    try {
  ...
  it("accepts plugin hook conversation-access policy via config set", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "eve-config-cli-plugin-hooks-"));
    const configPath = path.join(tempDir, "eve.json");
    const envSnapshot = captureEnv(["EVE_CONFIG_PATH", "EVE_TEST_FAST"]);
    try {
      fs.writeFileSync(
      ...
      await runConfigSet({
        path: "plugins.entries.eve-mem0.hooks.allowConversationAccess",
        value: "true",
        cliOptions: {},
      ...
      expect(afterWrite.plugins?.entries?.["eve-mem0"]?.hooks).toEqual({
  ...
  it("skips exec provider execution during dry-run by default", async () => {
    await withExecDryRunConfigHarness("eve-config-cli-int-exec-skip-", async (params) => {
  ...
  it("executes exec providers during dry-run when --allow-exec is set", async () => {
    await withExecDryRunConfigHarness("eve-config-cli-int-exec-allow-", async (params) => {
      const before = fs.readFileSync(params.configPath, "utf8");
      await runConfigSet({
        cliOptions: {
          batchFile: params.batchPath,
