...
import type { StaleEVEUpdateLaunchdJob } from "../../daemon/launchd.js";
...
const findStaleEVEUpdateLaunchdJobs = vi.fn<
  (env?: NodeJS.ProcessEnv) => Promise<StaleEVEUpdateLaunchdJob[]>
>(async () => []);
...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    EVE_STATE_DIR: "/tmp/eve-daemon",
    EVE_CONFIG_PATH: "/tmp/eve-daemon/eve.json",
  },
}));
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({
    ...
    const isDaemon = configPath.includes("/eve-daemon/");
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    setTestEnvValue("EVE_STATE_DIR", "/tmp/eve-cli");
    setTestEnvValue("EVE_CONFIG_PATH", "/tmp/eve-cli/eve.json");
    ...
    findStaleEVEUpdateLaunchdJobs.mockReset();
    findStaleEVEUpdateLaunchdJobs.mockResolvedValue([]);
  ...
  it("forwards requireRpc and configPath to the daemon probe", async () => {
    ...
    expect(probeInput.configPath).toBe("/tmp/eve-daemon/eve.json");
  });
  ...
  it("reuses the shared CLI config snapshot when the daemon uses the same config path", async () => {
    ...
    expect(readConfigFileSnapshotCalls).toHaveBeenCalledWith("/tmp/eve-cli/eve.json");
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.EVE_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.EVE_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("surfaces recent service restart handoffs only during deep status", async () => {
    ...
    expect(handoffInput.EVE_STATE_DIR).toBe("/tmp/eve-daemon");
    expect(handoffInput.EVE_CONFIG_PATH).toBe("/tmp/eve-daemon/eve.json");
    ...
    async () => {
      serviceReadCommand.mockResolvedValueOnce({
        ...
        environment: {
          EVE_STATE_DIR: "/tmp/eve-daemon",
          EVE_CONFIG_PATH: "/tmp/eve-daemon/eve.json",
          EVE_LAUNCHD_LABEL: "ai.eve.manual-update.gateway",
        },
      });
      findStaleEVEUpdateLaunchdJobs.mockResolvedValueOnce([
        {
      ...
      const staleScanEnv = findStaleEVEUpdateLaunchdJobs.mock.calls[0]?.[0];
      expect(staleScanEnv?.EVE_STATE_DIR).toBe("/tmp/eve-daemon");
      expect(staleScanEnv?.EVE_CONFIG_PATH).toBe("/tmp/eve-daemon/eve.json");
      expect(staleScanEnv?.EVE_LAUNCHD_LABEL).toBe("ai.eve.manual-update.gateway");
  ...
  it("does not read restart handoffs during normal status", async () => {
    ...
    expect(findStaleEVEUpdateLaunchdJobs).not.toHaveBeenCalled();
  ...
  it("uses the fast config path for plain same-file status reads", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "eve-status-config-"));
    const configPath = path.join(tmp, "eve.json");
    await fs.writeFile(
      configPath,
      JSON.stringify({
        gateway: {
    ...
    setTestEnvValue("EVE_STATE_DIR", tmp);
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    setTestEnvValue("EVE_GATEWAY_TOKEN", "env-token");
    setTestEnvValue("EVE_GATEWAY_PASSWORD", "env-password"); // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection when probe fails", async () => {
    ...
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "eve-gateway" }],
        hints: [],
      },
      healthy: false,
  ...
  it("reads install records from the merged daemon service environment, not the CLI process env", async () => {
    ...
    // The mock daemon service command sets EVE_STATE_DIR=/tmp/eve-daemon,
    // distinct from the CLI process EVE_STATE_DIR=/tmp/eve-cli. Drift
