...
describe("shouldSkipRespawnForArgv", () => {
  it.each([
    ...
    { argv: ["node", "eve", "-V"] },
    { argv: ["node", "eve", "tui"] },
    { argv: ["node", "eve", "terminal"] },
    { argv: ["node", "eve", "chat"] },
    { argv: ["node", "eve", "gateway"] },
    { argv: ["node", "eve", "gateway", "--port", "14720", "--bind", "loopback"] },
    { argv: ["node", "eve", "gateway", "run", "--port=14720", "--bind", "loopback"] },
    {
      argv: ["node", "eve", "--profile", "server", "gateway", "run", "--allow-unconfigured"],
    },
  ...
  it.each([
    ...
    { argv: ["node", "eve", "gateway", "status"] },
    { argv: ["node", "eve", "gateway", "call", "health"] },
...
describe("shouldSkipStartupEnvironmentRespawnForArgv", () => {
  it.each([
    { argv: ["node", "eve", "--help"] },
    { argv: ["node", "eve", "gateway"] },
    { argv: ["node", "eve", "gateway", "run", "--port=14720"] },
...
describe("dns cli", () => {
  it("prints setup info (no apply)", async () => {
    ...
    try {
      ...
      await program.parseAsync(["dns", "setup", "--domain", "eve.internal"], { from: "user" });
      const output = log.mock.calls.map((call) => call.join(" ")).join("\\n");
      expect(output).toContain("DNS setup");
      expect(output).toContain("eve.internal");
    } finally {
      log.mockRestore();
    }
  });
