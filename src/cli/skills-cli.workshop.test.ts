...
vi.mock("../terminal/links.js", () => ({
  formatDocsLink: () => "docs.eve.ai/cli/skills",
}));
...
describe("skills workshop cli", () => {
  ...
  beforeEach(async () => {
    testState = await createEVETestState({
      layout: "state-only",
      prefix: "eve-skills-cli-workshop-state-",
    });
    mocks.workspaceDir = await tempDirs.make("eve-skills-cli-workshop-");
  ...
  it("scopes list and inspect to the selected workspace", async () => {
    ...
    mocks.workspaceDir = await tempDirs.make("eve-skills-cli-workshop-second-");
    await runCommand(["skills", "workshop", "list"]);
