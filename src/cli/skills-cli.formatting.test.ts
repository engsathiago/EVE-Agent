...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    envSnapshot = captureEnv(["EVE_BUNDLED_SKILLS_DIR"]);
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "eve-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "eve-bundled-skills-test-"));
    process.env.EVE_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
