...
const resolveShellEnvExpectedKeys = vi.fn((_env?: NodeJS.ProcessEnv) => ["EVE_GATEWAY_TOKEN"]);
...
const writeDiagnosticStabilityBundleForFailureSync = vi.fn((_reason: string, _error: unknown) => ({
  ...
  message: "wrote stability bundle: /tmp/eve-stability.json",
  path: "/tmp/eve-stability.json",
}));
const controlUiState = vi.hoisted(() => ({
  root: "/tmp/eve-control-ui" as string | null,
}));
...
vi.mock("../../config/config.js", () => ({
  getConfigPath: () => "/tmp/eve-test-missing-config.json",
...
vi.mock("../../config/paths.js", () => ({
  CONFIG_PATH: "/tmp/eve-test-missing-config.json",
...
describe("gateway run option collisions", () => {
  ...
  beforeEach(() => {
    ...
    controlUiState.root = "/tmp/eve-control-ui";
  ...
  it("loads configured shell env fallback before final proxy refresh and gateway startup", async () => {
    await withEnvAsync({ EVE_GATEWAY_TOKEN: undefined }, async () => {
      const finalConfig = {
        env: {
          shellEnv: { enabled: true, timeoutMs: 1234 },
          vars: { EVE_GATEWAY_TOKEN: "config-token" },
        },
        gateway: {
          auth: { mode: "token", token: "${EVE_GATEWAY_TOKEN}" },
          mode: "local",
        },
        proxy: { enabled: true, proxyUrl: "http://127.0.0.1:29876" },
      };
      ...
      readConfigFileSnapshotWithPluginMetadata
        ...
        .mockImplementationOnce(async (options) => {
          expect(options?.lowerPrecedenceEnv).toEqual({
            EVE_GATEWAY_TOKEN: "shell-token",
          });
          expect(process.env.EVE_GATEWAY_TOKEN).toBe("shell-token");
          return {
            snapshot: {
              ...configState.snapshot,
              config: {
      ...
      loadShellEnvFallback.mockImplementationOnce((opts?: unknown) => {
        callOrder.push("shell-env");
        (opts as { env: NodeJS.ProcessEnv }).env.EVE_GATEWAY_TOKEN = "shell-token";
      });
      const uninstall = installGatewayRunRuntimeHooks({ refreshManagedProxy });
      try {
        await runGatewayCli(["gateway"]);
      ...
      expect(loadShellEnvFallback).toHaveBeenCalledWith({
        enabled: true,
        env: process.env,
        expectedKeys: ["EVE_GATEWAY_TOKEN"],
        logger: expect.any(Object),
        timeoutMs: 1234,
      });
      expect(readConfigFileSnapshotWithPluginMetadata).toHaveBeenCalledWith(
        expect.objectContaining({
          lowerPrecedenceEnv: { EVE_GATEWAY_TOKEN: "shell-token" },
        }),
      );
      expect(readConfigFileSnapshotWithPluginMetadata).toHaveBeenCalledTimes(2);
      expect(process.env.EVE_GATEWAY_TOKEN).toBe("config-token");
      expect(clearShellEnvAppliedKeys).toHaveBeenCalledWith(["EVE_GATEWAY_TOKEN"]);
  ...
  it("uses config env shell fallback controls without mutating the live env during planning", async () => {
    await withEnvAsync(
      {
        EVE_GATEWAY_TOKEN: undefined,
        EVE_LOAD_SHELL_ENV: undefined,
        EVE_SHELL_ENV_TIMEOUT_MS: undefined,
      },
      async () => {
        const finalConfig = {
          env: {
        ...
        shouldEnableShellEnvFallback.mockImplementationOnce(
          (env?: NodeJS.ProcessEnv) => env?.EVE_LOAD_SHELL_ENV === "1",
        );
        resolveShellEnvFallbackTimeoutMs.mockImplementationOnce((env?: NodeJS.ProcessEnv) =>
          Number(env?.EVE_SHELL_ENV_TIMEOUT_MS),
        );
        ...
        expect(process.env.EVE_LOAD_SHELL_ENV).toBe("1");
        expect(process.env.EVE_SHELL_ENV_TIMEOUT_MS).toBe("4321");
      },
    );
  });
  ...
  it("honors config env shell fallback deferral", async () => {
    await withEnvAsync(
      {
        EVE_DEFER_SHELL_ENV_FALLBACK: undefined,
        EVE_LOAD_SHELL_ENV: undefined,
      },
      async () => {
        const finalConfig = {
          env: {
            vars: {
              EVE_DEFER_SHELL_ENV_FALLBACK: "1",
              EVE_LOAD_SHELL_ENV: "1",
            },
          },
          gateway: { auth: { mode: "none" }, mode: "local" },
        ...
        shouldDeferShellEnvFallback.mockImplementationOnce(
          (env?: NodeJS.ProcessEnv) => env?.EVE_DEFER_SHELL_ENV_FALLBACK === "1",
        );
  ...
  it("ignores shell fallback controls from invalid config", async () => {
    ...
    await withEnvAsync(
      ...
      async () => {
        const invalidConfig = {
          env: { vars: { EVE_DEFER_SHELL_ENV_FALLBACK: "1" } },
          gateway: { mode: "local" },
        };
        configState.snapshot = {
  ...
  it("rejects an invalid final config after a prepared config selected runtime paths", async () => {
    const selectedStateDir = "/tmp/eve-prepared-selected-state";
    await withEnvAsync({ EVE_STATE_DIR: undefined }, async () => {
  ...
  it("replaces config-derived env when the final startup snapshot changes in place", async () => {
    await withEnvAsync(
      {
        EVE_GATEWAY_TOKEN: undefined,
        EVE_PROXY_URL: undefined,
        EVE_RAW_STREAM: undefined,
      },
      async () => {
        const oldConfig = {
          env: {
            vars: {
              ...
              EVE_PROXY_URL: "http://127.0.0.1:19876",
              EVE_RAW_STREAM: "1",
            },
          },
          gateway: { mode: "local" },
        ...
        const newConfig = {
          env: { vars: { EVE_GATEWAY_TOKEN: "new-token" } },
          gateway: { mode: "local" },
        };
        configState.snapshot = {
          config: oldConfig,
        ...
        expect(process.env.EVE_GATEWAY_TOKEN).toBe("old-token");
        expect(process.env.EVE_PROXY_URL).toBe("http://127.0.0.1:19876");
        ...
        readConfigFileSnapshotWithPluginMetadata.mockImplementationOnce(async () => {
          expect(process.env.EVE_GATEWAY_TOKEN).toBeUndefined();
          expect(process.env.EVE_PROXY_URL).toBeUndefined();
          return { snapshot: configState.snapshot };
        });
        await runGatewayCli(["gateway", "--raw-stream"]);

        expect(process.env.EVE_GATEWAY_TOKEN).toBe("new-token");
        expect(process.env.EVE_PROXY_URL).toBeUndefined();
        expect(process.env.EVE_RAW_STREAM).toBe("1");
      },
    );
  });
  ...
  it("marks descendants when the final config supplies the service marker", async () => {
    await withEnvAsync(
      ...
      async () => {
        const finalConfig = {
          env: { vars: { EVE_SERVICE_MARKER: "eve" } },
          gateway: { mode: "local" },
        };
        configState.snapshot = {
          config: finalConfig,
        ...
        expect(process.env.EVE_SERVICE_MARKER).toBe("eve");
        expect(process.env[GATEWAY_SERVICE_RUNTIME_PID_ENV]).toBe(String(process.pid));
      },
    );
  });
  ...
  it("blocks service-mode startup from an older binary with newer config", async () => {
    ...
    process.env.EVE_SERVICE_MARKER = "gateway";
    try {
    ...
    await withEnvAsync(
      ...
      async () => {
        ...
        ensureDevGatewayConfig.mockImplementationOnce(async () => {
          ...
          expect(process.env.EVE_HOME).toBeUndefined();
          expect(process.env.EVE_PROFILE).toBe("dev");
          expect(process.env.EVE_STATE_DIR).toBeUndefined();
          expect(process.env.EVE_GATEWAY_TOKEN).toBeUndefined();
          expect(process.env.EVE_WORKSPACE_DIR).toBe("/tmp/eve-reset-workspace");
          configState.snapshot = {
            exists: true,
            valid: true,
            config: { gateway: { mode: "local" } },
        ...
        loadGlobalRuntimeDotEnvFiles.mockImplementation(() => {
          process.env.EVE_GATEWAY_TOKEN ??= "trusted-token";
          process.env.EVE_PROFILE ??= "dev";
          process.env.EVE_WORKSPACE_DIR ??= "/tmp/eve-reset-workspace";
        });
        ...
        expect(process.env.EVE_GATEWAY_TOKEN).toBe("trusted-token");
  ...
  it("refuses dev reset if trusted dotenv retargets after pre-bootstrap", async () => {
    await withEnvAsync({ EVE_STATE_DIR: "/tmp/eve-reset-original" }, async () => {
      configState.snapshot = {
        config: { gateway: { mode: "local" } },
        exists: true,
        path: "/tmp/eve-reset-original/eve.json",
        sourceConfig: { gateway: { mode: "local" } },
        valid: true,
      };
      await prepareGatewayReset();
      loadGlobalRuntimeDotEnvFiles.mockImplementation(() => {
        process.env.EVE_STATE_DIR = "/tmp/eve-reset-retargeted";
        return {
          gatewayEnvAppliedKeys: [],
          stateEnvAppliedKeys: ["EVE_STATE_DIR"],
        };
      });
      ...
      expect(ensureDevGatewayConfig).not.toHaveBeenCalled();
      expect(process.env.EVE_STATE_DIR).toBe("/tmp/eve-reset-original");
      expect(runtimeErrors.join("\n")).toContain(
  ...
  it.each([
    ...
    await withEnvAsync({ [selector]: "/tmp/eve-reset-value" }, async () => {
      loadGlobalRuntimeDotEnvFiles.mockImplementation(() => {
        process.env[selector] = "/tmp/eve-reset-retargeted";
      });
      ...
      expect(process.env[selector]).toBe("/tmp/eve-reset-value");
      expect(runtimeErrors.join("\n")).toContain(
  ...
  it("blocks a final startup snapshot that changes guarded config selection", async () => {
    await withEnvAsync({ EVE_STATE_DIR: undefined }, async () => {
      configState.snapshot = {
        ...
        config: { gateway: { mode: "local" } },
        sourceConfig: {
          env: { vars: { EVE_STATE_DIR: "/tmp/eve-late-selection" } },
          gateway: { mode: "local" },
        },
      };
  ...
  it("blocks a final startup snapshot that changes an already-selected config selector", async () => {
    await withEnvAsync({ EVE_STATE_DIR: undefined }, async () => {
      const guardedConfig = {
        env: { vars: { EVE_STATE_DIR: "/tmp/eve-guarded-state" } },
        gateway: { mode: "local" },
      };
      configState.snapshot = {
      ...
      await selectGatewayRunEnvironment({ opts: {}, runtime: defaultRuntime });
      await prepareGatewayRunBootstrap({ opts: {}, runtime: defaultRuntime });
      expect(process.env.EVE_STATE_DIR).toBe("/tmp/eve-guarded-state");

      const finalConfig = {
        env: { vars: { EVE_STATE_DIR: "/tmp/eve-final-state" } },
        gateway: { mode: "local" },
      };
      configState.snapshot = {
        config: finalConfig,
  ...
  it.each([
    ...
    delete process.env.EVE_CLI_BACKEND_LOG_OUTPUT;
    ...
    expect(process.env.EVE_CLI_BACKEND_LOG_OUTPUT).toBe("1");
  });
  ...
  it("blocks startup when the observed snapshot loses gateway.mode", async () => {
    ...
    expect(runtimeErrors).toContain(
      "Gateway start blocked: existing config is missing gateway.mode. Treat this as suspicious or clobbered config. Re-run `eve onboard --mode local` or `eve setup`, set gateway.mode=local manually, or pass --allow-unconfigured.",
    );
    expect(runtimeErrors).toContain(
  ...
  it("blocks invalid startup config without automatic recovery", async () => {
    ...
    configState.snapshot = {
      exists: true,
      valid: false,
      path: "/tmp/eve-test-missing-config.json",
      config: {},
      parsed: null,
  ...
  it("allows password mode preflight when password is configured via SecretRef", async () => {
    configState.cfg = {
      gateway: {
        auth: {
          mode: "password",
          password: { source: "env", provider: "default", id: "EVE_GATEWAY_PASSWORD" },
        },
      },
      secrets: {
        defaults: {
