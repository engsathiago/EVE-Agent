// `eve plugins inspect`: renders plugin registry shape, capabilities, policy, diagnostics, and install records.
...
/** Options accepted by `eve plugins inspect`. */
...
export async function runPluginsInspectCommand(
  ...
  if (!inspect) {
    defaultRuntime.error(
      formatMissingPluginMessage({ id, listCommand: "eve plugins list --json" }),
    );
    return defaultRuntime.exit(1);
  }
  ...
  lines.push(`${theme.muted("Format:")} ${inspect.plugin.format ?? "eve"}`);
