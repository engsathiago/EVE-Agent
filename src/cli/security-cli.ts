...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["eve security audit", "Run a local security audit."],
          [
            "eve security audit --deep",
            "Include best-effort live Gateway probes and plugin-owned security audit collectors.",
          ],
          ["eve security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "eve security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          [
            "eve security audit --auth password --password <password>",
            "Audit a runtime-only password-mode Gateway secret.",
          ],
          ["eve security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["eve security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.eve.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("EVE security audit"));
      ...
      lines.push(muted(`Run deeper: ${formatCliCommand("eve security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("eve security audit --fix")}`));
        if (!fixResult) {
