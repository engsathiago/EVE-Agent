...
describe("proxy cli runtime", () => {
  const envKeys = [
    "EVE_STATE_DIR",
    "EVE_DEBUG_PROXY_CERT_DIR",
    "EVE_DEBUG_PROXY_SESSION_ID",
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(path.join(os.tmpdir(), "eve-proxy-cli-runtime-"));
    process.env.EVE_STATE_DIR = tempDir;
    process.env.EVE_DEBUG_PROXY_CERT_DIR = path.join(tempDir, "certs");
    delete process.env.EVE_DEBUG_PROXY_ENABLED;
    delete process.env.EVE_DEBUG_PROXY_SESSION_ID;
  ...
  afterEach(async () => {
    ...
    const { closeEVEStateDatabaseForTest } = await import("../state/eve-state-db.js");
  ...
  it("prints actionable disabled proxy config output", async () => {
    ...
    expect(process.stdout["write"]).toHaveBeenCalledWith(
      "Proxy validation failed\n\n" +
        ...
        "Next steps\n" +
        "  Enable proxy.enabled with proxy.proxyUrl or EVE_PROXY_URL, or pass --proxy-url for an explicit one-off validation.\n",
    );
  });
  ...
  it("redacts malformed proxy URLs in text output", async () => {
    ...
    expect(process.stdout["write"]).toHaveBeenCalledWith(
      "Proxy validation failed\n\n" +
        ...
        "Next steps\n" +
        "  Fix proxy.proxyUrl, EVE_PROXY_URL, or --proxy-url so it uses a reachable http:// or https:// proxy.\n",
    );
  });
