...
const EXAMPLES = [
  ["eve onboard", "Run guided setup for a local Gateway, workspace, auth, and channels."],
  ["eve setup", "Create the baseline config, workspace, and session folders."],
  ["eve configure", "Change models, Gateway, channels, plugins, skills, and health checks."],
  ["eve status", "Check Gateway, channel, model, and recent-session status."],
  ["eve doctor --fix", "Repair common config, service, plugin, and channel problems."],
  ["eve channels add", "Add or update a chat channel account with guided prompts."],
  ["eve channels status", "See connected messaging accounts and login state."],
  ["eve --dev gateway", "Run a dev Gateway (isolated state/config) on ws://127.0.0.1:19001."],
  ["eve gateway run --force", "Start the Gateway and replace anything bound to its port."],
  ["eve models status", "Show model/provider auth health before running agents."],
  ["eve plugins list", "Inspect enabled, disabled, and installed plugins."],
  [
    'eve agent --to +15555550123 --message "Run summary" --deliver',
    "Run one agent turn through the Gateway and optionally deliver the reply.",
  ],
  [
    'eve message send --channel telegram --target @mychat --message "Hi"',
...
export function configureProgramHelp(
  ...
  program
    ...
    .option(
      "--container <name>",
      "Run the CLI inside a running Podman/Docker container named <name> (default: env EVE_CONTAINER)",
    )
    .option(
      "--dev",
      "Dev profile: isolate state under ~/.eve-dev, default gateway port 19001, and shift derived ports (browser/canvas)",
    )
    .option(
      "--profile <name>",
      "Use a named profile (isolates EVE_STATE_DIR/EVE_CONFIG_PATH under ~/.eve-<name>)",
    )
    .option(
      "--log-level <level>",
  ...
  program.addHelpText("beforeAll", () => {
    if (hasEmittedCliBanner() || process.env.EVE_SUPPRESS_HELP_BANNER === "1") {
      return "";
    }
    const rich = isRich();
  ...
  program.addHelpText("afterAll", ({ command }) => {
    ...
    const docs = formatDocsLink("/cli", "docs.eve.ai/cli");
