...
export function registerAgentsCommands(program: Command): void {
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.eve.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ['eve agents set-identity --agent main --name "EVE" --emoji "🦞"', "Set name + emoji."],
  ["eve agents set-identity --agent main --avatar avatars/eve.png", "Set avatar path."],
  [
    "eve agents set-identity --workspace ~/.eve/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "eve agents set-identity --identity-file ~/.eve/workspace/IDENTITY.md --agent main",
