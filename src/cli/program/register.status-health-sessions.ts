...
export function registerStatusHealthSessionsCommands(program: Command) {
  program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["eve status", "Show channel health + session summary."],
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.eve.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.eve.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
  ...
  const sessionsCmd = addSessionsListOptions(
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["eve sessions", "List all sessions."],
          ["eve sessions --agent work", "List sessions for one agent."],
          ["eve sessions --all-agents", "Aggregate sessions across agents."],
          ["eve sessions --active 120", "Only last 2 hours."],
          ["eve sessions --limit 25", "Show the newest 25 sessions."],
          ["eve sessions --json", "Machine-readable output."],
          ["eve sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.eve.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["eve sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "eve sessions cleanup --dry-run --fix-missing",
  ...
  sessionsCmd
    ...
    .option("--output <path>", "Output directory name inside .eve/trajectory-exports")
  ...
  sessionsCmd
    ...
    .action(async (key: string, opts, command) => {
      ...
      // command, e.g. `eve sessions --agent work compact <key>`. Merge those
  ...
  const commitmentsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["eve commitments", "List pending inferred follow-ups."],
          ["eve commitments --all", "List all inferred follow-ups."],
          ["eve commitments --agent work", "List one agent's inferred follow-ups."],
          ["eve commitments dismiss cm_abc123", "Dismiss a follow-up."],
        ])}`,
    )
    .action(async (opts) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
