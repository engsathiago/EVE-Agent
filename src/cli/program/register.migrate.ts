...
export function registerMigrateCommand(program: Command) {
  const migrate = addVerifyPluginAppsOption(
    program
      ...
      .option("--no-backup", "Skip the pre-migration EVE backup")
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["eve migrate list", "Show available migration providers."],
          ["eve migrate hermes", "Preview Hermes migration, then prompt before applying."],
          ["eve migrate hermes --dry-run", "Preview Hermes migration only."],
          [
            "eve migrate apply hermes --yes",
            "Apply Hermes migration non-interactively after writing a verified backup.",
          ],
          [
            "eve migrate hermes --no-auth-credentials",
  ...
  addMigrationOptions(
    migrate
      ...
      .description("Preview a migration without changing EVE state"),
  ).action(async (provider, opts) => {
    await runCommandWithRuntime(defaultRuntime, async () => {
