...
vi.mock("../../agents/eve-tools.runtime.js", () => ({
  createEVETools: (...args: unknown[]) => createEVEToolsMock(...args),
}));
...
function mockToolDispatchedSkillCommand() {
  ...
  createEVEToolsMock.mockReturnValue([
    {
      name: "send_status",
      execute: toolExecute,
    },
...
describe("handleInlineActions", () => {
  ...
  it("passes sender identity into inline tool runtimes", async () => {
    ...
    expect(result).toEqual({ kind: "reply", reply: { text: "✅ Done." } });
    const toolsArgs = mockObjectArg(createEVEToolsMock, "createEVETools");
  ...
  it("passes sandboxed runtime state into inline tool construction", async () => {
    ...
    expect(createEVEToolsMock).toHaveBeenCalledWith(
