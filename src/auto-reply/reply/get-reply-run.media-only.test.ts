...
describe("runPreparedReply media-only handling", () => {
  ...
  it("hydrates current image MediaPaths by extension when MediaTypes are missing", async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), "eve-followup-image-"));
  ...
  it("runs room events as contextual events instead of direct user prompts", async () => {
    ...
    const call = requireLastRunReplyAgentCall();
    expect(call?.commandBody).toBe("[EVE room event]");
    expect(call?.transcriptCommandBody).toBe("");
    expect(call?.followupRun.prompt).toBe("[EVE room event]");
    expect(call?.followupRun.transcriptPrompt).toBe("");
    ...
    expect(call?.followupRun.currentInboundContext?.text).toContain("[EVE room event]");
    expect(call?.followupRun.currentInboundContext?.text).toContain(
  ...
  it("keeps heartbeat prompts out of visible transcript prompt", async () => {
    ...
    expect(call?.transcriptCommandBody).toBe("[EVE heartbeat poll]");
    expect(call?.followupRun.transcriptPrompt).toBe("[EVE heartbeat poll]");
  });
  ...
  it.each([
    ...
    async (commandText, startupAction) => {
      ...
      expect(call?.transcriptCommandBody).toBe(`[EVE session ${startupAction}]`);
      expect(call?.followupRun.transcriptPrompt).toBe(`[EVE session ${startupAction}]`);
