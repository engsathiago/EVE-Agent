...
export function resolveAcpInstallCommandHint(cfg: EVEConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(workspaceLocalPath)) {
      return `eve plugins install ${workspaceLocalPath}`;
    }
    ...
    if (bundledInstallHint) {
      const localPath = bundledInstallHint.replace(/^eve plugins install /u, "");
    ...
    return "eve plugins install acpx";
  }
