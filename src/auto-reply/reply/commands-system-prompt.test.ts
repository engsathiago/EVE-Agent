...
const { createEVECodingToolsMock } = vi.hoisted(() => ({
  createEVECodingToolsMock: vi.fn(() => []),
}));
...
describe("resolveCommandsSystemPromptBundle", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    createEVECodingToolsMock.mockClear();
  ...
  it("uses materialized sandbox skill paths for sandbox command prompts", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "eve-command-sandbox-skills-"));
    try {
  ...
  it("uses config-backed prompt settings for the target agent", async () => {
    ...
    createEVECodingToolsMock.mockReturnValue([{ name: "sessions_spawn" }] as never);
    const params = makeParams();
    params.cfg = {
      agents: {
        defaults: {
