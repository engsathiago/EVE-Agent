...
const EVE_CLI_WRAPPER_PATH = fileURLToPath(new URL("../../eve.mjs", import.meta.url));
const EVE_RUN_NODE_SCRIPT_PATH = fileURLToPath(
  new URL("../../scripts/run-node.mjs", import.meta.url),
);
const EVE_DIST_ENTRY_JS_PATH = fileURLToPath(new URL("../../dist/entry.js", import.meta.url));
const EVE_DIST_ENTRY_MJS_PATH = fileURLToPath(new URL("../../dist/entry.mjs", import.meta.url));
...
export function resolveLocalAuthCliInvocation(params?: {
  ...
  const hasDistEntry =
    params?.hasDistEntry ??
    (existsSync(EVE_DIST_ENTRY_JS_PATH) || existsSync(EVE_DIST_ENTRY_MJS_PATH));
  const hasRunNodeScript = params?.hasRunNodeScript ?? existsSync(EVE_RUN_NODE_SCRIPT_PATH);
  const command = params?.execPath ?? process.execPath;
  const wrapperPath = params?.wrapperPath ?? EVE_CLI_WRAPPER_PATH;
  const runNodePath = params?.runNodePath ?? EVE_RUN_NODE_SCRIPT_PATH;
...
export function resolveGatewayDisconnectState(reason?: string): {
  ...
  if (/pairing required/i.test(reasonLabel)) {
    return {
      ...
      activityStatus: "pairing required: run eve devices list",
      pairingHint:
        "Pairing required. Run `eve devices list`, approve your request ID, then reconnect.",
    };
  }
  return {
    connectionStatus: `gateway disconnected: ${reasonLabel}`,
...
export function scheduleProcessExitAfterTuiReturn(
  ...
  const timer = setTimeoutFn(() => {
    try {
      writeStderr("eve tui forcing process exit after return\n");
    } catch {
...
function resolveEmptySessionInfoDefaults(config: EVEConfig): SessionInfo {
  return {
...
export async function runTui(opts: RunTuiOptions): Promise<TuiResult> {
  ...
  const previousConsoleSubsystemFilter = isLocalMode
    ...
    : null;
  if (isLocalMode) {
    setConsoleSubsystemFilter(["__eve_tui_quiet__"]);
  }
  ...
  const updateHeader = () => {
    ...
    const title = opts.title ?? "eve tui";
    header.setText(
      theme.header(
  ...
  const forceExit = () => {
    try {
      process.stderr.write("eve tui forcing exit\n");
    } catch {
    ...
    void Promise.resolve()
      ...
      .catch((err: unknown) => {
        if (!isTuiTerminalLossError(err)) {
          try {
            process.stderr.write(`eve tui shutdown failed: ${String(err)}\n`);
          } catch {
