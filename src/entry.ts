#!/usr/bin/env node
// Boots the EVE CLI entry point under Node.
// CLI process entrypoint for EVE command execution.
...
import {
  enableEVECompileCache,
  resolveEntryInstallRoot,
  respawnWithoutEVECompileCacheIfNeeded,
} from "./entry.compile-cache.js";
import { buildCliRespawnPlan, runCliRespawnPlan } from "./entry.respawn.js";
import { tryHandleRootVersionFastPath } from "./entry.version-fast-path.js";
import { isTruthyEnvValue, normalizeEnv } from "./infra/env.js";
import { ensureEVEExecMarkerOnProcess } from "./infra/eve-exec-env.js";
import { isMainModule } from "./infra/is-main.js";
import { installProcessWarningFilter } from "./infra/warning-filter.js";

const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "eve.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "eve.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  const waitingForCompileCacheRespawn = respawnWithoutEVECompileCacheIfNeeded({
  ...
  if (!waitingForCompileCacheRespawn) {
    ...
    if (shouldForceReadOnlyAuthStore(process.argv)) {
      process.env.EVE_AUTH_STORE_READONLY = "1";
    }
    ...
    if (!ensureCliRespawnReady()) {
      ...
      if (!parsedContainer.ok) {
        console.error(`[eve] ${parsedContainer.error}`);
        process.exit(2);
      }
      ...
      if (!parsed.ok) {
        ...
        console.error(`[eve] ${parsed.error}`);
        process.exit(2);
      }

      const containerTargetName = resolveCliContainerTarget(process.argv);
      if (containerTargetName && parsed.profile) {
        console.error("[eve] --container cannot be combined with --profile/--dev");
        process.exit(2);
      }
