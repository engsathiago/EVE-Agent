...
/** Return true when an installed package needs an eve peer link repair. */
export function installedPackageNeedsEVEPeerLinkRepair(dir: string): boolean {
  ...
  try {
    fsSync.statSync(path.join(dir, "node_modules", "eve-agent"));
    return false;
  } catch (error) {
