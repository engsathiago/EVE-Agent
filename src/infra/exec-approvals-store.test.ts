...
const testEnvSnapshot = captureEnv(["EVE_HOME", "EVE_STATE_DIR"]);
...
describe("exec approvals store helpers", () => {
  it("expands home-prefixed default file and socket paths", () => {
    ...
    expect(path.normalize(resolveExecApprovalsPath())).toBe(
      path.normalize(path.join(dir, ".eve", "exec-approvals.json")),
    );
    expect(path.normalize(resolveExecApprovalsSocketPath())).toBe(
      path.normalize(path.join(dir, ".eve", "exec-approvals.sock")),
    );
    expect(resolveExecApprovalsDisplayPath()).toBe("~/.eve/exec-approvals.json");
  });

  it("uses EVE_STATE_DIR for default file and socket paths", () => {
    const dir = createHomeDir();
    ...
    expect(resolveExecApprovalsTranscriptPath()).toBe("$EVE_STATE_DIR/exec-approvals.json");
  ...
  it("fails closed without writing target approvals before state migration runs", () => {
    ...
    fs.writeFileSync(
      ...
      `${JSON.stringify({
        version: 1,
        socket: {
          path: path.join(dir, ".eve", "exec-approvals.sock"),
          token: "legacy-token",
        },
        defaults: {
          security: "deny",
  ...
  it("accepts a symlinked EVE_HOME as the trusted approvals root", () => {
    ...
    setTestEnvValue("EVE_HOME", linkedHome);
  ...
  it("refuses to traverse symlinked approvals components below a symlinked home", () => {
    ...
    fs.symlinkSync(realHome, linkedHome, "dir");
    fs.symlinkSync(linkedStateTarget, path.join(realHome, ".eve"), "dir");
    setTestEnvValue("EVE_HOME", linkedHome);
