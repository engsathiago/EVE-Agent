...
async function createTimelineEnv() {
  const dir = await mkdtemp(join(tmpdir(), "eve-diagnostics-timeline-"));
  tempDirs.push(dir);
  return {
    env: {
      EVE_DIAGNOSTICS: "timeline",
      EVE_DIAGNOSTICS_RUN_ID: "run-1",
      EVE_DIAGNOSTICS_ENV: "env-1",
      EVE_DIAGNOSTICS_TIMELINE_PATH: join(dir, "nested", "timeline.jsonl"),
...
describe("diagnostics timeline", () => {
  it("detects when timeline output is enabled", async () => {
    ...
    expect(
      isDiagnosticsTimelineEnabled({
        env: { ...env, EVE_DIAGNOSTICS: "diagnostics.timeline" },
      }),
    ).toBe(true);
    expect(
  ...
  it("honors config diagnostics flags after config is available", async () => {
    ...
    delete envWithoutFlag.EVE_DIAGNOSTICS;
    const configWithTimeline = { diagnostics: { flags: ["timeline"] } } as EVEConfig;
    const configWithWildcard = { diagnostics: { flags: ["*"] } } as EVEConfig;
    const configWithoutTimeline = { diagnostics: { flags: ["telegram.http"] } } as EVEConfig;
  ...
  it("writes JSONL diagnostic events with the stable envelope", async () => {
    ...
    expect(event?.schemaVersion).toBe("eve.diagnostics.v1");
  ...
  it("records span start and end events around successful work", async () => {
    ...
    delete configOnlyEnv.EVE_DIAGNOSTICS;

    await expect(
      measureDiagnosticsTimelineSpan("runtimeDeps.stage", () => "ok", {
        phase: "startup",
        attributes: { pluginCount: 3 },
        config: { diagnostics: { flags: ["timeline"] } } as EVEConfig,
        env: configOnlyEnv,
      }),
    ).resolves.toBe("ok");
