// Creates temporary EVE directories for runtime scratch work.
import fs from "node:fs";
import { tmpdir as getOsTmpDir } from "node:os";
import path from "node:path";

/** Preferred shared EVE temp root on POSIX systems when ownership and permissions are safe. */
export const POSIX_EVE_TMP_DIR = "/tmp/eve";
...
export type ResolvePreferredEVETmpDirOptions = {
...
export function resolvePreferredEVETmpDir(
  ...
  const fallback = (): string => {
    const suffix = uid === undefined ? "eve" : `eve-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[eve] tightened permissions on temp dir: ${candidatePath}`);
      return resolveDirState(candidatePath) === "available";
    } catch {
      return false;
    }
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback EVE temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback EVE temp dir: ${fallbackPath}`);
    }
  ...
  const preferredDir = POSIX_EVE_TMP_DIR;
