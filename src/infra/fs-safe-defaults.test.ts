// Covers EVE's default fs-safe Python helper configuration.
...
describe("fs-safe defaults", () => {
  afterEach(() => {
    ...
    delete process.env.EVE_FS_SAFE_PYTHON_MODE;
  });

  it("disables the Python helper by default in EVE", async () => {
  ...
  it("honors the EVE-specific env mode override", async () => {
    process.env.EVE_FS_SAFE_PYTHON_MODE = "auto";
