...
describe("tmp-eve-dir browser-safe import", () => {
  it("loads when a browser fs shim omits constants", async () => {
    const resultKey = `__eveTmpDirBrowserImport_${crypto.randomUUID().replaceAll("-", "_")}`;
    ...
    const bundled = await build({
      ...
      stdin: {
        contents: `
          import { POSIX_EVE_TMP_DIR, resolvePreferredEVETmpDir } from "./src/infra/tmp-eve-dir.ts";
          globalThis.${resultKey} = {
            posixTmpDir: POSIX_EVE_TMP_DIR,
            resolverType: typeof resolvePreferredEVETmpDir,
          };
        `,
        loader: "ts",
        resolveDir: process.cwd(),
        sourcefile: "tmp-eve-dir-browser-entry.ts",
      },
      write: false,
    });
