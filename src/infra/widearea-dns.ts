...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.EVE_WIDE_AREA_DOMAIN ?? null;
  try {
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*eve-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "eve";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "eve");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "eve-gw");
  ...
  records.push(`_eve-gw._tcp IN PTR ${instanceLabel}._eve-gw._tcp`);
  records.push(`${instanceLabel}._eve-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._eve-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; eve-content-hash: ${contentHash}\n${contentBody}`;
}
