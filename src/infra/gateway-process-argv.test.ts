...
describe("isGatewayArgv", () => {
  ...
  it("matches known entrypoints across slash and case variants", () => {
    expect(isGatewayArgv(["NODE", "C:\\EVE\\DIST\\ENTRY.JS", "gateway"])).toBe(true);
    expect(isGatewayArgv(["bun", "/srv/eve/scripts/run-node.mjs", "gateway"])).toBe(true);
    expect(isGatewayArgv(["node", "/srv/eve/eve.mjs", "gateway"])).toBe(true);
    expect(isGatewayArgv(["tsx", "/srv/eve/src/entry.ts", "gateway"])).toBe(true);
    expect(isGatewayArgv(["tsx", "/srv/eve/src/index.ts", "gateway"])).toBe(true);
  });

  it("matches the eve executable but gates the gateway binary behind the opt-in flag", () => {
    expect(isGatewayArgv(["C:\\bin\\eve.cmd", "gateway"])).toBe(true);
    expect(isGatewayArgv(["/usr/local/bin/eve-gateway", "gateway"])).toBe(false);
    expect(
      isGatewayArgv(["/usr/local/bin/eve-gateway", "gateway"], {
        allowGatewayBinary: true,
      }),
    ).toBe(true);
    expect(
      isGatewayArgv(["C:\\bin\\eve-gateway.EXE", "gateway"], {
        allowGatewayBinary: true,
      }),
    ).toBe(true);
  });
  ...
  it("rejects unknown gateway argv even when the token is present", () => {
    expect(isGatewayArgv(["node", "/srv/eve/custom.js", "gateway"])).toBe(false);
