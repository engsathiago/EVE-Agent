...
describe("isVolatileBackupPath", () => {
  ...
  it("does not match paths that escape the anchor via `..`", () => {
    // `/opt/eve/state/sessions/../config.jsonl` resolves to
    // `/opt/eve/state/config.jsonl`, which is NOT inside sessions/.
