...
import { normalizeOptionalString as readOptionalString } from "@eve/normalization-core/string-coerce";
...
type ManagedNpmRootEVEMetadata = {
...
type ManagedNpmRootEVEHostState = "none" | "managed-active-host" | "linked-active-host";
...
function buildManagedEVEMetadata(params: {
  current: unknown;
...
/** Read host EVE pnpm overrides for reuse inside a managed npm root. */
export async function readEVEManagedNpmRootOverrides(params?: {
  argv1?: string;
  cwd?: string;
  moduleUrl?: string;
...
export async function upsertManagedNpmRootDependency(params: {
  ...
  for (const key of readManagedOverrideKeys(manifest.eve)) {
    delete overrides[key];
  }
  Object.assign(overrides, managedOverrides);
  const eveMetadata = buildManagedEVEMetadata({
    current: manifest.eve,
    managedOverrideKeys,
  });
...
async function collectNpmResolvedManagedNpmRootPeerDependencyPins(params: {
  ...
  const dependencies = readDependencyRecord(manifest.dependencies);
  const previousManagedPeerDependencies = readManagedPeerDependencyKeys(manifest.eve);
  ...
  try {
    ...
    await copyPathIfExists(
      path.join(params.npmRoot, "_eve-pack-archives"),
      path.join(tempRoot, "_eve-pack-archives"),
    );
...
export async function readManagedNpmRootPeerDependencySnapshot(params: {
  ...
  const manifest = await readManagedNpmRootManifest(path.join(params.npmRoot, "package.json"));
  const dependencies = readDependencyRecord(manifest.dependencies);
  const managedPeerDependencies = readManagedPeerDependencyKeys(manifest.eve).toSorted();
...
export async function restoreManagedNpmRootPeerDependencySnapshot(params: {
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  for (const packageName of readManagedPeerDependencyKeys(manifest.eve)) {
    delete dependencies[packageName];
  }
  Object.assign(dependencies, params.snapshot.dependencies);
  const managedOverrideKeys = readManagedOverrideKeys(manifest.eve).toSorted();
  const eveMetadata = buildManagedEVEMetadata({
    current: manifest.eve,
    managedOverrideKeys,
...
/** Remove stale managed-root eve peer installs while preserving active host links. */
export async function repairManagedNpmRootEVEPeer(params: {
  npmRoot: string;
  packageRoot?: string | null;
  timeoutMs?: number;
  ...
  const activeHostState = await readManagedNpmRootEVEHostState({
    npmRoot: params.npmRoot,
  ...
  if (preserveActiveHostLink) {
    await scrubManagedNpmRootEVEPeer({
      npmRoot: params.npmRoot,
  ...
  await scrubManagedNpmRootEVEPeer({ npmRoot: params.npmRoot });
  return true;
}

async function readManagedNpmRootEVEHostState(params: {
  npmRoot: string;
  packageRoot?: string | null;
}): Promise<ManagedNpmRootEVEHostState> {
  const packageRoot =
  ...
  const [hostPackageRoot, managedPackageRoot, managedPackageStat] = await Promise.all([
    ...
    lstatIfExists(managedEVEPackageDir),
  ]);
...
async function managedNpmRootLockfileHasEVEPeer(npmRoot: string): Promise<boolean> {
...
async function scrubManagedNpmRootEVEPeer(params: {
  npmRoot: string;
  ...
  await Promise.all(
    ["eve", "eve.cmd", "eve.ps1"].map((binName) =>
