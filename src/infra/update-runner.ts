// Runs EVE package update checks, package steps, and restart handoff.
...
const DEFAULT_PACKAGE_NAME = "eve-agent";
...
const PREFLIGHT_TEMP_PREFIX = process.platform === "win32" ? "ocu-pf-" : "eve-update-preflight-";
...
const DEV_PREFLIGHT_LINT_ENV: NodeJS.ProcessEnv = {
  ...
  EVE_OXLINT_SHARDS_SERIAL: "1",
};
const DEV_PREFLIGHT_LINT_OPT_IN_ENV = "EVE_UPDATE_PREFLIGHT_LINT";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["eve-agent", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && (await pathsReferToSameLocation(gitRoot, pkgRoot))) {
    ...
    try {
      ...
      const doctorEntry = path.join(gitRoot, "eve.mjs");
      ...
      const doctorStep = await runStep(
        step("eve doctor", doctorArgv, gitRoot, {
