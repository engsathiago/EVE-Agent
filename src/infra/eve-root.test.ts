// Covers EVE package root resolution.
...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__eve_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "eve-root");
...
function expectResolvedPackageRoot(
  syncResolver: typeof import("./eve-root.js").resolveEVEPackageRootSync,
  asyncResolver: typeof import("./eve-root.js").resolveEVEPackageRoot,
  opts: Parameters<typeof import("./eve-root.js").resolveEVEPackageRootSync>[0],
...
vi.mock("./eve-root.fs.runtime.js", () => ({
  eveRootFsSync: mockFsModule(),
  eveRootFs: mockFsPromisesModule(),
}));

describe("resolveEVEPackageRoot", () => {
  let resolveEVEPackageRoot: typeof import("./eve-root.js").resolveEVEPackageRoot;
  let resolveEVEPackageRootSync: typeof import("./eve-root.js").resolveEVEPackageRootSync;
  let clearEVEPackageRootCaches: typeof import("./eve-root.js").testing.clearEVEPackageRootCaches;

  beforeAll(async () => {
    ({
      resolveEVEPackageRoot,
      resolveEVEPackageRootSync,
      testing: { clearEVEPackageRootCaches },
    } = await import("./eve-root.js"));
  });

  beforeEach(() => {
    clearEVEPackageRootCaches();
    state.entries.clear();
  ...
  it.each([
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "eve");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "eve.mjs")));
    ...
    {
      name: "falls through from a non-eve moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-eve");
    ...
    {
      name: "returns null for non-eve package roots",
      setup: () => {
        const pkgRoot = fx("not-eve");
        setPackageRoot(pkgRoot, "not-eve");
