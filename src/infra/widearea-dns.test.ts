...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "eve.internal", expected: "eve.internal." },
    { value: "eve.internal.", expected: "eve.internal." },
    { value: "  eve.internal  ", expected: "eve.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { EVE_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { EVE_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
    ...
    },
    {
      ...
      params: {
        env: { EVE_WIDE_AREA_DOMAIN: "foo/bar" } as NodeJS.ProcessEnv,
      },
      expected: null,
    },
  ...
  it("builds valid zone paths under the DNS config directory", () => {
    ...
    const zonePath = getWideAreaZonePath("eve.internal.");

    expect(zonePath).toBe(path.join(dnsDir, "eve.internal.db"));
    expect(path.relative(dnsDir, zonePath)).toBe("eve.internal.db");
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    ...
    expectZoneRecords(txt, [
      ...
      `_eve-gw._tcp IN PTR studio-london._eve-gw._tcp`,
      `studio-london._eve-gw._tcp IN SRV 0 0 18789 studio-london`,
      `displayName=Mac Studio (EVE)`,
      `gatewayPort=18789`,
      `sshPort=22`,
      `cliPath=/opt/homebrew/bin/eve`,
  ...
  it.each([
    ...
    },
    {
      ...
      records: [
        `$ORIGIN eve.internal.`,
        `studio-london IN A 100.123.224.76`,
        `studio-london._eve-gw._tcp IN TXT`,
        `displayName=Mac Studio (EVE)`,
        `gatewayTls=1`,
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("eve.internal."),
      changed: false,
    });
    expect(writeSpy).not.toHaveBeenCalled();
  });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("eve.internal."),
      expectedZoneText,
      "utf-8",
    );
  });
