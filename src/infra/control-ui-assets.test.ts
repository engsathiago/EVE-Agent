...
vi.mock("./eve-root.js", () => ({
  resolveEVEPackageRoot: vi.fn(async () => null),
  resolveEVEPackageRootSync: vi.fn(() => null),
}));
...
let eveRoot: typeof import("./eve-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    eveRoot = await import("./eve-root.js");
  });
  ...
  it("resolves dist control-ui index path for symlinked argv1 via realpath", async () => {
    const pkgRoot = abs("fixtures/bun-global/eve");
    const wrapperArgv1 = abs("fixtures/bin/eve");
  ...
  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-eve");
  ...
  it("prefers packaged app Control UI assets in Contents/Resources", () => {
    const execPath = abs("fixtures/EVE.app/Contents/MacOS/EVE");
    const bundledUiDir = abs("fixtures/EVE.app/Contents/Resources/control-ui");
  ...
  it("detects package-proven control-ui roots", () => {
    const pkgRoot = abs("fixtures/eve-package-root");
    const uiDir = path.join(pkgRoot, "dist", "control-ui");
    setDir(uiDir);
    setFile(path.join(uiDir, "index.html"), "<html></html>\n");
    (
