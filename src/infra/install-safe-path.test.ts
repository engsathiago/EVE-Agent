...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@eve/matrix", expected: "matrix" },
...
describe("packageNameMatchesId", () => {
  it.each([
    { packageName: "@eve/matrix", id: "matrix", expected: true },
    { packageName: "@eve/matrix", id: "@eve/matrix", expected: true },
    { packageName: "@eve/matrix", id: "quietchat", expected: false },
    { packageName: " ", id: "matrix", expected: false },
    { packageName: "@eve/matrix", id: " ", expected: false },
...
describe("resolveSafeInstallDir", () => {
  it("resolves install dirs under the base directory", () => {
    expect(
      ...
      ok: true,
      path: path.join("/tmp/plugins", "@eve__matrix"),
    });
  });
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    await withTempDir({ prefix: "eve-install-safe-" }, async (baseDir) => {
  ...
  it.skipIf(!canCreateDirectorySymlinks)(
    ...
    async () => {
      await withTempDir({ prefix: "eve-install-safe-" }, async (baseDir) => {
        await withTempDir({ prefix: "eve-install-safe-outside-" }, async (outsideDir) => {
  ...
  it.skipIf(!canCreateDirectorySymlinks)(
    ...
    async () => {
      await withTempDir({ prefix: "eve-install-safe-" }, async (parentDir) => {
