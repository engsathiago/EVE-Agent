...
/** Supported package managers for EVE global install and update flows. */
...
function isPnpmEVESourceInstallSpec(spec: string): boolean {
...
/**
 * Verifies that a global package root looks like a packaged EVE install
  ...
  await Promise.all(
    BUNDLED_RUNTIME_SIDECAR_PATHS.map(async (relativePath) => {
      ...
      if (
        ...
        (await pathExists(path.join(packageRoot, pluginRoot, "eve.plugin.json")))
      ) {
...
async function resolvePortableGitPathPrepend(): Promise<string[]> {
  ...
  const portableGitRoot = path.join(localAppData, "EVE", "deps", "portable-git");
  const candidates = [
 ...
 * Used for staged installs where EVE creates the prefix itself.
 */
...
/**
 * Detects an installed global EVE package by probing package-manager roots
...
/**
 * Builds the primary package-manager argv for a global EVE install.
...
export function globalInstallArgs(
  ...
  if (resolved.manager === "pnpm") {
    return [
      ...
      ...(isPnpmEVESourceInstallSpec(spec) ? [PNPM_EVE_BUILD_ALLOWLIST_FLAG] : []),
      spec,
    ];
  }
  if (resolved.manager === "bun") {
