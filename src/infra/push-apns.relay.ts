...
export const DEFAULT_APNS_RELAY_BASE_URL = "https://ios-push-relay.eve.ai";
const DEFAULT_APNS_RELAY_TIMEOUT_MS = 10_000;
const GATEWAY_DEVICE_ID_HEADER = "x-eve-gateway-device-id";
const GATEWAY_SIGNATURE_HEADER = "x-eve-gateway-signature";
const GATEWAY_SIGNED_AT_HEADER = "x-eve-gateway-signed-at-ms";
...
export function resolveApnsRelayConfigFromEnv(
  ...
  const envBaseUrl = normalizeNonEmptyString(env.EVE_APNS_RELAY_BASE_URL);
  ...
  if (!baseUrl) {
    return {
      ok: false,
      error:
        "APNs relay config missing: set gateway.push.apns.relay.baseUrl or EVE_APNS_RELAY_BASE_URL for relay registrations without the hosted relay origin",
    };
  }
