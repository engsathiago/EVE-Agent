...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "eve-provider-auth-suite-" });
...
describe("resolveProviderAuths key normalization", () => {
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".eve");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".eve", "agents", "main", "agent");
  }
