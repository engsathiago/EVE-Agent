...
function resolveAvailableKnownChannel(params: {
  ...
  // been eagerly loaded (e.g. `eve agent --local`). Already-loaded and
...
export function isConfiguredChannel(cfg: EVEConfig, channelId: string): boolean {
...
function formatMissingOfficialExternalChannelsMessage(
  ...
  return `Configured official external channels ${labels} are missing their plugins. Run: eve doctor --fix, or install individually: ${installCommands}.`;
}

function formatNoConfiguredChannelsMessage(): string {
  return [
    "Channel is required (no configured channels detected).",
    "Run eve channels add to configure one, or pass --channel <channel> after enabling a channel.",
    "Use eve channels list --all to see available channel ids.",
  ].join(" ");
}
...
async function isPluginConfigured(plugin: ChannelPlugin, cfg: EVEConfig): Promise<boolean> {
