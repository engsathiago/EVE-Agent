...
const fixtureRootTracker = createSuiteTempRootTracker({ prefix: "eve-update-" });
...
describe("runGatewayUpdate", () => {
  const preflightPrefixPattern = /(?:eve-update-preflight-|ocu-pf-)/;
  ...
  beforeEach(async () => {
    ...
    await fs.writeFile(path.join(tempDir, "eve.mjs"), "export {};\n", "utf-8");
  });
  ...
  async function createStableTagRunner(params: {
    ...
    const doctorKey = `${doctorNodePath} ${path.join(tempDir, "eve.mjs")} doctor --non-interactive --fix`;
  ...
  it("does not fetch tags for dev updates", async () => {
    ...
    const doctorNodePath = await resolveStableNodePath(process.execPath);
    const doctorCommand = `${doctorNodePath} ${path.join(tempDir, "eve.mjs")} doctor --non-interactive --fix`;
  ...
  it("uses pnpm highest resolution mode for update installs", async () => {
    ...
    const { runCommand } = createGitInstallRunner({
      ...
      doctorCommand: `${doctorNodePath} ${path.join(tempDir, "eve.mjs")} doctor --non-interactive --fix`,
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    const { runner, calls } = createRunner({
      ...
      "pnpm build": { stdout: "" },
      "pnpm ui:build": { stdout: "" },
      [`${doctorNodePath} ${path.join(tempDir, "eve.mjs")} doctor --non-interactive --fix`]: {
        stdout: "",
      },
    });
  ...
  it("bootstraps pnpm via npm when pnpm and corepack are unavailable", async () => {
    ...
    const { calls, runCommand } = createGitInstallRunner({
      ...
      buildCommand: "pnpm build",
      uiBuildCommand: "pnpm ui:build",
      doctorCommand: `${process.execPath} ${path.join(tempDir, "eve.mjs")} doctor --non-interactive`,
      onCommand: (key, options) => {
        if (key === "pnpm --version") {
          const envPath = options?.env?.PATH ?? options?.env?.Path ?? "";
          if (envPath.includes("eve-update-pnpm-")) {
  ...
  it("runs dev preflight lint in constrained mode when explicitly enabled", async () => {
    ...
    const result = await withEnvAsync({ EVE_UPDATE_PREFLIGHT_LINT: "1" }, async () =>
    ...
    expect(lintEnv[0]?.EVE_LOCAL_CHECK).toBe("1");
    expect(lintEnv[0]?.EVE_LOCAL_CHECK_MODE).toBe("throttled");
    expect(lintEnv[0]?.EVE_OXLINT_SHARDS_SERIAL).toBe("1");
  });
  ...
  it("retries windows pnpm git installs with --ignore-scripts for dev updates", async () => {
    ...
    await withMockedWindowsPlatform(async () => {
      const runCommand = async (
        ...
        if (key === "pnpm install") {
          if (options?.cwd && /(?:eve-update-preflight-|ocu-pf-)/.test(options.cwd)) {
  ...
  it("falls back to the cloned cwd when git root probing misses a fresh checkout", async () => {
    ...
    const doctorNodePath = await resolveStableNodePath(process.execPath);
    const doctorCommand = `${doctorNodePath} ${path.join(gitRoot, "eve.mjs")} doctor --non-interactive --fix`;
  ...
  it("runs doctor after global npm updates before reporting success", async () => {
    ...
    expect(result.steps.map((step) => step.name)).toContain("eve doctor");
    expect(doctorEnv?.EVE_UPDATE_IN_PROGRESS).toBe("1");
    expect(doctorEnv?.EVE_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE).toBe("1");
    expect(doctorEnv?.EVE_COMPATIBILITY_HOST_VERSION).toBe("2.0.0");
  });
  ...
  it("fails global npm updates when post-update doctor fails", async () => {
    ...
    expect(lastStep?.name).toBe("eve doctor");
    ...
    const { runCommand } = createGlobalInstallHarness({
      ...
      onInstall: async (options) => {
        ...
        if (options?.installPrefix) {
          ...
          await fs.writeFile(path.join(binDir, "eve"), "#!/bin/sh\n", "utf-8");
        }
      },
    });
  ...
  it("rejects git roots that are not a eve checkout", async () => {
    await fs.mkdir(path.join(tempDir, ".git"));
    ...
    expect(result.status).toBe("error");
    expect(result.reason).toBe("not-eve-root");
    expect(calls.filter((call) => call.includes("status --porcelain"))).toEqual([]);
  });

  it("fails with a clear reason when eve.mjs is missing", async () => {
    await setupGitCheckout({ packageManager: "pnpm@8.0.0" });
    await fs.rm(path.join(tempDir, "eve.mjs"), { force: true });
    ...
    expect(result.steps.some((step) => step.name === "eve doctor entry")).toBe(true);
