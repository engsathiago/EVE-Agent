...
describe("command-analysis risks", () => {
  ...
  it("builds executable payload candidates through carriers and shell wrappers", () => {
    ...
    expect(buildCommandPayloadCandidates(["sudo", "EVE_ENV=1", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    expect(buildCommandPayloadCandidates(["exec", "-a", "eve", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    ).toContain("eve channels login --channel whatsapp");
  });
