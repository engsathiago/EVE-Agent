...
async function withPackageManagerRoot<T>(
  ...
  return await withTempDir({ prefix: "eve-detect-pm-" }, async (root) => {
...
async function writePublishedEVERoot(root: string): Promise<void> {
...
describe("detectPackageManager", () => {
  ...
  it("keeps pnpm-owned virtual-store package roots that ship npm-shrinkwrap", async () => {
    await withTempDir({ prefix: "eve-detect-pm-pnpm-virtual-" }, async (base) => {
