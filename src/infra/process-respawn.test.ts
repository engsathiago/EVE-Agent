...
const triggerEVERestartMock = vi.hoisted(() => vi.fn());
...
vi.mock("./restart.js", () => ({
  triggerEVERestart: (...args: unknown[]) => triggerEVERestartMock(...args),
}));
...
afterEach(() => {
  ...
  spawnMock.mockClear();
  triggerEVERestartMock.mockClear();
...
function expectLaunchdSupervisedWithoutKickstart(params?: { launchJobLabel?: string }) {
  ...
  expect(triggerEVERestartMock).not.toHaveBeenCalled();
  expect(spawnMock).not.toHaveBeenCalled();
}

describe("restartGatewayProcessWithFreshPid", () => {
  it("returns disabled when EVE_NO_RESPAWN is set", () => {
    process.env.EVE_NO_RESPAWN = "1";
    const result = restartGatewayProcessWithFreshPid();
    expect(result.mode).toBe("disabled");
    expect(spawnMock).not.toHaveBeenCalled();
  });

  it("keeps EVE_NO_RESPAWN ahead of inherited supervisor hints", () => {
    clearSupervisorHints();
    setPlatform("darwin");
    process.env.EVE_NO_RESPAWN = "1";
    process.env.LAUNCH_JOB_LABEL = "ai.eve.gateway";
  ...
  it("returns supervised when EVE launchd markers are present on macOS (no kickstart)", () => {
    clearSupervisorHints();
    expectLaunchdSupervisedWithoutKickstart({ launchJobLabel: "ai.eve.gateway" });
  });
  ...
  it("returns supervised for a real gateway XPC launchd job without the injected marker", () => {
    clearSupervisorHints();
    setPlatform("darwin");
    process.env.XPC_SERVICE_NAME = "ai.eve.gateway";
  ...
  it("launchd supervisor never returns failed regardless of triggerEVERestart outcome", () => {
    clearSupervisorHints();
    setPlatform("darwin");
    process.env.EVE_LAUNCHD_LABEL = "ai.eve.gateway";
    // Even if triggerEVERestart *would* fail, launchd path must not call it.
    triggerEVERestartMock.mockReturnValue({
      ok: false,
      method: "launchctl",
  ...
  it("does not treat inherited XPC_SERVICE_NAME as launchd supervision", () => {
    clearSupervisorHints();
    setPlatform("darwin");
    process.env.XPC_SERVICE_NAME = "ai.eve.mac";
    process.env.EVE_PROFILE = "mac";
  ...
  it("uses in-process restart on unmanaged Unix so custom supervisors keep the tracked PID", () => {
    delete process.env.EVE_NO_RESPAWN;
    clearSupervisorHints();
    setPlatform("linux");
  ...
  it("returns supervised when EVE_LAUNCHD_LABEL is set (stock launchd plist)", () => {
    clearSupervisorHints();
    expectLaunchdSupervisedWithoutKickstart();
  });

  it("returns supervised when EVE_SYSTEMD_UNIT is set", () => {
    clearSupervisorHints();
    setPlatform("linux");
    process.env.EVE_SYSTEMD_UNIT = "eve-gateway.service";
    const result = restartGatewayProcessWithFreshPid();
    expect(result.mode).toBe("supervised");
    expect(spawnMock).not.toHaveBeenCalled();
  });

  it("returns supervised when EVE gateway task markers are set on Windows", () => {
    clearSupervisorHints();
    setPlatform("win32");
    process.env.EVE_SERVICE_MARKER = "eve";
    process.env.EVE_SERVICE_KIND = "gateway";
    triggerEVERestartMock.mockReturnValue({ ok: true, method: "schtasks" });
    const result = restartGatewayProcessWithFreshPid();
    expect(result.mode).toBe("supervised");
    expect(triggerEVERestartMock).toHaveBeenCalledOnce();
    expect(spawnMock).not.toHaveBeenCalled();
  });
  ...
  it("ignores node task script hints for gateway restart detection on Windows", () => {
    clearSupervisorHints();
    setPlatform("win32");
    process.env.EVE_TASK_SCRIPT = "C:\\eve\\node.cmd";
    process.env.EVE_TASK_SCRIPT_NAME = "node.cmd";
    process.env.EVE_SERVICE_MARKER = "eve";
    process.env.EVE_SERVICE_KIND = "node";
...
describe("respawnGatewayProcessForUpdate", () => {
  it("keeps EVE_NO_RESPAWN semantics for update restarts", () => {
    clearSupervisorHints();
    process.env.EVE_NO_RESPAWN = "1";

    const result = respawnGatewayProcessForUpdate();

    expect(result).toEqual({ mode: "disabled", detail: "EVE_NO_RESPAWN" });
    expect(spawnMock).not.toHaveBeenCalled();
  });
  ...
  it("rewrites a pnpm-versioned EVE entry before detached update respawn", () => {
    clearSupervisorHints();
    setPlatform("linux");
    process.execArgv = [];
    process.argv = [
  ...
  it("treats Linux EVE gateway service markers as supervised for update restarts", () => {
    clearSupervisorHints();
    setPlatform("linux");
    process.env.EVE_SERVICE_MARKER = "eve";
    process.env.EVE_SERVICE_KIND = "gateway";
