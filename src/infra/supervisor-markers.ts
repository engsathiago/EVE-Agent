...
const SUPERVISOR_HINTS = {
  launchd: ["EVE_LAUNCHD_LABEL"],
  systemd: ["EVE_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["EVE_WINDOWS_TASK_NAME"],
} as const;
...
export interface DetectRespawnSupervisorOptions {
  includeLinuxEVEGatewayServiceMarker?: boolean;
}
...
function isCurrentGatewayLaunchdJob(env: NodeJS.ProcessEnv): boolean {
  const expectedLabel = resolveGatewayLaunchAgentLabel(env.EVE_PROFILE);
  if (
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const marker = env.EVE_SERVICE_MARKER?.trim();
