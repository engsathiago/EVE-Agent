...
const EVE_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION = "eve.diagnostics.v1";
...
export function isDiagnosticsTimelineEnabled(options: DiagnosticsTimelineOptions = {}): boolean {
  ...
  return (
    (isDiagnosticFlagEnabled("timeline", config, env) ||
      ...
      isTruthyEnvValue(env.EVE_DIAGNOSTICS)) &&
    typeof env.EVE_DIAGNOSTICS_TIMELINE_PATH === "string" &&
    env.EVE_DIAGNOSTICS_TIMELINE_PATH.trim().length > 0
  );
}
...
function serializeTimelineEvent(event: DiagnosticsTimelineEvent, env: NodeJS.ProcessEnv): string {
  const normalized = {
    schemaVersion: EVE_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION,
    type: event.type,
    timestamp: event.timestamp ?? new Date().toISOString(),
    name: event.name,
    ...(env.EVE_DIAGNOSTICS_RUN_ID ? { runId: env.EVE_DIAGNOSTICS_RUN_ID } : {}),
    ...(env.EVE_DIAGNOSTICS_ENV ? { envName: env.EVE_DIAGNOSTICS_ENV } : {}),
    pid: process.pid,
...
export function emitDiagnosticsTimelineEvent(
  ...
  const path = env.EVE_DIAGNOSTICS_TIMELINE_PATH?.trim();
  if (!path) {
    return;
  }
