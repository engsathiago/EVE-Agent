...
describe("npm registry spec validation", () => {
  it.each([
    "@eve/voice-call",
    "@eve/voice-call@1.2.3",
    "@eve/voice-call@1.2.3-beta.4",
    "@eve/voice-call@latest",
    "@eve/voice-call@beta",
  ])("accepts %s", (spec) => {
  ...
  it.each([
    ...
    {
      spec: "git+ssh://github.com/engsathiago/eve-agent",
      expected: "URLs are not allowed",
    },
    {
      spec: "@eve/voice-call@",
...
describe("npm registry spec parsing helpers", () => {
  it.each([
    ...
    {
      spec: "@eve/voice-call@2026.5.3-1",
      expected: {
        name: "@eve/voice-call",
        raw: "@eve/voice-call@2026.5.3-1",
        selector: "2026.5.3-1",
    ...
    {
      spec: "@eve/voice-call@1.2.3-beta.1",
      expected: {
        name: "@eve/voice-call",
        raw: "@eve/voice-call@1.2.3-beta.1",
  ...
  it.each([
    { spec: "@eve/voice-call", expected: true },
    { spec: "@eve/voice-call@1.2.3", expected: true },
    { spec: "@other/voice-call", expected: false },
    { spec: "voice-call", expected: false },
    { spec: "npm:@eve/voice-call", expected: false },
    { spec: undefined, expected: false },
  ])("detects EVE-org npm specs for %s", ({ spec, expected }) => {
    expect(isEVEOrgNpmSpec(spec)).toBe(expected);
  });
  ...
  ])("detects EVE stable correction versions for %s", ({ value, expected }) => {
    expect(isEVEStableCorrectionVersion(value)).toBe(expected);
  });
  ...
  ])("compares EVE release versions for %s and %s", ({ left, right, expected }) => {
    expect(compareEVEReleaseVersions(left, right)).toBe(expected);
  });
});
  ...
  it.each([
    {
      spec: "@eve/voice-call",
      resolvedVersion: "1.2.3-beta.1",
      expected: `Use "@eve/voice-call@beta"`,
    },
    {
      spec: "@eve/voice-call@beta",
      resolvedVersion: "1.2.3-rc.1",
