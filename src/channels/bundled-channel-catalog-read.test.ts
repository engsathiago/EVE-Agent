...
// resolveEVEPackageRootSync. Isolate from the real repo by mocking
// moduleUrl/argv1 resolution to null and deriving only from the tmp cwd.
vi.mock("../infra/eve-root.js", () => ({
  resolveEVEPackageRootSync: (opts: { cwd?: string; argv1?: string; moduleUrl?: string }) =>
    opts.cwd ?? null,
  resolveEVEPackageRoot: async (opts: { cwd?: string; argv1?: string; moduleUrl?: string }) =>
    opts.cwd ?? null,
}));
...
const originalTrustBundledPluginsDir = process.env.EVE_TEST_TRUST_BUNDLED_PLUGINS_DIR;
...
afterEach(() => {
  ...
  if (originalTrustBundledPluginsDir === undefined) {
    ...
    process.env.EVE_TEST_TRUST_BUNDLED_PLUGINS_DIR = originalTrustBundledPluginsDir;
  }
...
function useBundledPluginsDir(extensionsRoot: string | undefined): void {
  if (extensionsRoot) {
    process.env.EVE_BUNDLED_PLUGINS_DIR = extensionsRoot;
...
function seedChannelPkg(
  ...
  writeJsonFile(path.join(pluginDir, "eve.plugin.json"), {
    id: opts.id,
...
describe("listBundledChannelCatalogEntries", () => {
  ...
  it("falls back to dist/channel-catalog.json when the resolver returns undefined", () => {
    // EVE_DISABLE_BUNDLED_PLUGINS, missing bundled tree, or an unresolvable
  ...
  it("falls back to dist/channel-catalog.json when the resolved dir has no plugin package.jsons", () => {
    // A stale staged dir or an EVE_BUNDLED_PLUGINS_DIR override pointing at
