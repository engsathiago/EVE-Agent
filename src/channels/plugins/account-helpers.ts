...
export function createAccountListHelpers(
  ...
  options?: {
    ...
    hasImplicitDefaultAccount?: (cfg: EVEConfig) => boolean;
  },
) {
  function hasImplicitDefaultAccount(cfg: EVEConfig): boolean {
  ...
  function resolveConfiguredDefaultAccountId(cfg: EVEConfig): string | undefined {
    const channel = cfg.channels?.[channelKey] as Record<string, unknown> | undefined;
  ...
  function listAccountIds(cfg: EVEConfig): string[] {
  ...
  function resolveDefaultAccountId(cfg: EVEConfig): string {
