...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): EVEConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as EVEConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedDefaultAccountCase(input: EVEConfig, expected: string) {
