// Temp home test helpers create isolated EVE home directories for plugin tests.
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked EVE_HOME.
  delete process.env.EVE_HOME;
  process.env.EVE_STATE_DIR = path.join(base, ".eve");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "eve-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".eve", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
