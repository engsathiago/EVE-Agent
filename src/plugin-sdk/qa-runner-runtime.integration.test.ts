...
const ORIGINAL_ENV = {
  EVE_DISABLE_BUNDLED_PLUGINS: process.env.EVE_DISABLE_BUNDLED_PLUGINS,
  EVE_CONFIG_PATH: process.env.EVE_CONFIG_PATH,
  EVE_STATE_DIR: process.env.EVE_STATE_DIR,
  EVE_TEST_FAST: process.env.EVE_TEST_FAST,
} as const;
...
describe("plugin-sdk qa-runner-runtime linked plugin smoke", () => {
  ...
  it("loads an activated qa runner from a linked plugin path without a bundled install fallback", async () => {
    const stateDir = makeTempDir("eve-qa-runner-state-");
