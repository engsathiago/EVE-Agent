...
const versionFixtureRoot = createSuiteTempRootTracker({ prefix: "eve-version-" });
...
describe("version resolution", () => {
  it("keeps bundled version injection as a direct define identifier", async () => {
    ...
    expect(source).toContain("typeof __EVE_VERSION__");
    expect(source).toContain("? __EVE_VERSION__");
  });

  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "eve-agent", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "eve-agent", version: "2.3.4" });
  ...
  it("ignores non-eve package and blank build-info versions", async () => {
    await withVersionFixtureDir(async (root) => {
  ...
  it("prefers EVE_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        EVE_VERSION: "9.9.9",
        EVE_SERVICE_VERSION: "2.2.2",
  ...
  it("prefers runtime VERSION over stale EVE_VERSION for compatibility checks", () => {
    const previous = process.env.EVE_VERSION;
    const previousService = process.env.EVE_SERVICE_VERSION;
    const previousPackage = process.env.npm_package_version;
    try {
      process.env.EVE_VERSION = "2026.3.25";
      process.env.EVE_SERVICE_VERSION = "2026.3.25-service";
      process.env.npm_package_version = "2026.3.25-package";
      expect(resolveCompatibilityHostVersion()).toBe(VERSION);
    } finally {
      restoreEnvValue("EVE_VERSION", previous);
      restoreEnvValue("EVE_SERVICE_VERSION", previousService);
  ...
  it("keeps explicit env-object overrides for compatibility checks in tests", () => {
    expect(
      resolveCompatibilityHostVersion({
        EVE_VERSION: "2026.3.99",
        EVE_SERVICE_VERSION: "2026.3.98",
  ...
  it("prefers explicit compatibility host overrides over runtime and stale env versions", () => {
    expect(
      resolveCompatibilityHostVersion({
        EVE_COMPATIBILITY_HOST_VERSION: "2026.4.8",
        EVE_VERSION: "2026.3.99",
        EVE_SERVICE_VERSION: "2026.3.98",
        npm_package_version: "2026.3.97",
      }),
