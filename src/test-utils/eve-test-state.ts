// Creates isolated EVE state directories for integration-style tests.
...
type EVETestStateLayout = "home" | "state-only" | "split";

type EVETestStateScenario =
  | "empty"
  | "minimal"
  | "update-stable"
  | "upgrade-survivor"
...
export type EVETestStateOptions = {
  prefix?: string;
  label?: string;
  layout?: EVETestStateLayout;
  scenario?: EVETestStateScenario;
  agentEnv?: "clear" | "main";
  applyEnv?: boolean;
...
const DEFAULT_PREFIX = "eve-test-state-";
const ENV_KEYS = [
  "HOME",
  "USERPROFILE",
  "HOMEDRIVE",
...
function resolveLayout(
  ...
  if (layout === "split") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(root, "config", "eve.json"),
...
function scenarioConfig(options: EVETestStateOptions): Record<string, unknown> | undefined {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.gateway?.token ?? "eve-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function scenarioEnv(options: EVETestStateOptions): Record<string, string | undefined> {
...
export async function createEVETestState(options: EVETestStateOptions = {}): Promise<EVETestState> {
  ...
  const state: EVETestState = {
    ...
    applyEnv: () => {
      ...
      for (const [key, value] of Object.entries(envVars)) {
        // Test fixtures apply a fixed EVE env set, not plugin-provided host env.
        if (value === undefined) {
...
export async function withEVETestState<T>(
  options: EVETestStateOptions,
  fn: (state: EVETestState) => Promise<T>,
): Promise<T> {
  const state = await createEVETestState(options);
  try {
