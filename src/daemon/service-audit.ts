...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds EVE_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `eve gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (isEnvironmentFileOnlySource(command.environmentValueSources?.EVE_GATEWAY_TOKEN)) {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.EVE_GATEWAY_TOKEN);
}
...
function auditGatewayServiceVersion(command: GatewayServiceCommand, issues: ServiceConfigIssue[]) {
  ...
  issues.push({
    ...
    message: `Gateway service was installed by EVE ${serviceVersion}; current CLI is ${VERSION}.`,
