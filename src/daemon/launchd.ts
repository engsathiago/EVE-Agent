...
const EVE_UPDATE_LAUNCHD_LABEL_PREFIX = "ai.eve.update.";
const EVE_MANUAL_UPDATE_LAUNCHD_LABEL_PATTERN = /^ai\.eve\.manual-update\.\d+$/;

export type StaleEVEUpdateLaunchdJob = {
  label: string;
  pid?: number;
  lastExitStatus?: number;
};

function normalizeEVEUpdateLaunchdLabel(label: unknown): string | null {
  if (typeof label !== "string") {
    return null;
  }
  const trimmed = label.trim();
  if (trimmed.startsWith(EVE_UPDATE_LAUNCHD_LABEL_PREFIX)) {
    return trimmed;
  }
  // Manual update jobs include a timestamp-like suffix and should be cleaned up
  // without matching arbitrary ai.eve labels.
  return EVE_MANUAL_UPDATE_LAUNCHD_LABEL_PATTERN.test(trimmed) ? trimmed : null;
}

function isCurrentGatewayLaunchdLabel(label: string, env: NodeJS.ProcessEnv): boolean {
  const gatewayProfileLabel = resolveGatewayLaunchAgentLabel(env.EVE_PROFILE);
  if (label === gatewayProfileLabel) {
    return true;
  }
  if (
    env.EVE_SERVICE_MARKER?.trim() !== GATEWAY_SERVICE_MARKER ||
    env.EVE_SERVICE_KIND?.trim() !== GATEWAY_SERVICE_KIND
  ) {
    return false;
  }
  const configuredLabel = env.EVE_LAUNCHD_LABEL?.trim();
  return Boolean(configuredLabel && label === configuredLabel);
}

function resolveCurrentEVEUpdateLaunchdJobLabel(
  ...
  for (const label of [
    ...
    env.EVE_LAUNCHD_LABEL,
  ]) {
    const normalized = normalizeEVEUpdateLaunchdLabel(label);
    if (normalized) {
...
function resolveLaunchAgentLabel(args?: { env?: Record<string, string | undefined> }): string {
  const envLabel = args?.env?.EVE_LAUNCHD_LABEL?.trim();
  if (envLabel) {
    return assertValidLaunchAgentLabel(envLabel);
  }
  return assertValidLaunchAgentLabel(resolveGatewayLaunchAgentLabel(args?.env?.EVE_PROFILE));
}
...
function buildLaunchAgentEnvironmentFile(entries: Array<[string, string]>): string {
  return [
    "# Generated by EVE. Do not edit while the gateway service is installed.",
...
export function parseLaunchctlListEVEUpdateJobs(output: string): StaleEVEUpdateLaunchdJob[] {
  ...
  for (const rawLine of output.split(/\r?\n/)) {
    ...
    const label = normalizeEVEUpdateLaunchdLabel(labelParts.join(" "));
    if (!label) {
      continue;
    }
...
export async function findStaleEVEUpdateLaunchdJobs(
  env: NodeJS.ProcessEnv = process.env,
): Promise<StaleEVEUpdateLaunchdJob[]> {
  ...
  return parseLaunchctlListEVEUpdateJobs(result.stdout).filter(
    (job) => !isCurrentGatewayLaunchdLabel(job.label, env),
  );
}

export async function disableEVEUpdateLaunchdJob(label: string): Promise<boolean> {
  const normalizedLabel = normalizeEVEUpdateLaunchdLabel(label);
...
export async function disableCurrentEVEUpdateLaunchdJob(
  env: NodeJS.ProcessEnv = process.env,
): Promise<boolean> {
  const label = resolveCurrentEVEUpdateLaunchdJobLabel(env);
  if (!label) {
    return false;
  }
  return await disableEVEUpdateLaunchdJob(label);
}
...
async function resolveLaunchAgentGatewayPort(env: GatewayServiceEnv): Promise<number | null> {
  ...
  const fromServiceEnv = parseTcpPort(command?.environment?.EVE_GATEWAY_PORT ?? "");
  if (fromServiceEnv !== null) {
    return fromServiceEnv;
  }
  return parseTcpPort(env.EVE_GATEWAY_PORT ?? "");
}
...
export function formatLaunchAgentGuiSessionError(params: {
  ...
  return [
    ...
    "Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://docs.eve.ai/gateway",
  ].join("\n");
}
...
export async function stopLaunchAgent({
  ...
  if (!persistDisable) {
    ...
    // `eve gateway start` re-enables cleanly without a manual `launchctl enable`.
...
async function writeLaunchAgentPlist({
  ...
  const domain = resolveGuiDomain();
  const label = resolveLaunchAgentLabel({ env });
  for (const legacyLabel of resolveLegacyGatewayLaunchAgentLabels(env.EVE_PROFILE)) {
...
async function activateLaunchAgent(params: { env: GatewayServiceEnv; plistPath: string }) {
  ...
  await bootstrapLaunchAgentOrThrow({
    ...
    actionHint: "eve gateway install --force",
  });
}
...
export async function restartLaunchAgent({
  ...
  // `eve gateway restart` is an explicit operator request to bring the
  ...
  if (plistReloadNeeded) {
    ...
    await bootstrapLaunchAgentOrThrow({
      ...
      serviceTarget,
      plistPath,
      actionHint: "eve gateway restart",
    });
