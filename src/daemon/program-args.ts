...
export const EVE_WRAPPER_ENV_KEY = "EVE_WRAPPER";
...
export async function resolveEVEWrapperPath(
  ...
  try {
    ...
    throw new Error(
      `${EVE_WRAPPER_ENV_KEY} must point to an executable file: ${resolved}${detail}`,
      { cause: error },
    );
  }
  return resolved;
...
async function resolveCliProgramArguments(params: {
  ...
  const wrapperPath = await resolveEVEWrapperPath(params.wrapperPath);
  if (wrapperPath) {
