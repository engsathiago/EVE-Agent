...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `eve-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      EVE_LAUNCHD_LABEL: `ai.eve.launchd-int-${testId}`,
      EVE_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
