...
export function isCurrentProcessLaunchdServiceLabel(
  ...
  const configuredLabel = normalizeOptionalString(env.EVE_LAUNCHD_LABEL);
  if (!configuredLabel || configuredLabel !== label) {
    return false;
  }
  if (
    normalizeOptionalString(env.EVE_SERVICE_MARKER) === "eve" &&
    Boolean(normalizeOptionalString(env.EVE_SERVICE_KIND))
  ) {
