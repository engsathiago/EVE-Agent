...
import { AcpRuntimeError } from "@eve/acp-core/runtime/errors";
...
export function isAcpEnabledByPolicy(cfg: EVEConfig): boolean {
  return cfg.acp?.enabled !== false;
}

/** Resolves the effective dispatch policy state for inbound ACP routing. */
function resolveAcpDispatchPolicyState(cfg: EVEConfig): AcpDispatchPolicyState {
...
export function resolveAcpDispatchPolicyMessage(cfg: EVEConfig): string | null {
...
export function resolveAcpDispatchPolicyError(cfg: EVEConfig): AcpRuntimeError | null {
...
export function resolveAcpExplicitTurnPolicyError(cfg: EVEConfig): AcpRuntimeError | null {
  if (isAcpEnabledByPolicy(cfg)) {
    return null;
  }
...
function isAcpAgentAllowedByPolicy(cfg: EVEConfig, agentId: string): boolean {
