...
const logPathTracker = createSuiteLogPathTracker("eve-log-redaction-");
...
describe("file log redaction", () => {
  ...
  it("uses logging.file from the active config path", () => {
    ...
    withEnv({ EVE_CONFIG_PATH: configPath, EVE_TEST_FILE_LOG: "1" }, () => {
  ...
  it("expands leading tilde in logging.file", () => {
    ...
    withEnv({ HOME: home }, () => {
      expect(loggerTest.resolveActiveLogFile("~/custom-eve.log")).toBe(
        path.join(home, "custom-eve.log"),
      );
    });
  });
