...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "eve-log-tail-redaction-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("readConfiguredLogTail redaction", () => {
  it("redacts raw auth headers before returning log lines", async () => {
    ...
    const logFile = path.join(dir, "eve.log");
    const configFile = path.join(dir, "eve.json");
    ...
    expect(text).toContain("X-EVE-Token: supers…7890");
    ...
    expect(text).not.toContain(eveToken);
