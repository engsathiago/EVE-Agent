...
const logPathTracker = createSuiteLogPathTracker("eve-test-env-log-level-");

describe("EVE_LOG_LEVEL", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["EVE_LOG_LEVEL"]);
  ...
  it("warns once and ignores invalid env values", () => {
    ...
    process.env.EVE_LOG_LEVEL = "nope";
    ...
    const warnings = stderrSpy.mock.calls
      ...
      .filter((line) => line.includes("EVE_LOG_LEVEL"));
    expect(warnings).toHaveLength(1);
    expect(warnings[0]).toContain('Ignoring invalid EVE_LOG_LEVEL="nope"');
  });
});
