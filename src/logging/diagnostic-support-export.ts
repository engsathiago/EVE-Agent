...
const SUPPORT_EXPORT_PREFIX = "eve-diagnostics-";
...
function resolveBonjourEnvOverride(
  ...
  const raw = env.EVE_DISABLE_BONJOUR?.trim().toLowerCase();
  if (!raw) {
    return "unset";
  }
  switch (raw) {
...
function readStabilityBundle(
  ...
  if (target === false) {
    return { status: "missing", dir: "$EVE_STATE_DIR/logs/stability" };
  }
...
function renderSummary(params: {
  ...
  return [
    ...
    "- `logs/eve-sanitized.jsonl`: sanitized log summaries and metadata",
