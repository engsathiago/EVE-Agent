...
beforeEach(() => {
  envSnapshot = captureEnv(["EVE_TEST_FILE_LOG", "EVE_LOG_LEVEL"]);
  delete process.env.EVE_TEST_FILE_LOG;
...
describe("getResolvedLoggerSettings", () => {
  ...
  it("reads logging config when test file logging is explicitly enabled", () => {
    ...
    expect(settings.file).toBe("/tmp/eve-configured.log");
