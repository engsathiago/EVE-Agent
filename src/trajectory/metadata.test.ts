...
describe("trajectory metadata", () => {
  it("redacts harness argv and local paths with the support redaction rules", () => {
    ...
    process.argv = [
      "node",
      "/Users/tester/project/eve.js",
      "--api-key",
      "super-secret",
      "--config=/Users/tester/.eve/eve.json",
    ];
    try {
      const metadata = buildTrajectoryRunMetadata({
        env: {
          HOME: "/Users/tester",
          EVE_STATE_DIR: "/Users/tester/.eve",
        },
      ...
      expect(harness.invocation).toEqual([
        ...
        "--api-key",
        "<redacted>",
        "--config=$EVE_STATE_DIR/eve.json",
      ]);
      expect(harness.entrypoint).toBe("~/project/eve.js");
  ...
  it("redactPathForSupport returns empty string for null/undefined input", () => {
    const ctx: SupportRedactionContext = { env: {}, stateDir: "/tmp/.eve" };
