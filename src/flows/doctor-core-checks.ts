...
export type CoreHealthCheckDeps = {
  readonly detectUnavailableSkills: (cfg: EVEConfig) => Promise<readonly SkillStatusEntry[]>;
  readonly collectSecurityWarnings: (cfg: EVEConfig) => Promise<readonly string[]>;
...
async function collectSecurityWarningsWithRuntime(cfg: EVEConfig): Promise<readonly string[]> {
...
const gatewayConfigCheck: HealthCheck = {
  ...
  kind: "core",
  description: "eve.jsonc gateway block is set and unambiguous.",
  source: "doctor",
  async detect(ctx) {
    ...
    if (!ctx.cfg.gateway?.mode) {
      findings.push({
        ...
        path: "gateway.mode",
        fixHint:
          "Run `eve configure` and set Gateway mode (local/remote), or `eve config set gateway.mode local`.",
      });
    }
    if (ctx.cfg.gateway?.mode !== "remote" && hasAmbiguousGatewayAuthModeConfig(ctx.cfg)) {
      findings.push({
        ...
        fixHint:
          "Set an explicit mode: `eve config set gateway.auth.mode token` or `... password`.",
      });
    }
    return findings;
  },
    ...
    return [
      {
        ...
        fixHint:
          "Set commands.ownerAllowFrom to your channel user id, e.g. `eve config set commands.ownerAllowFrom '[\"telegram:123456789\"]'`.",
      },
    ];
  },
};

function resolveDoctorMode(cfg: EVEConfig): "local" | "remote" {
...
export function buildGatewayTokenSecretRefFixHint(ref: SecretRef): string {
  if (ref.source === "exec") {
    return "Run `eve doctor --allow-exec` to verify exec SecretRefs during doctor, or `eve secrets audit --allow-exec` to audit all exec SecretRefs.";
  }
    ...
    return [
      {
        ...
        path: "gateway.auth",
        fixHint: "Run `eve doctor --fix --generate-gateway-token` to generate a token.",
      },
    ];
  },
};
  ...
  async detect(ctx) {
    ...
    return detected.preview.map(
      (line): HealthFinding => ({
        ...
        fixHint: "Run `eve doctor --fix` to migrate legacy state.",
      }),
    );
  },
};
      ...
      (issue): HealthFinding => ({
        ...
        fixHint: issue.blockedOutsideEntry
          ? [
              ...
              "or set the affected OpenAI models to an EVE runtime policy.",
            ].join(" ")
          : [
              "Run `eve doctor --fix`: it enables plugins.entries.codex,",
              "or set the affected OpenAI models to an EVE runtime policy.",
            ].join(" "),
      }),
    );
    ...
    return [
      {
        ...
        path: workspaceDir,
        fixHint:
          "Inspect the legacy directories and migrate or remove them; see `eve doctor` for the detailed migration prompt.",
      },
    ];
  },
};
...
function unavailableSkillToFinding(skill: SkillStatusEntry): HealthFinding {
  return {
    ...
    fixHint:
      "Install/configure the missing requirement, or run `eve doctor --fix` to disable unused unavailable skills.",
  };
}
...
function browserResidueFinding(residue: LegacyClawdBrowserProfileResidue): HealthFinding {
  return {
    ...
    fixHint:
      "Run `eve doctor --fix` to archive the stale clawd profile safely instead of deleting it in place.",
  };
}
...
const browserClawdProfileResidueCheck: HealthCheck = {
  ...
  kind: "core",
  description:
    "Legacy clawd managed browser profile residue has been archived after the EVE rename.",
  source: "doctor",
  async detect(ctx, scope) {
...
const finalConfigValidationCheck: HealthCheck = {
  ...
  kind: "core",
  description: "Active eve.jsonc parses and conforms to the config schema.",
  source: "doctor",
  async detect() {
