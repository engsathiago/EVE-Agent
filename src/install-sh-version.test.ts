...
function resolveInstallerVersionCases(params: { stdinCwd: string }): string[] {
  const output = execFileSync(
    ...
    [
      "-c",
      `${versionHelperSource}
fake_eve_decorated() { printf '%s\\n' 'EVE 2026.3.10 (abcdef0)'; }
fake_eve_raw() { printf '%s\\n' "EVE dev's build"; }
EVE_BIN=fake_eve_decorated resolve_eve_version
EVE_BIN=fake_eve_raw resolve_eve_version
(
  cd "$1"
  source /dev/stdin <<'EVE_STDIN_INSTALLER'
${versionHelperSource}
fake_eve_stdin() { printf '%s\\n' 'EVE 2026.3.10 (abcdef0)'; }
EVE_BIN=fake_eve_stdin
resolve_eve_version
EVE_STDIN_INSTALLER
)`,
      "eve-version-test",
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const hostileCwd = makeTempDir(tempRoots, "eve-install-stdin-");
...
`,
      ...
      ).toEqual(["2026.3.10", "EVE dev's build", "2026.3.10"]);
    },
  );
});
