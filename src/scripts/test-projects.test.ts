...
describe("test-projects args", () => {
  ...
  it("routes boundary targets to the boundary config", () => {
    expect(buildVitestRunPlans(["src/infra/eve-root.test.ts"])).toEqual([
      {
        config: "test/vitest/vitest.boundary.config.ts",
        forwardedArgs: [],
        includePatterns: ["src/infra/eve-root.test.ts"],
        watchMode: false,
      },
    ]);
  });
  ...
  it("keeps conservative local full-suite runs on leaf project configs", () => {
    const originalVitestMaxWorkers = process.env.EVE_VITEST_MAX_WORKERS;
    const originalTestWorkers = process.env.EVE_TEST_WORKERS;
    const originalProjectParallel = process.env.EVE_TEST_PROJECTS_PARALLEL;
    const originalLeafShards = process.env.EVE_TEST_PROJECTS_LEAF_SHARDS;
    ...
    try {
      ...
      if (originalVitestMaxWorkers === undefined) {
        ...
        process.env.EVE_VITEST_MAX_WORKERS = originalVitestMaxWorkers;
      }
      if (originalTestWorkers === undefined) {
        delete process.env.EVE_TEST_WORKERS;
      } else {
        process.env.EVE_TEST_WORKERS = originalTestWorkers;
      }
      if (originalProjectParallel === undefined) {
        delete process.env.EVE_TEST_PROJECTS_PARALLEL;
      } else {
        process.env.EVE_TEST_PROJECTS_PARALLEL = originalProjectParallel;
      }
      if (originalLeafShards === undefined) {
        delete process.env.EVE_TEST_PROJECTS_LEAF_SHARDS;
      } else {
        process.env.EVE_TEST_PROJECTS_LEAF_SHARDS = originalLeafShards;
      }
  ...
  it("uses a bounded local default for full-suite project parallelism", () => {
    expect(
      resolveParallelFullSuiteConcurrency(
        58,
        {
          EVE_TEST_PROJECTS_LEAF_SHARDS: "1",
        },
        {
          cpuCount: 8,
          loadAverage1m: 0,
  ...
  it("gives parallel Vitest shards separate filesystem module caches", () => {
    ...
    expect(firstEnv?.EVE_VITEST_FS_MODULE_CACHE_PATH).toBe(
      "/repo/node_modules/.experimental-vitest-cache/0-test-vitest-vitest.gateway.config.ts",
    );
    expect(specs[1]?.env.EVE_VITEST_FS_MODULE_CACHE_PATH).toBe(
  ...
  it("preserves explicit Vitest filesystem module cache paths", () => {
    ...
    expect(
      applyParallelVitestCachePaths(specs, {
        cwd: "/repo",
        env: {
          EVE_VITEST_FS_MODULE_CACHE_PATH: "/tmp/cache",
        },
      }),
    ).toBe(specs);
  });
  ...
  it("writes scoped include files for routed extension runs", () => {
    ...
    expect(spec?.includeFilePath).toContain("eve-vitest-include-");
    expect(spec?.env.EVE_VITEST_INCLUDE_FILE).toBe(spec?.includeFilePath);
  });

  it("rejects explicit test file targets that do not exist", () => {
    expect(findUnmatchedExplicitTestTargets(["src/not-a-real-eve-test.test.ts"])).toEqual([
      {
        target: "src/not-a-real-eve-test.test.ts",
  ...
  it("skips channel contract configs with no matching external include patterns", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "eve-contract-include-"));
    try {
