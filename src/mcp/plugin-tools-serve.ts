/**
 * Standalone MCP server that exposes EVE plugin-registered tools
...
function resolvePluginToolPolicy(config: EVEConfig): {
...
function resolveTools(config: EVEConfig): AnyAgentTool[] {
...
export function createPluginToolsMcpServer(
  ...
  return createToolsMcpServer({ name: "eve-plugin-tools", tools });
}
