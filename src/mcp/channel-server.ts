...
/**
 * MCP stdio server assembly for EVE channel conversations.
 *
...
async function resolveMcpConfig(config: EVEConfig | undefined): Promise<EVEConfig> {
  if (config) {
    return config;
  }
...
export async function createEVEChannelMcpServer(opts: EVEMcpServeOptions = {}): Promise<{
  server: McpServer;
...
export async function serveEVEChannelMcp(opts: EVEMcpServeOptions = {}): Promise<void> {
  const { server, start, close } = await createEVEChannelMcpServer(opts);
