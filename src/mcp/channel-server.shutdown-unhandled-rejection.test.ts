...
vi.mock("./channel-bridge.js", () => ({
  EVEChannelBridge: class MockEVEChannelBridge {
...
describe("serveEVEChannelMcp shutdown", () => {
  ...
  it("does not leak unhandled rejections when shutdown close fails", async () => {
    ...
    const { serveEVEChannelMcp } = await import("./channel-server.js");

    const servePromise = serveEVEChannelMcp({ verbose: false });
