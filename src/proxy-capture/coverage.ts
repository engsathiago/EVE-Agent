...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[eve proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[eve proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`eve proxy coverage\` for details.`,
  );
}
