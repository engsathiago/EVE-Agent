/** Builds the interactive `eve secrets configure` target list and apply plan. */
...
/** Credential target shown by `eve secrets configure` before a SecretRef is selected. */
...
function getSecretProviders(config: EVEConfig): Record<string, SecretProviderConfig> {
...
/** Builds configure candidates for the current EVE config only. */
export function buildConfigureCandidates(config: EVEConfig): ConfigureCandidate[] {
...
/** Builds configure candidates for EVE config plus an optional auth-profile scope. */
export function buildConfigureCandidatesForScope(params: {
  config: EVEConfig;
  authoredEVEConfig?: EVEConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredEVEConfig ?? params.config;

  const hasPathInAuthoredConfig = (pathSegments: string[]): boolean =>
    hasPath(authoredConfig, pathSegments);

  const eveCandidates = discoverConfigSecretTargets(params.config)
  ...
  return [...eveCandidates, ...authCandidates].toSorted((a, b) =>
...
export function buildSecretsConfigurePlan(params: {
  ...
  return {
    ...
    generatedBy: "eve secrets configure",
