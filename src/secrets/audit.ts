...
import { resolveStateDir, type EVEConfig } from "../config/config.js";
...
/** Stable finding codes emitted by `eve secrets audit`. */
...
export async function runSecretsAudit(
  ...
  const config = snapshot.valid ? snapshot.config : ({} as EVEConfig);
  let resolution = {
    refsChecked: 0,
    skippedExecRefs: 0,
