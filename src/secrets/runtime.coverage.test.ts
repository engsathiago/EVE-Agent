...
function createCoverageWebSearchProvider(params: {
  ...
  const readConfiguredCredential = (config?: EVEConfig): unknown =>
...
type SecretRegistryEntry = {
  id: string;
  configFile: "eve.json" | "auth-profiles.json";
  pathPattern: string;
...
const DEBUG_COVERAGE_BATCHES = process.env.EVE_DEBUG_RUNTIME_COVERAGE === "1";
...
const PLUGIN_OWNED_EVE_COVERAGE_EXCLUSIONS = new Set([
...
const previousTrustBundledPluginsDir = process.env.EVE_TEST_TRUST_BUNDLED_PLUGINS_DIR;

process.env.EVE_BUNDLED_PLUGINS_DIR ??= "extensions";
...
afterAll(() => {
  ...
  if (previousTrustBundledPluginsDir === undefined) {
    ...
    process.env.EVE_TEST_TRUST_BUNDLED_PLUGINS_DIR = previousTrustBundledPluginsDir;
  }
});
...
function addCoveragePluginLoadPath(config: EVEConfig, pluginId: string): void {
...
function collectEVECoverageEntries(options: {
  includePluginEntries: boolean;
}): SecretRegistryEntry[] {
  return COVERAGE_REGISTRY_ENTRIES.filter(
    (entry) =>
      entry.configFile === "eve.json" &&
      entry.id.startsWith("plugins.entries.") === options.includePluginEntries &&
      !PLUGIN_OWNED_EVE_COVERAGE_EXCLUSIONS.has(entry.id),
  );
}

function applyConfigForEVETarget(
  config: EVEConfig,
  entry: SecretRegistryEntry,
  envId: string,
  wildcardToken: string,
...
async function expectEVECoverageBatchResolved(
  label: string,
  ...
  for (const [index, entry] of batch.entries()) {
    const envId = toCoverageEnvRefId("EVE_SECRET_TARGET", entry.id);
    ...
    applyConfigForEVETarget(config, entry, envId, wildcardToken);
  }
...
const EVE_CORE_COVERAGE_BATCHES = buildCoverageBatches(
  collectEVECoverageEntries({ includePluginEntries: false }),
);
const EVE_PLUGIN_COVERAGE_BATCHES = buildCoverageBatches(
  collectEVECoverageEntries({ includePluginEntries: true }),
);
...
describe("secrets runtime target coverage", () => {
  beforeAll(async () => {
    ...
    const googleChatBatch = EVE_CORE_COVERAGE_BATCHES.find((batch) =>
      batch.some((entry) => entry.id === "channels.googlechat.serviceAccount"),
    );
    if (googleChatBatch) {
      await expectEVECoverageBatchResolved("eve.json core", googleChatBatch);
    }
  });

  describe("eve.json core and channel registry targets", () => {
    test.each(EVE_CORE_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      "handles $name",
      async ({ batch }) => {
        await expectEVECoverageBatchResolved("eve.json core", batch);
      },
      RUNTIME_COVERAGE_TEST_TIMEOUT_MS,
    );
  });

  describe("eve.json plugin registry targets", () => {
    test.each(EVE_PLUGIN_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      "handles $name",
      async ({ batch }) => {
        await expectEVECoverageBatchResolved("eve.json plugins", batch);
      },
      RUNTIME_COVERAGE_TEST_TIMEOUT_MS,
    );
  });
  ...
  describe("auth-profiles registry targets", () => {
    test.each(AUTH_PROFILE_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      ...
      async ({ batch }) => {
        ...
        for (const [index, entry] of batch.entries()) {
          const envId = toCoverageEnvRefId("EVE_AUTH_SECRET_TARGET", entry.id);
