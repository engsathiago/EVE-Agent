...
import { isRecord as isJsonObject } from "@eve/normalization-core/record-coerce";
...
export function listAuthProfileStoreAgentDirs(config: EVEConfig, stateDir: string): string[] {
...
function resolveActiveAgentDir(stateDir: string, env: NodeJS.ProcessEnv = process.env): string {
  const override = env.EVE_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim();
  if (override) {
