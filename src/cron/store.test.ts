...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "eve-cron-store-"));
});
...
describe("resolveCronStorePath", () => {
  ...
  it("uses EVE_HOME for tilde expansion", () => {
    vi.stubEnv("EVE_HOME", "/srv/eve-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/eve-home", "cron", "jobs.json"));
  });
});
