...
function resolveDefaultNodeInstallStateDir({
  ...
  if (platform !== "win32" && getuid?.() === 0) {
    return path.join(path.parse(cwd).root, "var", "lib", "eve");
  }
...
export async function installSkill(params: SkillInstallRequest): Promise<SkillInstallResult> {
  ...
  const scanResult = await evaluateSkillInstallPolicy({
    ...
    source:
      skillSource === "eve-bundled"
        ? { kind: "bundled", authority: "eve", mutable: false, network: false }
        : skillSource === "eve-managed" || skillSource === "eve-extra"
          ? { kind: "managed", authority: "eve", mutable: false, network: false }
  ...
  const trustedInstallSources = new Set(["eve-bundled", "eve-managed", "eve-extra"]);
