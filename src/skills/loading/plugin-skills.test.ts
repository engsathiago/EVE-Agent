    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "eve.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "eve.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "eve.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("eve-");
  const acpxRoot = await tempDirs.make("eve-acpx-plugin-");
  const helperRoot = await tempDirs.make("eve-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("eve-");
  const pluginRoot = await tempDirs.make("eve-plugin-");
  const outsideDir = await tempDirs.make("eve-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("eve-");
    const pluginRoot = await tempDirs.make("eve-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("eve-");
    const pluginRoot = await tempDirs.make("eve-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });
