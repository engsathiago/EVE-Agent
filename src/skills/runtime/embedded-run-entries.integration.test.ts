...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("eve-bundled-");
  const workspaceDir = await createTempDir("eve-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: EVEConfig) {
