...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("eve status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("eve security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set EVE_GATEWAY_TOKEN/EVE_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("eve security audit --deep")}".`,
    });
  }
  return findings;
}
