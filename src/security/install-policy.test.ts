...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "eve-install-policy-"));
  tempDirs.push(dir);
  return dir;
}
...
function configWithPolicy(scriptPath: string, env: Record<string, string>): EVEConfig {
  return {
    security: {
      installPolicy: {
        enabled: true,
...
describe("runInstallPolicy", () => {
  ...
  it("executes policy for skills when targets are omitted", async () => {
    ...
    expect(captured.eveVersion).toEqual(expect.any(String));
