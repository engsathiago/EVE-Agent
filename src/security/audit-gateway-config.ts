...
type CollectDangerousConfigFlags = (cfg: EVEConfig) => string[];
...
export function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.EVE_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.EVE_GATEWAY_PASSWORD);
