...
describe("security audit gateway config findings", () => {
  it("evaluates gateway auth presence and rate-limit guardrails", async () => {
    await Promise.all([
      ...
      (async () => {
        ...
        const resolvedConfig: EVEConfig = {
          gateway: {
            bind: "lan",
            auth: {},
          },
  ...
  it("warns when EVE_GATEWAY_TOKEN shadows a different configured token source", () => {
    const cfg: EVEConfig = {
  ...
  it("does not warn when gateway.auth.token resolves from EVE_GATEWAY_TOKEN", () => {
    const cfg: EVEConfig = {
