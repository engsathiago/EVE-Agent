// EVE state database tests cover state DB migrations and persistence.
...
import { resolveEVEStateSqlitePath } from "./eve-state-db.paths.js";
import {
  collectSqliteSchemaShape,
  createSqliteSchemaShapeFromSql,
} from "./sqlite-schema-shape.test-support.js";

type StateDbTestDatabase = Pick<EVEStateKyselyDatabase, "diagnostic_events" | "schema_meta">;

function createTempStateDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "eve-state-db-"));
}
...
describe("eve state database", () => {
  it("resolves under the shared state database directory", () => {
    const stateDir = createTempStateDir();

    expect(resolveEVEStateSqlitePath({ EVE_STATE_DIR: stateDir })).toBe(
      path.join(stateDir, "state", "eve.sqlite"),
    );
  });
  ...
  it("creates the shared state schema from the committed SQL shape", () => {
    ...
    expect(collectSqliteSchemaShape(database.db)).toEqual(
      createSqliteSchemaShapeFromSql(new URL("./eve-state-schema.sql", import.meta.url)),
    );
    expect(database.path).toBe(path.join(stateDir, "state", "eve.sqlite"));
  });
  ...
  it("migrates requester and executor attribution for existing cross-agent tasks", () => {
    ...
    const reopened = openEVEStateDatabase({
      env: { EVE_STATE_DIR: stateDir },
    });
    ...
    const currentReopened = openEVEStateDatabase({
      env: { EVE_STATE_DIR: stateDir },
    });
    expect(
      currentReopened.db
  ...
  it("does not chmod shared parent directories for explicit database paths", () => {
    const databasePath = path.join(
      os.tmpdir(),
      `eve-explicit-state-${process.pid}-${Date.now()}.sqlite`,
    );

    expect(() => openEVEStateDatabase({ path: databasePath })).not.toThrow();
  ...
  it("keeps cached handles open when another state path is opened", () => {
    ...
    const first = openEVEStateDatabase({ path: firstPath });
    const second = openEVEStateDatabase({ path: secondPath });

    expect(first.db.isOpen).toBe(true);
    expect(second.db.isOpen).toBe(true);
    expect(openEVEStateDatabase({ path: firstPath })).toBe(first);
    expect(readSqliteNumberPragma(first.db, "user_version")).toBe(1);
  });

  it("keys explicit relative paths by resolved database pathname", () => {
    const moduleUrl = new URL("./eve-state-db.ts", import.meta.url).href;
    const output = execFileSync(
      ...
      [
        ...
        `
          ...
          const root = fs.mkdtempSync(path.join(os.tmpdir(), "eve-state-db-relative-"));
          ...
          try {
            ...
            const first = openEVEStateDatabase({ path: "state.sqlite" });
            first.db
            ...
            const second = openEVEStateDatabase({ path: "state.sqlite" });
            second.db
              .prepare("INSERT INTO diagnostic_events (scope, event_key, payload_json, created_at) VALUES (?, ?, ?, ?)")
  ...
  it("uses savepoints for nested write transaction rollback", () => {
    const stateDir = createTempStateDir();
    const options = { env: { EVE_STATE_DIR: stateDir } };

    runEVEStateWriteTransaction((database) => {
      const stateDb = getNodeSqliteKysely<StateDbTestDatabase>(database.db);
      executeSqliteQuerySync(
        database.db,
      ...
      expect(() =>
        runEVEStateWriteTransaction((inner) => {
  ...
  it("rejects Promise-returning write transactions", () => {
    ...
    expect(() =>
      runEVEStateWriteTransaction(async () => {
        return "not sync";
      }, options),
