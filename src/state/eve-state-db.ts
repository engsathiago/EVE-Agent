// EVE state database manages shared persisted state and migrations.
...
/**
 * Shared EVE SQLite state database lifecycle and metadata writers.
 *
 ...
 */
const EVE_STATE_SCHEMA_VERSION = 1;
/** Shared timeout used by state and agent SQLite handles before surfacing busy errors. */
export const EVE_SQLITE_BUSY_TIMEOUT_MS = 30_000;
const EVE_STATE_DIR_MODE = 0o700;
const EVE_STATE_FILE_MODE = 0o600;

/** Open shared SQLite database handle plus WAL maintenance lifecycle. */
export type EVEStateDatabase = {
  db: DatabaseSync;
  path: string;
...
export type EVEStateDatabaseOptions = {
  env?: NodeJS.ProcessEnv;
  path?: string;
};

export type EVEStateDatabaseSchemaMigration = {
  kind: "agent-databases-composite-primary-key";
  path: string;
};

const cachedDatabases = new Map<string, EVEStateDatabase>();

type EVEStateMetadataDatabase = Pick<EVEStateKyselyDatabase, "schema_meta">;
...
function assertSupportedSchemaVersion(db: DatabaseSync, pathname: string): void {
  ...
  if (userVersion > EVE_STATE_SCHEMA_VERSION) {
    throw new Error(
      `EVE state database ${pathname} uses newer schema version ${userVersion}; this EVE build supports ${EVE_STATE_SCHEMA_VERSION}.`,
    );
  }
}
...
function ensureEVEStatePermissions(pathname: string, env: NodeJS.ProcessEnv): void {
  const dir = path.dirname(pathname);
  const defaultDir = resolveEVEStateSqliteDir(env);
  const isDefaultStateDatabase =
    path.resolve(pathname) === path.resolve(resolveEVEStateSqlitePath(env));
  if (isDefaultStateDatabase && dir !== defaultDir) {
    throw new Error(`EVE state database path resolved outside its state dir: ${pathname}`);
  }
  const dirExisted = existsSync(dir);
  mkdirSync(dir, { recursive: true, mode: EVE_STATE_DIR_MODE });
  ...
  if (isDefaultStateDatabase || !dirExisted) {
    bestEffortChmodSync(dir, EVE_STATE_DIR_MODE);
  }
  for (const candidate of resolveSqliteDatabaseFilePaths(pathname)) {
    if (existsSync(candidate)) {
      bestEffortChmodSync(candidate, EVE_STATE_FILE_MODE);
    }
  }
}
...
function assertCanonicalStateSchemaShape(db: DatabaseSync, pathname: string): void {
  ...
  throw new Error(
    `EVE state database ${pathname} has a legacy agent database registry schema; run eve doctor --fix to migrate it.`,
  );
}

export function detectEVEStateDatabaseSchemaMigrations(
  options: EVEStateDatabaseOptions = {},
): EVEStateDatabaseSchemaMigration[] {
...
export function repairEVEStateDatabaseSchema(options: EVEStateDatabaseOptions = {}): {
  changes: string[];
  warnings: string[];
} {
  ...
  ensureEVEStatePermissions(pathname, env);
  const sqlite = requireNodeSqlite();
  const db = new sqlite.DatabaseSync(pathname);
  db.exec(`PRAGMA busy_timeout = ${EVE_SQLITE_BUSY_TIMEOUT_MS};`);
  try {
...
function ensureSchema(db: DatabaseSync, pathname: string): void {
  ...
  db.exec(EVE_STATE_SCHEMA_SQL);
  ensureAdditiveStateColumns(db);
  db.exec(`PRAGMA user_version = ${EVE_STATE_SCHEMA_VERSION};`);
  const now = Date.now();
  const kysely = getNodeSqliteKysely<EVEStateMetadataDatabase>(db);
  executeSqliteQuerySync(
    db,
    kysely
      ...
      .values({
        meta_key: "primary",
        role: "global",
        schema_version: EVE_STATE_SCHEMA_VERSION,
        agent_id: null,
        app_version: null,
        created_at: now,
        updated_at: now,
...
function resolveDatabasePath(options: EVEStateDatabaseOptions = {}): string {
  return path.resolve(options.path ?? resolveEVEStateSqlitePath(options.env ?? process.env));
}
...
export function runEVEStateWriteTransaction<T>(
  operation: (database: EVEStateDatabase) => T,
  options: EVEStateDatabaseOptions = {},
): T {
  const database = openEVEStateDatabase(options);
  const result = runSqliteImmediateTransactionSync(database.db, () => operation(database));
  try {
    ensureEVEStatePermissions(database.path, options.env ?? process.env);
  } catch {
...
export function closeEVEStateDatabase(): void {
...
export function isEVEStateDatabaseOpen(): boolean {
  return Array.from(cachedDatabases.values()).some((database) => database.db.isOpen);
}

/** Test alias for closing shared state handles from teardown code. */
export const closeEVEStateDatabaseForTest = closeEVEStateDatabase;
