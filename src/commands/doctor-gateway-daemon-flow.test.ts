...
vi.mock("../daemon/constants.js", () => ({
  resolveGatewayLaunchAgentLabel: vi.fn(() => "ai.eve.gateway"),
  resolveNodeLaunchAgentLabel: vi.fn(() => "ai.eve.node"),
}));
...
describe("maybeRepairGatewayDaemon", () => {
  ...
  it("reports recent restart handoffs during deep doctor", async () => {
    ...
    service.readCommand.mockResolvedValueOnce({
      ...
      environment: {
        EVE_STATE_DIR: "/tmp/eve-service",
        EVE_CONFIG_PATH: "/tmp/eve-service/eve.json",
      },
    });
    ...
    const [handoffEnv] = readGatewayRestartHandoffSync.mock.calls[0] as unknown as [
      { EVE_STATE_DIR?: string; EVE_CONFIG_PATH?: string },
    ];
    expect(handoffEnv?.EVE_STATE_DIR).toBe("/tmp/eve-service");
    expect(handoffEnv?.EVE_CONFIG_PATH).toBe("/tmp/eve-service/eve.json");
    expect(note).toHaveBeenCalledWith(
  ...
  it("suppresses busy-port note for expected Gateway listeners", async () => {
    setPlatform("linux");
    const listeners = [{ pid: 5001, commandLine: "eve-gateway", address: "0.0.0.0:18789" }];
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners,
  ...
  it("keeps busy-port note for unexpected Gateway listeners", async () => {
    ...
    inspectPortUsage.mockResolvedValue({
      ...
      status: "busy",
      listeners: [
        { pid: 5001, commandLine: "eve-gateway", address: "0.0.0.0:18789" },
        { pid: 5002, commandLine: "eve-gateway", address: "127.0.0.1:18789" },
      ],
  ...
  it("skips gateway install during non-interactive doctor repairs", async () => {
    ...
    expect(service.restart).not.toHaveBeenCalled();
    expect(note).toHaveBeenCalledWith(
      `Run ${formatCliCommand("eve gateway install")} when you want to install the gateway service.`,
      "Gateway",
    );
  });
  ...
  it("skips gateway service install when a system EVE gateway service exists", async () => {
    setPlatform("linux");
    service.isLoaded.mockResolvedValue(false);
    findSystemGatewayServices.mockResolvedValue([
      {
        platform: "linux",
        label: "eve-gateway.service",
        detail: "unit: /etc/systemd/system/eve-gateway.service",
        scope: "system",
        marker: "eve",
        legacy: false,
      },
    ...
    expect(note).toHaveBeenCalledWith(
      [
        "System-level EVE gateway service detected while the user gateway service is not installed.",
        "- eve-gateway.service (unit: /etc/systemd/system/eve-gateway.service)",
        "EVE will not install a second user-level gateway service automatically.",
        "Run `eve gateway status --deep` or `eve doctor --deep` to inspect duplicate services.",
