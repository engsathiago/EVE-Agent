...
function isLocalTuiCommand(command: string): boolean {
  ...
  if (executable === "eve-tui") {
    return true;
  }
  return executable === "eve" && LOCAL_TUI_SUBCOMMANDS.has(argv[1] ?? "");
}
...
/** Lists local EVE TUI processes that can contend with gateway responsiveness. */
...
function hasWhatsappEnabled(cfg: EVEConfig): boolean {
...
export async function noteWhatsappResponsivenessHealth(params: {
  ...
  if (gatewayDegraded && tuiProcesses.length > 0) {
    ...
    if (params.shouldRepair) {
      ...
      warnings.push(
        `Fix: close those TUI sessions, or run ${formatCliCommand("eve doctor --fix")}.`,
      );
    }
  }
