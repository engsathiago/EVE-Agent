...
async function makeTestState(seed = "legacy-oauth-seed"): Promise<EVETestState> {
  const state = await createEVETestState({
    layout: "state-only",
    prefix: "eve-doctor-oauth-sidecar-",
    env: {
      EVE_AGENT_DIR: undefined,
      EVE_AUTH_PROFILE_SECRET_KEY: seed,
    },
  });
  states.push(state);
  return state;
...
function encryptLegacySidecarMaterial(params: {
  ref: { source: "eve-credentials"; provider: "openai-codex"; id: string };
  profileId: string;
  provider: string;
  seed: string;
...
describe("maybeRepairLegacyOAuthSidecarProfiles", () => {
  ...
  it("scans EVE_AGENT_DIR before treating sidecars as unreferenced", async () => {
    const state = await makeTestState();
    const previousAgentDir = process.env.EVE_AGENT_DIR;
    ...
    try {
      ...
      if (previousAgentDir === undefined) {
        ...
        process.env.EVE_AGENT_DIR = previousAgentDir;
      }
    }
  });
