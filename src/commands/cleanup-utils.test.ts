...
describe("buildCleanupPlan", () => {
  test("resolves inside-state flags and workspace dirs", () => {
    ...
    const defaultWorkspace = path.join(tmpRoot, "eve-workspace-default");
    const opsWorkspace = path.join(tmpRoot, "eve-workspace-ops");
    const cfg = {
      agents: {
    ...
    const plan = buildCleanupPlan({
      cfg: cfg as unknown as EVEConfig,
      stateDir: path.join(tmpRoot, "eve-state"),
      configPath: path.join(tmpRoot, "eve-state", "eve.json"),
      oauthDir: path.join(tmpRoot, "eve-oauth"),
    });
  ...
  test("includes implicit per-agent workspaces under the state dir", () => {
    const tmpRoot = path.join(path.parse(process.cwd()).root, "tmp", "eve-cleanup-plan");
...
describe("cleanup path removals", () => {
  ...
  it("removes state and only linked paths outside state", async () => {
    ...
    const tmpRoot = path.join(path.parse(process.cwd()).root, "tmp", "eve-cleanup");
    await removeStateAndLinkedPaths(
      {
        stateDir: path.join(tmpRoot, "state"),
        configPath: path.join(tmpRoot, "state", "eve.json"),
    ...
    expect(runtime.log.mock.calls.map(([line]) => line.replaceAll("\\", "/"))).toEqual([
      "[dry-run] remove /tmp/eve-cleanup/state",
      "[dry-run] remove /tmp/eve-cleanup/oauth",
    ]);
  });

  it("preserves nested workspace paths during state-only removal", async () => {
    const runtime = createRuntimeMock();
    const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "eve-cleanup-"));
    const stateDir = path.join(tmpRoot, ".eve");
  ...
  it("removes every workspace directory", async () => {
    const runtime = createRuntimeMock();
    const workspaces = ["/tmp/eve-workspace-1", "/tmp/eve-workspace-2"];

    await removeWorkspaceDirs(workspaces, runtime, { dryRun: true });

    const logs = runtime.log.mock.calls.map(([line]) => line);
    expect(logs).toEqual([
      "[dry-run] remove /tmp/eve-workspace-1",
      "[dry-run] remove /tmp/eve-workspace-2",
    ]);
  });

  it("removes owned legacy workspace attestations", async () => {
    const runtime = createRuntimeMock();
    const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "eve-cleanup-attest-"));
  ...
  it("refuses to remove a directory containing the current working directory", async () => {
    const runtime = createRuntimeMock();
    const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "eve-cleanup-cwd-"));
