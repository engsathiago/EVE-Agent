...
function isOpenAICompatibleMemoryProvider(providerId: string, cfg: EVEConfig): boolean {
...
function isKeyOptionalMemoryProvider(providerId: string, cfg: EVEConfig): boolean {
  return (
    providerId === "local" ||
    providerId === "ollama" ||
    providerId === "lmstudio" ||
...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("eve doctor --fix")} or ${formatCliCommand("eve memory status --fix")}`
    : `Verify: ${formatCliCommand("eve memory status --deep")}`;
  return [
...
export async function noteMemoryRecallHealth(cfg: EVEConfig): Promise<void> {
  try {
...
function hasActiveAlternateMemoryPluginSlot(cfg: EVEConfig): boolean {
 ...
 * Runs as part of `eve doctor` — config-only checks where possible;
...
export async function noteMemorySearchHealth(
      ...
      note(
        [
          ...
          workspaceProbeFailed
            ...
            : `- Set an explicit binary path: ${formatCliCommand("eve config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("eve config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("eve memory status --deep")}`,
        ]
          .filter(Boolean)
  ...
  if (provider === "local") {
    ...
    note(
      [
        ...
        "",
        "Fix (pick one):",
        `- Install the llama.cpp provider plugin: ${formatCliCommand("eve plugins install @eve/llama-cpp-provider")}`,
        `- Set a local GGUF model path in config`,
        suggestedRemoteProvider
          ? `- Switch to a remote provider: ${formatCliCommand(`eve config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
  ...
  if (
    ...
    note(
      [
        ...
        "",
        "Fix:",
        `- ${formatCliCommand("eve config set agents.defaults.memorySearch.remote.baseUrl http://127.0.0.1:1234/v1")}`,
        "",
        `Verify: ${formatCliCommand("eve memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  if (isOpenAICompatibleMemoryProvider(provider, cfg) && !normalizeOptionalString(resolved.model)) {
    note(
      [
        ...
        "",
        "Fix:",
        `- ${formatCliCommand("eve config set agents.defaults.memorySearch.model text-embedding-bge-m3")}`,
        "",
        `Verify: ${formatCliCommand("eve memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  if (isKeyOptionalMemoryProvider(provider, cfg)) {
    ...
    // not warn. A skipped probe means the user ran `eve doctor` without
  ...
  note(
    [
      ...
      `- Configure credentials: ${formatCliCommand("eve configure --section model")}`,
      `- To disable: ${formatCliCommand("eve config set agents.defaults.memorySearch.enabled false")}`,
      "",
      `Verify: ${formatCliCommand("eve memory status --deep")}`,
    ].join("\n"),
    "Memory search",
