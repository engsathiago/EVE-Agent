...
type ConfigSnapshotForWrite = {
  snapshot: { valid: boolean; resolved: EVEConfig };
...
const loadConfigMock = vi.hoisted(() => vi.fn<() => EVEConfig>());
...
const setRuntimeConfigSnapshotMock = vi.hoisted(() =>
  vi.fn<(cfg: EVEConfig, sourceConfig: EVEConfig) => void>(),
);
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "eve-agent-" });
}
...
function mockConfig(home: string, storePath: string): EVEConfig {
  const cfg = {
    agents: {
      defaults: {
