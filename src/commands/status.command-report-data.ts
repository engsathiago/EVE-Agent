// Builds the data model for the standard `eve status` text report.
...
export async function buildStatusCommandReportData(
  ...
  const securityAuditLines = params.securityAudit
    ...
    : [
        params.theme.muted(
          `Skipped in fast status. Full report: ${params.formatCliCommand("eve security audit")}`,
        ),
        params.theme.muted(`Deep probe: ${params.formatCliCommand("eve status --deep")}`),
      ];
  ...
  return {
    ...
    taskMaintenanceHint: `Task maintenance: ${params.formatCliCommand("eve tasks maintenance --apply")}`,
