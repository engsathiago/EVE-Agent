...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/eve-agent"),
    resolveAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/eve-agent/workspace"),
    ...
    resolveAuthStorePathForDisplay: vi.fn(
      (agentDir?: string) => `${agentDir ?? "/tmp/eve-agent"}/auth-profiles.json`,
    ),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/eve-dev/eve.json",
    }),
...
vi.mock("../../agents/workspace.js", () => ({
  resolveDefaultAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/eve-agent/workspace"),
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/eve-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  ...
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.configPath).toBe("/tmp/eve-dev/eve.json");
    expect(payload.auth.storePath).toBe("/tmp/eve-agent/auth-profiles.json");
  ...
  it("honors EVE_AGENT_DIR when no --agent override is provided", async () => {
    const localRuntime = createRuntime();
    mocks.resolveAgentDir.mockClear();
    await withEnvAsync({ EVE_AGENT_DIR: "/tmp/eve-isolated-agent" }, async () => {
      await modelsStatusCommand({ json: true }, localRuntime as never);
    });

    expect(mocks.resolveAgentDir).not.toHaveBeenCalled();
    expect(mocks.ensureAuthProfileStore).toHaveBeenCalledWith("/tmp/eve-isolated-agent");
    const payload = parseFirstJsonLog(localRuntime);
    expect(payload.agentDir).toBe("/tmp/eve-isolated-agent");
    expect(payload.auth.storePath).toBe("/tmp/eve-isolated-agent/auth-profiles.json");
  });

  it("honors deprecated PI_CODING_AGENT_DIR when EVE_AGENT_DIR is unset", async () => {
    const localRuntime = createRuntime();
    mocks.resolveAgentDir.mockClear();
    await withEnvAsync(
      {
    ...
    expect(mocks.resolveAgentDir).not.toHaveBeenCalled();
    expect(mocks.ensureAuthProfileStore).toHaveBeenCalledWith("/tmp/eve-legacy-agent");
    const payload = parseFirstJsonLog(localRuntime);
    expect(payload.agentDir).toBe("/tmp/eve-legacy-agent");
  });
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    await withAgentScopeOverrides(
      ...
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/eve-agent-custom");
        ...
        expect(openAiCodex?.effective).toEqual({
          kind: "profiles",
          detail: "/tmp/eve-agent-custom/auth-profiles.json",
        });
      },
    );
  });
  ...
  it("reports Gemini CLI OAuth for canonical Google text routed through the CLI runtime", async () => {
    ...
    try {
      ...
      expect(
        ...
        kind: "profiles",
        detail: "/tmp/eve-agent/auth-profiles.json",
      });
      expect(payload.auth.runtimeAuthRoutes).toEqual([
        {
          provider: "google",
  ...
  it("includes auth-evidence-only providers in the auth overview", async () => {
    ...
    mocks.resolveEnvApiKey.mockImplementation(
      (provider: string, _env?: NodeJS.ProcessEnv, options?: { workspaceDir?: string }) =>
        provider === "workspace-cloud" && options?.workspaceDir === "/tmp/eve-agent/workspace"
          ? {
