...
describe("resolveDaemonEVEBinDir", () => {
  it("uses the active eve command directory", () => {
    expect(
      resolveDaemonEVEBinDir({
        argv: ["node", "/Users/testuser/.npm-global/bin/eve", "gateway", "install"],
        env: { PATH: "" },
        platform: "darwin",
      }),
  ...
  it("finds the PATH shim that resolves to the active package entrypoint", () => {
    ...
    expect(
      resolveDaemonEVEBinDir({
        ...
        platform: "darwin",
        existsSync: (candidate) => candidate === "/Users/testuser/.npm-global/bin/eve",
        realpathSync: (candidate) => realpaths.get(candidate) ?? candidate,
      }),
    ).toEqual(["/Users/testuser/.npm-global/bin"]);
  });

  it("ignores unrelated eve commands elsewhere on PATH", () => {
    expect(
      resolveDaemonEVEBinDir({
        argv: ["node", "/opt/eve/eve.mjs", "gateway", "install"],
...
describe("resolveDaemonServicePathDirs", () => {
  it("combines node and active eve command directories", () => {
    expect(
      resolveDaemonServicePathDirs({
