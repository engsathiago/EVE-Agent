...
export function summarizeExistingConfig(config: EVEConfig): string {
  const rows: string[] = [];
...
function summarizeGatewayConfig(config: EVEConfig): string | null {
...
export function printWizardHeader(runtime: RuntimeEnv) {
  ...
  const title = supportsDecorativeEmoji() && icon ? `${icon} EVE ${icon}` : "EVE";
...
export function formatControlUiSshHint(params: {
  ...
  return [
    ...
    "Docs:",
    "https://docs.eve.ai/gateway/remote",
    "https://docs.eve.ai/web/control-ui",
  ]
    .filter(Boolean)
    .join("\n");
}
