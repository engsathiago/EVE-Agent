...
const resolveEVEPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  ...
  loadEVEPlugins: () => createEmptyPluginRegistry(),
...
vi.mock("eve-agent/plugin-sdk/provider-model-shared", () => ({
  DEFAULT_CONTEXT_TOKENS: 32768,
  normalizeProviderId: (value: string) => normalizeLowercaseStringOrEmpty(value),
}));

vi.mock("eve-agent/plugin-sdk/provider-stream-shared", () => ({
...
beforeEach(() => {
  ...
  resolveEVEPackageRoot.mockReset().mockResolvedValue(null);
  ...
  originalUpdateInProgress = process.env.EVE_UPDATE_IN_PROGRESS;
  process.env.EVE_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "eve-doctor-state-"));
