...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/eve-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = EVEConfig>(
  overrides: T = {} as T,
): EVEConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): EVEConfig {
