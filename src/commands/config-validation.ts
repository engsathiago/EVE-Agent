...
export async function requireValidConfigFileSnapshot(
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    runtime.error(`EVE config is invalid: ${snapshot.path}\n${issues}`);
    runtime.error(
    ...
    runtime.error(`Inspect: ${formatCliCommand("eve config validate")}`);
    runtime.exit(1);
    return null;
  }
...
/** Read and return a valid EVE config, or null after reporting validation errors. */
