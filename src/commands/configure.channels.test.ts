...
describe("removeChannelConfigWizard", () => {
  ...
  it("lists configured channels from eve.json even when no plugins are loaded", async () => {
  ...
  it("deletes the selected channel block from eve.json", async () => {
  ...
  it("deletes a real channel block named done", async () => {
    ...
    expect(confirmArg().message).toBe("Delete done configuration from ~/.eve/eve.json?");
