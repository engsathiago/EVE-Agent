...
function noteCliGatewayVersionSkew(status: StatusSummary | undefined): void {
  ...
  note(
    [
      `This command is EVE ${VERSION}; the running Gateway is EVE ${gatewayVersion}.`,
      "Check `eve --version`, `which eve`, and `eve gateway status --deep`.",
      "If this mismatch is unexpected, update PATH so `eve` points to the version you want, or reinstall the Gateway service from that same EVE install.",
    ].join("\n"),
    "EVE version mismatch",
  );
}
