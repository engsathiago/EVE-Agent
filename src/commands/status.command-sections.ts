...
export function buildStatusFooterLines(params: {
  ...
  return [
    "FAQ: https://docs.eve.ai/faq",
    "Troubleshooting: https://docs.eve.ai/troubleshooting",
    ...(params.updateHint ? ["", params.warn(params.updateHint)] : []),
    "Next steps:",
    `  Need to share?      ${params.formatCliCommand("eve status --all")}`,
    `  Need to debug live? ${params.formatCliCommand("eve logs --follow")}`,
    params.nodeOnlyGateway
      ? `  Need node service?  ${params.formatCliCommand("eve node status")}`
      : params.gatewayReachable
        ? `  Need to test channels? ${params.formatCliCommand("eve status --deep")}`
        : `  Fix reachability first: ${params.formatCliCommand("eve gateway probe")}`,
  ];
}
...
export function buildStatusPairingRecoveryLines(params: {
  ...
  return [
    ...
    ...(params.pairingRecovery.requestId
      ? [
          params.muted(
            `Recovery: ${params.formatCliCommand(`eve devices approve ${params.pairingRecovery.requestId}`)}`,
          ),
        ]
      : []),
    params.muted(`Fallback: ${params.formatCliCommand("eve devices approve --latest")}`),
    params.muted(`Inspect: ${params.formatCliCommand("eve devices list")}`),
  ];
}
