...
describe("channels list", () => {
  ...
  it("default output shows configured official external channels when the plugin is missing", async () => {
    ...
    mocks.resolveMissingOfficialExternalChannelPluginRepairHint.mockReturnValue({
      ...
      channelId: "discord",
      label: "Discord",
      installSpec: "@eve/discord",
      installCommand: "eve plugins install @eve/discord",
      doctorFixCommand: "eve doctor --fix",
      repairHint:
        "Install the official external plugin with: eve plugins install @eve/discord, or run: eve doctor --fix.",
    });
    mocks.readConfigFileSnapshot.mockResolvedValue({
      ...baseConfigSnapshot,
      config: {
