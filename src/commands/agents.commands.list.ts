// Implements `eve agents list` text and JSON summaries.
...
export async function agentsListCommand(
  ...
  lines.push(
    `Channel status reflects local config/creds. For live health: ${formatCliCommand("eve channels status --probe")}.`,
  );
