...
import { normalizeNullableString as normalizeId } from "@eve/normalization-core/string-coerce";
...
import { compareEVEVersions } from "../../../config/version.js";
import {
...
function isPluginsGloballyDisabled(cfg: EVEConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function isDenied(cfg: EVEConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function collectBlockedPluginIds(cfg: EVEConfig): string[] {
...
function isPluginEntryDisabled(cfg: EVEConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function isChannelDisabled(cfg: EVEConfig, channelId: string): boolean {
  const channels = asObjectRecord(cfg.channels);
  const entry = asObjectRecord(channels?.[channelId]);
  return entry?.enabled === false;
}

function isDisabled(cfg: EVEConfig, pluginId: string): boolean {
...
function collectMaterialPluginEntryIds(cfg: EVEConfig): string[] {
  const entries = asObjectRecord(cfg.plugins?.entries);
  if (!entries) {
    return [];
  }
...
function collectSlotPluginIds(cfg: EVEConfig): string[] {
...
function collectConfiguredChannelIds(cfg: EVEConfig, env: NodeJS.ProcessEnv): string[] {
  const ids = new Set<string>();
  const channels = asObjectRecord(cfg.channels);
  if (channels) {
...
function collectWebSearchPluginIds(cfg: EVEConfig): string[] {
...
function collectEnvWebSearchPluginIds(cfg: EVEConfig, env: NodeJS.ProcessEnv): string[] {
  if (cfg.tools?.web?.search?.enabled === false) {
    return [];
  }
...
function collectWebFetchPluginIds(cfg: EVEConfig): string[] {
...
function collectEnvWebFetchPluginIds(cfg: EVEConfig, env: NodeJS.ProcessEnv): string[] {
...
function collectSpeechPluginIds(cfg: EVEConfig): string[] {
  return resolveOfficialExternalProviderContractPluginIds({
...
function collectAcpRuntimePluginIds(cfg: EVEConfig): string[] {
...
function collectAllowOnlyOfficialPluginIds(cfg: EVEConfig): string[] {
...
function addEligiblePluginId(cfg: EVEConfig, pluginIds: Set<string>, pluginId: string): void {
...
export function shouldRunConfiguredPluginInstallReleaseStep(params: {
  ...
  const currentComparedToRelease = compareEVEVersions(
  ...
  const touchedComparedToRelease = compareEVEVersions(params.touchedVersion, releaseVersion);
