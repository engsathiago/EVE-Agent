...
import { bundledDistPluginRootAt, bundledPluginRootAt } from "eve-agent/plugin-sdk/test-fixtures";
...
describe("bundled plugin load path repair", () => {
  ...
  it("formats a doctor hint for legacy bundled plugin paths", () => {
    ...
    expect(warnings).toEqual([
      `- plugins.load.paths: bundled plugin path "${legacyPath}" still aliases feishu; EVE loads the packaged bundled plugin from "${bundledPath}".`,
      '- Run "eve doctor --fix" to remove these redundant bundled plugin paths.',
    ]);
  });

  it("ignores bundled plugins that already resolve to source extensions", () => {
    const sourcePath = path.resolve("repo", "eve", "extensions", "feishu");
    vi.spyOn(bundledSources, "resolveBundledPluginSources").mockReturnValue(
