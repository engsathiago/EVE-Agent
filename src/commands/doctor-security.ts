...
import type { EVEConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: EVEConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: EVEConfig): string[] {
  const warnings: string[] = [];
  const approvals = loadExecApprovals();
  const defaultRequestedSecuritySource = "EVE default (full)";
  const defaultRequestedAskSource = "EVE default (off)";
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("eve approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: EVEConfig): string[] {
  void cfg;
  return [];
}

function collectExecFilesystemPolicyWarnings(cfg: EVEConfig): string[] {
...
function collectPlaintextConfigSecretWarnings(cfg: EVEConfig): string[] {
  ...
  return [
    "- WARNING: eve.json contains plaintext secret-bearing config fields.",
    `  Paths: ${pathLine}`,
    "  Agents or workspace tools that can read config files may see these API keys/tokens.",
    `  Migrate them to SecretRefs with ${formatCliCommand("eve secrets configure")} or ${formatCliCommand("eve secrets apply")}, then verify with ${formatCliCommand("eve secrets audit --check")}.`,
  ];
}
...
export async function collectSecurityWarnings(
  ...
  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      ...
      `  Check local policy with: ${formatCliCommand("eve approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("eve configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("eve config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("eve doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("eve config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('eve config set session.dmScope "per-channel-peer"') +
...
export async function noteSecurityWarnings(cfg: EVEConfig) {
  const warnings = await collectSecurityWarnings(cfg);
  const auditHint = `- Run: ${formatCliCommand("eve security audit --deep")}`;
