...
vi.mock("../daemon/program-args.js", () => ({
  EVE_WRAPPER_ENV_KEY: "EVE_WRAPPER",
  resolveGatewayProgramArguments: mocks.resolveGatewayProgramArguments,
  resolveEVEWrapperPath: mocks.resolveEVEWrapperPath,
}));
...
function mockNodeGatewayPlanFixture(
  ...
  const {
    ...
    supported = true,
    warning,
    serviceEnvironment = { EVE_PORT: "3000" },
  } = params;
  const workingDirectory = Object.hasOwn(params, "workingDirectory")
    ? params.workingDirectory
    : "/Users/me";
  mocks.resolvePreferredNodePath.mockResolvedValue("/opt/node");
  mocks.resolveEVEWrapperPath.mockImplementation(async (value: string | undefined) =>
...
describe("buildGatewayInstallPlan", () => {
  // Prevent tests from reading the developer's real ~/.eve/.env when
  ...
  it("uses provided nodePath and returns plan", async () => {
    ...
    expect(plan.environment).toEqual({ EVE_PORT: "3000" });
  ...
  it("adds the active eve command bin directory to the managed service PATH", async () => {
    mockNodeGatewayPlanFixture();
    const originalArgv = process.argv;
    const eveBinPath = path.join(isolatedHome, ".npm-global", "bin", "eve");
    process.argv = ["node", eveBinPath, "gateway", "install"];
    ...
    ).toStrictEqual(["/opt/homebrew/opt/node/bin", path.dirname(eveBinPath)]);
  });
  ...
  it("uses the state dir as the default macOS launchd working directory", async () => {
    ...
    expect(plan.workingDirectory).toBe(path.join(isolatedHome, ".eve"));
    expect(mocks.buildServiceEnvironment).toHaveBeenCalledOnce();
  ...
  it("passes EVE_WRAPPER through program args and managed service env", async () => {
    const wrapperPath = path.resolve("/usr/local/bin/eve-doppler");
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        EVE_PORT: "3000",
    ...
    expect(mocks.buildServiceEnvironment).toHaveBeenCalledOnce();
    expect(
      firstMockArg(mocks.buildServiceEnvironment, "buildServiceEnvironment").env?.EVE_WRAPPER,
    ).toBe(wrapperPath);
    expect(plan.environment.EVE_WRAPPER).toBe(wrapperPath);
  });

  it("clears a Windows wrapper env that points at the generated gateway.cmd script", async () => {
    const selfWrapperPath = path.join(isolatedHome, ".eve", "gateway.cmd");
    const warn = vi.fn();
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        EVE_PORT: "3000",
  ...
  it("tracks safe config env keys without embedding literal values", async () => {
    ...
    expect(plan.environment.EVE_PORT).toBe("3000");
  ...
  it("includes env SecretRef values from config into the service environment", async () => {
    ...
    expect(plan.environment.EVE_SERVICE_MANAGED_ENV_KEYS).toBeUndefined();
  });
  ...
  it("allows safe inherited passEnv names while blocking dangerous exec SecretRef env", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        BASH_ENV: "/tmp/eve-test-bashenv",
        XDG_CONFIG_HOME: "/tmp/eve-test-xdg-home",
  ...
  it("does not embed gateway auth SecretRef values into the service environment", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        EVE_GATEWAY_TOKEN: "gateway-test-token",
      }),
      port: 3000,
      runtime: "node",
      config: {
        gateway: {
          auth: {
            token: { source: "env", provider: "default", id: "EVE_GATEWAY_TOKEN" },
          },
        },
      },
    });

    expect(plan.environment.EVE_GATEWAY_TOKEN).toBeUndefined();
    expect(plan.environment.EVE_SERVICE_MANAGED_ENV_KEYS).toBeUndefined();
  });
  ...
  it("does not inline config env SecretRef values already backed by state-dir dotenv", async () => {
    await writeStateDirDotEnv("DISCORD_BOT_TOKEN=discord-dotenv-token\n", {
      stateDir: path.join(isolatedHome, ".eve"),
    });
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        EVE_PORT: "3000",
    ...
    expect(plan.environment.EVE_SERVICE_MANAGED_ENV_KEYS).toBe("DISCORD_BOT_TOKEN");
  });
...
describe("buildGatewayInstallPlan — dotenv merge", () => {
  ...
  it("retains .env values for macOS LaunchAgent env SecretRefs", async () => {
    ...
    expect(plan.environment.EVE_SERVICE_MANAGED_ENV_KEYS).toBe("MINIMAX_API_KEY");
  });
  ...
  it("works when .env file does not exist", async () => {
    mockNodeGatewayPlanFixture({ serviceEnvironment: { EVE_PORT: "3000" } });
  ...
  it("preserves safe custom vars from an existing service env and merges PATH", async () => {
    ...
    expect(plan.environment.EVE_SERVICE_MARKER).toBeUndefined();
  });
  ...
  it("drops keys that were previously tracked as managed service env", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        BLOGWATCHER_HOME: "/Users/test/.blogwatcher",
        GOPATH: "/Users/test/.local/gopath",
        EVE_SERVICE_MANAGED_ENV_KEYS: "GOBIN,GOPATH",
      },
    });
  ...
  it("drops legacy inline env values when the key is now managed by .env", async () => {
    ...
    expect(plan.environment.EVE_SERVICE_MANAGED_ENV_KEYS).toBe("TAVILY_API_KEY");
  ...
  it("keeps source metadata for EnvironmentFile-backed preserved vars", async () => {
    ...
    expect(plan.environment.EVE_GATEWAY_TOKEN).toBeUndefined();
    expect(plan.environmentValueSources?.EVE_GATEWAY_TOKEN).toBeUndefined();
  });
  ...
  it("does not embed auth-profile env refs when the key is already durable", async () => {
    ...
    expect(plan.environment.EVE_SERVICE_MANAGED_ENV_KEYS).toBe("OPENAI_API_KEY");
  });
});
...
describe("gatewayInstallErrorHint", () => {
  it("returns platform-specific hints", () => {
    ...
    expect(gatewayInstallErrorHint("linux")).toMatch(
      /(?:eve|eve)( --profile isolated)? gateway install/,
    );
  });
});
...
describe("collectPreservedExistingServiceEnvVars — operator opt-in allowlist", () => {
  ...
  it("preserves EVE_CLI_CONTAINER_BYPASS and EVE_CONTAINER_HINT", () => {
    const result = collectPreservedExistingServiceEnvVars(
      {
        EVE_CLI_CONTAINER_BYPASS: "1",
        EVE_CONTAINER_HINT: "ci",
      },
      managedKeys,
    );
    expect(result.EVE_CLI_CONTAINER_BYPASS).toBe("1");
    expect(result.EVE_CONTAINER_HINT).toBe("ci");
  });

  it("still drops arbitrary EVE_FOO", () => {
    const result = collectPreservedExistingServiceEnvVars({ EVE_FOO: "bar" }, managedKeys);
    expect(result.EVE_FOO).toBeUndefined();
  });

  it("preserves container opt-ins while dropping unrelated EVE_* keys", () => {
    const result = collectPreservedExistingServiceEnvVars(
      {
        EVE_CLI_CONTAINER_BYPASS: "1",
        EVE_CONTAINER_HINT: "ci",
    ...
    expect(result.EVE_CLI_CONTAINER_BYPASS).toBe("1");
    expect(result.EVE_CONTAINER_HINT).toBe("ci");
    expect(result.EVE_BAZ).toBeUndefined();
  });
});
