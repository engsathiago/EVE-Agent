...
function expectEVESchemaInvalidPreservesField(params: {
  config: unknown;
  ...
  const res = EVESchema.safeParse(params.config);
...
describe("legacy config detection", () => {
  ...
  it("rejects removed legacy provider sections", () => {
    expectEVESchemaInvalidPreservesField({
  ...
  it("preserves claude-cli auth profile mode during validation", () => {
    ...
    const res = EVESchema.safeParse(config);
