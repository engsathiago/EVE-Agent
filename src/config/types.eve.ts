// Defines the top-level EVE configuration type.
...
/** Top-level EVE config as read from user/project config files. */
export type EVEConfig = {
  /** JSON schema URL used by editors and generated config files. */
  $schema?: string;
  meta?: {
    /** Last EVE version that wrote this config. */
  ...
  wizard?: {
    ...
    lastRunAt?: string;
    /** EVE version used by the last completed wizard run. */
  ...
  ui?: {
    /** Accent color for EVE UI chrome (hex). */
    seamColor?: string;
    assistant?: {
...
export type EVEConfigInput = Omit<EVEConfig, "models"> & {
  models?: ModelsConfigInput;
};

declare const eveConfigStateBrand: unique symbol;

type BrandedConfigState<TState extends string> = EVEConfig & {
  readonly [eveConfigStateBrand]?: TState;
};
