...
type EVEMcpHttpTransport = "sse" | "streamable-http";

const CLI_MCP_TYPE_TO_EVE_TRANSPORT: Record<string, EVEMcpHttpTransport | "stdio"> = {
...
/** Checks whether a raw MCP `type` value is a legacy CLI alias EVE can rewrite. */
export function isKnownCliMcpTypeAlias(value: unknown): boolean {
  return Object.hasOwn(CLI_MCP_TYPE_TO_EVE_TRANSPORT, normalizeMcpString(value));
}
...
export function canonicalizeConfiguredMcpServer(
  ...
  const next = { ...server };
  const transportAlias = resolveEVEMcpTransportAlias(next.type);
  // `transport` is EVE's canonical field; legacy `type` only fills a gap.
