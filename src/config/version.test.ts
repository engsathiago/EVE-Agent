...
import {
  compareEVEVersions,
  isSameEVEStableFamily,
  parseEVEVersion,
  shouldWarnOnTouchedVersion,
} from "./version.js";

describe("parseEVEVersion", () => {
  it("parses stable, correction, and beta forms", () => {
    expect(parseEVEVersion("2026.3.23")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseEVEVersion("2026.3.23-1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: 1,
    ...
    expect(parseEVEVersion("2026.3.23-beta.1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseEVEVersion("v2026.3.23.beta.2")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
  ...
  it("rejects invalid versions", () => {
    expect(parseEVEVersion("2026.3")).toBeNull();
    expect(parseEVEVersion("latest")).toBeNull();
  });
});

describe("compareEVEVersions", () => {
  it("treats correction publishes as newer than the base stable release", () => {
    expect(compareEVEVersions("2026.3.23", "2026.3.23-1")).toBe(-1);
    expect(compareEVEVersions("2026.3.23-1", "2026.3.23")).toBe(1);
    expect(compareEVEVersions("2026.3.23-2", "2026.3.23-1")).toBe(1);
  });

  it("treats stable as newer than beta and compares beta identifiers", () => {
    expect(compareEVEVersions("2026.3.23", "2026.3.23-beta.1")).toBe(1);
    expect(compareEVEVersions("2026.3.23-beta.2", "2026.3.23-beta.1")).toBe(1);
    expect(compareEVEVersions("2026.3.23.beta.1", "2026.3.23-beta.2")).toBe(-1);
  });
});

describe("isSameEVEStableFamily", () => {
  it("treats same-base stable and correction versions as one family", () => {
    expect(isSameEVEStableFamily("2026.3.23", "2026.3.23-1")).toBe(true);
    expect(isSameEVEStableFamily("2026.3.23-1", "2026.3.23-2")).toBe(true);
    expect(isSameEVEStableFamily("2026.3.23", "2026.3.24")).toBe(false);
    expect(isSameEVEStableFamily("2026.3.23-beta.1", "2026.3.23")).toBe(false);
  });
});
