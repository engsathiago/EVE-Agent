...
import { isSensitiveUrlConfigPath } from "@eve/net-policy/redact-sensitive-url";
...
describe("mapSensitivePaths", () => {
  ...
  it("main schema yields correct hints (samples)", () => {
    const schema = EVESchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "EVEConfig";
    const hints = mapSensitivePaths(EVESchema, "", {});
...
describe("collectMatchingSchemaPaths", () => {
  it("finds base-config URL fields that may embed secrets", () => {
    const paths = collectMatchingSchemaPaths(EVESchema, "", isSensitiveUrlConfigPath);
