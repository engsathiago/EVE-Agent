...
import { resetConfigRuntimeState, type EVEConfig } from "./config.js";
  ...
  try {
    return await withTempHomeBase(fn, {
      ...
      env: {
        ...
        EVE_MPM_CATALOG_PATHS: undefined,
...
export async function writeEVEConfig(home: string, config: unknown): Promise<string> {
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.EVE_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected EVE_STATE_DIR or explicit stateDir for .env test setup");
  }
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeEVEConfig(home, config);
