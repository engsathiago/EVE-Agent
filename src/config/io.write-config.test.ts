...
import type { ConfigFileSnapshot, EVEConfig } from "./types.eve.js";
...
describe("config io write", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "eve-config-io-" });
  const silentLogger = {
    warn: () => {},
    error: () => {},
  };
  ...
  const createFastConfigIO = (home: string) =>
    createConfigIO({
      env: { EVE_TEST_FAST: "1" } as NodeJS.ProcessEnv,
      homedir: () => home,
      logger: silentLogger,
    });
  ...
  it("loads shipped plugin install config records without mutating config or plugin index", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".eve", "eve.json");
      const pluginDir = path.join(home, ".eve", "plugins", "demo");
  ...
  it("retains included shipped plugin install records in write snapshots", async () => {
    await withSuiteHome(async (home) => {
      const configDir = path.join(home, ".eve");
  ...
  it("migrates shipped plugin install config records during explicit writes even when the manifest is missing", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".eve", "eve.json");
      const pluginDir = path.join(home, ".eve", "plugins", "missing");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        configPath,
        `${JSON.stringify(
  ...
  it("keeps shipped plugin install config records when index migration fails", async () => {
    ...
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".eve", "eve.json");
      const unwritableStatePath = path.join(home, ".eve");
  ...
  it("keeps writes inside an EVE_STATE_DIR override even when the real home config exists", async () => {
    await withSuiteHome(async (home) => {
      const liveConfigPath = path.join(home, ".eve", "eve.json");
      ...
      const env = { EVE_STATE_DIR: overrideDir } as NodeJS.ProcessEnv;
      const io = createConfigIO({
        env,
        homedir: () => home,
        logger: silentLogger,
      ...
      expect(io.configPath).toBe(path.join(overrideDir, "eve.json"));
      ...
      const overridePersisted = JSON.parse(
        await fs.readFile(path.join(overrideDir, "eve.json"), "utf-8"),
      ) as {
  ...
  it("prints missing-meta write anomalies when anomaly logging is requested", async () => {
    await withSuiteHome(async (home) => {
      ...
      const io = createConfigIO({
        env: {
          EVE_CONFIG_WRITE_ANOMALY_LOG: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: {
          warn,
  ...
  it("suppresses overwrite audit output when skipOutputLogs is set", async () => {
    await withSuiteHome(async (home) => {
      ...
      const io = createConfigIO({
        env: {
          VITEST: "true",
          EVE_TEST_CONFIG_OVERWRITE_LOG: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: {
          warn,
  ...
  it("rejects destructive internal writes before replacing the config", async () => {
    await withSuiteHome(async (home) => {
      ...
      const original = {
        ...
        agents: { list: [{ id: "main", default: true, workspace: "/tmp/eve-main" }] },
  ...
  it("keeps authored agent provider params during narrowed internal agent writes", async () => {
    await withSuiteHome(async (home) => {
      ...
      const persisted = JSON.parse(await fs.readFile(configPath, "utf-8")) as EVEConfig;
  ...
  it("returns the read-time environment snapshot for invalid config repairs", async () => {
    await withSuiteHome(async (home) => {
      ...
      const io = createConfigIO({
        env: {
          EVE_GATEWAY_TOKEN: "gateway-token-at-read",
          EVE_TEST_FAST: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: silentLogger,
      ...
      expect(result.writeOptions.envSnapshotForRestore?.EVE_GATEWAY_TOKEN).toBe(
  ...
  it("returns the snapshot-time hash when an included file is malformed", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".eve", "eve.json");
      const includePath = path.join(home, ".eve", "plugins.json5");
  ...
  it("returns a write guard that rejects a changed active config path", async () => {
    await withSuiteHome(async (home) => {
      const firstConfigPath = path.join(home, ".eve", "first.json");
      const secondConfigPath = path.join(home, ".eve", "second.json");
      ...
      const env = {
        EVE_CONFIG_PATH: firstConfigPath,
        EVE_TEST_FAST: "1",
      } as NodeJS.ProcessEnv;
      const io = createConfigIO({ env, homedir: () => home, logger: silentLogger });

      const result = await io.readConfigFileSnapshotForWrite();
      env.EVE_CONFIG_PATH = secondConfigPath;
  ...
  it("rejects local write ownership when config env changes path selection during the read", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".eve", "eve.json");
      const configuredNextPath = path.join(home, ".eve", "next.json");
      const sourceConfig = {
        env: { EVE_CONFIG_PATH: configuredNextPath },
        gateway: { mode: "local" },
      } satisfies EVEConfig;
      await fs.mkdir(path.dirname(configPath), { recursive: true });
  ...
  it("does not use expectedConfigPath as the write destination", async () => {
    await withSuiteHome(async (home) => {
      const expectedConfigPath = path.join(home, ".eve", "expected.json");
      const activeConfigPath = path.join(home, ".eve", "active.json");
      ...
      await withEnvAsync(
        {
          EVE_CONFIG_PATH: activeConfigPath,
          EVE_TEST_FAST: "1",
        },
        async () => {
          await writeConfigFile(
  ...
  it("rejects root-include partial writes instead of flattening the root config", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".eve", "eve.json");
      const includePath = path.join(home, ".eve", "extra.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
        `${JSON.stringify({ $schema: "https://eve.ai/config-from-include.json" }, null, 2)}\n`,
        "utf-8",
      );
      await fs.writeFile(
        configPath,
  ...
  it("rejects invalid include-backed repairs instead of persisting substituted secrets", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".eve", "eve.json");
      const includePath = path.join(home, ".eve", "gateway.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
        `${JSON.stringify(
      ...
      const io = createConfigIO({
        env: {
          EVE_GATEWAY_TOKEN: "gateway-token-runtime",
          EVE_TEST_FAST: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: silentLogger,
  ...
  it("repairs invalid root-authored siblings without flattening included config", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".eve", "eve.json");
      const includePath = path.join(home, ".eve", "agent-defaults.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
  ...
  it("rejects repairs that would flatten a valid outer include with a broken nested include", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".eve", "eve.json");
      const pluginsPath = path.join(home, ".eve", "plugins.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        pluginsPath,
  ...
  it("preserves escaped root literals before validating unrelated includes", async () => {
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/eve-test-literal-plugin",
          source: "/tmp/eve-test-literal-plugin/index.ts",
          manifestPath: "/tmp/eve-test-literal-plugin/eve.plugin.json",
          configSchema: {
            type: "object",
            properties: {
    ...
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".eve", "eve.json");
      const agentsPath = path.join(home, ".eve", "agents.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        agentsPath,
  ...
  it("repairs invalid config without flattening array-nested includes", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".eve", "eve.json");
      const includePath = path.join(home, ".eve", "main-agent.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
        `${JSON.stringify({ id: "main", workspace: "${EVE_AGENT_WORKSPACE}" }, null, 2)}\n`,
        "utf-8",
      );
      await fs.writeFile(
        configPath,
      ...
      const io = createConfigIO({
        env: {
          EVE_AGENT_WORKSPACE: "/resolved/agent-workspace",
          EVE_TEST_FAST: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: silentLogger,
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/eve-test-required-plugin",
          source: "/tmp/eve-test-required-plugin/index.ts",
          manifestPath: "/tmp/eve-test-required-plugin/eve.plugin.json",
          configSchema: {
            type: "object",
            properties: {
              token: { type: "string" },
  ...
  it("writes runtime-derived edits back to source SecretRef markers", async () => {
    await withSuiteHome(async (home) => {
      ...
      await withEnvAsync({ EVE_CONFIG_PATH: configPath }, async () => {
        setRuntimeConfigSnapshot(
          {
            gateway: { mode: "local" },
            models: {
      ...
      try {
        await withEnvAsync(
          ...
          async () => {
            ...
            expect(persisted.gateway?.auth?.token).toBe("${EVE_GATEWAY_TOKEN}");
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/eve-test-demo",
          source: "/tmp/eve-test-demo/index.ts",
          manifestPath: "/tmp/eve-test-demo/eve.plugin.json",
          configSchema: {
            type: "object",
            properties: {
      ...
      try {
        await withEnvAsync({ EVE_CONFIG_PATH: configPath }, async () => {
          ...
          const postWriteSnapshot = await createConfigIO({
            env: { EVE_CONFIG_PATH: configPath, VITEST: "true" } as NodeJS.ProcessEnv,
            homedir: () => home,
            logger: silentLogger,
  ...
  it("rolls back the root config when post-write runtime refresh fails", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".eve", "eve.json");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies EVEConfig;
  ...
  it("blocks runtime preflight failures before committing root writes", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".eve", "eve.json");
      const initialRaw = `${JSON.stringify({ gateway: { mode: "local" } }, null, 2)}\n`;
      let observedSource: EVEConfig | undefined;
  ...
  it("restores config env vars when post-write runtime refresh rollback succeeds", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".eve", "eve.json");
      const envKey = "EVE_TEST_RUNTIME_ROLLBACK_ENV";
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies EVEConfig;
      const initialRaw = `${JSON.stringify(initialConfig, null, 2)}\n`;
  ...
  it("rolls back root writes when canonical reread changes config path ownership", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".eve", "eve.json");
      const nextConfigPath = path.join(home, ".eve", "next.json");
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies EVEConfig;
      const initialRaw = `${JSON.stringify(initialConfig, null, 2)}\n`;
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(configPath, initialRaw, "utf-8");
        ...
        async () => {
          ...
          await expect(
            writeConfigFile(
              {
                gateway: { mode: "local", port: 19001 },
                env: { EVE_CONFIG_PATH: nextConfigPath },
              },
              {
                baseSnapshot: prepared.snapshot,
                ...prepared.writeOptions,
  ...
  it("uses injected filesystem operations when rolling back ownership loss", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".eve", "eve.json");
      const otherConfigPath = path.join(home, ".eve", "other.json");
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies EVEConfig;
      const initialRaw = `${JSON.stringify(initialConfig, null, 2)}\n`;
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(configPath, initialRaw, "utf-8");
        ...
        promises: {
          ...
          rename: async (from, to) => {
            ...
            if (!committed && to === configPath) {
              committed = true;
              env.EVE_CONFIG_PATH = otherConfigPath;
            }
          },
        },
      } as typeof fsNode;
    ...
    await withSuiteHome(async (home) => {
        ...
        plugins: [
          {
            ...
            skills: [],
            hooks: [],
            rootDir: "/tmp/eve-test-strict-plugin",
            source: "/tmp/eve-test-strict-plugin/index.ts",
            manifestPath: "/tmp/eve-test-strict-plugin/eve.plugin.json",
            configSchema: {
              type: "object",
      ...
      try {
        ...
        await withEnvAsync({ EVE_CONFIG_PATH: configPath }, async () => {
          ...
          await expect(
            writeConfigFile({ agents: { list: "not-array" } } as unknown as EVEConfig, {
  ...
  it("preserves authored tilde paths when runtime-shaped writes hand back absolute paths", async () => {
    await withSuiteHome(async (home) => {
      ...
      await fs.writeFile(
        ...
        `${JSON.stringify(
          {
            logging: { file: "~/eve-upgrade-survivor/gateway.jsonl" },
          },
          null,
          2,
        )}\n`,
      ...
      await io.writeConfigFile(
        {
          logging: {
            file: path.join(home, "eve-upgrade-survivor", "gateway.jsonl"),
            level: "debug",
          },
        },
        { baseSnapshot: snapshot },
      ...
      const persisted = JSON.parse(await fs.readFile(configPath, "utf-8")) as EVEConfig;
      expect(persisted.logging?.file).toBe("~/eve-upgrade-survivor/gateway.jsonl");
