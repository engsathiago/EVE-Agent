  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.eve/logs/eve.log" },
        hooks: {
          path: "~/.eve/hooks.json5",
        ...
        channels: {
          ...
          whatsapp: {
            accounts: {
              personal: {
                authDir: "~/.eve/credentials/wa-personal",
              },
            },
          },
          imessage: {
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".eve", "logs", "eve.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".eve", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".eve", "telegram.token"),
      );
      expect(cfg.channels?.whatsapp?.accounts?.personal?.authDir).toBe(
        path.join(home, ".eve", "credentials", "wa-personal"),
      );
      ...
      expect(cfg.agents?.list?.[0]?.agentDir).toBe(path.join(home, ".eve", "agents", "main"));
