...
export function isUnresolvedShellReference(value: string): boolean {
  ...
  //   $VAR_NAME          (simple reference, EVE env-var style)
...
type ParsedStateDirDotEnv = {
   ...
   * reference ($VAR, ${VAR}, or $(cmd)). These are still EVE-managed keys:
 ...
 * literal references for keys EVE previously managed.
 */
...
/**
 * Read and parse `~/.eve/.env` (or `$EVE_STATE_DIR/.env`), returning
