...
/**
 * Nix mode detection: When EVE_NIX_MODE=1, the gateway is running under Nix.
 * In this mode:
...
const CONFIG_FILENAME = "eve.json";
...
/** Build a homedir thunk that respects EVE_HOME for the given env. */
 ...
 * Can be overridden via EVE_STATE_DIR.
 * Default: ~/.eve
 */
...
export function normalizeStateDirEnv(env: NodeJS.ProcessEnv = process.env): void {
  ...
  const eveOverride = env.EVE_STATE_DIR?.trim();
  if (eveOverride) {
    env.EVE_STATE_DIR = resolveUserPath(eveOverride, env, effectiveHomedir);
  }
}
 ...
 * outside the config directory. Set via `EVE_INCLUDE_ROOTS` as a
 ...
 * directory containing `eve.json`.
 */
export function resolveIncludeRoots(
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = envHomedir(env),
): string[] {
  const raw = env.EVE_INCLUDE_ROOTS?.trim();
  if (!raw) {
    return [];
  }
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
 ...
 * Can be overridden via EVE_CONFIG_PATH.
 * Default: ~/.eve/eve.json (or $EVE_STATE_DIR/eve.json)
 */
export function resolveCanonicalConfigPath(
  env: NodeJS.ProcessEnv = process.env,
  stateDir: string = resolveStateDir(env, envHomedir(env)),
): string {
  const override = env.EVE_CONFIG_PATH?.trim();
  if (override) {
...
export function resolveDefaultConfigCandidates(
  ...
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
  const explicit = env.EVE_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveUserPath(explicit, env, effectiveHomedir)];
  }

  const candidates: string[] = [];
  const eveStateDir = env.EVE_STATE_DIR?.trim();
  if (eveStateDir) {
    const resolved = resolveUserPath(eveStateDir, env, effectiveHomedir);
 ...
 * Default: os.tmpdir()/eve-<uid> (uid suffix when available).
 */
export function resolveGatewayLockDir(tmpdir: () => string = os.tmpdir): string {
  const base = tmpdir();
  const uid = typeof process.getuid === "function" ? process.getuid() : undefined;
  const suffix = uid != null ? `eve-${uid}` : "eve";
 ...
 *
 * Precedence:
 * - `EVE_OAUTH_DIR` (explicit override)
...
export function resolveOAuthDir(
  ...
  const override = env.EVE_OAUTH_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, envHomedir(env));
  }
