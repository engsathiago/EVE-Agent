...
import { normalizeConfiguredProviderCatalogModelId } from "@eve/model-catalog-core/provider-model-id-normalization";
...
export function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  eve config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  eve config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
export function unsetPathForWrite(
  root: EVEConfig,
  pathSegments: string[],
): { changed: boolean; next: EVEConfig } {
