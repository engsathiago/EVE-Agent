...
function canReuseUnscopedCurrentPluginMetadataSnapshot(config: EVEConfig): boolean {
...
function isProviderConfigured(cfg: EVEConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: EVEConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: EVEConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: EVEConfig, plugin: PluginManifestRecord): boolean {
...
function hasConfiguredWebSearchPluginEntry(cfg: EVEConfig): boolean {
...
function hasConfiguredWebSearchProviderSelection(cfg: EVEConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: EVEConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    Boolean(entries) &&
    typeof entries === "object" &&
...
function hasConfiguredSpeechProviderSelection(cfg: EVEConfig): boolean {
  return collectConfiguredSpeechProviderIds(cfg).size > 0;
}

function hasConfiguredPluginConfigEntry(cfg: EVEConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    Boolean(entries) &&
    typeof entries === "object" &&
...
function hasBrowserToolReference(cfg: EVEConfig): boolean {
...
function collectConfiguredPluginEntryIds(cfg: EVEConfig): string[] {
  const entries = cfg.plugins?.entries;
...
function hasOwnPluginEntry(cfg: EVEConfig, pluginId: string): boolean {
  const entries = cfg.plugins?.entries;
  return Boolean(entries) && typeof entries === "object" && Object.hasOwn(entries, pluginId);
}

function isPluginEntryExplicitlyDisabled(cfg: EVEConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function hasNonDisabledPluginEntry(cfg: EVEConfig, pluginId: string): boolean {
...
function hasBrowserSetupAutoEnableRelevantConfig(cfg: EVEConfig): boolean {
...
function hasAcpxSetupAutoEnableRelevantConfig(cfg: EVEConfig): boolean {
...
function hasXaiSetupAutoEnableRelevantConfig(cfg: EVEConfig): boolean {
...
function resolveRelevantSetupAutoEnablePluginIds(cfg: EVEConfig): string[] {
...
function hasSetupAutoEnableRelevantConfig(cfg: EVEConfig): boolean {
  return (
...
function hasPluginEntries(cfg: EVEConfig): boolean {
  const entries = cfg.plugins?.entries;
  return Boolean(entries) && typeof entries === "object" && Object.keys(entries).length > 0;
}

function hasPluginAllowlistWithMaterialEntries(cfg: EVEConfig): boolean {
  if (
...
function hasConfiguredProviderModelOrHarness(cfg: EVEConfig, env: NodeJS.ProcessEnv): boolean {
...
function arePluginsGloballyDisabled(cfg: EVEConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function configMayNeedPluginManifestRegistry(cfg: EVEConfig, env: NodeJS.ProcessEnv): boolean {
...
function isPluginExplicitlyDisabled(cfg: EVEConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: EVEConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isPluginExplicitlySelected(cfg: EVEConfig, pluginId: string): boolean {
...
function isBuiltInChannelAlreadyEnabled(cfg: EVEConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
