...
export const EXPECTED_CODEX_MODELS_COMMAND_TEXT = [
  ...
  "Current EVE session status reports the active model as:",
] as const;
...
export function isExpectedCodexStatusCommandText(text: string): boolean {
  ...
  const mentionsEVEStatus =
    normalized.includes("eve is running on") ||
    /eve\s+\S+\s+is running on/u.test(normalized) ||
    normalized.includes("eve status:") ||
  ...
  return (
    ...
    isReadyStatus ||
    isOnlineIdleStatus ||
    (mentionsEVEStatus && mentionsHarnessSession && mentionsModel)
  );
}
...
export function isExpectedCodexModelsCommandText(text: string): boolean {
  ...
  const mentionsSessionModel =
    ...
    normalized.includes("current session model from eve status") ||
  ...
  const mentionsConfigSummary =
    ...
    normalized.includes("live eve config shows") ||
