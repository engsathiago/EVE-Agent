...
const CODEX_HARNESS_LIVE = process.env.EVE_LIVE_CODEX_HARNESS === "1";
const CODEX_HARNESS_DEBUG = process.env.EVE_LIVE_CODEX_HARNESS_DEBUG === "1";
...
function snapshotEnv(): LiveEnvSnapshot {
  return snapshotLiveEnv(["EVE_TRAJECTORY", "EVE_TRAJECTORY_DIR"]);
}
...
describeLive("gateway live trajectory export", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempDir = await fs.mkdtemp(path.join(process.cwd(), ".tmp-eve-trajectory-live-"));
      cleanup.push(async () => {
      ...
      const bundleDir = path.join(workspaceDir, ".eve", "trajectory-exports", "bundle");
