...
import { clearRuntimeConfigSnapshot, type EVEConfig } from "../config/config.js";
...
const CLI_LIVE = isTruthyEnvValue(process.env.EVE_LIVE_CLI_BACKEND);
const CLI_RESUME = isTruthyEnvValue(process.env.EVE_LIVE_CLI_BACKEND_RESUME_PROBE);
const CLI_DEBUG = isTruthyEnvValue(process.env.EVE_LIVE_CLI_BACKEND_DEBUG);
...
const CLI_BACKEND_REQUEST_TIMEOUT_MS = parsePositiveIntegerEnv(
  "EVE_LIVE_CLI_BACKEND_REQUEST_TIMEOUT_MS",
  15 * 60_000,
);
...
function openAiProviderConfigForCodexCli(
  modelKey: string,
): NonNullable<NonNullable<EVEConfig["models"]>["providers"]>["openai"] {
  ...
  it(
    ...
    async () => {
      const preservedEnv = new Set(
        parseJsonStringArray(
          "EVE_LIVE_CLI_BACKEND_PRESERVE_ENV",
          process.env.EVE_LIVE_CLI_BACKEND_PRESERVE_ENV,
        ) ?? [],
      );
      ...
      const rawModel = process.env.EVE_LIVE_CLI_BACKEND_MODEL ?? DEFAULT_MODEL;
      ...
      const cliClearEnv =
        parseJsonStringArray(
          "EVE_LIVE_CLI_BACKEND_CLEAR_ENV",
          process.env.EVE_LIVE_CLI_BACKEND_CLEAR_ENV,
        ) ??
      ...
      const disableMcpConfig = process.env.EVE_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG !== "0";
      ...
      const cfg: EVEConfig = {};
      const cfgWithCliBackends = cfg as EVEConfig & {
        agents?: {
          defaults?: {
