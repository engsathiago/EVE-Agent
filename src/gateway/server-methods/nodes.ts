...
import { normalizeUniqueTrimmedStringList } from "@eve/normalization-core/string-normalization";
import {
  type ConnectParams,
  ErrorCodes,
  errorShape,
...
export async function maybeWakeNodeWithApns(
  nodeId: string,
  opts?: { force?: boolean; wakeReason?: string; cfg?: EVEConfig },
  ...
  try {
    ...
    if (registration.transport === "relay") {
      ...
      result = await sendApnsAlert({
        registration,
        nodeId,
        title: "EVE needs a quick reopen",
        body: "Tap to reopen EVE and restore the node connection.",
        relayConfig: relay.relayConfig,
      });
    } else {
      const auth = await resolveDirectNodePushConfig();
...
export const nodeHandlers: GatewayRequestHandlers = {
  ...
  // Remove a node pairing (CLI: `eve nodes remove`). For a device-backed
