...
const createdStates: EVETestState[] = [];

async function createState(label: string): Promise<EVETestState> {
  const state = await createEVETestState({ label, layout: "state-only" });
