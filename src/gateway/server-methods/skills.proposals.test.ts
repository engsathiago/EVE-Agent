...
vi.mock("../../skills/security/clawhub-verdicts.js", () => ({
  ...
  fetchEVESkillSecurityVerdicts: vi.fn(),
}));
...
describe("skills proposal gateway handlers", () => {
  beforeEach(async () => {
    testState = await createEVETestState({
      layout: "state-only",
      prefix: "eve-skills-proposals-gateway-state-",
    });
    ...
    mocks.workspaceDir = await tempDirs.make("eve-skills-proposals-gateway-");
  ...
  it("scopes list and inspect to the resolved agent workspace", async () => {
    ...
    const secondWorkspaceDir = await tempDirs.make("eve-skills-proposals-gateway-second-");
