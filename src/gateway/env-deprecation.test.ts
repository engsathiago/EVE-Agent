...
describe("warnLegacyEVEEnvVars", () => {
  ...
  it("warns with counts and prefixes instead of secret-shaped env names", () => {
    ...
    expect(message).toContain("replacing the legacy prefix with EVE_");
    ...
    expect(options).toEqual({
      code: "EVE_LEGACY_ENV_VARS",
      type: "DeprecationWarning",
    });
  });

  it("does not warn for current EVE names", () => {
    warnLegacyEVEEnvVars({ EVE_GATEWAY_TOKEN: "token" });

    expect(emitWarning).not.toHaveBeenCalled();
  });

  it("warns only once after a successful emit", () => {
    warnLegacyEVEEnvVars({ CLAWDBOT_GATEWAY_TOKEN: "old-token" });
    warnLegacyEVEEnvVars({ MOLTBOT_GATEWAY_TOKEN: "old-token" });
  ...
  it("retries if emitWarning throws before the warning is emitted", () => {
    ...
    expect(() => warnLegacyEVEEnvVars({ CLAWDBOT_GATEWAY_TOKEN: "old-token" })).toThrow(
      "warning sink failed",
    );
    warnLegacyEVEEnvVars({ CLAWDBOT_GATEWAY_TOKEN: "old-token" });
