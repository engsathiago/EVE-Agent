...
function createGatewayStartupTrace() {
  const logEnabled = isTruthyEnvValue(process.env.EVE_GATEWAY_STARTUP_TRACE);
  let timelineConfig: EVEConfig | undefined;
  ...
  const eventLoopTimelineEnabled = () =>
    ...
    isTruthyEnvValue(process.env.EVE_DIAGNOSTICS_EVENT_LOOP);
...
function formatRuntimeGatewayAuthTokenWarning(): string {
  ...
  if (!isNixMode) {
    return `${base} Persist one with \`eve config set gateway.auth.mode token\` and \`eve config set gateway.auth.token <token>\`.`;
  }
  return [
    base,
    "In Nix mode, set gateway.auth.token in your Nix-managed EVE config and rebuild.",
    "For the first-party Nix flow, see https://github.com/eve/nix-eve#quick-start and https://docs.eve.ai/install/nix.",
  ].join(" ");
}
...
export type GatewayServerOptions = {
   ...
   * reparsing eve.json during server startup.
   */
...
export async function startGatewayServer(
  ...
  const resolveSharedGatewaySessionGenerationForConfig = (config: EVEConfig) =>
    ...
    const gatewayRequestContext = await startupTrace.measure(
      ...
      async () => {
        ...
        return createGatewayRequestContext({
          ...
          clients,
          enforceSharedGatewayAuthGenerationForConfigWrite: (nextConfig: EVEConfig) => {
            enforceSharedGatewaySessionGenerationForConfigWrite({
