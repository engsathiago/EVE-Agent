...
const mocks = vi.hoisted(() => ({
  ...
  resolveWideAreaDiscoveryDomain: vi.fn<ResolveWideAreaDiscoveryDomain>(() => "eve.internal."),
  writeWideAreaGatewayZone: vi.fn<WriteWideAreaGatewayZone>(async () => ({
    changed: true,
    zonePath: "/tmp/eve.internal.db",
  })),
  formatBonjourInstanceName: vi.fn((name: string) => `${name} (EVE)`),
  resolveBonjourCliPath: vi.fn(() => "/usr/local/bin/eve"),
...
async function expectSshPortOmitted(rawPort: string) {
  ...
  process.env.EVE_SSH_PORT = rawPort;
...
function startStuckDiscovery(timeoutMs: string) {
  vi.useFakeTimers();
  useDevelopmentDiscoveryEnv();
  process.env.EVE_GATEWAY_DISCOVERY_ADVERTISE_TIMEOUT_MS = timeoutMs;
...
describe("startGatewayDiscovery", () => {
  ...
  it("skips local discovery services for truthy EVE_DISABLE_BONJOUR values", async () => {
    process.env.NODE_ENV = "development";
    delete process.env.VITEST;
    process.env.EVE_DISABLE_BONJOUR = "yes";
  ...
  it("keeps wide-area DNS-SD publishing active when local discovery is off", async () => {
    ...
    const result = await startGatewayDiscovery({
      ...
      wideAreaDiscoveryDomain: "eve.internal.",
      tailscaleMode: "serve",
      mdnsMode: "off",
      gatewayDiscoveryServices: [service],
      logDiscovery: logs,
    ...
    expect(logs.info.mock.calls).toEqual([
      ["wide-area DNS-SD updated (eve.internal. → /tmp/eve.internal.db)"],
    ]);
    expect(result.bonjourStop).toBeNull();
  });
