...
describe("resolveGatewayInteractiveSurfaceAuth", () => {
  it("uses EVE_GATEWAY_TOKEN as remote interactive fallback", async () => {
    await expect(
  ...
  it("keeps configured remote token ahead of EVE_GATEWAY_TOKEN", async () => {
    await expect(
      resolveGatewayInteractiveSurfaceAuth({
  ...
  it("falls back to EVE_GATEWAY_TOKEN when the remote token ref is unresolved", async () => {
    await expect(
      resolveGatewayInteractiveSurfaceAuth({
        config: {
          ...remoteGatewayConfig({
