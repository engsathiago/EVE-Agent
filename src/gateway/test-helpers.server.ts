...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.EVE_CONFIG_PATH) {
    configPaths.add(process.env.EVE_CONFIG_PATH);
  }
  if (process.env.EVE_STATE_DIR) {
    configPaths.add(path.join(process.env.EVE_STATE_DIR, "eve.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "eve-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.EVE_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "eve-test-no-bundled-extensions")
    : "eve-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".eve-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".eve-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".eve-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>eve-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
const CONNECT_CHALLENGE_NONCE_KEY = "__eveTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__eveTestConnectChallengeTracked";
...
export async function startServer(token?: string, opts?: GatewayServerOptions) {
  ...
  const envSnapshot = captureEnv(["EVE_GATEWAY_TOKEN"]);
  const prev = process.env.EVE_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.EVE_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.EVE_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
