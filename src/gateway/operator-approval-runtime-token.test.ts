...
const envSnapshot = captureEnv(["HOME", "EVE_HOME"]);

const tempHomes: string[] = [];

function useTempHome(): string {
  const home = fs.mkdtempSync(path.join(os.tmpdir(), "eve-approval-runtime-"));
  tempHomes.push(home);
  setTestEnvValue("HOME", home);
  setTestEnvValue("EVE_HOME", home);
  return home;
}

function execApprovalsPath(home: string): string {
  return path.join(home, ".eve", "exec-approvals.json");
}

function writeExecApprovalsToken(home: string, token: string): void {
  fs.mkdirSync(path.join(home, ".eve"), { recursive: true });
  fs.writeFileSync(
