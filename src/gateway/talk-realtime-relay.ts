...
function buildForcedConsultCheckingPrompt(): string {
  return [
    "Briefly tell the person that you are checking with EVE.",
    "Do not answer the request yet. Wait for the EVE result before giving the actual answer.",
  ].join(" ");
}
  ...
  session.forcedConsults.schedule(handle, FORCED_CONSULT_FALLBACK_DELAY_MS, () => {
    ...
    broadcastToOwner(session.context, session.connId, {
      ...
      args: {
        ...
        context:
          "The realtime provider produced a final user transcript without invoking eve_agent_consult, so EVE is forcing the consult for realtime Talk.",
