...
export const EVE_MODEL_ID = "eve";
/** Default OpenAI-compatible model alias that targets the default EVE agent. */
export const EVE_DEFAULT_MODEL_ID = "eve/default";
...
export class GatewaySessionKeyOverrideError extends Error {
  constructor() {
    super("`x-eve-session-key` cannot use reserved internal session namespaces.");
...
function resolveAgentIdFromHeader(req: IncomingMessage): string | undefined {
  const raw =
    normalizeOptionalString(getHeader(req, "x-eve-agent-id")) ||
    normalizeOptionalString(getHeader(req, "x-eve-agent")) ||
    "";
  if (!raw) {
    return undefined;
  }
...
export function resolveAgentIdFromModel(
  ...
  if (lowered === EVE_MODEL_ID || lowered === EVE_DEFAULT_MODEL_ID) {
    return resolveDefaultAgentId(cfg);
  }

  const m =
    raw.match(/^eve[:/](?<agentId>[a-z0-9][a-z0-9_-]{0,63})$/i) ??
...
/** Validates and resolves the `x-eve-model` override for OpenAI-compatible requests. */
export async function resolveOpenAiCompatModelOverride(params: {
  ...
  if (requestModel && !resolveAgentIdFromModel(requestModel)) {
    return {
      errorMessage: "Invalid `model`. Use `eve` or `eve/<agentId>`.",
    };
  }

  const raw = getHeader(params.req, "x-eve-model")?.trim();
  if (!raw) {
    return {};
  }
  ...
  if (!parsed) {
    return { errorMessage: "Invalid `x-eve-model`." };
  }
...
function resolveSessionKey(params: {
  ...
  const explicit = getHeader(params.req, "x-eve-session-key")?.trim();
  if (explicit) {
...
export function resolveGatewayRequestContext(params: {
  ...
  const messageChannel = params.useMessageChannelHeader
    ? (normalizeMessageChannel(getHeader(params.req, "x-eve-message-channel")) ??
