...
describe("resolveGatewayRequestContext", () => {
  it("uses normalized x-eve-message-channel when enabled", () => {
    const result = resolveGatewayRequestContext({
      req: createReq({ "x-eve-message-channel": " Custom-Channel " }),
      model: "eve",
  ...
  it("preserves normal explicit session-key overrides", () => {
    const result = resolveGatewayRequestContext({
      req: createReq({ "x-eve-session-key": "customer-case-42" }),
      model: "eve",
      sessionPrefix: "openai",
      defaultMessageChannel: "webchat",
    });
  ...
  it.each([
    ...
    expect(() =>
      resolveGatewayRequestContext({
        req: createReq({ "x-eve-session-key": sessionKey }),
        model: "eve",
        sessionPrefix: "openai",
        defaultMessageChannel: "webchat",
      }),
  ...
  it("does not build session state for explicit unknown agent ids", () => {
    expect(() =>
      resolveGatewayRequestContext({
        req: createReq({ "x-eve-agent-id": "missing-agent" }),
        model: "eve",
        sessionPrefix: "openai",
        defaultMessageChannel: "webchat",
      }),
...
describe("resolveHttpSenderIsOwner", () => {
  it("requires operator.admin on a trusted HTTP scope-bearing request", () => {
    expect(
      resolveHttpSenderIsOwner(createReq({ "x-eve-scopes": "operator.admin" }), noneAuth),
    ).toBe(true);
    expect(
      resolveHttpSenderIsOwner(createReq({ "x-eve-scopes": "operator.write" }), noneAuth),
    ).toBe(false);
  });
...
describe("authorizeOpenAiCompatibleHttpModelOverride", () => {
  it("allows shared-secret bearer callers to use x-eve-model", () => {
    expect(
      authorizeOpenAiCompatibleHttpModelOverride(
        createReq({ authorization: "Bearer secret", "x-eve-model": "openai/gpt-5.4" }),
        { authMethod: "token", trustDeclaredOperatorScopes: false },
      ),
    ).toEqual({ allowed: true });
  });

  it("allows trusted admin callers to use x-eve-model", () => {
    expect(
      authorizeOpenAiCompatibleHttpModelOverride(
        createReq({
          "x-eve-scopes": "operator.admin, operator.write",
  ...
  it("rejects trusted write-only callers that try to use x-eve-model", () => {
    expect(
      authorizeOpenAiCompatibleHttpModelOverride(
        createReq({
          "x-eve-scopes": "operator.write",
