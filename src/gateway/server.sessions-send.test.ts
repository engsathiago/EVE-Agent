...
const { createEVETools } = await import("../agents/eve-tools.js");
...
type SessionSendTool = ReturnType<typeof createEVETools>[number];
...
beforeAll(async () => {
  envSnapshot = captureEnv(["EVE_GATEWAY_PORT", "EVE_GATEWAY_TOKEN"]);
  ...
  process.env.EVE_GATEWAY_PORT = String(gatewayPort);
  process.env.EVE_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send gateway loopback", () => {
  ...
  it(
    ...
    async () => {
      const dir = await fs.mkdtemp(path.join(os.tmpdir(), "eve-sessions-send-route-"));
