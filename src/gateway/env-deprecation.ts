...
// Emits a one-shot notice for ignored pre-EVE environment prefixes.
...
export function warnLegacyEVEEnvVars(env: NodeJS.ProcessEnv = process.env): void {
  ...
  process.emitWarning(
    [
      `Legacy ${detectedPrefixes} environment variables were detected (${legacyVarCount} total), but EVE only reads EVE_* names now.`,
      "Rename them by replacing the legacy prefix with EVE_; the old names are ignored.",
    ].join("\n"),
    { code: "EVE_LEGACY_ENV_VARS", type: "DeprecationWarning" },
  );
  warned = true;
}

/** Resets the one-shot legacy env warning latch for tests. */
export function resetLegacyEVEEnvWarningForTest(): void {
  warned = false;
}
