...
const hoisted = vi.hoisted(() => {
  ...
  const ensureEVEModelsJson = vi.fn(async () => {});
...
vi.mock("../config/paths.js", async () => {
  ...
  return {
    ...actual,
    STATE_DIR: "/tmp/eve-state",
    resolveConfigPath: vi.fn(() => "/tmp/eve-state/eve.json"),
    resolveGatewayPort: vi.fn(() => 18789),
    resolveStateDir: vi.fn(() => "/tmp/eve-state"),
  };
});
...
vi.mock("../agents/models-config.js", () => ({
  ensureEVEModelsJson: hoisted.ensureEVEModelsJson,
}));
...
describe("startGatewayPostAttachRuntime", () => {
  beforeEach(() => {
    vi.stubEnv("EVE_SKIP_CHANNELS", "0");
    vi.stubEnv("EVE_SKIP_PROVIDERS", "0");
    ...
    hoisted.ensureEVEModelsJson.mockReset();
    hoisted.ensureEVEModelsJson.mockResolvedValue(undefined);
  ...
  it("skips heavy restart sentinel refresh when no sentinel file exists", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "eve-no-sentinel-"));
  ...
  it("refreshes the restart sentinel when the sentinel file exists", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "eve-sentinel-"));
  ...
  it("expands tilde-based restart sentinel state paths", async () => {
    const osHome = fs.mkdtempSync(path.join(os.tmpdir(), "eve-home-"));
    try {
      const eveHome = path.join(osHome, "eve-home");
      const stateDirFromHome = path.join(eveHome, ".eve");
      ...
      const backslashStateDir = path.resolve(`${osHome}\\eve-state`);
  ...
  it("avoids sync filesystem probes while checking restart sentinel presence", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "eve-async-sentinel-"));
    try {
      fs.writeFileSync(path.join(stateDir, "restart-sentinel.json"), "{}\n");
  ...
  it("cleans startup session locks with bounded concurrency", async () => {
    ...
    let maxActive = 0;
    const cleanedLock = {
      lockPath: "/tmp/eve-state/agents/main/sessions/a.jsonl.lock",
      pid: null,
      pidAlive: false,
      createdAt: null,
      ageMs: null,
  ...
  it("starts channels when channel startup is enabled", async () => {
    await withEnvAsync(
      {
        EVE_SKIP_CHANNELS: undefined,
        EVE_SKIP_PROVIDERS: undefined,
      },
      async () => {
  ...
  it("starts and reports plugin services after channel startup completes", async () => {
    await withEnvAsync(
      { EVE_SKIP_CHANNELS: undefined, EVE_SKIP_PROVIDERS: undefined },
      async () => {
