...
function withStableOwnerDisplaySecretForTest(cfg: unknown): unknown {
  ...
  return {
    ...
    commands: {
      ...commands,
      ownerDisplaySecret: "eve-test-owner-display-secret",
    },
  };
}

/** Writes a temp EVE config, installs it as runtime state, then restores globals. */
export async function withTempConfig(params: {
  cfg: unknown;
  run: () => Promise<void>;
  prefix?: string;
}): Promise<void> {
  const prevConfigPath = process.env.EVE_CONFIG_PATH;

  const testConfig = withStableOwnerDisplaySecretForTest(params.cfg) as EVEConfig;
  const dir = await mkdtemp(path.join(os.tmpdir(), params.prefix ?? "eve-test-config-"));
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.EVE_CONFIG_PATH = prevConfigPath;
    }
    clearConfigCache();
    resetConfigRuntimeState();
    clearSecretsRuntimeSnapshot();
