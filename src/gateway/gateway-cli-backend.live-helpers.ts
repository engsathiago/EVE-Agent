...
export const CLI_BACKEND_LIVE_PROVIDER_SKIP_ENV = "EVE_LIVE_CLI_BACKEND_ALLOW_PROVIDER_SKIP";
export const CLI_BACKEND_LIVE_ADVISORY_ENV = "EVE_LIVE_CLI_BACKEND_ADVISORY";
...
export function resolveCliBackendLiveModelSelection(params: {
  ...
  if (!parsed) {
    throw new Error(
      `EVE_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${params.rawModel}`,
    );
  }

  if (parsed.provider === "codex-cli") {
    throw new Error(
      "EVE_LIVE_CLI_BACKEND_MODEL=codex-cli/... is no longer supported. Use a supported CLI backend such as claude-cli or google-gemini-cli.",
    );
  }
...
export function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("EVE_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}

export function shouldRunCliImageProbe(providerId: string): boolean {
  const raw = process.env.EVE_LIVE_CLI_BACKEND_IMAGE_PROBE?.trim();
  if (raw) {
...
export function shouldRunCliMcpProbe(providerId: string): boolean {
  const raw = process.env.EVE_LIVE_CLI_BACKEND_MCP_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function resolveCliBackendLiveArgs(params: {
  ...
  const resumeArgs =
    parseJsonStringArray(
      "EVE_LIVE_CLI_BACKEND_RESUME_ARGS",
      process.env.EVE_LIVE_CLI_BACKEND_RESUME_ARGS,
...
export function shouldRunCliModelSwitchProbe(providerId: string, modelRef: string): boolean {
  const raw = process.env.EVE_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function snapshotCliBackendLiveEnv(): CliBackendLiveEnvSnapshot {
  return {
    ...
    skipProviders: process.env.EVE_SKIP_PROVIDERS,
    ...
    skipBrowserControl: process.env.EVE_SKIP_BROWSER_CONTROL_SERVER,
    bundledPluginsDir: process.env.EVE_BUNDLED_PLUGINS_DIR,
    minimalGateway: process.env.EVE_TEST_MINIMAL_GATEWAY,
...
export function applyCliBackendLiveEnv(preservedEnv: ReadonlySet<string>): void {
  ...
  process.env.EVE_SKIP_PROVIDERS = "1";
  ...
  process.env.EVE_TEST_MINIMAL_GATEWAY = "1";
...
export function restoreCliBackendLiveEnv(snapshot: CliBackendLiveEnvSnapshot): void {
  restoreEnvVar("EVE_CONFIG_PATH", snapshot.configPath);
  restoreEnvVar("EVE_STATE_DIR", snapshot.stateDir);
  restoreEnvVar("EVE_GATEWAY_TOKEN", snapshot.token);
  restoreEnvVar("EVE_SKIP_CHANNELS", snapshot.skipChannels);
  restoreEnvVar("EVE_SKIP_PROVIDERS", snapshot.skipProviders);
  restoreEnvVar("EVE_SKIP_GMAIL_WATCHER", snapshot.skipGmail);
  restoreEnvVar("EVE_SKIP_CRON", snapshot.skipCron);
  restoreEnvVar("EVE_SKIP_CANVAS_HOST", snapshot.skipCanvas);
  restoreEnvVar("EVE_SKIP_BROWSER_CONTROL_SERVER", snapshot.skipBrowserControl);
  restoreEnvVar("EVE_BUNDLED_PLUGINS_DIR", snapshot.bundledPluginsDir);
  restoreEnvVar("EVE_TEST_MINIMAL_GATEWAY", snapshot.minimalGateway);
