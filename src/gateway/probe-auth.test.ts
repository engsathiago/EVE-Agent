...
function configWithDefaultEnvProvider(gateway: NonNullable<EVEConfig["gateway"]>) {
  return {
    gateway,
    secrets: {
      providers: {
...
function resolveSafeProbeAuth(cfg: EVEConfig, mode: "local" | "remote" = "local") {
...
function expectUnresolvedProbeTokenWarning(cfg: EVEConfig) {
  ...
  it("resolves env SecretRef token via async secret-inputs path", async () => {
    const result = await resolveGatewayProbeAuthSafeWithSecretInputs({
      cfg: configWithDefaultEnvProvider({
        auth: tokenAuthConfig("EVE_GATEWAY_TOKEN"),
      }),
      mode: "local",
      env: {
        EVE_GATEWAY_TOKEN: "test-token-from-env",
