...
type GatewayConnectionDetailResolvers = {
  ...
  resolveGatewayPort?: (cfg?: EVEConfig, env?: NodeJS.ProcessEnv) => number;
};
...
export function buildGatewayConnectionDetailsWithResolvers(
  ...
  const envUrlOverride =
    cliUrlOverride || options.ignoreEnvUrlOverride
      ? undefined
      : normalizeOptionalString(process.env.EVE_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.EVE_ALLOW_INSECURE_PRIVATE_WS === "1";
