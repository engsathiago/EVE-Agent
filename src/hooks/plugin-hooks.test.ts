...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "eve-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.EVE_BUNDLED_HOOKS_DIR;
    process.env.EVE_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(() => {
    ...
    if (previousBundledHooksDir === undefined) {
      delete process.env.EVE_BUNDLED_HOOKS_DIR;
    } else {
      process.env.EVE_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  async function writeBundleHookFixture(): Promise<string> {
    const bundleRoot = path.join(workspaceDir, ".eve", "extensions", "sample-bundle");
  ...
  function createConfig(enabled: boolean): EVEConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entry.hook.source).toBe("eve-plugin");
  ...
  it("does not treat Claude hooks.json bundles as EVE hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".eve", "extensions", "claude-bundle");
