...
vi.mock("../agents/agent-scope.js", () => ({
  ...
  resolveAgentWorkspaceDir: vi.fn(() => "/tmp/eve-agent"),
  resolveAgentDir: vi.fn(() => "/tmp/eve-agent/.eve-agent"),
  resolveAgentEffectiveModelPrimary: vi.fn((cfg: EVEConfig) => {
...
function requireFirstRunOptions(): Record<string, unknown> {
  ...
  if (!call) {
    throw new Error("expected embedded EVE agent run");
  }
  const [options] = call;
  if (!options || typeof options !== "object") {
    throw new Error("expected embedded EVE agent run options");
  }
