...
homepage: https://docs.eve.ai/automation/hooks#command-logger
metadata:
  {
    "eve":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.eve/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.eve/logs/commands.log ~/.eve/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/eve`:
   ```
   /home/username/.eve/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
