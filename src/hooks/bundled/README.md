...
This directory contains hooks that ship with EVE. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.eve/workspace`)
...
- **Managed hooks**: `~/.eve/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.eve/eve.json`:
...
2. Restart gateway: `pkill -9 -f 'eve.*gateway' && pnpm eve gateway`
3. Enable the hook: `eve hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.eve.ai/automation/hooks
