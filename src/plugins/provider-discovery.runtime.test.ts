...
function createManifestPlugin(id: string): PluginManifestRecord {
  return {
    ...
    rootDir: `/tmp/${id}`,
    source: "bundled",
    manifestPath: `/tmp/${id}/eve.plugin.json`,
...
describe("resolvePluginDiscoveryProvidersRuntime", () => {
  ...
  it("clears bundled dist discovery chunks from the dist root", () => {
    ...
    mocks.loadPluginMetadataSnapshot.mockReturnValue({
      ...
      manifestRegistry: {
        plugins: [
          {
            ...createManifestPlugin("deepseek"),
            rootDir: "/tmp/eve/dist/extensions/deepseek",
            manifestPath: "/tmp/eve/dist/extensions/deepseek/eve.plugin.json",
            providerDiscoverySource: "/tmp/eve/dist/extensions/deepseek/provider-discovery.js",
          },
        ],
        diagnostics: [],
      },
    ...
    expect(mocks.clearNativeRequireJavaScriptModuleCache).toHaveBeenCalledWith(
      "/tmp/eve/dist/extensions/deepseek/provider-discovery.js",
      { dependencyRoot: "/tmp/eve/dist" },
    );
  });
