...
export type PluginTextTransforms = {
  ...
  /** Rewrites applied to inbound assistant text before EVE consumes it. */
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether EVE should inject bundle MCP config for this backend.
   *
   * Keep this opt-in. Only backends that explicitly consume EVE's bundle
   ...
   *
   * Use this when the backend should consume a canonical EVE auth profile
   ...
   * `profile-only` treats the selected EVE auth profile as the sole auth
   ...
   * as static config, such as mapping EVE thinking levels to a backend's
   * native effort flag.
   */
  resolveExecutionArgs?: CliBackendResolveExecutionArgs;
  /**
   * Whether this CLI backend can expose native tools outside EVE's tool
