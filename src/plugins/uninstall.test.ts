...
type PluginConfig = NonNullable<EVEConfig["plugins"]>;
...
function createSinglePluginWithEmptySlotsConfig(): EVEConfig {
...
function createSingleNpmInstallConfig(installPath: string): EVEConfig {
  return createPluginConfig({
    entries: createSinglePluginEntries(),
    installs: {
...
describe("removePluginFromConfig", () => {
  ...
  it("removes absolute load path for a workspace-relative install source path", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(tempRoot, "eve-uninstall-portable-source-"));
    try {
    ...
    },
    {
      ...
      config: createPluginConfig({
        ...
        channels: {
          ...
          timbot: { sdkAppId: "123" },
        } as unknown as EVEConfig["channels"],
      }),
      pluginId: "timbot",
      expectedChannels: {
        defaults: { groupPolicy: "opt-in" },
...
describe("uninstallPlugin", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      config: createPluginConfig({
        ...
        loadPaths: ["/missing/eve/plugin", "/keep/this/plugin"],
      }),
      expectedActions: {
        entry: false,
        install: true,
  ...
  it("uninstalls npm-managed packages through npm before deleting the package directory", async () => {
    ...
    const pluginDir = path.join(npmRoot, "node_modules", "@eve", "kitchen-sink");
    ...
    const plan = planPluginUninstall({
      config: createPluginConfig({
        entries: createSinglePluginEntries("eve-kitchen-sink-fixture"),
        installs: {
          "eve-kitchen-sink-fixture": {
            source: "npm",
            spec: "@eve/kitchen-sink@1.0.0",
            installPath: pluginDir,
          },
        },
      }),
    ...
    expect(applied).toEqual({ directoryRemoved: true, warnings: [] });
    expectNpmUninstallCommand({ packageName: "@eve/kitchen-sink", npmRoot });
    await expectPathAccessState(pluginDir, "missing");
  });
  ...
  it("repairs remaining npm plugin eve peer links after npm uninstall prunes them", async () => {
    const stateDir = path.join(tempDir, "state");
    const npmRoot = path.join(stateDir, "npm");
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("returns managed install path for copied path installs", () => {
    const extensionsDir = path.join(os.tmpdir(), "eve-uninstall-safe");
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    ...
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        ...
        installPath: "/tmp/not-eve-plugin-install/my-plugin",
      },
      extensionsDir,
    });
  ...
  it("uses configured installPath when npm installed it under the managed npm root", () => {
    const stateDir = path.join(os.tmpdir(), "eve-uninstall-safe");
    const extensionsDir = path.join(stateDir, "extensions");
    const installPath = path.join(stateDir, "npm", "node_modules", "@eve", "kitchen-sink");
  ...
  it("uses configured installPath when npm installed it under a managed npm project", () => {
    ...
    const installPath = path.join(npmRoot, "node_modules", "@eve", "kitchen-sink");
    ...
    expect(
      resolveUninstallDirectoryTarget({
        ...
        installRecord: {
          source: "npm",
          spec: "@eve/kitchen-sink@latest",
          installPath: path.join(os.tmpdir(), "npm", "node_modules", "@eve", "kitchen-sink"),
        },
        extensionsDir,
      }),
    ).toBe(resolvePluginInstallDir("eve-kitchen-sink-fixture", extensionsDir));
  });
