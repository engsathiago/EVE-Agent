...
const mocks = vi.hoisted(() => ({
  loadEVEPlugins: vi.fn<typeof import("../loader.js").loadEVEPlugins>(),
...
function loadOptions(index = 0) {
  return requireRecord(mocks.loadEVEPlugins.mock.calls[index]?.[0], `load options ${index}`);
}
...
vi.mock("../loader.js", () => ({
  loadEVEPlugins: (...args: Parameters<typeof mocks.loadEVEPlugins>) =>
    mocks.loadEVEPlugins(...args),
  resolveCompatibleRuntimePluginRegistry: (
...
describe("ensurePluginRegistryLoaded", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadEVEPlugins.mockReturnValue(createEmptyPluginRegistry());
    mocks.resolveRuntimePluginRegistry.mockImplementation(
      (...args: Parameters<typeof mocks.loadEVEPlugins>) => mocks.loadEVEPlugins(...args),
    );
  ...
  it("does not reuse non-empty all-scope registries without loader compatibility", () => {
    ...
    mocks.loadEVEPlugins.mockClear();
