...
function collectConfiguredPluginEntryIds(config: EVEConfig): string[] {
...
function resolveRegister(mod: EVEPluginModule): {
...
function resolveSetupRegistration(record: PluginManifestRecord): {
  ...
  let mod: EVEPluginModule;
  try {
    mod = getModuleLoader(setupSource)(setupSource) as EVEPluginModule;
  } catch {
    return null;
  }

  const resolved = resolveRegister((mod as { default?: EVEPluginModule }).default ?? mod);
...
function pushDescriptorRuntimeDisabledDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    message:
      "setup.requiresRuntime is false, so EVE ignored the plugin setup runtime entry. Remove setup-api/eve.setupEntry or set requiresRuntime true if setup lookup still needs plugin code.",
  });
}
