...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(
    path.join(fs.realpathSync(os.tmpdir()), "eve-doctor-contract-load-paths-"),
  );
  tempDirs.push(dir);
  return dir;
}
...
function createDoctorPluginConfig(pluginRoot: string, pluginId: string): EVEConfig {
  return {
    plugins: {
...
function readPluginLlmPolicy(config: EVEConfig, pluginId: string): Record<string, unknown> {
