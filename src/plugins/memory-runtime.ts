...
function resolveMemoryRuntimePluginIds(config: EVEConfig): string[] {
...
function resolveMemoryRuntimeWorkspaceDir(cfg: EVEConfig): string | undefined {
...
function ensureMemoryRuntime(cfg?: EVEConfig) {
...
export function resolveActiveMemoryBackendConfig(params: { cfg: EVEConfig; agentId: string }) {
  return ensureMemoryRuntime(params.cfg)?.resolveMemoryBackendConfig(params) ?? null;
}

/** Closes all active plugin-backed memory search managers. */
export async function closeActiveMemorySearchManagers(cfg?: EVEConfig): Promise<void> {
  void cfg;
