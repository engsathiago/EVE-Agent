 ...
 */
export type EVEPluginConfigSchema = {
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). EVE uses this to expose
 ...
 *
 * Runs after EVE resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common EVE auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and EVE wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before EVE builds its generic
 ...
 * `buildReplayPolicy`, `normalizeToolSchemas`, and `wrapStreamFn`. EVE no
...
/**
 * Provider-owned stream wrapper hook after EVE applies its generic
 ...
 *
 * Return `true` or `false` to override EVE's built-in provider cache TTL
 ...
 *
 * Runs only after EVE exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that EVE should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after EVE loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, EVE falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * EVE calls this only from async model resolution paths. After it
   ...
   *
   * Runs after EVE performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before EVE registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic EVE wrappers.
   *
   ...
   *
   * Called after EVE resolves the raw configured credential but before the
   ...
   * hint than EVE's generic auth-store guidance.
   */
   ...
   * EVE's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when EVE should surface a provider-specific hint for
   ...
   * merging. EVE deduplicates by `provider/id`, so plugins only need to
   ...
   * or model exposes a custom set of thinking levels. EVE stores the
   ...
   * without replacing the full EVE-owned system prompt.
   */
   ...
   *
   * Runs after EVE's built-in overlay is resolved and before the
   ...
   *
   * Return undefined/null to keep EVE's default fallback policy.
   */
   ...
   * rewrites after EVE has assembled the complete prompt. Return
   ...
   * `output` applies to assistant text deltas/final text before EVE handles
   ...
   *
   * EVE uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `eve doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `eve doctor` should offer.
   *
   ...
   *
   * EVE calls this before falling back to the shared `shared model runtime` OAuth
   ...
   * path. Return nothing to keep EVE's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * EVE falls back to that stored profile.
   */
...
export type EVEPluginGatewayMethod = {
  method: string;
...
export type PluginCommandDiagnosticsSession = {
  ...
  sessionKey?: string;
  /** Ephemeral EVE session id when available. */
  sessionId?: string;
  /** Transcript file for this EVE session when available. */
  sessionFile?: string;
...
export type PluginCommandContext = {
  ...
  sessionId?: string;
  /** Transcript file for the active EVE session when available. */
  sessionFile?: string;
  /** Raw command arguments after the command name */
  args?: string;
  /** The full normalized command body */
  commandBody: string;
  /** Current EVE configuration */
  config: EVEConfig;
...
export type EVEPluginHttpRouteAuth = "gateway" | "plugin";
export type EVEPluginHttpRouteMatch = "exact" | "prefix";
export type EVEPluginGatewayRuntimeScopeSurface = "write-default" | "trusted-operator";
...
export type EVEPluginHttpRouteUpgradeHandler = (
  req: IncomingMessage,
  socket: Duplex,
  head: Buffer,
) => Promise<boolean | void> | boolean | void;

export type EVEPluginHttpRouteParams = {
  path: string;
...
export type EVEPluginHostedMediaResolver = (
  mediaUrl: string,
) => string | null | undefined | Promise<string | null | undefined>;

export type EVEPluginCliContext = {
  /**
   * Command object where this plugin should register its commands.
   *
   * For root CLI registrations this is the root `eve` program. For nested
...
export type EVEPluginCliRegistrar = (ctx: EVEPluginCliContext) => void | Promise<void>;

/**
 * Top-level CLI metadata for plugin-owned commands.
 *
 * Descriptors are the parse-time contract for lazy plugin CLI registration.
 * If you want EVE to keep a plugin command lazy-loaded while still
 ...
 */
export type EVEPluginCliCommandDescriptor = {
  name: string;
  description: string;
  hasSubcommands: boolean;
};

export type EVEPluginNodeCliFeatureOptions = {
  /** Explicit node feature command names owned under `eve nodes`. */
  commands?: string[];
  /**
   * Parse-time command descriptors for lazy node feature CLI registration.
   *
   * Descriptors are registered under `eve nodes`, so a descriptor named
   * `"camera"` exposes `eve nodes camera`.
   */
...
export type EVEPluginNodeInvokeTransportResult =
  | {
      ok: true;
      payload?: unknown;
...
export type EVEPluginNodeInvokeApprovalDecision = "allow-once" | "allow-always" | "deny";

export type EVEPluginNodeInvokePolicyApprovalRuntime = {
  request: (input: {
    title: string;
    description: string;
    ...
    id?: string;
    decision?: EVEPluginNodeInvokeApprovalDecision | null;
  }>;
};
...
export type EVEPluginNodeInvokePolicyContext = {
  ...
  approvals?: EVEPluginNodeInvokePolicyApprovalRuntime;
  invokeNode: (input?: {
    params?: unknown;
    timeoutMs?: number;
    idempotencyKey?: string;
...
export type EVEPluginNodeInvokePolicy = {
  ...
  ) => Promise<EVEPluginNodeInvokePolicyResult> | EVEPluginNodeInvokePolicyResult;
};
...
export type EVEPluginSecurityAuditCollector = (
  ctx: EVEPluginSecurityAuditContext,
...
export type EVEGatewayDiscoveryService = {
  id: string;
  advertise: (
    ctx: EVEGatewayDiscoveryAdvertiseContext,
...
export type EVEPluginService = {
  id: string;
  start: (ctx: EVEPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: EVEPluginServiceContext) => void | Promise<void>;
};

export type EVEPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type EVEPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  /**
   * @deprecated Declare exclusive plugin kind in `eve.plugin.json` via
  ...
  reload?: EVEPluginReloadRegistration;
  nodeHostCommands?: EVEPluginNodeHostCommand[];
  securityAuditCollectors?: EVEPluginSecurityAuditCollector[];
  register?: (api: EVEPluginApi) => void;
  activate?: (api: EVEPluginApi) => void;
};

export type EVEPluginModule = EVEPluginDefinition | ((api: EVEPluginApi) => void);
...
export type PluginConfigMigration = (config: EVEConfig) =>
  | {
      config: EVEConfig;
      changes: string[];
    }
...
/** Migration source implemented by a plugin and orchestrated by `eve migrate`. */
...
export type EVEPluginSessionStateApi = {
...
export type EVEPluginSessionWorkflowApi = {
...
export type EVEPluginSessionControlsApi = {
...
export type EVEPluginSessionApi = {
  state: EVEPluginSessionStateApi;
  workflow: EVEPluginSessionWorkflowApi;
  controls: EVEPluginSessionControlsApi;
};

export type EVEPluginAgentEventsApi = {
...
export type EVEPluginAgentApi = {
  events: EVEPluginAgentEventsApi;
};

export type EVEPluginRunContextApi = {
...
export type EVEPluginLifecycleApi = {
...
export type EVEPluginApi = {
   ...
   */
  session: EVEPluginSessionApi;
  /** Grouped facade for agent-event workflow seams. */
  agent: EVEPluginAgentApi;
  ...
  registerHook: (
    ...
    opts?: EVEPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: EVEPluginHttpRouteParams) => void;
  /** Register a plugin-owned resolver for browser-style hosted media URLs. */
  registerHostedMediaResolver: (resolver: EVEPluginHostedMediaResolver) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: EVEPluginChannelRegistration | ChannelPlugin) => void;
  /**
  ...
  registerCli: (
    ...
    opts?: {
       ...
       *
       * When descriptors cover every command exposed at `parentPath`, EVE
  ...
  /**
   * Register a plugin-owned node feature command group under `eve nodes`.
   *
  ...
  registerNodeCliFeature: (
    registrar: EVEPluginCliRegistrar,
    opts?: EVEPluginNodeCliFeatureOptions,
  ) => void;
  registerReload: (registration: EVEPluginReloadRegistration) => void;
  registerNodeHostCommand: (command: EVEPluginNodeHostCommand) => void;
  registerNodeInvokePolicy: (policy: EVEPluginNodeInvokePolicy) => void;
  registerSecurityAuditCollector: (collector: EVEPluginSecurityAuditCollector) => void;
  registerService: (service: EVEPluginService) => void;
  /** Register a local gateway discovery advertiser such as mDNS/Bonjour. */
  registerGatewayDiscoveryService: (service: EVEGatewayDiscoveryService) => void;
  ...
  /** Register an importer for `eve migrate` (migration capability). */
