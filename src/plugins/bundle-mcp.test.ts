...
function createEnabledBundleConfig(pluginIds: string[]): EVEConfig {
  return {
    plugins: {
...
describe("loadEnabledBundleMcpConfig", () => {
  ...
  it("uses a provided manifest registry instead of rediscovering bundle plugins", async () => {
    const homeDir = await tempHarness.createTempDir("eve-bundle-mcp-home-");
    const workspaceDir = await tempHarness.createTempDir("eve-bundle-mcp-workspace-");
