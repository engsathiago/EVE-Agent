...
import { normalizeOptionalString as normalizeString } from "@eve/normalization-core/string-coerce";
...
function getConfigPath(config: EVEConfig | undefined, path: string): unknown {
...
function setConfigPath(target: EVEConfig, path: string, value: unknown): void {
...
function buildProviderEntry(params: {
  ...
  return {
    ...
    applySelectionConfig: (config: EVEConfig) =>
