...
function resolveBundlePluginRoot(homeDir: string, pluginId: string) {
  return path.join(homeDir, ".eve", "extensions", pluginId);
}
...
describe("loadEnabledClaudeBundleCommands", () => {
  it("loads enabled Claude bundle markdown commands and skips disabled-model-invocation entries", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "EVE_HOME", "EVE_STATE_DIR"]);
    try {
      const homeDir = await createTempDir("eve-bundle-commands-home-");
      const workspaceDir = await createTempDir("eve-bundle-commands-workspace-");
