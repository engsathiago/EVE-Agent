...
function makeTempDir() {
  return makeTrackedTempDir("eve-installed-plugin-index-store", tempDirs);
}
...
function createIndex(overrides: Partial<InstalledPluginIndex> = {}): InstalledPluginIndex {
  return {
    ...
    plugins: [
      {
        pluginId: "demo",
        manifestPath: "/plugins/demo/eve.plugin.json",
  ...
  it("preserves startup config paths across persisted index roundtrips", async () => {
    ...
    const index = createIndex({
      plugins: [
        {
          pluginId: "browser",
          manifestPath: "/plugins/browser/eve.plugin.json",
      ...
      plugins: [
        {
          ...
          manifestPath: "/plugins/provider-owner/eve.plugin.json",
