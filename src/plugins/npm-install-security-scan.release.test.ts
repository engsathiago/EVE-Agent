...
const REQUIRED_REVIEWED_PUBLISHABLE_CRITICAL_FINDINGS = new Set([
  "@eve/acpx:dangerous-exec:src/codex-auth-bridge.ts",
  "@eve/acpx:dangerous-exec:src/runtime-internals/mcp-proxy.mjs",
  "@eve/codex:dangerous-exec:src/app-server/sandbox-exec-server/http.ts",
  "@eve/codex:dangerous-exec:src/app-server/sandbox-exec-server/processes.ts",
  "@eve/codex:dangerous-exec:src/app-server/transport-stdio.ts",
  "@eve/codex:dangerous-exec:src/node-cli-sessions.ts",
  "@eve/discord:dangerous-exec:src/voice/audio.ts",
  "@eve/google-meet:dangerous-exec:src/node-host.ts",
  "@eve/google-meet:dangerous-exec:src/realtime.ts",
  "@eve/matrix:dangerous-exec:src/matrix/deps.ts",
  "@eve/voice-call:dangerous-exec:src/tunnel.ts",
  "@eve/voice-call:dangerous-exec:src/webhook/tailscale.ts",
]);

const OPTIONAL_REVIEWED_PUBLISHABLE_DIST_CRITICAL_FINDINGS = new Set([
  "@eve/acpx:dangerous-exec:dist/mcp-proxy.mjs",
  "@eve/acpx:dangerous-exec:dist/service-<hash>.js",
  "@eve/codex:dangerous-exec:dist/client-<hash>.js",
  "@eve/google-meet:dangerous-exec:dist/index.js",
  "@eve/slack:dynamic-code-execution:dist/outbound-payload.test-harness-<hash>.js",
  "@eve/voice-call:dangerous-exec:dist/runtime-entry-<hash>.js",
]);
...
function stageScannerRelevantPackedFiles(
  ...
  const stageDir = mkdtempSync(join(tmpdir(), "eve-plugin-npm-scan-"));
...
function collectPublishablePluginPackages(): PublishablePluginPackage[] {
  return listPublishablePluginPackageDirs()
    .flatMap((packageDir) => {
      ...
      let packageJson: {
        name?: unknown;
        eve?: { release?: { publishToNpm?: unknown } };
      };
      try {
