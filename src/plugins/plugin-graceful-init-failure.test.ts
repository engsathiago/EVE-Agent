...
const fixtureRoot = makeTrackedTempDir("eve-plugin-graceful", fixtureTempDirs);
let tempDirIndex = 0;
const { loadEVEPlugins, clearPluginLoaderCache } = await import("./loader.js");
...
function readPluginId(pluginPath: string): string {
  const manifestPath = path.join(path.dirname(pluginPath), "eve.plugin.json");
...
describe("graceful plugin initialization failure", () => {
  ...
  it("logs a startup summary grouped by failure phase", async () => {
    ...
    expect(summary).toContain("eve plugins inspect <id> --runtime --json");
    expect(summary).toContain("eve plugins list");
  });
});
