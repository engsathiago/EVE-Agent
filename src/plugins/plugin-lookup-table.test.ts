...
vi.mock("../channels/config-presence.js", () => ({
  ...
  listExplicitlyDisabledChannelIdsForConfig: (config: EVEConfig) =>
...
function createManifestRecord(
  ...
  return {
    ...
    manifestPath: `/plugins/${plugin.id}/eve.plugin.json`,
    ...plugin,
  };
}
...
const indexDiagnostic = {
  level: "warn",
  source: "/plugins/demo/eve.plugin.json",
...
describe("loadPluginLookUpTable", () => {
  beforeEach(() => {
    ...
    listPotentialConfiguredChannelIds
      .mockReset()
      .mockImplementation((config: EVEConfig) => Object.keys(config.channels ?? {}));
  ...
  it("does not reuse lookup tables when channel env presence changes startup scope", async () => {
    listPotentialConfiguredChannelIds.mockImplementation(
      (_config: EVEConfig, env: NodeJS.ProcessEnv) => (env.TELEGRAM_TOKEN ? ["telegram"] : []),
    );
    const plugins = [
      createManifestRecord({
        id: "telegram",
  ...
  it("rebuilds when a provided metadata snapshot has stale plugin paths", async () => {
    ...
    const requestedPlugins = [
      createManifestRecord({
        ...
        manifestPath: "/plugins-moved/telegram/eve.plugin.json",
      }),
    ];
    const config = {
      channels: {
