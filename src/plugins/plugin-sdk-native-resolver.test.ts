...
import {
  ...
  resetEVEPluginSdkNativeResolverForTest,
...
function writeFakeEVEPackage(root: string): { distRoot: string; loaderModulePath: string } {
  ...
  fs.writeFileSync(path.join(root, "eve.mjs"), "#!/usr/bin/env node\n", "utf8");
...
describe("installEVEPluginSdkNativeResolver", () => {
  it("resolves installed plugin SDK imports to the dev source root", () => {
    const stableRoot = fs.mkdtempSync(path.join(os.tmpdir(), "eve-sdk-native-stable-"));
    const devRoot = fs.mkdtempSync(path.join(os.tmpdir(), "eve-sdk-native-dev-source-"));
    const { loaderModulePath } = writeFakeEVEPackage(stableRoot);
    writeFakeEVEPackage(devRoot);
    ...
    const previousDevSourceRoot = process.env.EVE_DEV_SOURCE_ROOT;
    process.env.EVE_DEV_SOURCE_ROOT = devRoot;

    try {
      const installedAliases = installEVEPluginSdkNativeResolver({
        modulePath: loaderModulePath,
        pluginModulePath: externalPluginEntry,
      });

      expect(installedAliases).toContain("eve-agent/plugin-sdk/agent-runtime");
      const requireFromPlugin = createRequire(externalPluginEntry);
      expect(fs.realpathSync(requireFromPlugin.resolve("eve-agent/plugin-sdk/agent-runtime"))).toBe(
        fs.realpathSync(path.join(devRoot, "dist", "plugin-sdk", "agent-runtime.js")),
      );
    } finally {
      if (previousDevSourceRoot === undefined) {
        delete process.env.EVE_DEV_SOURCE_ROOT;
      } else {
        process.env.EVE_DEV_SOURCE_ROOT = previousDevSourceRoot;
      }
    }
  });
  ...
  it("removes stale native SDK aliases when a later dev root omits a subpath", () => {
    ...
    expect(fs.realpathSync(requireFromPlugin.resolve("eve-agent/plugin-sdk/stable-extra"))).toBe(
    ...
    expect(() => requireFromPlugin.resolve("eve-agent/plugin-sdk/stable-extra")).toThrow();
  });

  it("keeps native aliases on JS dist artifacts when source files exist", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "eve-sdk-native-source-resolver-"));
    const { loaderModulePath } = writeFakeEVEPackage(root);
  ...
  it("lets built external plugins resolve EVE SDK subpaths with createRequire", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "eve-sdk-native-resolver-"));
    const { distRoot, loaderModulePath } = writeFakeEVEPackage(root);
    const externalPluginEntry = writeExternalPluginEntry(path.join(root, "external-plugin"));
    ...
    try {
      ...
      const sdk = requireFromPlugin("eve-agent/plugin-sdk/channel-outbound") as {
        defineChannelMessageAdapter?: () => string;
      };

      expect(sdk.defineChannelMessageAdapter?.()).toBe("adapter");
      expect(() => requireFromPlugin.resolve("eve/not-plugin-sdk/channel-message")).toThrow();
    } finally {
      if (process.platform !== "win32") {
  ...
  beforeAll(() => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "eve-sdk-native-esm-resolver-"));
    ...
    fs.writeFileSync(
      ...
      [
        ...
        'import path from "node:path";',
        'import { pathToFileURL } from "node:url";',
        `import { installEVEPluginSdkNativeResolver, resetEVEPluginSdkNativeResolverForTest } from ${JSON.stringify(resolverModuleUrl)};`,
        ...
        "fs.writeFileSync(",
        "  entryPath,",
        '  "import { defineChannelMessageAdapter } from \\"eve-agent/plugin-sdk/channel-outbound\\"; export const eager = defineChannelMessageAdapter(); export const loadLazy = () => import(\\"./lazy.js\\");\\n",',
        '  "utf8",',
        ");",
        "fs.writeFileSync(",
        "  lazyPath,",
        '  "import { defineChannelMessageAdapter } from \\"eve-agent/plugin-sdk/channel-outbound\\"; export const lazy = defineChannelMessageAdapter();\\n",',
        '  "utf8",',
        ");",
        "installEVEPluginSdkNativeResolver({",
        "  modulePath: loaderModulePath,",
  ...
  it("does not resolve SDK aliases for parents outside registered plugin roots", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "eve-sdk-native-guard-"));
    const { loaderModulePath } = writeFakeEVEPackage(root);
    const externalPluginEntry = writeExternalPluginEntry(path.join(root, "external-plugin"));
    const unrelatedRoot = fs.mkdtempSync(path.join(os.tmpdir(), "eve-sdk-native-outside-"));
    ...
    expect(requireFromPlugin.resolve("eve-agent/plugin-sdk/channel-outbound")).toBeTruthy();
    expect(() => requireFromOutside.resolve("eve-agent/plugin-sdk/channel-outbound")).toThrow();
  });

  it("resolves internal core packages only for EVE-owned source parents", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "eve-sdk-native-core-internal-"));
    const { loaderModulePath } = writeFakeEVEPackage(root);
    ...
    expect(installedAliases).toContain("@eve/normalization-core/string-coerce");
    expect(installedAliases).toContain("@eve/media-core/mime");
    expect(installedAliases).toContain("@eve/acp-core/runtime/types");
    expect(installedAliases).toContain("@eve/llm-core");
    const requireFromCoreSource = createRequire(coreSourceParent);
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(
      fs.realpathSync(requireFromCoreSource.resolve("@eve/normalization-core/string-coerce")),
    ).toBe(fs.realpathSync(normalizationSource));
    expect(fs.realpathSync(requireFromCoreSource.resolve("@eve/media-core/mime"))).toBe(
      fs.realpathSync(mediaCoreSource),
    );
    expect(fs.realpathSync(requireFromCoreSource.resolve("@eve/acp-core/runtime/types"))).toBe(
      fs.realpathSync(acpCoreSource),
    );
    expect(fs.realpathSync(requireFromCoreSource.resolve("@eve/llm-core"))).toBe(
      fs.realpathSync(llmCoreSource),
    );
    expect(() => requireFromPlugin.resolve("@eve/normalization-core/string-coerce")).toThrow();
    expect(() => requireFromPlugin.resolve("@eve/media-core/mime")).toThrow();
    expect(() => requireFromPlugin.resolve("@eve/acp-core/runtime/types")).toThrow();
    expect(() => requireFromPlugin.resolve("@eve/llm-core")).toThrow();
  });

  it("does not register source-only SDK subpaths for native resolution", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "eve-sdk-native-source-only-"));
    const { loaderModulePath } = writeFakeEVEPackage(root);
    ...
    expect(installedAliases).toContain("eve-agent/plugin-sdk/channel-outbound");
    expect(installedAliases).not.toContain("eve-agent/plugin-sdk/source-only");
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(() => requireFromPlugin.resolve("eve-agent/plugin-sdk/source-only")).toThrow();
  });

  it("scopes private SSRF SDK aliases to bundled local IPC native parents", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "eve-sdk-native-ssrf-"));
    const { loaderModulePath } = writeFakeEVEPackage(root);
    ...
    expect(installedAliases).toContain("eve-agent/plugin-sdk/ssrf-runtime-internal");
    const requireFromOllama = createRequire(ollamaEntry);
    expect(
      fs.realpathSync(requireFromOllama.resolve("eve-agent/plugin-sdk/ssrf-runtime-internal")),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromRuntimeOllama = createRequire(runtimeOllamaEntry);
    expect(
      fs.realpathSync(
        requireFromRuntimeOllama.resolve("eve-agent/plugin-sdk/ssrf-runtime-internal"),
      ),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromBrowser = createRequire(browserEntry);
    expect(
      fs.realpathSync(requireFromBrowser.resolve("eve-agent/plugin-sdk/ssrf-runtime-internal")),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromRuntimeBrowser = createRequire(runtimeBrowserEntry);
    expect(
      fs.realpathSync(
        requireFromRuntimeBrowser.resolve("eve-agent/plugin-sdk/ssrf-runtime-internal"),
      ),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromOther = createRequire(otherEntry);
    expect(() => requireFromOther.resolve("eve-agent/plugin-sdk/ssrf-runtime-internal")).toThrow();
  });
});
