...
describe("plugin session actions", () => {
  ...
  it("emits plugin-attributed agent events through the plugin API", () => {
    ...
    const { config, registry } = createPluginRegistryFixture();
    let bundledApi: EVEPluginApi | undefined;
    let workspaceApi: EVEPluginApi | undefined;
    registerTestPlugin({
      registry,
      config,
      record: createPluginRecord({
  ...
  it("blocks agent events from stale and non-activating plugin API closures", () => {
    ...
    try {
      ...
      let neverActiveApi: EVEPluginApi | undefined;
      registerTestPlugin({
      ...
      let inactiveApi: EVEPluginApi | undefined;
      registerTestPlugin({
