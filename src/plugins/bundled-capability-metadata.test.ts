  ...
  return listExtensionPackagePaths(extensionsDir)
    .filter((packagePath) => {
      const packageJson = JSON.parse(fs.readFileSync(packagePath, "utf-8")) as {
        eve?: EVEPackageManifest;
      };
      return normalizeBundledPluginStringList(packageJson.eve?.extensions).length > 0;
    })
    .map(
      (packagePath) =>
        JSON.parse(
          fs.readFileSync(path.join(path.dirname(packagePath), "eve.plugin.json"), "utf-8"),
        ) as PluginManifest,
    )
