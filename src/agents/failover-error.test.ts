...
// https://github.com/engsathiago/eve-agent/issues/33785
...
describe("failover-error", () => {
  ...
  it("403 OpenRouter 'Key limit exceeded' returns billing (model fallback trigger)", () => {
    // GitHub: engsathiago/eve-agent#53849 — OpenRouter returns 403 with "Key limit exceeded"
  ...
  describe("isNonProviderRuntimeCoordinationError", () => {
    ...
    const makeEmbeddedTakeoverError = () => {
      const err = new Error(
        "session file changed while embedded prompt lock was released: /tmp/eve/session.jsonl",
      );
...
describe("buildFailoverRemediationHint", () => {
  it("returns a copy-pasteable login command for auth failures", () => {
    ...
    expect(buildFailoverRemediationHint(err)).toBe(
      "Re-authenticate with: eve models auth login --provider 'anthropic' --force",
    );
  });
  ...
  it("returns a hint for auth_permanent as well", () => {
    ...
    expect(buildFailoverRemediationHint(err)).toBe(
      "Re-authenticate with: eve models auth login --provider 'google-gemini-cli' --force",
    );
  });

  it("quotes provider ids that contain shell metacharacters", () => {
    expect(buildProviderReauthCommand("custom;touch /tmp/pwned")).toBe(
      "eve models auth login --provider 'custom;touch /tmp/pwned' --force",
    );
    expect(buildProviderReauthCommand("custom'provider")).toBe(
      "eve models auth login --provider 'custom'\\''provider' --force",
    );
  });
  ...
  it("wraps rendered provider commands in the standard CLI formatter", () => {
    expect(buildProviderReauthCommand("anthropic", { EVE_PROFILE: "work" })).toBe(
      "eve --profile work models auth login --provider 'anthropic' --force",
    );
    expect(buildProviderReauthCommand("anthropic", { EVE_CONTAINER_HINT: "dev" })).toBe(
      "eve --container dev models auth login --provider 'anthropic' --force",
    );
  });
