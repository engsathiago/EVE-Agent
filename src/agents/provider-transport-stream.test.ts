...
describe("provider transport stream contracts", () => {
  it("covers the supported transport api alias matrix", () => {
    // Supported APIs can be projected to EVE transport aliases when needed.
    const cases = [
      {
        ...
        alias: "eve-openai-responses-transport",
      },
      {
      ...
      },
      {
        ...
        alias: "eve-openai-completions-transport",
      },
      {
        ...
        alias: "eve-azure-openai-responses-transport",
      },
      {
        ...
        alias: "eve-anthropic-messages-transport",
      },
      {
        ...
        alias: "eve-google-generative-ai-transport",
  ...
  it("keeps OpenAI API-key default streams on EVE transport", () => {
    const cases = [
    ...
    for (const model of cases) {
      expect(createBoundaryAwareStreamFnForModel(model)).toBeTypeOf("function");
      expect(createEVETransportStreamFnForModel(model)).toBeTypeOf("function");
      expect(createTransportAwareStreamFnForModel(model)).toBeUndefined();
      expect(buildTransportAwareSimpleStreamFn(model)).toBeUndefined();
      expect(prepareTransportAwareSimpleModel(model)).toBe(model);
    }
  ...
  it("routes localService models through the EVE simple-completion transport", () => {
    ...
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
    const preparedModel = prepareTransportAwareSimpleModel(model);
    expect(preparedModel.api).toBe("eve-openai-completions-transport");
    expect(preparedModel.provider).toBe("inferrs");
    expect(preparedModel.id).toBe("google/gemma-4-E2B-it");
  });

  it("keeps Codex defaults on the EVE transport until EVE preserves attribution", () => {
