...
describe("buildAgentSystemPrompt", () => {
  it("resolves helper session keys to scoped prompt surfaces", () => {
    ...
    expect(resolveAgentPromptSurfaceForSessionKey("agent:main")).toBe("eve_main");
    expect(resolveAgentPromptSurfaceForSessionKey(undefined)).toBe("eve_main");
  });
  ...
  it("omits extended sections in minimal prompt mode", () => {
    ...
    expect(prompt).not.toContain("Safety/oversight over completion");
  ...
  it("does not inject an EVE-owned behavioral policy in full prompts", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/eve",
    });
  ...
  it("includes an EVE control section", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/eve",
    });

    expect(prompt).toContain("## EVE Control");
    expect(prompt).toContain("prefer `gateway` tool");
    expect(prompt).toContain("CLI lifecycle only on explicit user request");
    expect(prompt).toContain("eve gateway status|restart|start|stop");
  ...
  it("includes embed guidance only for webchat sessions", () => {
    ...
    expect(prompt).toContain(
      '[embed url="/__eve__/canvas/documents/cv_123/index.html" title="Status" height="320" /]',
    );
    expect(prompt).toContain(
  ...
  it("lists available tools when provided", () => {
    ...
    expect(prompt).toContain("Available tool names are case-sensitive");
  ...
  it("keeps the EVE empty-tool fallback on the main prompt surface", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/eve",
      toolNames: [],
    });

    expect(prompt).toContain("EVE lists the standard tools above");
  ...
  it("omits ACP harness guidance when ACP is disabled", () => {
    ...
    expect(prompt).toContain("- sessions_spawn: Spawn an isolated sub-agent session");
    expect(prompt).toContain("- agents_list: List EVE agent ids allowed for sessions_spawn");
  });
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("Docs: /tmp/eve/docs");
    expect(prompt).toContain(
      "Docs are authoritative for EVE self-knowledge: before understanding how EVE works (memory/daily notes, sessions, tools, Gateway, config, commands, project context), use `Read` or search local docs first; treat AGENTS.md/project context, workspace/profile/memory notes, and `memory_search` as instruction context or user memory, not EVE design/implementation knowledge.",
    );
  });
  ...
  it("includes docs guidance when docsPath is provided", () => {
    ...
    expect(prompt).toContain("Docs: /tmp/eve/docs");
    expect(prompt).toContain("Source: /tmp/eve");
    expect(prompt).toContain(
      "Docs are authoritative for EVE self-knowledge: before understanding how EVE works (memory/daily notes, sessions, tools, Gateway, config, commands, project context), use `read` or search local docs first; treat AGENTS.md/project context, workspace/profile/memory notes, and `memory_search` as instruction context or user memory, not EVE design/implementation knowledge.",
    );
  ...
  it("falls back to public docs and GitHub source guidance when local docs are unavailable", () => {
    ...
    expect(prompt).toContain("Docs: https://docs.eve.ai");
    expect(prompt).toContain("Source: https://github.com/engsathiago/eve-agent");
    expect(prompt).toContain(
  ...
  it("includes bootstrap instructions in system prompt when bootstrap is pending", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/eve",
      bootstrapMode: "full",
      contextFiles: [{ path: "/tmp/eve/BOOTSTRAP.md", content: "Ask who I am." }],
    });
    ...
    expect(prompt).toContain("## /tmp/eve/BOOTSTRAP.md");
  ...
  it("adds ClaudeBot self-update guidance when gateway tool is available", () => {
    ...
    expect(prompt).toContain("## EVE Self-Update");
  ...
  it("describes Telegram rich text only for rich Telegram runtimes", () => {
    ...
    expect(telegramPrompt).toContain("EVE renders Telegram-safe rich messages");
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/eve. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
...
describe("buildAgentBootstrapSystemPromptSections", () => {
  it("can render bootstrap guidance without duplicating Project Context", () => {
    ...
    expect(sections).not.toContain("## /tmp/eve/BOOTSTRAP.md");
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("Do not use `exec` (`eve ...`, `acpx ...`)");
    expect(prompt).toContain("Use `subagents` only for EVE subagents");
