...
} from "@eve/acp-core/runtime/session-identifiers";
...
function resolveHeartbeatConfigForAgent(params: {
  cfg: EVEConfig;
  agentId: string;
}): NonNullable<NonNullable<EVEConfig["agents"]>["defaults"]>["heartbeat"] {
  ...
  if (requested) {
    ...
    if (configuredAgent && !isExplicitlyAllowedAcpAgent(params.cfg, requested)) {
      return {
        ok: false,
        error:
          `agentId "${requested}" is an EVE config agent, not an ACP harness. ` +
          'Use runtime="subagent" or omit runtime for EVE config agents. ' +
...
function isExplicitlyAllowedAcpAgent(cfg: EVEConfig, agentId: string): boolean {
...
function resolveConfiguredAcpSubagentTargetIds(cfg: EVEConfig): string[] {
