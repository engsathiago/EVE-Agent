...
import "./test-helpers/fast-eve-tools-sessions.js";
...
function createEVETools(options?: {
...
describe("sessions tools", () => {
  ...
  it.each([
    ...
    const tool = createEVETools().find((candidate) => candidate.name === "sessions_send");
    if (!tool) {
  ...
  it("sessions_list forwards mailbox filters and includes messages", async () => {
    ...
    const tool = createEVETools().find((candidate) => candidate.name === "sessions_list");
    if (!tool) {
  ...
  it("derives mailbox previews only after agent visibility filtering", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "eve-sessions-list-preview-"));
  ...
  it("sessions_history filters tool messages by default", async () => {
    ...
    const tool = createEVETools().find((candidate) => candidate.name === "sessions_history");
    if (!tool) {
    ...
    expect(details.messages).toContainEqual(
      expect.objectContaining({ provider: "eve", model: "gateway-injected" }),
    );
    expect(details.messages).toContainEqual(
  ...
  it("sessions_history caps oversized payloads and strips heavy fields", async () => {
    ...
    expect(thinkingBlock?.eveReasoningReplay).toBeUndefined();
  });
