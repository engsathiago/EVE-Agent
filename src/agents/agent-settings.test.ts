...
describe("isSilentOverflowProneModel", () => {
  ...
  // eve#75799 reporter's setup: an OpenAI-compatible in-house gateway
  ...
  // string. False positives only disable EVE runtime's secondary compaction path;
  // EVE's preemptive compaction continues to handle real overflow.
...
describe("applyAgentAutoCompactionGuard", () => {
  // Direct repro of eve#75799: shared model runtime's silent-overflow detection misfires
  // on a successful turn against z.ai-style providers, triggering EVE runtime's
  ...
  // state.messages intact; EVE's preemptive compaction continues to
  ...
  // Default-mode runs against ordinary providers must keep EVE runtime's auto-compaction
  // enabled. Disabling it across the board would silently remove EVE runtime's
