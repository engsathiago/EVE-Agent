...
import { type Api, completeSimple, type Model } from "eve-agent/plugin-sdk/llm";
...
let activeLiveCompletionConfig: EVEConfig | undefined;
...
function applyLiveOllamaProviderEnvCompat(params: {
  ...
  const liveBaseUrl = params.env?.EVE_LIVE_OLLAMA_BASE_URL?.trim();
...
function resolveLiveOllamaBaseUrl(model: Pick<Model, "baseUrl">, config?: EVEConfig): string {
  return (
...
describe("explicit live model discovery scope", () => {
  ...
  it("merges explicit model providers with EVE_LIVE_PROVIDERS", () => {
  ...
  it("uses the local Ollama auth marker for self-hosted live env URLs", () => {
    ...
    for (const baseUrl of [
      ...
      const result = applyLiveProviderDiscoveryPluginCompat({
        ...
        providers: ["ollama"],
        env: {
          EVE_LIVE_OLLAMA_BASE_URL: baseUrl,
        },
      });
  ...
  it("replaces configured Ollama auth when live env redirects to a different local endpoint", () => {
    ...
    const result = applyLiveProviderDiscoveryPluginCompat({
      ...
      providers: ["ollama"],
      env: {
        EVE_LIVE_OLLAMA_BASE_URL: "http://127.0.0.1:11434",
      },
    });
...
describeLive("live models (profile keys)", () => {
  it(
    ...
    async () => {
      ...
      const rawModels = process.env.EVE_LIVE_MODELS?.trim();
      ...
      const providers = parseProviderFilter(process.env.EVE_LIVE_PROVIDERS);
      ...
      if (!DIRECT_ENABLED) {
        logProgress(
          "[live-models] skipping (set EVE_LIVE_MODELS=modern|small|all|<list>; all=modern)",
        );
        return;
      }
      ...
      const perModelTimeoutMs = toInt(process.env.EVE_LIVE_MODEL_TIMEOUT_MS, 30_000);
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[live-models] capped to ${selectedCandidates.length}/${candidates.length} via EVE_LIVE_MAX_MODELS=${maxModels}`,
        );
      }
