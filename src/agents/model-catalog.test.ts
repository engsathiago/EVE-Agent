...
let prepareEVEModelsJsonSourceMock: ReturnType<typeof vi.fn>;
...
describe("loadModelCatalog", () => {
  beforeAll(async () => {
    ...
    prepareEVEModelsJsonSourceMock = vi.fn().mockResolvedValue({
      agentDir: "/tmp/eve",
    ...
    vi.doMock("./models-config.js", () => ({
      ...
      prepareEVEModelsJsonSource: prepareEVEModelsJsonSourceMock,
    }));
    ...
    vi.doMock("./agent-scope.js", () => ({
      resolveAgentWorkspaceDir: (cfg: EVEConfig, agentId: string) => {
        const entry = cfg.agents?.list?.find((entryEntry) => entryEntry.id === agentId);
        return entry?.workspace ?? cfg.agents?.defaults?.workspace ?? "/tmp/eve-workspace";
      },
      resolveDefaultAgentDir: () => "/tmp/eve",
      resolveDefaultAgentId: (cfg: EVEConfig) =>
  ...
  beforeEach(() => {
    ...
    prepareEVEModelsJsonSourceMock.mockReset();
    prepareEVEModelsJsonSourceMock.mockResolvedValue({
      agentDir: "/tmp/eve",
      fingerprint: "source-fingerprint",
      workspaceDir: "/tmp/eve-workspace",
      wrote: false,
  ...
  it("uses the state cached catalog before runtime discovery", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as EVEConfig });
    ...
    expect(prepareEVEModelsJsonSourceMock).not.toHaveBeenCalled();
  ...
  it("bypasses the state cached catalog when a refresh is requested", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as EVEConfig, useCache: false });
  ...
  it("does not publish a catalog load from an invalidated generation", async () => {
    ...
    const staleLoad = loadModelCatalog({ config: {} as EVEConfig });
    resetModelCatalogCache();
    await expect(loadModelCatalog({ config: {} as EVEConfig })).resolves.toBe(freshCatalog);
  ...
  it("misses the state cached catalog when source freshness changes", async () => {
    ...
    await expect(loadModelCatalog({ config: {} as EVEConfig })).resolves.toEqual([
      { id: "cached-stale", name: "Cached Stale", provider: "openai" },
    ]);
    resetModelCatalogCacheForTest();
    mockAgentDiscoveryModels([{ id: "fresh-fast", name: "Fresh Fast", provider: "openai" }]);
  ...
  it("reloads dynamic registry entries after clearing the cache", async () => {
    ...
    const first = await loadModelCatalog({ config: {} as EVEConfig });
    expect(first).toStrictEqual([
      {
        id: "existing",
        name: "Existing",
    ...
    const second = await loadModelCatalog({ config: {} as EVEConfig });
  ...
  it("filters suppressed built-ins from persisted read-only catalog rows", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as EVEConfig, readOnly: true });
  ...
  it("loads generated plugin catalog rows in read-only mode", async () => {
    const catalogPath = "/tmp/eve/plugins/read-only-shard/catalog.json";
    mkdirSync("/tmp/eve/plugins/read-only-shard", { recursive: true });
    ...
    try {
      ...
      rmSync("/tmp/eve/plugins/read-only-shard", { recursive: true, force: true });
    }
  });
  ...
  it("loads manifest catalog rows from the current metadata snapshot without provider runtime", () => {
    ...
    const result = loadManifestModelCatalog({ config: {} as EVEConfig });
  ...
  it("passes explicit env when checking current manifest catalog snapshot compatibility", () => {
    const env = { HOME: "/tmp/eve-model-catalog-env" } as NodeJS.ProcessEnv;
