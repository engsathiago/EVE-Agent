...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousEVEAgentDir: string | undefined;
  const originalFetch = globalThis.fetch;
  const shouldResetPluginLoaderState = opts?.resetPluginLoaderState !== false;

  beforeEach(() => {
    previousHome = process.env.HOME;
    previousEVEAgentDir = process.env.EVE_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousEVEAgentDir === undefined) {
      delete process.env.EVE_AGENT_DIR;
    } else {
      process.env.EVE_AGENT_DIR = previousEVEAgentDir;
    }
    clearRuntimeConfigSnapshot();
    clearConfigCache();
    if (shouldResetPluginLoaderState) {
...
export const MODELS_CONFIG_IMPLICIT_ENV_VARS = [
  "EVE_TEST_ONLY_PROVIDER_PLUGIN_IDS",
  "VITEST",
  "NODE_ENV",
  "AI_GATEWAY_API_KEY",
...
export const CUSTOM_PROXY_MODELS_CONFIG: EVEConfig = {
  models: {
    providers: {
      "custom-proxy": {
