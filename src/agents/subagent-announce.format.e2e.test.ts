...
function setConfigOverride(next: EVEConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/engsathiago/eve-agent/issues/31298
  ...
  beforeEach(() => {
    vi.useRealTimers();
    // EVE_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("EVE runtime context (internal):");
