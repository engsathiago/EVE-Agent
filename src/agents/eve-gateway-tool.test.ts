// Verifies EVE gateway tool schema, restart signaling, and config mutations.
...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "eve-test-"));

    try {
      await withEnvAsync({ EVE_STATE_DIR: stateDir, EVE_PROFILE: "isolated" }, async () => {
        const tool = requireGatewayTool();
          ...
          config: {
            tools: {
              exec: {
                ...
                safeBinTrustedDirs: ["/tmp/eve-bin"],
  ...
  it("rejects config.apply when it changes safe bin trusted directories", async () => {
    ...
    await expect(
      tool.execute("call-protected-safe-bin-trust-apply", {
        action: "config.apply",
        raw: '{ tools: { exec: { ask: "on-miss", security: "allowlist", safeBinTrustedDirs: ["/tmp/eve-bin"] } } }',
      }),
    ).rejects.toThrow(
