...
import type { UserMessage } from "eve-agent/plugin-sdk/llm";
...
vi.mock("eve-agent/plugin-sdk/agent-sessions", async () => {
  const actual = await vi.importActual<typeof import("eve-agent/plugin-sdk/agent-sessions")>(
    "eve-agent/plugin-sdk/agent-sessions",
  );
  return {
    ...actual,
