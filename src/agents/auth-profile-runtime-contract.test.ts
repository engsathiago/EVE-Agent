...
async function runAuthContractAttempt(params: {
  ...
  const cfg = params.cfg ?? ({} as EVEConfig);
...
describe("Auth profile runtime contract - embedded EVE and CLI adapter", () => {
  let tmpDir: string;
  ...
  it("forwards a legacy OpenAI Codex auth profile through the embedded EVE path", async () => {
    await runAuthContractAttempt({
      tmpDir,
      storePath,
  ...
  it("forwards an OpenAI auth profile through the explicit embedded OpenAI EVE path", async () => {
    await runAuthContractAttempt({
      tmpDir,
      storePath,
      ...
      authProfileProvider: AUTH_PROFILE_RUNTIME_CONTRACT.openAiProvider,
      authProfileOverride: AUTH_PROFILE_RUNTIME_CONTRACT.openAiProfileId,
      cfg: providerRuntimeConfig(AUTH_PROFILE_RUNTIME_CONTRACT.openAiProvider, "eve"),
    });
  ...
  it("routes explicit OpenAI EVE runs with legacy Codex OAuth through OpenAI transport", async () => {
    await runAuthContractAttempt({
      tmpDir,
      storePath,
      providerOverride: AUTH_PROFILE_RUNTIME_CONTRACT.openAiProvider,
