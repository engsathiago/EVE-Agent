...
function stripUnsafeBlockedRunMetadata(messages: AgentMessage[]): AgentMessage[] {
  ...
  const nextMessages = messages.map((message) => {
    const eve = (message as unknown as Record<string, unknown>)["__eve"];
    if (!eve || typeof eve !== "object") {
      return message;
    }
    const beforeAgentRunBlocked = (eve as { beforeAgentRunBlocked?: unknown })
