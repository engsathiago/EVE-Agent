// Runtime-context prompt tests keep hidden EVE context separate from the
...
describe("runtime context prompt submission", () => {
  ...
  it("ignores repeated inline marker mentions without recursive stack growth", () => {
    ...
    const inlineMarkers = Array.from(
      { length: 250 },
      () => "inline <<<BEGIN_EVE_INTERNAL_CONTEXT>>> marker",
    ).join("\n");
    const effectivePrompt = [
      inlineMarkers,
      "",
    ...
    expect(parts.modelPrompt).toContain("inline <<<BEGIN_EVE_INTERNAL_CONTEXT>>> marker");
  ...
  it("uses a marker prompt for runtime-only events", () => {
    ...
    expect(parts).toEqual({
      prompt: "Continue the EVE runtime event.",
  ...
  it("labels runtime-only events as system context", () => {
    ...
    expect(parts.runtimeSystemContext).toContain("EVE runtime event.");
