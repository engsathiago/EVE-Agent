...
describe("skill_workshop tool", () => {
  it("is exposed in the EVE tool set", async () => {
    const workspaceDir = await tempDirs.make("eve-skill-workshop-tool-");
  ...
  it("is not exposed from sandboxed EVE tool sets", async () => {
    const workspaceDir = await tempDirs.make("eve-skill-workshop-tool-");
    const tools = createEVETools({
      workspaceDir,
      config: {},
  ...
  it("scopes proposal discovery to the tool workspace", async () => {
    const firstWorkspaceDir = await tempDirs.make("eve-skill-workshop-tool-first-");
    const secondWorkspaceDir = await tempDirs.make("eve-skill-workshop-tool-second-");
