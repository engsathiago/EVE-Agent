// Covers locating EVE docs and source paths from package roots.
...
describe("resolveEVEDocsPath", () => {
  it("uses the workspace docs directory when it has canonical docs metadata", async () => {
    const root = await makePackageRoot("eve-docs-workspace-");
    await writeDocsJson(root);

    await expect(resolveEVEReferencePaths({ workspaceDir: root })).resolves.toMatchObject({
      docsPath: path.join(root, "docs"),
    });
  });

  it("finds bundled package docs from a nested package path", async () => {
    const root = await makePackageRoot("eve-docs-package-");
    await writeDocsJson(root);
    const nested = path.join(root, "dist", "agents");
    await fs.mkdir(nested, { recursive: true });

    await expect(resolveEVEReferencePaths({ cwd: nested })).resolves.toMatchObject({
      docsPath: path.join(root, "docs"),
    });
  });
  ...
  it("does not accept incomplete template-only docs directories", async () => {
    ...
    const root = await makePackageRoot("eve-docs-incomplete-");
    await fs.mkdir(path.join(root, "docs", "reference", "templates"), { recursive: true });

    await expect(resolveEVEReferencePaths({ cwd: root })).resolves.toMatchObject({
      docsPath: null,
    });
  });
});

describe("resolveEVESourcePath", () => {
  it("returns the package root only for git checkouts", async () => {
    const root = await makePackageRoot("eve-source-git-");
  ...
  it("omits source path for npm-style package installs", async () => {
    ...
    const root = await makePackageRoot("eve-source-npm-");
...
describe("resolveEVEReferencePaths", () => {
  it("returns docs and local source together for git checkouts", async () => {
    const root = await makePackageRoot("eve-reference-git-");
    await writeDocsJson(root);
    await fs.mkdir(path.join(root, ".git"));

    await expect(resolveEVEReferencePaths({ cwd: root })).resolves.toEqual({
      docsPath: path.join(root, "docs"),
      sourcePath: root,
    });
  });
