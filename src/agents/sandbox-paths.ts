...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const eveTmpDir = path.resolve(resolvePreferredEVETmpDir());
  if (!isPathInside(eveTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: eveTmpDir });
  return resolved;
}
