...
async function writeForeignNativeHookRelayBridgeRecordForTests(
  ...
  // without starting a second EVE process in the unit test.
...
function getNativeHookRelaySharedStateForTests(): NativeHookRelaySharedStateForTests {
  ...
  )[Symbol.for("eve.nativeHookRelay.state")];
  if (!state) {
...
describe("native hook relay registry", () => {
  it("registers a short-lived relay and builds hidden CLI commands", () => {
    const relay = registerNativeHookRelay({
      ...
      ttlMs: 10_000,
      command: {
        executable: "/opt/Open Claw/eve.mjs",
    ...
    expect(relay.commandForEvent("pre_tool_use")).toBe(
      "/usr/local/bin/node '/opt/Open Claw/eve.mjs' hooks relay --provider codex --relay-id " +
  ...
  it("uses the Codex no-op output when no EVE hook decides", async () => {
    const relay = registerNativeHookRelay({
      provider: "codex",
      sessionId: "session-1",
      runId: "run-1",
  ...
  it("maps Codex PreToolUse to EVE before_tool_call and blocks before execution", async () => {
  ...
  it("normalizes Codex exec_command cmd input before running EVE policy", async () => {
    const beforeToolCall = vi.fn(async () => ({
      block: true,
  ...
  it("normalizes Codex exec_command argv cmd input before running EVE policy", async () => {
    const beforeToolCall = vi.fn(async () => ({
      block: true,
  ...
  it("passes config to trusted policies for native pre-tool session extension reads", async () => {
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "eve-native-relay-policy-"));
  ...
  it("uses the Codex cwd when deriving apply_patch paths for PreToolUse", async () => {
    ...
    const cwd = path.join("/tmp", "eve-native-hook-cwd");
  ...
  it("maps Codex PostToolUse to EVE after_tool_call observation", async () => {
  ...
  it("maps Codex MCP PreToolUse to EVE before_tool_call and can block", async () => {
    const beforeToolCall = vi.fn(async () => ({
      block: true,
    ...
    const response = await invokeNativeHookRelay({
      ...
      rawPayload: {
        ...
        tool_input: {
          entities: [{ name: "EVE", entityType: "project", observations: ["test"] }],
        },
      },
    });
  ...
  it("lets security-style plugins block native MCP calls by scanning tool params", async () => {
    ...
    const response = await invokeNativeHookRelay({
      ...
      rawPayload: {
        ...
        tool_input: {
          command: "rm -rf /tmp/eve-important-state",
        },
      },
    });
  ...
  it("maps Codex MCP PostToolUse to EVE after_tool_call observation", async () => {
    const afterToolCall = vi.fn();
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "after_tool_call", handler: afterToolCall }]),
    );
    ...
    const response = await invokeNativeHookRelay({
      ...
      rawPayload: {
        ...
        tool_response: {
          content: [{ type: "text", text: '{ "name": "eve" }' }],
  ...
  it("routes Codex MCP PermissionRequest payloads through EVE approval policy", async () => {
    const relay = registerNativeHookRelay({
      provider: "codex",
      agentId: "agent-1",
      sessionId: "session-1",
  ...
  it("defers PermissionRequest when EVE approval does not decide", async () => {
  ...
  it("does not reuse pending PermissionRequest approvals when a tool call id is reused with different input", async () => {
    ...
    const second = invokeNativeHookRelay({
      ...
      rawPayload: {
        ...
        tool_name: "Bash",
        tool_use_id: "reused-call-id",
        tool_input: { command: "rm -rf /tmp/eve-important-state" },
      },
    });
...
describe("native hook relay command builder", () => {
  it("uses the Codex hook relay command shape", () => {
    expect(
      ...
      "eve hooks relay --provider codex --relay-id relay-1 --generation generation-1 --event permission_request --timeout 5000",
    );
  });
  ...
  it("includes explicit unavailable noop mode only for PreToolUse", () => {
    expect(
      ...
      "eve hooks relay --provider codex --relay-id relay-1 --generation generation-1 --event pre_tool_use --pre-tool-use-unavailable noop --timeout 5000",
    );
  });
});
