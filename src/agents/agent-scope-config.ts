...
export function listAgentEntries(cfg: EVEConfig): AgentEntry[] {
...
export function listAgentIds(cfg: EVEConfig): string[] {
...
export function resolveDefaultAgentId(cfg: EVEConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: EVEConfig, agentId: string): AgentEntry | undefined {
