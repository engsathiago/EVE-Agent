/** Tests projecting EVE user MCP servers into Codex app-server config. */
...
describe("buildCodexUserMcpServersThreadConfigPatch", () => {
  ...
  it("filters Codex-scoped user MCP servers by EVE agent id", () => {
    // Agent-scoped MCP servers should follow the active EVE agent, while
  ...
  it("omits scoped Codex MCP servers when no EVE agent id is available", () => {
    const patch = buildCodexUserMcpServersThreadConfigPatch({
      mcp: {
        servers: {
          atlas: {
