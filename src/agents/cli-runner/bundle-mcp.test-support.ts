...
export function setupCliBundleMcpTestHarness(): void {
  beforeAll(async () => {
    ...
    envSnapshot = captureEnv(["EVE_BUNDLED_PLUGINS_DIR"]);
    bundleProbeHomeDir = await tempHarness.createTempDir("eve-cli-bundle-mcp-home-");
    bundleProbeWorkspaceDir = await tempHarness.createTempDir("eve-cli-bundle-mcp-workspace-");
    const emptyBundledDir = await tempHarness.createTempDir("eve-cli-bundle-mcp-bundled-");
    setTestEnvValue("EVE_BUNDLED_PLUGINS_DIR", emptyBundledDir);
...
function createEnabledBundleProbeConfig(): EVEConfig {
  return {
    plugins: {
      entries: {
