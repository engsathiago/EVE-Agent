...
const LIVE = isLiveTestEnabled(["EVE_LIVE_CLI_MCP_GEMINI"]);
...
async function startLocalStreamableHttpMcpServer(): Promise<{
  ...
  const mcpServer = new McpServer({ name: "eve-gemini-live-probe", version: "1.0.0" });
  mcpServer.tool("eve_live_probe", "EVE Gemini MCP live probe", async () => ({
...
describeLive("Gemini CLI MCP settings smoke", () => {
  it("connects to an EVE-configured streamable-http server", async () => {
    const geminiCommand = process.env.EVE_LIVE_GEMINI_COMMAND ?? "gemini";
    ...
    try {
      ...
      expect(output).toContain("eveLiveProbe");
