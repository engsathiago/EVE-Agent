...
const CAPTURE_MARKER = "__EVE_SHELL_SNAPSHOT_CAPTURE__";
const ENV_MARKER = "__EVE_SHELL_SNAPSHOT_ENV__";
...
async function captureShellSnapshot(opts: ShellSnapshotWrapOptions): Promise<string | null> {
  ...
  const captureOutputDir = await fs.mkdtemp(path.join(os.tmpdir(), "eve-shell-snapshot-"));
...
function buildTrustedSnapshotCaptureEnv(
  ...
  // EVE_SHELL is injected by the exec runtime, so startup files can keep
  // their documented exec-specific branches without trusting model input.
  if (runtimeEnv.EVE_SHELL === "exec") {
    env.EVE_SHELL = "exec";
  }
  return env;
}
