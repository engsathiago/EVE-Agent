...
describe("host tool tilde expansion (non-workspace mode)", () => {
  ...
  it("edit readFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("eve-tilde-test-edit-");
  ...
  it("write writeFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("eve-tilde-test-write-");
  ...
  it("write mkdir expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("eve-tilde-test-mkdir-");
  ...
  it("ignores EVE_HOME for write operations", async () => {
    const eveHome = await createTempDir("eve-home-override-", os.tmpdir());
    const dir = await createTempDir("eve-tilde-test-write-");
    const testFile = path.join(dir, "os-home-write.txt");

    await withEnvAsync({ EVE_HOME: eveHome }, async () => {
      createHostWorkspaceWriteTool(eveHome, { workspaceOnly: false });
      ...
      await expectMissingPath(fs.access(path.join(eveHome, path.basename(testFile))));
    });
  });

  it("ignores EVE_HOME for mkdir operations", async () => {
    const eveHome = await createTempDir("eve-home-override-", os.tmpdir());
    const dir = await createTempDir("eve-tilde-test-mkdir-");
    ...
    await withEnvAsync({ EVE_HOME: eveHome }, async () => {
      ...
      expect((await fs.stat(newDir)).isDirectory()).toBe(true);
      await expectMissingPath(fs.access(path.join(eveHome, path.basename(newDir))));
    });
  });

  it("ignores EVE_HOME for readFile operations", async () => {
    const eveHome = await createTempDir("eve-home-override-", os.tmpdir());
    const dir = await createTempDir("eve-tilde-test-edit-");
    ...
    await withEnvAsync({ EVE_HOME: eveHome }, async () => {
      createHostWorkspaceEditTool(eveHome, { workspaceOnly: false });
      const content = await readEditOps().readFile(toTildePath(testFile));
  ...
  it("ignores EVE_HOME for access operations", async () => {
    const eveHome = await createTempDir("eve-home-override-", os.tmpdir());
    const dir = await createTempDir("eve-tilde-test-edit-");
