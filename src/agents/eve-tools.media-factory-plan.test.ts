...
import { resolveOptionalMediaToolFactoryPlan } from "./eve-tools.media-factory-plan.js";
...
async function createEVEToolsForTest(options?: CreateEVEToolsOptions) {
  return createEVEToolsForTestModule(options);
}
...
function createPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/plugins/${params.id}/eve.plugin.json`,
    channels: [],
    providers: [],
    cliBackends: [],
    skills: [],
...
function legacyModelProviderConfig(provider: Record<string, unknown>): EVEConfig {
  return {
    models: {
      providers: {
...
describe("optional media tool factory planning", () => {
  ...
  it("applies global tool policy before optional media factories run", () => {
    const config: EVEConfig = { tools: { deny: ["pdf"] } };
    installSnapshot(config, [
      createPlugin({
        id: "media-owner",
        contracts: { mediaUnderstandingProviders: ["anthropic"] },
  ...
  it("defers PDF model resolution from the tool-prep hot path", async () => {
    ...
    const tools = await createEVEToolsForTest({
      config,
      agentDir: "/tmp/eve-agent-main",
  ...
  it("does not register the image tool without cheap vision availability evidence", async () => {
    const config: EVEConfig = {};
    const workspaceDir = "/tmp/eve-workspace";
