...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): EVEConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCopilotConfig = (): EVEConfig =>
  ({
    models: {
      providers: {
        "github-copilot": {
...
async function withTimedAgentWorkspace<T>(
  ...
  try {
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "eve-workspace-"));
    const now = Date.now();
...
describe("runEmbeddedAgent auth profile rotation", () => {
  ...
  it("logs structured failover decision metadata for overloaded assistant rotation", async () => {
    ...
    setLoggerOverrideFn({
      level: "trace",
      consoleLevel: "silent",
      file: path.join(os.tmpdir(), `eve-auth-rotation-${Date.now()}.log`),
    });
