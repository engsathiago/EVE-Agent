/**
 * Broad coverage for createEVECodingTools.
...
import * as evePluginTools from "./eve-plugin-tools.js";
...
type EVECodingTool = ReturnType<typeof createEVECodingTools>[number];
type EVEToolsOptions = NonNullable<Parameters<typeof createEVETools>[0]>;

function toolNameList(tools: readonly { name: string }[]): string[] {
  return tools.map((tool) => tool.name);
}

function requireTool(tools: EVECodingTool[], name: string): EVECodingTool {
...
function requireToolExecute(tool: EVECodingTool): NonNullable<EVECodingTool["execute"]> {
  if (!tool.execute) {
...
function latestCreateEVEToolsOptions(): EVEToolsOptions {
  const calls = vi.mocked(createEVETools).mock.calls;
  const lastCall = calls.at(-1);
  const options = lastCall?.[0];
  if (!options) {
    throw new Error("expected createEVETools call");
  }
  return options;
}
...
function cronCreatorToolNames(
  list: EVEToolsOptions["cronCreatorToolAllowlist"] | undefined,
): string[] | undefined {
  return list?.map((entry) => (typeof entry === "string" ? entry : entry.name));
}

describe("createEVECodingTools", () => {
  ...
  it("exposes gateway config and restart actions to owner sessions", () => {
    const tools = createEVECodingTools({ config: testConfig });
  ...
  it("passes explicit hook channel ids to wrapped tool hooks", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "eve-hook-channel-"));
  ...
  it("re-wraps existing before_tool_call hooks once with the current context", async () => {
    ...
    vi.mocked(createEVETools).mockReturnValueOnce([wrapped as never]);

    const tools = createEVECodingTools({ agentId: "main", sessionId: "session-new" });
  ...
  it("keeps Tool Search controls when core EVE tools are not materialized", () => {
    ...
    expect(createEVEToolsMock).not.toHaveBeenCalled();
    expect(names.has("tool_search_code")).toBe(true);
    expect(names.has("tool_search")).toBe(true);
    expect(names.has("tool_describe")).toBe(true);
    expect(names.has("tool_call")).toBe(true);
  ...
  it("uses runtime toolsAllow when materializing plugin tools", () => {
    ...
    expect(createEVEToolsMock).toHaveBeenCalledTimes(1);
    const options = latestCreateEVEToolsOptions();
  ...
  it("preserves runtime allowlist groups containing message through restrictive profiles", () => {
    for (const runtimeToolAllowlist of [["group:messaging"], ["group:eve"], ["*"]]) {
      const tools = createEVECodingTools({
        config: { tools: { profile: "minimal" } },
  ...
  it("passes source reply delivery mode to EVE tool construction", () => {
    const createEVEToolsMock = vi.mocked(createEVETools);
    createEVEToolsMock.mockClear();
    ...
    expect(createEVEToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateEVEToolsOptions().sourceReplyDeliveryMode).toBe("message_tool_only");
  });
  ...
  it("passes plugin suppression into EVE tool construction plans", () => {
    const createEVEToolsMock = vi.mocked(createEVETools);
    createEVEToolsMock.mockClear();
    ...
    expect(createEVEToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateEVEToolsOptions().disablePluginTools).toBe(true);
  });

  it("keeps plugin-only construction off the EVE core factory", () => {
    const createEVEToolsMock = vi.mocked(createEVETools);
    createEVEToolsMock.mockClear();
  ...
  it("forwards active model metadata to plugin-only tool construction", () => {
    ...
    const resolvePluginToolsSpy = vi
      .spyOn(evePluginTools, "resolveEVEPluginToolsForOptions")
      .mockReturnValue([]);
  ...
  it("uses tools.alsoAllow for optional plugin discovery without widening to all plugins", () => {
    ...
    expect(createEVEToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateEVEToolsOptions().pluginToolAllowlist).toStrictEqual([
      "lobster",
      DEFAULT_PLUGIN_TOOLS_ALLOWLIST_ENTRY,
    ]);
  });

  it("passes explicit denylist entries to EVE tool factory planning", () => {
    const createEVEToolsMock = vi.mocked(createEVETools);
    createEVEToolsMock.mockClear();
    ...
    expect(createEVEToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateEVEToolsOptions().pluginToolDenylist, ["pdf"]);
  });

  it("passes inherited allowlist entries to EVE plugin discovery", async () => {
    const createEVEToolsMock = vi.mocked(createEVETools);
    createEVEToolsMock.mockClear();
    ...
    expect(createEVEToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateEVEToolsOptions().pluginToolAllowlist, [
      "custom_plugin_tool",
      "sessions_spawn",
    ]);
  });
  ...
  it("passes effective allow-list-restricted tool surface to spawned sessions", () => {
    ...
    expect(createEVEToolsMock).toHaveBeenCalledTimes(1);
    const inheritedAllow = latestCreateEVEToolsOptions().inheritedToolAllowlist;
  ...
  it("passes group-restricted tool surface to cron-created agent turns", () => {
    ...
    expect(createEVEToolsMock).toHaveBeenCalledTimes(1);
    const cronAllow = latestCreateEVEToolsOptions().cronCreatorToolAllowlist;
  ...
  it("preserves action enums in normalized schemas", () => {
    const defaultTools = createEVECodingTools({ config: testConfig });
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const openAiTools = createEVECodingTools({
      config: testConfig,
    ...
    const codexTools = createEVECodingTools({
      config: testConfig,
      modelProvider: "openai",
      modelId: "gpt-5.4",
    });
    ...
    const disabledConfig: EVEConfig = {
      tools: {
        exec: {
    ...
    const disabledOpenAiTools = createEVECodingTools({
    ...
    const anthropicTools = createEVECodingTools({
      config: disabledConfig,
    ...
    const allowModelsConfig: EVEConfig = {
      tools: {
        exec: {
    ...
    const allowed = createEVECodingTools({
    ...
    const denied = createEVECodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
    ...
    const oauthTools = createEVECodingTools({
      config: testConfig,
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createEVECodingTools({ messageProvider: "discord" });
    const names = new Set(tools.map((tool) => tool.name));
  ...
  it("separates the canonical message provider from transport tool policy", () => {
    vi.mocked(createEVETools).mockClear();
  ...
  it("keeps browser out of coding-profile subagents unless profile-stage alsoAllow adds it", () => {
    ...
    const codingSubagent = createEVECodingTools({
      sessionKey: "agent:main:subagent:test",
      config: baseConfig,
    });
    ...
    const subagentAllowOnly = createEVECodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
        tools: {
    ...
    const profileStageAlsoAllow = createEVECodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
  ...
  it("can keep message available when a cron route needs it under the coding profile", () => {
    const codingTools = createEVECodingTools({
      config: { tools: { profile: "coding" } },
    });
    expect(toolNameList(codingTools)).not.toContain("message");

    const cronTools = createEVECodingTools({
      config: { tools: { profile: "coding" } },
      forceMessageTool: true,
    });
  ...
  it("can keep message available when a cron route needs it under a provider coding profile", () => {
    const providerProfileTools = createEVECodingTools({
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createEVECodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createEVECodingTools({
      modelProvider: "xai",
      modelCompat: {
  ...
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    const defaultTools = createEVECodingTools();
    const readTool = requireTool(defaultTools, "read");
    const readExecute = requireToolExecute(readTool);

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "eve-read-"));
    try {
      ...
      const contents = "Hello from eve read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "eve-sandbox");
    const sandbox = createAgentToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "eve-workspace"),
    ...
    const tools = createEVECodingTools({ sandbox });
  ...
  it("accepts canonical parameters for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "eve-canonical-"));
    try {
      const tools = createEVECodingTools({ workspaceDir: tmpDir });
  ...
  it("roots memory flush append-only writes in the workspace when cwd differs", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "eve-memory-workspace-"));
    const taskCwd = await fs.mkdtemp(path.join(os.tmpdir(), "eve-memory-cwd-"));
  ...
  it("rejects legacy alias parameters", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "eve-legacy-alias-"));
    try {
      const tools = createEVECodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("rejects structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "eve-structured-write-"));
    try {
      const tools = createEVECodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects structured edit payloads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "eve-structured-edit-"));
    try {
