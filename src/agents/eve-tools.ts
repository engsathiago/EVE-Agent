/**
 * EVE built-in and plugin tool assembly.
 *
...
} from "./eve-tools.media-factory-plan.js";
import { applyNodesToolWorkspaceGuard } from "./eve-tools.nodes-workspace-guard.js";
import {
  collectPresentEVETools,
  shouldIncludeUpdatePlanToolForEVETools,
} from "./eve-tools.registration.js";
...
const defaultEVEToolsDeps: EVEToolsDeps = {
  callGateway,
};

let eveToolsDeps: EVEToolsDeps = defaultEVEToolsDeps;
...
export function createEVETools(
  ...
  const resolvedConfig = options?.config ?? eveToolsDeps.config;
  ...
  options?.recordToolPrepStage?.("eve-tools:session-workspace");
  ...
  const imageTool = resolveImageToolFactoryAvailable({
    ...
    : null;
  options?.recordToolPrepStage?.("eve-tools:image-tool");
  const imageGenerateTool = optionalMediaTools.imageGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("eve-tools:image-generate-tool");
  const videoGenerateTool = optionalMediaTools.videoGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("eve-tools:video-generate-tool");
  const musicGenerateTool = optionalMediaTools.musicGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("eve-tools:music-generate-tool");
  const pdfTool =
    optionalMediaTools.pdf && options?.agentDir?.trim()
      ...
      : null;
  options?.recordToolPrepStage?.("eve-tools:pdf-tool");
  ...
  options?.recordToolPrepStage?.("eve-tools:web-search-tool");
  ...
  options?.recordToolPrepStage?.("eve-tools:web-fetch-tool");
  ...
  options?.recordToolPrepStage?.("eve-tools:message-tool");
  ...
  options?.recordToolPrepStage?.("eve-tools:nodes-tool");
  ...
  const tools: AnyAgentTool[] = [
    ...
    ...collectPresentEVETools([heartbeatTool]),
    createTtsTool({
      agentChannel: options?.agentChannel,
      config: resolvedConfig,
      agentId: sessionAgentId,
    ...
    ...collectPresentEVETools([imageGenerateTool, musicGenerateTool, videoGenerateTool]),
    ...(embedded
      ? []
      : [
          createGatewayTool({
    ...
    ...(embedded
      ...
      : [
          createSessionsSendTool({
            ...
            sandboxed: options?.sandboxed,
            config: resolvedConfig,
            callGateway: eveToolsDeps.callGateway,
          }),
        ]),
    ...
    ...collectPresentEVETools([webSearchTool, webFetchTool, imageTool, pdfTool]),
  ];
  options?.recordToolPrepStage?.("eve-tools:core-tool-list");
  let allTools = tools;
  if (!options?.disablePluginTools) {
    ...
    options?.recordToolPrepStage?.("eve-tools:plugin-tools");
  }
  ...
  options?.recordToolPrepStage?.("eve-tools:tool-hooks");
...
export const testing = {
  resolveOptionalMediaToolFactoryPlan,
  setDepsForTest(overrides?: Partial<EVEToolsDeps>) {
    eveToolsDeps = overrides
      ? {
          ...defaultEVEToolsDeps,
          ...overrides,
        }
