/** Tests merging bundled MCP defaults with EVE user MCP configuration. */
...
describe("loadMergedBundleMcpConfig", () => {
  it("lets EVE mcp.servers override bundle defaults while preserving raw transport shape", () => {
  ...
  it("maps EVE transports to downstream CLI types when requested", () => {
    expect(
  ...
  it("keeps disabled EVE MCP servers out of embedded runtimes", () => {
    const merged = loadMergedBundleMcpConfig({
      workspaceDir: "/workspace",
      cfg: {
        mcp: {
  ...
  it("lets disabled EVE MCP servers tombstone bundle defaults with the same name", () => {
    const merged = loadMergedBundleMcpConfig({
      workspaceDir: "/workspace",
      cfg: {
        mcp: {
