...
import type { AssistantMessage, Message, Tool } from "eve-agent/plugin-sdk/llm";
...
describeCacheLive("embedded agent runner prompt caching (live)", () => {
  beforeAll(async () => {
    liveRunnerRootDir = await fs.mkdtemp(path.join(os.tmpdir(), "eve-live-cache-"));
    ...
    previousCacheTraceEnv = {
      enabled: process.env.EVE_CACHE_TRACE,
      file: process.env.EVE_CACHE_TRACE_FILE,
      messages: process.env.EVE_CACHE_TRACE_MESSAGES,
      prompt: process.env.EVE_CACHE_TRACE_PROMPT,
      system: process.env.EVE_CACHE_TRACE_SYSTEM,
    };
    setTestEnvValue("EVE_CACHE_TRACE", "1");
    setTestEnvValue("EVE_CACHE_TRACE_FILE", liveCacheTraceFile);
    setTestEnvValue("EVE_CACHE_TRACE_MESSAGES", "0");
    setTestEnvValue("EVE_CACHE_TRACE_PROMPT", "0");
    setTestEnvValue("EVE_CACHE_TRACE_SYSTEM", "0");
  }, 120_000);
  ...
  afterAll(async () => {
    if (previousCacheTraceEnv) {
      ...
      restore("EVE_CACHE_TRACE", previousCacheTraceEnv.enabled);
      restore("EVE_CACHE_TRACE_FILE", previousCacheTraceEnv.file);
      restore("EVE_CACHE_TRACE_MESSAGES", previousCacheTraceEnv.messages);
      restore("EVE_CACHE_TRACE_PROMPT", previousCacheTraceEnv.prompt);
      restore("EVE_CACHE_TRACE_SYSTEM", previousCacheTraceEnv.system);
    }
  ...
  describe("openai", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "EVE_LIVE_OPENAI_CACHE_MODEL",
  ...
  describe("anthropic", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "EVE_LIVE_ANTHROPIC_CACHE_MODEL",
