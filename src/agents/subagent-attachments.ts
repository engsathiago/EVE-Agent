...
function resolveAttachmentLimits(config: EVEConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".eve", "attachments");
  const relDir = path.posix.join(".eve", "attachments", attachmentId);
