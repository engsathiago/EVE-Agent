...
function resolveMainSessionKeyForSandbox(params: { cfg?: EVEConfig; agentId: string }): string {
...
export function resolveSandboxRuntimeStatus(params: { cfg?: EVEConfig; sessionKey?: string }): {
  agentId: string;
  sessionKey: string;
...
export function formatSandboxToolPolicyBlockedMessage(params: {
  ...
  const explainCommand = runtime.sessionKey
    ? hasUnsafeControlChars(runtime.sessionKey)
      ? `eve sandbox explain --agent ${runtime.agentId}`
      : `eve sandbox explain --session ${shellEscapeSingleArg(runtime.sessionKey)}`
    : "eve sandbox explain";
