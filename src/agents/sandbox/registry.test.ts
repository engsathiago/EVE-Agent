...
const {
  ...
  const baseDir = mkdtempSync(nodePath.join(tmpdir(), "eve-sandbox-registry-"));
  const previousStateDir = process.env.EVE_STATE_DIR;
  Reflect.set(process.env, "EVE_STATE_DIR", baseDir);

  return {
    TEST_STATE_DIR: baseDir,
    PREVIOUS_EVE_STATE_DIR: previousStateDir,
...
afterAll(async () => {
  ...
  if (PREVIOUS_EVE_STATE_DIR === undefined) {
    deleteTestEnvValue("EVE_STATE_DIR");
  } else {
    setTestEnvValue("EVE_STATE_DIR", PREVIOUS_EVE_STATE_DIR);
  }
});
