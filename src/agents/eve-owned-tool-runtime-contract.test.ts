// Verifies EVE-owned tool hooks preserve adjusted params and telemetry.
import type { AgentTool } from "eve-agent/plugin-sdk/agent-core";
import {
  installEVEOwnedToolHooks,
  resetEVEOwnedToolHooks,
  textToolResult,
} from "eve-agent/plugin-sdk/agent-runtime-test-contracts";
import type { ExtensionContext } from "eve-agent/plugin-sdk/agent-sessions";
...
describe("EVE-owned tool runtime contract - embedded agent adapter", () => {
  afterEach(() => {
    resetEVEOwnedToolHooks();
  });
  ...
  it("preserves partially adjusted before_tool_call params through execution and after_tool_call", async () => {
    ...
    const hooks = installEVEOwnedToolHooks({ adjustedParams });
  ...
  it("commits successful embedded agent messaging text, media, and target telemetry", async () => {
    const hooks = installEVEOwnedToolHooks();
    ...
    expect(ctx.state.messagingToolSentMediaUrls).toEqual(["/tmp/eve-reply.png"]);
    expect(
  ...
  it("fails closed when before_tool_call blocks an embedded agent dynamic tool", async () => {
    const hooks = installEVEOwnedToolHooks({ blockReason: "blocked by policy" });
