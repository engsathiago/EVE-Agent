// Verifies update_plan registration gates and base EVE tool inclusion policy.
...
import { shouldIncludeUpdatePlanToolForEVETools } from "./eve-tools.registration.js";
import { createUpdatePlanTool } from "./tools/update-plan-tool.js";

type UpdatePlanGatingParams = Parameters<typeof shouldIncludeUpdatePlanToolForEVETools>[0];
type CreateEVEToolsOptions = NonNullable<Parameters<typeof createEVETools>[0]>;

function expectUpdatePlanEnabled(params: UpdatePlanGatingParams, expected: boolean): void {
  expect(shouldIncludeUpdatePlanToolForEVETools(params)).toBe(expected);
}

function toolNames(tools: ReturnType<typeof createEVETools>): string[] {
  return tools.map((tool) => tool.name);
}

function createFastToolNames(options: CreateEVEToolsOptions): string[] {
...
function expectToolNamed(
  tools: ReturnType<typeof createEVETools>,
  name: string,
): ReturnType<typeof createEVETools>[number] {
...
describe("eve-tools update_plan gating", () => {
  afterEach(() => {
    setEmbeddedMode(false);
  });

  it("keeps update_plan disabled by default", () => {
    expectUpdatePlanEnabled({ config: {} as EVEConfig }, false);
  });
  ...
  it("does not expose update_plan from default tool construction", () => {
    ...
    expect(shouldIncludeUpdatePlanToolForEVETools(emptyAllowlistParams)).toBe(false);
  });

  it("wraps constructed tools with before-tool-call hooks by default", () => {
    const tools = createEVETools({
      config: {} as EVEConfig,
      disablePluginTools: true,
    });
    const unwrappedTools = createEVETools({
      config: {} as EVEConfig,
      disablePluginTools: true,
      wrapBeforeToolCallHook: false,
    });
  ...
  it("requires explicit transcripts enablement before registering the transcripts tool", () => {
    ...
    const enabledTools = createFastToolNames({
      config: { transcripts: { enabled: true } } as EVEConfig,
    });
  ...
  it("keeps explicitly allowed message tool in embedded completions", () => {
    setEmbeddedMode(true);
    const fromRuntimeAllowlist = createEVETools({
      config: {} as EVEConfig,
      disablePluginTools: true,
      pluginToolAllowlist: ["message"],
      wrapBeforeToolCallHook: false,
    });
    const fromGlobalAlsoAllow = createEVETools({
      config: { tools: { profile: "minimal", alsoAllow: ["message"] } } as EVEConfig,
      disablePluginTools: true,
      wrapBeforeToolCallHook: false,
    });
    const denied = createEVETools({
      config: {} as EVEConfig,
      disablePluginTools: true,
      pluginToolAllowlist: ["message"],
  ...
  it("includes update_plan when a config allowlist group includes it", () => {
    const includeUpdatePlan = shouldIncludeUpdatePlanToolForEVETools({
      config: { tools: { allow: ["group:agents"] } } as EVEConfig,
      modelProvider: "anthropic",
      modelId: "claude-sonnet-4-6",
    });
