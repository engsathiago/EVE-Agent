...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses EVE_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      EVE_HOME: "/srv/eve-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/eve-home"), ".eve", "workspace"));
  });

  it("prefers EVE_WORKSPACE_DIR for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      EVE_WORKSPACE_DIR: "/srv/eve-workspace",
      EVE_HOME: "/srv/eve-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.resolve("/srv/eve-workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = ["eve-workspace-state.json"] as const;
const LEGACY_WORKSPACE_STATE_PATH_SEGMENTS = [".eve", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("eve-workspace-");
  ...
  it("does not overwrite a foreign root workspace-state.json file", async () => {
    ...
    const foreignState = "not eve state\n";
  ...
  it("refuses to re-seed a recently attested workspace after only generated git metadata survives", async () => {
    ...
    await fs.mkdir(path.join(tempDir, ".eve"), { recursive: true });
  ...
  it("does not overwrite a sibling file that is not an EVE attestation marker", async () => {
    const tempDir = await makeTempWorkspace("eve-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "eve-workspace-hardlink-"));
    try {
