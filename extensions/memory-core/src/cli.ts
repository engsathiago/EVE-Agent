...
import {
  ...
  formatHelpExamples,
  theme,
} from "eve-agent/plugin-sdk/memory-core-host-runtime-cli";
import {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["eve memory status", "Show index and provider status."],
          [
            "eve memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["eve memory status --deep", "Probe embedding provider readiness."],
          ["eve memory index --force", "Force a full reindex."],
          ['eve memory search "meeting notes"', "Quick search using positional query."],
          [
          ...
          [
            "eve memory rem-backfill --path ./memory",
          ...
          ["eve memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.eve.ai/cli/memory")}\n`,
    );
