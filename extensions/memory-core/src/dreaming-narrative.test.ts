...
import * as memoryCoreHostRuntimeCoreModule from "eve-agent/plugin-sdk/memory-core-host-runtime-core";
import * as runtimeConfigSnapshotModule from "eve-agent/plugin-sdk/runtime-config-snapshot";
import * as sessionStoreRuntimeModule from "eve-agent/plugin-sdk/session-store-runtime";
...
describe("backfill diary entries", () => {
  ...
  it("builds a marked backfill diary entry", () => {
    ...
    expect(entry).toContain("eve:dreaming:backfill-entry");
    expect(entry).toContain("What Happened");
  });

  it("writes and replaces backfill diary entries", async () => {
    const workspaceDir = await createTempWorkspace("eve-dreaming-backfill-");
    ...
    expect(content.match(/eve:dreaming:backfill-entry/g)?.length).toBe(1);
  });
...
describe("appendNarrativeEntry", () => {
  it("creates DREAMS.md with diary header on fresh workspace", async () => {
    const workspaceDir = await createTempWorkspace("eve-dreaming-narrative-");
    ...
    expect(content).toContain("<!-- eve:dreaming:diary:start -->");
    expect(content).toContain("<!-- eve:dreaming:diary:end -->");
  });
  ...
  it("appends a second entry within the diary markers", async () => {
    ...
    const start = content.indexOf("<!-- eve:dreaming:diary:start -->");
    const end = content.indexOf("<!-- eve:dreaming:diary:end -->");
  ...
  it("dedupes only exact diary duplicates while keeping distinct timestamps", async () => {
    const workspaceDir = await createTempWorkspace("eve-dreaming-dedupe-");
    const dreamsPath = path.join(workspaceDir, "DREAMS.md");
    await fs.writeFile(
      dreamsPath,
      [
...
describe("generateAndAppendDreamNarrative", () => {
  ...
  it("scrubs stale dreaming entries and orphan transcripts after cleanup", async () => {
    const workspaceDir = await createTempWorkspace("eve-dreaming-narrative-");
    const stateDir = await createTempWorkspace("eve-dreaming-state-");
  ...
  it("isolates narrative sessions across workspaces even at the same timestamp", async () => {
    const firstWorkspaceDir = await createTempWorkspace("eve-dreaming-narrative-");
    const secondWorkspaceDir = await createTempWorkspace("eve-dreaming-narrative-");
...
describe("runDetachedDreamNarrative", () => {
  ...
  it("caps the number of in-flight detached narratives at 3", async () => {
    ...
    const workspaceDirs = await Promise.all(
      Array.from({ length: 5 }, () => createTempWorkspace("eve-dreaming-detach-")),
    );
    const logger = createMockLogger();
  ...
  it("serializes detached narratives that reuse a workspace and phase session", async () => {
    ...
    const workspaceDir = await createTempWorkspace("eve-dreaming-detach-");
    const logger = createMockLogger();
