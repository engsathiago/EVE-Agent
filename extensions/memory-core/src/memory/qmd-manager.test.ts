...
import { withMockedWindowsPlatform } from "eve-agent/plugin-sdk/test-node-mocks";
...
vi.mock("eve-agent/plugin-sdk/memory-core-host-engine-foundation", async () => {
  const actual = await vi.importActual<
    typeof import("eve-agent/plugin-sdk/memory-core-host-engine-foundation")
  >("eve-agent/plugin-sdk/memory-core-host-engine-foundation");
  return {
    ...actual,
...
vi.mock("eve-agent/plugin-sdk/file-lock", async () => {
  const actual = await vi.importActual<typeof import("eve-agent/plugin-sdk/file-lock")>(
    "eve-agent/plugin-sdk/file-lock",
  );
  return {
    ...actual,
...
describe("QmdMemoryManager", () => {
  ...
  it("rebinds a stale in-container collection root to the host workspace (sandbox-mode transition)", async () => {
    ...
    // /home/node/.eve/... stored that in-container path as the collection root. Resolved
    ...
    const containerRoot = "/home/node/.eve/teams/x/workspace";
  ...
  it("parseShownCollection extracts path and pattern from qmd collection show output", async () => {
    ...
    const sampleOutput = [
      ...
      "  Path:     /home/node/.eve/teams/example-team/workspace-example/memory",
      "  Pattern:  **/*.md",
      "  Include:  yes (default)",
    ].join("\n");

    const result = parser(sampleOutput);
    expect(result.path).toBe("/home/node/.eve/teams/example-team/workspace-example/memory");
