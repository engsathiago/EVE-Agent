...
import type { EmbeddingProviderAdapter } from "eve-agent/plugin-sdk/embedding-providers";
import type { MemoryEmbeddingProviderAdapter } from "eve-agent/plugin-sdk/memory-core-host-engine-embeddings";
...
const mockEmbeddingRegistry = vi.hoisted(() => ({
  ...
  genericLookupConfigs: [] as Array<EVEConfig | undefined>,
}));

vi.mock("eve-agent/plugin-sdk/embedding-providers", () => ({
  getEmbeddingProvider: (id: string, config?: EVEConfig) => {
...
vi.mock("eve-agent/plugin-sdk/memory-core-host-engine-embeddings", () => ({
