...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): EVEConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as EVEConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): EVEConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  function topicCacheScopeFor(cfg: EVEConfig, accountId: string): string {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["EVE_STATE_DIR", "TELEGRAM_BOT_TOKEN"]);
    resetTopicNameCacheForTest();
  ...
  it("persists sendMessage action deliveries before Telegram platform send", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "eve-telegram-action-durable-"));
    const {
      createOutboundTestPlugin,
      createTestRegistry,
      readQueuedDeliveryEntriesForTest,
      setActivePluginRegistry,
    } = await import("eve-agent/plugin-sdk/plugin-test-runtime");
  ...
  it("stores created forum topic names in the account-scoped cache", async () => {
    ...
    const cfg = {
      ...
      session: { store: path.join(os.tmpdir(), "eve-telegram-action-sessions.json") },
    } as EVEConfig;
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: EVEConfig, accountId = "media") {
    await handleTelegramAction(
