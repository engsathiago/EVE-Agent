...
const TELEGRAM_SENT_MESSAGES_STATE_KEY = Symbol.for("eve.telegramSentMessagesState");
const TELEGRAM_SENT_MESSAGES_STORE_FOR_TEST_KEY = Symbol.for(
  "eve.telegramSentMessagesStoreForTest",
);
...
function resolveSentMessageStorePath(cfg?: Pick<EVEConfig, "session">): string {
  return `${resolveStorePath(cfg?.session?.store)}.telegram-sent-messages.json`;
}

function resolveSentMessageScopeKey(cfg?: Pick<EVEConfig, "session">): string {
...
function getSentMessageBucket(cfg?: Pick<EVEConfig, "session">): SentMessageBucket {
...
function getSentMessages(cfg?: Pick<EVEConfig, "session">): SentMessageStore {
