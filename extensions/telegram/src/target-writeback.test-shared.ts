...
vi.mock("eve-agent/plugin-sdk/config-mutation", async () => {
  const actual = await vi.importActual<typeof import("eve-agent/plugin-sdk/config-mutation")>(
    "eve-agent/plugin-sdk/config-mutation",
  );
  return {
    ...actual,
...
vi.mock("eve-agent/plugin-sdk/cron-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("eve-agent/plugin-sdk/cron-store-runtime")>(
    "eve-agent/plugin-sdk/cron-store-runtime",
  );
  return {
    ...actual,
    loadCronStore,
...
export function installMaybePersistResolvedTelegramTargetTests(params?: {
  ...
  describe("maybePersistResolvedTelegramTarget", () => {
    ...
    it("writes back matching config and cron targets", async () => {
      ...
      expect(writeOptions.expectedConfigPath).toBe("/tmp/eve.json");
      expect(saveCronStore).toHaveBeenCalledTimes(1);
