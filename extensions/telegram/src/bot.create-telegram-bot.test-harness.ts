...
import type { MockFn } from "eve-agent/plugin-sdk/plugin-test-runtime";
...
type GetRuntimeConfigFn =
  typeof import("eve-agent/plugin-sdk/runtime-config-snapshot").getRuntimeConfig;
type LoadSessionStoreFn =
  typeof import("eve-agent/plugin-sdk/session-store-runtime").loadSessionStore;
type ResolveStorePathFn =
  typeof import("eve-agent/plugin-sdk/session-store-runtime").resolveStorePath;
type ReadSessionUpdatedAtFn =
  typeof import("eve-agent/plugin-sdk/session-store-runtime").readSessionUpdatedAt;
...
const { sessionStorePath } = vi.hoisted(() => {
  ...
  return {
    sessionStorePath: `${tempRoot.replace(/[\\/]+$/u, "")}${separator}eve-telegram-${
      process.pid
...
function resolveDefaultModelForAgentForTest(params: { cfg: EVEConfig }): {
  provider: string;
  model: string;
} {
...
function createModelsProviderDataFromConfig(cfg: EVEConfig): {
...
const DEFAULT_TELEGRAM_TEST_CONFIG: EVEConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: EVEConfig) => {
