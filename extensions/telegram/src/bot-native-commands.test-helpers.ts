...
import type { ChannelGroupPolicy } from "eve-agent/plugin-sdk/config-contracts";
...
vi.mock("eve-agent/plugin-sdk/plugin-runtime", () => ({
...
vi.mock("eve-agent/plugin-sdk/reply-dispatch-runtime", () => ({
...
export function createNativeCommandsHarness(params?: {
  ...
  const telegramDeps = {
    getRuntimeConfig: vi.fn(() => params?.cfg ?? ({} as EVEConfig)),
