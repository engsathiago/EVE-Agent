...
import type { ChannelAccountSnapshot } from "eve-agent/plugin-sdk/channel-contract";
...
import {
  ...
  type EVEStateKyselyDatabaseForTests,
...
function openTelegramSpoolTestKysely(spoolDir: string) {
  const database = openEVEStateDatabase({
    env: { ...process.env, EVE_STATE_DIR: spoolDir },
  });
  return {
    database,
...
async function withTempSpool<T>(fn: (spoolDir: string) => Promise<T>): Promise<T> {
  const spoolDir = await fs.mkdtemp(path.join(os.tmpdir(), "eve-telegram-spool-"));
  try {
...
describe("TelegramPollingSession", () => {
  ...
  it("does not call getUpdates for offset confirmation (avoiding 409 conflicts)", async () => {
    ...
    // EVE middleware still skips duplicates using the persisted update offset.
  ...
  it("initializes the main-thread bot before draining isolated ingress spool", async () => {
    const abort = new AbortController();
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "eve-telegram-spool-"));
  ...
  it("restarts isolated ingress on a getUpdates conflict instead of crashing the account", async () => {
    ...
    try {
      ...
      expectLogIncludes(log, "Another EVE gateway, script, or Telegram poller");
      expect(
        statusPatches(setStatus).some(
          (patch) =>
