...
const signalToolResultSessionStorePath = vi.hoisted(
  () => `/tmp/eve-signal-tool-result-sessions-${process.pid}.json`,
);
...
vi.mock("eve-agent/plugin-sdk/security-runtime", async () => {
  const actual = await vi.importActual<typeof import("eve-agent/plugin-sdk/security-runtime")>(
    "eve-agent/plugin-sdk/security-runtime",
  );
  return {
    ...actual,
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("eve-agent/plugin-sdk/reply-runtime"),
      import("eve-agent/plugin-sdk/system-event-runtime"),
    ]);
    resetInboundDedupe();
    config = {
