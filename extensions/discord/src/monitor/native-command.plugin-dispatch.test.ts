...
import { dispatchReplyWithDispatcher } from "eve-agent/plugin-sdk/reply-dispatch-runtime";
import {
  clearRuntimeConfigSnapshot,
  setRuntimeConfigSnapshot,
} from "eve-agent/plugin-sdk/runtime-config-snapshot";
import { getSessionEntry } from "eve-agent/plugin-sdk/session-store-runtime";
...
async function createNativeCommand(cfg: EVEConfig, commandSpec: NativeCommandSpec) {
...
async function createPluginCommand(params: { cfg: EVEConfig; name: string }) {
  return createDiscordNativeCommand({
    command: {
      name: params.name,
      description: "Pair",
...
describe("Discord native plugin command dispatch", () => {
  ...
  beforeEach(() => {
    ...
    discordNativeCommandTesting.setMatchPluginCommand(
      runtimeModuleMocks.matchPluginCommand as typeof import("eve-agent/plugin-sdk/plugin-runtime").matchPluginCommand,
    );
    discordNativeCommandTesting.setExecutePluginCommand(
      runtimeModuleMocks.executePluginCommand as typeof import("eve-agent/plugin-sdk/plugin-runtime").executePluginCommand,
    );
    ...
    discordNativeCommandTesting.setGetSessionEntry(
      runtimeModuleMocks.getSessionEntry as typeof import("eve-agent/plugin-sdk/session-store-runtime").getSessionEntry,
    );
  });
  ...
  it("refreshes native command routing config between invocations", async () => {
    ...
    const resolveRouteState = vi.fn(async (params: { cfg: EVEConfig }) =>
      createUnboundRouteState({
        sessionKey:
  ...
  it("passes the configured binding agent to plugin-owned Discord command sessions", async () => {
    const cfg = createConfig();
    const interaction = createInteraction();
    const pluginSessionKey = "plugin-binding:eve-codex-app-server:dm";
