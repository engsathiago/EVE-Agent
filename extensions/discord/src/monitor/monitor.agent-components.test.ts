...
import { peekSystemEvents, resetSystemEventsForTest } from "eve-agent/plugin-sdk/test-fixtures";
...
describe("agent components", () => {
  ...
  const createCfg = (): EVEConfig => ({}) as EVEConfig;
  ...
  it("sends pairing reply when DM sender is not allowlisted", async () => {
    ...
    expect(pairingText).toContain(`eve pairing approve discord ${code}`);
