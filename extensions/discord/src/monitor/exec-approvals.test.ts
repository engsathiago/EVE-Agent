...
vi.mock("eve-agent/plugin-sdk/approval-gateway-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("eve-agent/plugin-sdk/approval-gateway-runtime")>();
  return {
    ...actual,
...
function buildConfig(
  execApprovals?: NonNullable<NonNullable<EVEConfig["channels"]>["discord"]>["execApprovals"],
): EVEConfig {
  return {
    channels: {
      discord: {
