...
vi.mock("eve-agent/plugin-sdk/channel-outbound", async () => {
  const actual = await vi.importActual<typeof import("eve-agent/plugin-sdk/channel-outbound")>(
    "eve-agent/plugin-sdk/channel-outbound",
  );
  return {
    ...actual,
...
describe("deliverDiscordReply", () => {
      ...
      replies: [
        {
          text: [
            "[tool:exec]",
            "<parameter=command>",
            'cat /proc/mounts 2>/dev/null | grep -i "libra|rav|eve" | head -20',
            "</parameter>",
            "",
            "<function=exec>",
            "<parameter=command>",
  ...
  it("passes media roots and explicit off-mode payload reply tags to shared outbound", async () => {
    ...
    expect(params.mediaAccess).toEqual({ localRoots: ["/tmp/eve-media"] });
  });
