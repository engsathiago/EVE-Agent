...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: EVEConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): EVEConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("eve-agent/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("eve-agent/plugin-sdk/acp-runtime")>(
    "eve-agent/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("eve-agent/plugin-sdk/native-command-config-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("eve-agent/plugin-sdk/native-command-config-runtime")
  >("eve-agent/plugin-sdk/native-command-config-runtime");
  return {
    ...actual,
...
vi.mock("eve-agent/plugin-sdk/error-runtime", async () => {
  const actual = await vi.importActual<typeof import("eve-agent/plugin-sdk/error-runtime")>(
    "eve-agent/plugin-sdk/error-runtime",
  );
  return {
    ...actual,
