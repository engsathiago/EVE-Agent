...
vi.mock("eve-agent/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<object>("eve-agent/plugin-sdk/setup");
  return {
    ...actual,
...
vi.mock("eve-agent/plugin-sdk/channel-config-schema", async () => {
  const actual = await vi.importActual<object>("eve-agent/plugin-sdk/channel-config-schema");
  return {
    ...actual,
...
vi.mock("eve-agent/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<object>("eve-agent/plugin-sdk/webhook-ingress");
  return {
    ...actual,
...
vi.mock("./runtime.js", () => ({
  getSynologyRuntime: vi.fn(() => ({
    ...
    channel: {
      ...
      session: {
        resolveStorePath: vi.fn(() => "/tmp/eve/synology-chat-sessions.json"),
