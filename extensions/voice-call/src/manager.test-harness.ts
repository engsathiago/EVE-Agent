...
import { createPluginStateSyncKeyedStoreForTests } from "eve-agent/plugin-sdk/plugin-state-test-runtime";
...
export function createTestStorePath(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "eve-voice-call-test-"));
}
