...
} from "eve-agent/plugin-sdk/provider-setup";
import type { WizardPrompter } from "eve-agent/plugin-sdk/setup";
...
vi.mock("eve-agent/plugin-sdk/provider-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("eve-agent/plugin-sdk/provider-auth")>();
  return {
    ...actual,
...
vi.mock("eve-agent/plugin-sdk/provider-setup", async (importOriginal) => {
  const actual = await importOriginal<typeof import("eve-agent/plugin-sdk/provider-setup")>();
  return {
    ...actual,
...
afterAll(() => {
  ...
  vi.doUnmock("eve-agent/plugin-sdk/provider-setup");
  vi.resetModules();
});
...
function buildConfig(): EVEConfig {
  return {
    models: {
      providers: {
        lmstudio: {
...
describe("lmstudio setup", () => {
  ...
  it("non-interactive setup fails when requested model is missing", async () => {
    ...
    const dockerSetup = ["1", "true", "yes", "on"].includes(
      process.env.EVE_DOCKER_SETUP?.trim().toLowerCase() ?? "",
    );
