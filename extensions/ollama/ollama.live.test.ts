...
const RUN_EMBEDDINGS =
  process.env.EVE_LIVE_OLLAMA_EMBEDDINGS === "1" ||
  (process.env.EVE_LIVE_OLLAMA_EMBEDDINGS !== "0" && !isOllamaCloudBaseUrl(OLLAMA_BASE_URL));
...
async function withTempEVEState<T>(run: (paths: { root: string }) => Promise<T>): Promise<T> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "eve-ollama-cli-live-"));
  try {
    await fs.writeFile(
      path.join(root, "eve.json"),
      JSON.stringify(
        {
          models: {
            providers: {
...
async function runEVECli(args: string[], env: NodeJS.ProcessEnv) {
  ...
  const commandArgs = sourceRunnerAvailable
    ...
    : ["eve.mjs", ...args];
  const outputRoot = fsSync.mkdtempSync(path.join(os.tmpdir(), "eve-ollama-cli-output-"));
...
describe.skipIf(!LIVE)("ollama live", () => {
  it("runs infer model run through the local CLI path without static model discovery", async () => {
    await withTempEVEState(async ({ root }) => {
      const result = await runEVECli(
        [
          "infer",
          "model",
          "run",
