...
function buildCrabboxEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  if (!trimToValue(next.EVE_MANTIS_TELEGRAM_GROUP_ID)) {
...
function resolveTelegramGatewayEnvPayload(
  ...
  const groupId = trimToValue(env.EVE_QA_TELEGRAM_GROUP_ID);
  const driverToken = trimToValue(env.EVE_QA_TELEGRAM_DRIVER_BOT_TOKEN);
  const sutToken = trimToValue(env.EVE_QA_TELEGRAM_SUT_BOT_TOKEN);
  if (!groupId || !driverToken || !sutToken) {
    throw new Error(
      "Telegram desktop builder requires EVE_QA_TELEGRAM_GROUP_ID, EVE_QA_TELEGRAM_DRIVER_BOT_TOKEN, and EVE_QA_TELEGRAM_SUT_BOT_TOKEN when using --credential-source env.",
    );
  }
...
async function prepareGatewayCredentialEnv(params: {
  ...
  if (
    trimToValue(params.env.EVE_MANTIS_TELEGRAM_GROUP_ID) &&
    trimToValue(params.env.EVE_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN) &&
    trimToValue(params.env.EVE_MANTIS_TELEGRAM_SUT_BOT_TOKEN)
  ) {
    return {};
  }
  ...
  params.env.EVE_MANTIS_TELEGRAM_GROUP_ID = payload.groupId;
  params.env.EVE_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN = payload.driverToken;
  params.env.EVE_MANTIS_TELEGRAM_SUT_BOT_TOKEN = payload.sutToken;
  params.env.EVE_QA_TELEGRAM_GROUP_ID =
    trimToValue(params.env.EVE_QA_TELEGRAM_GROUP_ID) ?? payload.groupId;
  params.env.EVE_QA_TELEGRAM_DRIVER_BOT_TOKEN =
    trimToValue(params.env.EVE_QA_TELEGRAM_DRIVER_BOT_TOKEN) ?? payload.driverToken;
  params.env.EVE_QA_TELEGRAM_SUT_BOT_TOKEN =
    trimToValue(params.env.EVE_QA_TELEGRAM_SUT_BOT_TOKEN) ?? payload.sutToken;
  return {
    credentialLease,
    leaseHeartbeat,
  };
...
telegram_root="$HOME/.local/share/eve-mantis/telegram-desktop-bin"
...
if [ -n "\${EVE_MANTIS_TELEGRAM_DESKTOP_PROFILE_TGZ_B64:-}" ]; then
  printf '%s' "$EVE_MANTIS_TELEGRAM_DESKTOP_PROFILE_TGZ_B64" | base64 -d >"$out/telegram-profile.tgz"
...
qa_status=0
{
  ...
  if [ "$setup_gateway" = "1" ]; then
    export TELEGRAM_BOT_TOKEN="\${EVE_MANTIS_TELEGRAM_SUT_BOT_TOKEN:-\${TELEGRAM_BOT_TOKEN:-}}"
    telegram_group_id="\${EVE_MANTIS_TELEGRAM_GROUP_ID:-}"
    driver_token="\${EVE_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN:-}"
    if [ -z "$TELEGRAM_BOT_TOKEN" ] || [ -z "$telegram_group_id" ] || [ -z "$driver_token" ]; then
      echo "Gateway setup requires EVE_MANTIS_TELEGRAM_GROUP_ID, EVE_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN, and EVE_MANTIS_TELEGRAM_SUT_BOT_TOKEN." >&2
      exit 2
    fi
...
const token = process.env.EVE_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN;
...
node --input-type=module -e 'import fs from "node:fs"; const value = JSON.parse(fs.readFileSync(process.argv[1], "utf8")); process.stdout.write(String(value.id || ""));' "$out/telegram-driver-getme.json")"
    export EVE_HOME="$HOME/.eve-mantis/telegram-eve"
    mkdir -p "$EVE_HOME"
...
MANTIS_TELEGRAM_PATCH
    pnpm eve config patch --file "$out/telegram.patch.json5" --dry-run
    pnpm eve config patch --file "$out/telegram.patch.json5"
    node --input-type=module >"$out/telegram-ready-message.json" 2>"$out/telegram-ready-message.err" <<'MANTIS_TELEGRAM_READY'
const token = process.env.EVE_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN;
const chatId = process.env.EVE_MANTIS_TELEGRAM_GROUP_ID;
...
MANTIS_TELEGRAM_READY
    nohup pnpm eve gateway run --dev --allow-unconfigured --port 38974 --cli-backend-logs </dev/null >"$out/eve-gateway.log" 2>&1 &
    gateway_pid="$!"
...
function renderReport(summary: MantisTelegramDesktopBuilderSummary) {
  const lines = [
    ...
    "- EVE gateway log: `eve-gateway.log`",
...
export async function runMantisTelegramDesktopBuilder(
  ...
  const remoteOutputDir = `/tmp/eve-mantis-telegram-desktop-${startedAt
    .toISOString()
  ...
  try {
    ...
    if (gatewaySetup && !gatewaySetupCompleted) {
      throw new Error("Telegram desktop builder did not report a live EVE gateway.");
    }
    summary = {
      artifacts: {
        reportPath,
