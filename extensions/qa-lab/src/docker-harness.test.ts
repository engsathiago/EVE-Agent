...
describe("qa docker harness", () => {
  it("writes compose, env, config, and workspace scaffold files", async () => {
    ...
    for (const expectedFile of [
      ...
      path.join(outputDir, "state", "eve.json"),
    ...
    expect(compose).toContain("image: eve:qa-local-prebaked");
    ...
    expect(services["qa-mock-openai"]?.environment).not.toHaveProperty("EVE_CONFIG_PATH");
    expect(services["qa-mock-openai"]?.volumes).toBeUndefined();
    expect(services["qa-lab"]?.environment).toMatchObject({
      EVE_ENABLE_PRIVATE_QA_CLI: "1",
      EVE_CONFIG_PATH: "/opt/eve-scaffold/eve.json",
      EVE_STATE_DIR: "/tmp/eve/state",
    });
    expect(services["qa-lab"]?.volumes).toContain("./state:/opt/eve-scaffold:ro");
    expect(compose).toContain('      - "127.0.0.1:18889:18789"');
    expect(compose).toContain('      - "127.0.0.1:43124:43123"');
    expect(compose).toContain(":/opt/eve-qa-lab-ui:ro");
    expect(compose).toContain("      - sh");
    expect(compose).toContain("      - -lc");
    expect(compose).toContain(
      '        - fetch("http://127.0.0.1:18789/healthz").then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))',
    );
    expect(compose).toContain("--control-ui-proxy-target http://eve-qa-gateway:18789/");
    ...
    expect(compose).toContain("--ui-dist-dir /opt/eve-qa-lab-ui");
    expect(compose).toContain(":/opt/eve-repo:ro");
    expect(compose).toContain("./state:/opt/eve-scaffold:ro");
    expect(compose).toContain(
      "cp -R /opt/eve-scaffold/seed-workspace/. /tmp/eve/workspace/ && rm -rf /tmp/eve/workspace/repo && ln -s /opt/eve-repo /tmp/eve/workspace/repo",
    );
    expect(compose).toContain("EVE_CONFIG_PATH: /tmp/eve/eve.json");
    expect(compose).toContain("EVE_STATE_DIR: /tmp/eve/state");
    expect(compose).toContain('EVE_NO_RESPAWN: "1"');

    const envExample = await readFile(path.join(outputDir, ".env.example"), "utf8");
    expect(envExample).toContain("EVE_GATEWAY_TOKEN=qa-token");
    ...
    const configText = await readFile(path.join(outputDir, "state", "eve.json"), "utf8");
    ...
    expect(configText).toContain('"eveToolsMcpBridge": true');
    expect(configText).toContain("/app/dist/control-ui");
    expect(configText).toContain("C-3PO QA");
    expect(configText).toContain('"/tmp/eve/workspace"');
  ...
  it("builds the reusable QA image with bundled QA extensions", async () => {
    ...
    expect(result.imageName).toBe("eve:qa-local-prebaked");
    expect(calls).toEqual([
      "docker build -t eve:qa-local-prebaked --build-arg EVE_EXTENSIONS=qa-channel qa-lab -f Dockerfile . @/repo/eve",
    ]);
  });
