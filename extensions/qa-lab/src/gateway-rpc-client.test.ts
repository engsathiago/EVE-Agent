...
vi.mock("eve-agent/plugin-sdk/gateway-runtime", () => ({
...
describe("startQaGatewayRpcClient", () => {
  ...
  it("calls the in-process gateway cli helper without mutating process.env", async () => {
    const originalHome = process.env.EVE_HOME;
    ...
    try {
      ...
      if (originalHome === undefined) {
        ...
        process.env.EVE_HOME = originalHome;
      }
    }

    expect(process.env.EVE_HOME).toBe(originalHome);
  });
