...
export const QA_LIVE_ANTHROPIC_SETUP_TOKEN_ENV = "EVE_QA_LIVE_ANTHROPIC_SETUP_TOKEN";
export const QA_LIVE_SETUP_TOKEN_VALUE_ENV = "EVE_LIVE_SETUP_TOKEN_VALUE";
const QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE_ENV = "EVE_QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE";
...
function qaLiveOpenAiUsesCodexByDefault(cfg: EVEConfig): boolean {
...
function resolveQaLiveProviderConfig(params: { cfg: EVEConfig; providerId: string }) {
...
function hasQaLiveStagedApiKeyProfile(params: { cfg: EVEConfig; providerId: string }) {
...
function qaLiveRequiresCodexAuth(params: {
  ...
  const forcedRuntime = params.env.EVE_QA_FORCE_RUNTIME?.trim().toLowerCase();
  if (forcedRuntime === "eve") {
    return false;
  }
...
export function assertQaLiveCodexAuthAvailable(params: {
  ...
  throw new Error(
    [
      ...
      "Set OPENAI_API_KEY or EVE_LIVE_OPENAI_KEY for an API-key fallback, or set CODEX_HOME to a logged-in Codex CLI home.",
      "Host EVE OAuth refresh profiles are not copied into QA temp stores.",
    ].join(" "),
  );
}
