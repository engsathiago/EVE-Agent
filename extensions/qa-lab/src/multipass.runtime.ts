...
const MULTIPASS_MOUNTED_REPO_PATH = "/workspace/eve-host";
const MULTIPASS_GUEST_REPO_PATH = "/workspace/eve";
const MULTIPASS_GUEST_CODEX_HOME_PATH = "/workspace/eve-codex-home";
...
export function createQaMultipassPlan(params: {
  ...
  const hostLiveProviderConfigPath =
    liveProviderConfig && fs.existsSync(liveProviderConfig.path)
      ...
      : undefined;
  const vmName = `eve-qa-${createVmSuffix()}`;
...
export function renderQaMultipassGuestScript(
  ...
  const qaCommand = [
    ...Object.entries(plan.forwardedEnv)
      .filter(
        ([key]) =>
          key !== "CODEX_HOME" &&
          key !== "EVE_CONFIG_PATH" &&
          key !== "EVE_QA_LIVE_PROVIDER_CONFIG_PATH",
      )
    ...
    ...(plan.guestLiveProviderConfigPath
      ? [
          `EVE_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
          `EVE_QA_LIVE_PROVIDER_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
        ]
      : []),
...
export async function runQaMultipass(params: {
  ...
  try {
    ...
    throw new Error(
      `Multipass is not installed on this host. Install it with '${resolveMultipassInstallHint()}', then rerun 'pnpm eve qa suite --runner multipass'.`,
      { cause: error },
    );
  }

  const hostTransferDirPath = await fs.promises.mkdtemp(
    path.join(resolvePreferredEVETmpDir(), `${plan.vmName}-qa-suite-`),
  );
