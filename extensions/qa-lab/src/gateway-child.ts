...
export function buildQaRuntimeEnv(params: {
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ...(provider?.appliesLiveEnvAliases
      ...
      : {}),
    EVE_HOME: params.homeDir,
    EVE_CONFIG_PATH: params.configPath,
    EVE_STATE_DIR: params.stateDir,
    EVE_OAUTH_DIR: path.join(params.stateDir, "credentials"),
    ...
    EVE_NO_RESPAWN: "1",
    EVE_TEST_FAST: "1",
    EVE_EMBEDDED_ABORT_SETTLE_TIMEOUT_MS: "2000",
    EVE_QA_PARENT_PID: String(process.pid),
    EVE_QA_TEMP_ROOT: params.tempRoot,
    ...(params.stagedBundledPluginsRoot
      ? { EVE_QA_STAGED_RUNTIME_ROOT: params.stagedBundledPluginsRoot }
      : {}),
    EVE_QA_ALLOW_LOCAL_IMAGE_PROVIDER: "1",
    ...
    EVE_ALLOW_SLOW_REPLY_TESTS: "1",
    XDG_CONFIG_HOME: params.xdgConfigHome,
    XDG_DATA_HOME: params.xdgDataHome,
    XDG_CACHE_HOME: params.xdgCacheHome,
    ...(params.bundledPluginsDir ? { EVE_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir } : {}),
    ...(params.compatibilityHostVersion
      ? { EVE_COMPATIBILITY_HOST_VERSION: params.compatibilityHostVersion }
      : {}),
  };
...
export async function startQaGatewayChild(params: {
  ...
  const keepTemp = process.env.EVE_QA_KEEP_TEMP === "1";
  let gatewayPort = 0;
  let baseUrl = "";
  let wsUrl = "";
