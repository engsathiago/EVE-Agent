...
const QA_LAB_UI_OVERLAY_DIR = "/opt/eve-qa-lab-ui";
...
${params.bindUiDist ? `      - ${yamlDoubleQuoted(`${qaLabUiMount}:${QA_LAB_UI_OVERLAY_DIR}:ro`)}\n` : ""}    healthcheck:
    ...
    command:
      - sh
      - -lc
      - EVE_QA_CONTROL_UI_PROXY_TOKEN="$(node -e 'const fs=require("node:fs");const cfg=JSON.parse(fs.readFileSync("/opt/eve-scaffold/eve.json","utf8"));process.stdout.write(cfg.gateway?.auth?.token ?? "")')" exec node dist/index.js qa ui --host 0.0.0.0 --port ${QA_LAB_INTERNAL_PORT} --advertise-host 127.0.0.1 --advertise-port ${params.qaLabPort} --control-ui-url http://127.0.0.1:${params.gatewayPort}/ --control-ui-proxy-target http://eve-qa-gateway:18789/${params.bindUiDist ? ` --ui-dist-dir ${QA_LAB_UI_OVERLAY_DIR}` : ""} --auto-kickoff-target direct --send-kickoff-on-start --embedded-gateway disabled
    depends_on:
      qa-mock-openai:
...
${imageBlock}    pull_policy: never
    ...
    environment:
      EVE_CONFIG_PATH: /tmp/eve/eve.json
      EVE_STATE_DIR: /tmp/eve/state
      EVE_NO_RESPAWN: "1"
      EVE_SKIP_GMAIL_WATCHER: "1"
      EVE_SKIP_BROWSER_CONTROL_SERVER: "1"
    ...
    volumes:
      - ./state:/opt/eve-scaffold:ro
      - ${yamlDoubleQuoted(`${repoMount}:/opt/eve-repo:ro`)}
    healthcheck:
      test:
        - CMD
        - node
...
`
    ...
    command:
      - sh
      - -lc
      - mkdir -p /tmp/eve/workspace /tmp/eve/state && cp /opt/eve-scaffold/eve.json /tmp/eve/eve.json && cp -R /opt/eve-scaffold/seed-workspace/. /tmp/eve/workspace/ && rm -rf /tmp/eve/workspace/repo && ln -s /opt/eve-repo /tmp/eve/workspace/repo && exec node dist/index.js gateway run --port 18789 --bind lan --allow-unconfigured
`;
}
...
1. Build the prebaked image once:
   - \`docker build -t eve:qa-local-prebaked --build-arg EVE_EXTENSIONS="qa-channel qa-lab" -f Dockerfile .\`
2. Start the stack:
...
export async function writeQaDockerHarnessFiles(params: {
  ...
  const imageName = params.imageName ?? "eve:qa-local-prebaked";
  ...
  const files = [
    ...
    path.join(params.outputDir, "state", "eve.json"),
  ];
