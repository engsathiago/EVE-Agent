...
const QA_CREDENTIALS_ALLOW_INSECURE_HTTP_ENV_KEY = "EVE_QA_ALLOW_INSECURE_HTTP";
...
export function normalizeQaCredentialConvexSiteUrl(params: {
  ...
  if (url.protocol !== "http:") {
    throw toError("EVE_QA_CONVEX_SITE_URL must use https://.");
  }
  ...
  if (!allowInsecureHttp || !isLoopbackHost(url.hostname)) {
    throw toError(
      `EVE_QA_CONVEX_SITE_URL must use https://. http:// is only allowed for loopback hosts when ${QA_CREDENTIALS_ALLOW_INSECURE_HTTP_ENV_KEY}=1.`,
    );
  }
  const text = url.toString();
  return text.endsWith("/") ? text.slice(0, -1) : text;
