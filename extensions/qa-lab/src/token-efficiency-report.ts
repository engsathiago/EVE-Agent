...
export type TokenEfficiencyRow = {
  ...
  eve: TokenEfficiencyRuntimeUsage;
...
export type TokenEfficiencyReport = {
  ...
  aggregate: {
    eve: { totalTokens: number; p50PerScenario: number; p90PerScenario: number };
...
const ZERO_AGGREGATE: TokenEfficiencyReport["aggregate"] = {
  eve: { totalTokens: 0, p50PerScenario: 0, p90PerScenario: 0 },
...
function deltaPercent(eveTotalTokens: number, codexTotalTokens: number): number {
  if (eveTotalTokens === 0) {
    return codexTotalTokens === 0 ? 0 : 100;
  }
  return ((codexTotalTokens - eveTotalTokens) / eveTotalTokens) * 100;
}
...
function buildRow(params: {
  ...
  const eve = runtimeUsage(params.result.cells.eve);
  const codex = runtimeUsage(params.result.cells.codex);
  const delta = deltaPercent(eve.totalTokens, codex.totalTokens);
  ...
  return {
    ...
    classification,
    flagged,
    toolsUsed: toolNamesForCells(params.result.cells.eve, params.result.cells.codex),
  };
}

function buildAggregate(rows: readonly TokenEfficiencyRow[]): TokenEfficiencyReport["aggregate"] {
  const eveTotals = rows.map((row) => row.eve.totalTokens);
  const codexTotals = rows.map((row) => row.codex.totalTokens);
  const eveTotalTokens = eveTotals.reduce((sum, value) => sum + value, 0);
  const codexTotalTokens = codexTotals.reduce((sum, value) => sum + value, 0);
  return {
    eve: {
      totalTokens: eveTotalTokens,
      p50PerScenario: percentile(eveTotals, 50),
      p90PerScenario: percentile(eveTotals, 90),
    },
    codex: {
    ...
    deltaPercent: deltaPercent(eveTotalTokens, codexTotalTokens),
...
function liveEvidenceFailures(row: TokenEfficiencyRow): string[] {
  ...
  if (row.eve.totalTokens <= 0) {
    failures.push(`${row.scenarioId} eve live usage totalTokens=${row.eve.totalTokens}`);
  }
...
export function buildTokenEfficiencyReport(
  ...
  return {
    ...
    notes: [
      ...
      "Codex savings are reported as savings and do not fail the gate; only positive Codex-over-EVE live deltas exceed the threshold.",
...
export function renderTokenEfficiencyMarkdownReport(report: TokenEfficiencyReport): string {
  const lines = [
    `# EVE Runtime Token Efficiency - ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",
  ...
  lines.push(
    ...
    "| --- | ---: | ---: | ---: |",
    `| eve | ${report.aggregate.eve.totalTokens} | ${report.aggregate.eve.p50PerScenario} | ${report.aggregate.eve.p90PerScenario} |`,
  ...
  if (report.rows.length > 0) {
    lines.push(
      "## Scenario Efficiency",
      "",
      "| Scenario | Source | EVE in/out/total/tools | Codex in/out/total/tools | Token delta | Classification | Flagged | Tools used |",
      "| --- | --- | ---: | ---: | ---: | --- | --- | --- |",
    );
    for (const row of report.rows) {
      lines.push(
        `| ${row.scenarioId} | ${row.usageSource} | ${row.eve.inputTokens}/${row.eve.outputTokens}/${row.eve.totalTokens}/${row.eve.toolCallCount} | ${row.codex.inputTokens}/${row.codex.outputTokens}/${row.codex.totalTokens}/${row.codex.toolCallCount} | ${formatPercent(row.deltaPercent)} | ${row.classification} | ${row.flagged ? "yes" : "no"} | ${row.toolsUsed.join(", ")} |`,
      );
    }
    lines.push("");
  }
