...
describe("qa cli runtime", () => {
  ...
  beforeEach(async () => {
    ...
    listTelegramQaScenarioCatalog.mockReturnValue([
      {
        ...
        regressionRefs: ["engsathiago/eve-agent#74698"],
      },
    ]);
    ...
    writeQaDockerHarnessFiles.mockResolvedValue({
      outputDir: "/tmp/eve-repo/.artifacts/qa-docker",
    });
    buildQaDockerHarnessImage.mockResolvedValue({
      imageName: "eve:qa-local-prebaked",
    });
  ...
  it("dispatches a taxonomy-backed profile category through the suite runner", async () => {
    const previousProfile = process.env.EVE_QA_PROFILE;
    process.env.EVE_QA_PROFILE = "release";
    try {
      runQaSuite.mockImplementationOnce(async () => {
        expect(process.env.EVE_QA_PROFILE).toBe("smoke-ci");
        await fs.writeFile(
          suiteEvidencePath,
          JSON.stringify(
            makeQaEvidence([
      ...
      expectFields(suiteArgs, {
        repoRoot: path.resolve("/tmp/eve-repo"),
        outputDir: path.resolve("/tmp/eve-repo", ".artifacts/qa-e2e/smoke-ci"),
        transportId: "qa-channel",
      ...
      expect(process.env.EVE_QA_PROFILE).toBe("release");
      ...
      if (previousProfile === undefined) {
        delete process.env.EVE_QA_PROFILE;
      } else {
        process.env.EVE_QA_PROFILE = previousProfile;
      }
    }
  });
  ...
  it("resolves suite repo-root-relative paths before dispatching", async () => {
    ...
    expect(runQaSuite).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/eve-repo"),
      outputDir: path.resolve("/tmp/eve-repo", ".artifacts/qa/frontier"),
      transportId: "qa-channel",
      providerMode: "live-frontier",
      primaryModel: "openai/gpt-5.5",
      alternateModel: "anthropic/claude-sonnet-4-6",
  ...
  it("rejects unknown runtime-pair ids at the CLI boundary", async () => {
    ...
    ).rejects.toThrow('--runtime-pair only supports "eve" and "codex".');
    expect(runQaSuite).not.toHaveBeenCalled();
  });
  ...
  it("resolves telegram qa repo-root-relative paths before dispatching", async () => {
    ...
    expect(runTelegramQaLive).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/eve-repo"),
      outputDir: path.resolve("/tmp/eve-repo", ".artifacts/qa/telegram"),
      providerMode: "live-frontier",
      primaryModel: "openai/gpt-5.5",
      alternateModel: "openai/gpt-5.5",
      fastMode: true,
  ...
  it("rejects output dirs that escape the repo root", () => {
    expect(() => resolveRepoRelativeOutputDir("/tmp/eve-repo", "../outside")).toThrow(
      "--output-dir must stay within the repo root.",
    );
    expect(() => resolveRepoRelativeOutputDir("/tmp/eve-repo", "/tmp/outside")).toThrow(
  ...
  it("prints telegram scenario catalog without starting the live lane", async () => {
    ...
    expectWriteContains(
      stdoutWrite,
      "telegram-status-command\tdefault\tTelegram status command reply\tstatus rationale refs=engsathiago/eve-agent#74698",
    );
  });
  ...
  it("runs a host-only parity preflight against the sentinel scenario", async () => {
    const repoRoot = path.resolve("/tmp/eve-repo");
    await runQaSuiteCommand({
      repoRoot: "/tmp/eve-repo",
      providerMode: "mock-openai",
      primaryModel: "openai/gpt-5.5",
  ...
  it("prints a markdown tool coverage report from runtime tool fixtures", async () => {
    ...
    expectWriteContains(stdoutWrite, "# EVE Runtime Tool Coverage");
  ...
  it("writes a curated mock JSONL replay report and summary", async () => {
    ...
    try {
      ...
      expect(report).toContain("# EVE JSONL Replay Report - eve vs codex");
  ...
  it("resolves character eval paths and passes model refs through", async () => {
    ...
    expectFields(characterEvalArgs, {
      repoRoot: path.resolve("/tmp/eve-repo"),
      outputDir: path.resolve("/tmp/eve-repo", ".artifacts/qa/character"),
  ...
  it("routes suite runs through multipass when the runner is selected", async () => {
    ...
    expect(runQaMultipass).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/eve-repo"),
      outputDir: path.resolve("/tmp/eve-repo", ".artifacts/qa-multipass"),
      transportId: "qa-channel",
      providerMode: "mock-openai",
      primaryModel: undefined,
      alternateModel: undefined,
  ...
  it("resolves self-check repo-root-relative paths before starting the lab server", async () => {
    ...
    expect(startQaLabServer).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/eve-repo"),
      outputPath: path.resolve("/tmp/eve-repo", ".artifacts/qa/self-check.md"),
    });
  });
  ...
  it("rejects oversized credential payload files before broker setup", async () => {
    const previousMaxBytes = process.env.EVE_QA_CREDENTIAL_PAYLOAD_MAX_BYTES;
    ...
    process.env.EVE_QA_CREDENTIAL_PAYLOAD_MAX_BYTES = "32";
    ...
    try {
      ...
      ).rejects.toThrow("Payload file exceeds EVE_QA_CREDENTIAL_PAYLOAD_MAX_BYTES (32 bytes).");
    } finally {
      if (previousMaxBytes === undefined) {
        delete process.env.EVE_QA_CREDENTIAL_PAYLOAD_MAX_BYTES;
      } else {
        process.env.EVE_QA_CREDENTIAL_PAYLOAD_MAX_BYTES = previousMaxBytes;
      }
    }
  });
  ...
  it("resolves docker scaffold paths relative to the explicit repo root", async () => {
    ...
    expect(writeQaDockerHarnessFiles).toHaveBeenCalledWith({
      outputDir: path.resolve("/tmp/eve-repo", ".artifacts/qa-docker"),
      repoRoot: path.resolve("/tmp/eve-repo"),
  ...
  it("resolves docker up paths relative to the explicit repo root", async () => {
    ...
    expect(runQaDockerUp).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/eve-repo"),
      outputDir: path.resolve("/tmp/eve-repo", ".artifacts/qa-up"),
      gatewayPort: undefined,
      qaLabPort: undefined,
