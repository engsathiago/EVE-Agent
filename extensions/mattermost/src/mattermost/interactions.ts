...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__eve_channel_id";
...
type InteractionCallbackConfig = Pick<EVEConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "eve-mattermost-interactions").update(botToken).digest("hex");
}
