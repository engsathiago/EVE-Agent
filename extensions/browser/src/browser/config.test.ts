...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.driver).toBe("eve");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const eve = resolveProfile(resolved, "eve");
    expect(eve?.driver).toBe("eve");
    expect(eve?.cdpPort).toBe(18800);
    expect(eve?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from EVE_GATEWAY_PORT when unset", () => {
    withEnv({ EVE_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      expect(resolved.controlPort).toBe(19003);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const eve = resolveProfile(resolved, "eve");
      expect(eve?.cdpPort).toBe(19012);
      expect(eve?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ EVE_GATEWAY_PORT: undefined }, () => {
      ...
      const eve = resolveProfile(resolved, "eve");
      expect(eve?.cdpPort).toBe(19022);
      expect(eve?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(eve?.cdpPort).toBe(19000);
    expect(eve?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "eve");
  ...
  describe("managed browser headless mode", () => {
    const noDisplayEnv = {
      ...
      [EVE_BROWSER_HEADLESS_ENV]: undefined,
    };
    ...
    it("lets explicit profile headless=false beat the Linux no-display fallback", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        profiles: {
          eve: { cdpPort: 18800, color: "#FF4500", headless: false },
        },
      });
      const profile = resolveProfile(resolved, "eve")!;
    ...
    it("lets EVE_BROWSER_HEADLESS override profile/global config", () => {
      const resolved = resolveBrowserConfig({
        profiles: {
          eve: { cdpPort: 18800, color: "#FF4500", headless: false },
        },
      ...
      expect(
        resolveManagedBrowserHeadlessMode(resolved, profile, {
          platform: "linux",
          env: { ...noDisplayEnv, [EVE_BROWSER_HEADLESS_ENV]: "1" },
        }),
    ...
    it("lets request-local headless override beat env and profile/global config", () => {
      ...
      expect(
        resolveManagedBrowserHeadlessMode(resolved, profile, {
          ...
          platform: "linux",
          env: { ...noDisplayEnv, [EVE_BROWSER_HEADLESS_ENV]: "0" },
        }),
    ...
    it("returns an actionable error only when headed mode is explicitly selected", () => {
      ...
      const defaultProfile = resolveProfile(defaultResolved, "eve")!;
      expect(
      ...
      const profile = resolveProfile(profileResolved, "eve")!;
      expect(
        getManagedBrowserMissingDisplayError(profileResolved, profile, {
          platform: "linux",
          env: noDisplayEnv,
        }),
      ).toContain("browser.profiles.eve.headless=false");
    });
  });
  ...
  it("sets usesChromeMcp only for existing-session profiles", () => {
    ...
    const managed = resolveProfile(resolved, "eve")!;
  ...
  describe("default profile preference", () => {
    it("defaults to eve profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
      expect(resolved.defaultProfile).toBe("eve");
    });

    it("keeps eve default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("eve");
    ...
    it("keeps eve default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("eve");
    ...
    it("keeps eve default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
