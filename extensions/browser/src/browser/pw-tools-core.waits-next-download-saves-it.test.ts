...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredEVETmpDir: vi.fn(() => "/tmp/eve"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-eve-dir.js");
...
describe("pw-tools-core", () => {
  ...
  beforeAll(async () => {
    ...
    tmpDirModule = await import("../infra/tmp-eve-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredEVETmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredEVETmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredEVETmpDir.mockReturnValue("/tmp/eve");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "eve-browser-download-test-"));
    try {
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredEVETmpDir.mockReturnValue("/tmp/eve-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "eve-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "eve-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredEVETmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(outPath)).not.toBe(
      path.resolve(path.join(path.sep, "tmp", "eve-preferred", "downloads")),
    );
    expect(path.basename(outPath)).toContain(path.basename(res.path));
    expect(path.basename(outPath)).toMatch(/\.part$/);
    await expect(fs.readFile(res.path, "utf8")).resolves.toBe("download-content");
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "eve-preferred", "downloads")}${path.sep}`),
    );
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir(async (tempDir) => {
        ...
        tmpDirMocks.resolvePreferredEVETmpDir.mockReturnValue(tempDir);
