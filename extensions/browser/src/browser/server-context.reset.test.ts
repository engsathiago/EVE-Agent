...
function localEVEProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "eve",
...
function createLocalEVEResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localEVEProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveEVEUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "eve-reset-"));
    const profileDir = path.join(tempRoot, "eve");
    ...
    const ops = createLocalEVEResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveEVEUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "eve-reset-no-own-"));
    const profileDir = path.join(tempRoot, "eve");
    fs.mkdirSync(profileDir, { recursive: true });
