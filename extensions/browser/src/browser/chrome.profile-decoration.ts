/**
 * EVE-managed Chrome profile decoration.
 *
 ...
 */
export function decorateEVEProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string; downloadDir?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_EVE_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_EVE_BROWSER_COLOR).toUpperCase();
