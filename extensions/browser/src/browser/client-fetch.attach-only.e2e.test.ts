...
import { clearRuntimeConfigSnapshot } from "eve-agent/plugin-sdk/runtime-config-snapshot";
...
describe("browser client fetch attachOnly diagnostics", () => {
  ...
  it("does not suggest gateway restart when an attachOnly CDP endpoint hangs", async () => {
    tempHome = await createTempHomeEnv("eve-browser-client-fetch-live-");
    ...
    const configPath = path.join(tempHome.home, ".eve", "eve.json");
    await fs.writeFile(
      configPath,
      JSON.stringify(
        {
    ...
    try {
      ...
      expect(message).toContain("browser profile is external to EVE");
      expect(message).toContain("Restarting the EVE gateway will not launch it");
      expect(message).not.toContain("Restart the EVE gateway");
