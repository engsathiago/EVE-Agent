...
import { createQueuedWizardPrompter } from "eve-agent/plugin-sdk/plugin-test-runtime";
...
vi.mock("eve-agent/plugin-sdk/setup", async () => {
  ...
  return {
    ...
    setSetupChannelEnabled: (cfg: EVEConfig, channel: string, enabled: boolean) => ({
      ...cfg,
      channels: {
        ...cfg.channels,
        [channel]: {
          ...(cfg.channels?.[channel as keyof NonNullable<EVEConfig["channels"]>] as object),
          enabled,
        },
      },
    }),
