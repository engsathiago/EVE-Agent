...
describe("WhatsApp auth dir profile resolution", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["EVE_STATE_DIR", "EVE_OAUTH_DIR"]);
    tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "eve-wa-profile-"));
  ...
  it("resolves the default web auth dir from EVE_STATE_DIR at call time", async () => {
