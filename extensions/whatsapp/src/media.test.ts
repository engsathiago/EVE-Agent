...
import { captureEnv } from "eve-agent/plugin-sdk/test-env";
...
import { withMockedWindowsPlatform, withRestoredMocks } from "eve-agent/plugin-sdk/test-node-mocks";
import { optimizeImageToPng } from "eve-agent/plugin-sdk/web-media";
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(resolvePreferredEVETmpDir(), "eve-media-test-"));
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub EVE_STATE_DIR.
    // Also keep it outside the EVE temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["EVE_STATE_DIR"]);
    process.env.EVE_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "eve-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredEVETmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("accepts win32 dev=0 stat mismatch for local file loads", async () => {
    ...
    // os.tmpdir() fallback rather than the POSIX `/tmp/eve` root that
    // actually holds `tinyPngFile` on this Linux test runner (#60713).
    const realTmpRoot = resolvePreferredEVETmpDir();
  ...
  it("allows default EVE state workspace and sandbox roots", async () => {
  ...
  it("rejects default EVE state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
