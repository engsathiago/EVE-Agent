...
describe("web logout", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "eve-test-web-logout-"));
    previousOAuthDir = process.env.EVE_OAUTH_DIR;
    process.env.EVE_OAUTH_DIR = path.join(fixtureRoot, "oauth");
  ...
  it("does not delete through symlinked auth dirs inside the EVE auth root", async () => {
  ...
  it("does not delete through intermediate symlinks inside the EVE auth root", async () => {
