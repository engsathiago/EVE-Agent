...
import { generateSecureUuid } from "eve-agent/plugin-sdk/core";
import { redactIdentifier } from "eve-agent/plugin-sdk/logging-core";
import {
  convertMarkdownTables,
  resolveMarkdownTableMode,
} from "eve-agent/plugin-sdk/markdown-table-runtime";
import { requireRuntimeConfig } from "eve-agent/plugin-sdk/plugin-config-runtime";
import { normalizePollInput, type PollInput } from "eve-agent/plugin-sdk/poll-runtime";
import { createSubsystemLogger, getChildLogger } from "eve-agent/plugin-sdk/runtime-env";
import {
...
function requireOutboundActiveWebListener(params: { cfg: EVEConfig; accountId?: string }): {
  accountId: string;
  ...
  if (!listener) {
    throw new Error(
      `No active WhatsApp Web listener (account: ${resolvedAccountId}). Start the gateway, then link WhatsApp with: ${formatCliCommand(`eve channels login --channel whatsapp --account ${resolvedAccountId}`)}.`,
    );
  }
