...
import { info, success } from "eve-agent/plugin-sdk/runtime-env";
...
export async function pickWebChannel(
  ...
  if (!auth.exists) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("eve channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
