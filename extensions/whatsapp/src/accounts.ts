...
import type { DmPolicy, GroupPolicy, ReplyToMode } from "eve-agent/plugin-sdk/config-contracts";
import { resolveOAuthDir } from "eve-agent/plugin-sdk/state-paths";
...
export function listWhatsAppAuthDirs(cfg: EVEConfig): string[] {
...
export function hasAnyWhatsAppAuth(cfg: EVEConfig): boolean {
...
export function resolveWhatsAppAuthDir(params: { cfg: EVEConfig; accountId: string }): {
  authDir: string;
  isLegacy: boolean;
} {
...
export function listEnabledWhatsAppAccounts(cfg: EVEConfig): ResolvedWhatsAppAccount[] {
