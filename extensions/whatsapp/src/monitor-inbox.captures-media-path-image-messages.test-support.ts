...
vi.mock("eve-agent/plugin-sdk/logging-core", async () => {
  const actual = await vi.importActual<typeof import("eve-agent/plugin-sdk/logging-core")>(
    "eve-agent/plugin-sdk/logging-core",
  );
  return {
    ...actual,
    getChildLogger: () => ({
...
describe("web monitor inbox", () => {
  ...
  it("detaches inbound listeners and ends the socket on close()", async () => {
    ...
    expect(closeError?.message).toBe("EVE WhatsApp listener close");
