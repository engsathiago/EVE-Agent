...
vi.mock("eve-agent/plugin-sdk/runtime-env", async (importOriginal) => {
  const actual = await importOriginal<typeof import("eve-agent/plugin-sdk/runtime-env")>();
  const makeLogger = () => {
    const logger = {
      subsystem: "test",
      isEnabled: () => true,
...
vi.mock("eve-agent/plugin-sdk/system-event-runtime.js", () => ({
  enqueueSystemEvent: (...args: unknown[]) => messageQueueMock(...args),
}));
