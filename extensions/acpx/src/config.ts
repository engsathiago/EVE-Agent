...
import { formatPluginConfigIssue } from "eve-agent/plugin-sdk/extension-shared";
...
const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "eve-plugin-tools";
const ACPX_EVE_TOOLS_MCP_SERVER_NAME = "eve-tools";
const requireFromHere = createRequire(import.meta.url);

function isAcpxPluginRoot(dir: string): boolean {
  return (
    fs.existsSync(path.join(dir, "eve.plugin.json")) &&
...
function resolveAcpxPluginRootFromEVELayout(moduleUrl: string): string | null {
  let cursor = path.dirname(fileURLToPath(moduleUrl));
  for (let i = 0; i < 5; i += 1) {
    const candidates = [
...
export function resolveAcpxPluginRoot(moduleUrl: string = import.meta.url): string {
  ...
  return (
    ...
    // Scan common EVE layouts before falling back to the nearest path guess.
    resolveAcpxPluginRootFromEVELayout(moduleUrl) ??
    resolvedRoot
  );
}
...
function resolveEVERoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
...
function resolvePluginToolsMcpServerConfig(moduleUrl: string = import.meta.url): McpServerConfig {
  ...
  const eveRoot = resolveEVERoot(pluginRoot);
  const distEntry = path.join(eveRoot, "dist", "mcp", "plugin-tools-serve.js");
  ...
  const sourceEntry = path.join(eveRoot, "src", "mcp", "plugin-tools-serve.ts");
  return {
    command: process.execPath,
...
function resolveEVEToolsMcpServerConfig(moduleUrl: string = import.meta.url): McpServerConfig {
  const pluginRoot = resolveAcpxPluginRoot(moduleUrl);
  const eveRoot = resolveEVERoot(pluginRoot);
  const distEntry = path.join(eveRoot, "dist", "mcp", "eve-tools-serve.js");
  if (fs.existsSync(distEntry)) {
    return {
      command: process.execPath,
      args: [distEntry],
  ...
  const sourceEntry = path.join(eveRoot, "src", "mcp", "eve-tools-serve.ts");
  return {
    command: process.execPath,
    args: ["--import", resolveTsxImportSpecifier(), sourceEntry],
  };
...
function resolveConfiguredMcpServers(params: {
  ...
  if (params.eveToolsMcpBridge && resolved[ACPX_EVE_TOOLS_MCP_SERVER_NAME]) {
    throw new Error(
      `mcpServers.${ACPX_EVE_TOOLS_MCP_SERVER_NAME} is reserved when eveToolsMcpBridge=true`,
    );
  }
...
/** Convert EVE MCP server config into ACPX runtime MCP server entries. */
...
export function resolveAcpxPluginConfig(params: {
  ...
  const eveToolsMcpBridge = normalized.eveToolsMcpBridge === true;
