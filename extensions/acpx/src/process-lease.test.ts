...
describe("createAcpxProcessLeaseStore", () => {
  ...
  beforeEach(async () => {
    resetPluginStateStoreForTests();
    stateDir = await mkdtemp(path.join(tmpdir(), "eve-acpx-leases-"));
...
describe("withAcpxLeaseEnvironment", () => {
  it("adds lease environment and wrapper args on POSIX", () => {
    ...
    expect(command).toBe(
      [
        "env",
        `${EVE_ACPX_LEASE_ID_ENV}=lease-test`,
        `${EVE_GATEWAY_INSTANCE_ID_ENV}=gateway-test`,
        "node /tmp/eve/acpx/codex-acp-wrapper.mjs",
        EVE_ACPX_LEASE_ID_ARG,
        "lease-test",
        EVE_GATEWAY_INSTANCE_ID_ARG,
        "gateway-test",
  ...
  it("keeps Windows logs keyed by lease id with wrapper args", () => {
    const command = withAcpxLeaseEnvironment({
      command: "node C:/eve/acpx/codex-acp-wrapper.mjs",
      leaseId: "lease-test",
      gatewayInstanceId: "gateway-test",
      platform: "win32",
    });
    ...
    expect(command).toBe(
      [
        "node C:/eve/acpx/codex-acp-wrapper.mjs",
        EVE_ACPX_LEASE_ID_ARG,
        "lease-test",
        EVE_GATEWAY_INSTANCE_ID_ARG,
        "gateway-test",
    ...
    expect(command).not.toContain(`${EVE_ACPX_LEASE_ID_ENV}=`);
    expect(command).not.toContain(`${EVE_GATEWAY_INSTANCE_ID_ENV}=`);
  });
});
