/**
 * Persistent lease store for ACPX wrapper processes. Leases let EVE attach
...
export const EVE_ACPX_LEASE_ID_ENV = "EVE_ACPX_LEASE_ID";
/** Environment variable carrying the owning gateway instance id. */
export const EVE_GATEWAY_INSTANCE_ID_ENV = "EVE_GATEWAY_INSTANCE_ID";
/** CLI argument carrying the ACPX process lease id for platforms without env wrapping. */
export const EVE_ACPX_LEASE_ID_ARG = "--eve-acpx-lease-id";
/** CLI argument carrying the owning gateway instance id. */
export const EVE_GATEWAY_INSTANCE_ID_ARG = "--eve-gateway-instance-id";
...
export function withAcpxLeaseEnvironment(params: {
  ...
  return [
    "env",
    `${EVE_ACPX_LEASE_ID_ENV}=${quoteEnvValue(params.leaseId)}`,
    `${EVE_GATEWAY_INSTANCE_ID_ENV}=${quoteEnvValue(params.gatewayInstanceId)}`,
