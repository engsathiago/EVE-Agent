...
The ACPX extension is a thin EVE wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `eve/acpx`, not in this extension.
...
Use this flow when EVE needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `eve/acpx` repo first.
2. In EVE, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild EVE and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for EVE ACP validation.
...
If a change feels like shared ACP runtime behavior instead of EVE-specific glue, move it to `eve/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
