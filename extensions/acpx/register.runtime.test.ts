...
vi.mock("eve-agent/plugin-sdk/acp-runtime-backend", () => ({
...
const previousSkipRuntime = process.env.EVE_SKIP_ACPX_RUNTIME;

function restoreEnv(): void {
  if (previousSkipRuntime === undefined) {
    delete process.env.EVE_SKIP_ACPX_RUNTIME;
  } else {
    process.env.EVE_SKIP_ACPX_RUNTIME = previousSkipRuntime;
  }
}

function createServiceContext() {
  return {
    workspaceDir: "/tmp/eve-acpx-register-test",
    stateDir: "/tmp/eve-acpx-register-test/state",
    config: {},
    logger: {
      info: vi.fn(),
      warn: vi.fn(),
