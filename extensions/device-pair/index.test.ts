...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredEVETmpDir: vi.fn(() => path.join(os.tmpdir(), "eve-device-pair-tests")),
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredEVETmpDir: pluginApiMocks.resolvePreferredEVETmpDir,
...
function createApi(params?: {
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as EVEPluginApi["runtime"],
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredEVETmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredEVETmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the EVE iOS app:");
    ...
    expect(text).not.toContain("![EVE pairing QR]");
  });
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the EVE iOS app:");
...
describe("device-pair /pair default setup code", () => {
  ...
  it("allows mdns cleartext setup urls", async () => {
    ...
    expect(pluginApiMocks.issueDeviceBootstrapToken).toHaveBeenCalledTimes(1);
    expect(requireText(result)).toContain("Gateway: ws://eve.local:18789");
  });
