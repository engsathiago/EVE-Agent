...
describe("msteams polls", () => {
  ...
  it("stores and records poll votes", async () => {
    const home = await fs.promises.mkdtemp(path.join(os.tmpdir(), "eve-msteams-polls-"));
...
const createStateStore = async () => {
  const stateDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "eve-msteams-polls-"));
...
describe("state poll store", () => {
  ...
  it("ignores legacy JSON polls at runtime", async () => {
    ...
    await expect(
      fs.promises.access(path.join(stateDir, "state", "eve.sqlite")),
  ...
  it("deletes vote buckets when pruning over the poll cap", async () => {
    const stateDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "eve-msteams-polls-"));
    const env = { ...process.env, EVE_STATE_DIR: stateDir };
