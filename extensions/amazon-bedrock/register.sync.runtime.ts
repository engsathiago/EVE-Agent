...
import { registerApiProvider, streamSimple } from "eve-agent/plugin-sdk/llm";
...
import { streamWithPayloadPatch } from "eve-agent/plugin-sdk/provider-stream-shared";
 ...
 * IDs that may not contain the model name). When EVE's `isAnthropicBedrockModel`
...
function needsCachePointInjection(modelId: string): boolean {
  ...
  // Check if EVE identifies this as an Anthropic model via the ARN heuristic.
 ...
 * same matcher EVE uses for direct model IDs.
 */
 ...
 * the EVE config region differs from the profile's home region.
 */
...
export function registerAmazonBedrockPlugin(api: EVEPluginApi): void {
