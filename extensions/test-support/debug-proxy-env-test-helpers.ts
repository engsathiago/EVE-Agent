...
export function installDebugProxyTestResetHooks() {
  ...
  afterEach(async () => {
    const { closeDebugProxyCaptureStore } = await import("eve-agent/plugin-sdk/proxy-capture");
    const { closeEVEStateDatabaseForTest } =
      await import("eve-agent/plugin-sdk/sqlite-runtime-testing");
