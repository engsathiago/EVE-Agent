...
vi.mock("eve-agent/plugin-sdk/media-understanding", async () => {
  const actual = await vi.importActual<typeof import("eve-agent/plugin-sdk/media-understanding")>(
    "eve-agent/plugin-sdk/media-understanding",
  );
  return {
    ...actual,
...
afterAll(() => {
  vi.doUnmock("eve-agent/plugin-sdk/media-understanding");
  vi.resetModules();
});
