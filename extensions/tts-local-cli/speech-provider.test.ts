...
import type { SpeechProviderConfig, SpeechSynthesisRequest } from "eve-agent/plugin-sdk/speech-core";
...
const TEST_CFG = {} as EVEConfig;

function createCliFixture(): { dir: string; script: string } {
  const dir = mkdtempSync(path.join(os.tmpdir(), "eve-cli-tts-test-"));
...
describe("buildCliSpeechProvider", () => {
  ...
  it("can synthesize through a real local CLI fixture and ffmpeg", async () => {
    if (process.env.EVE_LIVE_TEST !== "1") {
      return;
    }
    const fixture = createCliFixture();
    const rawFfmpeg = await vi.importActual<typeof import("eve-agent/plugin-sdk/media-runtime")>(
