...
export function isCanvasPluginEnabled(config?: EVEConfig): boolean {
  if (!config) {
    return true;
  }
...
export function isCanvasHostEnabled(config?: EVEConfig): boolean {
...
export const canvasConfigSchema: CanvasPluginConfigSchema = {
  ...
  uiHints: {
    ...
    "host.root": {
      ...
      help: "Directory to serve. Defaults to the EVE state canvas directory.",
      advanced: true,
    },
    "host.port": {
