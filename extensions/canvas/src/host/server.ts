...
import { detectMime } from "eve-agent/plugin-sdk/media-mime";
import { isTruthyEnvValue, type RuntimeEnv } from "eve-agent/plugin-sdk/runtime-env";
import { resolveStateDir } from "eve-agent/plugin-sdk/state-paths";
import {
  lowercasePreservingWhitespace,
  normalizeOptionalString,
} from "eve-agent/plugin-sdk/string-coerce-runtime";
import { ensureDir, resolveUserPath } from "eve-agent/plugin-sdk/text-utility-runtime";
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.eveCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.eveCanvasA2UIAction &&
        typeof window.eveCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.eveSendUserAction === "function";
  ...
  window.addEventListener("eve:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android EVE node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.eveSendUserAction === "function"
        ? window.eveSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.EVE_SKIP_CANVAS_HOST)) {
    return true;
  }
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>EVE Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
