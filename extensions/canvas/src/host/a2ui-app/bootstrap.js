...
class EVEA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  connectedCallback() {
    ...
    globalThis.eveA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["eve:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    if (handler?.postMessage) {
      try {
        ...
        if (handler === globalThis.eveCanvasA2UIAction) {
...
if (!customElements.get("eve-a2ui-host")) {
  customElements.define("eve-a2ui-host", EVEA2UIHost);
}
