...
import { withStateDirEnv } from "eve-agent/plugin-sdk/test-env";
...
describe("canvas host state dir defaults", () => {
  ...
  it("uses EVE_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("eve-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("EVE Canvas");
      } finally {
        await handler.close();
      }
    });
