...
async function main() {
  ...
  if (!hasA2uiPackage || !hasAppDir) {
    ...
    if (process.env.EVE_SPARSE_PROFILE || process.env.EVE_A2UI_SKIP_MISSING === "1") {
      console.error(
        "A2UI package missing; skipping bundle because EVE_A2UI_SKIP_MISSING=1 or EVE_SPARSE_PROFILE is set.",
      );
      return;
    }
