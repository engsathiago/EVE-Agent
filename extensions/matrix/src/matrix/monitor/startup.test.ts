...
function createDeps(
  ...
  return {
    ...
    summarizeMatrixDeviceHealth: vi.fn(() => ({
      ...
      staleEVEDevices: [] as MatrixManagedDeviceInfo[],
      currentEVEDevices: [] as MatrixManagedDeviceInfo[],
    })),
...
describe("runMatrixStartupMaintenance", () => {
  ...
  it("reports stale devices, pending verification, and restored legacy backups", async () => {
    ...
    expect(params.logger.warn).toHaveBeenCalledWith(
      "matrix: stale EVE devices detected for @bot:example.org: DEV123. Run 'eve matrix devices prune-stale --account ops' to keep encrypted-room trust healthy.",
    );
