...
function formatMatrixPostHealthySyncDecryptionHint(accountId: string): string {
  return (
    ...
    `Check 'eve matrix verify status --verbose --account ${accountId}' and 'eve matrix devices list --account ${accountId}'.`
  );
}
...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another EVE runtime is using the same account. " +
    `Check 'eve matrix verify status --verbose --account ${accountId}' and 'eve matrix devices list --account ${accountId}'.`
  );
}
