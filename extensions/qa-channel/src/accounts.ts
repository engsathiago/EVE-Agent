...
import { createAccountListHelpers } from "eve-agent/plugin-sdk/account-helpers";
import { DEFAULT_ACCOUNT_ID, normalizeAccountId } from "eve-agent/plugin-sdk/account-id";
import { resolveMergedAccountConfig } from "eve-agent/plugin-sdk/account-resolution-runtime";
...
export function resolveQaChannelAccount(params: {
  ...
  const botUserId = merged.botUserId?.trim() || "eve";
  const botDisplayName = merged.botDisplayName?.trim() || "EVE QA";
  return {
    accountId,
    enabled,
