...
const LIVE_CACHE_MODEL_ID =
  process.env.EVE_LIVE_OPENROUTER_CACHE_MODEL?.trim() || "deepseek/deepseek-v3.2";
const liveEnabled = OPENROUTER_API_KEY.trim().length > 0 && process.env.EVE_LIVE_TEST === "1";
const liveCatalogEnabled = process.env.EVE_LIVE_TEST === "1";
const describeLive = liveEnabled ? describe : describe.skip;
const describeCatalogLive = liveCatalogEnabled ? describe : describe.skip;
const describeCacheLive =
  liveEnabled && process.env.EVE_LIVE_CACHE_TEST === "1" ? describe : describe.skip;
