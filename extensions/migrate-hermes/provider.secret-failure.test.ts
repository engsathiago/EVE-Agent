...
import { resolveAuthStorePathForDisplay } from "eve-agent/plugin-sdk/agent-runtime";
...
vi.mock("eve-agent/plugin-sdk/provider-auth", async (importOriginal) => ({
  ...(await importOriginal<typeof import("eve-agent/plugin-sdk/provider-auth")>()),
...
async function makeTempRoot() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "eve-hermes-secret-failure-"));
  tempRoots.add(root);
  return root;
}
