...
import type { EVEConfig } from "eve-agent/plugin-sdk/provider-onboard";
...
describe("kimi web search provider", () => {
  ...
  it("accepts final responses backed by Kimi web search tool replay", async () => {
      ...
      .mockResolvedValueOnce(
        jsonResponse({
          choices: [
            {
              finish_reason: "stop",
              message: { content: "EVE is available on GitHub." },
            },
          ],
        }),
      );
    ...
    await withEnvAsync({ KIMI_API_KEY: "kimi-test-key" }, async () => {
      ...
      expect(result.provider).toBe("kimi");
      expectStringFieldContains(result, "content", "EVE is available on GitHub.");
  ...
  it("accepts final responses with search result citations", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      jsonResponse({
        search_results: [{ title: "EVE", url: "https://github.com/engsathiago/eve-agent" }],
        choices: [
          {
            finish_reason: "stop",
            message: { content: "EVE is on GitHub." },
          },
        ],
      }),
    );
    ...
    await withEnvAsync({ KIMI_API_KEY: "kimi-test-key" }, async () => {
      ...
      expect(result.provider).toBe("kimi");
      expectStringFieldContains(result, "content", "EVE is on GitHub.");
      expect(result.citations).toEqual(["https://github.com/engsathiago/eve-agent"]);
      expect(result).not.toHaveProperty("error");
    });
  });
