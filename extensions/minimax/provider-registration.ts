...
import { MINIMAX_FAST_MODE_STREAM_HOOKS } from "eve-agent/plugin-sdk/provider-stream-family";
import { fetchMinimaxUsage } from "eve-agent/plugin-sdk/provider-usage";
...
function getProviderBaseUrl(cfg: EVEConfig, providerId: string): string | undefined {
  return normalizeOptionalString(cfg.models?.providers?.[providerId]?.baseUrl);
}

function resolveMinimaxUsageBaseUrl(cfg: EVEConfig): string | undefined {
...
export function registerMinimaxProviders(api: EVEPluginApi) {
