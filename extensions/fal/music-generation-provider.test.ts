...
import { expectExplicitMusicGenerationCapabilities } from "eve-agent/plugin-sdk/provider-test-contracts";
...
vi.mock("eve-agent/plugin-sdk/provider-http", async (importOriginal) => {
  const original = await importOriginal<typeof import("eve-agent/plugin-sdk/provider-http")>();
  return {
    ...original,
