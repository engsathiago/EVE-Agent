...
`eve.invoke --each`) without adding new EVE code per workflow.
...
This extension depends on EVE internal modules (the embedded agent runner).
It is intended to ship as a **bundled** EVE extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.eve/extensions` as a standalone plugin directory.
