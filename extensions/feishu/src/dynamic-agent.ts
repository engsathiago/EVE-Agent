...
import { resolveChannelConfigWrites } from "eve-agent/plugin-sdk/channel-config-writes";
import { normalizeAccountId, resolveAgentRoute } from "eve-agent/plugin-sdk/routing";
import type { EVEConfig, PluginRuntime } from "../runtime-api.js";
import { resolveFeishuAccount } from "./accounts.js";
import type { DynamicAgentCreationConfig } from "./types.js";

type MaybeCreateDynamicAgentResult = {
  created: boolean;
  updatedCfg: EVEConfig;
  agentId?: string;
};
...
class DynamicAgentMutationSkipped extends Error {
  constructor(readonly cfg: EVEConfig) {
...
export async function maybeCreateDynamicAgent(params: {
  ...
  accountId: string;
  senderOpenId: string;
  canCreateForConfig: (cfg: EVEConfig) => Promise<boolean>;
  log: (msg: string) => void;
  ...
  const currentCfg = runtime.config.current() as EVEConfig;
  ...
  let skippedCfg: EVEConfig | undefined;
  ...
  return {
    ...
    updatedCfg: runtime.config.current() as EVEConfig,
