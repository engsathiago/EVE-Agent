...
import { createTestWizardPrompter } from "eve-agent/plugin-sdk/plugin-test-runtime";
import { NON_ENV_SECRETREF_MARKER } from "eve-agent/plugin-sdk/provider-auth-runtime";
import { createNonExitingRuntime } from "eve-agent/plugin-sdk/runtime-env";
...
vi.mock("eve-agent/plugin-sdk/provider-auth", async (importOriginal) => {
  const original = await importOriginal<typeof import("eve-agent/plugin-sdk/provider-auth")>();
  return {
    ...original,
...
describe("xai web search config resolution", () => {
  ...
  it("treats unresolved non-env SecretRefs as missing credentials instead of using env fallback", async () => {
    await withEnvAsync({ XAI_API_KEY: "ambient-xai-test-key" }, async () => {
      ...
      const result = await maybeTool.execute({ query: "EVE" });
  ...
  it("uses xAI OAuth auth before API-key fallback for web search", async () => {
    ...
    const tool = provider.createTool({
      config: {
        agents: {
          list: [{ id: "main", default: true, agentDir: "/tmp/eve-xai-main-agent" }],
        },
        plugins: {
          entries: {
            xai: {
    ...
    await tool.execute({ query: "EVE Grok OAuth web search" });

    expect(providerAuthRuntimeMocks.resolveApiKeyForProvider).toHaveBeenCalledWith(
      expect.objectContaining({
        provider: "xai",
        agentDir: "/tmp/eve-xai-main-agent",
      }),
    );
  ...
  it("uses the active agentDir for xAI OAuth web search auth", async () => {
    ...
    const provider = createXaiWebSearchProvider();
    const tool = provider.createTool({
      agentDir: "/tmp/eve-xai-active-agent",
      config: {
        agents: {
          list: [
            { id: "main", default: true, agentDir: "/tmp/eve-xai-main-agent" },
            { id: "side", agentDir: "/tmp/eve-xai-active-agent" },
          ],
        },
      },
    });
    ...
    await tool.execute({ query: "EVE Grok active agent OAuth web search" });
  ...
  it("refreshes xAI OAuth auth and retries web search after a 401", async () => {
    ...
    const result = await tool.execute({ query: "EVE Grok OAuth refresh test" });
  ...
  it("falls back to xAI API-key auth when OAuth refresh cannot recover", async () => {
    ...
    const result = await tool.execute({ query: "EVE Grok API fallback test" });
  ...
  it("falls back to an xAI API-key auth profile when stale OAuth remains first", async () => {
    ...
    const result = await tool.execute({ query: "EVE Grok profile fallback test" });
  ...
  it("falls back to env auth after a stale xAI API-key auth profile returns unauthorized", async () => {
    ...
    const result = await tool.execute({ query: "EVE Grok API-key fallback test" });
  ...
  it("routes Grok web search through plugin webSearch.baseUrl", async () => {
    ...
    await tool.execute({ query: "EVE Grok proxy test" });
  ...
  it("reports malformed xAI web search JSON as a provider error", async () => {
    ...
    await expect(tool.execute({ query: "EVE" })).rejects.toThrow(
...
describe("xai provider models", () => {
  ...
  it("publishes the remaining Grok 3 family in the EVE catalog", () => {
