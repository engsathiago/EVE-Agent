// Shared User-Agent for xAI sidecar HTTP/WS requests; mirrors `formatEVEUserAgent`.

import { EVE_VERSION as PACKAGE_VERSION } from "eve-agent/plugin-sdk/agent-harness-runtime";

const ORIGINATOR = "eve";
...
function resolveXaiUserAgentVersion(): string {
  ...
  const envVersion = trimToUndefined(process.env.EVE_VERSION);
  if (envVersion) {
    return envVersion;
  }
  ...
  return (
    trimToUndefined(process.env.EVE_SERVICE_VERSION) ??
...
// an empty record so the eve identity is not forwarded to the proxy.
