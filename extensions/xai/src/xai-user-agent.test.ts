...
describe("xaiUserAgent", () => {
  ...
  it("prefers EVE_VERSION env over the bundled package version", () => {
    vi.stubEnv("EVE_VERSION", "2026.3.22");
    expect(xaiUserAgent()).toBe("eve/2026.3.22");
  });

  it("falls back to EVE_SERVICE_VERSION when EVE_VERSION is unset", () => {
    vi.stubEnv("EVE_VERSION", "");
    vi.stubEnv("EVE_SERVICE_VERSION", "2026.3.99");
    // EVE_VERSION from the SDK is the bundled VERSION constant. In a dev
    ...
    expect(result.startsWith("eve/")).toBe(true);
    expect(result).not.toBe("eve/");
  });

  it("returns the eve/<version> shape", () => {
    vi.stubEnv("EVE_VERSION", "2026.5.16");
    expect(xaiUserAgent()).toMatch(/^eve\/\d+\.\d+\.\d+$/u);
  });
});
