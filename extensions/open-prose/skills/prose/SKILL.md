...
metadata: { "eve": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == EVE `sessions_spawn`
- **File I/O** == EVE `read`/`write`
- **Remote fetch** == EVE `web_fetch` (or `exec` with curl when POST is required)
