...
description: Maintain the EVE memory wiki vault with deterministic pages, managed blocks, and source-backed updates.
---
...
- Use `eve wiki ingest`, `eve wiki compile`, and `eve wiki lint` as the default maintenance loop.
- In `bridge` mode, run `eve wiki bridge import` before relying on search results if you need the latest public memory artifacts pulled in.
- In `unsafe-local` mode, use `eve wiki unsafe-local import` only when the user explicitly opted into private local path access.
