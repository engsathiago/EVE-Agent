...
Persistent wiki compiler and Obsidian-friendly knowledge vault for **EVE**.
...
Key beliefs can live in structured `claims` frontmatter with per-claim evidence, confidence, and status. Compile also emits machine-readable digests under `.eve-wiki/cache/` so agent/runtime consumers do not have to scrape markdown pages.
...
Unmanaged raw Markdown can live under `sources/` without EVE page frontmatter. Add `<!-- eve:wiki:raw-source -->` near the top of the page body to opt it out of wiki page metadata and freshness lint; generated or source-sync tracked imports still require their structured metadata.
...
- The compiled agent digests in `.eve-wiki/cache/agent-digest.json` and `.eve-wiki/cache/claims.jsonl` are the stable machine-facing view of the wiki.
