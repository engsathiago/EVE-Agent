...
import { retryTransientMemoryRead } from "eve-agent/plugin-sdk/memory-core-host-engine-storage";
import {
...
const AGENT_DIGEST_PATH = ".eve-wiki/cache/agent-digest.json";
...
async function writeDashboardPage(params: {
  ...
  const updatedBody = replaceManagedMarkdownBlock({
    ...
    heading: "## Generated",
    startMarker: `<!-- eve:wiki:${path.basename(params.definition.relativePath, ".md")}:start -->`,
    endMarker: `<!-- eve:wiki:${path.basename(params.definition.relativePath, ".md")}:end -->`,
  ...
  for (const group of COMPILE_PAGE_GROUPS) {
    ...
    if (
      await writeManagedMarkdownFile({
        ...
        relativePath,
        title: group.heading,
        startMarker: `<!-- eve:wiki:${group.dir}:index:start -->`,
        endMarker: `<!-- eve:wiki:${group.dir}:index:end -->`,
