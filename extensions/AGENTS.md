...
- Extension production code should import from `eve-agent/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `eve.plugin.json` and the package
  `eve` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `eve-agent/plugin-sdk/*`.
