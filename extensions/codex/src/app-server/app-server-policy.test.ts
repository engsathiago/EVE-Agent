...
describe("Codex app-server policy", () => {
  it("keeps implicit Codex yolo approval policy when untrusted approvals are disallowed", () => {
    ...
    const resolved = resolveCodexAppServerForEVEToolPolicy({
      appServer,
  ...
  it("promotes implicit yolo approval policy when EVE tool policy requires review", () => {
    const appServer = resolveCodexAppServerRuntimeOptions({ env: {}, requirementsToml: null });
  ...
  it("preserves explicit operator app-server policy", () => {
    ...
    const explicitConfig = resolveCodexAppServerForEVEToolPolicy({
      appServer,
    ...
    const explicitEnv = resolveCodexAppServerForEVEToolPolicy({
      appServer,
      pluginConfig: readCodexPluginConfig({}),
      env: { EVE_CODEX_APP_SERVER_APPROVAL_POLICY: "never" },
      shouldPromote: true,
      canUseUntrustedApprovalPolicy: true,
    });
    const explicitRequirements = resolveCodexAppServerForEVEToolPolicy({
