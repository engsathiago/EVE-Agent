...
import type { AuthProfileStore } from "eve-agent/plugin-sdk/agent-runtime";
...
const EVE_CODEX_APP_SERVER_ARGS_ENV_VAR = "EVE_CODEX_APP_SERVER_ARGS";
...
function buildPrivateCodexAppServerStartOptions(
  ...
  const privateEnv = Object.fromEntries(
    Object.entries(start.env ?? {}).filter(
      ([name]) => name.trim().toUpperCase() !== EVE_CODEX_APP_SERVER_ARGS_ENV_VAR,
    ),
  );
  const clearEnv = (start.clearEnv ?? []).filter((name) => {
    const normalized = name.trim().toUpperCase();
    return normalized !== "CODEX_HOME" && normalized !== EVE_CODEX_APP_SERVER_ARGS_ENV_VAR;
  });
  return {
    ...start,
    ...
    clearEnv: [...clearEnv, EVE_CODEX_APP_SERVER_ARGS_ENV_VAR],
  };
}
...
function createCodexBoundedApprovalHandler(taskLabel: string) {
  return (request: { method: string }): JsonValue | undefined => {
    if (
      ...
      return {
        decision: "decline",
        reason: `EVE Codex ${taskLabel} does not grant tool or file approvals.`,
      };
    }
    ...
    if (request.method.includes("requestApproval")) {
      return {
        decision: "decline",
        reason: `EVE Codex ${taskLabel} does not grant native approvals.`,
      };
    }
