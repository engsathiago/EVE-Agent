...
import { initializeGlobalHookRunner } from "eve-agent/plugin-sdk/hook-runtime";
...
describe("runCodexAppServerAttempt dynamic tools", () => {
  it("passes the live run session key to Codex dynamic tools when sandbox policy uses another key", () => {
    ...
    expect(testing.resolveEVECodingToolsSessionKeys(params, "agent:main:main")).toEqual({
  ...
  it("emits normalized tool progress around app-server dynamic tool requests", async () => {
    ...
    expect(toolResult.contentItems?.[0]?.text).toMatch(/^Unknown EVE tool: lookup$/u);
    ...
    expect(resultEvent?.data?.result?.content?.[0]?.text).toBe("Unknown EVE tool: lookup");
      ...
      emitDynamicToolTerminalDiagnostic({
        ...
        response: {
          ...
          contentItems: [
            {
              type: "inputText",
              text: "EVE dynamic tool call timed out after 1ms while running tool echo.",
            },
          ],
        },
      });
