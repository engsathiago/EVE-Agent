...
import { resetDiagnosticEventsForTest } from "eve-agent/plugin-sdk/diagnostic-runtime";
import { clearInternalHooks, resetGlobalHookRunner } from "eve-agent/plugin-sdk/hook-runtime";
import { clearMemoryPluginState } from "eve-agent/plugin-sdk/memory-core-host-runtime-core";
import { clearPluginCommands } from "eve-agent/plugin-sdk/plugin-runtime";
...
export function setupRunAttemptTestHooks(): void {
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(resolvePreferredEVETmpDir(), "eve-codex-run-"));
  });
  ...
  afterEach(async () => {
    ...
    testing.resetEVECodingToolsFactoryForTests();
