/**
 * Policy promotion for Codex app-server runs that can safely use EVE tool
 * approvals.
 */
import {
 ...
 * requirements allow EVE to handle tool approvals.
 */
export function resolveCodexAppServerForEVEToolPolicy(params: {
  ...
  const explicitMode =
    ...
    isCodexAppServerPolicyMode(params.env.EVE_CODEX_APP_SERVER_MODE);
  const explicitApprovalPolicy =
    params.pluginConfig.appServer?.approvalPolicy !== undefined ||
    isCodexAppServerApprovalPolicy(params.env.EVE_CODEX_APP_SERVER_APPROVAL_POLICY) ||
