...
import {
  ...
  isPendingEVEDynamicToolCompletionNotification,
...
export function applyCodexTurnNotificationState(params: {
  ...
  pendingEVEDynamicToolCompletionIds: Set<string>;
  ...
  const trackedDynamicToolCompletion = isPendingEVEDynamicToolCompletionNotification(
    notification,
    params.pendingEVEDynamicToolCompletionIds,
  );
  ...
  if (
    ...
    // accepts a turn or after EVE hands a turn-scoped request result
    // back to Codex. Bookkeeping that closes the just-served EVE
  ...
  if (trackedDynamicToolCompletion) {
    ...
    if (itemId) {
      params.pendingEVEDynamicToolCompletionIds.delete(itemId);
      params.onScheduleTerminalDynamicToolReleaseCheck();
    }
  }
