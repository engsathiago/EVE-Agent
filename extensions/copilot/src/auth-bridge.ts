 ...
 *     use cases.
 *   - Falls back to EVE_GITHUB_TOKEN, COPILOT_GITHUB_TOKEN,
 ...
 *     (`<eveHome>/.eve/agents/<agentId>/copilot`, or
 *     `<agentDir>/copilot` when an agent directory is supplied) that
 *     respects `EVE_HOME` for the home directory root.
 ...
 *   4. EVE_GITHUB_TOKEN, then COPILOT_GITHUB_TOKEN, then
...
function resolveCopilotHome(args: {
  ...
  // "copilot" subdir so it cannot collide with EVE's own files
  ...
  const eveHome = readString(args.env.EVE_HOME);
  const rootHome = eveHome ? resolve(eveHome) : safeHomeDir(args.homeDir);
  // Per-agent isolation per proposal section 3.6:
  //   <eveHome>/.eve/agents/<agentId>/copilot
  return resolve(join(rootHome, ".eve", "agents", args.agentId, "copilot"));
}
...
function readEnvTokenFallback(
  ...
  // EVE_GITHUB_TOKEN is the harness-specific override and stays at
  ...
  const candidates: Array<{ name: string; value: string | undefined }> = [
    { name: "EVE_GITHUB_TOKEN", value: readString(env.EVE_GITHUB_TOKEN) },
