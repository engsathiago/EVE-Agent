...
import type { AgentHarnessAttemptParams } from "eve-agent/plugin-sdk/agent-harness-runtime";
...
vi.mock("eve-agent/plugin-sdk/agent-harness", async (importOriginal) => {
  const actual = await importOriginal<typeof import("eve-agent/plugin-sdk/agent-harness")>();

  return {
    ...actual,
    createEVECodingTools: vi.fn(() => [
      {
...
const LIVE = isLiveTestEnabled(["EVE_COPILOT_AGENT_LIVE_TEST"]);
const TOKEN =
  process.env.EVE_COPILOT_AGENT_LIVE_TOKEN ||
...
describeLive("copilot agent runtime live smoke", () => {
  it("runs one turn on gpt-4.1 with one custom tool", async () => {
    ...
    const copilotHome = await mkdtemp(join(tmpdir(), "eve-copilot-live-"));
