...
import type { AnyAgentTool, SandboxContext } from "eve-agent/plugin-sdk/agent-harness-runtime";
import { afterEach, describe, expect, it, vi } from "vitest";
import {
  createCopilotToolBridge,
  convertEVEToolToSdkTool,
  supportsModelTools,
} from "./tool-bridge.js";
...
describe("supportsModelTools", () => {
  it("returns true for github-copilot and false otherwise", () => {
    ...
    expect(supportsModelTools("eve")).toBe(false);
...
describe("createCopilotToolBridge", () => {
  it("returns empty arrays for unsupported providers without calling the seam", async () => {
    const createEVECodingTools = vi.fn(async () => [makeTool()]);
    ...
    expect(createEVECodingTools).toHaveBeenCalledTimes(0);
  });

  it("forwards supported fields to injected createEVECodingTools", async () => {
    ...
    expect(createEVECodingTools).toHaveBeenCalledTimes(1);
    ...
    expect(createEVECodingTools).toHaveBeenCalledWith(
  ...
  it("returns sdkTools and sourceTools with matching lengths", async () => {
    ...
    const result = await createCopilotToolBridge({
      agentId: "agent-1",
      createEVECodingTools: async () => sourceTools,
      modelId: "gpt-4o",
      modelProvider: "github-copilot",
      sessionId: "session-1",
    });
  ...
  it("throws when createEVECodingTools returns a non-array", async () => {
    await expect(
      createCopilotToolBridge({
        agentId: "agent-1",
        createEVECodingTools: async () => ({ tools: [] }) as never,
        modelId: "gpt-4o",
        modelProvider: "github-copilot",
        sessionId: "session-1",
      }),
    ).rejects.toThrow("createEVECodingTools must return an array");
  });

  it("throws when createEVECodingTools rejects and includes the cause", async () => {
    await expect(
      createCopilotToolBridge({
        agentId: "agent-1",
        createEVECodingTools: async () => {
  ...
  // F6: PI-parity tool context. The bridged EVE tools register
  ...
  describe("PI-parity attempt context (F6)", () => {
    function captureCall() {
      ...
      return {
        createEVECodingTools,
        getOpts: () =>
          (createEVECodingTools.mock.calls[0] as unknown[] | undefined)?.[0] as Record<
            string,
            unknown
          >,
      };
    ...
    it("forwards identity, owner/policy, and channel/routing fields from attemptParams", async () => {
      const { createEVECodingTools, getOpts } = captureCall();
    ...
    it("prefers the unscoped toolAuthProfileStore when building EVE tools", async () => {
      const { createEVECodingTools, getOpts } = captureCall();
  ...
  describe("sandbox forwarding (PR #86155 [P1])", () => {
    ...
    it("defaults sandbox to undefined and derives spawnWorkspaceDir from workspaceDir when no sandbox is passed (back-compat)", async () => {
      ...
      const opts = (createEVECodingTools.mock.calls[0] as unknown[] | undefined)?.[0] as {
        sandbox?: unknown;
    ...
    it("forwards an explicit sandbox and spawnWorkspaceDir verbatim to createEVECodingTools", async () => {
  ...
  // EVE attempt would suppress. These tests pin the contract.
  describe("tool-surface gating (PR #86155 [P1] round-6)", () => {
    it("short-circuits when attemptParams.disableTools is true and never calls createEVECodingTools", async () => {
      const createEVECodingTools = vi.fn(async () => [makeTool()]);
      const result = await createCopilotToolBridge({
        agentId: "agent-1",
    ...
    it("filters constructed tools to exactly the allowlist when toolsAllow is narrow", async () => {
      const createEVECodingTools = vi.fn(async () => [
        makeTool({ name: "read" }),
        makeTool({ name: "edit" }),
    ...
    it("leaves the tool list unchanged when toolsAllow is undefined", async () => {
      ...
      const createEVECodingTools = vi.fn(async () => tools);
      const result = await createCopilotToolBridge({
        agentId: "agent-1",
  ...
  describe("tool-name aliases (PR #86155 [P1] round-7)", () => {
    ...
    it("keeps core tools available for glob allowlists", async () => {
      ...
      const options = (createEVECodingTools.mock.calls[0] as unknown[] | undefined)?.[0] as {
        toolConstructionPlan?: { includeEVETools?: boolean };
      };
      expect(options?.toolConstructionPlan?.includeEVETools).toBe(true);
    });
...
describe("convertEVEToolToSdkTool", () => {
  it("throws on empty and non-string names", () => {
    expect(() => convertEVEToolToSdkTool(makeTool({ name: "" as never }), {})).toThrow(
      "tool name must be a non-empty string",
    );
    expect(() => convertEVEToolToSdkTool(makeTool({ name: 42 as never }), {})).toThrow(
      "tool name must be a non-empty string",
    );
  });

  it("throws on non-function execute", () => {
    expect(() => convertEVEToolToSdkTool(makeTool({ execute: "nope" as never }), {})).toThrow(
  ...
  it("preserves name, description, and parameters exactly", () => {
    ...
    const result = convertEVEToolToSdkTool(sourceTool, {});
  ...
  it("sets skipPermission: true so EVE's wrapped-tool internal enforcement handles permission decisions (PI-parity model)", () => {
    // Per the harness docs: every bridged EVE tool comes from
    // `createEVECodingTools`, which already wraps each tool with
  ...
  it("marks every bridged tool as overridesBuiltInTool so EVE owns names that collide with Copilot CLI built-ins (edit/read/write/bash/...)", () => {
    // Real-world dogfood found that eve's createEVECodingTools
    ...
    for (const name of ["edit", "read", "write", "bash", "live_echo"]) {
      const result = convertEVEToolToSdkTool(makeTool({ name }), {}) as SdkTool & {
  ...
  it("returns a failure result when the signal is already aborted", async () => {
    ...
    controller.abort();
    const sourceTool = makeTool();
    const sdkTool = convertEVEToolToSdkTool(sourceTool, { abortSignal: controller.signal });
  ...
  it("calls beforeExecute with the invocation context before execute", async () => {
    ...
    const sourceTool = makeTool();
    const sdkTool = convertEVEToolToSdkTool(sourceTool, { beforeExecute });
  ...
  it("returns a failure result when beforeExecute throws", async () => {
    ...
    const sourceTool = makeTool();
    const sdkTool = convertEVEToolToSdkTool(sourceTool, {
  ...
  it("calls prepareArguments and passes the prepared args and toolCallId to execute", async () => {
    ...
    const sdkTool = convertEVEToolToSdkTool(sourceTool, { onToolCompleted });
  ...
  it("converts single text content to an exact textResultForLlm", async () => {
    ...
    const sdkTool = convertEVEToolToSdkTool(makeTool({}, sourceResult), { onAgentToolResult });
  ...
  it("reports terminal tool results to the harness lifecycle bridge", async () => {
    ...
    const sdkTool = convertEVEToolToSdkTool(makeTool({}, sourceResult), {
      onToolCompleted,
    });
  ...
  it("reports thrown tool failures to the private result observer", async () => {
    ...
    const onAgentToolResult = vi.fn();
    const sdkTool = convertEVEToolToSdkTool(
      makeTool({
        execute: vi.fn(async () => {
          throw error;
        }),
  ...
  it("reports returned EVE error results to both tool observers", async () => {
    const onAgentToolResult = vi.fn();
    const onToolCompleted = vi.fn();
    const sourceResult = {
