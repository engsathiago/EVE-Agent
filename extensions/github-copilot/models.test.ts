...
vi.mock("eve-agent/plugin-sdk/provider-model-shared", async (importOriginal) => ({
  ...(await importOriginal<typeof import("eve-agent/plugin-sdk/provider-model-shared")>()),
...
vi.mock("eve-agent/plugin-sdk/json-store", () => ({
  loadJsonFile: jsonStoreMocks.loadJsonFile,
  saveJsonFile: jsonStoreMocks.saveJsonFile,
}));

vi.mock("eve-agent/plugin-sdk/state-paths", () => ({
  resolveStateDir: () => "/tmp/eve-state",
}));

import type { ProviderResolveDynamicModelContext } from "eve-agent/plugin-sdk/core";
...
describe("github-copilot token", () => {
  const cachePath = "/tmp/eve-state/credentials/github-copilot.token.json";
