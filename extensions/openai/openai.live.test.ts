...
import { runRealtimeSttLiveTest } from "eve-agent/plugin-sdk/provider-test-contracts";
...
const LIVE_MODEL_ID = process.env.EVE_LIVE_OPENAI_PLUGIN_MODEL?.trim() || "gpt-5.5";
const LIVE_IMAGE_MODEL = process.env.EVE_LIVE_OPENAI_IMAGE_MODEL?.trim() || "gpt-image-2";
const LIVE_VISION_MODEL = process.env.EVE_LIVE_OPENAI_VISION_MODEL?.trim() || "gpt-5.4-mini";
const liveEnabled = OPENAI_API_KEY.trim().length > 0 && process.env.EVE_LIVE_TEST === "1";
...
describeLive("openai plugin live", () => {
  ...
  it("streams realtime STT through the registered transcription provider", async () => {
    ...
    const cfg = createLiveConfig();
    const ttsConfig = createLiveTtsConfig();
    const phrase = "Testing EVE OpenAI realtime transcription integration test OK.";
