...
} from "eve-agent/plugin-sdk/channel-pairing";
 ...
 * for EVE. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<ResolvedTwitchAccount> =
  createChatChannelPlugin<ResolvedTwitchAccount>({
    ...
    base: {
      ...
      config: {
        listAccountIds: (cfg: EVEConfig): string[] => listAccountIds(cfg),
        resolveAccount: (cfg: EVEConfig, accountId?: string | null): ResolvedTwitchAccount => {
        ...
        defaultAccountId: (cfg: EVEConfig): string => resolveDefaultTwitchAccountId(cfg),
        isConfigured: (_account: unknown, cfg: EVEConfig): boolean =>
      ...
      resolver: {
        resolveTargets: async ({
          ...
          runtime: import("eve-agent/plugin-sdk/runtime-env").RuntimeEnv;
