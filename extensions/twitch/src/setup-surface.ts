...
import { getChatChannelMeta, type ChannelPlugin } from "eve-agent/plugin-sdk/core";
import {
  formatDocsLink,
...
function resolveSetupAccountId(cfg: EVEConfig, requestedAccountId?: string): string {
...
export const twitchSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId: requestedAccountId, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.EVE_TWITCH_ACCESS_TOKEN?.trim();
