...
import { resolvePositiveTimeoutSeconds } from "eve-agent/plugin-sdk/provider-web-search";
import {
...
export function resolveTavilySearchConfig(cfg?: EVEConfig): TavilySearchConfig {
...
export function resolveTavilyApiKey(cfg?: EVEConfig): string | undefined {
...
export function resolveTavilyBaseUrl(cfg?: EVEConfig): string {
  const search = resolveTavilySearchConfig(cfg);
  const configured =
