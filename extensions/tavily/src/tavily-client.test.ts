...
vi.mock("eve-agent/plugin-sdk/provider-web-search", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: eve", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "eve" });
  });
  ...
  it("runTavilyExtract sends X-Client-Source: eve", async () => {
