...
import { coerceSecretRef, normalizeSecretInputString } from "eve-agent/plugin-sdk/secret-input";
...
export function listQQBotAccountIds(cfg: EVEConfig): string[] {
  return listAccountIds(cfg as unknown as Record<string, unknown>);
}

/** Resolve the default QQBot account ID. */
export function resolveDefaultQQBotAccountId(cfg: EVEConfig): string {
...
/** Apply account config updates back into the EVE config object. */
