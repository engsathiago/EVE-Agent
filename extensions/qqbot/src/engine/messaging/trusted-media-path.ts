import { resolvePreferredEVETmpDir } from "eve-agent/plugin-sdk/sandbox";
import { resolveLocalPathFromRootsSync } from "eve-agent/plugin-sdk/security-runtime";
...
function trustedEVETmpRoot(): string | null {
  if (cachedTrustedTmpRoot === undefined) {
    try {
      cachedTrustedTmpRoot = resolvePreferredEVETmpDir();
    } catch {
      return null;
    }
  }
 ...
 * roots, this also trusts EVE's permission-hardened temp root, where
...
export function resolveTrustedOutboundMediaPath(
  ...
  const tmpRoot = trustedEVETmpRoot();
  if (!tmpRoot) {
    return null;
  }
  return (
