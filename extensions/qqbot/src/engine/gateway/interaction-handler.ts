...
import { isImplicitSameChatApprovalAuthorization } from "eve-agent/plugin-sdk/approval-auth-runtime";
...
async function handleApprovalButtonInteraction(params: {
  ...
  event: InteractionEvent;
  getActiveCfg?: () => EVEConfig | Record<string, unknown>;
  log?: EngineLogger;
  ...
  let cfg: EVEConfig;
  try {
    cfg = params.getActiveCfg() as EVEConfig;
  } catch (err) {
    await acknowledgeApprovalInteraction(params.creds, params.event, params.log, {
      content: "Approval is unavailable.",
    });
