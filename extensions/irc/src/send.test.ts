...
import { createSendCfgThreadingRuntime } from "eve-agent/plugin-sdk/channel-test-helpers";
...
vi.mock("eve-agent/plugin-sdk/plugin-config-runtime", async () => {
  const original = (await vi.importActual("eve-agent/plugin-sdk/plugin-config-runtime")) as Record<
    string,
    unknown
  >;
  return {
...
vi.mock("eve-agent/plugin-sdk/text-chunking", async () => {
  const original = (await vi.importActual("eve-agent/plugin-sdk/text-chunking")) as Record<
    string,
    unknown
  >;
  return {
...
afterAll(() => {
  ...
  vi.doUnmock("eve-agent/plugin-sdk/plugin-config-runtime");
