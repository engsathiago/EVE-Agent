...
import type { DiagnosticEventPrivateData } from "eve-agent/plugin-sdk/diagnostic-runtime";
// Diagnostics Prometheus tests cover service plugin behavior.
...
describe("diagnostics-prometheus service", () => {
  it("records trusted run metrics without raw diagnostic identifiers", () => {
    ...
    expect(rendered).toContain("# TYPE eve_run_completed_total counter");
    expect(rendered).toContain(
      'eve_run_completed_total{channel="discord",model="gpt-5.4",outcome="completed",provider="openai",trigger="message"} 1',
    );
    expect(rendered).toContain(
      'eve_run_duration_seconds_sum{channel="discord",model="gpt-5.4",outcome="completed",provider="openai",trigger="message"} 1.5',
    );
  ...
  it("records hook-blocked run metrics with safe blocker originator only", () => {
    ...
    expect(rendered).toContain(
      'eve_run_completed_total{blocked_by="policy-plugin",channel="slack",model="gpt-5.4",outcome="blocked",provider="openai",trigger="message"} 1',
    );
    expect(rendered).not.toContain("run-should-not-export");
    expect(rendered).not.toContain("session-should-not-export");
  ...
  it("records sanitized async diagnostic queue drop summaries from core diagnostics", () => {
    ...
    expect(rendered).toContain("eve_diagnostic_async_queue_length 0");
  });
  ...
  it("redacts and bounds label values", () => {
    ...
    expect(rendered).toContain(
      'eve_tool_execution_total{error_category="other",outcome="error",params_kind="unknown",tool="tool",tool_owner="none",tool_source="core"} 1',
    );
  ...
  it("records operator-critical diagnostic signals missing from generic run metrics", () => {
    ...
    expect(rendered).toContain(
      'eve_tool_execution_blocked_total{denied_reason="tools.deny",params_kind="object",tool="browser",tool_owner="browser-tools",tool_source="mcp"} 1',
    );
    expect(rendered).toContain(
      'eve_model_failover_total{from_model="claude-opus-4-6",from_provider="anthropic",lane="session",reason="overloaded",suspended="true",to_model="gpt-5.4",to_provider="openai"} 1',
    );
    expect(rendered).toContain(
      'eve_session_stuck_total{reason="startup-sweep",state="processing"} 1',
    );
    expect(rendered).toContain(
      'eve_session_stuck_age_seconds_sum{reason="startup-sweep",state="processing"} 12',
    );
    expect(rendered).toContain(
      'eve_payload_large_total{action="rejected",channel="web",plugin="none",reason="body-too-large",surface="gateway.frame"} 1',
    );
    expect(rendered).toContain(
      'eve_payload_large_bytes_sum{action="rejected",channel="web",plugin="none",reason="body-too-large",surface="gateway.frame"} 2048',
    );
    expect(rendered).not.toContain("session-should-not-export");
  ...
  it("records webhook ingress and liveness warning metrics", () => {
    ...
    expect(rendered).toContain('eve_liveness_warning_total{reason="event_loop_delay:cpu"} 1');
    expect(rendered).toContain('eve_liveness_sessions{state="active"} 2');
    expect(rendered).toContain(
      'eve_liveness_event_loop_delay_p99_seconds_sum{reason="event_loop_delay:cpu"} 0.25',
    );
    expect(rendered).toContain(
      'eve_liveness_cpu_core_ratio_sum{reason="event_loop_delay:cpu"} 1.4',
    );
  ...
  it("drops session-shaped agent labels", () => {
    ...
    expect(rendered).toContain(
      'eve_model_tokens_total{agent="unknown",channel="unknown",model="gpt-5.4",provider="openai",token_type="input"} 12',
    );
    expect(rendered).not.toContain("Agent:qa:otel-trace-smoke");
  });
  ...
  it("drops session-shaped queue lane labels", () => {
    ...
    expect(rendered).toContain('eve_queue_lane_size{lane="session"} 2');
    expect(rendered).not.toContain("Agent:qa:otel-trace-smoke");
  });
  ...
  it("keeps only the bounded prefix from scoped queue lane labels", () => {
    ...
    expect(rendered).toContain('eve_queue_lane_size{lane="dreaming-narrative"} 2');
  ...
  it("records skill usage metrics without raw paths or session identifiers", () => {
    ...
    expect(rendered).toContain("# TYPE eve_skill_used_total counter");
    expect(rendered).toContain(
      'eve_skill_used_total{activation="read",agent="main",skill="tiny-llm-brainstorm",source="workspace"} 1',
    );
    expect(rendered).not.toContain("run-should-not-export");
    expect(rendered).not.toContain("session-should-not-export");
  ...
  it("bounds messaging labels without exporting raw chat identifiers", () => {
    ...
    expect(rendered).toContain(
      'eve_message_delivery_started_total{channel="matrix",delivery_kind="text"} 1',
    );
    expect(rendered).toContain(
      'eve_message_processed_total{channel="unknown",outcome="completed",reason="none"} 1',
    );
    expect(rendered).toContain(
      'eve_message_delivery_total{channel="unknown",delivery_kind="other",error_category="TimeoutError",outcome="error"} 1',
    );
    expect(rendered).not.toContain("chat-should-not-export");
  ...
  it("records inbound dispatch and session turn telemetry", () => {
    ...
    expect(rendered).toContain(
      'eve_message_dispatch_completed_total{channel="telegram",outcome="completed",reason="none",source="webhook"} 1',
    );
    expect(rendered).toContain(
      'eve_message_dispatch_duration_seconds_sum{channel="telegram",outcome="completed",reason="none",source="webhook"} 0.25',
    );
    expect(rendered).toContain(
      'eve_message_dispatch_completed_total{channel="unknown",outcome="completed",reason="none",source="unknown"} 1',
    );
    expect(rendered).toContain(
      'eve_message_dispatch_duration_seconds_sum{channel="unknown",outcome="completed",reason="none",source="unknown"} 0.3',
    );
    expect(rendered).toContain(
      'eve_session_turn_created_total{agent="agent.default",channel="telegram",trigger="user"} 1',
    );
    expect(rendered).not.toContain("run-should-not-export");
  });
  ...
  it("records session recovery and talk metrics without exporting raw ids or content", () => {
    ...
    expect(rendered).toContain(
      'eve_session_recovery_total{action="abort-active-run",active_work_kind="tool_call",state="processing",status="released"} 1',
    );
    expect(rendered).toContain(
      'eve_session_recovery_age_seconds_sum{action="abort-active-run",active_work_kind="tool_call",state="processing",status="released"} 12',
    );
    expect(rendered).toContain(
      'eve_talk_event_total{brain="agent-consult",event_type="input.audio.delta",mode="realtime",provider="openai",transport="gateway-relay"} 1',
    );
    expect(rendered).toContain(
      'eve_talk_audio_bytes_sum{brain="agent-consult",event_type="input.audio.delta",mode="realtime",provider="openai",transport="gateway-relay"} 320',
    );
    expect(rendered).not.toContain("session-should-not-export");
    expect(rendered).not.toContain("key-should-not-export");
  ...
  it("caps metric series growth and reports dropped series", () => {
    ...
    expect(rendered).toContain("# TYPE eve_prometheus_series_dropped_total counter");
    expect(rendered).toContain("eve_prometheus_series_dropped_total ");
  });
  ...
  it("subscribes to internal diagnostics and renders scrape text", () => {
    ...
    exporter.service.start({
      config: {} as never,
      stateDir: "/tmp/eve-prometheus-test",
      logger: {
        info: vi.fn(),
        warn: vi.fn(),
        error: vi.fn(),
