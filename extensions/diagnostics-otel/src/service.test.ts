...
import type { EVEPluginServiceContext } from "../api.js";
import { emitDiagnosticEvent } from "../api.js";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/eve-diagnostics-otel-test";
...
const ORIGINAL_EVE_OTEL_PRELOADED = process.env.EVE_OTEL_PRELOADED;
...
type OtelContextFlags = {
  ...
  protocol?: NonNullable<
    NonNullable<EVEPluginServiceContext["config"]["diagnostics"]>["otel"]
  >["protocol"];
...
function createTraceOnlyContext(endpoint: string): EVEPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  beforeEach(() => {
    resetDiagnosticEventsForTest();
    delete process.env.EVE_OTEL_PRELOADED;
  ...
  afterEach(() => {
    resetDiagnosticEventsForTest();
    if (ORIGINAL_EVE_OTEL_PRELOADED === undefined) {
      delete process.env.EVE_OTEL_PRELOADED;
    } else {
      process.env.EVE_OTEL_PRELOADED = ORIGINAL_EVE_OTEL_PRELOADED;
    }
  ...
  test("drops camelCase and snake_case diagnostic id log attributes before export", async () => {
    ...
    expect(emitCall.attributes?.["eve.provider"]).toBe("openai");
    for (const key of [
      "eve.callId",
      "eve.call_id",
      "eve.chatId",
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("eve.webhook.received")?.add).toHaveBeenCalledWith(1, {
    ...
    expect(telemetryState.histograms.get("eve.queue.depth")?.record).toHaveBeenCalledTimes(2);
    expect(telemetryState.histograms.get("eve.queue.depth")?.record).toHaveBeenCalledWith(2, {
      "eve.channel": "telegram",
      "eve.source": "telegram",
    });
    expect(telemetryState.histograms.get("eve.queue.depth")?.record).toHaveBeenCalledWith(3, {
      "eve.lane": "main",
    });
    expect(telemetryState.counters.get("eve.message.processed")?.add).toHaveBeenCalledWith(1, {
      "eve.channel": "telegram",
      "eve.outcome": "completed",
    });
    expect(telemetryState.counters.get("eve.message.received")?.add).toHaveBeenCalledWith(1, {
      "eve.channel": "telegram",
      "eve.source": "webhook",
    });
    expect(telemetryState.counters.get("eve.message.received")?.add).toHaveBeenCalledWith(1, {
    ...
    expect(
      telemetryState.histograms.get("eve.message.dispatch.duration_ms")?.record,
    ).toHaveBeenCalledWith(
      25,
      expect.objectContaining({
        "eve.channel": "telegram",
    ...
    expect(spanNames).toContain("eve.message.processed");
    expect(spanNames).toContain("eve.session.stuck");
    const webhookSpanOptions = startedSpanOptions("eve.webhook.processed");
    expect(webhookSpanOptions?.attributes).not.toHaveProperty("eve.chatId");
    expect(webhookSpanOptions?.startTime).toBeTypeOf("number");
    const messageSpanOptions = startedSpanOptions("eve.message.processed");
    expect(messageSpanOptions?.attributes?.["eve.channel"]).toBe("telegram");
    expect(messageSpanOptions?.attributes?.["eve.outcome"]).toBe("completed");
    expect(messageSpanOptions?.attributes?.["eve.reason"]).toBe("unknown");
    expect(messageSpanOptions?.attributes).not.toHaveProperty("eve.chatId");
    expect(messageSpanOptions?.attributes).not.toHaveProperty("eve.messageId");
  ...
  test("uses a preloaded OpenTelemetry SDK without dropping diagnostic listeners", async () => {
    process.env.EVE_OTEL_PRELOADED = "1";
    const service = createDiagnosticsOtelService();
    const ctx = createOtelContext(OTEL_TEST_ENDPOINT, { traces: true, metrics: true, logs: true });
    await service.start(ctx);
    ...
    const runDurationRecordCall = lastHistogramRecord("eve.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    const runDurationAttributes = runDurationRecordCall?.[1];
    expect(runDurationAttributes?.["eve.provider"]).toBe("openai");
    expect(runDurationAttributes?.["eve.model"]).toBe("gpt-5.4");
    const runSpanOptions = startedSpanOptions("eve.run");
    expect(runSpanOptions?.attributes?.["eve.outcome"]).toBe("completed");
    expect(logEmit).toHaveBeenCalled();
  ...
  test("exports trusted security events as bounded OTLP logs", async () => {
    ...
    expect(emitCall.body).toBe("eve.security.event");
    expect(emitCall.severityText).toBe("WARN");
    expect(emitCall.severityNumber).toBe(13);
    expect(emitCall.attributes).toMatchObject({
      "eve.security.event_id": "security-event-1",
      "eve.security.category": "tool",
      "eve.security.action": "tool.execution.blocked",
      "eve.security.outcome": "denied",
      "eve.security.severity": "medium",
      "eve.security.reason": "tools.deny",
      "eve.security.actor.kind": "agent",
      "eve.security.actor.id_hash": "agent-hash-1",
      "eve.security.actor.role": "operator",
      "eve.security.actor.scopes": "operator.read,operator.approvals",
      "eve.security.target.kind": "plugin",
      "eve.security.target.name": "@acme/security-event-plugin",
      "eve.security.target.owner": "plugin-installer",
      "eve.security.policy.id": "tools.exec",
      "eve.security.policy.decision": "deny",
      "eve.security.policy.reason": "allowlist.miss",
      "eve.security.control.id": "exec-approval",
      "eve.security.control.family": "approval",
      "eve.security.attribute.params_kind": "object",
      "eve.security.attribute.secretish": "unknown",
    });
    ...
    expect(Object.hasOwn(emitCall.attributes ?? {}, "eve.security.attribute.__proto__")).toBe(
      false,
    );
  ...
  test("exports trusted security events as stdout JSONL logs", async () => {
    ...
    try {
      ...
      expect(record.body).toBe("eve.security.event");
      expect(record.severityText).toBe("WARN");
      expect(record.severityNumber).toBe(13);
      expect(record.attributes).toMatchObject({
        "eve.security.event_id": "security-event-stdout",
        "eve.security.category": "tool",
        "eve.security.action": "tool.execution.blocked",
        "eve.security.outcome": "denied",
        "eve.security.severity": "medium",
      ...
      expect(Object.hasOwn(record.attributes ?? {}, "eve.security.attribute.__proto__")).toBe(
        false,
      );
  ...
  test("records liveness warning diagnostics", async () => {
    ...
    expect(telemetryState.counters.get("eve.liveness.warning")?.add).toHaveBeenCalledWith(1, {
      "eve.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("eve.liveness.event_loop_delay_p99_ms")?.record,
    ).toHaveBeenCalledWith(250, {
      "eve.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("eve.liveness.cpu_core_ratio")?.record,
  ...
  test("records oversized payload metrics without raw identifiers", async () => {
    ...
    expect(telemetryState.counters.get("eve.payload.large")?.add).toHaveBeenCalledWith(1, {
  ...
  test("ignores untrusted telemetry exporter events for OTEL metrics", async () => {
    ...
    await service.start(ctx);
    telemetryState.counters.get("eve.telemetry.exporter.events")?.add.mockClear();
    emitDiagnosticEvent({
  ...
  test("records hook-blocked run metrics with safe blocker originator", async () => {
    ...
    const runDurationRecordCall = lastHistogramRecord("eve.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    expect(runDurationRecordCall?.[1]?.["eve.outcome"]).toBe("blocked");
    expect(runDurationRecordCall?.[1]?.["eve.blocked_by"]).toBe("policy-plugin");
  ...
  test("honors disabled traces when an OpenTelemetry SDK is preloaded", async () => {
    ...
    const runDurationRecordCall = lastHistogramRecord("eve.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    expect(runDurationRecordCall?.[1]?.["eve.provider"]).toBe("openai");
    expect(telemetryState.tracer.startSpan).not.toHaveBeenCalled();
  ...
  test("treats omitted diagnostics enabled flag as enabled", async () => {
    ...
    const attrs = (modelCall?.[1] as { attributes?: Record<string, unknown> } | undefined)
      ?.attributes;
    expect(attrs?.["eve.content.input_messages"]).toBe("user prompt");
  ...
  test("exports diagnostic logs as stdout JSONL without constructing the OTLP log exporter", async () => {
    ...
    ctx.config.diagnostics!.otel!.serviceName = "rovoclaw-eve";
    const stdout = captureStdoutWrites();
    ...
    try {
      ...
      expect(record.signal).toBe("eve.diagnostic.log");
      expect(record["service.name"]).toBe("rovoclaw-eve");
      expect(record.severityText).toBe("WARN");
      expect(record.severityNumber).toBe(13);
      ...
      expect(record.attributes).toMatchObject({
        "eve.log.level": "WARN",
        "eve.subsystem": "diagnostic",
      });
      const tokenAttr = record.attributes?.["eve.token"];
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["eve.token"];
    expect(tokenAttr).not.toBe("ghp_abcdefghijklmnopqrstuvwxyz123456"); // pragma: allowlist secret
  ...
  test("does not attach untrusted diagnostic trace context to exported logs", async () => {
    ...
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "eve.traceId")).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "eve.spanId")).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "eve.traceFlags")).toBe(false);
  ...
  test("bounds plugin-emitted log attributes and omits source paths", async () => {
    ...
    emitDiagnosticEvent({
      ...
      attributes,
      code: {
        filepath: "/Users/alice/eve/src/private.ts",
        line: 42,
        functionName: "handler",
        location: "/Users/alice/eve/src/private.ts:42",
      },
    ...
    expect(String(emitCall.attributes["eve.good"])).toMatch(/^y+/);
    expect(emitCall.attributes["code.lineno"]).toBe(42);
    expect(emitCall.attributes["code.function"]).toBe("handler");
    expect(String(emitCall.attributes["eve.good"]).length).toBeLessThanOrEqual(4200);
    expect(Object.hasOwn(emitCall.attributes, `eve.${PROTO_KEY}`)).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "eve.constructor")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "eve.prototype")).toBe(false);
    expect(
      Object.hasOwn(
        emitCall.attributes,
        "eve.sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
      ),
    ).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "eve.bad key")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "code.filepath")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "eve.code.location")).toBe(false);
    await service.stop?.(ctx);
  });
  ...
  test("exports GenAI client token usage histogram for input and output only", async () => {
    ...
    const tokens = telemetryState.counters.get("eve.tokens");
  ...
  test("bounds agent identifiers on model usage metric attributes", async () => {
    ...
    expect(telemetryState.counters.get("eve.tokens")?.add).toHaveBeenCalledWith(2, {
      "eve.channel": "unknown",
      "eve.agent": "unknown",
      "eve.provider": "openai",
      "eve.model": "gpt-5.4",
  ...
  test("exports GenAI usage attributes on model usage spans without diagnostic identifiers", async () => {
    ...
    const modelUsageOptions = startedSpanOptions("eve.model.usage");
    ...
    expect(Object.hasOwn(modelUsageOptions?.attributes ?? {}, "eve.sessionKey")).toBe(false);
    expect(Object.hasOwn(modelUsageOptions?.attributes ?? {}, "eve.sessionId")).toBe(false);
  ...
  test("exports skill usage counter and span without raw identifiers", async () => {
    ...
    const expectedAttrs = {
      "eve.agent": "main",
      "eve.skill.activation": "read",
      "eve.skill.name": "tiny-llm-brainstorm",
      "eve.skill.source": "workspace",
      "eve.toolName": "read",
    };
    expect(telemetryState.counters.get("eve.skill.used")?.add).toHaveBeenCalledWith(
      1,
      expectedAttrs,
    );
    const skillSpanCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "eve.skill.used",
    );
  ...
  test("exports run, model call, and tool execution lifecycle spans", async () => {
    ...
    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("eve.run");
    expect(spanNames).toContain("eve.model.call");
    expect(spanNames).toContain("eve.harness.run");
    expect(spanNames).toContain("eve.tool.execution");

    const runOptions = startedSpanOptions("eve.run");
    expect(runOptions?.attributes?.["eve.outcome"]).toBe("completed");
    expect(runOptions?.attributes?.["eve.provider"]).toBe("openai");
    expect(runOptions?.attributes?.["eve.model"]).toBe("gpt-5.4");
    expect(runOptions?.attributes?.["eve.channel"]).toBe("webchat");
    ...
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "eve.runId")).toBe(false);
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "eve.sessionKey")).toBe(false);
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "eve.traceId")).toBe(false);
    expect(runOptions?.startTime).toBeTypeOf("number");

    const modelCall = startedSpanCall("eve.model.call");
    ...
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "eve.callId")).toBe(false);
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "eve.runId")).toBe(false);
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "eve.sessionKey")).toBe(false);
    ...
    const harnessCall = startedSpanCall("eve.harness.run");
    const harnessOptions = harnessCall?.[1];
    expect(harnessOptions?.attributes?.["eve.harness.id"]).toBe("codex");
    expect(harnessOptions?.attributes?.["eve.harness.plugin"]).toBe("codex-plugin");
    expect(harnessOptions?.attributes?.["eve.outcome"]).toBe("completed");
    expect(harnessOptions?.attributes?.["eve.provider"]).toBe("codex");
    expect(harnessOptions?.attributes?.["eve.model"]).toBe("gpt-5.4");
    expect(harnessOptions?.attributes?.["eve.channel"]).toBe("qa");
    expect(harnessOptions?.attributes?.["eve.harness.result_classification"]).toBe(
      "reasoning-only",
    );
    expect(harnessOptions?.attributes?.["eve.harness.yield_detected"]).toBe(true);
    expect(harnessOptions?.attributes?.["eve.harness.items.started"]).toBe(3);
    expect(harnessOptions?.attributes?.["eve.harness.items.completed"]).toBe(2);
    expect(harnessOptions?.attributes?.["eve.harness.items.active"]).toBe(1);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "eve.runId")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "eve.sessionId")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "eve.sessionKey")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "eve.traceId")).toBe(false);
    expect(harnessOptions?.startTime).toBeTypeOf("number");
    expect(harnessCall?.[2]).toBeUndefined();

    const toolCall = startedSpanCall("eve.tool.execution");
    const toolOptions = toolCall?.[1];
    expect(toolOptions?.attributes?.["eve.toolName"]).toBe("read");
    expect(toolOptions?.attributes?.["eve.tool.source"]).toBe("core");
    expect(toolOptions?.attributes?.["eve.errorCategory"]).toBe("TypeError");
    expect(toolOptions?.attributes?.["eve.errorCode"]).toBe("429");
    expect(toolOptions?.attributes?.["eve.tool.params.kind"]).toBe("object");
    expect(toolOptions?.attributes?.["gen_ai.tool.name"]).toBe("read");
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "eve.toolCallId")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "eve.runId")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "eve.sessionKey")).toBe(false);
    ...
    expect(modelCallDuration?.[1]?.["eve.provider"]).toBe("openai");
    expect(modelCallDuration?.[1]?.["eve.model"]).toBe("gpt-5.4");
    const requestBytes = lastHistogramRecord("eve.model_call.request_bytes");
    expect(requestBytes?.[0]).toBe(1234);
    expect(requestBytes?.[1]?.["eve.provider"]).toBe("openai");
    expect(requestBytes?.[1]?.["eve.model"]).toBe("gpt-5.4");
    const responseBytes = lastHistogramRecord("eve.model_call.response_bytes");
    expect(responseBytes?.[0]).toBe(567);
    expect(responseBytes?.[1]?.["eve.provider"]).toBe("openai");
    expect(responseBytes?.[1]?.["eve.model"]).toBe("gpt-5.4");
    const timeToFirstByte = lastHistogramRecord("eve.model_call.time_to_first_byte_ms");
    expect(timeToFirstByte?.[0]).toBe(45);
    expect(timeToFirstByte?.[1]?.["eve.provider"]).toBe("openai");
    expect(timeToFirstByte?.[1]?.["eve.model"]).toBe("gpt-5.4");
    const modelSpanAttributes = firstSpanAttributes("eve.model.call");
    expect(modelSpanAttributes["eve.model_call.request_bytes"]).toBe(1234);
    expect(modelSpanAttributes["eve.model_call.response_bytes"]).toBe(567);
    expect(modelSpanAttributes["eve.model_call.time_to_first_byte_ms"]).toBe(45);
    const runDuration = lastHistogramRecord("eve.run.duration_ms");
    expect(runDuration?.[0]).toBe(100);
    expect(Object.hasOwn(runDuration?.[1] ?? {}, "eve.runId")).toBe(false);
    const harnessDuration = lastHistogramRecord("eve.harness.duration_ms");
    expect(harnessDuration?.[0]).toBe(90);
    expect(harnessDuration?.[1]?.["eve.harness.id"]).toBe("codex");
    expect(harnessDuration?.[1]?.["eve.harness.plugin"]).toBe("codex-plugin");
    expect(harnessDuration?.[1]?.["eve.outcome"]).toBe("completed");
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "eve.runId")).toBe(false);
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "eve.sessionKey")).toBe(false);
    const toolDuration = lastHistogramRecord("eve.tool.execution.duration_ms");
    expect(toolDuration?.[0]).toBe(20);
    expect(toolDuration?.[1]?.["eve.tool.source"]).toBe("core");
    expect(Object.hasOwn(toolDuration?.[1] ?? {}, "eve.errorCode")).toBe(false);
    expect(Object.hasOwn(toolDuration?.[1] ?? {}, "eve.runId")).toBe(false);

    const toolSpan = spanByName("eve.tool.execution");
    expect(toolSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "TypeError",
    });
    expect(firstSpanEndTime("eve.tool.execution")).toBeTypeOf("number");
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    await service.stop?.(ctx);
  });
  ...
  test("exports model failover spans", async () => {
    ...
    const failoverOptions = startedSpanOptions("eve.model.failover");
    expect(failoverOptions?.attributes?.["eve.provider"]).toBe("anthropic");
    expect(failoverOptions?.attributes?.["eve.model"]).toBe("claude-opus-4-6");
    expect(failoverOptions?.attributes?.["eve.failover.to_provider"]).toBe("openai");
    expect(failoverOptions?.attributes?.["eve.failover.to_model"]).toBe("gpt-5.4");
    expect(failoverOptions?.attributes?.["eve.failover.reason"]).toBe("overloaded");
    expect(failoverOptions?.attributes?.["eve.failover.suspended"]).toBe(true);
    expect(failoverOptions?.attributes?.["eve.failover.cascade_depth"]).toBe(1);
    expect(failoverOptions?.attributes?.["eve.lane"]).toBe("main");
    expect(Object.hasOwn(failoverOptions?.attributes ?? {}, "eve.sessionId")).toBe(false);
    expect(Object.hasOwn(failoverOptions?.attributes ?? {}, "eve.sessionKey")).toBe(false);
    expect(failoverOptions?.startTime).toBeTypeOf("number");
    expect(firstSpanEndTime("eve.model.failover")).toBeTypeOf("number");
    expect(firstCounterAddCall("eve.model.failover")).toStrictEqual([
      1,
      {
        "eve.failover.reason": "overloaded",
        "eve.failover.suspended": "true",
        "eve.lane": "main",
        "eve.model": "claude-opus-4-6",
        "eve.provider": "anthropic",
        "eve.failover.to_model": "gpt-5.4",
        "eve.failover.to_provider": "openai",
      },
    ]);
    await service.stop?.(ctx);
  });
  ...
  test("records blocked tool metrics even when traces are disabled", async () => {
    ...
    expect(firstCounterAddCall("eve.tool.execution.blocked")).toStrictEqual([
      1,
      {
        "eve.toolName": "browser",
        "eve.tool.source": "mcp",
        "gen_ai.tool.name": "browser",
        "eve.tool.owner": "browser-tools",
        "eve.tool.params.kind": "object",
        "eve.deniedReason": "tools.deny",
      },
    ]);
  ...
  test("drops session-shaped queue lanes from model failover spans", async () => {
    ...
    const failoverOptions = startedSpanOptions("eve.model.failover");
    expect(failoverOptions?.attributes?.["eve.lane"]).toBe("session");
  ...
  test("maps model call APIs to GenAI operation names and error type", async () => {
    ...
    const modelCallAttrs = telemetryState.tracer.startSpan.mock.calls
      .filter((call) => call[0] === "eve.model.call")
  ...
  test("uses latest GenAI inference span shape only when semconv opt-in is set", async () => {
    ...
    expect(startedSpanOptions("eve.model.call")).toBeUndefined();
  ...
  test("records upstream request id hashes as model call span events only", async () => {
    ...
    expect(span?.addEvent).toHaveBeenCalledWith("eve.provider.request", {
      "eve.upstreamRequestIdHash": "sha256:123456abcdef",
    });
    const modelCallDuration = lastHistogramRecord("eve.model_call.duration_ms");
  ...
  test("exports trusted context assembly spans without prompt content", async () => {
    ...
    const contextCall = startedSpanCall("eve.context.assembled");
    const contextOptions = contextCall?.[1];
    const runSpan = telemetryState.spans.find((span) => span.name === "eve.run");
    const runSpanId = runSpan?.spanContext.mock.results[0]?.value?.spanId;
    expect(contextOptions?.attributes?.["eve.provider"]).toBe("openai");
    expect(contextOptions?.attributes?.["eve.model"]).toBe("gpt-5.4");
    expect(contextOptions?.attributes?.["eve.channel"]).toBe("webchat");
    expect(contextOptions?.attributes?.["eve.trigger"]).toBe("message");
    expect(contextOptions?.attributes?.["eve.context.message_count"]).toBe(12);
    expect(contextOptions?.attributes?.["eve.context.history_text_chars"]).toBe(1234);
    expect(contextOptions?.attributes?.["eve.context.history_image_blocks"]).toBe(2);
    expect(contextOptions?.attributes?.["eve.context.max_message_text_chars"]).toBe(456);
    expect(contextOptions?.attributes?.["eve.context.system_prompt_chars"]).toBe(789);
    expect(contextOptions?.attributes?.["eve.context.prompt_chars"]).toBe(42);
    expect(contextOptions?.attributes?.["eve.context.prompt_images"]).toBe(1);
    expect(contextOptions?.attributes?.["eve.context.token_budget"]).toBe(128_000);
    expect(contextOptions?.attributes?.["eve.context.reserve_tokens"]).toBe(4096);
  ...
  test("exports tool loop diagnostics without loop messages or session identifiers", async () => {
    ...
    expect(telemetryState.counters.get("eve.tool.loop")?.add).toHaveBeenCalledWith(1, {
      "eve.toolName": "process",
      "eve.loop.level": "critical",
      "eve.loop.action": "block",
      "eve.loop.detector": "known_poll_no_progress",
      "eve.loop.count": 20,
      "eve.loop.paired_tool": "read",
    });
    const loopSpanCall = startedSpanCall("eve.tool.loop");
    const loopOptions = loopSpanCall?.[1];
    expect(loopOptions?.attributes?.["eve.toolName"]).toBe("process");
    expect(loopOptions?.attributes?.["eve.loop.level"]).toBe("critical");
    expect(loopOptions?.attributes?.["eve.loop.action"]).toBe("block");
    expect(loopOptions?.attributes?.["eve.loop.detector"]).toBe("known_poll_no_progress");
    expect(loopOptions?.attributes?.["eve.loop.count"]).toBe(20);
    expect(loopOptions?.attributes?.["eve.loop.paired_tool"]).toBe("read");
    const loopSpan = telemetryState.spans.find((span) => span.name === "eve.tool.loop");
  ...
  test("exports diagnostic memory samples and pressure without session identifiers", async () => {
    ...
    expect(telemetryState.histograms.get("eve.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      100,
      {},
    );
    expect(telemetryState.histograms.get("eve.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      200,
      {
        "eve.memory.level": "critical",
        "eve.memory.reason": "rss_growth",
      },
    );
    expect(telemetryState.counters.get("eve.memory.pressure")?.add).toHaveBeenCalledWith(1, {
      "eve.memory.level": "critical",
      "eve.memory.reason": "rss_growth",
    });
    const pressureCall = startedSpanCall("eve.memory.pressure");
    const pressureOptions = pressureCall?.[1];
    expect(pressureOptions?.attributes?.["eve.memory.level"]).toBe("critical");
    expect(pressureOptions?.attributes?.["eve.memory.reason"]).toBe("rss_growth");
    expect(pressureOptions?.attributes?.["eve.memory.rss_bytes"]).toBe(200);
    expect(pressureOptions?.attributes?.["eve.memory.heap_used_bytes"]).toBe(50);
    expect(pressureOptions?.attributes?.["eve.memory.heap_total_bytes"]).toBe(90);
    expect(pressureOptions?.attributes?.["eve.memory.external_bytes"]).toBe(20);
    expect(pressureOptions?.attributes?.["eve.memory.array_buffers_bytes"]).toBe(6);
  ...
  test("records async diagnostic queue drop summaries", async () => {
    ...
    const counter = telemetryState.counters.get("eve.diagnostic.async_queue.dropped");
    expect(counter?.add).toHaveBeenCalledWith(4, {
      "eve.diagnostic.async_queue.drop_class": "total",
    });
    expect(counter?.add).toHaveBeenCalledWith(1, {
      "eve.diagnostic.async_queue.drop_class": "trusted",
    });
    expect(counter?.add).toHaveBeenCalledWith(2, {
      "eve.diagnostic.async_queue.drop_class": "untrusted",
    });
    expect(counter?.add).toHaveBeenCalledWith(1, {
      "eve.diagnostic.async_queue.drop_class": "priority",
    });
  ...
  test("parents trusted diagnostic lifecycle spans from active started spans", async () => {
    ...
    const runSpan = telemetryState.spans.find((span) => span.name === "eve.run");
    const modelSpan = telemetryState.spans.find((span) => span.name === "eve.model.call");
    const toolSpan = telemetryState.spans.find((span) => span.name === "eve.tool.execution");
    const runSpanId = runSpan?.spanContext.mock.results[0]?.value?.spanId;
    ...
    expect(parentBySpanName["eve.run"]).toBeUndefined();
    expect(parentBySpanName["eve.model.call"]).toBe(runSpanId);
    expect(parentBySpanName["eve.tool.execution"]).toBe(modelSpanId);
    expect(toolSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "TypeError",
    });
  ...
  test("correlates one channel message waterfall across message, harness, usage, and model spans", async () => {
    ...
    const messageSpan = spanByName("eve.message.processed");
    const harnessSpan = spanByName("eve.harness.run");
    const runSpan = spanByName("eve.run");
    const usageSpan = spanByName("eve.model.usage");
    const modelCallSpan = spanByName("eve.model.call");
    ...
    expect(parentBySpanName["eve.message.processed"]?.spanId).toBe(SPAN_ID);
    expect(parentBySpanName["eve.harness.run"]?.spanId).toBe(messageSpanContext.spanId);
    expect(parentBySpanName["eve.run"]?.spanId).toBe(harnessSpanContext.spanId);
    expect(parentBySpanName["eve.model.usage"]?.spanId).toBe(harnessSpanContext.spanId);
    expect(parentBySpanName["eve.model.call"]?.spanId).toBe(runSpanContext.spanId);
    await service.stop?.(ctx);
  });
  ...
  test("does not force a remote parent for root message lifecycle helpers", async () => {
    ...
    expect(spanByName("eve.message.processed").spanContext().traceId).toBe(TRACE_ID);
    expect(startedSpanParentContexts("eve.message.processed")[0]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("parents outbound delivery spans under the active message lifecycle span", async () => {
    ...
    const messageSpanContext = spanByName("eve.message.processed").spanContext();
    const deliveryParentContexts = startedSpanParentContexts("eve.message.delivery");
  ...
  test("correlates skipped duplicate message lifecycle helpers to the active inbound trace", async () => {
    ...
    const messageSpan = spanByName("eve.message.processed");
    const messageSpanContext = messageSpan.spanContext();
    const parentContext = startedSpanParentContexts("eve.message.processed")[0];

    expect(messageSpanContext.traceId).toBe(TRACE_ID);
    expect(parentContext?.traceId).toBe(TRACE_ID);
    expect(parentContext?.spanId).toBe(SPAN_ID);
    expect(firstSpanAttributes("eve.message.processed")["eve.reason"]).toBe("duplicate");
    expect(messageSpan.end).toHaveBeenCalledTimes(1);
    await service.stop?.(ctx);
  });
  ...
  test("does not retain fallback message processed spans as active parents", async () => {
    ...
    expect(spanByName("eve.message.processed").end).toHaveBeenCalledTimes(1);
    ...
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    expect(startedSpanCall("eve.harness.run")?.[2]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("retains trusted run context long enough for exact post-completion usage parenting", async () => {
    ...
    expect(firstSpanEndTime("eve.run")).toBeTypeOf("number");
    await service.stop?.(ctx);
  });
  ...
  test("does not parent sibling active runs through shared upstream aliases", async () => {
    ...
    const runContexts = startedSpanParentContextsByName("eve.run");
  ...
  test("parents retained upstream alias events only when the owner matches", async () => {
    ...
    const runSpanContext = spanByName("eve.run").spanContext();
    const modelParentContext = startedSpanParentContexts("eve.model.call")[0];
  ...
  test("parents multi-batch late model spans from the retained run context", async () => {
    ...
    const modelParentContexts = startedSpanParentContexts("eve.model.call");
  ...
  test("removes retained run contexts after queued diagnostics drain", async () => {
    ...
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    expect(startedSpanCall("eve.model.usage")?.[2]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("does not force remote parents for completed-only trusted lifecycle spans", async () => {
    ...
    expect(parentBySpanName["eve.run"]).toBeUndefined();
    expect(parentBySpanName["eve.model.call"]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("does not parent untrusted diagnostic lifecycle spans from injected trace ids", async () => {
    ...
    expect(parentBySpanName["eve.run"]).toBeUndefined();
    expect(parentBySpanName["eve.model.call"]).toBeUndefined();
    expect(parentBySpanName["eve.tool.execution"]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("exports exec process spans without command text", async () => {
    ...
    const execDuration = lastHistogramRecord("eve.exec.duration_ms");
    expect(execDuration?.[0]).toBe(30);
    expect(execDuration?.[1]?.["eve.exec.target"]).toBe("host");
    expect(execDuration?.[1]?.["eve.exec.mode"]).toBe("child");
    expect(execDuration?.[1]?.["eve.outcome"]).toBe("failed");
    expect(execDuration?.[1]?.["eve.failureKind"]).toBe("runtime-error");

    const execCall = startedSpanCall("eve.exec");
    const execOptions = execCall?.[1];
    expect(execOptions?.attributes?.["eve.exec.target"]).toBe("host");
    expect(execOptions?.attributes?.["eve.exec.mode"]).toBe("child");
    expect(execOptions?.attributes?.["eve.outcome"]).toBe("failed");
    expect(execOptions?.attributes?.["eve.exec.command_length"]).toBe(42);
    expect(execOptions?.attributes?.["eve.exec.exit_code"]).toBe(1);
    expect(execOptions?.attributes?.["eve.exec.timed_out"]).toBe(false);
    expect(execOptions?.attributes?.["eve.failureKind"]).toBe("runtime-error");
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "eve.exec.command")).toBe(false);
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "eve.exec.workdir")).toBe(false);
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "eve.sessionKey")).toBe(false);
    expect(execOptions?.startTime).toBeTypeOf("number");

    const execSpan = spanByName("eve.exec");
    expect(execSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "runtime-error",
    });
    expect(firstSpanEndTime("eve.exec")).toBeTypeOf("number");
    await service.stop?.(ctx);
  });
  ...
  test("exports message delivery spans and metrics with low-cardinality attributes", async () => {
    ...
    expect(deliveryDurationRecords[0]?.[1]["eve.channel"]).toBe("matrix");
    expect(deliveryDurationRecords[0]?.[1]["eve.delivery.kind"]).toBe("text");
    expect(deliveryDurationRecords[0]?.[1]["eve.outcome"]).toBe("completed");
    expect(deliveryDurationRecords[1]?.[0]).toBe(40);
    expect(deliveryDurationRecords[1]?.[1]["eve.channel"]).toBe("discord");
    expect(deliveryDurationRecords[1]?.[1]["eve.delivery.kind"]).toBe("media");
    expect(deliveryDurationRecords[1]?.[1]["eve.outcome"]).toBe("error");
    expect(deliveryDurationRecords[1]?.[1]["eve.errorCategory"]).toBe("TypeError");

    const deliverySpanCalls = telemetryState.tracer.startSpan.mock.calls.filter(
      (call) => call[0] === "eve.message.delivery",
    );
    ...
    const firstDeliveryOptions = deliverySpanCalls[0]?.[1] as
      ...
      | undefined;
    expect(firstDeliveryOptions?.attributes?.["eve.channel"]).toBe("matrix");
    expect(firstDeliveryOptions?.attributes?.["eve.delivery.kind"]).toBe("text");
    expect(firstDeliveryOptions?.attributes?.["eve.outcome"]).toBe("completed");
    expect(firstDeliveryOptions?.attributes?.["eve.delivery.result_count"]).toBe(1);
    expect(firstDeliveryOptions?.startTime).toBeTypeOf("number");
    const secondDeliveryOptions = deliverySpanCalls[1]?.[1] as
      | { attributes?: Record<string, unknown>; startTime?: unknown }
      | undefined;
    expect(secondDeliveryOptions?.attributes?.["eve.channel"]).toBe("discord");
    expect(secondDeliveryOptions?.attributes?.["eve.delivery.kind"]).toBe("media");
    expect(secondDeliveryOptions?.attributes?.["eve.outcome"]).toBe("error");
    expect(secondDeliveryOptions?.attributes?.["eve.errorCategory"]).toBe("TypeError");
    ...
    for (const call of deliverySpanCalls) {
      ...
      expect(Object.hasOwn(options.attributes ?? {}, "eve.chatId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "eve.sessionKey")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "eve.messageId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "eve.conversationId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "eve.content")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "eve.to")).toBe(false);
      expect(options.startTime).toBeTypeOf("number");
    }
    const errorSpan = telemetryState.spans.find(
      (span) => span.name === "eve.message.delivery" && span.setStatus.mock.calls.length > 0,
    );
  ...
  test("bounds unsafe message delivery attributes before export", async () => {
    ...
    const deliveryDuration = lastHistogramRecord("eve.message.delivery.duration_ms");
    expect(deliveryDuration?.[0]).toBe(20);
    expect(deliveryDuration?.[1]?.["eve.channel"]).toBe("unknown");
    expect(deliveryDuration?.[1]?.["eve.delivery.kind"]).toBe("other");
    expect(deliveryDuration?.[1]?.["eve.outcome"]).toBe("completed");
    const deliverySpanCall = startedSpanCall("eve.message.delivery");
    const deliveryOptions = deliverySpanCall?.[1];
    expect(deliveryOptions?.attributes?.["eve.channel"]).toBe("unknown");
    expect(deliveryOptions?.attributes?.["eve.delivery.kind"]).toBe("other");
    expect(deliveryOptions?.attributes?.["eve.outcome"]).toBe("completed");
    expect(deliveryOptions?.attributes?.["eve.delivery.result_count"]).toBe(1);
  ...
  test("exports session recovery and talk metrics with bounded attributes", async () => {
    ...
    const recoveryRequestedCall = firstCounterAddCall("eve.session.recovery.requested");
    expect(recoveryRequestedCall[0]).toBe(1);
    expect(recoveryRequestedCall[1]?.["eve.state"]).toBe("processing");
    expect(recoveryRequestedCall[1]?.["eve.action"]).toBe("abort");
    expect(recoveryRequestedCall[1]?.["eve.active_work_kind"]).toBe("tool_call");
    const recoveryCompletedCall = firstCounterAddCall("eve.session.recovery.completed");
    expect(recoveryCompletedCall[0]).toBe(1);
    expect(recoveryCompletedCall[1]?.["eve.state"]).toBe("processing");
    expect(recoveryCompletedCall[1]?.["eve.status"]).toBe("released");
    expect(recoveryCompletedCall[1]?.["eve.action"]).toBe("abort-active-run");
    const recoveryAgeRecord = lastHistogramRecord("eve.session.recovery.age_ms");
    expect(recoveryAgeRecord?.[0]).toBe(13_000);
    expect(recoveryAgeRecord?.[1]?.["eve.status"]).toBe("released");
    expect(telemetryState.counters.get("eve.talk.event")?.add).toHaveBeenCalledWith(1, {
      "eve.talk.brain": "agent-consult",
      "eve.talk.event_type": "input.audio.delta",
      "eve.talk.mode": "realtime",
      "eve.talk.provider": "openai",
      "eve.talk.transport": "gateway-relay",
    });
    expect(telemetryState.histograms.get("eve.talk.audio.bytes")?.record).toHaveBeenCalledWith(
      320,
      {
        "eve.talk.brain": "agent-consult",
        "eve.talk.event_type": "input.audio.delta",
    ...
    expect(
      telemetryState.histograms.get("eve.talk.event.duration_ms")?.record,
    ).toHaveBeenCalledWith(45, {
      "eve.talk.brain": "agent-consult",
      "eve.talk.event_type": "latency.metrics",
      "eve.talk.mode": "realtime",
      "eve.talk.provider": "openai",
      "eve.talk.transport": "gateway-relay",
    });
    ...
    const talkCounterCalls = JSON.stringify(
      telemetryState.counters.get("eve.talk.event")?.add.mock.calls,
    );
  ...
  test("exports bounded redacted content when capture fields are opted in", async () => {
    ...
    expect(modelAttrs?.["eve.content.output_messages"]).toBe("model reply");
    expect(modelAttrs?.["eve.content.system_prompt"]).toBe("system prompt");
    expect(String(modelAttrs?.["eve.content.input_messages"])).not.toContain(
      "sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
    );
    expect(toolAttrs?.["eve.content.tool_input"]).toBe("tool input");
    expect(String(toolAttrs?.["eve.content.tool_output"]).length).toBeLessThanOrEqual(
      MAX_TEST_OTEL_CONTENT_ATTRIBUTE_CHARS + OTEL_TRUNCATED_SUFFIX_MAX_CHARS,
    );
    expect(String(toolAttrs?.["eve.content.tool_output"])).not.toContain("a".repeat(11));
    await service.stop?.(ctx);
  });
  ...
  test("omits absent model content fields when capture fields are opted in", async () => {
    ...
    expect(attrs["eve.content.input_messages"]).toBe("user prompt");
    expect(Object.hasOwn(attrs, "eve.content.output_messages")).toBe(false);
    expect(Object.hasOwn(attrs, "eve.content.system_prompt")).toBe(false);
    expect(Object.hasOwn(attrs, "eve.content.tool_definitions")).toBe(false);
  ...
  test("exports tool definitions without requiring input message capture", async () => {
    ...
    expect(Object.hasOwn(attrs ?? {}, "eve.content.input_messages")).toBe(false);
    expect(JSON.parse(stringAttribute(attrs, "gen_ai.tool.definitions"))).toEqual([
      {
        type: "function",
        name: "lookup",
    ...
    expect(JSON.parse(String(attrs?.["eve.content.tool_definitions"]))).toEqual([
      {
        name: "lookup",
        description: "Lookup data",
        parameters: { type: "object" },
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionStateCall = firstCounterAddCall("eve.session.state");
    const attrs = sessionStateCall[1];
    expect(sessionStateCall[0]).toBe(1);
    expect(String(attrs?.["eve.reason"])).toContain("…");
    expect(typeof attrs?.["eve.reason"]).toBe("string");
    expect(String(attrs?.["eve.reason"])).not.toContain(
