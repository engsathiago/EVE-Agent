...
import type { PluginLogger } from "eve-agent/plugin-sdk/plugin-entry";
import { isTruthyEnvValue } from "eve-agent/plugin-sdk/runtime-env";
...
/** Input data used to publish EVE gateway Bonjour records. */
...
// See https://github.com/engsathiago/eve-agent/issues/72481
...
function readBonjourDisableOverride(): boolean | null {
  const raw = process.env.EVE_DISABLE_BONJOUR;
...
function truncateToDnsLabel(name: string, fallback = "EVE"): string {
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? truncateToDnsLabel(trimmed) : "EVE";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(EVE\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw =
      process.env.EVE_MDNS_HOSTNAME?.trim() || resolveSystemMdnsHostname() || "eve";
    ...
    const recreateAdvertiser = async (reason: string, optsLocal?: { stuckState?: boolean }) => {
      ...
      recreatePromise = (async () => {
        ...
        if (tooManyConsecutive || tooManyStuckStates || tooManyInWindow) {
          ...
          logger.warn(
            `bonjour: disabling advertiser after ${detail} (${reason}); set discovery.mdns.mode="off" or EVE_DISABLE_BONJOUR=1 to disable mDNS discovery`,
          );
