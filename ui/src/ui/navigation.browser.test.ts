...
function expectConfirmedGatewayChange(app: ReturnType<typeof mountApp>) {
  expect(app.settings.gatewayUrl).toBe("wss://other-gateway.example/eve");
...
describe("control UI routing", () => {
  ...
  it("hydrates hash tokens, preserves same-scope URL edits, and reloads after gateway changes", async () => {
    ...
    expect(app.settings.token).toBe("abc123");
    expect(JSON.parse(localStorage.getItem("eve.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
    ...
    gatewayUrlInput.value = "wss://missing-token.example/eve";
    gatewayUrlInput.dispatchEvent(new Event("input", { bubbles: true }));
    await refreshed.updateComplete;
    ...
    expect(refreshed.settings.gatewayUrl).toBe("wss://other-gateway.example/eve");
