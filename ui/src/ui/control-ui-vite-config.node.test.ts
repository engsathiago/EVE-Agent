...
describe("Control UI Vite config", () => {
  it("resolves root tsconfig package aliases for source imports", () => {
    expect(findStringAlias("@eve/net-policy/ip")?.replacement).toBe(
  ...
  it("resolves Control UI dev-server source aliases for internal packages", () => {
    ...
    expect(
      aliases.find((alias) => alias.find === "@eve/normalization-core/string-coerce"),
    )?.toEqual({
  ...
  it("keeps specific tsconfig aliases ahead of broad package aliases", () => {
    ...
    const broadEVEWildcardIndex = aliases.findIndex(
    ...
    expect(broadEVEWildcardIndex).toBeGreaterThanOrEqual(0);
    expect(netPolicyIpIndex).toBeLessThan(netPolicyPackageIndex);
    expect(netPolicyWildcardIndex).toBeLessThan(broadEVEWildcardIndex);
  });
