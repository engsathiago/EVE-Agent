...
describe("resolveLoginFailureFeedback", () => {
  ...
  it("preserves pairing request ids in the approval command", () => {
    ...
    expect(feedback?.steps).toEqual([
      "Run eve devices list on the Gateway host.",
      "Approve this request: eve devices approve req-123.",
  ...
  it("keeps generic browser WebSocket constructor failures on the network path", () => {
    ...
    expect(feedback?.steps).toEqual([
      "Confirm the Gateway is running with eve status or eve gateway run.",
      "Check the WebSocket URL and use wss:// when the Gateway is behind HTTPS/Tailscale Serve.",
      "Reopen the dashboard with eve dashboard --no-open to recopy the current URL and auth details.",
    ]);
  });
  ...
  it("redacts credential-shaped values from displayed raw errors", () => {
    const feedback = resolveLoginFailureFeedback({
      connected: false,
      lastError:
        "failed ws://host/eve#token=secret-token Authorization: Bearer secret-bearer token=inline-secret",
      lastErrorCode: null,
      hasToken: false,
      hasPassword: false,
    });
    ...
    expect(feedback?.rawError).toBe(
      "failed ws://host/eve#[redacted-credential] Authorization: Bearer [redacted] token=[redacted]",
    );
  });
});
