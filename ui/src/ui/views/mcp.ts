...
function renderServerRow(props: McpViewProps, server: McpServerRow) {
  ...
  const probeCommand = `eve mcp probe ${quotedName}`;
  const loginCommand = `eve mcp login ${quotedName}`;
  return html`
    ...
    <section class="mcp-page">
      ...
      <section class="card mcp-command-card">
        ...
        <div class="mcp-command-card__grid">
          <code>eve mcp status --verbose</code>
          <code>eve mcp doctor --probe</code>
          <code>eve mcp login &lt;name&gt;</code>
          <code>eve mcp reload</code>
        </div>
      </section>
