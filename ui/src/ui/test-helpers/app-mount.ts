...
import "../app.ts";
import type { EVEApp } from "../app.ts";
...
async function waitForAppUpdates(apps: Iterable<EVEApp>) {
  for (const app of apps) {
    await app.updateComplete;
  }
}

async function drainAppWork(apps: Iterable<EVEApp>) {
  const snapshot = [...apps];
  await nextMicrotask();
...
export function mountApp(pathname: string) {
  ...
  const app = document.createElement("eve-app") as EVEApp;
  mountedApps.add(app);
...
export function registerAppMountHooks() {
  beforeEach(async () => {
    ...
    window["__EVE_CONTROL_UI_BASE_PATH__"] = undefined;
