...
declare global {
  interface Window {
    __EVE_CONTROL_UI_BASE_PATH__?: string;
  }
}
  ...
  async toggleRealtimeTalk() {
    ...
    const session = new RealtimeTalkSession(
      ...
      this.sessionKey,
      {
        ...
        onTranscript: (entry) => {
          this.realtimeTalkTranscript = `${entry.role === "user" ? "You" : "EVE"}: ${entry.text}`;
...
if (!customElements.get("eve-app")) {
  customElements.define("eve-app", EVEApp);
}
