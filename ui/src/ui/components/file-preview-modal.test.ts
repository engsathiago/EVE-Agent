...
import type { EVEFilePreviewModal } from "./file-preview-modal.ts";
...
async function renderPreview(query = "") {
  ...
  const modal = container.querySelector<EVEFilePreviewModal>("eve-file-preview-modal");
...
function shadowText(modal: EVEFilePreviewModal): string {
  return modal.shadowRoot?.textContent ?? "";
}

describe("eve-file-preview-modal", () => {
  beforeEach(() => {
