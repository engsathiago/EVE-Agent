...
describe("resolveSafeExternalUrl", () => {
  const baseHref = "https://eve.ai/chat";
  ...
  it("allows relative URLs resolved against the current origin", () => {
    expect(resolveSafeExternalUrl("/assets/pic.png", baseHref)).toBe(
      "https://eve.ai/assets/pic.png",
    );
  });

  it("allows blob URLs", () => {
    expect(resolveSafeExternalUrl("blob:https://eve.ai/abc-123", baseHref)).toBe(
      "blob:https://eve.ai/abc-123",
    );
  });
