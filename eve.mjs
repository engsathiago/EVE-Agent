...
const COMPILE_CACHE_DISABLED_RESPAWNED_ENV = "EVE_COMPILE_CACHE_DISABLED_RESPAWNED";
...
const ensureSupportedNodeVersion = () => {
  ...
  process.stderr.write(
    `eve: Node.js v${MIN_NODE_VERSION}+ is required (current: v${process.versions.node}).\n` +
      "If you use nvm, run:\n" +
...
const runRespawnedChild = (command, args, env) => {
  ...
  child.once("error", (error) => {
    detach();
    process.stderr.write(
      `[eve] Failed to respawn launcher: ${
...
const respawnWithPackagedCompileCacheIfNeeded = () => {
  ...
  if (process.env.EVE_PACKAGED_COMPILE_CACHE_RESPAWNED === "1") {
    return false;
  }
  ...
  const env = {
    ...
    EVE_PACKAGED_COMPILE_CACHE_RESPAWNED: "1",
  };
  return runRespawnedChild(
    process.execPath,
    [...process.execArgv, fileURLToPath(import.meta.url), ...process.argv.slice(2)],
...
const buildMissingEntryErrorMessage = async () => {
  const lines = ["eve: missing dist/entry.(m)js (build output)."];
...
const resolveLauncherHomeDir = () => {
  const explicit = normalizeLauncherHomeValue(process.env.EVE_HOME);
  const rawHome =
...
const resolveLauncherConfigPaths = () => {
  const explicit = process.env.EVE_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveLauncherUserPath(explicit)];
  }
  const stateOverride = process.env.EVE_STATE_DIR?.trim();
  if (stateOverride) {
    const stateDir = resolveLauncherUserPath(stateOverride);
    return [path.join(stateDir, "eve.json"), path.join(stateDir, "clawdbot.json")];
  }
  const homeDir = resolveLauncherHomeDir();
  return [
    path.join(homeDir, ".eve", "eve.json"),
    path.join(homeDir, ".eve", "clawdbot.json"),
    path.join(homeDir, ".clawdbot", "eve.json"),
...
const shouldDeferRootHelpToRuntimeEntry = () => {
  if (
    process.env.EVE_BUNDLED_PLUGINS_DIR?.trim() ||
    process.env.EVE_DISABLE_BUNDLED_PLUGINS?.trim()
  ) {
    return true;
  }
...
function tryOutputLauncherVersion(argv) {
  try {
    if (normalizeLauncherMetadataValue(process.env.EVE_CONTAINER)) {
      return false;
    }
    ...
    process.stdout.write(commit ? `EVE ${version} (${commit})\n` : `EVE ${version}\n`);
    return true;
  } catch {
    return false;
  }
...
function resolveLauncherVersion() {
  ...
  return normalizeLauncherMetadataValue(process.env.EVE_BUNDLED_VERSION) ?? "0.0.0";
}
